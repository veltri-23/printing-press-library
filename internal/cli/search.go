...
In local mode: searches locally synced data only.`,
  ...
  theclose-pp-cli search "error timeout"

  # Force local search only
  theclose-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  theclose-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  theclose-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch resourceType {
			case "notifications":
				results, err = db.SearchNotifications(query, limit)
			case "tasks":
