...
func newWebhooksHandleInboundEmailCmd(flags *rootFlags) *cobra.Command {
	var flagBodyJSON string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "handle-inbound-email",
		Short:       "Receives inbound emails, associates with transaction via reply address, stores as received email.",
		Example:     "  theclose-pp-cli webhooks handle-inbound-email",
		Annotations: map[string]string{"pp:endpoint": "webhooks.handle-inbound-email", "pp:method": "POST", "pp:path": "/api/webhooks/email-inbound"},
