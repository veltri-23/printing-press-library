...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  theclose-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  theclose-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "admin-llm-corrections", "admin-llm-field-accuracy", "admin-llm-prompt-versions", "admin-llm-regression-set", "agent-actions", "agent-tokens", "billing", "callouts", "close-developer-health", "close-developer-profile", "contacts", "events", "schema", "tasks", "templates", "transactions", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  theclose-pp-cli workflow status

  # Show status as JSON
  theclose-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("theclose-pp-cli")
			}
