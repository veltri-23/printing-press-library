...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Close Developer",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  theclose-pp-cli auth setup\n  theclose-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Create a scoped agent token in The Close Settings > API.")
			fmt.Fprintln(w, "Docs: /developers")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export THECLOSE_BASE_URL=\"http://localhost:3000\"")
			fmt.Fprintln(w, "  export THECLOSE_API_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  theclose-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "/developers"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  theclose-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export THECLOSE_API_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  theclose-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  theclose-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  theclose-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("THECLOSE_API_TOKEN") != "" {
				envStillSet = "THECLOSE_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("CLOSE_DEVELOPER_BEARER_AUTH") != "" {
				envStillSet = "CLOSE_DEVELOPER_BEARER_AUTH"
			}
