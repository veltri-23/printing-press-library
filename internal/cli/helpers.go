...
func (e *cliError) Error() string { return redactKnownSecrets(e.err.Error()) }
...
func writeAPIErrorEnvelope(flags *rootFlags, err error, code int) {
	...
	_ = json.NewEncoder(os.Stdout).Encode(map[string]any{
		"error": redactKnownSecrets(err.Error()),
		"code":  code,
	})
}

// PATCH: The Close diagnostics must never echo bearer tokens from env, config, or response text.
func redactKnownSecrets(s string) string {
	for _, envName := range []string{"THECLOSE_API_TOKEN", "CLOSE_DEVELOPER_BEARER_AUTH"} {
	...
	jsonSecretPattern := regexp.MustCompile(`(?i)("?(?:api[_-]?key|access[_-]?token|refresh[_-]?token|system[_-]?key|secret|password|authorization|credential|session[_-]?token)"?\s*[:=]\s*)("[^"]+"|[A-Za-z0-9._~+/=-]+)`)
	return jsonSecretPattern.ReplaceAllString(s, "${1}[REDACTED]")
}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export THECLOSE_API_TOKEN=<your-token>"+
			"\n      See API docs: /developers"+
			"\n      Run 'theclose-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: theclose-pp-cli auth set-token <token>"+
			"\n      or: export THECLOSE_API_TOKEN=<your-token>"+
			"\n      See API docs: /developers"+
			"\n      Run 'theclose-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your API key has the required permissions."+
			"\n      Set it with: export THECLOSE_API_TOKEN=<your-token>"+
			"\n      See API docs: /developers"+
			"\n      Run 'theclose-pp-cli doctor' to check auth status.", err))
