...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".theclose-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("THECLOSE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("THECLOSE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "theclose-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.theclose-pp-cli/feedback.jsonl.
When ` + "`THECLOSE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`THECLOSE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  theclose-pp-cli feedback list
  theclose-pp-cli feedback list --limit 5
  theclose-pp-cli feedback list --json`,
