...
Exit codes & warnings:
  ...
  theclose-pp-cli sync

  # Sync specific resources only
  theclose-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  theclose-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  theclose-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  theclose-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  theclose-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("theclose-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/theclose-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "address":
		return db.UpsertAddress(data)
	case "admin":
		return db.UpsertAdmin(data)
	case "agent-actions":
		return db.UpsertAgentActions(data)
	case "accept_contact_suggestion":
		return db.UpsertAcceptContactSuggestion(data)
	case "approve":
		return db.UpsertApprove(data)
	case "dismiss_contact_suggestion":
		return db.UpsertDismissContactSuggestion(data)
	case "mark_stale":
		return db.UpsertMarkStale(data)
	case "reject":
		return db.UpsertReject(data)
	case "agent-tokens":
		return db.UpsertAgentTokens(data)
	case "revoke":
		return db.UpsertRevoke(data)
	case "usages":
		return db.UpsertUsages(data)
	case "callouts":
		return db.UpsertCallouts(data)
	case "contacts":
	...
	case "submit":
		return db.UpsertSubmit(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "commit":
		return db.UpsertCommit(data)
	case "notifications":
	...
	case "transactions_contacts":
		return db.UpsertTransactionsContacts(data)
	case "transactions_documents":
		return db.UpsertTransactionsDocuments(data)
	case "email_filter":
		return db.UpsertEmailFilter(data)
	case "transactions_events":
		return db.UpsertTransactionsEvents(data)
	case "fields":
		return db.UpsertFields(data)
	case "transactions_portal":
		return db.UpsertTransactionsPortal(data)
	case "transactions_tasks":
		return db.UpsertTransactionsTasks(data)
	case "webhooks":
...
func defaultSyncResources() []string {
	return []string{
		"admin",
		"admin-llm-corrections",
		"admin-llm-field-accuracy",
		"admin-llm-prompt-versions",
		"admin-llm-regression-set",
		"agent-actions",
		"agent-tokens",
		"billing",
		"callouts",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"admin":                     "/api/admin/dlq",
		"admin-llm-corrections":     "/api/admin/llm/corrections",
		"admin-llm-field-accuracy":  "/api/admin/llm/field-accuracy",
		"admin-llm-prompt-versions": "/api/admin/llm/prompt-versions",
		"admin-llm-regression-set":  "/api/admin/llm/regression-set",
		"agent-actions":             "/api/agent-actions",
		"agent-tokens":              "/api/agent-tokens",
		"billing":                   "/api/billing/summary",
		"callouts":                  "/api/callouts/seen",
		"close-developer-health":    "/api/health",
		"close-developer-profile":   "/api/profile",
		"contacts":                  "/api/contacts",
		"events":                    "/api/events",
		"schema":                    "/api/schema/fields",
		"tasks":                     "/api/tasks",
		"templates":                 "/api/templates",
		"transactions":              "/api/transactions",
		"webhooks":                  "/api/webhooks/subscriptions",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "transactions_contacts", ParentTable: "transactions", ParentIDParam: "transactionId", PathTemplate: "/api/transactions/{transactionId}/contacts", KeyField: ""},
		{Name: "transactions_documents", ParentTable: "transactions", ParentIDParam: "transactionId", PathTemplate: "/api/transactions/{transactionId}/documents", KeyField: ""},
		{Name: "transactions_emails", ParentTable: "transactions", ParentIDParam: "transactionId", PathTemplate: "/api/transactions/{transactionId}/emails", KeyField: ""},
		{Name: "transactions_events", ParentTable: "transactions", ParentIDParam: "transactionId", PathTemplate: "/api/transactions/{transactionId}/events", KeyField: ""},
		{Name: "transactions_fields", ParentTable: "transactions", ParentIDParam: "transactionId", PathTemplate: "/api/transactions/{transactionId}/fields", KeyField: ""},
		{Name: "transactions_tasks", ParentTable: "transactions", ParentIDParam: "transactionId", PathTemplate: "/api/transactions/{transactionId}/tasks", KeyField: ""},
		{Name: "usages", ParentTable: "agent-tokens", ParentIDParam: "tokenId", PathTemplate: "/api/agent-tokens/{tokenId}/usages", KeyField: ""},
	}
}
...
var resourceIDFieldOverrides = map[string]string{
	...
	"templates":                 "id",
	"transactions":              "id",
	"transactions_contacts":     "id",
	"transactions_documents":    "id",
	"transactions_emails":       "id",
	"transactions_events":       "id",
	"transactions_fields":       "fieldKey",
	"transactions_tasks":        "id",
	"webhooks":                  "id",
}
