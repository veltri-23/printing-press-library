...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  theclose-pp-cli doctor
  theclose-pp-cli doctor --json
  theclose-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export THECLOSE_API_TOKEN=<your-token>"
					report["auth_docs_url"] = "/developers"
				} else {
					authConfigured = true
			...
			if os.Getenv("THECLOSE_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "THECLOSE_API_TOKEN")
			} else if os.Getenv("CLOSE_DEVELOPER_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "CLOSE_DEVELOPER_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "THECLOSE_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					reachBody, reachErr := c.Get("/openapi.json", nil)
					...
					case errors.As(reachErr, &reachAPIErr):
						...
						if vendor := looksLikeDoctorInterstitial([]byte(reachAPIErr.Body)); vendor != "" {
							...
							report["api"] = fmt.Sprintf("reachable (HTTP %d at /openapi.json)", status)
						}
					default:
					...
					if authHeader == "" {
						...
						_, credErr := c.GetWithHeaders("/api/transactions", map[string]string{"archive": "false"}, nil)
						if credErr == nil {
							report["credentials"] = "valid"
						} else {
							var apiErr *client.APIError
							if errors.As(credErr, &apiErr) {
								switch apiErr.StatusCode {
								case 401:
									report["credentials"] = "invalid token (HTTP 401)"
								case 403:
									report["credentials"] = "forbidden scope (HTTP 403)"
								default:
									report["credentials"] = fmt.Sprintf("probe failed (HTTP %d)", apiErr.StatusCode)
								}
							} else {
								report["credentials"] = fmt.Sprintf("probe failed: %s", credErr)
							}
						}
					}
				}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'theclose-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'theclose-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'theclose-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'theclose-pp-cli sync' to refresh."
	}
	return report
}
