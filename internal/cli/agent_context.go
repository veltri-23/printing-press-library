...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	envVars := []agentContextAuthEnvVar{
		{
			Name:        "THECLOSE_API_TOKEN",
			Kind:        "per_call",
			Required:    true,
			Sensitive:   true,
			Description: "Set to a scoped The Close agent API token.",
		},
		{
			Name:        "THECLOSE_BASE_URL",
			Kind:        "per_call",
			Required:    false,
			Sensitive:   false,
			Description: "Set when targeting a non-default The Close API origin.",
		},
	}
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "theclose-pp-cli",
			Description: "Agent-facing API for The Close. Use scoped bearer tokens for transaction, field, document, task, event, and connector proposal workflows.",
