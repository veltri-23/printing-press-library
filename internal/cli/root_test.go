...
func TestCliErrorRedactsKnownToken(t *testing.T) {
	t.Setenv("THECLOSE_API_TOKEN", "tc_secret_token_123")
	err := authErr(errors.New("Authorization: Bearer tc_secret_token_123"))
	got := err.Error()
	if strings.Contains(got, "tc_secret_token_123") {
		t.Fatalf("error leaked token: %s", got)
	}
	if !strings.Contains(got, "[REDACTED]") {
		t.Fatalf("error should include redaction marker, got: %s", got)
	}
}

func TestCliErrorRedactsProviderSecrets(t *testing.T) {
	err := apiErr(errors.New(`provider payload failed: {"apiKey":"sentinel-one","system_key":"sentinel-two","safe":"keep"}`))
	got := err.Error()
	for _, leaked := range []string{"sentinel-one", "sentinel-two"} {
		if strings.Contains(got, leaked) {
			t.Fatalf("error leaked provider secret %q: %s", leaked, got)
		}
	}
	if !strings.Contains(got, `"safe":"keep"`) {
		t.Fatalf("safe context should remain, got: %s", got)
	}
}
...
func TestCompactFieldsKeepsHighGravityDealFields(t *testing.T) {
	input := json.RawMessage(`[
		{"id":"tx-1","status":"under_contract","health":"AT_RISK","street":"101 Main","description":"verbose","notes":{"x":1}},
		{"id":"tx-2","status":"active","health":"ON_TRACK","street":"202 Oak","description":"verbose","notes":{"x":2}}
	]`)

	got := compactFields(input)
	var rows []map[string]any
	if err := json.Unmarshal(got, &rows); err != nil {
		t.Fatalf("compact output is invalid JSON: %v", err)
	}
	if len(rows) != 2 {
		t.Fatalf("rows = %d, want 2", len(rows))
	}
	for _, row := range rows {
		for _, key := range []string{"id", "status", "health", "street"} {
			if _, ok := row[key]; !ok {
				t.Fatalf("compact row missing %s: %#v", key, row)
			}
		}
		if _, ok := row["description"]; ok {
			t.Fatalf("compact row kept verbose description: %#v", row)
		}
		if _, ok := row["notes"]; ok {
			t.Fatalf("compact row kept nested notes: %#v", row)
		}
	}
}
