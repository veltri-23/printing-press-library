...
func TestRedactJSONSecrets(t *testing.T) {
	t.Parallel()
	body := []byte(`{"apiKey":"abc","nested":{"system_key":"def","safe":"keep"},"items":[{"accessToken":"ghi"}]}`)
	got := string(redactJSONSecrets(body))
	...
	if !strings.Contains(got, `"safe":"keep"`) {
		t.Fatalf("non-sensitive fields should remain: %s", got)
	}
}
