...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("WITHINGS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "withings-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("WITHINGS_ACCESS_TOKEN"); v != "" {
		cfg.AccessToken = v
		cfg.markEnvOverride("AccessToken")
		cfg.AuthSource = "env:WITHINGS_ACCESS_TOKEN"
	}
	...
	if v := os.Getenv("WITHINGS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:WITHINGS_ACCESS_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"access_token":          c.AccessToken,
			"WITHINGS_ACCESS_TOKEN": c.AccessToken,
			"token":                 c.AccessToken,
		})
	}
	if c.AccessToken != "" {
