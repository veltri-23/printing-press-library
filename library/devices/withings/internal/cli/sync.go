...
Resource scoping:
  ...
  withings-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  withings-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  withings-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  withings-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  withings-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "withings-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/withings-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "heart":
		return db.UpsertHeart(data)
	case "measure":
		return db.UpsertMeasure(data)
	case "notify":
