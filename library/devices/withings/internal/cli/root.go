...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "withings-pp-cli",
		Short: `Withings CLI — Every Withings metric in one offline-first, agent-native CLI — with a local SQLite mirror and recomposition, recovery, …`,
		Long: `Withings CLI — Every Withings metric in one offline-first, agent-native CLI — with a local SQLite mirror and recomposition, recovery, …
...
Health check: run 'withings-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("withings-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'withings-pp-cli profile list')")
	...
	rootCmd.AddCommand(newHeartCmd(flags))
	...
	// Withings sync replaces the generated GET-based sync: the Withings API is
	// action-based form-POST, so newWithingsSyncCmd (withings_sync.go) issues
	// WithingsForm POSTs and unpacks the per-endpoint body list. The generated
	// newSyncCmd remains in the package (its helpers are reused) but is not
	// wired into the command tree.
	rootCmd.AddCommand(newWithingsSyncCmd(flags))
	// Web-extras tier: fragile cookie-based passthrough to scalews (see withings_web.go).
	rootCmd.AddCommand(newWebCmd(flags))
	rootCmd.AddCommand(newTailCmd(flags))
	rootCmd.AddCommand(newAnalyticsCmd(flags))
	rootCmd.AddCommand(newWorkflowCmd(flags))
	rootCmd.AddCommand(newNovelBpReportCmd(flags))
	rootCmd.AddCommand(newNovelCorrelateCmd(flags))
	rootCmd.AddCommand(newNovelDigestCmd(flags))
	rootCmd.AddCommand(newNovelRecompCmd(flags))
	rootCmd.AddCommand(newNovelRecoveryCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newDevicesPromotedCmd(flags))
	rootCmd.AddCommand(newGoalsPromotedCmd(flags))
	rootCmd.AddCommand(newMeasurePromotedCmd(flags))
	rootCmd.AddCommand(newWorkoutsPromotedCmd(flags))
