...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "withings-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WITHINGS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WITHINGS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "withings-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/withings-pp-cli/feedback.jsonl.
When ` + "`WITHINGS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WITHINGS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  withings-pp-cli feedback list
  withings-pp-cli feedback list --limit 5
  withings-pp-cli feedback list --json`,
