# Withings Printed CLI Agent Guide

This directory is a generated `withings-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
withings-pp-cli doctor --json
withings-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
withings-pp-cli which "<capability>" --json
withings-pp-cli <command> --help
```
...
```bash
withings-pp-cli <command> --help
withings-pp-cli <command> --dry-run --agent
```
