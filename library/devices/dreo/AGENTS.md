# Dreo Printed CLI Agent Guide

This directory is a generated `dreo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
dreo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dreo-pp-cli which "<capability>" --json
dreo-pp-cli <command> --help
```
...
```bash
dreo-pp-cli <command> --help
dreo-pp-cli <command> --dry-run --agent
```
