...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dreo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DREO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DREO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dreo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dreo-pp-cli/feedback.jsonl.
When ` + "`DREO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DREO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dreo-pp-cli feedback list
  dreo-pp-cli feedback list --limit 5
  dreo-pp-cli feedback list --json`,
