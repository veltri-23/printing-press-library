...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "dreo-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "dreo-pp-cli/0.1.0")
		}
...
func (c *Client) authHeader() (string, error) {
	...
	// Lazy login: if no access token but DREO_USERNAME + DREO_PASSWORD
	// are available (via env or config), do the OAuth exchange now and
	// cache the result. This is what makes the CLI work under
	// dogfood --live's scoped HOME, and for first-time users who haven't
	// run `auth login` yet.
	if c.Config.AccessToken == "" && c.Config.DreoUsername != "" && c.Config.DreoPassword != "" {
		if err := c.lazyLogin(); err != nil {
			return "", err
		}
	}
...
// lazyLogin performs the username/password → bearer token exchange and
// persists the token. Called only when the client has credentials but
// no token. Errors are surfaced so callers can return them as the
// auth error rather than silently sending unauthenticated requests.
func (c *Client) lazyLogin() error {
	resp, err := dreoauth.Login(context.Background(), c.Config.BaseURL, c.Config.DreoUsername, c.Config.DreoPassword)
	if err != nil {
		return fmt.Errorf("lazy login: %w", err)
	}
	c.Config.AccessToken = resp.AccessToken
	c.Config.Region = resp.Region
	// Best-effort persist; ignore errors so a read-only HOME doesn't kill
	// the request. dogfood --live runs in a tempdir HOME where Save may
	// fail silently — we keep the token in memory and re-login next time.
	_ = c.Config.SaveTokens(c.Config.ClientID, c.Config.ClientSecret, resp.AccessToken, "", time.Time{})
	return nil
}
