# Hayward OmniLogic Printed CLI Agent Guide

This directory is a generated `hayward-omnilogic-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hayward-omnilogic-pp-cli doctor --json
hayward-omnilogic-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hayward-omnilogic-pp-cli which "<capability>" --json
hayward-omnilogic-pp-cli <command> --help
```
...
```bash
hayward-omnilogic-pp-cli <command> --agent
```
...
```bash
hayward-omnilogic-pp-cli <command> --help
hayward-omnilogic-pp-cli <command> --dry-run --agent
```
