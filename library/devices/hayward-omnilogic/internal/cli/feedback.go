...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".hayward-omnilogic-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HAYWARD_OMNILOGIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HAYWARD_OMNILOGIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hayward-omnilogic-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.hayward-omnilogic-pp-cli/feedback.jsonl.
When ` + "`HAYWARD_OMNILOGIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HAYWARD_OMNILOGIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hayward-omnilogic-pp-cli feedback list
  hayward-omnilogic-pp-cli feedback list --limit 5
  hayward-omnilogic-pp-cli feedback list --json`,
