# Vestaboard Printed CLI Agent Guide

This directory is a generated `vestaboard-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
vestaboard-pp-cli doctor --json
vestaboard-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
vestaboard-pp-cli which "<capability>" --json
vestaboard-pp-cli <command> --help
```
...
```bash
vestaboard-pp-cli <command> --agent
```
...
```bash
vestaboard-pp-cli <command> --help
vestaboard-pp-cli <command> --dry-run --agent
```
