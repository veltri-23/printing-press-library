...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "vestaboard-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VESTABOARD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VESTABOARD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vestaboard-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/vestaboard-pp-cli/feedback.jsonl.
When ` + "`VESTABOARD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VESTABOARD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vestaboard-pp-cli feedback list
  vestaboard-pp-cli feedback list --limit 5
  vestaboard-pp-cli feedback list --json`,
