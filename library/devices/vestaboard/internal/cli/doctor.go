...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vestaboard-pp-cli doctor
  vestaboard-pp-cli doctor --json
  vestaboard-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export VESTABOARD_API_KEY=<your-key>"
					report["auth_key_url"] = "https://web.vestaboard.com"
					report["auth_instructions"] = "Create a Cloud API token in the API tab on web.vestaboard.com (or in the mobile app under Settings -> Advanced Settings). Pass it as the X-Vestaboard-Token header."
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "VESTABOARD_API_KEY")
				if os.Getenv("VESTABOARD_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "VESTABOARD_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("VESTABOARD_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "VESTABOARD_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "VESTABOARD_API_KEY optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "vestaboard-pp-cli", suggestion)
						} else {
