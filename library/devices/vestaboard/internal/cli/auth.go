...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Vestaboard",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  vestaboard-pp-cli auth setup\n  vestaboard-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://web.vestaboard.com")
			fmt.Fprintln(w, "  Create a Cloud API token in the API tab on web.vestaboard.com (or in the mobile app under Settings -> Advanced Settings). Pass it as the X-Vestaboard-Token header.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export VESTABOARD_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  vestaboard-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://web.vestaboard.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  vestaboard-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export VESTABOARD_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  vestaboard-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  vestaboard-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  vestaboard-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("VESTABOARD_API_KEY") != "" {
				envStillSet = "VESTABOARD_API_KEY"
			}
