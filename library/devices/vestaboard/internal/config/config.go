...
type Config struct {
	...
	Path             string            `toml:"-"`
	VestaboardApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "vestaboard-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("VESTABOARD_API_KEY"); v != "" {
		cfg.VestaboardApiKey = v
		cfg.AuthSource = "env:VESTABOARD_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.VestaboardApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("VESTABOARD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
