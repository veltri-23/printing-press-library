...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tesla-pp-cli doctor
  tesla-pp-cli doctor --json
  tesla-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TESLA_AUTH_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://owner-api.teslamotors.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("TESLA_AUTH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TESLA_AUTH_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TESLA_AUTH_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "tesla-pp-cli", suggestion)
						} else {
			...
			// U5 signed-command path sections. fleet_auth and relay use the
			// OK/INFO/WARN/FAIL string-prefix dispatch convention defined
			// below; signed_command_paths is a status string the dispatch
			// table classifies. None of these surfaces emit token literals.
			report["fleet_auth"] = collectFleetAuthDoctorStatus(cfg)
			report["relay"] = collectRelayDoctorStatus()
			report["signed_command_paths"] = collectSignedCommandPathsDoctorStatus(cfg)
			...
			checkKeys := []struct{ key, label string }{
				...
				{"relay", "Relay"},
				{"signed_command_paths", "Signed Cmd Paths"},
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'tesla-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'tesla-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'tesla-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'tesla-pp-cli sync' to refresh."
	}
	return report
}

// collectFleetAuthDoctorStatus inspects the persisted [fleet] config block
// and TESLA_FLEET_TOKEN env override to report Fleet API auth health. The
// returned string uses the OK/INFO/WARN/FAIL prefix convention the dispatch
// table at ~line 317 of doctor.go classifies. NEVER returns token literals,
// client_secret, or refresh_token bytes; surface presence, the static
// audience constant, and the persisted expiry timestamp only.
func collectFleetAuthDoctorStatus(cfg *config.Config) string {
	envTok := os.Getenv("TESLA_FLEET_TOKEN")
	if cfg == nil && envTok == "" {
		return "INFO not configured; run `tesla auth fleet-login` to enable signed-command dispatch"
	}
	var ft config.FleetConfig
	if cfg != nil {
		ft = cfg.FleetTokens()
	}
	if envTok == "" && ft.AccessToken == "" {
		return "INFO not configured; run `tesla auth fleet-login` to enable signed-command dispatch"
	}
	// Token present. Surface expiry without leaking the token literal.
	if !ft.TokenExpiry.IsZero() && time.Now().After(ft.TokenExpiry) && envTok == "" {
		return fmt.Sprintf("WARN access token expired at %s; run `tesla auth fleet-refresh`", ft.TokenExpiry.UTC().Format(time.RFC3339))
	}
	exp := ""
	if !ft.TokenExpiry.IsZero() {
		exp = ft.TokenExpiry.UTC().Format(time.RFC3339)
	}
	if exp != "" {
		return fmt.Sprintf("OK present (audience=%s; expires %s)", fleetAPIAudience, exp)
	}
	return fmt.Sprintf("OK present (audience=%s)", fleetAPIAudience)
}

// collectRelayDoctorStatus reads the Hermes relay PID/port state. The relay
// is OPTIONAL (Fleet path covers everything Hermes does plus VCSEC), so the
// missing case is INFO not FAIL.
func collectRelayDoctorStatus() string {
	paths, err := newRelayPaths()
	if err != nil {
		return "INFO relay paths unresolved; Hermes path unavailable"
	}
	pid, port, alive := readRelayState(paths)
	if alive {
		if port > 0 {
			return fmt.Sprintf("OK running (pid=%d port=%d)", pid, port)
		}
		return fmt.Sprintf("OK running (pid=%d)", pid)
	}
	// Not running. Detect whether the binary is even installed for a
	// better INFO message.
	if _, ferr := findRelayBinary(); ferr != nil {
		return "INFO not installed (optional; install tesla-http-proxy for the free Hermes path)"
	}
	return "INFO not running; run `tesla relay start` to enable the free Hermes path"
}

// collectSignedCommandPathsDoctorStatus reports which transport paths are
// available right now for issuing signed commands. Reuses the PickPath
...
func collectSignedCommandPathsDoctorStatus(cfg *config.Config) string {
	...
	// Probe the picker on the worst-case command class (VCSEC) for a
	// signed-cmd vehicle: that determines whether the user can dispatch
	// the full command surface or is locked into infotainment-only.
	vcsecChoice, vcsecErr := PickPath(ClassVCSEC, VehicleClassSignedCmd, fleetReady, hermesRunning, "auto")
	ownerChoice, _ := PickPath(ClassOwnerAPI, VehicleClassSignedCmd, fleetReady, hermesRunning, "auto")
	switch {
	case vcsecErr == nil && vcsecChoice.Path == PathFleet:
		return fmt.Sprintf("OK full coverage via Fleet (owner-api routes via %s)", ownerChoice.Path)
	case fleetReady && hermesRunning:
		return fmt.Sprintf("OK fleet+hermes available (owner-api -> %s, vcsec -> %s)", ownerChoice.Path, vcsecChoice.Path)
	case hermesRunning && !fleetReady:
		return "WARN hermes-only (owner-api works; lock/unlock/trunk require Fleet API; run `tesla auth fleet-login`)"
	case fleetReady && !hermesRunning:
		return fmt.Sprintf("OK fleet-only (owner-api -> %s, vcsec -> %s)", ownerChoice.Path, vcsecChoice.Path)
	default:
		return "WARN no cloud transport configured; BLE recipe is the only path. Run `tesla auth fleet-login` or `tesla relay start`"
	}
}
