...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".tesla-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TESLA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TESLA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tesla-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.tesla-pp-cli/feedback.jsonl.
When ` + "`TESLA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TESLA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tesla-pp-cli feedback list
  tesla-pp-cli feedback list --limit 5
  tesla-pp-cli feedback list --json`,
