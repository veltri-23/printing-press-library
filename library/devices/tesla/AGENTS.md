# Tesla Printed CLI Agent Guide

This directory is a generated `tesla-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tesla-pp-cli doctor --json
tesla-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tesla-pp-cli which "<capability>" --json
...
```bash
tesla-pp-cli <command> --help
tesla-pp-cli <command> --dry-run --agent
```
