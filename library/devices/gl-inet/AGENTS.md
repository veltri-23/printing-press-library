# GL.iNet Printed CLI Agent Guide

This directory is a generated `gl-inet-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gl-inet-pp-cli doctor --json
gl-inet-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gl-inet-pp-cli which "<capability>" --json
gl-inet-pp-cli <command> --help
```
...
```bash
gl-inet-pp-cli <command> --help
gl-inet-pp-cli <command> --dry-run --agent
```
