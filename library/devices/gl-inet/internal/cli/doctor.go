...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gl-inet-pp-cli doctor
  gl-inet-pp-cli doctor --json
  gl-inet-pp-cli doctor --fail-on warn
  gl-inet-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "gl-inet-pp-cli", suggestion)
						} else {
			...
			// GL.iNet device probe: model/firmware/capabilities over RPC plus
			// the OpenWrt/LuCI versions and reachability over SSH.
			report["gl"] = collectGLReport(cmd.Context(), flags)
			...
			if glAny, ok := report["gl"]; ok {
				if glRep, ok := glAny.(map[string]any); ok {
					renderGLReport(w, glRep)
				}
			}
			return doctorExitForFailOn(failOn, report)
		},
...
// collectGLReport probes the GL.iNet device: model/firmware/capabilities over
// the JSON-RPC layer and OpenWrt/LuCI versions + reachability over SSH. The
// RPC layer self-short-circuits under verify; the SSH probe is gated here.
func collectGLReport(ctx context.Context, flags *rootFlags) map[string]any {
	rep := map[string]any{}
	...
	if infoErr != nil {
		rep["rpc"] = "unreachable: " + infoErr.Error()
	} else {
		rep["rpc"] = "reachable"
		rep["model"] = info.Model
		rep["firmware"] = info.FirmwareVersion
		rep["country_code"] = info.CountryCode
		rep["openwrt"] = info.BoardInfo.OpenwrtVersion
		if info.SoftwareFeature != nil {
			feats := map[string]bool{}
			for k, v := range info.SoftwareFeature {
				feats[k] = featureEnabled(v)
			}
			rep["software_feature"] = feats
		}
	}

	if cliutil.IsVerifyEnv() {
		rep["ssh"] = "skipped (verify mode)"
		rep["status"] = "ok"
		return rep
	}

	cfg, sshErr := glssh.ResolveConfig(c.RequestBaseURL())
	if sshErr != nil {
		rep["ssh"] = "unavailable: " + sshErr.Error()
	} else if gv, gerr := glssh.Run(ctx, cfg, "cat /etc/glversion 2>/dev/null"); gerr == nil {
		rep["ssh"] = "connected"
		rep["glversion"] = strings.TrimSpace(gv)
		if rel, rerr := glssh.Run(ctx, cfg, "cat /etc/openwrt_release 2>/dev/null"); rerr == nil {
			r := parseOpenwrtRelease(rel)
			rep["openwrt_target"] = r["DISTRIB_TARGET"]
			rep["openwrt_arch"] = r["DISTRIB_ARCH"]
		}
		if luci, lerr := glssh.Run(ctx, cfg, "opkg list-installed 2>/dev/null | grep '^luci '"); lerr == nil {
			rep["luci"] = parseLuciVersion(luci)
		}
	} else {
		rep["ssh"] = "failed: " + gerr.Error()
	}
...
// featureEnabled coerces a software_feature value (often 1/0 or true/false) to
// a boolean availability flag.
func featureEnabled(v any) bool {
	switch t := v.(type) {
	case bool:
		return t
	case float64:
...
func renderGLReport(w io.Writer, rep map[string]any) {
	fmt.Fprintf(w, "  GL.iNet:\n")
	for _, k := range []string{"rpc", "ssh", "model", "firmware", "openwrt", "glversion", "luci", "country_code"} {
		if v, ok := rep[k]; ok && fmt.Sprintf("%v", v) != "" {
			fmt.Fprintf(w, "    %s: %v\n", k, v)
		}
	}
	if featsAny, ok := rep["software_feature"]; ok {
		if feats, ok := featsAny.(map[string]bool); ok && len(feats) > 0 {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("gl-inet-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'gl-inet-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'gl-inet-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'gl-inet-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'gl-inet-pp-cli sync' to refresh."
	}
	return report
}
