...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'gl-inet-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'gl-inet-pp-cli doctor' to check connectivity.", err))
...
func printOutputWithFlags(w io.Writer, data json.RawMessage, flags *rootFlags) error {
	...
	// --plain: render as tab-separated text
	if flags.plain {
...
// printPlain renders array data as tab-separated rows (header + values),
// or prints a single object/scalar as raw JSON. Tabs and newlines inside
// values are collapsed to spaces so each record stays on one line.
	...
	for _, item := range items {
		...
		for _, k := range keys {
			...
			if f, ok := v.(float64); ok {
				...
				vals = append(vals, clean(fmt.Sprintf("%v", v)))
			}
		}
		fmt.Fprintln(w, strings.Join(vals, "\t"))
	}
