...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "gl-inet-pp-cli",
		Short: `Manage a GL.iNet travel router from the terminal — config snapshots, travel macros, and version-aware diagnostics over the local API`,
		Long: `Manage a GL.iNet travel router from the terminal — config snapshots, travel macros, and version-aware diagnostics over the local API
...
Health check: run 'gl-inet-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("gl-inet-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'gl-inet-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.password, "password", "", "Router admin (web-UI) password for the local API/SSH; falls back to GL_INET_PASSWORD. Never persisted.")

	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		// Make the router admin password available to the GL JSON-RPC and SSH
		// layers for this process. Never written to any config or cache file;
		// only the derived session id is cached by the client.
		if strings.TrimSpace(flags.password) != "" {
			client.SetPasswordOverride(flags.password)
		}
	...
	rootCmd.AddCommand(newNovelConfigCmd(flags))
	rootCmd.AddCommand(newNovelRpcCmd(flags))
	rootCmd.AddCommand(newNovelSnapshotCmd(flags))
	rootCmd.AddCommand(newNovelTroubleshootCmd(flags))
	rootCmd.AddCommand(newNovelUplinkCmd(flags))
	rootCmd.AddCommand(newNovelVenueCmd(flags))
	rootCmd.AddCommand(newNovelWanCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newClientsPromotedCmd(flags))
	rootCmd.AddCommand(newSnapshotsPromotedCmd(flags))
	rootCmd.AddCommand(newVpnPromotedCmd(flags))
	rootCmd.AddCommand(newWifiPromotedCmd(flags))
