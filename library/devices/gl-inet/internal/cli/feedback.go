...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GL_INET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GL_INET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gl-inet-pp-cli/feedback")
...
When ` + "`GL_INET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GL_INET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gl-inet-pp-cli feedback list
  gl-inet-pp-cli feedback list --limit 5
  gl-inet-pp-cli feedback list --json`,
