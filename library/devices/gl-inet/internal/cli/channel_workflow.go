...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  gl-inet-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  gl-inet-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clients", "snapshots", "vpn"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  gl-inet-pp-cli workflow status

  # Show status as JSON
  gl-inet-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("gl-inet-pp-cli")
			}
