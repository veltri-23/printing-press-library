# Admin By Request Printed CLI Agent Guide

This directory is a generated `adminbyrequest-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
adminbyrequest-pp-cli doctor --json
adminbyrequest-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
adminbyrequest-pp-cli which "<capability>" --json
...
```bash
adminbyrequest-pp-cli <command> --agent
```
...
```bash
adminbyrequest-pp-cli <command> --help
adminbyrequest-pp-cli <command> --dry-run --agent
```
