...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  adminbyrequest-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  adminbyrequest-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"auditlog", "auditlog-delta", "events", "inventory", "requests"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  adminbyrequest-pp-cli workflow status

  # Show status as JSON
  adminbyrequest-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("adminbyrequest-pp-cli")
			}
