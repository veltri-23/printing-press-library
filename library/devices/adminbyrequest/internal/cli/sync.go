...
Exit codes & warnings:
  ...
  adminbyrequest-pp-cli sync

  # Sync specific resources only
  adminbyrequest-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  adminbyrequest-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  adminbyrequest-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  adminbyrequest-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  adminbyrequest-pp-cli sync --latest-only`,
