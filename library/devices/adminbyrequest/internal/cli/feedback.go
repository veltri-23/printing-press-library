...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".adminbyrequest-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ADMINBYREQUEST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ADMINBYREQUEST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "adminbyrequest-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.adminbyrequest-pp-cli/feedback.jsonl.
When ` + "`ADMINBYREQUEST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ADMINBYREQUEST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  adminbyrequest-pp-cli feedback list
  adminbyrequest-pp-cli feedback list --limit 5
  adminbyrequest-pp-cli feedback list --json`,
