...
native streaming instead of polling.`,
  ...
  adminbyrequest-pp-cli tail --interval 10s

  # Tail a specific resource
  adminbyrequest-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  adminbyrequest-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Initial fetch — also the *only* fetch when --follow=false, so a
			// script can do "tail events --follow=false" for a one-shot snapshot.
