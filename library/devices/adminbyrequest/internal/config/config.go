...
type Config struct {
	...
	Path                 string            `toml:"-"`
	AdminbyrequestApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://dc1api.adminbyrequest.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("ADMINBYREQUEST_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "adminbyrequest-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ADMINBYREQUEST_API_KEY"); v != "" {
		cfg.AdminbyrequestApiKey = v
		cfg.AuthSource = "env:ADMINBYREQUEST_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.AdminbyrequestApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("ADMINBYREQUEST_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.AdminbyrequestApiKey
	if token == "" {
		return ""
	}
	if c.AdminbyrequestApiKey == "" {
		return ""
	}
	replacements := map[string]string{
		"token":                  token,
		"api_key":                c.AdminbyrequestApiKey,
		"ADMINBYREQUEST_API_KEY": c.AdminbyrequestApiKey,
	}
