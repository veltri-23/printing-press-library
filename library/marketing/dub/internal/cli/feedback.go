...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dub-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DUB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DUB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dub-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dub-pp-cli/feedback.jsonl.
When ` + "`DUB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DUB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
