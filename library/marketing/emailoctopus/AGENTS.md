# EmailOctopus Printed CLI Agent Guide

This directory is a generated `emailoctopus-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
emailoctopus-pp-cli doctor --json
emailoctopus-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
emailoctopus-pp-cli which "<capability>" --json
emailoctopus-pp-cli <command> --help
```
...
```bash
emailoctopus-pp-cli <command> --agent
```
...
```bash
emailoctopus-pp-cli <command> --help
emailoctopus-pp-cli <command> --dry-run --agent
```
