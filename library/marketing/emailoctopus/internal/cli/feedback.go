...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".emailoctopus-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EMAILOCTOPUS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EMAILOCTOPUS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "emailoctopus-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.emailoctopus-pp-cli/feedback.jsonl.
When ` + "`EMAILOCTOPUS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EMAILOCTOPUS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  emailoctopus-pp-cli feedback list
  emailoctopus-pp-cli feedback list --limit 5
  emailoctopus-pp-cli feedback list --json`,
