...
func TestUpsertBatch_TypedFailureDoesNotStrandQueueGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("queue count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "automations_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandReportsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("reports count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "campaigns_id")
	}
}

// TestUpsertBatch_SetsReportsParentID verifies that dependent-resource
...
func TestUpsertBatch_SetsReportsParentID(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	var matchedA int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE parent_id = ?`, "reports")
...
func TestUpsertBatch_TypedFailureDoesNotStrandContactsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("contacts count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "lists_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandFieldsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("fields count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "lists_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandTagsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("tags count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "lists_id")
	}
}
