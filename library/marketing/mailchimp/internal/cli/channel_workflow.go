...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  mailchimp-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  mailchimp-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account-exports", "activity-feed", "audiences", "authorized-apps", "automations", "batch-webhooks", "batches", "campaign-folders", "campaigns", "connected-sites", "conversations", "ecommerce", "ecommerce-stores", "facebook-ads", "file-manager", "file-manager-folders", "landing-pages", "lists", "reporting", "reporting-facebook-ads", "reporting-landing-pages", "reports", "sms-campaigns", "template-folders", "templates"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  mailchimp-pp-cli workflow status

  # Show status as JSON
  mailchimp-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("mailchimp-pp-cli")
			}
