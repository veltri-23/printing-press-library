...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "mailchimp-pp-cli",
		Short: `Mailchimp CLI — Every Mailchimp endpoint plus the workflow commands the API forces you to compose yourself.`,
		Long: `Mailchimp CLI — Every Mailchimp endpoint plus the workflow commands the API forces you to compose yourself.
...
Highlights (not in the official API docs):
  ...
  • bulk-subscribe   Read a CSV, fan out through Mailchimp's /batches endpoint (the official 10-concurrent-connection escape hatch), poll until done, decode the tar.gz of JSONL results within the 10-minute response URL w…
  • digest   Single-campaign mode (digest <id>) joins report + email-activity + ecommerce-product-activity into one summary with opens/clicks/bounces/revenue + top-clicked links + top-converted products. Rollup m…
  ...
  • deliverability   Roll up domain-performance reports across the last N campaigns. Surfaces per-domain (gmail.com, yahoo.com, outlook.com) bounce, spam, open, and click rates. Highlights domains performing below the ac…
  • compare   Side-by-side metric diff for two campaigns. Fetches both campaigns' report + email-activity in parallel, renders open rate / CTR / click-to-open / bounces / unsubscribes / revenue with a winner per m…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'mailchimp-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("mailchimp-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'mailchimp-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAccountExportsCmd(flags))
	rootCmd.AddCommand(newAudiencesCmd(flags))
	rootCmd.AddCommand(newAuthorizedAppsCmd(flags))
	rootCmd.AddCommand(newAutomationsCmd(flags))
	rootCmd.AddCommand(newBatchWebhooksCmd(flags))
	rootCmd.AddCommand(newBatchesCmd(flags))
	rootCmd.AddCommand(newCampaignFoldersCmd(flags))
	rootCmd.AddCommand(newCampaignsCmd(flags))
	rootCmd.AddCommand(newConnectedSitesCmd(flags))
	rootCmd.AddCommand(newConversationsCmd(flags))
	rootCmd.AddCommand(newEcommerceCmd(flags))
	rootCmd.AddCommand(newFacebookAdsCmd(flags))
	rootCmd.AddCommand(newFileManagerCmd(flags))
	rootCmd.AddCommand(newLandingPagesCmd(flags))
	rootCmd.AddCommand(newListsCmd(flags))
	rootCmd.AddCommand(newReportingCmd(flags))
	rootCmd.AddCommand(newReportsCmd(flags))
	rootCmd.AddCommand(newSmsCampaignsCmd(flags))
	rootCmd.AddCommand(newTemplateFoldersCmd(flags))
	rootCmd.AddCommand(newTemplatesCmd(flags))
	rootCmd.AddCommand(newVerifiedDomainsCmd(flags))
	...
	rootCmd.AddCommand(newActivityFeedPromotedCmd(flags))
	rootCmd.AddCommand(newCustomerJourneysPromotedCmd(flags))
	rootCmd.AddCommand(newPingPromotedCmd(flags))
	rootCmd.AddCommand(newSearchCampaignsPromotedCmd(flags))
	rootCmd.AddCommand(newSearchMembersPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Novel workflow commands — see novel_*.go. The Highlights block in the
	// root help (above) lists them; new commands here should also appear
	// in research.json's novel_features array so README + SKILL + agent-help
	// stay in sync via the dogfood sync.
	rootCmd.AddCommand(newSubscribeCmd(flags))
	rootCmd.AddCommand(newBulkSubscribeCmd(flags))
	...
	rootCmd.AddCommand(newSendCheckedCmd(flags))
