...
In local mode: searches locally synced data only.`,
  ...
  mailchimp-pp-cli search "error timeout"

  # Force local search only
  mailchimp-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  mailchimp-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  mailchimp-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search-members
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/search-members", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'mailchimp-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "authorized-apps":
				results, err = db.SearchAuthorizedApps(query, limit)
			case "ecommerce":
				results, err = db.SearchEcommerce(query, limit)
			case "landing-pages":
				results, err = db.SearchLandingPages(query, limit)
			case "reporting":
				results, err = db.SearchReporting(query, limit)
			case "templates":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAuthorizedApps(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search authorized_apps failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchEcommerce(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search ecommerce failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchLandingPages(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search landing_pages failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchReporting(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search reporting failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
