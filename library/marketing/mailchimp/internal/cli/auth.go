...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Mailchimp Marketing",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  mailchimp-pp-cli auth setup\n  mailchimp-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://mailchimp.com/developer/marketing/")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export MAILCHIMP_API_KEY=\"<your-key-dc>\"  # e.g. abc...-us6; suffix routes to the correct datacenter")
			fmt.Fprintln(w, "  # Or legacy Basic-auth pair:")
			fmt.Fprintln(w, "  #   export MAILCHIMP_USERNAME=\"anystring\"  &&  export MAILCHIMP_PASSWORD=\"<your-key>\"")
			fmt.Fprintln(w, "  mailchimp-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://mailchimp.com/developer/marketing/"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mailchimp-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export MAILCHIMP_API_KEY=\"your-key-with-dc-suffix\"  # e.g. abc...-us6")
				fmt.Fprintln(w, "  # Or legacy Basic-auth pair:")
				fmt.Fprintln(w, "  #   export MAILCHIMP_USERNAME=\"anystring\"  &&  export MAILCHIMP_PASSWORD=\"your-key\"")
				fmt.Fprintf(w, "  mailchimp-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mailchimp-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mailchimp-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("MAILCHIMP_USERNAME") != "" {
				envStillSet = "MAILCHIMP_USERNAME"
			}
			if envStillSet == "" && os.Getenv("MAILCHIMP_PASSWORD") != "" {
				envStillSet = "MAILCHIMP_PASSWORD"
			}
