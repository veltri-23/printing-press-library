...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".mailchimp-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MAILCHIMP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MAILCHIMP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mailchimp-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.mailchimp-pp-cli/feedback.jsonl.
When ` + "`MAILCHIMP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MAILCHIMP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mailchimp-pp-cli feedback list
  mailchimp-pp-cli feedback list --limit 5
  mailchimp-pp-cli feedback list --json`,
