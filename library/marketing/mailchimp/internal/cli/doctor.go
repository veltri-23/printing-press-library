...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mailchimp-pp-cli doctor
  mailchimp-pp-cli doctor --json
  mailchimp-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MAILCHIMP_API_KEY=<your-key-dc> (the -dc suffix routes requests to the correct datacenter; legacy MAILCHIMP_USERNAME/MAILCHIMP_PASSWORD also accepted)"
					report["auth_docs_url"] = "https://mailchimp.com/developer/marketing/"
				} else {
					authConfigured = true
			...
			if os.Getenv("MAILCHIMP_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "MAILCHIMP_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MAILCHIMP_USERNAME")
			}
			if os.Getenv("MAILCHIMP_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "MAILCHIMP_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MAILCHIMP_PASSWORD")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'mailchimp-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'mailchimp-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'mailchimp-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'mailchimp-pp-cli sync' to refresh."
	}
	return report
}
