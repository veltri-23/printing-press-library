...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".trendhunter-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRENDHUNTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRENDHUNTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "trendhunter-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.trendhunter-pp-cli/feedback.jsonl.
When ` + "`TRENDHUNTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRENDHUNTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  trendhunter-pp-cli feedback list
  trendhunter-pp-cli feedback list --limit 5
  trendhunter-pp-cli feedback list --json`,
