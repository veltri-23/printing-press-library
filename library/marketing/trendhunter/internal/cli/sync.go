...
Exit codes & warnings:
  ...
  trendhunter-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  trendhunter-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  trendhunter-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  trendhunter-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  trendhunter-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/trendhunter-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "category":
		return db.UpsertCategory(data)
	case "megatrends":
		return db.UpsertMegatrends(data)
	case "popular":
		return db.UpsertPopular(data)
	case "reports":
		return db.UpsertReports(data)
	case "results":
		return db.UpsertResults(data)
	case "rss":
		return db.UpsertRss(data)
	case "scoreboard":
		return db.UpsertScoreboard(data)
	case "sitemap":
		return db.UpsertSitemap(data)
	case "trends":
		return db.UpsertTrends(data)
	default:
