...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "trendhunter-pp-cli",
		Short: `Search, sync, and explore TrendHunter trends from the terminal - offline FTS, agent-native JSON, FAQ Q&A extraction, no API key.`,
		Long: `Search, sync, and explore TrendHunter trends from the terminal - offline FTS, agent-native JSON, FAQ Q&A extraction, no API key.
...
Health check: run 'trendhunter-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("trendhunter-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'trendhunter-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCategoryPromotedCmd(flags))
	rootCmd.AddCommand(newMegatrendsPromotedCmd(flags))
	rootCmd.AddCommand(newPopularPromotedCmd(flags))
	rootCmd.AddCommand(newReportsPromotedCmd(flags))
	rootCmd.AddCommand(newResultsPromotedCmd(flags))
	rootCmd.AddCommand(newRssPromotedCmd(flags))
	rootCmd.AddCommand(newScoreboardPromotedCmd(flags))
	rootCmd.AddCommand(newSitemapPromotedCmd(flags))
	rootCmd.AddCommand(newTrendsPromotedCmd(flags))
	// PATCH: Wire parsed TrendHunter parser/store commands advertised in Long help.
	registerTrendhunterCommands(rootCmd, flags)
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("trendhunter-pp-cli %s\n", version)
		},
	}
}
