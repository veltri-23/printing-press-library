...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  trendhunter-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  trendhunter-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"megatrends", "reports"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  trendhunter-pp-cli workflow status

  # Show status as JSON
  trendhunter-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("trendhunter-pp-cli")
			}
