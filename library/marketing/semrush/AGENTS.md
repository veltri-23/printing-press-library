# Semrush Printed CLI Agent Guide

This directory is a generated `semrush-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
semrush-pp-cli doctor --json
semrush-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
semrush-pp-cli which "<capability>" --json
...
```bash
semrush-pp-cli <command> --help
semrush-pp-cli <command> --dry-run --agent
```
