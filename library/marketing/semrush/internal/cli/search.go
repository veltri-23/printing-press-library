...
In local mode: searches locally synced data only.`,
  ...
  semrush-pp-cli search "error timeout"

  # Force local search only
  semrush-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  semrush-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  semrush-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /position-tracking/management/v1/info/locations
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/position-tracking/management/v1/info/locations", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("semrush-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'semrush-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "audit":
				results, err = db.SearchAudit(query, limit)
			case "domain":
				results, err = db.SearchDomain(query, limit)
			case "keyword":
				results, err = db.SearchKeyword(query, limit)
			case "subdomain":
				results, err = db.SearchSubdomain(query, limit)
			case "subfolder":
				results, err = db.SearchSubfolder(query, limit)
			case "tracking":
				results, err = db.SearchTracking(query, limit)
			case "url":
				results, err = db.SearchUrl(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAudit(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search audit failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDomain(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search domain failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchKeyword(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search keyword failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSubdomain(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search subdomain failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSubfolder(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search subfolder failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTracking(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search tracking failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchUrl(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search url failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/semrush-pp-cli/data.db)")
