...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "semrush-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SEMRUSH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SEMRUSH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "semrush-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/semrush-pp-cli/feedback.jsonl.
When ` + "`SEMRUSH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SEMRUSH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  semrush-pp-cli feedback list
  semrush-pp-cli feedback list --limit 5
  semrush-pp-cli feedback list --json`,
