...
Resource scoping:
  ...
  semrush-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  semrush-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  semrush-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  semrush-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  semrush-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "subdomain":
		return db.UpsertSubdomain(data)
	case "subfolder":
		return db.UpsertSubfolder(data)
	case "tracking":
		return db.UpsertTracking(data)
	case "url":
		return db.UpsertUrl(data)
	default:
