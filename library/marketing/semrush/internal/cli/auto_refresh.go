...
var readCommandResources = map[string][]string{
	"semrush-pp-cli project":        {"project"},
	"semrush-pp-cli project list":   {"project"},
	"semrush-pp-cli project get":    {"project"},
	"semrush-pp-cli project search": {"project"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "SEMRUSH_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale semrush-pp-cli cache (refresh failed: %v)\n", err)
