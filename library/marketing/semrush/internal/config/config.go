...
type Config struct {
	...
	Path          string            `toml:"-"`
	SemrushApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SEMRUSH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "semrush-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SEMRUSH_API_KEY"); v != "" {
		cfg.SemrushApiKey = v
		cfg.AuthSource = "env:SEMRUSH_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SemrushApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SEMRUSH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
