# Kit Printed CLI Agent Guide

This directory is a generated `kit-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
kit-pp-cli doctor --json
kit-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
kit-pp-cli which "<capability>" --json
kit-pp-cli <command> --help
```
...
```bash
kit-pp-cli <command> --help
kit-pp-cli <command> --dry-run --agent
```
