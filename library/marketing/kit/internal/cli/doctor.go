...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kit-pp-cli doctor
  kit-pp-cli doctor --json
  kit-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export KIT_API_KEY=<your-key>"
					report["auth_key_url"] = "https://app.kit.com/account_settings/developer_settings"
					report["auth_instructions"] = "Generate a v4 API key in Account Settings → Developer."
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "KIT_API_KEY")
				if os.Getenv("KIT_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "KIT_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("KIT_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "KIT_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "KIT_API_KEY optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						verifyPath := "/v4/account"
						if !strings.HasPrefix(verifyPath, "/") {
							verifyPath = "/" + verifyPath
						}
						authParams := map[string]string{}
						authHeaders := map[string]string{}
						authHeaders["X-Kit-Api-Key"] = authHeader
						authHeaders["User-Agent"] = "kit-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'kit-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'kit-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'kit-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'kit-pp-cli sync' to refresh."
	}
	return report
}
