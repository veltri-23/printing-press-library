...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".kit-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KIT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KIT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kit-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.kit-pp-cli/feedback.jsonl.
When ` + "`KIT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KIT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kit-pp-cli feedback list
  kit-pp-cli feedback list --limit 5
  kit-pp-cli feedback list --json`,
