...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("KIT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "kit-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("KIT_API_KEY"); v != "" {
		cfg.KitApiKey = v
		cfg.AuthSource = "env:KIT_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.KitApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("KIT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
