...
type Config struct {
	...
	Path                        string `toml:"-"`
	GoogleBusinessProfileOauth2 string `toml:"business_profile_oauth2"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GOOGLE_BUSINESS_PROFILE_CONFIG")
	}
	if path == "" {
	...
	// Env var overrides
	if v := os.Getenv("GOOGLE_BUSINESS_PROFILE_OAUTH2"); v != "" {
		cfg.GoogleBusinessProfileOauth2 = v
		cfg.AuthSource = "env:GOOGLE_BUSINESS_PROFILE_OAUTH2"
	}
	...
	if cfg.AuthSource == "" && cfg.GoogleBusinessProfileOauth2 != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GOOGLE_BUSINESS_PROFILE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("GOOGLE_BUSINESS_PROFILE_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("GOOGLE_BUSINESS_PROFILE_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GoogleBusinessProfileOauth2 != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOOGLE_BUSINESS_PROFILE_OAUTH2"
		}
		return "Bearer " + c.GoogleBusinessProfileOauth2
	}
	if c.AccessToken != "" {
