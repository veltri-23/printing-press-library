...
In local mode: searches locally synced data only.`,
  ...
  google-business-profile-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/chains:search
			...
			case "my-business-business-accounts":
				results, err = db.SearchMyBusinessBusinessAccounts(query, limit)
			case "my-business-q-locations":
				results, err = db.SearchMyBusinessQLocations(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchMyBusinessBusinessAccounts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search my_business_business_accounts failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchMyBusinessQLocations(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search my_business_q_locations failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
