...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".google-search-console-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLE_SEARCH_CONSOLE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLE_SEARCH_CONSOLE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "google-search-console-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.google-search-console-pp-cli/feedback.jsonl.
When ` + "`GOOGLE_SEARCH_CONSOLE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLE_SEARCH_CONSOLE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "google-search-console-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-search-console-pp-cli feedback list
  google-search-console-pp-cli feedback list --limit 5
  google-search-console-pp-cli feedback list --json`,
