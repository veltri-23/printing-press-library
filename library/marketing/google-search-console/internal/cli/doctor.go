...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-search-console-pp-cli doctor
  google-search-console-pp-cli doctor --json
  google-search-console-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth
			// PATCH(oauth-login): doctor now leads with the auth-login path
			// (interactive, auto-refreshes). The env-var fallback is still
			// surfaced for CI/scripted use but no longer the headline.
			if cfg != nil {
				header := cfg.AuthHeader()
				if header == "" {
					report["auth"] = "not configured"
					report["auth_hint"] = "auth set-client <id> <secret> && auth login (recommended)  -OR-  export GSC_ACCESS_TOKEN=<token> (CI/one-shot)"
				} else {
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					report["has_refresh_token"] = cfg.RefreshToken != ""
					if !cfg.TokenExpiry.IsZero() {
						report["token_expiry"] = cfg.TokenExpiry.Format(time.RFC3339)
					}
				}
			}
			...
			if os.Getenv("GSC_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GSC_ACCESS_TOKEN")
			} else if cfg != nil && cfg.AuthHeader() != "" {
				// PATCH(oauth-login): auth is configured via `auth login` (config
				// file), so the env var is not required. Avoid a false FAIL.
				authEnvInfo = append(authEnvInfo, "GSC_ACCESS_TOKEN not set; using auth login credentials from "+cfg.Path)
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GSC_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "google-search-console-pp-cli"
