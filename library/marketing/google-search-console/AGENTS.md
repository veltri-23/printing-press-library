# Google Search Console Printed CLI Agent Guide

This directory is a generated `google-search-console-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
google-search-console-pp-cli doctor --json
google-search-console-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
google-search-console-pp-cli which "<capability>" --json
...
```bash
google-search-console-pp-cli <command> --agent
```
...
```bash
google-search-console-pp-cli <command> --help
google-search-console-pp-cli <command> --dry-run --agent
```
