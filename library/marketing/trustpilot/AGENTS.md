# Trustpilot Printed CLI Agent Guide

This directory is a generated `trustpilot-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
trustpilot-pp-cli doctor --json
trustpilot-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
trustpilot-pp-cli which "<capability>" --json
...
```bash
trustpilot-pp-cli <command> --agent
```
...
```bash
trustpilot-pp-cli <command> --help
trustpilot-pp-cli <command> --dry-run --agent
```
