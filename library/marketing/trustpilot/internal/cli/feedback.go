...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".trustpilot-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRUSTPILOT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRUSTPILOT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "trustpilot-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.trustpilot-pp-cli/feedback.jsonl.
When ` + "`TRUSTPILOT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRUSTPILOT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  trustpilot-pp-cli feedback list
  trustpilot-pp-cli feedback list --limit 5
  trustpilot-pp-cli feedback list --json`,
