...
func newCompaniesPromotedCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						"error": "search_build_id is required",
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "search_build_id"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("search_build_id is required\nUsage: %s <%s>", cmd.CommandPath(), "search_build_id"))
			}
			path = replacePathParam(path, "search_build_id", args[0])
	...
	cmd.Flags().StringVar(&flagQuery, "query", "", "Company name or fragment")
