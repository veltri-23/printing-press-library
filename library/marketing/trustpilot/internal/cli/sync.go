...
Exit codes & warnings:
  ...
  trustpilot-pp-cli sync

  # Sync specific resources only
  trustpilot-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  trustpilot-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  trustpilot-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  trustpilot-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  trustpilot-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/trustpilot-pp-cli/data.db)")
