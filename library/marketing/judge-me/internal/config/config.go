...
type Config struct {
	...
	JudgeMePrivateApikey string            `toml:"me_private_apikey"`
	ShopDomain           string            `toml:"shop_domain"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.judge.me/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("JUDGE_ME_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "judge-me-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("JUDGE_ME_API_TOKEN"); v != "" {
		cfg.JudgeMePrivateApikey = v
		cfg.markEnvOverride("JudgeMePrivateApikey")
		cfg.AuthSource = "env:JUDGE_ME_API_TOKEN"
	}
	if v := os.Getenv("JUDGE_ME_PRIVATE_APIKEY"); v != "" {
		cfg.JudgeMePrivateApikey = v
		cfg.markEnvOverride("JudgeMePrivateApikey")
		cfg.AuthSource = "env:JUDGE_ME_PRIVATE_APIKEY"
	}
	if v := os.Getenv("JUDGE_ME_SHOP_DOMAIN"); v != "" {
		cfg.ShopDomain = v
		cfg.markEnvOverride("ShopDomain")
	}
	...
	if cfg.AuthSource == "" && cfg.JudgeMePrivateApikey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("JUDGE_ME_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) DefaultQueryParams() map[string]string {
	if c == nil || c.ShopDomain == "" {
		return nil
	}
	return map[string]string{"shop_domain": c.ShopDomain}
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.JudgeMePrivateApikey = token
	delete(c.envOverrides, "JudgeMePrivateApikey")
	c.updateFileConfigField("JudgeMePrivateApikey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["JudgeMePrivateApikey"] {
			out.JudgeMePrivateApikey = c.fileConfig.JudgeMePrivateApikey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "JudgeMePrivateApikey":
		c.fileConfig.JudgeMePrivateApikey = c.JudgeMePrivateApikey
	}
}
