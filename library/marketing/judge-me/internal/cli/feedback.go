...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "judge-me-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JUDGE_ME_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JUDGE_ME_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "judge-me-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/judge-me-pp-cli/feedback.jsonl.
When ` + "`JUDGE_ME_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JUDGE_ME_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  judge-me-pp-cli feedback list
  judge-me-pp-cli feedback list --limit 5
  judge-me-pp-cli feedback list --json`,
