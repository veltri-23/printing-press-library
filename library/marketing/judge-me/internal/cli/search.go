...
In local mode: searches locally synced data only.`,
  ...
  judge-me-pp-cli search "error timeout"

  # Force local search only
  judge-me-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  judge-me-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  judge-me-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'judge-me-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "reviews":
				results, err = db.SearchReviews(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchReviews(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search reviews failed: %w", searchErr)
					}
