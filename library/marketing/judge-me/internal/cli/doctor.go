...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  judge-me-pp-cli doctor
  judge-me-pp-cli doctor --json
  judge-me-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export JUDGE_ME_PRIVATE_APIKEY=<your-key>"
					report["auth_key_url"] = "https://help.judge.me/en/articles/8278390-integrating-with-judge-me"
				} else {
					authConfigured = true
			...
			if os.Getenv("JUDGE_ME_PRIVATE_APIKEY") != "" {
				authEnvSet = append(authEnvSet, "JUDGE_ME_PRIVATE_APIKEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JUDGE_ME_PRIVATE_APIKEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "judge-me-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("judge-me-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'judge-me-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'judge-me-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'judge-me-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'judge-me-pp-cli sync' to refresh."
	}
	return report
}
