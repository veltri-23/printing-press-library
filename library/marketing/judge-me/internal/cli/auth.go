...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Judge Me",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  judge-me-pp-cli auth setup\n  judge-me-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://help.judge.me/en/articles/8278390-integrating-with-judge-me")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export JUDGE_ME_PRIVATE_APIKEY=\"<your-token>\"")
			fmt.Fprintln(w, "  judge-me-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://help.judge.me/en/articles/8278390-integrating-with-judge-me"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  judge-me-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export JUDGE_ME_PRIVATE_APIKEY=\"your-token-here\"")
				fmt.Fprintf(w, "  judge-me-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  judge-me-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  judge-me-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("JUDGE_ME_PRIVATE_APIKEY") != "" {
				envStillSet = "JUDGE_ME_PRIVATE_APIKEY"
			}
