# Braze Printed CLI Agent Guide

This directory is a generated `braze-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
braze-pp-cli doctor --json
braze-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
braze-pp-cli which "<capability>" --json
braze-pp-cli <command> --help
```
...
```bash
braze-pp-cli <command> --help
braze-pp-cli <command> --dry-run --agent
```
