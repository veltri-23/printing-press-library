...
Resource scoping:
  ...
  braze-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  braze-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  braze-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  braze-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  braze-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/mvanhorn/printing-press-library/library/marketing/braze/data.db)")
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "items", ParentTable: "catalogs", ParentIDParam: "catalog_name", PathTemplate: "/catalogs/{catalog_name}/items", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "catalog_name", Field: "name"},
		}},
	}
}
