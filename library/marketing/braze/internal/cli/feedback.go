...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "braze-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BRAZE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BRAZE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/braze/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/github.com/mvanhorn/printing-press-library/library/marketing/braze/feedback.jsonl.
When ` + "`BRAZE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BRAZE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  braze-pp-cli feedback list
  braze-pp-cli feedback list --limit 5
  braze-pp-cli feedback list --json`,
