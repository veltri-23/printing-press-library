...
func TestUpsertBatch_TypedFailureDoesNotStrandItemsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("items count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "catalogs_id")
	}
}
...
func TestUpsertBatch_KeysItemsByChildAndParent(t *testing.T) {
	...
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "catalogs_id" = ?`, "items")
