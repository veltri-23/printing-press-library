...
type Config struct {
	...
	BrazeBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BRAZE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "braze-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("BRAZE_BEARER_AUTH"); v != "" {
		cfg.BrazeBearerAuth = v
		cfg.markEnvOverride("BrazeBearerAuth")
		cfg.AuthSource = "env:BRAZE_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.BrazeBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("BRAZE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BrazeBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BRAZE_BEARER_AUTH"
		}
		return "Bearer " + c.BrazeBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.BrazeBearerAuth = ""
	delete(c.envOverrides, "BrazeBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BrazeBearerAuth"] {
			out.BrazeBearerAuth = c.fileConfig.BrazeBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BrazeBearerAuth":
		c.fileConfig.BrazeBearerAuth = c.BrazeBearerAuth
	}
}
