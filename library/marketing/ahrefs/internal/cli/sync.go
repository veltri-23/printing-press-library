...
Exit codes & warnings:
  ...
  ahrefs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ahrefs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ahrefs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ahrefs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ahrefs-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "rank_tracker":
		return db.UpsertRankTracker(data)
	case "site_audit":
		return db.UpsertSiteAudit(data)
	case "serp_overview":
		return db.UpsertSerpOverview(data)
	case "site_explorer":
		return db.UpsertSiteExplorer(data)
	case "keywords_explorer":
		return db.UpsertKeywordsExplorer(data)
	default:
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"public":     "/public/crawler-ips",
		"site_audit": "/site-audit/projects",
	}
