...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ahrefs-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AHREFS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AHREFS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ahrefs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ahrefs-pp-cli/feedback.jsonl.
When ` + "`AHREFS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AHREFS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
