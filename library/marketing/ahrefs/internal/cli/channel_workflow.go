...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ahrefs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ahrefs-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"public", "site_audit"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ahrefs-pp-cli workflow status

  # Show status as JSON
  ahrefs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ahrefs-pp-cli")
			}
