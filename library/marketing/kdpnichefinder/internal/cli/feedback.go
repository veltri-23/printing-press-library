...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "kdpnichefinder-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KDPNICHEFINDER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KDPNICHEFINDER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kdpnichefinder-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/kdpnichefinder-pp-cli/feedback.jsonl.
When ` + "`KDPNICHEFINDER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KDPNICHEFINDER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kdpnichefinder-pp-cli feedback list
  kdpnichefinder-pp-cli feedback list --limit 5
  kdpnichefinder-pp-cli feedback list --json`,
