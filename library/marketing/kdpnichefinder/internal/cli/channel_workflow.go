...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  kdpnichefinder-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  kdpnichefinder-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "folders", "saved"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  kdpnichefinder-pp-cli workflow status

  # Show status as JSON
  kdpnichefinder-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("kdpnichefinder-pp-cli")
			}
