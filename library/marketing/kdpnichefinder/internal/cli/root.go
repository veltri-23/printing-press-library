...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "kdpnichefinder-pp-cli",
		Short: `Kdpnichefinder CLI — Every KDP Niche Finder bucket as a local research database — rank niches across buckets, track whether revenue is risin…`,
		Long: `Kdpnichefinder CLI — Every KDP Niche Finder bucket as a local research database — rank niches across buckets, track whether revenue is risin…
...
Health check: run 'kdpnichefinder-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("kdpnichefinder-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'kdpnichefinder-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelCompetitorsCmd(flags))
	rootCmd.AddCommand(newNovelDriftCmd(flags))
	rootCmd.AddCommand(newNovelDupesCmd(flags))
	rootCmd.AddCommand(newNovelExportCmd(flags))
	rootCmd.AddCommand(newNovelKeywordsCmd(flags))
	rootCmd.AddCommand(newNovelRankCmd(flags))
	rootCmd.AddCommand(newNovelSaturationCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBooksPromotedCmd(flags))
	rootCmd.AddCommand(newCategoriesPromotedCmd(flags))
	rootCmd.AddCommand(newNichesPromotedCmd(flags))
	rootCmd.AddCommand(newSavedPromotedCmd(flags))
