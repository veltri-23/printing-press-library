...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "kdpnichefinder-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" && c.HTTPClient.Jar == nil {
			// Cookie auth without a populated CookieJar: send the stored
			// cookie string as the Cookie header. `auth login` persists the
			// captured "name=value; name2=value2" jar into Config.AccessToken;
			// AuthHeader() prepends a "Bearer " scheme we must strip for a
			// Cookie header. When a real CookieJar is present, net/http carries
			// cookies natively and we skip manual setting to avoid duplicates.
			cookieStr := c.Config.AccessToken
			if cookieStr == "" {
				cookieStr = c.Config.AuthHeaderVal
			}
			cookieStr = strings.TrimSpace(strings.TrimPrefix(cookieStr, "Bearer "))
			if cookieStr != "" {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "kdpnichefinder-pp-cli auth set-token <token>")
}
