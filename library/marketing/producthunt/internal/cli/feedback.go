...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".producthunt-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRODUCTHUNT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRODUCTHUNT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "producthunt-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.producthunt-pp-cli/feedback.jsonl.
When ` + "`PRODUCTHUNT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRODUCTHUNT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
