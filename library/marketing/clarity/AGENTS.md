# PP Clarity Printed CLI Agent Guide

This directory is a generated `clarity-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clarity-pp-cli doctor --json
clarity-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clarity-pp-cli which "<capability>" --json
...
```bash
clarity-pp-cli <command> --help
clarity-pp-cli <command> --dry-run --agent
```
