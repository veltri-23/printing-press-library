...
Exit codes & warnings:
  ...
  clarity-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  clarity-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  clarity-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  clarity-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  clarity-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/clarity-pp-cli/data.db)")
