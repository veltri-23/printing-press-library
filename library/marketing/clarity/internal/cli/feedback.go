...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".clarity-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLARITY_CLIENT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLARITY_CLIENT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clarity-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.clarity-pp-cli/feedback.jsonl.
When ` + "`CLARITY_CLIENT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLARITY_CLIENT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clarity-pp-cli feedback list
  clarity-pp-cli feedback list --limit 5
  clarity-pp-cli feedback list --json`,
