...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".customer-io-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CUSTOMER_IO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CUSTOMER_IO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "customer-io-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.customer-io-pp-cli/feedback.jsonl.
When ` + "`CUSTOMER_IO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CUSTOMER_IO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  customer-io-pp-cli feedback list
  customer-io-pp-cli feedback list --limit 5
  customer-io-pp-cli feedback list --json`,
