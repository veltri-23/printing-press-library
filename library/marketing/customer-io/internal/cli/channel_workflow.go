...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  customer-io-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  customer-io-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"cdp_destinations", "cdp_reverse_etl", "cdp_sources", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  customer-io-pp-cli workflow status

  # Show status as JSON
  customer-io-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("customer-io-pp-cli")
			}
