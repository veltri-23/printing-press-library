...
Exit codes & warnings:
  ...
  customer-io-pp-cli sync

  # Sync specific resources only
  customer-io-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  customer-io-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  customer-io-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  customer-io-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  customer-io-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/customer-io-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "suppressions":
		return db.UpsertSuppressions(data)
	case "transactional":
		return db.UpsertTransactional(data)
	case "webhooks":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"cdp_destinations": "/cdp/api/destinations",
		"cdp_reverse_etl":  "/cdp/api/reverse_etl/syncs",
		"cdp_sources":      "/cdp/api/sources",
		"workspaces":       "/v1/accounts/current",
	}
