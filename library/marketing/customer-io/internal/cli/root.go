...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "customer-io-pp-cli",
		Short: `Manage Customer.io campaigns, broadcasts, segments, deliveries, exports, and Reverse-ETL via the Service Account token.`,
		Long: `Manage Customer.io campaigns, broadcasts, segments, deliveries, exports, and Reverse-ETL via the Service Account token.
...
Highlights (not in the official API docs):
  ...
  • deliveries triage   Filter live + local deliveries by template + status + window, write a self-contained bundle (summary.md with SQL group-by error reasons, deliveries.jsonl, recipients.txt) ready to paste into an incid…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'customer-io-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("customer-io-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'customer-io-pp-cli profile list')")
	...
	// Spec-resource parents extended with hand-written transcendence commands.
	broadcastsCmd := newBroadcastsCmd(flags)
	broadcastsCmd.AddCommand(newBroadcastsPreflightCmd(flags))
	rootCmd.AddCommand(broadcastsCmd)

	campaignsCmd := newCampaignsCmd(flags)
	campaignsCmd.AddCommand(newCampaignsFunnelCmd(flags))
	rootCmd.AddCommand(campaignsCmd)

	customersCmd := newCustomersCmd(flags)
	customersCmd.AddCommand(newCustomersTimelineCmd(flags))
	rootCmd.AddCommand(customersCmd)

	deliveriesCmd := newDeliveriesCmd(flags)
	deliveriesCmd.AddCommand(newDeliveriesTriageCmd(flags))
	rootCmd.AddCommand(deliveriesCmd)

	rootCmd.AddCommand(newExportsCmd(flags))

	segmentsCmd := newSegmentsCmd(flags)
	segmentsCmd.AddCommand(newSegmentsOverlapCmd(flags))
	rootCmd.AddCommand(segmentsCmd)

	suppressionsCmd := newSuppressionsCmd(flags)
	suppressionsCmd.AddCommand(newSuppressionsAuditCmd(flags))
	suppressionsCmd.AddCommand(newSuppressionsBulkCmd(flags))
	rootCmd.AddCommand(suppressionsCmd)

	rootCmd.AddCommand(newTransactionalCmd(flags))
	...
	rootCmd.AddCommand(newCdpDestinationsPromotedCmd(flags))

	cdpReverseEtlCmd := newCdpReverseEtlPromotedCmd(flags)
	cdpReverseEtlCmd.AddCommand(newCdpReverseEtlHealthCmd(flags))
	rootCmd.AddCommand(cdpReverseEtlCmd)

	rootCmd.AddCommand(newCdpSourcesPromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("customer-io-pp-cli %s\n", version)
		},
	}
}
