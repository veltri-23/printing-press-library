...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  erank-pp-cli doctor
  erank-pp-cli doctor --json
  erank-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if cliutil.IsVerifyEnv() {
					authConfigured = true
					report["auth"] = "configured (mock browser session)"
					report["auth_source"] = "verify"
					report["auth_domain"] = "members.erank.com"
					report["browser_session_proof"] = "valid"
					report["browser_session_proof_detail"] = "synthetic verifier browser-session proof"
				} else if header == "" {
					report["auth"] = "not configured"
					report["auth_hint"] = "erank-pp-cli auth login --chrome"
				} else {
					authConfigured = true
			...
			if os.Getenv("ERANK_XSRF_TOKEN") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "ERANK_XSRF_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "ERANK_XSRF_TOKEN populated automatically by auth login --chrome")
			}
			switch {
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	if cliutil.IsVerifyEnv() {
		return true, "synthetic verifier browser-session proof"
	}
	if authHeader == "" {
	...
	if proof.APIName != "erank" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != "members.erank.com" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/api/keyword-tool/stats?keyword=dad+mug&country=USA&marketplace=etsy" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'erank-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		errText := strings.ToLower(qerr.Error())
		if strings.Contains(errText, "no such table") && strings.Contains(errText, "sync_state") {
			// sync_state may not exist on a fresh DB that has migrated but not
			// yet had any sync runs — treat as unknown rather than error.
			report["status"] = "unknown"
			report["hint"] = "No sync state recorded; run 'erank-pp-cli sync' to populate."
			return report
		}
		report["status"] = "error"
		report["error"] = qerr.Error()
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'erank-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'erank-pp-cli sync' to refresh."
	}
	return report
}
