...
func profileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".erank-pp-cli")
...
present (other than --profile and --config).`,
		Example: `  erank-pp-cli profile save my-defaults --json --compact
  erank-pp-cli profile save tonight-defaults --region US`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			visit := func(fl *pflag.Flag) {
				if fl.Changed && !skip[fl.Name] && !profileFlagLooksSensitive(fl.Name) {
...
func profileFlagLooksSensitive(name string) bool {
	name = strings.ToLower(name)
	for _, marker := range []string{"auth", "cookie", "password", "secret", "token", "xsrf"} {
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  erank-pp-cli profile use my-defaults
  erank-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  erank-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  erank-pp-cli profile show my-defaults
  erank-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  erank-pp-cli profile delete my-defaults --yes
  erank-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
