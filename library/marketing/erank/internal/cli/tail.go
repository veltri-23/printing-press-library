...
native streaming instead of polling.`,
  ...
  erank-pp-cli tail --interval 10s

  # Tail a specific resource
  erank-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  erank-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if interval <= 0 {
				return usageErr(fmt.Errorf("flag \"interval\" must be greater than 0"))
			}
			...
			if follow {
				fmt.Fprintf(cmd.ErrOrStderr(), "Tailing %s every %s (Ctrl+C to stop)\n", resource, interval)
			}

			// Initial fetch
			if err := fetchAndEmit(cmd.Context(), c, path, enc); err != nil {
				fmt.Fprintf(cmd.ErrOrStderr(), "warning: initial fetch failed: %v\n", err)
			}
			if !follow {
				return nil
			}
			...
			for {
				...
				case <-ticker.C:
					if err := fetchAndEmit(cmd.Context(), c, path, enc); err != nil {
						fmt.Fprintf(cmd.ErrOrStderr(), "warning: poll failed: %v\n", err)
					}
				}
			}
		},
