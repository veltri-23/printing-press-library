...
In local mode: searches locally synced data only.`,
  ...
  erank-pp-cli search "error timeout"

  # Force local search only
  erank-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  erank-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  erank-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if limit <= 0 {
				return usageErr(fmt.Errorf("flag \"limit\" must be greater than 0"))
			}
			query := strings.Join(args, " ")
			// This API has a search endpoint: GET /api/keyword-tool/related-searches
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/keyword-tool/related-searches", map[string]string{
					"keyword": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'erank-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			default:
				results, err = db.SearchResourceType(query, resourceType, limit)
			}
			if err != nil {
