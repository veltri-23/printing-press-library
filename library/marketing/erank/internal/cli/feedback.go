...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "erank-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ERANK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ERANK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "erank-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/erank-pp-cli/feedback.jsonl.
When ` + "`ERANK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ERANK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if endpoint := feedbackEndpoint(); endpoint != "" && (send || feedbackAutoSend()) {
				if err := postFeedback(endpoint, entry); err != nil {
					...
					upstreamResult["endpoint_configured"] = true
				}
			}
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  erank-pp-cli feedback list
  erank-pp-cli feedback list --limit 5
  erank-pp-cli feedback list --json`,
