...
type syncLockedWriter struct {
	mu sync.Mutex
	w  io.Writer
}

func (w *syncLockedWriter) Write(p []byte) (int, error) {
	w.mu.Lock()
	defer w.mu.Unlock()
	return w.w.Write(p)
}
...
Exit codes & warnings:
  ...
  Exit non-zero when any selected resource fails to sync. Access-policy
  warnings are reported separately and exit non-zero when no resource
  syncs successfully.
...
Resource scoping:
  ...
  erank-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  erank-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  erank-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  erank-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  erank-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			syncEventWriter := &syncLockedWriter{w: cmd.OutOrStdout()}
			...
			if latestOnly {
				if since == "" {
					...
					for _, resource := range resources {
						existing, _, _, err := db.GetSyncState(resource)
						if err != nil {
							return fmt.Errorf("read sync state for %s: %w", resource, err)
						}
						if existing != "" {
							if err := db.SaveSyncState(resource, "", 0); err != nil {
								return fmt.Errorf("reset sync state for %s: %w", resource, err)
	...
	cmd.Flags().BoolVar(&strict, "strict", false, "Deprecated compatibility flag; sync now exits non-zero on any per-resource failure.")
...
// syncResourceSinceParam returns the query parameter name used for
// incremental temporal filtering. The eRank endpoints accept an
// updated_at lower-bound filter for incremental refreshes.
...
func defaultSyncResources() []string {
	return []string{
		"account",
		"account-user-preferences-keyword-tool-near-matches-table-config-columns",
		"account-user-preferences-kt-keyword-ideas",
		"account-user-preferences-search-country",
		"account-user-preferences-side-bar-collapse",
		"build",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"account": "/api/account/member-preferences",
		"account-user-preferences-keyword-tool-near-matches-table-config-columns": "/api/account/user-preferences/keyword-tool.near-matches.table.config.columns",
		"account-user-preferences-kt-keyword-ideas":                               "/api/account/user-preferences/kt_keyword_ideas",
		"account-user-preferences-search-country":                                 "/api/account/user-preferences/searchCountry",
		"account-user-preferences-side-bar-collapse":                              "/api/account/user-preferences/sideBarCollapse",
		"build":                    "/build/version.json",
		"check-paddle-restriction": "/api/check-paddle-restriction",
		"intercom":                 "/api/intercom",
		"keywordlist":              "/api/keywordlist/names",
		"keywordlist-terms":        "/api/keywordlist/terms",
		"member-shops":             "/api/member-shops",
		"motd-v3":                  "/api/motd-v3/keyword-tool",
		"oauth":                    "/api/oauth/check-token-validity",
		"quota":                    "/api/quota/daily",
		"refresh-data":             "/api/refresh-data/listings/last-refresh",
	}
