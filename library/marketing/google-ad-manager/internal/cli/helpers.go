...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'google-ad-manager-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: google-ad-manager-pp-cli auth set-token <token>"+
			"\n      or: export GOOGLE_AD_MANAGER_ACCESS_TOKEN=<your-token>"+
			"\n      See API docs: https://google.com"+
			"\n      Run 'google-ad-manager-pp-cli doctor' to check auth status.", err))
