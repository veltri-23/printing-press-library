...
type Config struct {
	...
	GoogleAdManagerAccessToken string            `toml:"ad_manager_access_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GOOGLE_AD_MANAGER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "google-ad-manager-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GOOGLE_AD_MANAGER_ACCESS_TOKEN"); v != "" {
		cfg.GoogleAdManagerAccessToken = v
		cfg.markEnvOverride("GoogleAdManagerAccessToken")
		cfg.AuthSource = "env:GOOGLE_AD_MANAGER_ACCESS_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.GoogleAdManagerAccessToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GOOGLE_AD_MANAGER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GoogleAdManagerAccessToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GOOGLE_AD_MANAGER_ACCESS_TOKEN"
		}
		return "Bearer " + c.GoogleAdManagerAccessToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.GoogleAdManagerAccessToken = ""
	delete(c.envOverrides, "GoogleAdManagerAccessToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GoogleAdManagerAccessToken"] {
			out.GoogleAdManagerAccessToken = c.fileConfig.GoogleAdManagerAccessToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GoogleAdManagerAccessToken":
		c.fileConfig.GoogleAdManagerAccessToken = c.GoogleAdManagerAccessToken
	}
}
