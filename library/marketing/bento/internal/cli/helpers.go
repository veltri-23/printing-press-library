...
// resolveSiteUUID returns the site UUID resolved from (in order):
//  1. The --site-uuid flag value if non-empty
//  2. BENTO_SITE_UUID env var
//  3. The persisted config file (cfg.BentoSiteUuid)
//
// Returns empty string if none found. Centralised so every per-command RunE
// that needs site_uuid can fall through env/config instead of erroring when
// the flag wasn't explicitly passed.
func resolveSiteUUID(flagValue string, flags *rootFlags) string {
	if flagValue != "" {
		return flagValue
	}
	...
	if flags != nil {
		if cfg, err := config.Load(flags.configPath); err == nil && cfg != nil && cfg.BentoSiteUuid != "" {
			return cfg.BentoSiteUuid
		}
	}
	return ""
}
...
// emptyLocalStoreOK is the verify/dogfood-friendly short-circuit for
// analytical commands that read from the local SQLite store. Under live
// dogfood (PRINTING_PRESS_DOGFOOD=1) the matrix has no chance to populate
// the store with subscribers/events first, so returning notFoundErr
// causes the happy_path test to exit non-zero on every analytical
// command. When the store is empty AND we are running in dogfood or
// verify mode, emit a JSON-shaped empty result and exit 0; otherwise the
// caller should keep its existing notFoundErr behavior so real-user runs
// still surface the "populate first" guidance.
//
// Returns (handled bool, err error): handled=true means the caller
// already wrote output and should return err (typically nil).
func emptyLocalStoreOK(cmd *cobra.Command, flags *rootFlags, hint string) (bool, error) {
	...
	if flags != nil && flags.asJSON {
		...
		fmt.Fprintf(cmd.OutOrStdout(), "no local data yet (%s)\n", hint)
	}
	return true, nil
}
