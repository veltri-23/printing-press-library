...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  bento-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  bento-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Mirrors defaultSyncResources(); kept aligned so 'workflow
			// archive' touches the same resources 'sync' does. "experimental"
			// is intentionally excluded — it's a single-shot probe (no list
			// shape), not a list endpoint, so syncing it would fail with
			// "missing id for experimental".
			...
			// syncResource writes NDJSON sync_* events to os.Stdout. That
			// pollutes our final --json envelope on stdout (json_fidelity
			// tests parse stdout as a single JSON object). When --json is
			// active, swap os.Stdout for os.Stderr around the sync loop so
			// the envelope is the only thing on stdout, then restore it
			// before the Encode call below.
			...
			for _, resource := range resources {
				res := syncResource(cmd.Context(), c, s, resource, "", full, 100, false, nil, flags)
				if res.Err != nil {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  bento-pp-cli workflow status

  # Show status as JSON
  bento-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("bento-pp-cli")
			}
