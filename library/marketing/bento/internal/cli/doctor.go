...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bento-pp-cli doctor
  bento-pp-cli doctor --json
  bento-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BENTO_PUBLISHABLE_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("BENTO_PUBLISHABLE_KEY") != "" {
				authEnvSet = append(authEnvSet, "BENTO_PUBLISHABLE_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BENTO_PUBLISHABLE_KEY")
			}
			if os.Getenv("BENTO_SECRET_KEY") != "" {
				authEnvSet = append(authEnvSet, "BENTO_SECRET_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BENTO_SECRET_KEY")
			}
			if os.Getenv("BENTO_SITE_UUID") != "" {
				authEnvSet = append(authEnvSet, "BENTO_SITE_UUID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BENTO_SITE_UUID")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Probe an actual API endpoint (/api/v1/stats/site) instead
					// of "/" — the root path hits the marketing site which is
					// fronted by Cloudflare's JS challenge and reports a false
					// "blocked by Cloudflare interstitial" on every healthy
					// install. /api/v1/stats/site is the lightest authenticated
					// probe: returns 200 with site counts when auth + site_uuid
					// are valid, 401 when auth is wrong.
					siteUUID := os.Getenv("BENTO_SITE_UUID")
					if siteUUID == "" && cfg != nil {
						siteUUID = cfg.BentoSiteUuid
					}
					probePath := "/api/v1/stats/site"
					if siteUUID != "" {
						probePath += "?site_uuid=" + siteUUID
					}
					reachBody, reachErr := c.Get(cmd.Context(), probePath, nil)
					...
					case errors.As(reachErr, &reachAPIErr):
						...
						if vendor := looksLikeDoctorInterstitial([]byte(reachAPIErr.Body)); vendor != "" {
							...
							report["api"] = fmt.Sprintf("reachable (HTTP %d at /api/v1/stats/site)", status)
						}
					default:
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "bento-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'bento-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'bento-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'bento-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'bento-pp-cli sync' to refresh."
	}
	return report
}
