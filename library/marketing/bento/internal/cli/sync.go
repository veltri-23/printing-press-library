...
Resource scoping:
  ...
  bento-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  bento-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  bento-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  bento-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  bento-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if err != nil {
			// Bento's /fetch/search endpoint is Enterprise-only and returns
			// HTTP 422 "enterprise" for everyone else. Treat that specific
			// shape as a warning (not an error) and point the operator at
			// the populate-by-email path so a multi-resource sync still
			// exits 0 when only this gated resource is unreachable.
			if resource == "subscribers" {
				var apiErr *client.APIError
				if errors.As(err, &apiErr) && apiErr.StatusCode == 422 && strings.Contains(strings.ToLower(apiErr.Body), "enterprise") {
					msg := "Bento /fetch/search requires an Enterprise account. Populate subscribers locally via: bento-pp-cli subscribers fetch-batch --emails-from emails.txt --store  OR  bento-pp-cli subscribers import-csv --from bento-export.csv --store"
					if !humanFriendly {
						fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","status":422,"reason":"enterprise_gated","message":"%s"}`+"\n",
							resource, strings.ReplaceAll(msg, `"`, `\"`))
					} else {
						fmt.Fprintf(os.Stderr, "  %s: %s\n", resource, msg)
					}
					return syncResult{Resource: resource, Count: totalCount, Warn: fmt.Errorf("skipped %s: enterprise_gated", resource), Duration: time.Since(started)}
				}
			}
	...
	// Snapshot per resource so 'sync diff' and 'tags drift' have a time
	// series to compare. For tags, capture per-tag subscriber counts when
	// the resource has them; non-tag resources record row-count only.
	var tagCounts map[string]int
	if resource == "tags" {
		tagCounts = collectTagCounts(db)
	}
	_ = db.RecordSnapshot(resource, totalCount, tagCounts)
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "experimental":
		return db.UpsertExperimental(data)
	case "fetch":
		return db.UpsertFetch(data)
	case "stats":
...
// parseSinceDuration converts a since-style value into a time.Time. Accepts:
//   - Relative durations: "7d", "24h", "30m", "1w"
//   - Absolute ISO dates: "2026-05-19" or RFC3339 "2026-05-19T12:00:00Z"
func parseSinceDuration(s string) (time.Time, error) {
	...
	if matches == nil {
		return time.Time{}, fmt.Errorf("expected format like 7d, 24h, 1w, 30m, or an absolute date like 2026-05-19")
	}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"tags":        "/api/v1/fetch/tags",
		"fields":      "/api/v1/fetch/fields",
		"broadcasts":  "/api/v1/fetch/broadcasts",
		"subscribers": "/api/v1/fetch/search",
	}
...
// collectTagCounts walks synced subscribers and counts how many carry each
// tag. Used by snapshots so 'tags drift' can diff week-over-week.
func collectTagCounts(db *store.Store) map[string]int {
