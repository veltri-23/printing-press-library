...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bento-pp-cli",
		Short: `Bento CLI — Every Bento feature, plus the ones their own CLI does not have, plus a local SQLite store that turns 'who are my top sp…`,
		Long: `Bento CLI — Every Bento feature, plus the ones their own CLI does not have, plus a local SQLite store that turns 'who are my top sp…
...
Highlights (not in the official API docs):
  ...
  • events review-window   Emit a dated Bento event stream for orders shipped 10 days ago, skipping any order Stamped already triggered.
...
Health check: run 'bento-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bento-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'bento-pp-cli profile list')")
	...
	rootCmd.AddCommand(newExperimentalCmd(flags))
	...
	syncCmd.AddCommand(newSyncDiffCmd(flags))
	rootCmd.AddCommand(syncCmd)
	rootCmd.AddCommand(newSubscribersCmd(flags))
	rootCmd.AddCommand(newEventsCmd(flags))
	rootCmd.AddCommand(newTagsCmd(flags))
	rootCmd.AddCommand(newBroadcastsCmd(flags))
	rootCmd.AddCommand(newFieldsCmd(flags))
	rootCmd.AddCommand(newHygieneCmd(flags))
