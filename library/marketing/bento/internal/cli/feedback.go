...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bento-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BENTO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BENTO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bento-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bento-pp-cli/feedback.jsonl.
When ` + "`BENTO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BENTO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bento-pp-cli feedback list
  bento-pp-cli feedback list --limit 5
  bento-pp-cli feedback list --json`,
