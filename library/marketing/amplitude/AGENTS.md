# Amplitude Printed CLI Agent Guide

This directory is a generated `amplitude-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
amplitude-pp-cli doctor --json
amplitude-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
amplitude-pp-cli which "<capability>" --json
amplitude-pp-cli <command> --help
```
...
```bash
amplitude-pp-cli <command> --agent
```
...
```bash
amplitude-pp-cli <command> --help
amplitude-pp-cli <command> --dry-run --agent
```
