...
type Config struct {
	...
	AmplitudeUsername string            `toml:"username"`
	AmplitudePassword string            `toml:"password"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("AMPLITUDE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "amplitude-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AMPLITUDE_USERNAME"); v != "" {
		cfg.AmplitudeUsername = v
		cfg.markEnvOverride("AmplitudeUsername")
		cfg.AuthSource = "env:AMPLITUDE_USERNAME"
	}
	if v := os.Getenv("AMPLITUDE_PASSWORD"); v != "" {
		cfg.AmplitudePassword = v
		cfg.markEnvOverride("AmplitudePassword")
		cfg.AuthSource = "env:AMPLITUDE_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.AmplitudeUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.AmplitudePassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AMPLITUDE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AmplitudeUsername == "" || c.AmplitudePassword == "" {
		return ""
	}
	credentials := c.AmplitudeUsername + ":" + c.AmplitudePassword
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["AmplitudeUsername"] {
			out.AmplitudeUsername = c.fileConfig.AmplitudeUsername
		}
		if c.envOverrides["AmplitudePassword"] {
			out.AmplitudePassword = c.fileConfig.AmplitudePassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "AmplitudeUsername":
		c.fileConfig.AmplitudeUsername = c.AmplitudeUsername
	case "AmplitudePassword":
		c.fileConfig.AmplitudePassword = c.AmplitudePassword
	}
}
