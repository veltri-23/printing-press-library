...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  amplitude-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  amplitude-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(cmd.ErrOrStderr(), "amplitude-pp-cli workflow archive: no sync resources in spec; populate the store via single-fetch commands.")
				} else {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  amplitude-pp-cli workflow status

  # Show status as JSON
  amplitude-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("amplitude-pp-cli")
			}
