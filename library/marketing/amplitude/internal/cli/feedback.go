...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "amplitude-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMPLITUDE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMPLITUDE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "amplitude-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/amplitude-pp-cli/feedback.jsonl.
When ` + "`AMPLITUDE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMPLITUDE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amplitude-pp-cli feedback list
  amplitude-pp-cli feedback list --limit 5
  amplitude-pp-cli feedback list --json`,
