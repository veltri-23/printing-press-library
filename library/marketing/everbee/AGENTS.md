# EverBee Printed CLI Agent Guide

This directory is a generated `everbee-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
everbee-pp-cli doctor --json
everbee-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
everbee-pp-cli which "<capability>" --json
everbee-pp-cli <command> --help
```
...
```bash
everbee-pp-cli <command> --help
everbee-pp-cli <command> --dry-run --agent
```
