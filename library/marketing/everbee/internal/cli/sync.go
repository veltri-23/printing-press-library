...
Resource scoping:
  ...
  everbee-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  everbee-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  everbee-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  everbee-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  everbee-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/everbee-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		applyEverbeeSyncDefaults(resource, params)
...
func syncResourceSinceParam(resource string) string {
	switch resource {
	case "keyword_research", "product_analytics", "shops":
		return "updated_after"
	case "management_modals":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"folders":           "/folders",
		"keyword_research":  "/keyword_research/default_keyword_suggestion",
		"product_analytics": "/product_analytics/default_product_analytics",
		"shops":             "/shops",
	}
