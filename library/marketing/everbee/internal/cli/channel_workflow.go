...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  everbee-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  everbee-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"keyword_research", "product_analytics", "shops"}
			totalSynced := 0
			restoreHumanFriendly := humanFriendly
			if flags.asJSON {
				humanFriendly = true
				defer func() { humanFriendly = restoreHumanFriendly }()
			}
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  everbee-pp-cli workflow status

  # Show status as JSON
  everbee-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("github.com/mvanhorn/printing-press-library/library/marketing/everbee")
			}
