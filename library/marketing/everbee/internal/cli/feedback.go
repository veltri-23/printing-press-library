...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".everbee-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EVERBEE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EVERBEE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/everbee/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.everbee-pp-cli/feedback.jsonl.
When ` + "`EVERBEE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EVERBEE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "github.com/mvanhorn/printing-press-library/library/marketing/everbee",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  everbee-pp-cli feedback list
  everbee-pp-cli feedback list --limit 5
  everbee-pp-cli feedback list --json`,
