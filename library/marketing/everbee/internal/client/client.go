...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "github.com/mvanhorn/printing-press-library/library/marketing/everbee", "http")
...
func (c *Client) cacheKey(path string, params map[string]string, headers map[string]string) string {
	...
	if c.Config != nil {
		...
		key += sortedKeyValues("config_header", c.Config.Headers)
	}
	key += sortedKeyValues("param", params)
	key += sortedKeyValues("header", headers)
	h := sha256.Sum256([]byte(key))
	return hex.EncodeToString(h[:8])
}

func sortedKeyValues(prefix string, values map[string]string) string {
	if len(values) == 0 {
		return ""
	}
	...
	for _, k := range keys {
		...
		b.WriteString(k)
		b.WriteString("=")
		b.WriteString(strconv.Itoa(len(values[k])))
		b.WriteString(":")
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "x-access-token", maskToken(authHeader))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export EVERBEE_ACCESS_TOKEN=<your-token> or everbee-pp-cli auth set-token <token>")
}
