...
const defaultAdaptiveLimiterFloor = 0.5
...
func NewAdaptiveLimiter(ratePerSec float64) *AdaptiveLimiter {
	return NewAdaptiveLimiterWithFloor(ratePerSec, defaultAdaptiveLimiterFloor)
}

// NewAdaptiveLimiterWithFloor returns a limiter with an explicit lower bound.
func NewAdaptiveLimiterWithFloor(ratePerSec, floor float64) *AdaptiveLimiter {
	if ratePerSec <= 0 {
		return nil
	}
	if floor <= 0 {
		floor = defaultAdaptiveLimiterFloor
	}
...
func (l *AdaptiveLimiter) Wait() {
	...
	if !l.lastRequest.IsZero() {
		nextRequest = l.lastRequest.Add(delay)
		if nextRequest.Before(now) {
			nextRequest = now
		}
	}
	sleepFor := nextRequest.Sub(now)
	l.lastRequest = nextRequest
	l.mu.Unlock()
	if sleepFor > 0 {
		time.Sleep(sleepFor)
	}
