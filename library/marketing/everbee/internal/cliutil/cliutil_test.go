...
func TestProbeReachable_302_Reachable(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	status, code, err := ProbeReachable(context.Background(), client, srv.URL)
	if err != nil {
		t.Fatalf("unexpected err: %v", err)
	}
	if status != ReachabilityReachable || code != http.StatusFound {
		t.Fatalf("status=%s code=%d, want reachable %d", status, code, http.StatusFound)
	}
}

func TestProbeReachable_SuppliedClientBoundedByContext(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	defer cancel()
	_, _, err := ProbeReachable(ctx, srv.Client(), srv.URL)
	if err == nil {
...
func TestAdaptiveLimiter_ConfigurableFloor(t *testing.T) {
	limiter := NewAdaptiveLimiterWithFloor(1, 0.25)
	limiter.OnRateLimit()
	limiter.OnRateLimit()
	if got := limiter.Rate(); got != 0.25 {
		t.Fatalf("Rate() = %v, want 0.25", got)
	}
}

func TestAdaptiveLimiter_ConcurrentWaitReservesSlots(t *testing.T) {
	limiter := NewAdaptiveLimiter(100)
	start := time.Now()
	limiter.lastRequest = start
	...
	if limiter.lastRequest.Before(start.Add(30 * time.Millisecond)) {
		t.Fatalf("lastRequest advanced to %s, want at least %s", limiter.lastRequest.Sub(start), 30*time.Millisecond)
	}
}
