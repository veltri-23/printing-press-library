# Google Play Printed CLI Agent Guide

This directory is a generated `google-play-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
google-play-pp-cli doctor --json
google-play-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
google-play-pp-cli which "<capability>" --json
google-play-pp-cli <command> --help
```
...
```bash
google-play-pp-cli <command> --agent
```
...
```bash
google-play-pp-cli <command> --help
google-play-pp-cli <command> --dry-run --agent
```
