...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "google-play-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLE_PLAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLE_PLAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "google-play-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/google-play-pp-cli/feedback.jsonl.
When ` + "`GOOGLE_PLAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLE_PLAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-play-pp-cli feedback list
  google-play-pp-cli feedback list --limit 5
  google-play-pp-cli feedback list --json`,
