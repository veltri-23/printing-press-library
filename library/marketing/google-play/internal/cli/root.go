...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "google-play-pp-cli",
		Short: `Google Play CLI — Every public Google Play surface in one Go binary, plus a local SQLite mirror for rank history and listing change detec…`,
		Long: `Google Play CLI — Every public Google Play surface in one Go binary, plus a local SQLite mirror for rank history and listing change detec…
...
Health check: run 'google-play-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("google-play-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'google-play-pp-cli profile list')")
	...
	rootCmd.PersistentFlags().StringVar(&localeCountry, "country", "us", "Two-letter country code (Play gl param)")
	rootCmd.PersistentFlags().StringVar(&localeLang, "lang", "en", "Two-letter language code (Play hl param)")
	...
	rootCmd.AddCommand(newNovelKeywordHistoryCmd(flags))
	rootCmd.AddCommand(newNovelKeywordRankCmd(flags))
	rootCmd.AddCommand(newNovelMoversCmd(flags))
	rootCmd.AddCommand(newNovelRankHistoryCmd(flags))
	rootCmd.AddCommand(newNovelReviewDigestCmd(flags))
	rootCmd.AddCommand(newNovelWatchListingCmd(flags))
	// Hand-authored absorbed live commands (Play public store surfaces).
	rootCmd.AddCommand(newAppCmd(flags))
	rootCmd.AddCommand(newTopCmd(flags))
	rootCmd.AddCommand(newSearchStoreCmd(flags))
	rootCmd.AddCommand(newSuggestCmd(flags))
	rootCmd.AddCommand(newReviewsCmd(flags))
	rootCmd.AddCommand(newSimilarCmd(flags))
	rootCmd.AddCommand(newDeveloperCmd(flags))
	rootCmd.AddCommand(newPermissionsCmd(flags))
	rootCmd.AddCommand(newDataSafetyCmd(flags))
