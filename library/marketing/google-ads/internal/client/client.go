...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "google-ads-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		c.applyGoogleAdsHeaders(req)
		// Per-endpoint header overrides (e.g., different API version per resource)
		for k, v := range headerOverrides {
			req.Header.Set(k, v)
		}
		req.Header.Set("User-Agent", "google-ads-pp-cli/v22")
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if c.Config != nil {
		if token := strings.TrimSpace(c.Config.GoogleAdsDeveloperToken); token != "" {
			fmt.Fprintf(os.Stderr, "  %s: %s\n", "developer-token", maskToken(token))
		}
		if customerID := strings.TrimSpace(c.Config.GoogleAdsLoginCustomerId); customerID != "" {
...
func (c *Client) applyGoogleAdsHeaders(req *http.Request) {
	if c.Config == nil {
		return
	}
	if token := strings.TrimSpace(c.Config.GoogleAdsDeveloperToken); token != "" {
		req.Header.Set("developer-token", token)
	}
	if customerID := strings.TrimSpace(c.Config.GoogleAdsLoginCustomerId); customerID != "" {
