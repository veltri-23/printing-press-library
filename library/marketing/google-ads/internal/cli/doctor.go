...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-ads-pp-cli doctor
  google-ads-pp-cli doctor --json
  google-ads-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export GOOGLE_ADS_ACCESS_TOKEN=<your-key>"
				} else {
			...
			if os.Getenv("GOOGLE_ADS_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_ADS_ACCESS_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GOOGLE_ADS_ACCESS_TOKEN")
			}
			if os.Getenv("GOOGLE_ADS_DEVELOPER_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_ADS_DEVELOPER_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GOOGLE_ADS_DEVELOPER_TOKEN")
			}
			if strings.Contains("Optional manager account ID sent as the login-customer-id header.", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "GOOGLE_ADS_LOGIN_CUSTOMER_ID")
				if os.Getenv("GOOGLE_ADS_LOGIN_CUSTOMER_ID") != "" {
					authEnvSet = append(authEnvSet, "GOOGLE_ADS_LOGIN_CUSTOMER_ID")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("GOOGLE_ADS_LOGIN_CUSTOMER_ID") != "" {
				authEnvSet = append(authEnvSet, "GOOGLE_ADS_LOGIN_CUSTOMER_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "GOOGLE_ADS_LOGIN_CUSTOMER_ID optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						verifyPath := "/v22/customers:listAccessibleCustomers"
						...
						authHeaders["User-Agent"] = "google-ads-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'google-ads-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'google-ads-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'google-ads-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'google-ads-pp-cli sync' to refresh."
	}
	return report
}
