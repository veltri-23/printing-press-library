...
In local mode: searches locally synced data only.`,
  ...
  google-ads-pp-cli search "error timeout"

  # Force local search only
  google-ads-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  google-ads-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  google-ads-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v22/googleAdsFields:search
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'google-ads-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
