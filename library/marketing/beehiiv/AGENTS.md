# Beehiiv Printed CLI Agent Guide

This directory is a generated `beehiiv-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
beehiiv-pp-cli doctor --json
beehiiv-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
beehiiv-pp-cli which "<capability>" --json
...
```bash
beehiiv-pp-cli <command> --help
beehiiv-pp-cli <command> --dry-run --agent
```
