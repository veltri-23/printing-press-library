...
Exit codes & warnings:
  ...
  beehiiv-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  beehiiv-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  beehiiv-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  beehiiv-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  beehiiv-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/beehiiv-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "advertisement_opportunities":
		return db.UpsertAdvertisementOpportunities(data)
	case "authors":
		return db.UpsertAuthors(data)
	case "automations":
		return db.UpsertAutomations(data)
	case "emails":
		return db.UpsertEmails(data)
	case "journeys":
		return db.UpsertJourneys(data)
	case "bulk_subscription_updates":
		return db.UpsertBulkSubscriptionUpdates(data)
	case "bulk_subscriptions":
		return db.UpsertBulkSubscriptions(data)
	case "condition_sets":
		return db.UpsertConditionSets(data)
	case "custom_fields":
		return db.UpsertCustomFields(data)
	case "data_privacy":
		return db.UpsertDataPrivacy(data)
	case "email_blasts":
		return db.UpsertEmailBlasts(data)
	case "engagements":
		return db.UpsertEngagements(data)
	case "newsletter_lists":
		return db.UpsertNewsletterLists(data)
	case "newsletter_lists_subscriptions":
		return db.UpsertNewsletterListsSubscriptions(data)
	case "polls":
		return db.UpsertPolls(data)
	case "responses":
		return db.UpsertResponses(data)
	case "post_templates":
		return db.UpsertPostTemplates(data)
	case "posts":
		return db.UpsertPosts(data)
	case "publications":
		return db.UpsertPublications(data)
	case "referral_program":
		return db.UpsertReferralProgram(data)
	case "segments":
		return db.UpsertSegments(data)
	case "members":
		return db.UpsertMembers(data)
	case "recalculate":
		return db.UpsertRecalculate(data)
	case "results":
		return db.UpsertResults(data)
	case "subscriptions":
		return db.UpsertSubscriptions(data)
	case "jwt_token":
		return db.UpsertJwtToken(data)
	case "tags":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "authors", ParentTable: "publications", ParentIDParam: "publicationId", PathTemplate: "/publications/{publicationId}/authors"},
