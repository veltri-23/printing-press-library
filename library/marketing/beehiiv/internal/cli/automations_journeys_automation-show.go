...
func newAutomationsJourneysAutomationShowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(args) < 3 {
				return usageErr(fmt.Errorf("automationJourneyId is required\nUsage: %s <%s>", cmd.CommandPath(), "automationJourneyId"))
			}
			path = replacePathParam(path, "automationJourneyId", args[2])
