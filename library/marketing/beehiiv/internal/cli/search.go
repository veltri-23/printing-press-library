...
In local mode: searches locally synced data only.`,
  ...
  beehiiv-pp-cli search "error timeout"

  # Force local search only
  beehiiv-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  beehiiv-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  beehiiv-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'beehiiv-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "automations":
				results, err = db.SearchAutomations(query, limit)
			case "newsletter_lists":
				results, err = db.SearchNewsletterLists(query, limit)
			case "polls":
				results, err = db.SearchPolls(query, limit)
			case "tiers":
				results, err = db.SearchTiers(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchAutomations(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search automations failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchNewsletterLists(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search newsletter_lists failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPolls(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search polls failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTiers(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search tiers failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
