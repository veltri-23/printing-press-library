...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  beehiiv-pp-cli doctor
  beehiiv-pp-cli doctor --json
  beehiiv-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BEEHIIV_BEARER_AUTH=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("BEEHIIV_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "BEEHIIV_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BEEHIIV_BEARER_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'beehiiv-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'beehiiv-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'beehiiv-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'beehiiv-pp-cli sync' to refresh."
	}
	return report
}
