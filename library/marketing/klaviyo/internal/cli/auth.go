...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage KLAVIYO_API_KEY credentials",
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  klaviyo-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if header == "" {
				...
				fmt.Fprintln(w, "  export KLAVIYO_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  klaviyo-pp-cli auth set-token <token>\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  klaviyo-pp-cli auth set-token sk_live_abc123",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  klaviyo-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if os.Getenv("KLAVIYO_API_KEY") != "" {
				fmt.Fprintf(cmd.OutOrStdout(), "Config cleared. Note: KLAVIYO_API_KEY env var is still set.\n")
				return nil
			}
