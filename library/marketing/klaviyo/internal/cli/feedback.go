...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".klaviyo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KLAVIYO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KLAVIYO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "klaviyo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.klaviyo-pp-cli/feedback.jsonl.
When ` + "`KLAVIYO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KLAVIYO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
