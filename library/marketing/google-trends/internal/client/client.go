...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "google-trends-pp-cli auth set-token <token>")
}
...
func sanitizeJSONResponse(body []byte) []byte {
	...
	// ")]}'," (comma included) must precede ")]}'\n" / ")]}'" — those are
	// themselves byte-prefixes of the comma variant, so checking them first
	// would strip 4 bytes and leave a dangling "," that breaks
