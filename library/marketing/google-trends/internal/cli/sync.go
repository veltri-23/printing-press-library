...
Resource scoping:
  ...
  google-trends-pp-cli sync

  # Sync specific resources only
  google-trends-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  google-trends-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  google-trends-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  google-trends-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  google-trends-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "google-trends-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		"trends": "/trends/api/widgetdata/relatedsearches",
	}
