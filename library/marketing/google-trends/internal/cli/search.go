...
In local mode: searches locally synced data only.`,
  ...
  google-trends-pp-cli search "error timeout"

  # Force local search only
  google-trends-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  google-trends-pp-cli search "status" --type trends --data-source local
  # JSON output for piping
  google-trends-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /trends/api/widgetdata/relatedsearches
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/trends/api/widgetdata/relatedsearches", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'google-trends-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
