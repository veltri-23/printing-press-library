...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLE_TRENDS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLE_TRENDS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "google-trends-pp-cli/feedback")
...
When ` + "`GOOGLE_TRENDS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLE_TRENDS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-trends-pp-cli feedback list
  google-trends-pp-cli feedback list --limit 5
  google-trends-pp-cli feedback list --json`,
