# Google Trends Printed CLI Agent Guide

This directory is a generated `google-trends-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
google-trends-pp-cli doctor --json
google-trends-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
google-trends-pp-cli which "<capability>" --json
google-trends-pp-cli <command> --help
```
...
```bash
google-trends-pp-cli <command> --agent
```
...
```bash
google-trends-pp-cli <command> --help
google-trends-pp-cli <command> --dry-run --agent
```
