...
In local mode: searches locally synced data only.`,
  ...
  meta-ads-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			switch resourceType {
			case "adcreatives":
				results, err = db.SearchAdcreatives(query, limit)
			case "customaudiences":
				results, err = db.SearchCustomaudiences(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAdcreatives(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search adcreatives failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchCustomaudiences(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search customaudiences failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
