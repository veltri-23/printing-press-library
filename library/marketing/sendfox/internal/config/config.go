...
type Config struct {
	...
	Path              string            `toml:"-"`
	SendfoxBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SENDFOX_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sendfox-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides. SENDFOX_API_TOKEN is the public docs alias; the
	// generated SENDFOX_BEARER_AUTH name remains supported for compatibility.
	if v := os.Getenv("SENDFOX_API_TOKEN"); v != "" {
		cfg.SendfoxBearerAuth = v
		cfg.AuthSource = "env:SENDFOX_API_TOKEN"
	} else if v := os.Getenv("SENDFOX_BEARER_AUTH"); v != "" {
		cfg.SendfoxBearerAuth = v
		cfg.AuthSource = "env:SENDFOX_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.SendfoxBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SENDFOX_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SendfoxBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SENDFOX_BEARER_AUTH"
		}
		return "Bearer " + c.SendfoxBearerAuth
	}
	if c.AccessToken != "" {
