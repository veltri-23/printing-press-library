...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sendfox-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SENDFOX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SENDFOX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/marketing/sendfox/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sendfox-pp-cli/feedback.jsonl.
When ` + "`SENDFOX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SENDFOX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sendfox-pp-cli feedback list
  sendfox-pp-cli feedback list --limit 5
  sendfox-pp-cli feedback list --json`,
