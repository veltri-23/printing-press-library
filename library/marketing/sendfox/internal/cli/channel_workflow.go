...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sendfox-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sendfox-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"campaigns", "contacts", "lists"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sendfox-pp-cli workflow status

  # Show status as JSON
  sendfox-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sendfox-pp-cli")
			}
