# Sendfox Printed CLI Agent Guide

This directory is a generated `sendfox-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sendfox-pp-cli doctor --json
sendfox-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sendfox-pp-cli which "<capability>" --json
sendfox-pp-cli <command> --help
```
...
```bash
sendfox-pp-cli <command> --help
sendfox-pp-cli <command> --dry-run --agent
```
