...
type Config struct {
	...
	Path               string            `toml:"-"`
	StackadaptApiToken string            `toml:"api_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.stackadapt.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("STACKADAPT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "stackadapt-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("STACKADAPT_API_TOKEN"); v != "" {
		cfg.StackadaptApiToken = v
		cfg.AuthSource = "env:STACKADAPT_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.StackadaptApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("STACKADAPT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.StackadaptApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:STACKADAPT_API_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"api_token":            c.StackadaptApiToken,
			"STACKADAPT_API_TOKEN": c.StackadaptApiToken,
			"token":                c.StackadaptApiToken,
		})
	}
	if c.AccessToken != "" {
