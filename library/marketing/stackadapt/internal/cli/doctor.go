...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  stackadapt-pp-cli doctor
  stackadapt-pp-cli doctor --json
  stackadapt-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export STACKADAPT_API_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("STACKADAPT_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "STACKADAPT_API_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "STACKADAPT_API_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "stackadapt-pp-cli", suggestion)
						} else {
