...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "stackadapt-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STACKADAPT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STACKADAPT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "stackadapt-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/stackadapt-pp-cli/feedback.jsonl.
When ` + "`STACKADAPT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STACKADAPT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  stackadapt-pp-cli feedback list
  stackadapt-pp-cli feedback list --limit 5
  stackadapt-pp-cli feedback list --json`,
