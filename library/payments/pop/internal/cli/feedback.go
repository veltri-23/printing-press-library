...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pop-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pop-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pop-pp-cli/feedback.jsonl.
When ` + "`POP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pop-pp-cli feedback list
  pop-pp-cli feedback list --limit 5
  pop-pp-cli feedback list --json`,
