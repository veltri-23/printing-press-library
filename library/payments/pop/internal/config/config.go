...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("POP_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "pop-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("POP_API_KEY"); v != "" {
		cfg.PopApiKey = v
		cfg.AuthSource = "env:POP_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PopApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("POP_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
