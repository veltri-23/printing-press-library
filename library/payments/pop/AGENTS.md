# Pop Printed CLI Agent Guide

This directory is a generated `pop-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pop-pp-cli doctor --json
pop-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pop-pp-cli which "<capability>" --json
pop-pp-cli <command> --help
```
...
```bash
pop-pp-cli <command> --help
pop-pp-cli <command> --dry-run --agent
```
