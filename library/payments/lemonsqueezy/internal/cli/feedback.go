...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "lemonsqueezy-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LEMONSQUEEZY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LEMONSQUEEZY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lemonsqueezy-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/lemonsqueezy-pp-cli/feedback.jsonl.
When ` + "`LEMONSQUEEZY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LEMONSQUEEZY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lemonsqueezy-pp-cli feedback list
  lemonsqueezy-pp-cli feedback list --limit 5
  lemonsqueezy-pp-cli feedback list --json`,
