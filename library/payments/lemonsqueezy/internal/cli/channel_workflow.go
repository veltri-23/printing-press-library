...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  lemonsqueezy-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  lemonsqueezy-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"affiliates", "checkouts", "customers", "discount-redemptions", "discounts", "files", "license-key-instances", "license-keys", "order-items", "orders", "prices", "products", "stores", "subscription-invoices", "subscription-items", "subscriptions", "usage-records", "variants", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  lemonsqueezy-pp-cli workflow status

  # Show status as JSON
  lemonsqueezy-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("lemonsqueezy-pp-cli")
			}
