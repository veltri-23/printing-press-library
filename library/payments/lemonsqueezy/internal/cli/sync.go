...
Resource scoping:
  ...
  lemonsqueezy-pp-cli sync

  # Sync specific resources only
  lemonsqueezy-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  lemonsqueezy-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  lemonsqueezy-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  lemonsqueezy-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  lemonsqueezy-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/lemonsqueezy-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "current_usage":
		return db.UpsertCurrentUsage(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"discount-redemptions":  "/v1/discount-redemptions",
		"discounts":             "/v1/discounts",
		"files":                 "/v1/files",
		"license-key-instances": "/v1/license-key-instances",
		"license-keys":          "/v1/license-keys",
		"order-items":           "/v1/order-items",
		"orders":                "/v1/orders",
		"prices":                "/v1/prices",
		"products":              "/v1/products",
		"stores":                "/v1/stores",
		"subscription-invoices": "/v1/subscription-invoices",
		"subscription-items":    "/v1/subscription-items",
		"subscriptions":         "/v1/subscriptions",
		"usage-records":         "/v1/usage-records",
