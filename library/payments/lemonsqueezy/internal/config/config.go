...
type Config struct {
	...
	Path               string            `toml:"-"`
	LemonsqueezyApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("LEMONSQUEEZY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "lemonsqueezy-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("LEMONSQUEEZY_API_KEY"); v != "" {
		cfg.LemonsqueezyApiKey = v
		cfg.AuthSource = "env:LEMONSQUEEZY_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.LemonsqueezyApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("LEMONSQUEEZY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.LemonsqueezyApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:LEMONSQUEEZY_API_KEY"
		}
		return "Bearer " + c.LemonsqueezyApiKey
	}
	if c.AccessToken != "" {
