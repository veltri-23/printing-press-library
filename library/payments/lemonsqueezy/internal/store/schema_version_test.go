...
// TestMigrate_AddsColumnsOnUpgrade_CurrentUsage verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_CurrentUsage(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "current_usage" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("current_usage")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from current_usage after migrate", want)
		}
	}
}
