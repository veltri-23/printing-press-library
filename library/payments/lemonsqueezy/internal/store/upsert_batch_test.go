...
// TestUpsertBatch_PopulatesCurrentUsageTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed current_usage table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesCurrentUsageTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "subscription_items_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "subscription_items_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "subscription_items_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("current_usage", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "current_usage").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "current_usage")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count current_usage: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("current_usage count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandCurrentUsageGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandCurrentUsageGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "subscription_items_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("current_usage", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("current_usage count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "subscription_items_id")
	}
}
