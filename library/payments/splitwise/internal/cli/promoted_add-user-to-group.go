...
func newAddUserToGroupPromotedCmd(flags *rootFlags) *cobra.Command {
	var flagBodyJSON string

	cmd := &cobra.Command{
		Use:         "add-user-to-group",
		Short:       "**Note**: 200 OK does not indicate a successful response. You must check the `success` value of the response.",
		Long:        "**Note**: 200 OK does not indicate a successful response. You must check the `success` value of the response.",
		Example:     "  splitwise-pp-cli add-user-to-group",
		Annotations: map[string]string{"pp:endpoint": "add-user-to-group.create", "pp:method": "POST", "pp:path": "/add_user_to_group"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "add-user-to-group", data, "")
			}
