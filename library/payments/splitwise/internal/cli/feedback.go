...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "splitwise-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SPLITWISE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SPLITWISE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "splitwise-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/splitwise-pp-cli/feedback.jsonl.
When ` + "`SPLITWISE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SPLITWISE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  splitwise-pp-cli feedback list
  splitwise-pp-cli feedback list --limit 5
  splitwise-pp-cli feedback list --json`,
