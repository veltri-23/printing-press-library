...
type Config struct {
	...
	Path            string            `toml:"-"`
	SplitwiseApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SPLITWISE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "splitwise-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SPLITWISE_API_KEY"); v != "" {
		cfg.SplitwiseApiKey = v
		cfg.AuthSource = "env:SPLITWISE_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SplitwiseApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SPLITWISE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SplitwiseApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SPLITWISE_API_KEY"
		}
		return "Bearer " + c.SplitwiseApiKey
	}
	if c.AccessToken != "" {
