# Splitwise Printed CLI Agent Guide

This directory is a generated `splitwise-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
splitwise-pp-cli doctor --json
splitwise-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
splitwise-pp-cli which "<capability>" --json
splitwise-pp-cli <command> --help
```
...
```bash
splitwise-pp-cli <command> --agent
```
...
```bash
splitwise-pp-cli <command> --help
splitwise-pp-cli <command> --dry-run --agent
```
