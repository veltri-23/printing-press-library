...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lunch-money-pp-cli doctor
  lunch-money-pp-cli doctor --json
  lunch-money-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export LUNCHMONEY_API_KEY=<your-key>"
					report["auth_key_url"] = "https://my.lunchmoney.app/developers"
				} else {
					authConfigured = true
			...
			// PATCH(env-trim): single canonical env var; config file is the
			// fallback when the env var is unset.
			authEnvSet := []string{}
			authEnvRequiredMissing := []string{}
			authEnvInfo := []string{}
			if os.Getenv("LUNCHMONEY_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "LUNCHMONEY_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "LUNCHMONEY_API_KEY")
			}
			switch {
			...
			default:
				report["env_vars"] = fmt.Sprintf("OK %d/1 available", len(authEnvSet))
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'lunch-money-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'lunch-money-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		actualRows := cachedResourceRows(s.DB(), rtype, count)
		r := map[string]any{"type": rtype, "rows": actualRows}
		if actualRows != count {
			r["last_sync_count"] = count
		}
		if lastSynced.Valid {
			haveAny = true
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'lunch-money-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'lunch-money-pp-cli sync' to refresh."
	}
	return report
}

// PATCH: Doctor should report actual cached rows, not the last sync batch count.
func cachedResourceRows(db *sql.DB, resourceType string, fallback int64) int64 {
	var count int64
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, resourceType).Scan(&count); err == nil {
		return count
	}
	return fallback
}
