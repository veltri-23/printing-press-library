...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/payments/lunch-money/internal/cliutil"
...
func writeNoop(flags *rootFlags, reason, prose string) error {
	if flags != nil && flags.asJSON {
		...
		if flags.deliverBuf != nil {
			// PATCH: classify* helpers do not receive a Cobra command, so mirror
			// root's --deliver MultiWriter for structured idempotent no-op output.
			w = io.MultiWriter(os.Stdout, flags.deliverBuf)
		}
		return json.NewEncoder(w).Encode(noopResult{Status: "noop", Reason: reason})
	}
	fmt.Fprintln(os.Stderr, prose)
	return nil
}

// PATCH: Share machine-output detection between error rendering and client
// setup so --agent/--json never mix human stderr with JSON stdout.
func wantsMachineOutput(flags *rootFlags) bool {
	if flags == nil {
		return false
	}
	return flags.asJSON || flags.agent || flags.compact || flags.csv || flags.plain || flags.quiet
}

func wantsStructuredError(flags rootFlags) bool {
	// PATCH: Generated commands auto-emit JSON when stdout is piped, even
	// without --json/--agent. Render errors the same way so `2>&1 | jq`
	// pipelines stay parseable on API failures.
	return wantsMachineOutput(&flags) || !isTerminal(os.Stdout)
}
...
func structuredError(err error) error {
	...
	// PATCH: Preserve the original exit code while making err.Error()
	// directly parseable for --agent/--json stderr pipelines.
	return &cliError{code: code, err: errors.New(string(b))}
}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 409"):
		...
		// PATCH: Structured errors are rendered once at Execute/main level.
		return apiErr(err)
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		//nolint:staticcheck // PATCH: user-facing multi-line remediation text intentionally uses sentence punctuation.
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export LUNCHMONEY_API_KEY=<your-key>"+
			"\n      Get a key at: https://my.lunchmoney.app/developers"+
			"\n      Run 'lunch-money-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		//nolint:staticcheck // PATCH: user-facing multi-line remediation text intentionally uses sentence punctuation.
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: lunch-money-pp-cli auth set-token <token>"+
			"\n      or: export LUNCHMONEY_API_KEY=<your-token>"+
			"\n      Get a key at: https://my.lunchmoney.app/developers"+
			"\n      Run 'lunch-money-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		//nolint:staticcheck // PATCH: user-facing multi-line remediation text intentionally uses sentence punctuation.
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your API key has the required permissions."+
			"\n      Set it with: export LUNCHMONEY_API_KEY=<your-key>"+
			"\n      Get a key at: https://my.lunchmoney.app/developers"+
			"\n      Run 'lunch-money-pp-cli doctor' to check auth status.", err))
...
// PATCH: Generated update commands must use flag presence, not zero values,
// so explicit false/0/""/null requests can clear fields through the API.
func addStringBodyIfChanged(cmd *cobra.Command, body map[string]any, flagName, wireName, value string) {
	if cmd.Flags().Changed(flagName) {
		body[wireName] = value
	}
}

func addNullableStringBodyIfChanged(cmd *cobra.Command, body map[string]any, flagName, wireName, value string) {
	if !cmd.Flags().Changed(flagName) {
		return
	}
...
func addIntBodyIfChanged(cmd *cobra.Command, body map[string]any, flagName, wireName string, value int) {
	if cmd.Flags().Changed(flagName) {
		body[wireName] = value
	}
}

func addFloatBodyIfChanged(cmd *cobra.Command, body map[string]any, flagName, wireName string, value float64) {
	if cmd.Flags().Changed(flagName) {
		body[wireName] = value
	}
}

func addBoolBodyIfChanged(cmd *cobra.Command, body map[string]any, flagName, wireName string, value bool) {
	if cmd.Flags().Changed(flagName) {
		body[wireName] = value
	}
}

func addJSONBodyIfChanged(cmd *cobra.Command, body map[string]any, flagName, wireName, raw string) error {
	if !cmd.Flags().Changed(flagName) {
		return nil
	}
	var parsed any
	if err := json.Unmarshal([]byte(raw), &parsed); err != nil {
		return fmt.Errorf("parsing --%s JSON: %w", flagName, err)
	}
	body[wireName] = parsed
	return nil
}
				...
				if hasMoreField != "" {
					if moreRaw, ok := rawAtPath(obj, hasMoreField); ok {
						...
						if json.Unmarshal(moreRaw, &more) == nil && more {
							// PATCH: Offset-paginated endpoints such as
							// transactions expose has_more without a next
							// cursor. Advance offset before the next request.
							if cursorParam == "offset" && advanceOffsetCursor(clean, pageItemsCount) {
								continue
							}
							break
						}
...
func advanceOffsetCursor(params map[string]string, itemCount int) bool {
	step := itemCount
	if step <= 0 {
		step = parsePositiveInt(params["limit"])
	}
	if step <= 0 {
		return false
	}
	current := parsePositiveInt(params["offset"])
	params["offset"] = fmt.Sprintf("%d", current+step)
	return true
}
