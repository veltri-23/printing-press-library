...
large datasets as it has no memory pressure.`,
  ...
  lunch-money-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  lunch-money-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  lunch-money-pp-cli export <resource> --format jsonl | jq '.id'`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if outputFile != "" {
				f, err := createPrivateOutputFile(outputFile)
				if err != nil {
...
// PATCH: Exported Lunch Money data contains financial records, so files should be owner-only.
func createPrivateOutputFile(path string) (io.WriteCloser, error) {
