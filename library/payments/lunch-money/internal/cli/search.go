...
In local mode: searches locally synced data only.`,
  ...
  lunch-money-pp-cli search "error timeout"

  # Force local search only
  lunch-money-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  lunch-money-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  lunch-money-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nrun 'lunch-money-pp-cli sync' first to populate the local database", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "":
				// PATCH: Actually run the all-resource FTS query.
				results, err = db.Search(query, limit)
			default:
				// PATCH: Honor --type instead of searching every resource.
