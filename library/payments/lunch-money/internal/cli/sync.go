...
Exit codes & warnings:
  ...
  lunch-money-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  lunch-money-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  lunch-money-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  lunch-money-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  lunch-money-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			// PATCH: Route sync event streams through Cobra's writer so --deliver captures them.
			out := cmd.OutOrStdout()
			...
			for i := 0; i < concurrency; i++ {
				...
				go func() {
					...
					for resource := range work {
						res := syncResource(c, db, resource, sinceTS, full, maxPages, effectiveLatestOnly, userParams, out)
						results <- res
					}
				}()
			}
			...
			if humanFriendly {
				...
				fmt.Fprintf(out, `{"event":"sync_summary","total_records":%d,"resources":%d,"success":%d,"warned":%d,"errored":%d,"duration_ms":%d}`+"\n",
			...
			if errCount > 0 && !strict && criticalErrCount == 0 && successCount > 0 {
				if !humanFriendly {
					...
					fmt.Fprintf(out, `{"event":"sync_warning","reason":"exit_policy_default_changed","errored":%d,"message":"%s"}`+"\n",
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/lunch-money-pp-cli/data.db)")
...
func syncResource(c interface {
	...
	RateLimit() float64
}, db *store.Store, resource, sinceTS string, full bool, maxPages int, latestOnly bool, userParams *syncUserParams, out io.Writer) syncResult {
	started := time.Now()
	if out == nil {
		out = io.Discard
	}
	...
	if !humanFriendly {
		fmt.Fprintf(out, `{"event":"sync_start","resource":"%s"}`+"\n", resource)
	}
	...
	if effectiveSince != "" && sinceParam == "" {
		if humanFriendly {
			...
			fmt.Fprintf(out, `{"event":"sync_warning","resource":"%s","reason":"resource_not_incremental","message":"endpoint does not declare a temporal filter parameter; incremental sync has no effect for this resource"}`+"\n", resource)
		}
		effectiveSince = ""
	}
	...
	for {
		...
		// Set page size — only when the resource's list endpoint declares
		// the limit/offset parameters. Sending limit to /categories,
		// /tags, /budgets/settings, /manual_accounts, /plaid_accounts,
		// /recurring_items is rejected with HTTP 400 "Invalid query
		// parameter: 'limit'." by Lunch Money. The transactions endpoint
		// (and any other paginated list) does accept them.
		...
		if err != nil {
			if w, ok := isSyncAccessWarning(err); ok {
				if !humanFriendly {
					fmt.Fprintf(out, `{"event":"sync_warning","resource":"%s","status":%d,"reason":"%s","message":"%s"}`+"\n",
			...
			if !humanFriendly {
				fmt.Fprintln(out, syncErrorJSON(resource, "", err))
			}
		...
		if len(items) > 0 && stored == 0 {
			if humanFriendly {
				...
				fmt.Fprintf(out, `{"event":"sync_anomaly","resource":"%s","consumed":%d,"stored":0,"reason":"all_items_failed_id_extraction"}`+"\n", resource, len(items))
			}
			anomalyEmitted = true
			...
			if humanFriendly {
				...
				fmt.Fprintf(out, `{"event":"sync_anomaly","resource":"%s","consumed":%d,"stored":%d,"count":%d,"reason":"primary_key_unresolved"}`+"\n", resource, len(items), stored, extractFailures)
			}
			anomalyEmitted = true
		}
		...
		if humanFriendly {
			...
			if currentRate > 0 {
				fmt.Fprintf(out, `{"event":"sync_progress","resource":"%s","fetched":%d,"rate_rps":%.1f}`+"\n", resource, atomic.LoadInt64(&progressCount), currentRate)
			} else {
				fmt.Fprintf(out, `{"event":"sync_progress","resource":"%s","fetched":%d}`+"\n", resource, atomic.LoadInt64(&progressCount))
			}
		}
		...
		if maxPages > 0 && pagesFetched >= maxPages {
			if !latestOnly {
				if humanFriendly {
					...
					fmt.Fprintf(out, `{"event":"sync_warning","resource":"%s","reason":"max_pages_cap_hit","message":"reached --max-pages cap of %d; data may be truncated. Re-run with --max-pages 0 (unlimited) or higher to verify."}`+"\n", resource, maxPages)
				}
			}
			break
		}
		...
		if nextCursor != "" && nextCursor == lastNextCursor {
			if humanFriendly {
				...
				fmt.Fprintf(out, `{"event":"sync_warning","resource":"%s","reason":"stuck_pagination","message":"API returned the same next cursor across two pages for resource %s; aborting to prevent budget waste."}`+"\n", resource, resource)
			}
			break
		}
		...
		// Offset-based pagination: when the API uses has_more+offset (Lunch Money,
		// most paginated REST APIs without a cursor token) the response carries
		// no cursor field. Compute the next offset client-side from the running
		// position so the loop advances. Cursor-based APIs that did return a
		// cursor have nextCursor populated and reach this branch already.
		if nextCursor == "" {
	...
	if consumedTotal > 0 && totalCount == 0 && extractFailureTotal < consumedTotal {
		if humanFriendly {
			...
			fmt.Fprintf(out, `{"event":"sync_anomaly","resource":"%s","consumed":%d,"stored":0,"extract_failures":%d,"reason":"stored_count_zero_after_extraction"}`+"\n", resource, consumedTotal, extractFailureTotal)
		}
	}

	if !humanFriendly {
		fmt.Fprintf(out, `{"event":"sync_complete","resource":"%s","total":%d,"duration_ms":%d}`+"\n", resource, totalCount, time.Since(started).Milliseconds())
	}
...
func determinePaginationDefaults() paginationDefaults {
	return paginationDefaults{
		cursorParam: "offset",
		limitParam:  "limit",
		// Lunch Money v2 caps per-page reads at 1000 transactions for the
		// list endpoint; the default 100 made --full sync require 10x more
		// API round-trips than necessary against a 100/window rate budget.
		limit: 1000,
	}
}

// resourceSupportsPagination reports whether the resource's list endpoint
// accepts limit/offset query params. Lunch Money's v2 spec declares
// pagination only on /transactions; the other list endpoints reject
// limit/offset with HTTP 400 VALIDATION_ERROR. Crypto endpoints are not
// deployed yet (HTTP 404) so they don't matter — they'll be added when GA.
...
func syncResourceSinceParam(resource string) string {
	switch resource {
	case "transactions":
		// updated_since covers edits to existing rows; created_since
		// would miss every transaction whose category/payee/notes
		// changed since the last sync, breaking offline-store
		// freshness for the most common day-to-day workflow.
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"crypto-synced":    "/crypto/synced",
		"cryptocurrencies": "/cryptocurrencies",
		"manual-accounts":  "/manual_accounts",
		"plaid-accounts":   "/plaid_accounts",
