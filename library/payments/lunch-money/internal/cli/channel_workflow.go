...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  lunch-money-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  lunch-money-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"balance-history", "budgets", "categories", "crypto", "crypto-synced", "cryptocurrencies", "manual-accounts", "plaid-accounts", "recurring-items", "tags", "transactions"}
			totalSynced := 0
			...
			for _, resource := range resources {
				// PATCH: Preserve --deliver routing for workflow archive's delegated sync events.
				res := syncResource(c, s, resource, "", full, 100, false, nil, cmd.OutOrStdout())
				if res.Err != nil {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  lunch-money-pp-cli workflow status

  # Show status as JSON
  lunch-money-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("lunch-money-pp-cli")
			}
