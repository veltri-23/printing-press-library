...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Lunch Money",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  lunch-money-pp-cli auth setup\n  lunch-money-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://my.lunchmoney.app/developers")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			// PATCH(env-trim): single canonical env var.
			fmt.Fprintln(w, "  export LUNCHMONEY_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  lunch-money-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://my.lunchmoney.app/developers"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  lunch-money-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				// PATCH(env-trim): single canonical env var.
				fmt.Fprintln(w, "  export LUNCHMONEY_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  lunch-money-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  lunch-money-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  lunch-money-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			// Identify whether the auth env var is still exported so the
			// JSON envelope and the human prose can both surface it.
			// PATCH(env-trim): single canonical env var.
			envStillSet := ""
			if os.Getenv("LUNCHMONEY_API_KEY") != "" {
				envStillSet = "LUNCHMONEY_API_KEY"
			}
