...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".lunch-money-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LUNCH_MONEY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LUNCH_MONEY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lunch-money-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.lunch-money-pp-cli/feedback.jsonl.
When ` + "`LUNCH_MONEY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LUNCH_MONEY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lunch-money-pp-cli feedback list
  lunch-money-pp-cli feedback list --limit 5
  lunch-money-pp-cli feedback list --json`,
