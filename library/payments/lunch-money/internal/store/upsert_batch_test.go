...
// PATCH: A non-positive List limit means unlimited, matching resolveLocal's contract.
func TestListZeroReturnsAllRows(t *testing.T) {
	...
	for i := 0; i < 205; i++ {
		items = append(items, json.RawMessage(fmt.Sprintf(`{"id":"item-%03d","name":"Item %03d"}`, i, i)))
	}
	if _, _, err := s.UpsertBatch("widgets", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}

	got, err := s.List("widgets", 0)
	if err != nil {
		t.Fatalf("List: %v", err)
	}
	if len(got) != len(items) {
		t.Fatalf("List returned %d rows, want %d", len(got), len(items))
	}
}

// PATCH: Typed FTS search must restrict by resource type when requested.
func TestSearchResourceFiltersType(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("transactions", []json.RawMessage{
		json.RawMessage(`{"id":"tx-1","payee":"Example Brand Outlet","notes":"fashion inventory expense"}`),
	}); err != nil {
	...
	all, err := s.Search("Example Brand", 10)
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
	if len(all) != 2 {
		t.Fatalf("Search returned %d rows, want 2", len(all))
	}

	transactions, err := s.SearchResource("transactions", "Example Brand", 10)
	if err != nil {
		t.Fatalf("SearchResource: %v", err)
	}
	if len(transactions) != 1 || !strings.Contains(string(transactions[0]), "tx-1") {
		t.Fatalf("SearchResource returned %#v, want only transaction", transactions)
	}
}
