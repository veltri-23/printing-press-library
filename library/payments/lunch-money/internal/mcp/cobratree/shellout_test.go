...
// TestSplitShellArgs pins the whitespace + quoted-token splitting used by
// the args-field passthrough. Behavior we rely on: bare whitespace splits,
// tabs split, double-quoted and single-quoted spans stay together, empty
// input yields nil.
// PATCH(shell-quotes): added single-quote coverage so POSIX-style callers
// passing values like 'Bob's diner' through MCP don't get silently split.
func TestSplitShellArgs(t *testing.T) {
	cases := []struct {
		...
		{"single-quoted mixed", `foo 'bar baz' qux`, []string{"foo", "bar baz", "qux"}},
		{"double quote inside single", `'she said "hi"'`, []string{`she said "hi"`}},
		{"single quote inside double", `"Bob's diner"`, []string{"Bob's diner"}},
	}
...
// PATCH: MCP shell-out defaults keep output parseable and non-interactive
// without auto-confirming destructive operations.
func TestMCPShelloutDefaults(t *testing.T) {
	got := mcpShelloutDefaults()
	want := []string{"--json", "--no-input", "--no-color"}
	if !reflect.DeepEqual(got, want) {
		t.Fatalf("mcpShelloutDefaults = %v, want %v", got, want)
	}
	for _, tok := range got {
		if tok == "--agent" || tok == "--yes" {
			t.Fatalf("mcpShelloutDefaults included auto-confirming flag %q", tok)
		}
	}
}
