...
// PATCH: Code-orchestration execute must resolve all path placeholders, not
// only generator-selected positional args.
func TestResolveCodeOrchPathUsesAllPlaceholders(t *testing.T) {
	params := map[string]any{
		"account_type": "manual",
		"account_id":   float64(123),
		"start_date":   "2026-05-01",
	}
	path, err := resolveCodeOrchPath("/balance_history/{account_type}/{account_id}", params)
	if err != nil {
		t.Fatalf("resolveCodeOrchPath: %v", err)
	}
	if path != "/balance_history/manual/123" {
		t.Fatalf("path = %q", path)
	}
	if _, ok := params["account_type"]; ok {
		t.Fatalf("account_type was not consumed: %#v", params)
	}
	if _, ok := params["account_id"]; ok {
		t.Fatalf("account_id was not consumed: %#v", params)
	}
	if params["start_date"] != "2026-05-01" {
		t.Fatalf("non-path param was consumed: %#v", params)
	}
}

func TestResolveCodeOrchPathRequiresPlaceholders(t *testing.T) {
	if _, err := resolveCodeOrchPath("/balance_history/{account_type}/{account_id}", map[string]any{"account_id": 123}); err == nil {
		t.Fatal("resolveCodeOrchPath returned nil error, want missing account_type")
	}
}
