...
type Client struct {
	...
	DryRun     bool
	// PATCH: Let agent/json callers suppress the human dry-run preview so
	// merged stderr/stdout streams remain valid JSON.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "lunch-money-pp-cli", "http")
...
// PATCH: Some generated endpoints declare query parameters on mutating methods.
...
func (c *Client) Delete(path string, params ...map[string]string) (json.RawMessage, int, error) {
	var query map[string]string
	if len(params) > 0 {
		query = params[0]
	}
	return c.do("DELETE", path, query, nil, nil)
}

func (c *Client) DeleteWithBody(path string, params map[string]string, body any) (json.RawMessage, int, error) {
	return c.do("DELETE", path, params, body, nil)
}
...
// PATCH: Some generated endpoints declare query parameters on PUT requests.
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "lunch-money-pp-cli/2.11.0")
		}
		...
		// Server error - retry with backoff. Only retry methods that are
		// safe to repeat: GET (read), PUT (idempotent update), DELETE
		// (idempotent in REST). POST is intentionally NOT retried because
		// the server may have committed the resource before returning 5xx,
...
// 5xx. GET is read-only; PUT and DELETE are idempotent by REST convention.
// POST and PATCH are excluded because the server may have committed before
// returning the 5xx, and a retry would produce a duplicate or unexpected
// second mutation. See `// PATCH(retry-non-idempotent)` in do().
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	// PATCH: Route the preview through a configurable writer so agent/json
	// callers can discard it and keep merged streams parseable.
	w := c.DryRunOutput
	if w == nil {
		w = os.Stderr
	}
