# Lunch Money Printed CLI Agent Guide

This directory is a generated `lunch-money-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
lunch-money-pp-cli doctor --json
lunch-money-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
lunch-money-pp-cli which "<capability>" --json
lunch-money-pp-cli <command> --help
```
...
```bash
lunch-money-pp-cli <command> --agent
```
...
```bash
lunch-money-pp-cli <command> --help
lunch-money-pp-cli <command> --dry-run --agent
```
