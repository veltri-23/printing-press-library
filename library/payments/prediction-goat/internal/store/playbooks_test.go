...
func openPlaybooksTestStore(t *testing.T) *Store {
	t.Helper()
...
func TestUpsertPlaybook_Happy(t *testing.T) {
	t.Parallel()
	s := openPlaybooksTestStore(t)

	id, inserted, err := s.UpsertPlaybook(UpsertPlaybookInput{
		QueryFamily:  "odds team season",
		PlaybookJSON: `{"steps":[{"cmd":"polymarket events list --series {series.id}"}]}`,
		NotesText:    "kalshi events get needs --with-markets for child markets",
	})
	if err != nil {
	...
	row, ok, err := s.GetPlaybookByFamily("odds team season")
	if err != nil || !ok {
		t.Fatalf("get: err=%v ok=%v", err, ok)
	}
	if !strings.Contains(row.PlaybookJSON, "polymarket events") {
		t.Errorf("playbook content mismatch: %q", row.PlaybookJSON)
	}
	if !strings.Contains(row.NotesText, "with-markets") {
...
func TestUpsertPlaybook_NotesOnly(t *testing.T) {
	...
	_, _, err := s.UpsertPlaybook(UpsertPlaybookInput{
		...
		NotesText:   "remember the with-markets flag",
	})
	if err != nil {
...
func TestUpsertPlaybook_PartialUpdate(t *testing.T) {
	t.Parallel()
	s := openPlaybooksTestStore(t)
...
func TestUpsertPlaybook_PreserveExistingNotes_Seed(t *testing.T) {
	t.Parallel()
	s := openPlaybooksTestStore(t)
...
func TestAppendPlaybookNotes_InsertsWhenAbsent(t *testing.T) {
	t.Parallel()
	s := openPlaybooksTestStore(t)
	id, inserted, err := s.AppendPlaybookNotes("odds nba finals", "\n\n[amend 2026-05-25T03:14Z]: first")
	if err != nil {
	...
	row, ok, err := s.GetPlaybookByFamily("odds nba finals")
	if err != nil || !ok {
