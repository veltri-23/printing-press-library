...
In local mode: searches locally synced data only.`,
  ...
  prediction-goat-pp-cli search "error timeout"

  # Force local search only
  prediction-goat-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  prediction-goat-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  prediction-goat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /public-search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/public-search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'prediction-goat-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "markets":
				results, err = db.SearchMarkets(query, limit)
			case "series":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchMarkets(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search markets failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
