...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".prediction-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PREDICTION_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PREDICTION_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "prediction-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.prediction-goat-pp-cli/feedback.jsonl.
When ` + "`PREDICTION_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PREDICTION_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  prediction-goat-pp-cli feedback list
  prediction-goat-pp-cli feedback list --limit 5
  prediction-goat-pp-cli feedback list --json`,
