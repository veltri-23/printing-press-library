...
In local mode: searches locally synced data only.`,
  ...
  revenuecat-pp-cli search "error timeout"

  # Force local search only
  revenuecat-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  revenuecat-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  revenuecat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /projects/{project_id}/purchases
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/projects/{project_id}/purchases", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'revenuecat-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "charts":
				results, err = db.SearchCharts(query, limit)
			case "virtual-currencies":
				results, err = db.SearchVirtualCurrencies(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCharts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search charts failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchVirtualCurrencies(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search virtual_currencies failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
