...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "revenuecat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("REVENUECAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("REVENUECAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "revenuecat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/revenuecat-pp-cli/feedback.jsonl.
When ` + "`REVENUECAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`REVENUECAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  revenuecat-pp-cli feedback list
  revenuecat-pp-cli feedback list --limit 5
  revenuecat-pp-cli feedback list --json`,
