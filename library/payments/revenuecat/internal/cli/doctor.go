...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  revenuecat-pp-cli doctor
  revenuecat-pp-cli doctor --json
  revenuecat-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export REVENUECAT_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "REVENUECAT_API_KEY")
				if os.Getenv("REVENUECAT_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "REVENUECAT_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("REVENUECAT_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "REVENUECAT_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "REVENUECAT_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "DEVELOPER_BEARER_AUTH")
				if os.Getenv("DEVELOPER_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "DEVELOPER_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("DEVELOPER_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "DEVELOPER_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "DEVELOPER_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "revenuecat-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'revenuecat-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'revenuecat-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'revenuecat-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'revenuecat-pp-cli sync' to refresh."
	}
	return report
}
