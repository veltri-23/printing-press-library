...
type Config struct {
	...
	Path                string            `toml:"-"`
	RevenuecatApiKey    string            `toml:"api_key"`
	DeveloperBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("REVENUECAT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "revenuecat-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("REVENUECAT_API_KEY"); v != "" {
		cfg.RevenuecatApiKey = v
		cfg.AuthSource = "env:REVENUECAT_API_KEY"
	}
	if v := os.Getenv("DEVELOPER_BEARER_AUTH"); v != "" {
		cfg.DeveloperBearerAuth = v
		cfg.AuthSource = "env:DEVELOPER_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.RevenuecatApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.DeveloperBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("REVENUECAT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.RevenuecatApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:REVENUECAT_API_KEY"
		}
		return "Bearer " + c.RevenuecatApiKey
	}
	if c.DeveloperBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:DEVELOPER_BEARER_AUTH"
		}
		return "Bearer " + c.DeveloperBearerAuth
	}
	if c.AccessToken != "" {
