...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("find_entitlement_desync",
			mcplib.WithDescription("Surfaces customers whose entitlement state disagrees with their subscription state across the whole base."),
			mcplib.WithBoolean("flag_disagreements", mcplib.Description("Override the recipe's --flag-disagreements value.")),
		),
		handleFindEntitlementDesync,
	)
}

// handleFindEntitlementDesync runs the find_entitlement_desync recipe intent: Surfaces customers whose entitlement state disagrees with their subscription state across the whole base.
func handleFindEntitlementDesync(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "flag-disagreements", input["flag_disagreements"], true)
