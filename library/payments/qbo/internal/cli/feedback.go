...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "qbo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("QBO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("QBO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "qbo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/qbo-pp-cli/feedback.jsonl.
When ` + "`QBO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`QBO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  qbo-pp-cli feedback list
  qbo-pp-cli feedback list --limit 5
  qbo-pp-cli feedback list --json`,
