...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  qbo-pp-cli doctor
  qbo-pp-cli doctor --json
  qbo-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export QBO_CLIENT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("QBO_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "QBO_CLIENT_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "QBO_CLIENT_ID")
			}
			if os.Getenv("QBO_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "QBO_CLIENT_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "QBO_CLIENT_SECRET")
			}
			if os.Getenv("QBO_REALM_ID") != "" {
				authEnvSet = append(authEnvSet, "QBO_REALM_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "QBO_REALM_ID")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "qbo-pp-cli", suggestion)
						} else {
