# Qbo Printed CLI Agent Guide

This directory is a generated `qbo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
qbo-pp-cli doctor --json
qbo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
qbo-pp-cli which "<capability>" --json
qbo-pp-cli <command> --help
```
...
```bash
qbo-pp-cli <command> --help
qbo-pp-cli <command> --dry-run --agent
```
