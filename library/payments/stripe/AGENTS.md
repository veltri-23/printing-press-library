# Stripe Printed CLI Agent Guide

This directory is a generated `stripe-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
stripe-pp-cli doctor --json
stripe-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
stripe-pp-cli which "<capability>" --json
...
```bash
stripe-pp-cli <command> --help
stripe-pp-cli <command> --dry-run --agent
```
