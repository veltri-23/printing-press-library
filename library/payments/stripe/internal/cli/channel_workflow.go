...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  stripe-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  stripe-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "apple-pay", "application-fees", "balance-transactions", "billing", "billing-credit-balance-transactions", "billing-credit-grants", "billing-meters", "billing-portal", "charges", "checkout", "climate", "climate-products", "climate-suppliers", "country-specs", "coupons", "credit-notes", "customers", "disputes", "entitlements", "events", "exchange-rates", "file-links", "files", "financial-connections", "forwarding", "identity", "identity-verification-sessions", "invoice-payments", "invoice-rendering-templates", "invoiceitems", "invoices", "issuing", "issuing-authorizations", "issuing-cardholders", "issuing-disputes", "issuing-personalization-designs", "issuing-physical-bundles", "issuing-transactions", "payment-intents", "payment-links", "payment-method-configurations", "payment-method-domains", "payment-methods", "payouts", "plans", "prices", "products", "promotion-codes", "quotes", "radar", "radar-early-fraud-warnings", "refunds", "reporting", "reporting-report-types", "reviews", "setup-intents", "shipping-rates", "sigma", "subscription-schedules", "subscriptions", "tax", "tax-codes", "tax-ids", "tax-rates", "terminal", "terminal-configurations", "terminal-locations", "test-helpers", "topups", "transfers", "treasury", "webhook-endpoints"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  stripe-pp-cli workflow status

  # Show status as JSON
  stripe-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("stripe-pp-cli")
			}
