...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".stripe-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("STRIPE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("STRIPE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "stripe-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.stripe-pp-cli/feedback.jsonl.
When ` + "`STRIPE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`STRIPE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  stripe-pp-cli feedback list
  stripe-pp-cli feedback list --limit 5
  stripe-pp-cli feedback list --json`,
