...
func RegisterAll(s *server.MCPServer, root *cobra.Command, cliPath func() (string, error)) {
	...
	walk(root, nil, func(cmd *cobra.Command, path []string) {
		...
		} else if annotationValue(cmd, ReadOnlyAnnotation) == "false" {
			options = append(options, mcplib.WithReadOnlyHintAnnotation(false))
		}
		if risk := annotationValue(cmd, RiskAnnotation); risk != "" {
			options = append(options, withRiskMeta(risk, isMCPReadOnly(cmd)))
		}
