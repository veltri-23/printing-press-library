...
// TestClient_WriteBarrier_NoEnv pins the PP-side operator path: with no
// verify env vars and no ROBINHOOD_PP_ALLOW_WRITES=1, mutating verbs fail
// closed before dialing.
func TestClient_WriteBarrier_NoEnv(t *testing.T) {
	// Explicitly unset to defend against test-runner inherited values.
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("ROBINHOOD_PP_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)

	_, _, err := c.do(context.Background(), "DELETE", "/test", nil, nil, nil)

	if err == nil {
		t.Fatalf("mutating verb without ROBINHOOD_PP_ALLOW_WRITES=1 should fail")
	}
	if rec.calls != 0 {
...
// TestClient_WriteBarrier_AllowEnvDials pins the live approval escape hatch.
func TestClient_WriteBarrier_AllowEnvDials(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("ROBINHOOD_PP_ALLOW_WRITES", "1")
	c, rec := newClientWithRecorder(t)

	_, _, _ = c.do(context.Background(), "DELETE", "/test", nil, nil, nil)

	if rec.calls < 1 {
		t.Fatalf("ROBINHOOD_PP_ALLOW_WRITES=1 should allow dial; recorder saw %d calls", rec.calls)
	}
}
