...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "robinhood-pp-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if headers, err := c.authHeaders(req.Context(), req.Method, req.URL.RequestURI(), nil); err == nil {
...
func (c *Client) writeCache(path string, params map[string]string, data json.RawMessage) {
	...
	// PATCH(chmod-existing): MkdirAll/WriteFile only apply perm on creation;
	// re-secure a cache dir/file that may survive from an older 0o755/0o644 build.
...
func liveWriteBlockedError(method, path string) error {
	return fmt.Errorf("live Robinhood write blocked for %s %s: rerun with --dry-run to preview, or export %s=1 only after explicit approval", method, path, AllowWritesEnv)
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	if c.DryRun {
		return c.dryRun(ctx, method, targetURL, path, params, bodyBytes, headerOverrides, authHeader)
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeaders, err := c.authHeaders(ctx, method, req.URL.RequestURI(), bodyBytes); err != nil {
			return nil, 0, err
		} else {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "robinhood-pp-cli/")
		}
...
func (c *Client) dryRun(ctx context.Context, method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	fmt.Fprintf(os.Stderr, "%s %s\n", method, targetURL)
	signedPath := pathWithParams(path, params)
	queryPrinted := false
	if params != nil {
	...
	if authHeader != "" {
		...
		if signedHeaders, err := c.authHeaders(ctx, method, signedPath, body); err == nil {
			if ts := signedHeaders["x-timestamp"]; ts != "" {
				fmt.Fprintf(os.Stderr, "  %s: %s\n", "x-timestamp", ts)
			}
			if sig := signedHeaders["x-signature"]; sig != "" {
				fmt.Fprintf(os.Stderr, "  %s: %s\n", "x-signature", maskToken(sig))
			}
		}
	}
...
func (c *Client) authHeaders(ctx context.Context, method, path string, body []byte) (map[string]string, error) {
	authHeader, err := c.authHeader(ctx)
	if err != nil {
		return nil, err
	}
	...
	headers := map[string]string{"x-api-key": authHeader}
	privateKey := ""
	if c != nil && c.Config != nil {
		privateKey = c.Config.PrivateKeyBase64()
	}
	if privateKey == "" {
		if !cliutil.IsVerifyEnv() {
			return nil, fmt.Errorf("ROBINHOOD_PRIVATE_KEY_B64 is required for Robinhood Crypto signed requests")
		}
	...
	signature, err := signRobinhoodRequest(privateKey, authHeader, timestamp, path, method, string(body))
	if err != nil {
		if cliutil.IsVerifyEnv() {
			headers["x-timestamp"] = "0"
			headers["x-signature"] = "mock-signature"
...
func signRobinhoodRequest(privateKeyBase64, apiKey, timestamp, path, method, body string) (string, error) {
	keyBytes, err := base64.StdEncoding.DecodeString(privateKeyBase64)
	if err != nil {
		return "", fmt.Errorf("decoding ROBINHOOD_PRIVATE_KEY_B64: %w", err)
	}
	switch len(keyBytes) {
	...
	default:
		return "", fmt.Errorf("decoding ROBINHOOD_PRIVATE_KEY_B64: expected %d-byte seed or %d-byte private key, got %d bytes", ed25519.SeedSize, ed25519.PrivateKeySize, len(keyBytes))
	}
	message := apiKey + timestamp + path + strings.ToUpper(method) + body
	signature := ed25519.Sign(ed25519.PrivateKey(keyBytes), []byte(message))
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export ROBINHOOD_API_KEY=<your-token> or robinhood-pp-cli auth set-token <token>")
}
