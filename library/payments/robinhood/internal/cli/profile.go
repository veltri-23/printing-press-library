...
func profileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".robinhood-pp-cli")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return "", fmt.Errorf("creating state dir: %w", err)
	}
	// PATCH(chmod-existing): MkdirAll only sets perm on creation; re-secure an
	// existing state dir that may have been created world-readable previously.
...
func saveProfileStore(s *profileStore) error {
	...
	// PATCH(chmod-existing): the renamed file keeps the tmp's mode (and an
	// existing profiles.json keeps its old mode if Rename overwrites in place);
	// re-secure the owner-only profile store holding credentials.
	_ = os.Chmod(p, 0o600)
	return nil
}
...
present (other than --profile and --config).`,
		Example: `  robinhood-pp-cli profile save my-defaults --json --compact
  robinhood-pp-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  robinhood-pp-cli profile use my-defaults
  robinhood-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  robinhood-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  robinhood-pp-cli profile show my-defaults
  robinhood-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  robinhood-pp-cli profile delete my-defaults --yes
  robinhood-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
