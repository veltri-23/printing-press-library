...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Robinhood Crypto Trading",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  robinhood-pp-cli auth setup\n  robinhood-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://robinhood.com/account/crypto")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export ROBINHOOD_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  export ROBINHOOD_PRIVATE_KEY_B64=\"<your-private-key-base64>\"")
			fmt.Fprintln(w, "  robinhood-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://robinhood.com/account/crypto"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  robinhood-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set your API key and private signing key:")
				fmt.Fprintln(w, "  export ROBINHOOD_API_KEY=\"your-token-here\"")
				fmt.Fprintln(w, "  export ROBINHOOD_PRIVATE_KEY_B64=\"your-private-key-base64\"")
				fmt.Fprintln(w, "  robinhood-pp-cli auth set-token <token>")
				return authErr(fmt.Errorf("no credentials configured"))
			}
			...
			fmt.Fprintln(w, "  Signing key: present")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  robinhood-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  robinhood-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("ROBINHOOD_API_KEY") != "" {
				envStillSet = "ROBINHOOD_API_KEY"
			}
