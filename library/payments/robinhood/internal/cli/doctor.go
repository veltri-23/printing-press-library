...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  robinhood-pp-cli doctor
  robinhood-pp-cli doctor --json
  robinhood-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				signingKey := cfg.PrivateKeyBase64()
				switch {
				case header == "" && signingKey == "":
					report["auth"] = "not configured"
					report["auth_hint"] = "export ROBINHOOD_API_KEY=<your-key> and ROBINHOOD_PRIVATE_KEY_B64=<your-private-key-base64>"
					report["auth_key_url"] = "https://robinhood.com/account/crypto"
				case header == "":
					report["auth"] = "missing API key"
					report["auth_hint"] = "export ROBINHOOD_API_KEY=<your-key>"
					report["auth_key_url"] = "https://robinhood.com/account/crypto"
				case signingKey == "":
					report["auth"] = "missing private signing key"
					report["auth_hint"] = "export ROBINHOOD_PRIVATE_KEY_B64=<your-private-key-base64>"
					report["auth_key_url"] = "https://robinhood.com/account/crypto"
				default:
					authConfigured = true
			...
			if os.Getenv("ROBINHOOD_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "ROBINHOOD_API_KEY")
			} else if cfg != nil && cfg.AuthHeader() != "" {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				}
				authEnvInfo = append(authEnvInfo, "ROBINHOOD_API_KEY available from "+authSource)
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ROBINHOOD_API_KEY")
			}
			if os.Getenv("ROBINHOOD_PRIVATE_KEY_B64") != "" {
				authEnvSet = append(authEnvSet, "ROBINHOOD_PRIVATE_KEY_B64")
			} else if cfg != nil && cfg.PrivateKeyBase64() != "" {
				authEnvInfo = append(authEnvInfo, "ROBINHOOD_PRIVATE_KEY_B64 available from config")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ROBINHOOD_PRIVATE_KEY_B64")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "robinhood-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("robinhood-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'robinhood-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'robinhood-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'robinhood-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'robinhood-pp-cli sync' to refresh."
	}
	return report
}
