...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROBINHOOD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROBINHOOD_FEEDBACK_AUTO_SEND")))
...
func appendFeedback(entry FeedbackEntry) error {
	...
	defer f.Close()
	// PATCH(chmod-existing): O_CREATE only applies perm when creating the file;
	// re-secure an existing 0o644 feedback ledger that may contain account context.
	_ = os.Chmod(p, 0o600)
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "robinhood-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.robinhood-pp-cli/feedback.jsonl.
When ` + "`ROBINHOOD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROBINHOOD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  robinhood-pp-cli feedback list
  robinhood-pp-cli feedback list --limit 5
  robinhood-pp-cli feedback list --json`,
