...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  robinhood-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  robinhood-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"crypto", "crypto-marketdata-best-bid-ask", "crypto-trading-accounts", "crypto-trading-holdings", "crypto-trading-trading-pairs"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  robinhood-pp-cli workflow status

  # Show status as JSON
  robinhood-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("robinhood-pp-cli")
			}
