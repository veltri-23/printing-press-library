...
Exit codes & warnings:
  ...
  coingecko-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  coingecko-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  coingecko-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  coingecko-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  coingecko-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "market_chart":
		return db.UpsertMarketChart(data)
	case "ohlc":
		return db.UpsertOhlc(data)
	case "simple":
		return db.UpsertSimple(data)
	default:
