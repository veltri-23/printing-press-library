...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  coingecko-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  coingecko-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"coingecko-search-2", "coins", "global", "ping", "simple"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  coingecko-pp-cli workflow status

  # Show status as JSON
  coingecko-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("coingecko-pp-cli")
			}
