...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".coingecko-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COINGECKO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COINGECKO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "coingecko-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.coingecko-pp-cli/feedback.jsonl.
When ` + "`COINGECKO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COINGECKO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
