...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".mercury-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MERCURY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MERCURY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mercury-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.mercury-pp-cli/feedback.jsonl.
When ` + "`MERCURY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MERCURY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mercury-pp-cli feedback list
  mercury-pp-cli feedback list --limit 5
  mercury-pp-cli feedback list --json`,
