...
Exit codes & warnings:
  ...
  mercury-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  mercury-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  mercury-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  mercury-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  mercury-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "request_send_money":
		return db.UpsertRequestSendMoney(data)
	case "statements":
		return db.UpsertStatements(data)
	case "transaction":
		return db.UpsertTransaction(data)
	case "transactions":
		return db.UpsertTransactions(data)
	case "ar":
		return db.UpsertAr(data)
	case "books":
		return db.UpsertBooks(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "freeze":
		return db.UpsertFreeze(data)
	case "unfreeze":
		return db.UpsertUnfreeze(data)
	case "categories":
	...
	case "recipient":
		return db.UpsertRecipient(data)
	case "attachments":
		return db.UpsertAttachments(data)
	case "recipients":
		return db.UpsertRecipients(data)
	case "safes":
		return db.UpsertSafes(data)
	case "document":
		return db.UpsertDocument(data)
	case "pdf":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"accounts":           "/accounts",
		"ar":                 "/ar/invoices",
		"books":              "/books/agent-coa-templates",
		"cards":              "/cards",
		...
		"request-send-money": "/request-send-money",
		"safes":              "/safes",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "books", ParentTable: "books", ParentIDParam: "booksId", PathTemplate: "/books/journal-entries/{booksId}"},
		{Name: "statements", ParentTable: "treasury", ParentIDParam: "treasuryId", PathTemplate: "/treasury/{treasuryId}/statements"},
		{Name: "transactions", ParentTable: "treasury", ParentIDParam: "treasuryId", PathTemplate: "/treasury/{treasuryId}/transactions"},
	}
}
