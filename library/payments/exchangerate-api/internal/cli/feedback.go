...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".exchangerate-api-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXCHANGERATE_API_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXCHANGERATE_API_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "exchangerate-api-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.exchangerate-api-pp-cli/feedback.jsonl.
When ` + "`EXCHANGERATE_API_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXCHANGERATE_API_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  exchangerate-api-pp-cli feedback list
  exchangerate-api-pp-cli feedback list --limit 5
  exchangerate-api-pp-cli feedback list --json`,
