...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  exchangerate-api-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  exchangerate-api-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  exchangerate-api-pp-cli workflow status

  # Show status as JSON
  exchangerate-api-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("exchangerate-api-pp-cli")
			}
