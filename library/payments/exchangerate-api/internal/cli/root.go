...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "exchangerate-api-pp-cli",
		Short: `Every ExchangeRate-API endpoint, plus a local rate history, watchlists, drift alerts, and an MCP server.`,
		Long: `Every ExchangeRate-API endpoint, plus a local rate history, watchlists, drift alerts, and an MCP server.
...
Health check: run 'exchangerate-api-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("exchangerate-api-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'exchangerate-api-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCodesPromotedCmd(flags))
	quotaCmd := newQuotaPromotedCmd(flags)
	quotaCmd.AddCommand(newQuotaBurnCmd(flags))
	rootCmd.AddCommand(quotaCmd)
	// Novel transcendence commands (hand-built; see absorb manifest)
	rootCmd.AddCommand(newConvertCmd(flags))
	rootCmd.AddCommand(newConvertBatchCmd(flags))
	rootCmd.AddCommand(newMatrixCmd(flags))
	rootCmd.AddCommand(newOpenCmd(flags))
	rootCmd.AddCommand(newPlanCheckCmd(flags))
	rootCmd.AddCommand(newSyncRatesCmd(flags))
	rootCmd.AddCommand(newHistoryCacheCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("exchangerate-api-pp-cli %s\n", version)
		},
	}
}
