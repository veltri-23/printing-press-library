...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "exchangerate-api-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// ExchangeRate-API authenticates via a path segment (/v6/{key}/...) which
		// each command builds explicitly from c.Config.ExchangerateApiKey. The
		// spec declared a query-string apikey_placeholder param to wire the env
		// var through the generated config; the live API ignores extra query
		// params but emitting them clutters logs and exposes the key. Skip it.
		_ = authHeader
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "exchangerate-api-pp-cli/0.1.0")
		}
		...
		if err != nil {
			// Transport errors (DNS, timeout, TLS, refused) carry the full URL
			// in both our outer wrapping context AND inside the Go *url.Error
			// nested via %w. Mask BOTH:
			//   - the path we render ourselves
			//   - the inner error's message string (Go's url.Error formats the
			//     full URL it tried to reach, which contains the credential
			//     for path-key APIs like ExchangeRate-API)
			// We drop the %w chain in favor of %s with the masked text — these
			// are transport errors that nothing downstream errors.As on, so the
			// chain isn't load-bearing. classifyAPIError checks for *APIError,
			// which is constructed only on the HTTP-response path below.
			mask := func(s string) string {
				if c.Config != nil && c.Config.ExchangerateApiKey != "" {
					return strings.ReplaceAll(s, c.Config.ExchangerateApiKey, maskToken(c.Config.ExchangerateApiKey))
				}
				return s
			}
			lastErr = fmt.Errorf("%s %s: %s", method, mask(path), mask(err.Error()))
			continue
		}
		...
		// ExchangeRate-API embeds the API key in the URL path. Storing the
		// raw key on APIError would propagate it into every error string and
		// log line; redact at construction so consumers can never see it.
		errPath := path
		if c.Config != nil && c.Config.ExchangerateApiKey != "" {
			errPath = strings.ReplaceAll(errPath, c.Config.ExchangerateApiKey, maskToken(c.Config.ExchangerateApiKey))
		}
		apiErr := &APIError{
			Method:     method,
			Path:       errPath,
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	// ExchangeRate-API embeds the API key as a path segment: /v6/{key}/.... The
	// upstream template only masks the query-string preview below, which would
	// leak the credential in the URL line. Redact the path occurrence too.
	displayURL := targetURL
	if c.Config != nil && c.Config.ExchangerateApiKey != "" {
		displayURL = strings.ReplaceAll(displayURL, c.Config.ExchangerateApiKey, maskToken(c.Config.ExchangerateApiKey))
	}
	...
	// The spec declared a query-string apikey_placeholder param for auth wiring
	// (the real auth lives in the path), but it is harmless garbage and noisy
	// on dry-run output. Suppress it.
	_ = authHeader
	_ = queryPrinted
	if body != nil {
