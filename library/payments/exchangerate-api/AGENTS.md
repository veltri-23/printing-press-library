# ExchangeRate-API Printed CLI Agent Guide

This directory is a generated `exchangerate-api-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
exchangerate-api-pp-cli doctor --json
exchangerate-api-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
exchangerate-api-pp-cli which "<capability>" --json
...
```bash
exchangerate-api-pp-cli <command> --agent
```
...
```bash
exchangerate-api-pp-cli <command> --help
exchangerate-api-pp-cli <command> --dry-run --agent
```
