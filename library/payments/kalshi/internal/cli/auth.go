...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage KALSHI_API_KEY credentials",
	}
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export KALSHI_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  kalshi-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  kalshi-pp-cli auth set-token sk_live_abc123",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fmt.Fprintln(cmd.OutOrStdout(), "Note: KALSHI_PRIVATE_KEY_PATH=~/.kalshi/private_key.pem is required alongside the API key.")
			return nil
		},
	}
}
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("KALSHI_API_KEY") != "" {
				envStillSet = "KALSHI_API_KEY"
			}
