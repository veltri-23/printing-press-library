...
Exit codes & warnings:
  ...
  kalshi-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  kalshi-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  kalshi-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  kalshi-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  kalshi-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&since, "since", "", "Incremental sync threshold: duration (e.g. 7d, 24h, 1w, 30m) or absolute date (e.g. 2026-04-01)")
...
func syncResource(c interface {
	...
	for {
		...
		// Set since filter (converted to the wire format — Unix seconds)
		if wire := sinceParamValue(effectiveSince); wire != "" {
			params[sinceParam] = wire
		}
...
// Kalshi's endpoints take min_ts (Unix SECONDS), not a generic "since" — the old
// param was silently ignored upstream, so every "incremental" sync re-fetched
// the full history (audit 2026-06-09). The RFC3339 value from sync_state is
// converted in sinceParamValue.
func determineSinceParam() string {
	return "min_ts"
}

// sinceParamValue converts a since timestamp (RFC3339 from sync_state, or a
// user-passed value) into the wire format for determineSinceParam. Accepts
// raw Unix seconds and YYYY-MM-DD as conveniences; returns "" (= no filter,
// full fetch) when the value can't be parsed — better a slow correct sync
// than a silently-ignored filter.
func sinceParamValue(v string) string {
	if v == "" {
		return ""
	}
	if _, err := strconv.ParseInt(v, 10, 64); err == nil {
		return v // already Unix seconds
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "structured_targets":
		return db.UpsertStructuredTargets(data)
	case "events":
		return db.UpsertEvents(data)
	case "metadata":
		return db.UpsertMetadata(data)
	case "markets":
		return db.UpsertMarkets(data)
	case "orderbook":
		return db.UpsertOrderbook(data)
	case "communications":
		return db.UpsertCommunications(data)
	case "fcm":
		return db.UpsertFcm(data)
	case "incentive_programs":
		return db.UpsertIncentivePrograms(data)
	case "milestones":
		return db.UpsertMilestones(data)
	case "series":
		return db.UpsertSeries(data)
	case "historical":
		return db.UpsertHistorical(data)
	case "multivariate_event_collections":
		return db.UpsertMultivariateEventCollections(data)
	case "lookup":
		return db.UpsertLookup(data)
	case "live_data":
		return db.UpsertLiveData(data)
	case "milestone":
		return db.UpsertMilestone(data)
	case "portfolio":
		return db.UpsertPortfolio(data)
	case "api_keys":
...
// plus absolute "YYYY-MM-DD" dates (UTC midnight), matching the --since
// semantics of portfolio attribution/winrate so every --since flag accepts
// both shapes. (polish 2026-06-09)
func parseSinceDuration(s string) (time.Time, error) {
	...
	if matches == nil {
		return time.Time{}, fmt.Errorf("expected a duration like 7d, 24h, 1w, or 30m, or a date like 2026-04-01")
	}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"events":                         "/events",
		"exchange":                       "/exchange/schedule",
		"fcm":                            "/fcm/orders",
		"historical":                     "/historical/fills",
		"incentive-programs":             "/incentive_programs",
		"kalshi-trade-manual-search":     "/search/filters_by_sport",
		"kalshi-trade-manual-search-2":   "/search/tags_by_categories",
		"markets":                        "/markets",
		...
		// PATCH (audit 2026-06-09): positions were never synced, so portfolio
		// exposure/calendar/stale always reported an empty book — a false
		// flat-book signal if used as a risk cross-check.
