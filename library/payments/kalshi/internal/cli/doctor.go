...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kalshi-pp-cli doctor
  kalshi-pp-cli doctor --json
  kalshi-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth. Kalshi auth is two-part: the access key id
			// (KALSHI_API_KEY) plus the RSA private key
			// (KALSHI_PRIVATE_KEY_PATH / KALSHI_PRIVATE_KEY) that signs every
			// request. The old hint named KALSHI_TRADE_MANUAL_KALSHI_ACCESS_KEY,
			// which nothing ever read (audit 2026-06-09).
			if cfg != nil {
				switch {
				case cfg.HasAuth():
				...
				case cfg.AuthHeader() != "":
					report["auth"] = "incomplete: access key id set but no private key loaded"
					report["auth_hint"] = "export KALSHI_PRIVATE_KEY_PATH=<path-to.pem> (or KALSHI_PRIVATE_KEY)"
				default:
					report["auth"] = "not configured"
					report["auth_hint"] = "export KALSHI_API_KEY=<access-key-id> KALSHI_PRIVATE_KEY_PATH=<path-to.pem>"
				}
			}

			// Check auth environment variables (the ones config.Load reads)
			authEnvChecked := 0
			authEnvSet := 0
			for _, v := range []string{"KALSHI_API_KEY", "KALSHI_PRIVATE_KEY_PATH", "KALSHI_PRIVATE_KEY"} {
				authEnvChecked++
				if os.Getenv(v) != "" {
					authEnvSet++
				}
			...
			if authEnvSet == 0 {
				report["env_vars"] = fmt.Sprintf("none set (checked %d; config.toml may still provide auth)", authEnvChecked)
			} else {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Step 2: Validate credentials with a SIGNED authenticated
					// probe against a real portfolio endpoint. The old probe
					// sent a bare KALSHI-ACCESS-KEY header (no RSA-PSS
					// signature) at "/", so it could never reproduce — or
					// clear — the INCORRECT_API_KEY_SIGNATURE failure class
					// (key-id/.pem mismatch), and always reported
					// "inconclusive" (audit 2026-06-09). c.Get signs the
					// request exactly like every real command.
					if !cfg.HasAuth() {
						if cfg.AuthHeader() != "" {
							report["credentials"] = "skipped (private key not loaded — cannot sign; see auth_hint)"
						}
						// else: no auth configured at all — nothing to validate
					} else if reachErr != nil && !errors.As(reachErr, &reachAPIErr) {
						report["credentials"] = "skipped (API unreachable)"
					} else {
						verifyPath := "/portfolio/balance"
						_, authErr := c.Get(verifyPath, nil)
						var authAPIErr *client.APIError
						switch {
						case authErr == nil:
							report["credentials"] = "valid (signed GET /portfolio/balance succeeded)"
						case errors.As(authErr, &authAPIErr):
							body := authAPIErr.Body
							switch {
							case authAPIErr.StatusCode == 401 && strings.Contains(body, "INCORRECT_API_KEY_SIGNATURE"):
								report["credentials"] = "INVALID: signature rejected — the private key does not match the access key id (key-id/.pem mismatch). Check which .pem KALSHI_PRIVATE_KEY_PATH points at."
							case authAPIErr.StatusCode == 401:
								report["credentials"] = fmt.Sprintf("invalid (HTTP 401 from %s) — check KALSHI_API_KEY: %s", verifyPath, truncate(body, 120))
							case authAPIErr.StatusCode == 403:
								report["credentials"] = "valid signature, but this key lacks portfolio read access (HTTP 403)"
							default:
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'kalshi-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'kalshi-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'kalshi-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'kalshi-pp-cli sync' to refresh."
	}
	return report
}
