...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".kalshi-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("KALSHI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("KALSHI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "kalshi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.kalshi-pp-cli/feedback.jsonl.
When ` + "`KALSHI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`KALSHI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  kalshi-pp-cli feedback list
  kalshi-pp-cli feedback list --limit 5
  kalshi-pp-cli feedback list --json`,
