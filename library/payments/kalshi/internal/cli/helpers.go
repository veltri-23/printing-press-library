...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 409"):
		...
		return apiErr(err)
	// Auth hints below name the env vars config.Load actually reads
	// (KALSHI_API_KEY + KALSHI_PRIVATE_KEY_PATH / KALSHI_PRIVATE_KEY).
	// The generated KALSHI_TRADE_MANUAL_KALSHI_ACCESS_KEY was never read by
	// anything — every hint was a dead end (audit 2026-06-09).
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your access key id + private key: export KALSHI_API_KEY=<access-key-id> KALSHI_PRIVATE_KEY_PATH=<path-to.pem>"+
			"\n      Run 'kalshi-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your credentials."+
			...
			"\n      INCORRECT_API_KEY_SIGNATURE in the response = the private key doesn't match the access key id."+
			"\n      Run 'kalshi-pp-cli doctor' to check auth status.", err))
...
func paginatedGet(c interface {
	...
	// Fetch all pages.
	//
	// PATCH (audit 2026-06-09): the original loop only knew generic envelope
	// keys (data/items/results/...) — never Kalshi's resource-named envelopes
	// ({"fills":[...],"cursor":"..."}) — and every generated call site passes
	// an empty nextCursorPath, so cursor-following was dead code. The result:
	// --all returned `null` (zero items, exit 0) on every paginated read.
	// Items now extract via extractEnvelopeItems (explicit keys + single-
	// array-key fallback) and the cursor defaults to Kalshi's "cursor".
	if cursorParam == "" {
		cursorParam = "cursor"
	}
	if nextCursorPath == "" {
	...
	page := 0
	prevToken := ""
	const maxPages = 1000 // runaway-loop guard
	for {
		page++
		if humanFriendly {
		...
		// Bare array response: nothing to paginate.
		...
		if json.Unmarshal(data, &obj) != nil {
			// Not an array, not an object — pass through what we got.
			if page == 1 {
				return data, nil
			}
			break
		...
		nested, found := extractEnvelopeItems(obj)
		if !found {
			// Unknown envelope shape: returning the raw first page is safer
			// than fabricating an empty array out of a response we couldn't
			// read (the pre-patch behavior this replaces).
			if page == 1 {
				return data, nil
			}
			break
		...
		// Follow the cursor. Empty/missing/repeated token = last page.
		token := ""
		if tokenRaw, ok := obj[nextCursorPath]; ok {
			_ = json.Unmarshal(tokenRaw, &token)
		}
		if token != "" && token != prevToken && page < maxPages {
			prevToken = token
			clean[cursorParam] = token
			continue
		}

		// has_more fallback for APIs that signal continuation without a cursor.
		if hasMoreField != "" {
			if moreRaw, ok := obj[hasMoreField]; ok {
				var more bool
				if json.Unmarshal(moreRaw, &more) == nil && more && page < maxPages {
					continue
				}
			}
		}
	...
	if allItems == nil {
		allItems = []json.RawMessage{} // marshal to [], never null
	}
	result, _ := json.Marshal(allItems)
	return json.RawMessage(result), nil
}

// envelopeItemKeys is the priority order for locating the items array inside a
// response envelope. Generic keys first (pre-existing behavior), then Kalshi's
// resource-named envelopes. market_positions deliberately precedes
// event_positions: --all flattens to the items array, and the market-level
// rows are what the portfolio commands operate on.
var envelopeItemKeys = []string{
	"data", "items", "results", "messages", "members", "values",
	"fills", "orders", "settlements", "market_positions", "event_positions",
	"markets", "events", "trades", "series", "candlesticks", "milestones",
	"structured_targets",
}

// extractEnvelopeItems pulls the items array out of a response envelope.
// Falls back to "exactly one array-valued key" so future resource-named
// envelopes work without a code change (same strategy as sync's
// extractPageItems).
func extractEnvelopeItems(obj map[string]json.RawMessage) ([]json.RawMessage, bool) {
	for _, field := range envelopeItemKeys {
...
// countResultItems reports how many items a command's result payload holds,
// for the provenance line. Handles bare arrays AND envelope objects — the old
// inline `json.Unmarshal(data, &countItems)` printed "0 results" for every
// envelope response (audit 2026-06-09).
func countResultItems(data json.RawMessage) int {
	var items []json.RawMessage
	if json.Unmarshal(data, &items) == nil {
		return len(items)
	}
	var obj map[string]json.RawMessage
	if json.Unmarshal(data, &obj) == nil {
		if nested, ok := extractEnvelopeItems(obj); ok {
			return len(nested)
		}
		return 1 // a single object (e.g. balance) is one result
	}
	return 0
}
