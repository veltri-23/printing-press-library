# authentik Printed CLI Agent Guide

This directory is a generated `authentik-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
authentik-pp-cli doctor --json
authentik-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
authentik-pp-cli which "<capability>" --json
...
```bash
authentik-pp-cli <command> --agent
```
...
```bash
authentik-pp-cli <command> --help
authentik-pp-cli <command> --dry-run --agent
```
