...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "authentik-pp-cli",
		Short: `Authentik CLI — Agent-native admin CLI for authentik identity provider with offline SQLite cache and an MCP server for Claude Desktop.`,
		Long: `Authentik CLI — Agent-native admin CLI for authentik identity provider with offline SQLite cache and an MCP server for Claude Desktop.
...
Health check: run 'authentik-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("authentik-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'authentik-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAuthenticatorsCmd(flags))
	rootCmd.AddCommand(newCoreCmd(flags))
	rootCmd.AddCommand(newEnterpriseCmd(flags))
	...
	rootCmd.AddCommand(newPropertymappingsCmd(flags))
	rootCmd.AddCommand(newProvidersCmd(flags))
	rootCmd.AddCommand(newRacCmd(flags))
	rootCmd.AddCommand(newRbacCmd(flags))
	rootCmd.AddCommand(newSourcesCmd(flags))
	rootCmd.AddCommand(newSsfCmd(flags))
	...
	rootCmd.AddCommand(newRootPromotedCmd(flags))
	rootCmd.AddCommand(newSchemaPromotedCmd(flags))
