...
In local mode: searches locally synced data only.`,
  ...
  authentik-pp-cli search "error timeout"

  # Force local search only
  authentik-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  authentik-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  authentik-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'authentik-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "core":
				results, err = db.SearchCore(query, limit)
			case "events":
				results, err = db.SearchEvents(query, limit)
			case "flows":
				results, err = db.SearchFlows(query, limit)
			case "policies":
				results, err = db.SearchPolicies(query, limit)
			case "propertymappings":
				results, err = db.SearchPropertymappings(query, limit)
			case "providers":
				results, err = db.SearchProviders(query, limit)
			case "sources":
				results, err = db.SearchSources(query, limit)
			case "stages":
				results, err = db.SearchStages(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCore(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search core failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchFlows(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search flows failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPolicies(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search policies failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPropertymappings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search propertymappings failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProviders(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search providers failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSources(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sources failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchStages(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search stages failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
