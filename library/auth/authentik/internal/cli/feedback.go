...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".authentik-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AUTHENTIK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AUTHENTIK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "authentik-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.authentik-pp-cli/feedback.jsonl.
When ` + "`AUTHENTIK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AUTHENTIK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  authentik-pp-cli feedback list
  authentik-pp-cli feedback list --limit 5
  authentik-pp-cli feedback list --json`,
