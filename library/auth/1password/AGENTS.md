# 1Password Printed CLI Agent Guide

This directory is a generated `1password-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
1password-pp-cli doctor --json
1password-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
1password-pp-cli which "<capability>" --json
1password-pp-cli <command> --help
```
...
```bash
1password-pp-cli <command> --agent
```
...
```bash
1password-pp-cli <command> --help
1password-pp-cli <command> --dry-run --agent
```
