...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "1password-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("API_1PASSWORD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("API_1PASSWORD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "1password-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/1password-pp-cli/feedback.jsonl.
When ` + "`API_1PASSWORD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`API_1PASSWORD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  1password-pp-cli feedback list
  1password-pp-cli feedback list --limit 5
  1password-pp-cli feedback list --json`,
