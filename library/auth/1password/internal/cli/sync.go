...
Resource scoping:
  ...
  1password-pp-cli sync

  # Sync specific resources only
  1password-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  1password-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  1password-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  1password-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  1password-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "1password-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/1password-pp-cli/data.db)")
