...
type Config struct {
	...
	Path                string            `toml:"-"`
	MultimailBearerAuth string            `toml:"bearer_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.multimail.dev",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("MULTIMAIL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "multimail-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MULTIMAIL_BEARER_AUTH"); v != "" {
		cfg.MultimailBearerAuth = v
		cfg.AuthSource = "env:MULTIMAIL_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.MultimailBearerAuth != "" {
		cfg.AuthSource = "config"
	}
