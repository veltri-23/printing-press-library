...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "multimail-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MULTIMAIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MULTIMAIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/social-and-messaging/multimail/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/multimail-pp-cli/feedback.jsonl.
When ` + "`MULTIMAIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MULTIMAIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  multimail-pp-cli feedback list
  multimail-pp-cli feedback list --limit 5
  multimail-pp-cli feedback list --json`,
