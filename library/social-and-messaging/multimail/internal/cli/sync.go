...
Resource scoping:
  ...
  multimail-pp-cli sync

  # Sync specific resources only
  multimail-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  multimail-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  multimail-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  multimail-pp-cli sync --concurrency 8
  ...
  multimail-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/multimail-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "request_upgrade":
		return db.UpsertRequestUpgrade(data)
	case "send":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "mailboxes_emails", ParentTable: "mailboxes", ParentIDParam: "mailboxId", PathTemplate: "/v1/mailboxes/{mailboxId}/emails", KeyField: "", PathParams: []dependentPathParamDef{
