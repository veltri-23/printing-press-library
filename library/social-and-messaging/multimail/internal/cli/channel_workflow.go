...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  multimail-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  multimail-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"account", "api-keys", "audit-log", "auth-md", "confirm", "contacts", "data_export", "domains", "emails", "health", "mailboxes", "operator", "oversight", "suppression", "usage", "webhook-deliveries", "webhooks", "well-known", "well-known-oauth-authorization-server", "well-known-oauth-protected-resource"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  multimail-pp-cli workflow status

  # Show status as JSON
  multimail-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("multimail-pp-cli")
			}
