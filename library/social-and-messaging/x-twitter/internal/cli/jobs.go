...
// to ~/.x-twitter-pp-cli/jobs.jsonl; the latest row for a given JobID wins
...
~/.x-twitter-pp-cli/jobs.jsonl. This command lists, inspects, and prunes them.
...
func newJobsGetCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example:     "  x-twitter-pp-cli jobs get 1750000000000000000 --json",
		Annotations: map[string]string{"mcp:read-only": "true"},
...
func newJobsPruneCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  x-twitter-pp-cli jobs prune --older-than 168h",
		RunE: func(cmd *cobra.Command, _ []string) error {
			rows, err := readJobRows()
			if err != nil {
				return err
