...
func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://www.techtwitter.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("TECHTWITTER_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "techtwitter-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("TECHTWITTER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
