...
func newTweetsMonthlyCmd(flags *rootFlags) *cobra.Command {
	var flagYear int
	var flagMonth int

	cmd := &cobra.Command{
		Use:         "monthly",
		Short:       "Curated tweets for a given month",
		Example:     "  techtwitter-pp-cli tweets monthly --year 2026 --month 6",
		Annotations: map[string]string{"pp:endpoint": "tweets.monthly", "pp:method": "GET", "pp:path": "/api/tweets/monthly", "mcp:read-only": "true", "pp:happy-args": "--year=2026;--month=6"},
	...
	cmd.Flags().IntVar(&flagYear, "year", 0, "Four-digit year, e.g. 2026")
