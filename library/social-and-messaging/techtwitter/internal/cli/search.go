...
In local mode: searches locally synced data only.`,
  ...
  techtwitter-pp-cli search "error timeout"

  # Force local search only
  techtwitter-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  techtwitter-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  techtwitter-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/tweets/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/tweets/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'techtwitter-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "command":
				results, err = db.SearchCommand(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCommand(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search command failed: %w", searchErr)
					}
