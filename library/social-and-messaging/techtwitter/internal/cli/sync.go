...
Resource scoping:
  ...
  techtwitter-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  techtwitter-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  techtwitter-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  techtwitter-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  techtwitter-pp-cli sync --latest-only`,
...
func resourceSupportsPagination(resource string) bool {
	...
	case "articles":
		return true
	case "command-hot-takes":
		return true
	case "command-tweets-by-date":
		return true
	case "tweets":
		return true
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "command":
		return db.UpsertCommand(data)
	case "newsletters":
		return db.UpsertNewsletters(data)
	case "profiles":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"command":                "/api/command/heatmap",
		"command-hot-takes":      "/api/command/hot-takes",
		"command-main-character": "/api/command/main-character",
		"command-tweets-by-date": "/api/command/tweets-by-date",
		"newsletters":            "/api/newsletters",
		"tweets":                 "/api/tweets/trending",
	}
