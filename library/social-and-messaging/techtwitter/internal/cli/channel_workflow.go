...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  techtwitter-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  techtwitter-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"articles", "command", "command-hot-takes", "command-tweets-by-date", "newsletters", "tweets"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  techtwitter-pp-cli workflow status

  # Show status as JSON
  techtwitter-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("techtwitter-pp-cli")
			}
