...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "techtwitter-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TECHTWITTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TECHTWITTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "techtwitter-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/techtwitter-pp-cli/feedback.jsonl.
When ` + "`TECHTWITTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TECHTWITTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  techtwitter-pp-cli feedback list
  techtwitter-pp-cli feedback list --limit 5
  techtwitter-pp-cli feedback list --json`,
