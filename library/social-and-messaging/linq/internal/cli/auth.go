...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Linq",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  linq-pp-cli auth setup\n  linq-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://dashboard.linqapp.com/sandbox-signup")
			fmt.Fprintln(w, "  Use a Linq sandbox API key for testing; never use real end-user data or sensitive data in examples, logs, or smoke tests.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Set one of:")
			fmt.Fprintln(w, "  export LINQ_API_V3_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  export LINQ_API_KEY=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://dashboard.linqapp.com/sandbox-signup"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  linq-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  linq-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  linq-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("LINQ_API_V3_API_KEY") != "" {
				envStillSet = "LINQ_API_V3_API_KEY"
			}
			if envStillSet == "" && os.Getenv("LINQ_API_KEY") != "" {
				envStillSet = "LINQ_API_KEY"
			}
