...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "linq-pp-cli",
		Short: "Manage linq resources via the linq API",
		Long: `Manage linq resources via the linq API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'linq-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("linq-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'linq-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCapabilityCmd(flags))
	rootCmd.AddCommand(newChatsCmd(flags))
	rootCmd.AddCommand(newComposeCmd(flags))
	rootCmd.AddCommand(newContactCardCmd(flags))
	rootCmd.AddCommand(newContactShareCmd(flags))
	rootCmd.AddCommand(newEffectsCmd(flags))
	rootCmd.AddCommand(newLinkPreviewCmd(flags))
	rootCmd.AddCommand(newMessagesCmd(flags))
	rootCmd.AddCommand(newReactCmd(flags))
	rootCmd.AddCommand(newTypingCmd(flags))
	...
	rootCmd.AddCommand(newPhoneNumbersPromotedCmd(flags))
	rootCmd.AddCommand(newPhonenumbersPromotedCmd(flags))
	rootCmd.AddCommand(newWebhookEventsPromotedCmd(flags))
