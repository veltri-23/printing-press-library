...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  linq-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  linq-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"chats", "phone-numbers", "phonenumbers", "webhook-events", "webhook-subscriptions"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  linq-pp-cli workflow status

  # Show status as JSON
  linq-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("linq-pp-cli")
			}
