...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "linq-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LINQ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LINQ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/social-and-messaging/linq/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/linq-pp-cli/feedback.jsonl.
When ` + "`LINQ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LINQ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  linq-pp-cli feedback list
  linq-pp-cli feedback list --limit 5
  linq-pp-cli feedback list --json`,
