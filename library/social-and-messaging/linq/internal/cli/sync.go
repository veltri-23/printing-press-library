...
Resource scoping:
  ...
  linq-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  linq-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  linq-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  linq-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  linq-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/linq-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "share_contact_card":
		return db.UpsertShareContactCard(data)
	case "typing":
		return db.UpsertTyping(data)
	case "voicememo":
		return db.UpsertVoicememo(data)
	case "reactions":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"chats":                 "/v3/chats",
		"contact-card":          "/v3/contact_card",
		"phone-numbers":         "/v3/phone_numbers",
		"phonenumbers":          "/v3/phonenumbers",
		"webhook-events":        "/v3/webhook-events",
		"webhook-subscriptions": "/v3/webhook-subscriptions",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "chats_messages", ParentTable: "chats", ParentIDParam: "chatId", PathTemplate: "/v3/chats/{chatId}/messages", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "chatId", Field: "id"},
		}},
	}
}
