...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  linq-pp-cli doctor
  linq-pp-cli doctor --json
  linq-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set one of: export LINQ_API_V3_API_KEY=\"your-token-here\" or export LINQ_API_KEY=\"your-token-here\""
					report["auth_key_url"] = "https://dashboard.linqapp.com/sandbox-signup"
					report["auth_instructions"] = "Use a Linq sandbox API key for testing; never use real end-user data or sensitive data in examples, logs, or smoke tests."
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "LINQ_API_V3_API_KEY")
				if os.Getenv("LINQ_API_V3_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "LINQ_API_V3_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("LINQ_API_V3_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "LINQ_API_V3_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "LINQ_API_V3_API_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "LINQ_API_KEY")
				if os.Getenv("LINQ_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "LINQ_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("LINQ_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "LINQ_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "LINQ_API_KEY optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "linq-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("linq-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'linq-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'linq-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'linq-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'linq-pp-cli sync' to refresh."
	}
	return report
}
