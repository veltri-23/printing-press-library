...
type Config struct {
	...
	LinqApiV3ApiKey string            `toml:"api_v3_api_key"`
	LinqApiKey      string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("LINQ_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "linq-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("LINQ_API_V3_API_KEY"); v != "" {
		cfg.LinqApiV3ApiKey = v
		cfg.markEnvOverride("LinqApiV3ApiKey")
		cfg.AuthSource = "env:LINQ_API_V3_API_KEY"
	}
	if v := os.Getenv("LINQ_API_KEY"); v != "" {
		cfg.LinqApiKey = v
		cfg.markEnvOverride("LinqApiKey")
		cfg.AuthSource = "env:LINQ_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.LinqApiV3ApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.LinqApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("LINQ_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.LinqApiV3ApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:LINQ_API_V3_API_KEY"
		}
		return "Bearer " + c.LinqApiV3ApiKey
	}
	if c.LinqApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:LINQ_API_KEY"
		}
		return "Bearer " + c.LinqApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.LinqApiV3ApiKey = ""
	delete(c.envOverrides, "LinqApiV3ApiKey")
	c.LinqApiKey = ""
	delete(c.envOverrides, "LinqApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["LinqApiV3ApiKey"] {
			out.LinqApiV3ApiKey = c.fileConfig.LinqApiV3ApiKey
		}
		if c.envOverrides["LinqApiKey"] {
			out.LinqApiKey = c.fileConfig.LinqApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "LinqApiV3ApiKey":
		c.fileConfig.LinqApiV3ApiKey = c.LinqApiV3ApiKey
	case "LinqApiKey":
		c.fileConfig.LinqApiKey = c.LinqApiKey
	}
}
