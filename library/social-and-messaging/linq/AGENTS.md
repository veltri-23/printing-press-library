# Linq Printed CLI Agent Guide

This directory is a generated `linq-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
linq-pp-cli doctor --json
linq-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
linq-pp-cli which "<capability>" --json
linq-pp-cli <command> --help
```
...
```bash
linq-pp-cli <command> --help
linq-pp-cli <command> --dry-run --agent
```
