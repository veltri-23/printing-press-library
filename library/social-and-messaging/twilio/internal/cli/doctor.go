...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twilio-pp-cli doctor
  twilio-pp-cli doctor --json
  twilio-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TWILIO_ACCOUNT_SID=AC...; export TWILIO_AUTH_TOKEN=... (or TWILIO_API_KEY_SID + TWILIO_API_KEY_SECRET)"
				} else {
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					// Detect scoped vs master credential and parent/sub mismatches.
					if cfg.APIKeySid != "" {
						report["auth_mode"] = "api_key (scoped)"
						if !strings.HasPrefix(cfg.APIKeySid, "SK") {
							report["auth_warning"] = "TWILIO_API_KEY_SID does not start with SK; verify it is an API Key SID, not an Account SID"
						}
					} else if cfg.AccountSid != "" && cfg.AuthToken != "" {
						report["auth_mode"] = "account_sid + auth_token (master)"
					}
					if cfg.AccountSid != "" && !strings.HasPrefix(cfg.AccountSid, "AC") {
						report["account_sid_warning"] = "TWILIO_ACCOUNT_SID does not start with AC; URL paths require an Account SID, not an API Key SID"
					}
				}
			}

			// Check auth environment variables.
			// Twilio supports two credential pairs:
			//   (Account SID + Auth Token) OR (API Key SID + API Key Secret)
			// The Account SID is required either way because it appears in the URL path.
			authEnvSet := []string{}
			authEnvRequiredMissing := []string{}
			authEnvInfo := []string{}
			if os.Getenv("TWILIO_ACCOUNT_SID") != "" {
				authEnvSet = append(authEnvSet, "TWILIO_ACCOUNT_SID")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TWILIO_ACCOUNT_SID")
			}
			hasAuthToken := os.Getenv("TWILIO_AUTH_TOKEN") != ""
			hasAPIKey := os.Getenv("TWILIO_API_KEY_SID") != "" && os.Getenv("TWILIO_API_KEY_SECRET") != ""
			if hasAuthToken {
				authEnvSet = append(authEnvSet, "TWILIO_AUTH_TOKEN")
			}
			if hasAPIKey {
				authEnvSet = append(authEnvSet, "TWILIO_API_KEY_SID", "TWILIO_API_KEY_SECRET")
			}
			if !hasAuthToken && !hasAPIKey {
				authEnvInfo = append(authEnvInfo, "set TWILIO_AUTH_TOKEN or both TWILIO_API_KEY_SID + TWILIO_API_KEY_SECRET")
			}
			switch {
			...
			default:
				report["env_vars"] = fmt.Sprintf("OK %d available", len(authEnvSet))
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "twilio-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'twilio-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'twilio-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'twilio-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'twilio-pp-cli sync' to refresh."
	}
	return report
}
