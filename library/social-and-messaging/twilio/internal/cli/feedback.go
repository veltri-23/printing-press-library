...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".twilio-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TWILIO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TWILIO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "twilio-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.twilio-pp-cli/feedback.jsonl.
When ` + "`TWILIO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TWILIO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twilio-pp-cli feedback list
  twilio-pp-cli feedback list --limit 5
  twilio-pp-cli feedback list --json`,
