# Instagram Printed CLI Agent Guide

This directory is a generated `instagram-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
instagram-pp-cli doctor --json
instagram-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
instagram-pp-cli which "<capability>" --json
instagram-pp-cli <command> --help
```
...
```bash
instagram-pp-cli <command> --agent
```
...
```bash
instagram-pp-cli <command> --help
instagram-pp-cli <command> --dry-run --agent
```
