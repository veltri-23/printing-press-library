...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "instagram-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INSTAGRAM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INSTAGRAM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "instagram-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/instagram-pp-cli/feedback.jsonl.
When ` + "`INSTAGRAM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INSTAGRAM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  instagram-pp-cli feedback list
  instagram-pp-cli feedback list --limit 5
  instagram-pp-cli feedback list --json`,
