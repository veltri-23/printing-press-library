...
Resource scoping:
  ...
  instagram-pp-cli sync

  # Sync specific resources only
  instagram-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  instagram-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  instagram-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  instagram-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  instagram-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "account-insights":
		return db.UpsertAccountInsights(data)
	case "accounts":
		return db.UpsertAccounts(data)
	case "comments":
		return db.UpsertComments(data)
	case "hashtags":
		return db.UpsertHashtags(data)
	case "media":
		return db.UpsertMedia(data)
	case "stories":
		return db.UpsertStories(data)
	case "tags":
