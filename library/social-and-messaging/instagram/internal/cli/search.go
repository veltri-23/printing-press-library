...
In local mode: searches locally synced data only.`,
  ...
  instagram-pp-cli search "error timeout"

  # Force local search only
  instagram-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  instagram-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  instagram-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /ig_hashtag_search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/ig_hashtag_search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'instagram-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "account-insights":
				results, err = db.SearchAccountInsights(query, limit)
			case "media":
				results, err = db.SearchMedia(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAccountInsights(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search account_insights failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchMedia(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search media failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
