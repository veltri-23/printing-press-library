...
Resource scoping:
  ...
  luma-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  luma-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  luma-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  luma-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  luma-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "luma-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/luma-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"discover": "/discover/category/list-categories",
		"events":   "/discover/get-paginated-events",
	}
