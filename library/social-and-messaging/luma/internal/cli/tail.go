...
func newTailCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:no-error-path-probe — tail tolerates an initial fetch failure by
		// design (it warns and keeps polling), so a bad-resource probe is not a
		// meaningful error-path test.
...
native streaming instead of polling.`,
  ...
  luma-pp-cli tail --interval 10s

  # Tail a specific resource
  luma-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  luma-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
