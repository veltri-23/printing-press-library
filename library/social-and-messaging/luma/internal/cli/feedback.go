...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "luma-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LUMA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LUMA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "luma-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/luma-pp-cli/feedback.jsonl.
When ` + "`LUMA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LUMA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  luma-pp-cli feedback list
  luma-pp-cli feedback list --limit 5
  luma-pp-cli feedback list --json`,
