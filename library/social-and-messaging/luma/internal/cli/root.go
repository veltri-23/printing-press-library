...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "luma-pp-cli",
		Short: `Luma CLI — Browse, search, and export public Luma events from one Go binary — no account, no API key, plus offline full-text searc…`,
		Long: `Luma CLI — Browse, search, and export public Luma events from one Go binary — no account, no API key, plus offline full-text searc…
...
Health check: run 'luma-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("luma-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'luma-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelIcsCmd(flags))
	rootCmd.AddCommand(newNovelNearCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	// newCalendarsPromotedCmd already wires the `compare` subcommand internally,
	// so do not add it again here (double AddCommand duplicated it in help output).
