# Luma Printed CLI Agent Guide

This directory is a generated `luma-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
luma-pp-cli doctor --json
luma-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
luma-pp-cli which "<capability>" --json
luma-pp-cli <command> --help
```
...
```bash
luma-pp-cli <command> --help
luma-pp-cli <command> --dry-run --agent
```
