# Bird Printed CLI Agent Guide

This directory is a generated `bird-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bird-pp-cli doctor --json
bird-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bird-pp-cli which "<capability>" --json
bird-pp-cli <command> --help
```
...
```bash
bird-pp-cli <command> --help
bird-pp-cli <command> --dry-run --agent
```
