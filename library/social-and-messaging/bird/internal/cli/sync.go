...
Exit codes & warnings:
  ...
  bird-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  bird-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  bird-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  bird-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  bird-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/bird-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "channel-config":
		return db.UpsertChannelConfig(data)
	case "channel-media":
		return db.UpsertChannelMedia(data)
	case "channels":
	...
	case "conversations_messages":
		return db.UpsertConversationsMessages(data)
	case "conversations_participants":
		return db.UpsertConversationsParticipants(data)
	case "media":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"messages":      "/messages",
		"rules":         "/conversation-settings/allow-block-rules",
	}
