...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  bird-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  bird-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"channels", "conversations", "messages", "rules"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  bird-pp-cli workflow status

  # Show status as JSON
  bird-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("bird-pp-cli")
			}
