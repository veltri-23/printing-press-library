...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bird-pp-cli",
		Short: `A terminal-native CLI for Bird's Conversations and SMS APIs, with offline search, batch reconcile, and a local SQLite mirror nobody else ships.`,
		Long: `A terminal-native CLI for Bird's Conversations and SMS APIs, with offline search, batch reconcile, and a local SQLite mirror nobody else ships.
...
Health check: run 'bird-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bird-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'bird-pp-cli profile list')")
	...
	rootCmd.AddCommand(newChannelConfigCmd(flags))
	...
	rootCmd.AddCommand(newChannelMediaPromotedCmd(flags))
	rootCmd.AddCommand(newMediaPromotedCmd(flags))
	rootCmd.AddCommand(newSmsParentCmd(flags))
	rootCmd.AddCommand(newTenantCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	addTranscendenceCommands(rootCmd, flags)
	return rootCmd
}
