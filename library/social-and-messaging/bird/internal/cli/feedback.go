...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bird-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BIRD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BIRD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bird-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bird-pp-cli/feedback.jsonl.
When ` + "`BIRD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BIRD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bird-pp-cli feedback list
  bird-pp-cli feedback list --limit 5
  bird-pp-cli feedback list --json`,
