# Pushover Printed CLI Agent Guide

This directory is a generated `pushover-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pushover-pp-cli doctor --json
pushover-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pushover-pp-cli which "<capability>" --json
pushover-pp-cli <command> --help
```
...
```bash
pushover-pp-cli <command> --help
pushover-pp-cli <command> --dry-run --agent
```
