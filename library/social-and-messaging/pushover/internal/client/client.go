...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "pushover-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "pushover-pp-cli/0.1.0")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if params != nil {
		...
		for _, k := range keys {
			...
			fmt.Fprintf(os.Stderr, "  %s%s=%s\n", sep, k, redactValueForKey(k, params[k]))
			queryPrinted = true
		}
	}
	_ = queryPrinted
	if body != nil {
		...
		if json.Unmarshal(body, &parsed) == nil {
			...
			enc.Encode(redactSensitiveJSON(parsed))
		}
	}
	if authHeader != "" {
...
func redactSensitiveJSON(v any) any {
	...
	case map[string]any:
		...
		for k, child := range typed {
			if isSensitiveKey(k) {
				out[k] = redactValueForKey(k, fmt.Sprint(child))
				continue
			}
			out[k] = redactSensitiveJSON(child)
		}
		return out
	case []any:
...
func isSensitiveKey(key string) bool {
	...
	if key == "url" || key == "url_title" || key == "message" || key == "title" || key == "device" {
		return false
	}
	for _, marker := range []string{"token", "secret", "password", "user", "email", "key", "license"} {
