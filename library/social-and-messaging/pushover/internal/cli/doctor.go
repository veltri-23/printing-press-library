...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pushover-pp-cli doctor
  pushover-pp-cli doctor --json
  pushover-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Check auth. Pushover carries credentials as endpoint
			// parameters instead of an Authorization header, so generated
			// auth_type is none while real workflows still need secrets.
			report["auth"] = "parameter credentials"
			report["env_vars"] = pushoverDoctorEnvSummary()
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Step 2: Validate Pushover parameter credentials when
					// both required send values are available.
					appToken, _ := firstPushoverEnv("PUSHOVER_APP_TOKEN", "PUSHOVER_TOKEN")
					userKey, _ := firstPushoverEnv("PUSHOVER_USER_KEY", "PUSHOVER_USER")
					if appToken == "" || userKey == "" {
						report["credentials"] = "missing PUSHOVER_APP_TOKEN/PUSHOVER_USER_KEY for send workflows"
					} else if reachErr != nil && !errors.As(reachErr, &reachAPIErr) {
						report["credentials"] = "skipped (API unreachable)"
					} else {
						validateBody := map[string]any{"token": appToken, "user": userKey}
						data, _, validateErr := c.Post("/1/users/validate.json", validateBody)
						if validateErr != nil {
							report["credentials"] = fmt.Sprintf("invalid or rejected: %s", validateErr)
						} else {
							var payload map[string]any
							_ = json.Unmarshal(data, &payload)
							if intFromAny(payload["status"]) == 1 {
								report["credentials"] = "valid"
							} else {
								report["credentials"] = "present but validation response was not successful"
							}
						}
					}
				}
...
func pushoverDoctorEnvSummary() string {
	checks := []struct {
		label string
		envs  []string
	}{
		{label: "app_token", envs: []string{"PUSHOVER_APP_TOKEN", "PUSHOVER_TOKEN"}},
		{label: "user_key", envs: []string{"PUSHOVER_USER_KEY", "PUSHOVER_USER"}},
		{label: "client_secret", envs: []string{"PUSHOVER_CLIENT_SECRET", "PUSHOVER_SECRET"}},
		{label: "device_id", envs: []string{"PUSHOVER_DEVICE_ID"}},
	}
	parts := make([]string, 0, len(checks))
	for _, check := range checks {
		_, source := firstPushoverEnv(check.envs...)
		if source == "" {
			parts = append(parts, check.label+"=missing")
			continue
		}
		parts = append(parts, check.label+"=present("+source+")")
	}
	return strings.Join(parts, ", ")
}

func firstPushoverEnv(names ...string) (string, string) {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'pushover-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'pushover-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'pushover-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'pushover-pp-cli sync' to refresh."
	}
	return report
}
