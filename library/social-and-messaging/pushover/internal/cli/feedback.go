...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pushover-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PUSHOVER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PUSHOVER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pushover-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pushover-pp-cli/feedback.jsonl.
When ` + "`PUSHOVER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PUSHOVER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pushover-pp-cli feedback list
  pushover-pp-cli feedback list --limit 5
  pushover-pp-cli feedback list --json`,
