...
func applyPushoverEnvDefaults(cmd *cobra.Command) error {
	if cmd == nil {
		return nil
	}
	envDefaults := []struct {
		...
		{flag: "app-token", envs: []string{"PUSHOVER_APP_TOKEN", "PUSHOVER_TOKEN"}},
		{flag: "user-key", envs: []string{"PUSHOVER_USER_KEY", "PUSHOVER_USER"}},
		{flag: "team-token", envs: []string{"PUSHOVER_TEAM_TOKEN"}},
		{flag: "client-secret", envs: []string{"PUSHOVER_CLIENT_SECRET", "PUSHOVER_SECRET"}},
		{flag: "device-id", envs: []string{"PUSHOVER_DEVICE_ID"}},
		{flag: "license-code", envs: []string{"PUSHOVER_LICENSE_CODE"}},
	}
	for _, d := range envDefaults {
		if err := setFlagDefaultFromEnv(cmd, d.flag, d.envs...); err != nil {
			return err
		}
	}
	return nil
...
func setFlagDefaultFromEnv(cmd *cobra.Command, flagName string, envNames ...string) error {
	fs := cmd.Flags()
	if fs == nil {
		return nil
	}
	...
	if fs.Changed(flagName) && !isPlaceholderFlagValue(flag.Value.String()) {
		return nil
	}
	for _, envName := range envNames {
		...
		if err := fs.Set(flagName, value); err != nil {
			return fmt.Errorf("applying %s to --%s: %w", envName, flagName, err)
		}
		return nil
	}
	return nil
...
func isPlaceholderFlagValue(value string) bool {
	switch strings.TrimSpace(value) {
	case "your-token-here", "example-value", "example-token", "example-secret", "example-key":
		return true
	default:
		return false
	}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'pushover-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'pushover-pp-cli doctor' to check connectivity.", err))
