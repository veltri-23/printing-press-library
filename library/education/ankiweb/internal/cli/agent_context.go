...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "ankiweb-pp-cli",
			Description: "The only terminal-native way to search, rank",
...
func buildAgentDiscoveryContext() *agentContextDiscovery {
	return &agentContextDiscovery{
		...
		TargetURL:     "https://ankiweb.net/shared/decks",
		EntryCount:    6,
		APIEntryCount: 5,
		Reachability:  "standard_http (85% confidence)",
		Protocols: []string{
			"protobuf (95% confidence)",
			"sveltekit_spa (90% confidence)",
		},
		AuthCandidates: []string{
			"cookie — cookies: has_auth",
		},
		Protections: []string{},
		GenerationHints: []string{
			"protobuf_responses_require_handwritten_decoders",
			"cookie_auth_validated",
			"download_requires_signed_token",
		},
		Warnings: []string{
		...
		CandidateCommands: []string{
			"search — GET /svc/shared/list-decks?search= returns repeated deck protobuf {id,title,upvotes,downvotes,modified,notes,audio,images}",
			"info — GET /svc/shared/item-info?sharedId= returns full deck detail + reviews protobuf",
			"download — GET /svc/shared/download-deck/{id}?t= requires client-minted signed token (op=sdd); token generation unresolved",
			"list — POST /svc/decks/deck-list-info returns user's synced decks; cookie-gated",
		},
	}
}
