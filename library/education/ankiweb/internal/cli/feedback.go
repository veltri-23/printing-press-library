...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ankiweb-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ANKIWEB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ANKIWEB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ankiweb-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ankiweb-pp-cli/feedback.jsonl.
When ` + "`ANKIWEB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ANKIWEB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ankiweb-pp-cli feedback list
  ankiweb-pp-cli feedback list --limit 5
  ankiweb-pp-cli feedback list --json`,
