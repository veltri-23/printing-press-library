...
Resource scoping:
  ...
  ankiweb-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ankiweb-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ankiweb-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ankiweb-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ankiweb-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			// AnkiWeb has no bulk-list resource endpoints; the only syncable
			// data is the public shared-deck catalog, keyed by search term.
			// `sync --search <term>` (or a sensible default set) fetches
			// list-decks and upserts SharedDeck rows into the typed `shared`
			// table so the offline `search` command can FTS them.
			if len(searchTerms) > 0 || !cmd.Flags().Changed("resources") {
				terms := searchTerms
				if len(terms) == 0 {
					terms = defaultSharedSearchTerms()
				}
				return runSharedSync(cmd, flags, dbPath, terms)
			}
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "ankiweb-pp-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
	...
	cmd.Flags().StringSliceVar(&searchTerms, "search", nil, "Shared-deck search term(s) to sync into the local store for offline search (repeatable/comma-separated). Default: a small category set.")
