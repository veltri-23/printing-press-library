...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	result, err := handleWhatAnatomyDecksAreNewThisWeek(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesAndRejectsArgs(t *testing.T) {
	...
	got := recipeToolText(t, result)
	if strings.Contains(got, "--since-last-sync") {
		t.Fatalf("boolean false override should suppress --since-last-sync: %q", got)
	}
}
