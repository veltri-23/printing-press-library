...
Resource scoping:
  ...
  quranku-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  quranku-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  quranku-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  quranku-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  quranku-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("quranku-pp-cli")
			}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "juz":
		return db.UpsertJuz(data)
	case "recitation":
		return db.UpsertRecitation(data)
	case "surah":
		return db.UpsertSurah(data)
	case "tafsirs":
		return db.UpsertTafsirs(data)
	case "websearch":
		return db.UpsertWebsearch(data)
	default:
...
func responsePathForResource(resource, path string) []string {
	...
	case "chapters\x00/chapters/{id}/info":
		return []string{"chapter_info"}
	case "chapters\x00/chapters":
		return []string{"chapters"}
	case "juz\x00/juzs":
		return []string{"juzs"}
	case "recitation\x00/chapter_recitations/{reciter}/{chapter}":
		return []string{"audio_file"}
	case "surah\x00/surahs/{id}":
		return []string{"data"}
	case "surah\x00/surahs":
		return []string{"data"}
	case "tafsirs\x00/resources/tafsirs":
		return []string{"tafsirs"}
	case "uthmani\x00/quran/verses/uthmani":
		return []string{"verses"}
	case "verses\x00/verses/by_chapter/{chapter}":
		return []string{"verses"}
	case "websearch\x00/search":
