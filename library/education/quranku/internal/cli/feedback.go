...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("QURANKU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("QURANKU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "quranku-pp-cli/feedback")
...
When ` + "`QURANKU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`QURANKU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  quranku-pp-cli feedback list
  quranku-pp-cli feedback list --limit 5
  quranku-pp-cli feedback list --json`,
