# QuranKu Printed CLI Agent Guide

This directory is a generated `quranku-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
quranku-pp-cli doctor --json
quranku-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
quranku-pp-cli which "<capability>" --json
quranku-pp-cli <command> --help
```
...
```bash
quranku-pp-cli <command> --help
quranku-pp-cli <command> --dry-run --agent
```
