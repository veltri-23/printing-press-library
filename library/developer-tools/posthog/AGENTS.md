# PostHog Printed CLI Agent Guide

This directory is a generated `posthog-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
posthog-pp-cli doctor --json
posthog-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
posthog-pp-cli which "<capability>" --json
...
```bash
posthog-pp-cli <command> --help
posthog-pp-cli <command> --dry-run --agent
```
