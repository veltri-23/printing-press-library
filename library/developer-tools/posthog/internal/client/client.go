...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "posthog-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	// If path is already an absolute URL (e.g. a pagination `next` pointer),
	// use it directly; otherwise prepend the base URL.
	targetURL := path
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "posthog-pp-cli/1.0.0")
		}
...
func (c *Client) refreshAccessToken() error {
	...
	// PostHog personal API keys do not expire. OAuth2 access tokens can expire,
	// but the CLI has no refresh endpoint configured. Surface an actionable error
	// rather than silently attaching an expired token to every request.
	return fmt.Errorf("access token has expired; run 'posthog-pp-cli auth login' to re-authenticate")
}
