...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  posthog-pp-cli doctor
  posthog-pp-cli doctor --json
  posthog-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export POSTHOG_PERSONAL_APIKEY_AUTH=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("POSTHOG_PERSONAL_APIKEY_AUTH") != "" {
				authEnvSet = append(authEnvSet, "POSTHOG_PERSONAL_APIKEY_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "POSTHOG_PERSONAL_APIKEY_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'posthog-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'posthog-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'posthog-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'posthog-pp-cli sync' to refresh."
	}
	return report
}
