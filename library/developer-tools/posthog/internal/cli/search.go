...
In local mode: searches locally synced data only.`,
  ...
  posthog-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  posthog-pp-cli search "critical" --type transactions --data-source local
  ...
  posthog-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /api/environments/{project_id}/mcp_tools/docs_search/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/api/environments/{project_id}/mcp_tools/docs_search/", map[string]any{
					"query": query,
				})
				if getErr == nil {
