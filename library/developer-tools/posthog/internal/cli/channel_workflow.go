...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  posthog-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  posthog-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"organizations", "public-hog-function-templates", "users"}
			totalSynced := 0
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path override, default is ~/.local/share/posthog-pp-cli/data.db")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  posthog-pp-cli workflow status

  # Show status as JSON
  posthog-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("posthog-pp-cli")
			}
