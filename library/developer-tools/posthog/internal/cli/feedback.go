...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".posthog-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POSTHOG_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POSTHOG_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "posthog-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.posthog-pp-cli/feedback.jsonl.
When ` + "`POSTHOG_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POSTHOG_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  posthog-pp-cli feedback list
  posthog-pp-cli feedback list --limit 5
  posthog-pp-cli feedback list --json`,
