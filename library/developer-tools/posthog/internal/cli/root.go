...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "posthog-pp-cli",
		Short: `Posthog CLI — Every PostHog resource in one CLI — with offline search, agent-native output, and cross-resource analytics no dashboard…`,
		Long: `Posthog CLI — Every PostHog resource in one CLI — with offline search, agent-native output, and cross-resource analytics no dashboard…
...
Health check: run 'posthog-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("posthog-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'posthog-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		autoRefreshIfStale(cmd.Context(), cmd.ErrOrStderr(), defaultStaleAfter)
		return nil
	}
	rootCmd.AddCommand(newProjectsCmd(flags))
	rootCmd.AddCommand(newUserHomeSettingsCmd(flags))
	...
	rootCmd.AddCommand(newPublicHogFunctionTemplatesPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	rootCmd.AddCommand(newFlagsCmd(flags))
	rootCmd.AddCommand(newLLMCmd(flags))
	rootCmd.AddCommand(newPersonsCmd(flags))
	rootCmd.AddCommand(newEventsCmd(flags))
	rootCmd.AddCommand(newExperimentsCmd(flags))
