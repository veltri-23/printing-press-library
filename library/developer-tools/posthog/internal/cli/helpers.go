...
// isTerminal reports whether w is a character device (isatty check via os.ModeCharDevice).
// Equivalent to calling isatty.IsTerminal on the underlying file descriptor.
...
func isSyncAccessWarning(err error) (*accessWarning, bool) {
	...
	var apiErrVal *client.APIError
	if errors.As(err, &apiErrVal) {
		switch apiErrVal.StatusCode {
		case 403:
			return &accessWarning{Status: 403, Reason: "forbidden", Message: apiErrVal.Body}, true
		case 400:
			if looksLikeAccessDenial(apiErrVal.Body) {
				return &accessWarning{Status: 400, Reason: "insufficient_access", Message: apiErrVal.Body}, true
			}
		}
	}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Set your API key: export POSTHOG_PERSONAL_APIKEY_AUTH=<your-key>"+
			"\n      Run 'posthog-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: posthog-pp-cli auth set-token <token>"+
			"\n      or: export POSTHOG_PERSONAL_APIKEY_AUTH=<your-token>"+
			"\n      Run 'posthog-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your credentials are valid but lack access to this resource."+
			"\n      Check that your API key has the required permissions."+
			"\n      Set it with: export POSTHOG_PERSONAL_APIKEY_AUTH=<your-key>"+
			"\n      Run 'posthog-pp-cli doctor' to check auth status.", err))
