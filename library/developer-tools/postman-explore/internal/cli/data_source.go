...
func resolveLocal(flags *rootFlags, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	...
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'postman-explore-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'postman-explore-pp-cli sync' first", resourceType)
		}
	...
	if item == nil {
		return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'postman-explore-pp-cli sync' first", resourceType, id)
	}
