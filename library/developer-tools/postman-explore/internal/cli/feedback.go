...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".postman-explore-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POSTMAN_EXPLORE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POSTMAN_EXPLORE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "postman-explore-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.postman-explore-pp-cli/feedback.jsonl.
When ` + "`POSTMAN_EXPLORE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POSTMAN_EXPLORE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
