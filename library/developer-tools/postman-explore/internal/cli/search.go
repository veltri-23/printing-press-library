...
In local mode: searches locally synced data only.`,
  ...
  postman-explore-pp-cli search "error timeout"

  # Force local search only
  postman-explore-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  postman-explore-pp-cli search "critical" --type transactions --data-source local
  ...
  postman-explore-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /search-all
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'postman-explore-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
