# Pangolin Printed CLI Agent Guide

This directory is a generated `pangolin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pangolin-pp-cli doctor --json
pangolin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pangolin-pp-cli which "<capability>" --json
pangolin-pp-cli <command> --help
```
...
```bash
pangolin-pp-cli <command> --help
pangolin-pp-cli <command> --dry-run --agent
```
