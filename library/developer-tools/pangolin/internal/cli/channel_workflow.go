...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pangolin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pangolin-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"domains", "idp", "openapi-json", "openapi-yaml", "orgs", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pangolin-pp-cli workflow status

  # Show status as JSON
  pangolin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pangolin-pp-cli")
			}
