...
func resolveRead(ctx context.Context, c *client.Client, flags *rootFlags, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'pangolin-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, defaultDBPath("pangolin-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
	...
	if json.Unmarshal(data, &items) != nil || len(items) == 0 {
		...
		if json.Unmarshal(data, &envelope) == nil {
			...
			// natural plural before falling through to single-row treatment.
			if items == nil {
				for _, outerKey := range []string{"data", "result"} {
					raw, ok := envelope[outerKey]
					if !ok {
						continue
					}
					...
					for _, k := range []string{"results", "data", "items", resourceType} {
						if innerRaw, ok := inner[k]; ok {
							var arr []json.RawMessage
							if json.Unmarshal(innerRaw, &arr) == nil && len(arr) > 0 {
								items = arr
								break
							}
						}
					...
					if items == nil {
						// Last-resort: pick the single inner array key.
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "pangolin-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'pangolin-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'pangolin-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'pangolin-pp-cli sync' first", resourceType)
		}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'pangolin-pp-cli sync' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
