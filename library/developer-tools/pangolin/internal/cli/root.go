...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pangolin-pp-cli",
		Short: `Pangolin CLI — The first agent-native CLI for Pangolin — every endpoint, plus offline SQLite, cross-org audits, and one-shot config ba…`,
		Long: `Pangolin CLI — The first agent-native CLI for Pangolin — every endpoint, plus offline SQLite, cross-org audits, and one-shot config ba…
...
Health check: run 'pangolin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pangolin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pangolin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newIdpCmd(flags))
	rootCmd.AddCommand(newOrgCmd(flags))
	rootCmd.AddCommand(newResourceCmd(flags))
	rootCmd.AddCommand(newRoleCmd(flags))
	rootCmd.AddCommand(newSiteCmd(flags))
	rootCmd.AddCommand(newSiteResourceCmd(flags))
	rootCmd.AddCommand(newTargetCmd(flags))
	...
	rootCmd.AddCommand(newCertificatePromotedCmd(flags))
	rootCmd.AddCommand(newDomainPromotedCmd(flags))
	rootCmd.AddCommand(newDomainsPromotedCmd(flags))
	rootCmd.AddCommand(newMaintenancePromotedCmd(flags))
	rootCmd.AddCommand(newOpenapiJsonPromotedCmd(flags))
	rootCmd.AddCommand(newOpenapiYamlPromotedCmd(flags))
	rootCmd.AddCommand(newOrgsPromotedCmd(flags))
	rootCmd.AddCommand(newUserPromotedCmd(flags))
	rootCmd.AddCommand(newUsersPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-written novel features (transcendence). Each is a leaf command that
	// reads from the local SQLite store or sequences live API calls in a
	// shape the dashboard cannot reproduce in a single click.
	rootCmd.AddCommand(newAuditCmd(flags))
	rootCmd.AddCommand(newCertWatchCmd(flags))
	rootCmd.AddCommand(newAccessGraphCmd(flags))
	rootCmd.AddCommand(newBackupCmd(flags))
	rootCmd.AddCommand(newRestoreCmd(flags))
	rootCmd.AddCommand(newExposeCmd(flags))
