...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pangolin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PANGOLIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PANGOLIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pangolin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pangolin-pp-cli/feedback.jsonl.
When ` + "`PANGOLIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PANGOLIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pangolin-pp-cli feedback list
  pangolin-pp-cli feedback list --limit 5
  pangolin-pp-cli feedback list --json`,
