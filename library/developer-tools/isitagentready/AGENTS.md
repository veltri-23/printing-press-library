# Is It Agent Ready Printed CLI Agent Guide

This directory is a generated `isitagentready-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
isitagentready-pp-cli doctor --json
isitagentready-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
isitagentready-pp-cli which "<capability>" --json
...
```bash
isitagentready-pp-cli <command> --agent
```
...
```bash
isitagentready-pp-cli <command> --help
isitagentready-pp-cli <command> --dry-run --agent
```
