...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("get_the_copy_paste_fixes_for_a_site",
			mcplib.WithDescription("Prints every next-level fix prompt as one pasteable block, ready to drop into a coding agent."),
			mcplib.WithBoolean("copy", mcplib.Description("Override the recipe's --copy value.")),
		),
		handleGetTheCopyPasteFixesForASite,
	)
}

// handleGetTheCopyPasteFixesForASite runs the get_the_copy_paste_fixes_for_a_site recipe intent: Prints every next-level fix prompt as one pasteable block, ready to drop into a coding agent.
func handleGetTheCopyPasteFixesForASite(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "copy", input["copy"], true)
