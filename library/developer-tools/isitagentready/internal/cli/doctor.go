...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  isitagentready-pp-cli doctor
  isitagentready-pp-cli doctor --json
  isitagentready-pp-cli doctor --fail-on warn
  isitagentready-pp-cli doctor --fail-on stale`,
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "isitagentready-pp-cli", suggestion)
						} else {
