...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ISITAGENTREADY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ISITAGENTREADY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "isitagentready-pp-cli/feedback")
...
When ` + "`ISITAGENTREADY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ISITAGENTREADY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  isitagentready-pp-cli feedback list
  isitagentready-pp-cli feedback list --limit 5
  isitagentready-pp-cli feedback list --json`,
