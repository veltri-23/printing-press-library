...
// TestUpsertBatch_PopulatesDockerHubSearchTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed docker_hub_search table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesDockerHubSearchTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("docker_hub_search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "docker_hub_search").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "docker_hub_search")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count docker_hub_search: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("docker_hub_search count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestUpsertBatch_PopulatesDockerfileTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("dockerfile", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
