...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  docker-hub-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  docker-hub-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"docker-hub-search"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  docker-hub-pp-cli workflow status

  # Show status as JSON
  docker-hub-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("docker-hub-pp-cli")
			}
