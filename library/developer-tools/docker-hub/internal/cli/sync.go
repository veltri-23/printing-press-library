...
Exit codes & warnings:
  ...
  docker-hub-pp-cli sync

  # Sync specific resources only
  docker-hub-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  docker-hub-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  docker-hub-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  docker-hub-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  docker-hub-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("docker-hub-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/docker-hub-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "docker_hub_search":
		return db.UpsertDockerHubSearch(data)
	case "dockerfile":
		return db.UpsertDockerfile(data)
	case "tags":
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"docker-hub-search": "/v2/search/repositories/",
	}
