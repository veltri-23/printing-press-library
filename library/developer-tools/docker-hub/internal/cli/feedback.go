...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".docker-hub-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOCKER_HUB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOCKER_HUB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "docker-hub-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.docker-hub-pp-cli/feedback.jsonl.
When ` + "`DOCKER_HUB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOCKER_HUB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
