...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GODADDY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GODADDY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "godaddy-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.godaddy-pp-cli/feedback.jsonl.
When ` + "`GODADDY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GODADDY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  godaddy-pp-cli feedback list
  godaddy-pp-cli feedback list --limit 5
  godaddy-pp-cli feedback list --json`,
