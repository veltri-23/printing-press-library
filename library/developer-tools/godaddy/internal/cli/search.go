...
In local mode: searches locally synced data only.`,
  ...
  godaddy-pp-cli search "error timeout"

  # Force local search only
  godaddy-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  godaddy-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  godaddy-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/agents
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v1/agents", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'godaddy-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "agreements":
				results, err = db.SearchAgreements(query, limit)
			case "domains":
				results, err = db.SearchDomains(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAgreements(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search agreements failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDomains(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search domains failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
