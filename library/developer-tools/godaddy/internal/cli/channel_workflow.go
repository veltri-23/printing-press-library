...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  godaddy-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  godaddy-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"abuse", "abuse-tickets", "agents", "agents-events", "certificates", "domains", "domains-maintenances", "domains-suggest", "domains-tlds", "orders", "subscriptions", "subscriptions-product-groups"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  godaddy-pp-cli workflow status

  # Show status as JSON
  godaddy-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("godaddy-pp-cli")
			}
