...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/godaddy/internal/cliutil"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/godaddy/internal/store"
...
Resource scoping:
  ...
  godaddy-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  godaddy-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  godaddy-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  godaddy-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  godaddy-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("godaddy-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/godaddy-pp-cli/data.db)")
...
func resourceSupportsPagination(resource string) bool {
	...
	case "domains":
		return true
	case "domains-maintenances":
		return true
	case "domains-suggest":
		return true
	case "orders":
		return true
	case "records":
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "abuse":
		return db.UpsertAbuse(data)
	case "aftermarket":
		return db.UpsertAftermarket(data)
	case "agents":
		return db.UpsertAgents(data)
	case "agents_certificates":
		return db.UpsertAgentsCertificates(data)
	case "csrs":
		return db.UpsertCsrs(data)
	case "agents_revoke":
		return db.UpsertAgentsRevoke(data)
	case "verify_acme":
		return db.UpsertVerifyAcme(data)
	case "verify_dns":
		return db.UpsertVerifyDns(data)
	case "agreements":
		return db.UpsertAgreements(data)
	case "api_customers_certificates":
		return db.UpsertApiCustomersCertificates(data)
	case "auctions-aftermarket":
		return db.UpsertAuctionsAftermarket(data)
	case "certificates":
		return db.UpsertCertificates(data)
	case "actions":
		return db.UpsertActions(data)
	case "callback":
		return db.UpsertCallback(data)
	case "cancel":
	...
	case "reissue":
		return db.UpsertReissue(data)
	case "certificates_renew":
		return db.UpsertCertificatesRenew(data)
	case "certificates_revoke":
		return db.UpsertCertificatesRevoke(data)
	case "site_seal":
		return db.UpsertSiteSeal(data)
	case "verify_domain_control":
		return db.UpsertVerifyDomainControl(data)
	case "countries":
	...
	case "privacy":
		return db.UpsertPrivacy(data)
	case "records":
		return db.UpsertRecords(data)
	case "domains_renew":
		return db.UpsertDomainsRenew(data)
	case "transfer":
		return db.UpsertTransfer(data)
	case "verify_registrant_email":
		return db.UpsertVerifyRegistrantEmail(data)
	case "domains_customers_domains":
		return db.UpsertDomainsCustomersDomains(data)
	case "orders":
		return db.UpsertOrders(data)
	case "parking":
		return db.UpsertParking(data)
	case "shoppers":
		return db.UpsertShoppers(data)
	case "factors":
		return db.UpsertFactors(data)
	case "status":
...
func defaultSyncResources() []string {
	return []string{
		...
		"certificates",
		"domains",
		"domains-maintenances",
		"domains-suggest",
		"domains-tlds",
		"orders",
		"subscriptions",
		"subscriptions-product-groups",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"abuse":                        "/v1/abuse/tickets",
		"abuse-tickets":                "/v2/abuse/tickets",
		"agents":                       "/v1/agents",
		"agents-events":                "/v1/agents/events",
		"certificates":                 "/v2/certificates/subscriptions/search",
		"domains":                      "/v1/domains",
		"domains-maintenances":         "/v2/domains/maintenances",
		"domains-suggest":              "/v1/domains/suggest",
		"domains-tlds":                 "/v1/domains/tlds",
		"orders":                       "/v1/orders",
		"subscriptions":                "/v1/subscriptions",
		"subscriptions-product-groups": "/v1/subscriptions/productGroups",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "records", ParentTable: "domains", ParentIDParam: "domain", PathTemplate: "/v1/domains/{domain}/records/{type}/{name}", KeyField: ""},
	}
}
