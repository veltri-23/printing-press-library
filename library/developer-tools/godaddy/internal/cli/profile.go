...
func profileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".godaddy-pp-cli")
	...
	// owner-only; best-effort. (profiles.json itself is written via a fresh
	// 0o600 tmp + rename, so the file inode always lands at 0o600.)
...
present (other than --profile and --config).`,
		Example: `  godaddy-pp-cli profile save my-defaults --json --compact
  godaddy-pp-cli profile save tonight-defaults --region US`,
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  godaddy-pp-cli profile use my-defaults
  godaddy-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  godaddy-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  godaddy-pp-cli profile show my-defaults
  godaddy-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  godaddy-pp-cli profile delete my-defaults --yes
  godaddy-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
