...
func TestClient_VerifyShortCircuit_NoEnv(t *testing.T) {
	...
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GODADDY_ALLOW_WRITES", "1")
	c, rec := newClientWithRecorder(t)
...
func TestClient_LiveWriteRequiresApproval(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GODADDY_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)

	_, _, err := c.do(context.Background(), "DELETE", "/v1/domains/example.com", nil, nil, nil)
	if !errors.Is(err, ErrWriteRequiresApproval) {
		t.Fatalf("error = %v, want ErrWriteRequiresApproval", err)
	}
	if rec.calls != 0 {
		t.Fatalf("write barrier should not dial; recorder saw %d calls", rec.calls)
	}
}

func TestClient_LiveWriteBarrierAllowsDryRun(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GODADDY_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)
	...
	if rec.calls != 0 {
		t.Fatalf("dry-run should not dial; recorder saw %d calls", rec.calls)
	}
}

func TestClient_LiveWriteBarrierAllowsValidationPost(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GODADDY_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)

	_, _, err := c.do(context.Background(), "POST", "/v1/domains/available", nil, nil, nil)
	if err != nil {
		t.Fatalf("validation-style POST returned error: %v", err)
	}
	if rec.calls != 1 {
		t.Fatalf("validation-style POST should dial once; recorder saw %d calls", rec.calls)
	}
}
