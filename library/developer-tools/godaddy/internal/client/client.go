...
var ErrWriteRequiresApproval = errors.New("live GoDaddy write requires GODADDY_ALLOW_WRITES=1; rerun with --dry-run to preview without sending")
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "godaddy-pp-cli", "http")
...
func (c *Client) writeCache(path string, params map[string]string, data json.RawMessage) {
	...
	// cache dir/file from an older 0o755/0o644 build keeps world-readable perms
	// while holding cached API data. Re-assert owner-only; best-effort.
...
func isValidationStyleMutation(method, path string) bool {
	if method != "POST" {
		return false
	}
	switch path {
...
func writesAllowed() bool {
	...
	return os.Getenv("GODADDY_ALLOW_WRITES") == "1"
}
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	if !readOnlyIntent && isMutatingVerb(method) && !isValidationStyleMutation(method, path) && !c.DryRun && !writesAllowed() {
		return nil, 0, ErrWriteRequiresApproval
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "godaddy-pp-cli/2.0.0")
		}
