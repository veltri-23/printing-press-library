...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "claude-agent-sdk-python-docs-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLAUDE_AGENT_SDK_PYTHON_DOCS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLAUDE_AGENT_SDK_PYTHON_DOCS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "claude-agent-sdk-python-docs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/claude-agent-sdk-python-docs-pp-cli/feedback.jsonl.
When ` + "`CLAUDE_AGENT_SDK_PYTHON_DOCS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLAUDE_AGENT_SDK_PYTHON_DOCS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "claude-agent-sdk-python-docs-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  claude-agent-sdk-python-docs-pp-cli feedback list
  claude-agent-sdk-python-docs-pp-cli feedback list --limit 5
  claude-agent-sdk-python-docs-pp-cli feedback list --json`,
