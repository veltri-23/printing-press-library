...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CLAUDE_AGENT_SDK_PYTHON_DOCS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "claude-agent-sdk-python-docs-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	if v := os.Getenv("CLAUDE_AGENT_SDK_PYTHON_DOCS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
