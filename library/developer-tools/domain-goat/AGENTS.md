# Domain Goat Printed CLI Agent Guide

This directory is a generated `domain-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
domain-goat-pp-cli doctor --json
domain-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
domain-goat-pp-cli which "<capability>" --json
domain-goat-pp-cli <command> --help
```
...
```bash
domain-goat-pp-cli <command> --agent
```
...
```bash
domain-goat-pp-cli <command> --help
domain-goat-pp-cli <command> --dry-run --agent
```
