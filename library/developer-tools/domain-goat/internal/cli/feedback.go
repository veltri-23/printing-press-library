...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".domain-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOMAIN_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOMAIN_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "domain-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.domain-goat-pp-cli/feedback.jsonl.
When ` + "`DOMAIN_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOMAIN_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  domain-goat-pp-cli feedback list
  domain-goat-pp-cli feedback list --limit 5
  domain-goat-pp-cli feedback list --json`,
