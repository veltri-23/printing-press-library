...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  domain-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  domain-goat-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  domain-goat-pp-cli workflow status

  # Show status as JSON
  domain-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("domain-goat-pp-cli")
			}
