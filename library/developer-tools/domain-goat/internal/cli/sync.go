...
Exit codes & warnings:
  ...
  domain-goat-pp-cli sync

  # Sync specific resources only
  domain-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  domain-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  domain-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  domain-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  domain-goat-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/domain-goat-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "rdap_bootstrap":
		return db.UpsertRdapBootstrap(data)
	default:
