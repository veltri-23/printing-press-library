...
// apiErr classifies upstream errors into actionable cliError values with
// structured exit codes and hint text. Pattern-matches the most common HTTP
// status strings; falls back to a generic code:5 wrapper for everything else.
//
// Exit codes:
//
//	3  not found      — HTTP 404
//	4  auth           — HTTP 401, 403
//	5  generic API    — anything else
//	7  rate limit     — HTTP 429
//	8  already exists — HTTP 409 (used to be a hard error; --idempotent flag
//	                   was removed because no command in this CLI creates
//	                   resources upstream)
func apiErr(err error) error {
	...
	msg := err.Error()
	switch {
	case strings.Contains(msg, "HTTP 404") || strings.Contains(msg, "not found"):
		return &cliError{code: 3, err: fmt.Errorf("%w\nhint: resource not found. Run 'domain-goat-pp-cli doctor' to verify connectivity", err)}
	case strings.Contains(msg, "HTTP 401"):
		return &cliError{code: 4, err: fmt.Errorf("%w\nhint: check your API credentials. Run 'domain-goat-pp-cli doctor' to check auth status", err)}
	case strings.Contains(msg, "HTTP 403"):
		return &cliError{code: 4, err: fmt.Errorf("%w\nhint: permission denied — the service may be blocking by rate limit, geography, or bot protection. Run 'domain-goat-pp-cli doctor' to check connectivity", err)}
	case strings.Contains(msg, "HTTP 409") || strings.Contains(msg, "already exists"):
		return &cliError{code: 8, err: fmt.Errorf("%w\nhint: resource already exists; try the matching read or update command instead", err)}
	case strings.Contains(msg, "HTTP 429") || strings.Contains(msg, "rate limited"):
		return &cliError{code: 7, err: fmt.Errorf("%w\nhint: rate limited — wait the Retry-After window and retry, or lower --rate-limit", err)}
	default:
