...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "azure-devops-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AZURE_DEVOPS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AZURE_DEVOPS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "azure-devops-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/azure-devops-pp-cli/feedback.jsonl.
When ` + "`AZURE_DEVOPS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AZURE_DEVOPS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  azure-devops-pp-cli feedback list
  azure-devops-pp-cli feedback list --limit 5
  azure-devops-pp-cli feedback list --json`,
