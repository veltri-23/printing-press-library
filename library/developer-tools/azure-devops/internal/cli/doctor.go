...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  azure-devops-pp-cli doctor
  azure-devops-pp-cli doctor --json
  azure-devops-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export AZURE_DEVOPS_TOKEN=<your-pat> (generate at dev.azure.com/{org}/_usersSettings/tokens)"
				} else {
					authConfigured = true
			...
			// AZURE_DEVOPS_TOKEN is the canonical PAT env var; ADO_PAT is the fallback.
			if os.Getenv("AZURE_DEVOPS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "AZURE_DEVOPS_TOKEN")
			} else if os.Getenv("ADO_PAT") != "" {
				authEnvSet = append(authEnvSet, "ADO_PAT")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AZURE_DEVOPS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "azure-devops-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'azure-devops-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'azure-devops-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'azure-devops-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'azure-devops-pp-cli sync' to refresh."
	}
	return report
}
