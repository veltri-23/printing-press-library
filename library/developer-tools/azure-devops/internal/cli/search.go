...
In local mode: searches locally synced data only.`,
  ...
  azure-devops-pp-cli search "error timeout"

  # Force local search only
  azure-devops-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  azure-devops-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  azure-devops-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /{organization}/_apis/search/packagesearchresults
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/{organization}/_apis/search/packagesearchresults", map[string]any{
					"searchText":    query,
					"$orderBy":      []any{},
					"$skip":         nil,
					"$top":          nil,
			...
			// Local FTS search
			if dbPath == "" {
				dbPath = defaultDBPath("azure-devops-pp-cli")
			}

			db, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'azure-devops-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "apis":
				results, err = db.SearchApis(query, limit)
			case "build-apis":
				results, err = db.SearchBuildApis(query, limit)
			case "git-apis":
				results, err = db.SearchGitApis(query, limit)
			case "release-apis":
				results, err = db.SearchReleaseApis(query, limit)
			case "search-apis":
				results, err = db.SearchSearchApis(query, limit)
			case "wiki-apis":
				results, err = db.SearchWikiApis(query, limit)
			case "work-apis":
				results, err = db.SearchWorkApis(query, limit)
			case "workitemtracking-apis":
				results, err = db.SearchWorkitemtrackingApis(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search apis failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchBuildApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search build_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchGitApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search git_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchReleaseApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search release_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSearchApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search search_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchWikiApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search wiki_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchWorkApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search work_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchWorkitemtrackingApis(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search workitemtracking_apis failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/azure-devops-pp-cli/data.db)")
