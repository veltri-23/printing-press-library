...
Resource scoping:
  ...
  azure-devops-pp-cli sync

  # Sync specific resources only
  azure-devops-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  azure-devops-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  azure-devops-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  azure-devops-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  azure-devops-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "azure-devops-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "apis":
		return db.UpsertApis(data)
	case "build-apis":
		return db.UpsertBuildApis(data)
	case "git-apis":
		return db.UpsertGitApis(data)
	case "pipelines-apis":
		return db.UpsertPipelinesApis(data)
	case "release-apis":
		return db.UpsertReleaseApis(data)
	case "search-apis":
		return db.UpsertSearchApis(data)
	case "wiki-apis":
		return db.UpsertWikiApis(data)
	case "work-apis":
		return db.UpsertWorkApis(data)
	case "workitemtracking-apis":
		return db.UpsertWorkitemtrackingApis(data)
	default:
