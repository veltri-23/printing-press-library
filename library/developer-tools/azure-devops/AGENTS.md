# Azure DevOps Printed CLI Agent Guide

This directory is a generated `azure-devops-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
azure-devops-pp-cli doctor --json
azure-devops-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
azure-devops-pp-cli which "<capability>" --json
azure-devops-pp-cli <command> --help
```
...
```bash
azure-devops-pp-cli <command> --agent
```
...
```bash
azure-devops-pp-cli <command> --help
azure-devops-pp-cli <command> --dry-run --agent
```
