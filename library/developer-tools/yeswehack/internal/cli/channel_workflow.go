...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  yeswehack-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  yeswehack-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"business_units", "events", "hacktivity", "programs", "ranking", "taxonomies", "taxonomies-urls", "taxonomies-vulnerable-part", "user", "user-email-aliases", "user-invitations"}
			totalSynced := 0
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/yeswehack-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  yeswehack-pp-cli workflow status

  # Show status as JSON
  yeswehack-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("yeswehack-pp-cli")
			}
