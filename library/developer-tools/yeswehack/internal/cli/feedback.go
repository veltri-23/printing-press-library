...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".yeswehack-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YESWEHACK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YESWEHACK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "yeswehack-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.yeswehack-pp-cli/feedback.jsonl.
When ` + "`YESWEHACK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YESWEHACK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yeswehack-pp-cli feedback list
  yeswehack-pp-cli feedback list --limit 5
  yeswehack-pp-cli feedback list --json`,
