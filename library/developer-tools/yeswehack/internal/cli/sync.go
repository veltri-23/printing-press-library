...
Exit codes & warnings:
  ...
  yeswehack-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  yeswehack-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  yeswehack-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  yeswehack-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  yeswehack-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"taxonomies-urls":            "/utils/profile/urls",
		"taxonomies-vulnerable-part": "/types/vulnerable-part",
		"user":                       "/user/reports",
