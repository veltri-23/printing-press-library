# YesWeHack Printed CLI Agent Guide

This directory is a generated `yeswehack-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
yeswehack-pp-cli doctor --json
yeswehack-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
yeswehack-pp-cli which "<capability>" --json
...
```bash
yeswehack-pp-cli <command> --agent
```
...
```bash
yeswehack-pp-cli <command> --help
yeswehack-pp-cli <command> --dry-run --agent
```
