...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Trigger Dev CLI — Every Trigger.dev management endpoint, plus offline FTS over runs, span-cost rollups, and zombie-schedule detection no …`,
		Long: `Trigger Dev CLI — Every Trigger.dev management endpoint, plus offline FTS over runs, span-cost rollups, and zombie-schedule detection no …
...
Highlights (not in the official API docs):
  ...
  • runs get   runs get returns typed exit codes: 0=COMPLETED, 20=FAILED, 21=CRASHED, 22=SYSTEM_FAILURE, 23=CANCELED, 3=not-found, 4=auth-error. Cobra annotation pp:typed-exit-codes makes verify and agents read the…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'trigger-dev-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'trigger-dev-pp-cli profile list')")
	...
	rootCmd.AddCommand(newRunsCmd(flags))
	rootCmd.AddCommand(newSchedulesCmd(flags))
	rootCmd.AddCommand(newWaitpointsCmd(flags))
	...
	rootCmd.AddCommand(newEnvvarsCmd(flags))
