# Supabase Printed CLI Agent Guide

This directory is a generated `supabase-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
supabase-pp-cli doctor --json
supabase-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
supabase-pp-cli which "<capability>" --json
supabase-pp-cli <command> --help
```
...
```bash
supabase-pp-cli <command> --help
supabase-pp-cli <command> --dry-run --agent
```
