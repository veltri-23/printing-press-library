...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  supabase-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  supabase-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"organizations", "projects", "snippets", "supabase-profile"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  supabase-pp-cli workflow status

  # Show status as JSON
  supabase-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("supabase-pp-cli")
			}
