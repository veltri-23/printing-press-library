...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".supabase-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUPABASE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUPABASE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "supabase-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.supabase-pp-cli/feedback.jsonl.
When ` + "`SUPABASE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUPABASE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  supabase-pp-cli feedback list
  supabase-pp-cli feedback list --limit 5
  supabase-pp-cli feedback list --json`,
