...
Exit codes & warnings:
  ...
  supabase-pp-cli sync

  # Sync specific resources only
  supabase-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  supabase-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  supabase-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  supabase-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  supabase-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "diff":
		return db.UpsertDiff(data)
	case "merge":
		return db.UpsertMerge(data)
	case "push":
		return db.UpsertPush(data)
	case "reset":
		return db.UpsertReset(data)
	case "branches_restore":
		return db.UpsertBranchesRestore(data)
	case "organizations":
	...
	case "project_claim":
		return db.UpsertProjectClaim(data)
	...
	case "projects_branches":
		return db.UpsertProjectsBranches(data)
	case "claim_token":
		return db.UpsertClaimToken(data)
	case "cli":
		return db.UpsertCli(data)
	case "config":
		return db.UpsertConfig(data)
	case "custom_hostname":
		return db.UpsertCustomHostname(data)
	case "database":
		return db.UpsertDatabase(data)
	case "functions":
		return db.UpsertFunctions(data)
	case "health":
		return db.UpsertHealth(data)
	case "jit_access":
		return db.UpsertJitAccess(data)
	case "network_bans":
		return db.UpsertNetworkBans(data)
	case "network_restrictions":
		return db.UpsertNetworkRestrictions(data)
	case "pause":
		return db.UpsertPause(data)
	case "pgsodium":
		return db.UpsertPgsodium(data)
	case "postgrest":
		return db.UpsertPostgrest(data)
	case "read_replicas":
		return db.UpsertReadReplicas(data)
	case "readonly":
		return db.UpsertReadonly(data)
	case "projects_restore":
		return db.UpsertProjectsRestore(data)
	case "secrets":
		return db.UpsertSecrets(data)
	case "ssl_enforcement":
		return db.UpsertSslEnforcement(data)
	case "storage":
		return db.UpsertStorage(data)
	case "types":
		return db.UpsertTypes(data)
	case "upgrade":
		return db.UpsertUpgrade(data)
	case "vanity_subdomain":
		return db.UpsertVanitySubdomain(data)
	case "snippets":
		return db.UpsertSnippets(data)
	case "supabase-profile":
		return db.UpsertSupabaseProfile(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "actions", ParentTable: "projects", ParentIDParam: "ref", PathTemplate: "/v1/projects/{ref}/actions", KeyField: ""},
		{Name: "organizations_projects", ParentTable: "organizations", ParentIDParam: "slug", PathTemplate: "/v1/organizations/{slug}/projects", KeyField: ""},
	}
}
