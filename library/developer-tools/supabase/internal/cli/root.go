...
The official Supabase CLI is local-dev tooling (Docker, migrations, types). This CLI is the API-surface companion, with --json --select --dry-run consistency across every command.

Cross-project queries (secrets where-name, branches drift, functions inventory) operate over the local store; auth-admin lookup, pgrst schema, and storage usage hit live project APIs.

Known gaps (see README "## Known Gaps"): Auth Admin user CRUD, Storage object lifecycle, PostgREST row CRUD, and Edge Function runtime invoke are not wrapped — use supabase-js or the dashboard for those.
...
Health check: run 'supabase-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("supabase-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'supabase-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBranchesCmd(flags))
	...
	rootCmd.AddCommand(newAuthAdminCmd(flags))
	rootCmd.AddCommand(newPgrstCmd(flags))
	rootCmd.AddCommand(newStorageTopCmd(flags))
	...
	rootCmd.AddCommand(newSupabaseProfilePromotedCmd(flags))
