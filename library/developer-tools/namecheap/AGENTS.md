# Namecheap Printed CLI Agent Guide

This directory is a generated `namecheap-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
namecheap-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
namecheap-pp-cli which "<capability>" --json
...
```bash
namecheap-pp-cli <command> --agent
```
...
```bash
namecheap-pp-cli <command> --help
namecheap-pp-cli <command> --dry-run --agent
```
