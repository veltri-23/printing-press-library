...
// TestMigrate_AddsColumnsOnUpgrade_XmlResponse verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_XmlResponse(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE xml_response (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(xml_response)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from xml_response after migrate", want)
		}
	}
}
