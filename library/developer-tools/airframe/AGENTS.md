# airframe Printed CLI Agent Guide

This directory is a generated `airframe-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
airframe-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airframe-pp-cli which "<capability>" --json
airframe-pp-cli <command> --help
```
...
```bash
airframe-pp-cli <command> --help
airframe-pp-cli <command> --dry-run --agent
```
