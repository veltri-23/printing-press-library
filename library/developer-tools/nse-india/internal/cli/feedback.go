...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".nse-india-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NSE_INDIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NSE_INDIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nse-india-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.nse-india-pp-cli/feedback.jsonl.
When ` + "`NSE_INDIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NSE_INDIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nse-india-pp-cli feedback list
  nse-india-pp-cli feedback list --limit 5
  nse-india-pp-cli feedback list --json`,
