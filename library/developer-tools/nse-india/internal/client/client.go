...
func newHTTPClient(timeout time.Duration, jar http.CookieJar) *http.Client {
	// NSE India requires HTTP/1.1 — their servers return HTTP/2 INTERNAL_ERROR on Go's default H2 transport.
	// Force HTTP/1.1 by overriding TLSNextProto with an empty map.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "nse-india-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// NSE India requires browser-like headers or requests are blocked.
