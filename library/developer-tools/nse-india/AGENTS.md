# NSE India Printed CLI Agent Guide

This directory is a generated `nse-india-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nse-india-pp-cli which "<capability>" --json
nse-india-pp-cli <command> --help
```
...
```bash
nse-india-pp-cli <command> --agent
```
...
```bash
nse-india-pp-cli <command> --help
nse-india-pp-cli <command> --dry-run --agent
```
