...
func TestUpsertBatch_PopulatesDeepResearchTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("deep_research", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
...
// TestUpsertBatch_PopulatesFirecrawlSearchTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed firecrawl_search table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesFirecrawlSearchTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("firecrawl_search", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "firecrawl_search").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "firecrawl_search")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count firecrawl_search: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("firecrawl_search count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
...
func TestUpsertBatch_PopulatesExtractTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("extract", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
...
func TestUpsertBatch_PopulatesLlmstxtTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("llmstxt", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
