...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "scrape-do-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCRAPE_DO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCRAPE_DO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "scrape-do-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/scrape-do-pp-cli/feedback.jsonl.
When ` + "`SCRAPE_DO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCRAPE_DO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scrape-do-pp-cli feedback list
  scrape-do-pp-cli feedback list --limit 5
  scrape-do-pp-cli feedback list --json`,
