...
type Config struct {
	...
	Path           string            `toml:"-"`
	ScrapedoApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SCRAPE_DO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "scrape-do-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SCRAPEDO_API_KEY"); v != "" {
		cfg.ScrapedoApiKey = v
		cfg.AuthSource = "env:SCRAPEDO_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ScrapedoApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SCRAPE_DO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		...
		"SCRAPEDO_API_KEY": c.ScrapedoApiKey,
	}
