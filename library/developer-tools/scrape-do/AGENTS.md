# Scrape.do Printed CLI Agent Guide

This directory is a generated `scrape-do-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
scrape-do-pp-cli doctor --json
scrape-do-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
scrape-do-pp-cli which "<capability>" --json
scrape-do-pp-cli <command> --help
```
...
```bash
scrape-do-pp-cli <command> --agent
```
...
```bash
scrape-do-pp-cli <command> --help
scrape-do-pp-cli <command> --dry-run --agent
```
