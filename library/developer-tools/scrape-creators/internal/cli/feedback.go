...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SCRAPE_CREATORS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SCRAPE_CREATORS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "scrape-creators-pp-cli/feedback")
...
When ` + "`SCRAPE_CREATORS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SCRAPE_CREATORS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scrape-creators-pp-cli feedback list
  scrape-creators-pp-cli feedback list --limit 5
  scrape-creators-pp-cli feedback list --json`,
