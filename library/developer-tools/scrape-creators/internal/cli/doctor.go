...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  scrape-creators-pp-cli doctor
  ...
  scrape-creators-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("SCRAPECREATORS_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "SCRAPECREATORS_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SCRAPECREATORS_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "scrape-creators-pp-cli"
						verifyPath := "/v1/github/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'scrape-creators-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'scrape-creators-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	default:
		...
		report["hint"] = "Some resources are older than stale_after; run 'scrape-creators-pp-cli sync' to refresh."
	}
	return report
}
