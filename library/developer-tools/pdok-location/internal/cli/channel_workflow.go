...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pdok-location-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pdok-location-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "collections-adres", "collections-functioneel-gebied", "collections-gebouw", "collections-gemeentegebied", "collections-geografisch-gebied", "collections-inrichtingselement", "collections-perceel", "collections-plaats", "collections-provinciegebied", "collections-spoorbaandeel", "collections-waterdeel", "collections-wegdeel", "collections-woonplaats", "free", "reverse"}
			totalSynced := 0

			// Suppress syncResource's per-resource NDJSON event stream when
			// the parent is rendering a single JSON document. Without this,
			// `workflow archive --json` stdout interleaves NDJSON events
			// with the final summary object and `json_fidelity` probes
			// reject it as not-a-single-JSON-document. The events stay
			// visible in the default (non-JSON) human path because sync.go
			// gates them on humanFriendly being false; we don't change that
			// global — we just redirect os.Stdout to /dev/null while sync
			// runs, then restore it before emitting the summary so the
			// final JSON object still reaches the real stdout.
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pdok-location-pp-cli workflow status

  # Show status as JSON
  pdok-location-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pdok-location-pp-cli")
			}
