...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pdok-location-pp-cli doctor
  pdok-location-pp-cli doctor --json
  pdok-location-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Step 3: Per-source probes. PDOK Location bundles TWO
					// upstream services under one base URL; the base-URL
					// probe above does not distinguish which one is down.
					// Probe each explicitly so users see which surface is
					// available right now.
					sources := map[string]any{}
					if _, lsErr := c.Get("/bzk/locatieserver/search/v3_1/free", map[string]string{"q": "amsterdam", "rows": "1", "fl": "id"}); lsErr == nil {
						sources["locatieserver"] = map[string]any{
							"status": "reachable",
							"base":   "/bzk/locatieserver/search/v3_1",
							"role":   "primary — text geocoding (Solr): geocode/free, suggest, lookup, reverse",
						}
					} else {
						sources["locatieserver"] = map[string]any{
							"status": "unreachable",
							...
							"role":   "primary — text geocoding (Solr): geocode/free, suggest, lookup, reverse",
							"error":  lsErr.Error(),
							"impact": "geocode/free, suggest, lookup, reverse, resolve, top, nearest, gemeente of-point, batch geocode, perceel lookup, search will fail. features search (OGC) may still work.",
						}
					}
					if _, kErr := c.Get("/kadaster/location-api/v1/conformance", nil); kErr == nil {
						sources["kadaster_location_api"] = map[string]any{
							"status": "reachable",
							"base":   "/kadaster/location-api/v1",
							"role":   "secondary — OGC API Features: collections, conformance, features search",
						}
					} else {
						sources["kadaster_location_api"] = map[string]any{
							"status": "unreachable",
							...
							"role":   "secondary — OGC API Features: collections, conformance, features search",
							"error":  kErr.Error(),
							"impact": "collections, conformance, features search, features get, and perceel lookup --aanduiding will fail. All Locatieserver-backed commands above continue to work.",
						}
					}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'pdok-location-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'pdok-location-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'pdok-location-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'pdok-location-pp-cli sync' to refresh."
	}
	return report
}
