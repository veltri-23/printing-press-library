...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pdok-location-pp-cli",
		Short: `Pdok Location CLI — One Go binary for every Dutch location workflow — geocode, reverse-geocode, batch CSVs, full GeoJSON geometries, and RD…`,
		Long: `Pdok Location CLI — One Go binary for every Dutch location workflow — geocode, reverse-geocode, batch CSVs, full GeoJSON geometries, and RD…
...
Health check: run 'pdok-location-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pdok-location-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pdok-location-pp-cli profile list')")
	...
	rootCmd.AddCommand(newConformancePromotedCmd(flags))
	rootCmd.AddCommand(newFreePromotedCmd(flags))
	rootCmd.AddCommand(newLookupPromotedCmd(flags))
	rootCmd.AddCommand(newPdokLocationApiPromotedCmd(flags))
	// Intentionally NOT registered: newPdokLocationSearchPromotedCmd. The raw
	// /search endpoint requires per-collection opt-in via bracketed
	// `<col>[version]=1` syntax that this generator's generic command can't
	// construct. Use `features search` instead (registered below).
	// rootCmd.AddCommand(newPdokLocationSearchPromotedCmd(flags))
	rootCmd.AddCommand(newReversePromotedCmd(flags))
	rootCmd.AddCommand(newSuggestPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Novel commands (hand-built per the absorb manifest's transcendence rows).
	rootCmd.AddCommand(newResolveCmd(flags))
	rootCmd.AddCommand(newTopCmd(flags))
	rootCmd.AddCommand(newNearestCmd(flags))
	rootCmd.AddCommand(newConvertCmd(flags))
	rootCmd.AddCommand(newGemeenteCmd(flags))
	rootCmd.AddCommand(newProvincieCmd(flags))
	rootCmd.AddCommand(newBatchCmd(flags))
	rootCmd.AddCommand(newFeaturesCmd(flags))
	rootCmd.AddCommand(newPerceelCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("pdok-location-pp-cli %s\n", version)
		},
	}
}
