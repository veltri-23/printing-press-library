...
In local mode: searches locally synced data only.`,
  ...
  pdok-location-pp-cli search "error timeout"

  # Force local search only
  pdok-location-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pdok-location-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pdok-location-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// Use Locatieserver /free as the live text-search fallback. The
			// other candidate (Kadaster /search) requires per-collection
			// opt-in via bracketed <col>[version]=1 params that this
			// generic generated path can't construct; callers wanting the
			// OGC surface should use `features search` instead.
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/bzk/locatieserver/search/v3_1/free", map[string]string{
					"q":    query,
					"rows": "10",
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pdok-location-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
