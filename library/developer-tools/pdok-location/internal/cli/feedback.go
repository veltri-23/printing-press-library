...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pdok-location-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PDOK_LOCATION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PDOK_LOCATION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pdok-location-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pdok-location-pp-cli/feedback.jsonl.
When ` + "`PDOK_LOCATION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PDOK_LOCATION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pdok-location-pp-cli feedback list
  pdok-location-pp-cli feedback list --limit 5
  pdok-location-pp-cli feedback list --json`,
