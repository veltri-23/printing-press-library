# Apple Developer Documentation Printed CLI Agent Guide

This directory is a generated `apple-docs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
apple-docs-pp-cli doctor --json
apple-docs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
apple-docs-pp-cli which "<capability>" --json
apple-docs-pp-cli <command> --help
```
...
```bash
apple-docs-pp-cli <command> --agent
```
...
```bash
apple-docs-pp-cli <command> --help
apple-docs-pp-cli <command> --dry-run --agent
```
