...
Resource scoping:
  ...
  apple-docs-pp-cli sync

  # Sync specific resources only
  apple-docs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  apple-docs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  apple-docs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  apple-docs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  apple-docs-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("apple-docs-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/apple-docs-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "doc":
		return db.UpsertDoc(data)
	case "technologies":
		return db.UpsertTechnologies(data)
	default:
...
func defaultSyncResources() []string {
	// Apple DocC's JSON endpoints return single documents, not lists; they
	// don't fit the generic list-and-paginate sync shape. Returning empty here
	// makes `apple-docs-pp-cli sync` a no-op that exits 0; users should reach
	// for `snapshot save <framework>` to capture index snapshots, or just rely
	// on the per-command HTTP caching the client already does.
	return nil
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"technologies": "/tutorials/data/documentation/technologies.json",
	}
