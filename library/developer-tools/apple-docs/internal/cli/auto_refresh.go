...
var readCommandResources = map[string][]string{
	"apple-docs-pp-cli technologies":        {"technologies"},
	"apple-docs-pp-cli technologies list":   {"technologies"},
	"apple-docs-pp-cli technologies get":    {"technologies"},
	"apple-docs-pp-cli technologies search": {"technologies"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "APPLE_DOCS_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale apple-docs-pp-cli cache (refresh failed: %v)\n", err)
