...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "apple-docs-pp-cli",
		Short: `The fastest way to read Apple's developer docs from your terminal — offline FTS, deprecation cliff reports, and an MCP server in one Go binary.`,
		Long: `The fastest way to read Apple's developer docs from your terminal — offline FTS, deprecation cliff reports, and an MCP server in one Go binary.
...
Health check: run 'apple-docs-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("apple-docs-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'apple-docs-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelConformanceCmd(flags))
	rootCmd.AddCommand(newNovelDeprecationCliffCmd(flags))
	rootCmd.AddCommand(newNovelDocCmd(flags))
	rootCmd.AddCommand(newNovelGrepCmd(flags))
	rootCmd.AddCommand(newNovelPortToCmd(flags))
	rootCmd.AddCommand(newNovelSnapshotCmd(flags))
	rootCmd.AddCommand(newNovelWwdcCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	// newDocPromotedCmd was registered here, but the novel `doc` parent
	// (above) shadows it — Cobra resolves `doc <path>` to the novel
	// command, which has no positional. Removing the promoted form
	// eliminates the dead surface so SKILL Command Reference stops
	// suggesting `doc <path>` and the verify-skill positional-args
	// warning goes quiet. Use `doc get <path>` instead.
	rootCmd.AddCommand(newIndexPromotedCmd(flags))
	rootCmd.AddCommand(newTechnologiesPromotedCmd(flags))
