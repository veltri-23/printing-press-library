...
func TestUpsertBatch_PopulatesDocTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("doc", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "doc").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "doc")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count doc: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("doc count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
