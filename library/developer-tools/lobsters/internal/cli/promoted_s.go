...
func newSPromotedCmd(flags *rootFlags) *cobra.Command {
			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "short_id"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("short_id is required\nUsage: %s <%s>", cmd.CommandPath(), "short_id"))
			}
			path = replacePathParam(path, "short_id", args[0])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "s", false, path, params, nil)
			if err != nil {
