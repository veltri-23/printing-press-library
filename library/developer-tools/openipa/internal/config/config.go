...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OPENIPA_CONFIG")
	}
	if path == "" {
		home, err := os.UserHomeDir()
		if err != nil {
			return nil, fmt.Errorf("impossibile determinare la home directory: %w", err)
		}
		path = filepath.Join(home, ".config", "openipa-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("IPA_auth_id"); v != "" {
		cfg.IpaAuthId = v
		cfg.AuthSource = "env:IPA_auth_id"
	}
	...
	if cfg.AuthSource == "" && cfg.IpaAuthId != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("OPENIPA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
