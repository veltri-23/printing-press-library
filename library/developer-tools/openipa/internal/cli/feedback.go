...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".openipa-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENIPA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENIPA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/developer-tools/openipa/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.openipa-pp-cli/feedback.jsonl.
When ` + "`OPENIPA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENIPA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openipa-pp-cli feedback list
  openipa-pp-cli feedback list --limit 5
  openipa-pp-cli feedback list --json`,
