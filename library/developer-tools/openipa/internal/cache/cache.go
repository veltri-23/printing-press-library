...
// Set stores a value in the cache. Errors are silently swallowed — cache
// misses fall back to live API calls, so a write failure is non-fatal.
func (s *Store) Set(key string, value json.RawMessage) error {
	if err := os.MkdirAll(s.Dir, 0o755); err != nil {
		return err
	}
	return os.WriteFile(s.path(key), []byte(value), 0o644)
}
