# IndicePA Printed CLI Agent Guide

This directory is a generated `openipa-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
openipa-pp-cli doctor --json
openipa-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
openipa-pp-cli which "<capability>" --json
openipa-pp-cli <command> --help
```
...
```bash
openipa-pp-cli <command> --help
openipa-pp-cli <command> --dry-run --agent
```
