...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github-contents-pp-cli doctor
  github-contents-pp-cli doctor --json
  github-contents-pp-cli doctor --fail-on warn
  github-contents-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export GITHUB_TOKEN=\"your-token-here\" GH_TOKEN=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("GITHUB_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GITHUB_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GITHUB_TOKEN")
			}
			if os.Getenv("GH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "GH_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "GH_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "github-contents-pp-cli", suggestion)
						} else {
