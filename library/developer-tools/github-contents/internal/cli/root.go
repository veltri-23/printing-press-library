...
type rootFlags struct {
	...
	// invocation. Mirrors the GITHUB_CONTENTS_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "github-contents-pp-cli",
		Short: `Github Contents CLI — Download any GitHub folder without cloning — plan it first, verify it after, re-sync only what changed.`,
		Long: `Github Contents CLI — Download any GitHub folder without cloning — plan it first, verify it after, re-sync only what changed.
...
Health check: run 'github-contents-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("github-contents-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'github-contents-pp-cli profile list')")
	...
	// releases download is a hand-written child wired here via local-variable
	// capture rather than by editing the generated releases.go.
	releasesCmd := newReleasesCmd(flags)
	releasesCmd.AddCommand(newReleasesDownloadCmd(flags))
	rootCmd.AddCommand(releasesCmd)
	rootCmd.AddCommand(newReposCmd(flags))
	...
	rootCmd.AddCommand(newNovelSyncDirCmd(flags))
	rootCmd.AddCommand(newNovelVerifyCmd(flags))
	rootCmd.AddCommand(newFetchCmd(flags))
	rootCmd.AddCommand(newTarballCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newBlobsPromotedCmd(flags))
	rootCmd.AddCommand(newContentsPromotedCmd(flags))
	rootCmd.AddCommand(newRateLimitPromotedCmd(flags))
	rootCmd.AddCommand(newTreesPromotedCmd(flags))
