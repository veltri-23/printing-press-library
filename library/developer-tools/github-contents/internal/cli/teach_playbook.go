...
func newTeachPlaybookCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:happy-args use the inline --notes path (no fixture file
		// needed) and only write local learning state.
		Annotations: map[string]string{
			"pp:typed-exit-codes": "0,2",
			"mcp:local-write":     "true",
			"pp:happy-args":       "--query=how to mirror a github folder;--notes=prefer sync-dir for repeat downloads",
		},
...
agents can record playbooks without a file on disk.`,
		Example: `  github-contents-pp-cli teach-playbook \
...
func newPlaybookAmendCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:happy-args create a notes-only playbook when the family has
		// none, so the invocation exits 0 even on a fresh store; only
		// local learning state is written.
		Annotations: map[string]string{
			"pp:typed-exit-codes": "0,2",
			"mcp:local-write":     "true",
			"pp:happy-args":       "--query=how to mirror a github folder;--add-note=raw CDN escapes spaces via EscapePath",
		},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  github-contents-pp-cli playbook amend \
...
func newPlaybookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     `  github-contents-pp-cli playbook list --agent`,
