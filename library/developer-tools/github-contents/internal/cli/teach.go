...
// github-contents-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/developer-tools/github-contents/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/developer-tools/github-contents/internal/learn/patterns"
...
const noLearnEnvVar = "GITHUB_CONTENTS_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("github-contents-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:happy-args: --quiet=false is included so the json_fidelity
		// probe (happy invocation + --json) gets a JSON envelope instead of
		// teach's default fire-and-forget silence. Writes only local
		// learning state.
		Annotations: map[string]string{
			"pp:typed-exit-codes": "0,2",
			"mcp:local-write":     "true",
			"pp:happy-args":       "--query=which repo has the transformers book;--resource-type=trees;--resource=mjwoon/AI-readings@main:books/LLM/transformers-book.pdf;--quiet=false",
		},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  github-contents-pp-cli teach --query "<question>" --resource-type <type> \
    --resource <id> --resource <id> &`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// --json always emits the envelope: agents that ask for JSON must
			// never get empty stdout, regardless of the --quiet default.
			// (Divergence from the generator template, filed for upstream.)
			if flags.asJSON {
...
when learnings exist.`,
		Example: `  github-contents-pp-cli recall "<question>" --agent
  github-contents-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github-contents-pp-cli learnings list --agent
  github-contents-pp-cli learnings list --query "<substring>"
  github-contents-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  github-contents-pp-cli learnings forget "<question>" --resource <id>
  github-contents-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
func newTeachPatternCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// pp:happy-args mirror the command's own Example values; the only
		// write is one row in the local search_patterns table.
		Annotations: map[string]string{
...
a whole family.`,
		Example: `  github-contents-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  github-contents-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
