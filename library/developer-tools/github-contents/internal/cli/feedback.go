...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GITHUB_CONTENTS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GITHUB_CONTENTS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github-contents-pp-cli/feedback")
...
When ` + "`GITHUB_CONTENTS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GITHUB_CONTENTS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  github-contents-pp-cli feedback list
  github-contents-pp-cli feedback list --limit 5
  github-contents-pp-cli feedback list --json`,
