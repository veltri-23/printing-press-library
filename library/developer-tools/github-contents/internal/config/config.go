...
type Config struct {
	...
	GithubToken      string          `toml:"token"`
	GhToken          string          `toml:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if v := os.Getenv("GITHUB_TOKEN"); v != "" {
		cfg.GithubToken = v
		cfg.markEnvOverride("GithubToken")
		cfg.AuthSource = "env:GITHUB_TOKEN"
		cfg.CredentialSource = "env:GITHUB_TOKEN"
	}
	if v := os.Getenv("GH_TOKEN"); v != "" {
		cfg.GhToken = v
		cfg.markEnvOverride("GhToken")
		cfg.AuthSource = "env:GH_TOKEN"
		cfg.CredentialSource = "env:GH_TOKEN"
	}
	...
	if v := os.Getenv("GITHUB_CONTENTS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("GITHUB_CONTENTS_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "github-contents-pp-cli", "config.toml"), nil
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.GithubToken = creds.GithubToken
	c.GhToken = creds.GhToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GithubToken = ""
	delete(c.envOverrides, "GithubToken")
	c.updateFileConfigField("GithubToken")
	c.GhToken = ""
	delete(c.envOverrides, "GhToken")
	c.updateFileConfigField("GhToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GithubToken"] {
			out.GithubToken = c.fileConfig.GithubToken
		}
		if c.envOverrides["GhToken"] {
			out.GhToken = c.fileConfig.GhToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GithubToken":
		c.fileConfig.GithubToken = c.GithubToken
	case "GhToken":
		c.fileConfig.GhToken = c.GhToken
	}
}
