...
type Config struct {
	...
	Path              string            `toml:"-"`
	SecEdgarUserAgent string            `toml:"edgar_user_agent"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SEC_EDGAR_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sec-edgar-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("SEC_EDGAR_USER_AGENT"); v != "" {
		cfg.SecEdgarUserAgent = v
		cfg.AuthSource = "env:SEC_EDGAR_USER_AGENT"
	}
	...
	if cfg.AuthSource == "" && cfg.SecEdgarUserAgent != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SEC_EDGAR_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
