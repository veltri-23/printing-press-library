...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".sec-edgar-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SEC_EDGAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SEC_EDGAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sec-edgar-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.sec-edgar-pp-cli/feedback.jsonl.
When ` + "`SEC_EDGAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SEC_EDGAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sec-edgar-pp-cli feedback list
  sec-edgar-pp-cli feedback list --limit 5
  sec-edgar-pp-cli feedback list --json`,
