...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'sec-edgar-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set it with: export SEC_EDGAR_USER_AGENT=<your-key>"+
			"\n      Get a key at: https://www.sec.gov/os/accessing-edgar-data"+
			"\n      Run 'sec-edgar-pp-cli doctor' to check auth status.", err))
	...
	case strings.Contains(msg, "HTTP 404"):
		return notFoundErr(fmt.Errorf("%w\nhint: resource not found on SEC EDGAR. Double-check the identifier or period — XBRL frames don't exist for every concept × period (try a different --period, or 'companies lookup <ticker>' to resolve a CIK)", err))
