...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sec-edgar-pp-cli doctor
  sec-edgar-pp-cli doctor --json
  sec-edgar-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SEC_EDGAR_USER_AGENT=<your-key>"
					report["auth_key_url"] = "https://www.sec.gov/os/accessing-edgar-data"
				} else {
					authConfigured = true
			...
			if os.Getenv("SEC_EDGAR_USER_AGENT") != "" {
				authEnvSet = append(authEnvSet, "SEC_EDGAR_USER_AGENT")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SEC_EDGAR_USER_AGENT")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("sec-edgar-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'sec-edgar-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'sec-edgar-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'sec-edgar-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'sec-edgar-pp-cli sync' to refresh."
	}
	return report
}
