...
Exit codes & warnings:
  ...
  sec-edgar-pp-cli sync

  # Sync specific resources only
  sec-edgar-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sec-edgar-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sec-edgar-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sec-edgar-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sec-edgar-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/sec-edgar-pp-cli/data.db)")
