...
// TestMigrate_AddsColumnsOnUpgrade_Json verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Json(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE json (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(json)`)
	if err != nil {
