...
Exit codes & warnings:
  ...
  nvd-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nvd-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nvd-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nvd-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nvd-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nvd-pp-cli/data.db)")
