...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".nvd-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NVD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NVD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nvd-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.nvd-pp-cli/feedback.jsonl.
When ` + "`NVD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NVD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
