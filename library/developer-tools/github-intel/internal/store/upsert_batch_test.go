...
// TestUpsertBatch_KeysReleasesByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysReleasesByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "repos_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "repos_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "repos_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "repos_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("releases", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("releases count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "repos_id" = ?`, "releases")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by repos_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("repos_id=parent_A count = %d, want 2", parentMatches)
	}
}
