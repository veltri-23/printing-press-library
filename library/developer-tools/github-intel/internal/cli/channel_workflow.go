...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  github-intel-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  github-intel-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  github-intel-pp-cli workflow status

  # Show status as JSON
  github-intel-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("github-intel-pp-cli")
			}
