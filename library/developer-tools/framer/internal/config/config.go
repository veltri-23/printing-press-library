...
type Config struct {
	...
	FramerApiKey  string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://framer.com/api",
	}
	...
	// Env var overrides
	if v := os.Getenv("FRAMER_API_KEY"); v != "" {
		cfg.FramerApiKey = v
		cfg.markEnvOverride("FramerApiKey")
		cfg.AuthSource = "env:FRAMER_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.FramerApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("FRAMER_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.FramerApiKey = token
	delete(c.envOverrides, "FramerApiKey")
	c.updateFileConfigField("FramerApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FramerApiKey"] {
			out.FramerApiKey = c.fileConfig.FramerApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FramerApiKey":
		c.fileConfig.FramerApiKey = c.FramerApiKey
	}
}
