...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  framer-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  framer-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"changes", "cms-collections", "code", "fonts", "nodes", "pages", "redirects", "styles-colors", "styles-text"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  framer-pp-cli workflow status

  # Show status as JSON
  framer-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("framer-pp-cli")
			}
