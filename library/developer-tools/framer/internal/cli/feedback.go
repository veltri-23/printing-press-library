...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "framer-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FRAMER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FRAMER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "framer-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/framer-pp-cli/feedback.jsonl.
When ` + "`FRAMER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FRAMER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  framer-pp-cli feedback list
  framer-pp-cli feedback list --limit 5
  framer-pp-cli feedback list --json`,
