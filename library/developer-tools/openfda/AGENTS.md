# Openfda Printed CLI Agent Guide

This directory is a generated `openfda-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
openfda-pp-cli doctor --json
openfda-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
openfda-pp-cli which "<capability>" --json
...
```bash
openfda-pp-cli <command> --help
openfda-pp-cli <command> --dry-run --agent
```
