...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OPENFDA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "openfda-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("FDA_API_KEY"); v != "" {
		cfg.FdaApiKey = v
		cfg.AuthSource = "env:FDA_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.FdaApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("OPENFDA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
