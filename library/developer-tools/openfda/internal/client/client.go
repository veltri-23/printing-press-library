...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "openfda-pp-cli")
...
// encodeOpenFDAParams builds a raw query string for OpenFDA API requests.
// The "search" parameter uses OpenFDA's Elasticsearch syntax with literal +
// signs for AND/OR operators and unescaped quotes around values. Standard
// url.Values.Encode() would percent-encode these, breaking the query.
// All other parameters are percent-encoded normally.
func encodeOpenFDAParams(params map[string]string) string {
	var parts []string
	for k, v := range params {
		...
		if k == "search" {
			// The search value uses OpenFDA's Elasticsearch syntax:
			// field:"value"+AND+field:"value"
			// The + signs are literal space replacements, and quotes delimit values.
			// Percent-encode only characters that would break URL parsing (space, &, =, #)
			// but preserve + and " as-is.
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if params != nil {
			req.URL.RawQuery = encodeOpenFDAParams(params)
		}

		if authHeader != "" {
			// API key goes in query parameter — append to existing raw query
			// to preserve OpenFDA search syntax that encodeOpenFDAParams set.
			if req.URL.RawQuery != "" {
				req.URL.RawQuery += "&api_key=" + url.QueryEscape(authHeader)
			} else {
				req.URL.RawQuery = "api_key=" + url.QueryEscape(authHeader)
			}
		}
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "openfda-pp-cli/1.0")
		}
