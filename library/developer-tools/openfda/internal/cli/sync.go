...
Exit codes & warnings:
  ...
  openfda-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openfda-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openfda-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openfda-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openfda-pp-cli sync --latest-only`,
...
func syncResource(c interface {
	...
	// Parse existing cursor as an integer offset.
	skipOffset := 0
	if existingCursor != "" && !full {
		if n, err := strconv.Atoi(existingCursor); err == nil {
			skipOffset = n
		}
	}
	_ = existingCursor // suppress unused warning
	...
	for {
		...
		// Set offset for OpenFDA pagination
		if skipOffset > 0 {
			params["skip"] = strconv.Itoa(skipOffset)
		}

		// Set since filter — OpenFDA uses search= range syntax, not a since param.
		// Convert RFC3339 timestamp to YYYYMMDD and use the date field for this resource.
		if effectiveSince != "" {
			dateField := syncResourceDateField(resource)
			if dateField != "" {
				sinceDate := effectiveSince
				// Parse RFC3339 to extract YYYYMMDD
				if t, err := time.Parse(time.RFC3339, effectiveSince); err == nil {
					sinceDate = t.Format("20060102")
				} else if len(effectiveSince) >= 10 {
					sinceDate = strings.ReplaceAll(effectiveSince[:10], "-", "")
				}
				searchClause := fmt.Sprintf("%s:[%s+TO+21000101]", dateField, sinceDate)
				if existing, ok := params["search"]; ok && existing != "" {
					params["search"] = existing + "+AND+" + searchClause
				} else {
					params["search"] = searchClause
				}
			}
		}
		...
		items, _, _ := extractPageItems(data, pageSize.cursorParam)
		...
		skipOffset += len(items)

		// Save offset as cursor for resumability
		if err := db.SaveSyncState(resource, strconv.Itoa(skipOffset), totalCount); err != nil {
		...
		// OpenFDA caps skip at 25000 — stop when we hit the limit
		if skipOffset >= 25000 {
			if humanFriendly {
				fmt.Fprintf(os.Stderr, "\n  %s: reached OpenFDA pagination limit (skip=25000)\n", resource)
			} else {
				fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","reason":"pagination_limit","message":"reached OpenFDA skip=25000 cap; use --since for incremental sync"}`+"\n", resource)
			}
			break
		}
...
// syncResourceDateField returns the date field used for incremental sync
// filtering for each resource type. OpenFDA uses search= range syntax:
// search=field:[YYYYMMDD+TO+YYYYMMDD].
func syncResourceDateField(resource string) string {
	fields := map[string]string{
		"drug-events":           "receiptdate",
		"drug-recalls":          "report_date",
		"drug-labels":           "effective_time",
		"drug-ndc":              "",
		"drug-approvals":        "",
		"drug-shortages":        "",
		"device-events":         "date_received",
		"device-510k":           "decision_date",
		"device-classification": "",
		"device-covid19":        "",
		"device-pma":            "decision_date",
		"device-recalls":        "report_date",
		"device-recall-detail":  "",
		"device-registration":   "",
		"device-udi":            "",
		"food-events":           "date_started",
		"food-recalls":          "report_date",
		"animal-events":         "",
		"tobacco-problems":      "date_submitted",
		"nsde":                  "",
		"substance":             "",
	}
	if f, ok := fields[resource]; ok {
		return f
	}
	return ""
}
