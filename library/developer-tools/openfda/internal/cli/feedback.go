...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".openfda-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENFDA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENFDA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openfda-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.openfda-pp-cli/feedback.jsonl.
When ` + "`OPENFDA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENFDA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openfda-pp-cli feedback list
  openfda-pp-cli feedback list --limit 5
  openfda-pp-cli feedback list --json`,
