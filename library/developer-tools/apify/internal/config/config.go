...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("APIFY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "apify-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("APIFY_TOKEN"); v != "" {
		cfg.ApifyToken = v
		cfg.AuthSource = "env:APIFY_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ApifyToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("APIFY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ApifyToken != "" {
		c.AuthSource = "env:APIFY_TOKEN"
		return "Bearer " + c.ApifyToken
	}
	if c.AccessToken != "" {
		c.AuthSource = "oauth2"
