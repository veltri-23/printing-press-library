...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".apify-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("APIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("APIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "apify-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.apify-pp-cli/feedback.jsonl.
When ` + "`APIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`APIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  apify-pp-cli feedback list
  apify-pp-cli feedback list --limit 5
  apify-pp-cli feedback list --json`,
