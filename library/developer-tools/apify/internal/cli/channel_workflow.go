...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  apify-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  apify-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"actor-builds", "actor-runs", "actor-tasks", "acts", "browser-info", "datasets", "key-value-stores", "request-queues", "schedules", "store", "users", "users-me-limits", "users-me-usage-monthly", "webhook-dispatches", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  apify-pp-cli workflow status

  # Show status as JSON
  apify-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("apify-pp-cli")
			}
