# Apify Printed CLI Agent Guide

This directory is a generated `apify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
apify-pp-cli doctor --json
apify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
apify-pp-cli which "<capability>" --json
apify-pp-cli <command> --help
```
...
```bash
apify-pp-cli <command> --help
apify-pp-cli <command> --dry-run --agent
```
