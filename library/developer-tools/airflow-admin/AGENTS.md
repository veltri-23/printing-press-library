# Airflow Admin Printed CLI Agent Guide

This directory is a generated `airflow-admin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
airflow-admin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airflow-admin-pp-cli which "<capability>" --json
airflow-admin-pp-cli <command> --help
```
...
```bash
airflow-admin-pp-cli <command> --agent
```
...
```bash
airflow-admin-pp-cli <command> --help
airflow-admin-pp-cli <command> --dry-run --agent
```
