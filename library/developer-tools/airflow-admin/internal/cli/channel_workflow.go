...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  airflow-admin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  airflow-admin-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"connections", "dags", "pools", "variables"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  airflow-admin-pp-cli workflow status

  # Show status as JSON
  airflow-admin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			dbPath := flags.dbPathOrDefault()
			s, err := store.OpenWithContext(cmd.Context(), dbPath)
			if err != nil {
