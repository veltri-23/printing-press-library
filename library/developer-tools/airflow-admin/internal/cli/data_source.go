...
func resolveReadWithStrategy(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'airflow-admin-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
var listEnvelopeMetadataKeys = map[string]bool{
	...
	"dags": true, "dag_runs": true, "task_instances": true,
	"tasks": true, "pools": true, "variables": true, "connections": true,
	"Results": true, "Data": true, "Items": true,
	"Records": true, "Nodes": true, "Entries": true, "Features": true,
...
func writeThroughCache(ctx context.Context, flags *rootFlags, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, flags.dbPathOrDefault())
	if err != nil {
		return
	}
	defer db.Close()
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, flags, "airflow-admin-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'airflow-admin-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'airflow-admin-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'airflow-admin-pp-cli sync' first", resourceType)
		}
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'airflow-admin-pp-cli sync' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
