...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "airflow-admin-pp-cli",
		Short: "Manage airflow-admin resources via the airflow-admin API",
		Long: `Manage airflow-admin resources via the airflow-admin API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'airflow-admin-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("airflow-admin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.dbPath, "db", "", "Database path (default: ~/.local/share/airflow-admin-pp-cli/data.db)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'airflow-admin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDagsCmd(flags))
	rootCmd.AddCommand(newPoolsCmd(flags))
	...
	rootCmd.AddCommand(newApacheAirflowAdminAuthPromotedCmd(flags))
	rootCmd.AddCommand(newApacheAirflowAdminVersionPromotedCmd(flags))
	rootCmd.AddCommand(newConnectionsPromotedCmd(flags))
	rootCmd.AddCommand(newMonitorPromotedCmd(flags))
	rootCmd.AddCommand(newVariablesPromotedCmd(flags))
...
func (f *rootFlags) dbPathOrDefault() string {
	if f != nil && f.dbPath != "" {
		return f.dbPath
	}
	return defaultDBPath("airflow-admin-pp-cli")
}
