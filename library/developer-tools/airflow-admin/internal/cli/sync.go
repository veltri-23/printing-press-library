...
Resource scoping:
  ...
  airflow-admin-pp-cli sync

  # Sync specific resources only
  airflow-admin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  airflow-admin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  airflow-admin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  airflow-admin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  airflow-admin-pp-cli sync --latest-only`,
...
func extractItemsByKnownKeys(envelope map[string]json.RawMessage) ([]json.RawMessage, bool) {
	for _, key := range pageItemKeys {
		if raw, ok := envelope[key]; ok {
			if items, ok := extractObjectArrayOrEmpty(raw); ok {
				return items, true
			}
		}
	}
...
func extractObjectArrayOrEmpty(raw json.RawMessage) ([]json.RawMessage, bool) {
	var items []json.RawMessage
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "dags":
		return db.UpsertDags(data)
	case "dag_runs":
		return db.UpsertDagRuns(data)
	case "details":
		return db.UpsertDetails(data)
	case "tasks":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "dag_runs", ParentTable: "dags", ParentIDParam: "dag_id", PathTemplate: "/api/v2/dags/{dag_id}/dagRuns", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "dag_id", Field: "id"},
		}},
		{Name: "dag_runs_task_instances", ParentTable: "dag_runs", ParentIDParam: "dag_run_id", PathTemplate: "/api/v2/dags/{dag_id}/dagRuns/{dag_run_id}/taskInstances", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "dag_id", Field: "dags_id"},
			{Param: "dag_run_id", Field: "id"},
		}},
	}
}
...
var pageItemKeys = []string{
	...
	"dags", "dag_runs", "task_instances", "tasks", "pools", "variables", "connections",
	"Data", "Results", "Items", "Records", "Nodes", "Entries", "Features",
}
...
var pageEnvelopeMetadataKeys = map[string]bool{
	...
	"total_count": true, "totalCount": true, "TotalCount": true, "total_entries": true,
