...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'airflow-admin-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: airflow-admin-pp-cli auth set-token <token>"+
			"\n      or: export AIRFLOW_ADMIN_BEARER_AUTH=<your-token>"+
			"\n      Run 'airflow-admin-pp-cli doctor' to check auth status.", err))
...
func paginatedGet(ctx context.Context, c interface {
	...
	// Cursor params are exempt from the "0" strip: offset-paginated APIs send
	// offset=0 on the first page. Boolean false is preserved so commands can
	// intentionally send filters such as --paused=false.
	clean := map[string]string{}
	for k, v := range params {
		if v == "" {
			continue
		}
		if k == cursorParam || v != "0" {
			clean[k] = v
		}
	}
