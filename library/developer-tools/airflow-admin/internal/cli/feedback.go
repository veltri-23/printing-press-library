...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "airflow-admin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRFLOW_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRFLOW_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airflow-admin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/airflow-admin-pp-cli/feedback.jsonl.
When ` + "`AIRFLOW_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRFLOW_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airflow-admin-pp-cli feedback list
  airflow-admin-pp-cli feedback list --limit 5
  airflow-admin-pp-cli feedback list --json`,
