...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airflow-admin-pp-cli doctor
  airflow-admin-pp-cli doctor --json
  airflow-admin-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export AIRFLOW_ADMIN_BEARER_AUTH=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("AIRFLOW_ADMIN_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "AIRFLOW_ADMIN_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AIRFLOW_ADMIN_BEARER_AUTH")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "airflow-admin-pp-cli", suggestion)
						} else {
			...
			report["cache"] = collectCacheReport(cmd.Context(), flags.dbPathOrDefault(), "")
...
func collectCacheReport(ctx context.Context, dbPath string, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'airflow-admin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'airflow-admin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'airflow-admin-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'airflow-admin-pp-cli sync' to refresh."
	}
	return report
}
