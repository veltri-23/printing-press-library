...
func isNilOrEmpty(raw json.RawMessage) bool {
	...
	for _, key := range []string{"title", "name", "identifier", "id", "dag_id", "dag_run_id", "task_id", "pool_name"} {
		if v, ok := obj[key]; ok {
			if v == nil {
				continue
			}
	...
	if doc, ok := obj["document"]; ok {
		if docMap, ok := doc.(map[string]interface{}); ok {
			for _, key := range []string{"title", "name", "identifier", "id", "slug", "dag_id", "dag_run_id", "task_id", "pool_name"} {
...
In local mode: searches locally synced data only.`,
  ...
  airflow-admin-pp-cli search "error timeout"

  # Force local search only
  airflow-admin-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  airflow-admin-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  airflow-admin-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'airflow-admin-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
