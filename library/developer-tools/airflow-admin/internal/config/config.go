...
type Config struct {
	...
	Path                   string            `toml:"-"`
	AirflowAdminBearerAuth string            `toml:"airflow_admin_bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("AIRFLOW_ADMIN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "airflow-admin-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("AIRFLOW_ADMIN_BEARER_AUTH"); v != "" {
		cfg.AirflowAdminBearerAuth = v
		cfg.AuthSource = "env:AIRFLOW_ADMIN_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.AirflowAdminBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("AIRFLOW_ADMIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.AirflowAdminBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:AIRFLOW_ADMIN_BEARER_AUTH"
		}
		return "Bearer " + c.AirflowAdminBearerAuth
	}
	if c.AccessToken != "" {
