...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".uspto-tsdr-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("USPTO_TSDR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("USPTO_TSDR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "uspto-tsdr-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.uspto-tsdr-pp-cli/feedback.jsonl.
When ` + "`USPTO_TSDR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`USPTO_TSDR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  uspto-tsdr-pp-cli feedback list
  uspto-tsdr-pp-cli feedback list --limit 5
  uspto-tsdr-pp-cli feedback list --json`,
