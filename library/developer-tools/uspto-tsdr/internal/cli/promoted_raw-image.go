			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "serial_number"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("serial_number is required\nUsage: %s <%s>", cmd.CommandPath(), "serial_number"))
			}
			path = replacePathParam(path, "serial_number", args[0])
			params := map[string]string{}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "raw-image", false, path, params, nil)
			if err != nil {
