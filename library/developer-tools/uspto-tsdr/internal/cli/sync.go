...
Exit codes & warnings:
  ...
  uspto-tsdr-pp-cli sync

  # Sync specific resources only
  uspto-tsdr-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  uspto-tsdr-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  uspto-tsdr-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  uspto-tsdr-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  uspto-tsdr-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/uspto-tsdr-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "casedoc_content_pdf":
		return db.UpsertCasedocContentPdf(data)
	case "casedoc_content_zip":
		return db.UpsertCasedocContentZip(data)
	case "casedoc_download_pdf":
		return db.UpsertCasedocDownloadPdf(data)
	case "casedoc_download_zip":
		return db.UpsertCasedocDownloadZip(data)
	case "casedoc_info":
		return db.UpsertCasedocInfo(data)
	case "media":
		return db.UpsertMedia(data)
	case "bundle":
		return db.UpsertBundle(data)
	case "casedocs_content_pdf":
		return db.UpsertCasedocsContentPdf(data)
	case "casedocs_content_zip":
		return db.UpsertCasedocsContentZip(data)
	case "casedocs_download_pdf":
		return db.UpsertCasedocsDownloadPdf(data)
	case "casedocs_download_zip":
		return db.UpsertCasedocsDownloadZip(data)
	case "casedocs_info":
		return db.UpsertCasedocsInfo(data)
	case "content_html":
		return db.UpsertContentHtml(data)
	case "casestatus_content_pdf":
		return db.UpsertCasestatusContentPdf(data)
	case "casestatus_content_zip":
		return db.UpsertCasestatusContentZip(data)
	case "casestatus_download_pdf":
		return db.UpsertCasestatusDownloadPdf(data)
	case "casestatus_download_zip":
		return db.UpsertCasestatusDownloadZip(data)
	case "casestatus_info":
		return db.UpsertCasestatusInfo(data)
	case "v1":
		return db.UpsertV1(data)
	default:
