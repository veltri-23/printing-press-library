...
type Config struct {
	...
	Path             string            `toml:"-"`
	TsdrApikeyHeader string            `toml:"apikey_header"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("USPTO_TSDR_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "uspto-tsdr-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TSDR_APIKEY_HEADER"); v != "" {
		cfg.TsdrApikeyHeader = v
		cfg.AuthSource = "env:TSDR_APIKEY_HEADER"
	}
	...
	if cfg.AuthSource == "" && cfg.TsdrApikeyHeader != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("USPTO_TSDR_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TsdrApikeyHeader == "" {
		return ""
	}
	return c.TsdrApikeyHeader
}
