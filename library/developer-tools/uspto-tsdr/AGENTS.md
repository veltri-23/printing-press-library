# Uspto Tsdr Printed CLI Agent Guide

This directory is a generated `uspto-tsdr-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
uspto-tsdr-pp-cli doctor --json
uspto-tsdr-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
uspto-tsdr-pp-cli which "<capability>" --json
uspto-tsdr-pp-cli <command> --help
```
...
```bash
uspto-tsdr-pp-cli <command> --agent
```
...
```bash
uspto-tsdr-pp-cli <command> --help
uspto-tsdr-pp-cli <command> --dry-run --agent
```
