...
In local mode: searches locally synced data only.`,
  ...
  forgejo-pp-cli search "error timeout"

  # Force local search only
  forgejo-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  forgejo-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  forgejo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /admin/users
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/admin/users", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'forgejo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "label":
				results, err = db.SearchLabel(query, limit)
			case "licenses":
				results, err = db.SearchLicenses(query, limit)
			case "orgs":
				results, err = db.SearchOrgs(query, limit)
			case "repos":
				results, err = db.SearchRepos(query, limit)
			case "repositories":
				results, err = db.SearchRepositories(query, limit)
			case "teams":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchLabel(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search label failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchLicenses(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search licenses failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchOrgs(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search orgs failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRepos(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search repos failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchRepositories(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search repositories failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
