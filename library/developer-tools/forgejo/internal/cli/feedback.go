...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "forgejo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FORGEJO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FORGEJO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "forgejo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/forgejo-pp-cli/feedback.jsonl.
When ` + "`FORGEJO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FORGEJO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  forgejo-pp-cli feedback list
  forgejo-pp-cli feedback list --limit 5
  forgejo-pp-cli feedback list --json`,
