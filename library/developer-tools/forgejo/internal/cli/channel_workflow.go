...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  forgejo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  forgejo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"admin", "admin-actions-runners", "admin-actions-runners-jobs", "admin-emails", "admin-hooks", "admin-orgs", "admin-quota-groups", "admin-quota-rules", "admin-runners-jobs", "admin-unadopted", "admin-users", "gitignore", "label", "licenses", "notifications", "orgs", "repos", "user", "user-actions-runners", "user-actions-runners-jobs", "user-actions-variables", "user-applications-oauth2", "user-emails", "user-followers", "user-following", "user-gpg-keys", "user-hooks", "user-list-blocked", "user-orgs", "user-quota-artifacts", "user-quota-attachments", "user-quota-packages", "user-repos", "user-starred", "user-stopwatches", "user-subscriptions", "user-teams", "user-times", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  forgejo-pp-cli workflow status

  # Show status as JSON
  forgejo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("forgejo-pp-cli")
			}
