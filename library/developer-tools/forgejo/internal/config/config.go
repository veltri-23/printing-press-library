...
type Config struct {
	...
	Path          string            `toml:"-"`
	ForgejoToken  string            `toml:"token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("FORGEJO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "forgejo-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("FORGEJO_TOKEN"); v != "" {
		cfg.ForgejoToken = v
		cfg.AuthSource = "env:FORGEJO_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ForgejoToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("FORGEJO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
