# Forgejo Printed CLI Agent Guide

This directory is a generated `forgejo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
forgejo-pp-cli doctor --json
forgejo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
forgejo-pp-cli which "<capability>" --json
forgejo-pp-cli <command> --help
```
...
```bash
forgejo-pp-cli <command> --help
forgejo-pp-cli <command> --dry-run --agent
```
