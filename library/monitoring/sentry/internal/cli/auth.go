...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage SENTRY_AUTH_TOKEN credentials",
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sentry-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if header == "" {
				...
				fmt.Fprintln(w, "  export SENTRY_AUTH_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  sentry-pp-cli auth set-token <token>\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sentry-pp-cli auth set-token sk_live_abc123",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sentry-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if os.Getenv("SENTRY_AUTH_TOKEN") != "" {
				fmt.Fprintf(cmd.OutOrStdout(), "Config cleared. Note: SENTRY_AUTH_TOKEN env var is still set.\n")
				return nil
			}
