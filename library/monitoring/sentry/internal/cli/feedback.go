...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".sentry-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SENTRY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SENTRY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sentry-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.sentry-pp-cli/feedback.jsonl.
When ` + "`SENTRY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SENTRY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
