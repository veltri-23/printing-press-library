...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sentry-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sentry-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"organizations", "seer"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sentry-pp-cli workflow status

  # Show status as JSON
  sentry-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sentry-pp-cli")
			}
