...
Exit codes & warnings:
  ...
  sentry-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sentry-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sentry-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sentry-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sentry-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("sentry-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/sentry-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "sentry_apps":
		return db.UpsertSentryApps(data)
	case "projects":
	...
	case "preprod":
		return db.UpsertPreprod(data)
	case "preprodartifacts":
		return db.UpsertPreprodartifacts(data)
	case "tags":
	...
	case "user_feedback":
		return db.UpsertUserFeedback(data)
	case "issues":
		return db.UpsertIssues(data)
	case "stats":
		return db.UpsertStats(data)
	case "symbol_sources":
		return db.UpsertSymbolSources(data)
	case "teams":
	...
	case "ownership":
		return db.UpsertOwnership(data)
	case "users":
		return db.UpsertUsers(data)
	case "replays":
		return db.UpsertReplays(data)
	case "releases":
		return db.UpsertReleases(data)
	case "events":
		return db.UpsertEvents(data)
	case "external_issues":
		return db.UpsertExternalIssues(data)
	case "external_teams":
		return db.UpsertExternalTeams(data)
	case "organizations":
		return db.UpsertOrganizations(data)
	case "scim":
		return db.UpsertScim(data)
	case "shortids":
		return db.UpsertShortids(data)
	case "alert_rules":
		return db.UpsertAlertRules(data)
	case "events_timeseries":
		return db.UpsertEventsTimeseries(data)
	case "stats_summary":
		return db.UpsertStatsSummary(data)
	case "stats_v2":
		return db.UpsertStatsV2(data)
	case "user_teams":
		return db.UpsertUserTeams(data)
	case "config":
		return db.UpsertConfig(data)
	case "discover":
		return db.UpsertDiscover(data)
	case "replay_count":
		return db.UpsertReplayCount(data)
	case "forwarding":
		return db.UpsertForwarding(data)
	case "integrations":
		return db.UpsertIntegrations(data)
	case "project_keys":
		return db.UpsertProjectKeys(data)
	case "repos":
		return db.UpsertRepos(data)
	case "spike_protections":
		return db.UpsertSpikeProtections(data)
	case "eventids":
		return db.UpsertEventids(data)
	case "detectors":
		return db.UpsertDetectors(data)
	case "release_threshold_statuses":
		return db.UpsertReleaseThresholdStatuses(data)
	case "notifications":
		return db.UpsertNotifications(data)
	case "external_users":
		return db.UpsertExternalUsers(data)
	case "sessions":
		return db.UpsertSessions(data)
	case "workflows":
		return db.UpsertWorkflows(data)
	case "prevent":
		return db.UpsertPrevent(data)
	case "dashboards":
		return db.UpsertDashboards(data)
	case "relay_usage":
		return db.UpsertRelayUsage(data)
	case "replay_selectors":
		return db.UpsertReplaySelectors(data)
	default:
