# Adguard Home Printed CLI Agent Guide

This directory is a generated `adguard-home-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
adguard-home-pp-cli doctor --json
adguard-home-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
adguard-home-pp-cli which "<capability>" --json
adguard-home-pp-cli <command> --help
```
...
```bash
adguard-home-pp-cli <command> --agent
```
...
```bash
adguard-home-pp-cli <command> --help
adguard-home-pp-cli <command> --dry-run --agent
```
