...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".adguard-home-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ADGUARD_HOME_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ADGUARD_HOME_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/monitoring/adguard-home/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.adguard-home-pp-cli/feedback.jsonl.
When ` + "`ADGUARD_HOME_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ADGUARD_HOME_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  adguard-home-pp-cli feedback list
  adguard-home-pp-cli feedback list --limit 5
  adguard-home-pp-cli feedback list --json`,
