...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  adguard-home-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  adguard-home-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"access", "blocked-services", "blocked-services-services", "clients", "dns-info", "parental", "querylog", "querylog-config", "querylog-info", "rewrite", "safebrowsing", "stats"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  adguard-home-pp-cli workflow status

  # Show status as JSON
  adguard-home-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("adguard-home-pp-cli")
			}
