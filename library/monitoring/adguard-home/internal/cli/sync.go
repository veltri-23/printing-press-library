...
Resource scoping:
  ...
  adguard-home-pp-cli sync

  # Sync specific resources only
  adguard-home-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  adguard-home-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  adguard-home-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  adguard-home-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  adguard-home-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/adguard-home-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "adguard-home-profile":
		return db.UpsertAdguardHomeProfile(data)
	case "blocked-services":
		return db.UpsertBlockedServices(data)
	case "dhcp":
		return db.UpsertDhcp(data)
	case "dns-info":
		return db.UpsertDnsInfo(data)
	case "filtering":
		return db.UpsertFiltering(data)
	case "i18n":
		return db.UpsertI18n(data)
	case "install":
		return db.UpsertInstall(data)
	case "parental":
		return db.UpsertParental(data)
	case "querylog":
		return db.UpsertQuerylog(data)
	case "rewrite":
		return db.UpsertRewrite(data)
	case "safebrowsing":
		return db.UpsertSafebrowsing(data)
	case "safesearch":
		return db.UpsertSafesearch(data)
	case "stats":
	...
	case "version-json":
		return db.UpsertVersionJson(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"querylog":                  "/querylog",
		"querylog-config":           "/querylog/config",
