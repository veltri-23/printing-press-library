...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  adguard-home-pp-cli doctor
  adguard-home-pp-cli doctor --json
  adguard-home-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export ADGUARD_HOME_USERNAME=<your-key>"
					report["auth_docs_url"] = "https://github.com/AdguardTeam/AdGuardHome"
					report["auth_instructions"] = "Use your AdGuard Home admin username and password. Set ADGUARD_HOME_URL to your instance URL (e.g. http://10.0.4.86:3001)."
				} else {
					authConfigured = true
			...
			if os.Getenv("ADGUARD_HOME_USERNAME") != "" {
				authEnvSet = append(authEnvSet, "ADGUARD_HOME_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ADGUARD_HOME_USERNAME")
			}
			if os.Getenv("ADGUARD_HOME_PASSWORD") != "" {
				authEnvSet = append(authEnvSet, "ADGUARD_HOME_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ADGUARD_HOME_PASSWORD")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'adguard-home-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'adguard-home-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'adguard-home-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'adguard-home-pp-cli sync' to refresh."
	}
	return report
}
