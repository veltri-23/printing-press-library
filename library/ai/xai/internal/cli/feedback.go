...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "xai-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("XAI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("XAI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "xai-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/xai-pp-cli/feedback.jsonl.
When ` + "`XAI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`XAI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  xai-pp-cli feedback list
  xai-pp-cli feedback list --limit 5
  xai-pp-cli feedback list --json`,
