...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  xai-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  xai-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"embedding-models", "files", "image-generation-models", "language-models", "models", "skills", "video-generation-models"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  xai-pp-cli workflow status

  # Show status as JSON
  xai-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("xai-pp-cli")
			}
