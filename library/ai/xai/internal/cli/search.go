...
In local mode: searches locally synced data only.`,
  ...
  xai-pp-cli search "error timeout"

  # Force local search only
  xai-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  xai-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  xai-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v1/documents/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/v1/documents/search", map[string]any{
					"query":          query,
					"filter":         "",
					"group_by":       "",
					"instructions":   "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'xai-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
