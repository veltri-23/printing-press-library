...
Resource scoping:
  ...
  xai-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  xai-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  xai-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  xai-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  xai-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("xai-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/xai-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "completions":
		return db.UpsertCompletions(data)
	case "documents":
		return db.UpsertDocuments(data)
	case "embedding-models":
		return db.UpsertEmbeddingModels(data)
	case "files":
		return db.UpsertFiles(data)
	case "files_content":
		return db.UpsertFilesContent(data)
	case "image-generation-models":
		return db.UpsertImageGenerationModels(data)
	case "images":
		return db.UpsertImages(data)
	case "language-models":
		return db.UpsertLanguageModels(data)
	case "me":
	...
	case "input_items":
		return db.UpsertInputItems(data)
	case "skills":
		return db.UpsertSkills(data)
	case "skills_content":
		return db.UpsertSkillsContent(data)
	case "tokenize-text":
		return db.UpsertTokenizeText(data)
	case "video-generation-models":
		return db.UpsertVideoGenerationModels(data)
	case "videos":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"files":                   "/v1/files",
		"image-generation-models": "/v1/image-generation-models",
		"language-models":         "/v1/language-models",
		"models":                  "/v1/models",
		"skills":                  "/v1/skills",
		"video-generation-models": "/v1/video-generation-models",
	}
