...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("XAI_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "xai-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("XAI_API_KEY"); v != "" {
		cfg.XaiApiKey = v
		cfg.markEnvOverride("XaiApiKey")
		cfg.AuthSource = "env:XAI_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.XaiApiKey != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) AuthHeader() string {
	...
	if c.XaiApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:XAI_API_KEY"
		}
		return "Bearer " + c.XaiApiKey
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.XaiApiKey = ""
	delete(c.envOverrides, "XaiApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["XaiApiKey"] {
			out.XaiApiKey = c.fileConfig.XaiApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "XaiApiKey":
		c.fileConfig.XaiApiKey = c.XaiApiKey
	}
}
