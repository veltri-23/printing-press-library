# Xai Printed CLI Agent Guide

This directory is a generated `xai-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
xai-pp-cli doctor --json
xai-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
xai-pp-cli which "<capability>" --json
xai-pp-cli <command> --help
```
...
```bash
xai-pp-cli <command> --help
xai-pp-cli <command> --dry-run --agent
```
