# Openart Printed CLI Agent Guide

This directory is a generated `openart-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
openart-pp-cli doctor --json
openart-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
openart-pp-cli which "<capability>" --json
...
```bash
openart-pp-cli <command> --help
openart-pp-cli <command> --dry-run --agent
```
