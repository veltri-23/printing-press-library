...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  openart-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  openart-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"credits", "project", "templates", "workspace"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  openart-pp-cli workflow status

  # Show status as JSON
  openart-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("openart-pp-cli")
			}
