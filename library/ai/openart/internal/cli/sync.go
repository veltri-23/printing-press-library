...
Exit codes & warnings:
  ...
  openart-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openart-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openart-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openart-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openart-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/openart-pp-cli/data.db)")
...
//
// OpenArt-specific override: /credits/logs and /resources both cap server-side
// at 50; the generator's default of 100 trips a 400 "Too big" response on the
// first credits page and short-circuits subsequent resources. Capping at 50
// keeps every resource within the safe envelope.
