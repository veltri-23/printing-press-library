...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".openart-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENART_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENART_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openart-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.openart-pp-cli/feedback.jsonl.
When ` + "`OPENART_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENART_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openart-pp-cli feedback list
  openart-pp-cli feedback list --limit 5
  openart-pp-cli feedback list --json`,
