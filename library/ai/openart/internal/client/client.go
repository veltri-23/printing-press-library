...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "openart-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			// OpenArt uses cookie auth: the imported "auth header" is actually
			// a serialised Cookie string ("k=v; k=v; ..."). Detect that shape
			// (presence of ";" or NextAuth/Clerk session cookie names) and
			// route to the Cookie header instead of Authorization. Fall back
			// to Authorization for any other shape so this stays harmless if
			// the spec is later switched back to a token-based auth mode.
...
// looksLikeCookieString returns true when the auth-header value parses as
// a serialised browser cookie jar (the shape OpenArt's auth login --chrome
// imports) rather than a Bearer/Basic token. Detection is intentionally
// conservative: it requires an "=" pair AND either a ";" separator or a
// session-cookie name fragment seen in the wild.
func looksLikeCookieString(s string) bool {
