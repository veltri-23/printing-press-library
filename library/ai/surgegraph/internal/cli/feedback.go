...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".surgegraph-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SURGEGRAPH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SURGEGRAPH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "surgegraph-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.surgegraph-pp-cli/feedback.jsonl.
When ` + "`SURGEGRAPH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SURGEGRAPH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  surgegraph-pp-cli feedback list
  surgegraph-pp-cli feedback list --limit 5
  surgegraph-pp-cli feedback list --json`,
