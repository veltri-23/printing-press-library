...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "surgegraph-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "surgegraph-pp-cli/0.1.0")
		}
...
// PATCH(greptile-1): wire to oauth.Refresh so expired access tokens actually rotate instead of being passed through stale.
func (c *Client) refreshAccessToken() error {
	if c.Config == nil {
		return nil
	}
	if c.Config.RefreshToken == "" || c.Config.ClientID == "" {
		return fmt.Errorf("refresh required but credentials incomplete; run `surgegraph-pp-cli auth login`")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	meta, err := oauth.Discover(ctx, c.Config.BaseURL)
	if err != nil {
		return fmt.Errorf("oauth discover: %w", err)
	}
	oc := &oauth.Client{ClientID: c.Config.ClientID, ClientSecret: c.Config.ClientSecret}
	tokens, err := oauth.Refresh(ctx, meta, oc, c.Config.RefreshToken)
	if err != nil {
		return fmt.Errorf("oauth refresh: %w", err)
	}
	newRefresh := tokens.RefreshToken
	if newRefresh == "" {
		newRefresh = c.Config.RefreshToken // server may omit a rotated refresh; keep the existing one
	}
	return c.Config.SaveTokens(c.Config.ClientID, c.Config.ClientSecret, tokens.AccessToken, newRefresh, tokens.ExpiresAt)
}
