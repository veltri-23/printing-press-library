# SurgeGraph Printed CLI Agent Guide

This directory is a generated `surgegraph-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
surgegraph-pp-cli doctor --json
surgegraph-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
surgegraph-pp-cli which "<capability>" --json
...
```bash
surgegraph-pp-cli <command> --agent
```
...
```bash
surgegraph-pp-cli <command> --help
surgegraph-pp-cli <command> --dry-run --agent
```
