...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "context-dev-pp-cli",
		Short: "Manage context-dev resources via the context-dev API",
		Long: `Manage context-dev resources via the context-dev API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'context-dev-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("context-dev-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'context-dev-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		noColor.Store(flags.noColor)
		humanFriendly.Store(flags.humanFriendly)
	...
	rootCmd.AddCommand(newStyleguideCmd(flags))
	rootCmd.AddCommand(newScreenshotCmd(flags))
	rootCmd.AddCommand(newEntityDiscoverCmd(flags))
	rootCmd.AddCommand(newBrandBriefCmd(flags))
	rootCmd.AddCommand(newCompetitorMapCmd(flags))
	rootCmd.AddCommand(newCrawlBudgetPlanCmd(flags))
	rootCmd.AddCommand(newSourcePackCmd(flags))
	rootCmd.AddCommand(newWebsiteChangeDigestCmd(flags))
	rootCmd.AddCommand(newSchemaLabCmd(flags))
	rootCmd.AddCommand(newBrandKitCmd(flags))
	rootCmd.AddCommand(newTrustCheckCmd(flags))
	rootCmd.AddCommand(newLeadEnrichBatchCmd(flags))
	rootCmd.AddCommand(newBrandQACmd(flags))
	rootCmd.AddCommand(newEmailEnrichCmd(flags))
	rootCmd.AddCommand(newTickerEnrichCmd(flags))
