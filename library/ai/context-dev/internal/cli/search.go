...
In local mode: searches locally synced data only.`,
  ...
  context-dev-pp-cli search "error timeout"

  # Force local search only
  context-dev-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  context-dev-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  context-dev-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /web/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/web/search", map[string]any{
					"query":           query,
					"excludeDomains":  nil,
					"freshness":       "last_24_hours",
					"includeDomains":  nil,
					"markdownOptions": map[string]any{},
					"queryFanout":     nil,
					"timeoutMS":       nil,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'context-dev-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "web":
				results, err = db.SearchWeb(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchWeb(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search web failed: %w", searchErr)
					}
