...
// noColor is set from the --no-color flag before command execution.
var noColor atomic.Bool

// humanFriendly is set from the --human-friendly flag; colors are off by default (agent-safe).
var humanFriendly atomic.Bool

func humanFriendlyEnabled() bool {
	return humanFriendly.Load()
}
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'context-dev-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token."+
			"\n      Set it with: context-dev-pp-cli auth set-token <token> or export CONTEXT_DEV_API_KEY=\"your-token-here\""+
			"\n      Run 'context-dev-pp-cli doctor' to check auth status.", err))
