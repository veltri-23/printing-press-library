...
func looksLikeDoctorInterstitial(body []byte) string {
	...
	case strings.Contains(prefix, "akamai") && (strings.Contains(prefix, "request unsuccessful") || strings.Contains(prefix, "access denied")):
		return "Akamai"
	case strings.Contains(prefix, "vercel") && strings.Contains(prefix, "challenge"):
		return "Vercel"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  context-dev-pp-cli doctor
  context-dev-pp-cli doctor --json
  context-dev-pp-cli doctor --fail-on warn
  context-dev-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: context-dev-pp-cli auth set-token <token> or export CONTEXT_DEV_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("CONTEXT_DEV_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CONTEXT_DEV_API_KEY")
			} else if os.Getenv("CONTEXT_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "CONTEXT_API_KEY")
			} else if os.Getenv("CONTEXT_DEV_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "CONTEXT_DEV_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CONTEXT_DEV_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "context-dev-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("context-dev-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'context-dev-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'context-dev-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'context-dev-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'context-dev-pp-cli sync' to refresh."
	}
	return report
}
