...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONTEXT_DEV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONTEXT_DEV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/ai/context-dev/feedback")
...
When ` + "`CONTEXT_DEV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONTEXT_DEV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  context-dev-pp-cli feedback list
  context-dev-pp-cli feedback list --limit 5
  context-dev-pp-cli feedback list --json`,
