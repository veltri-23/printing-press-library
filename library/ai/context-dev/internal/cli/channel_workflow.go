...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  context-dev-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  context-dev-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"web", "web-screenshot", "web-styleguide"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  context-dev-pp-cli workflow status

  # Show status as JSON
  context-dev-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("context-dev-pp-cli")
			}
