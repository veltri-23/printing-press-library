...
Resource scoping:
  ...
  context-dev-pp-cli sync

  # Sync specific resources only
  context-dev-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  context-dev-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  context-dev-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  context-dev-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  context-dev-pp-cli sync --latest-only`,
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if resourceSupportsPagination(resource) && nextCursor == "" && pageSize.cursorParam != "offset" && len(items) >= pageSize.limit && pageMayHaveMore(data) {
			emitSyncMissingPaginationCursorWarning(syncEvents, humanFriendlyEnabled(), resource, "")
		}
