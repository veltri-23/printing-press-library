...
type Config struct {
	...
	ContextDevBearerAuth string          `toml:"dev_bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides. CONTEXT_DEV_API_KEY is the public contract for this
	// CLI; CONTEXT_API_KEY is a compatibility fallback. Keep the generated
	// ContextDevBearerAuth field as the internal bearer-token slot.
	if v := os.Getenv("CONTEXT_DEV_API_KEY"); v != "" {
		cfg.ContextDevBearerAuth = v
		cfg.markEnvOverride("ContextDevBearerAuth")
		cfg.AuthSource = "env:CONTEXT_DEV_API_KEY"
		cfg.CredentialSource = "env:CONTEXT_DEV_API_KEY"
		if os.Getenv("CONTEXT_API_KEY") != "" {
			fmt.Fprintln(os.Stderr, "warning: both CONTEXT_DEV_API_KEY and CONTEXT_API_KEY are set; using CONTEXT_DEV_API_KEY")
		}
	} else if v := os.Getenv("CONTEXT_API_KEY"); v != "" {
		cfg.ContextDevBearerAuth = v
		cfg.markEnvOverride("ContextDevBearerAuth")
		cfg.AuthSource = "env:CONTEXT_API_KEY"
		cfg.CredentialSource = "env:CONTEXT_API_KEY"
	} else if v := os.Getenv("CONTEXT_DEV_BEARER_AUTH"); v != "" {
		cfg.ContextDevBearerAuth = v
		cfg.markEnvOverride("ContextDevBearerAuth")
		cfg.AuthSource = "env:CONTEXT_DEV_BEARER_AUTH"
		cfg.CredentialSource = "env:CONTEXT_DEV_BEARER_AUTH"
	}
	...
	if v := os.Getenv("CONTEXT_DEV_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("CONTEXT_DEV_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "context-dev-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ContextDevBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CONTEXT_DEV_BEARER_AUTH"
		}
		return "Bearer " + c.ContextDevBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ContextDevBearerAuth: c.ContextDevBearerAuth,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ContextDevBearerAuth = creds.ContextDevBearerAuth
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.ContextDevBearerAuth = ""
	delete(c.envOverrides, "ContextDevBearerAuth")
	c.updateFileConfigField("ContextDevBearerAuth")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ContextDevBearerAuth"] {
			out.ContextDevBearerAuth = c.fileConfig.ContextDevBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ContextDevBearerAuth":
		c.fileConfig.ContextDevBearerAuth = c.ContextDevBearerAuth
	}
}
