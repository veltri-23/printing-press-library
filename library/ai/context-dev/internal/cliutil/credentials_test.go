...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"CONTEXT_DEV_CONFIG",
		"CONTEXT_DEV_CONFIG_DIR",
		"CONTEXT_DEV_DATA_DIR",
		"CONTEXT_DEV_STATE_DIR",
		"CONTEXT_DEV_CACHE_DIR",
		"CONTEXT_DEV_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
		"CONTEXT_DEV_BEARER_AUTH",
	} {
		t.Setenv(name, "")
	}
	...
	return home, filepath.Join(home, ".config", "context-dev-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "context-dev-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("CONTEXT_DEV_BEARER_AUTH", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "context-dev-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "context-dev-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "context-dev-pp-cli", "credentials.toml")
