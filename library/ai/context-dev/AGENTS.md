# Context Dev Printed CLI Agent Guide

This directory is a generated `context-dev-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
context-dev-pp-cli doctor --json
context-dev-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
context-dev-pp-cli which "<capability>" --json
context-dev-pp-cli <command> --help
```
...
```bash
context-dev-pp-cli <command> --agent
```
...
```bash
context-dev-pp-cli <command> --help
context-dev-pp-cli <command> --dry-run --agent
```
