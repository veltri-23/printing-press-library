...
type Config struct {
	...
	Path              string            `toml:"-"`
	OllamaCloudApiKey string            `toml:"cloud_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OLLAMA_CLOUD_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "ollama-cloud-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("OLLAMA_CLOUD_API_KEY"); v != "" {
		cfg.OllamaCloudApiKey = v
		cfg.AuthSource = "env:OLLAMA_CLOUD_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.OllamaCloudApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("OLLAMA_CLOUD_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.OllamaCloudApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:OLLAMA_CLOUD_API_KEY"
		}
		return "Bearer " + c.OllamaCloudApiKey
	}
	if c.AccessToken != "" {
