...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ollama-cloud-pp-cli doctor
  ollama-cloud-pp-cli doctor --json
  ollama-cloud-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export OLLAMA_CLOUD_API_KEY=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("OLLAMA_CLOUD_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "OLLAMA_CLOUD_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "OLLAMA_CLOUD_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ollama-cloud-pp-cli", suggestion)
						} else {
