...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ollama-cloud-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OLLAMA_CLOUD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OLLAMA_CLOUD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ollama-cloud-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ollama-cloud-pp-cli/feedback.jsonl.
When ` + "`OLLAMA_CLOUD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OLLAMA_CLOUD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ollama-cloud-pp-cli feedback list
  ollama-cloud-pp-cli feedback list --limit 5
  ollama-cloud-pp-cli feedback list --json`,
