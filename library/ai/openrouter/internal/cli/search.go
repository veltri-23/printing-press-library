...
In local mode: searches locally synced data only.`,
  ...
  openrouter-pp-cli search "error timeout"

  # Force local search only
  openrouter-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  openrouter-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  openrouter-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'openrouter-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "keys":
				results, err = db.SearchKeys(query, limit)
			case "models":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchKeys(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search keys failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
