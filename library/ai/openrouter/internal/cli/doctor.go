...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'openrouter-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'openrouter-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	default:
		...
		report["hint"] = "Some resources are older than stale_after; run 'openrouter-pp-cli sync' to refresh."
	}
	return report
}
