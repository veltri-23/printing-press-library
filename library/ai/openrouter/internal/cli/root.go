...
// PATCH transcendence-commands: registers usage + budget top-level parents (hand-additions on generated root).
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "openrouter-pp-cli",
		Short: `Openrouter CLI — Agent-first OpenRouter introspection — terse '--llm' output for cron and AI agents, local SQLite catalog, free MCP serv…`,
		Long: `Openrouter CLI — Agent-first OpenRouter introspection — terse '--llm' output for cron and AI agents, local SQLite catalog, free MCP serv…
...
Health check: run 'openrouter-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("openrouter-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'openrouter-pp-cli profile list')")
	...
	rootCmd.AddCommand(newOpenrouterAuthCmd(flags))
	...
	rootCmd.AddCommand(newActivityPromotedCmd(flags))
	rootCmd.AddCommand(newCreditsPromotedCmd(flags))
	rootCmd.AddCommand(newEndpointsPromotedCmd(flags))
	rootCmd.AddCommand(newKeyPromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("openrouter-pp-cli %s\n", version)
		},
	}
}
