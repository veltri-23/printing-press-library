...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".openrouter-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENROUTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENROUTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openrouter-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.openrouter-pp-cli/feedback.jsonl.
When ` + "`OPENROUTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENROUTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openrouter-pp-cli feedback list
  openrouter-pp-cli feedback list --limit 5
  openrouter-pp-cli feedback list --json`,
