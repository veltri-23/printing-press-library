# Midjourney Printed CLI Agent Guide

This directory is a generated `midjourney-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
midjourney-pp-cli which "<capability>" --json
...
```bash
midjourney-pp-cli <command> --agent
```
...
```bash
midjourney-pp-cli <command> --help
midjourney-pp-cli <command> --dry-run --agent
```
