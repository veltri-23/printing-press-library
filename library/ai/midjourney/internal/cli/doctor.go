...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  midjourney-pp-cli doctor
  midjourney-pp-cli doctor --json
  midjourney-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export MIDJOURNEY_COOKIE_HEADER=<your-key>"
					report["auth_docs_url"] = "https://www.midjourney.com"
					report["auth_instructions"] = "Use an authenticated browser session. Export MIDJOURNEY_COOKIE_HEADER from the logged-in Chrome profile; do not commit or print it."
				} else {
					authConfigured = true
			...
			if os.Getenv("MIDJOURNEY_COOKIE_HEADER") != "" {
				authEnvSet = append(authEnvSet, "MIDJOURNEY_COOKIE_HEADER")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "MIDJOURNEY_COOKIE_HEADER")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("midjourney-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'midjourney-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'midjourney-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'midjourney-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'midjourney-pp-cli sync' to refresh."
	}
	return report
}
