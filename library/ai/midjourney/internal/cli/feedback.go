...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".midjourney-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MIDJOURNEY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MIDJOURNEY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "midjourney-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.midjourney-pp-cli/feedback.jsonl.
When ` + "`MIDJOURNEY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MIDJOURNEY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  midjourney-pp-cli feedback list
  midjourney-pp-cli feedback list --limit 5
  midjourney-pp-cli feedback list --json`,
