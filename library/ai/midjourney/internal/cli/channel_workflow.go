...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  midjourney-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  midjourney-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"explore", "explore-explore-styles-likes", "folders", "generations", "generations-imagine-update", "moodboards", "profiles", "profiles-personalized-profiles", "rankings", "rankings-contests-ranking-count", "storage"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  midjourney-pp-cli workflow status

  # Show status as JSON
  midjourney-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("midjourney-pp-cli")
			}
