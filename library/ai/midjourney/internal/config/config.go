...
type Config struct {
	...
	Path                   string            `json:"-"`
	MidjourneyCookieHeader string            `json:"cookie_header"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("MIDJOURNEY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "midjourney-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MIDJOURNEY_COOKIE_HEADER"); v != "" {
		cfg.MidjourneyCookieHeader = v
		cfg.AuthSource = "env:MIDJOURNEY_COOKIE_HEADER"
	}
	...
	if cfg.AuthSource == "" && cfg.MidjourneyCookieHeader != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("MIDJOURNEY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.MidjourneyCookieHeader
	if token == "" {
		return ""
	}
	replacements := map[string]string{
		...
		"MIDJOURNEY_COOKIE_HEADER": c.MidjourneyCookieHeader,
	}
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.MidjourneyCookieHeader = token
	return c.save()
}
