# Elevenlabs Printed CLI Agent Guide

This directory is a generated `elevenlabs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
elevenlabs-pp-cli doctor --json
elevenlabs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
elevenlabs-pp-cli which "<capability>" --json
elevenlabs-pp-cli <command> --help
```
...
```bash
elevenlabs-pp-cli <command> --agent
```
...
```bash
elevenlabs-pp-cli <command> --help
elevenlabs-pp-cli <command> --dry-run --agent
```
