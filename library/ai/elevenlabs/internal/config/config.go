...
type Config struct {
	...
	Path             string            `toml:"-"`
	ElevenlabsApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ELEVENLABS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "elevenlabs-pp-cli", "config.toml")
	}
	cfg.Path = path
