...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for ElevenLabs",
	}
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  elevenlabs-pp-cli auth setup\n  elevenlabs-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://elevenlabs.io/app/settings/api-keys")
			fmt.Fprintln(w, "  Set ELEVENLABS_API_KEY to an ElevenLabs API key; requests send it as the xi-api-key header.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export ELEVENLABS_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  elevenlabs-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://elevenlabs.io/app/settings/api-keys"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  elevenlabs-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export ELEVENLABS_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  elevenlabs-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  elevenlabs-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  elevenlabs-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("ELEVENLABS_API_KEY") != "" {
				envStillSet = "ELEVENLABS_API_KEY"
			}
