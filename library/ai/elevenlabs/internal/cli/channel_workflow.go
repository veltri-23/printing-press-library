...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  elevenlabs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  elevenlabs-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"audio-isolation", "convai", "convai-agent-testing", "convai-agents", "convai-analytics-live-count", "convai-api-integrations-docs", "convai-batch-calling-workspace", "convai-conversations", "convai-environment-variables", "convai-knowledge-base", "convai-knowledge-base-rag-index", "convai-mcp-servers", "convai-phone-numbers", "convai-secrets", "convai-settings", "convai-settings-dashboard", "convai-test-invocations", "convai-tools", "convai-users", "convai-whatsapp-accounts", "dubbing", "history", "models", "pronunciation-dictionaries", "service-accounts", "shared-voices", "speech-engine", "studio", "user", "user-subscription", "voices", "voices-settings-default", "workspace", "workspace-audit-logs", "workspace-auth-connections", "workspace-webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  elevenlabs-pp-cli workflow status

  # Show status as JSON
  elevenlabs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("elevenlabs-pp-cli")
			}
