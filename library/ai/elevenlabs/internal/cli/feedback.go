...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".elevenlabs-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ELEVENLABS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ELEVENLABS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "elevenlabs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.elevenlabs-pp-cli/feedback.jsonl.
When ` + "`ELEVENLABS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ELEVENLABS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  elevenlabs-pp-cli feedback list
  elevenlabs-pp-cli feedback list --limit 5
  elevenlabs-pp-cli feedback list --json`,
