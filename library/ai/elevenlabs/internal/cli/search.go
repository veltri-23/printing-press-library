...
In local mode: searches locally synced data only.`,
  ...
  elevenlabs-pp-cli search "error timeout"

  # Force local search only
  elevenlabs-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  elevenlabs-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  elevenlabs-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/workspace/groups/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/v1/workspace/groups/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'elevenlabs-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "convai":
				results, err = db.SearchConvai(query, limit)
			case "models":
				results, err = db.SearchModels(query, limit)
			case "pronunciation-dictionaries":
				results, err = db.SearchPronunciationDictionaries(query, limit)
			case "studio":
				results, err = db.SearchStudio(query, limit)
			case "text-to-voice":
				results, err = db.SearchTextToVoice(query, limit)
			case "voices":
				results, err = db.SearchVoices(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchConvai(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search convai failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchModels(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search models failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPronunciationDictionaries(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search pronunciation_dictionaries failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchStudio(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search studio failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTextToVoice(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search text_to_voice failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchVoices(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search voices failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
