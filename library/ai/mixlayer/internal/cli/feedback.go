...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MIXLAYER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MIXLAYER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mixlayer-pp-cli/feedback")
...
When ` + "`MIXLAYER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MIXLAYER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mixlayer-pp-cli feedback list
  mixlayer-pp-cli feedback list --limit 5
  mixlayer-pp-cli feedback list --json`,
