# Twelvelabs Printed CLI Agent Guide

This directory is a generated `twelvelabs-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
twelvelabs-pp-cli doctor --json
twelvelabs-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
twelvelabs-pp-cli which "<capability>" --json
twelvelabs-pp-cli <command> --help
```
...
```bash
twelvelabs-pp-cli <command> --agent
```
...
```bash
twelvelabs-pp-cli <command> --help
twelvelabs-pp-cli <command> --dry-run --agent
```
