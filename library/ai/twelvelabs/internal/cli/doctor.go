...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twelvelabs-pp-cli doctor
  twelvelabs-pp-cli doctor --json
  twelvelabs-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TWELVELABS_X_API_KEY=<your-key>"
					report["auth_key_url"] = "https://playground.twelvelabs.io/dashboard/api-keys"
				} else {
					authConfigured = true
			...
			if os.Getenv("TWELVELABS_X_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "TWELVELABS_X_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TWELVELABS_X_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "twelvelabs-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'twelvelabs-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'twelvelabs-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'twelvelabs-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'twelvelabs-pp-cli sync' to refresh."
	}
	return report
}
