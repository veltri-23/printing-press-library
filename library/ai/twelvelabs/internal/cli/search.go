...
In local mode: searches locally synced data only.`,
  ...
  twelvelabs-pp-cli search "error timeout"

  # Force local search only
  twelvelabs-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  twelvelabs-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  twelvelabs-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, _, getErr := c.Post(cmd.Context(), "/search", map[string]any{
					...
					"query_media_url":         "",
					"query_text":              "",
					"search_options":          []any{"visual", "audio", "transcription"},
					"sort_option":             "score",
					"threshold":               "low",
					"transcription_options":   []any{"lexical", "semantic"},
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'twelvelabs-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "entity-collections":
				results, err = db.SearchEntityCollections(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchEntityCollections(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search entity_collections failed: %w", searchErr)
					}
