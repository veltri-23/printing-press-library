...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  twelvelabs-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  twelvelabs-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"assets", "assets-multipart-uploads", "embed", "embed-v2", "entity-collections", "indexes", "tasks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  twelvelabs-pp-cli workflow status

  # Show status as JSON
  twelvelabs-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("twelvelabs-pp-cli")
			}
