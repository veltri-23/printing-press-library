...
Resource scoping:
  ...
  twelvelabs-pp-cli sync

  # Sync specific resources only
  twelvelabs-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  twelvelabs-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  twelvelabs-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  twelvelabs-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  twelvelabs-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("twelvelabs-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/twelvelabs-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "embed":
		return db.UpsertEmbed(data)
	case "embed-v2":
		return db.UpsertEmbedV2(data)
	case "entity-collections":
		return db.UpsertEntityCollections(data)
	case "entities":
		return db.UpsertEntities(data)
	case "gist":
		return db.UpsertGist(data)
	case "indexes":
		return db.UpsertIndexes(data)
	case "indexed_assets":
		return db.UpsertIndexedAssets(data)
	case "videos":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"assets":                   "/assets",
		"assets-multipart-uploads": "/assets/multipart-uploads",
		"embed":                    "/embed/tasks",
		"embed-v2":                 "/embed-v2/tasks",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "entities", ParentTable: "entity-collections", ParentIDParam: "entity_collection_id", PathTemplate: "/entity-collections/{entity_collection_id}/entities", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "entity_collection_id", Field: "id"},
		}},
		{Name: "indexed_assets", ParentTable: "indexes", ParentIDParam: "index-id", PathTemplate: "/indexes/{index-id}/indexed-assets", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "index-id", Field: "id"},
		}},
		{Name: "videos", ParentTable: "indexes", ParentIDParam: "index-id", PathTemplate: "/indexes/{index-id}/videos", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "index-id", Field: "id"},
		}},
	}
}
