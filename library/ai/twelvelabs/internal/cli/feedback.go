...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "twelvelabs-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TWELVELABS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TWELVELABS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "twelvelabs-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/twelvelabs-pp-cli/feedback.jsonl.
When ` + "`TWELVELABS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TWELVELABS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twelvelabs-pp-cli feedback list
  twelvelabs-pp-cli feedback list --limit 5
  twelvelabs-pp-cli feedback list --json`,
