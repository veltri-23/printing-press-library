...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "twelvelabs-pp-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		// cross-domain or scheme-downgrade 3xx must not receive the auth
		// credential, even though we are inside CheckRedirect where Go's
		// automatic stripping has already run.
		if req.URL.Host == via[0].URL.Host && req.URL.Scheme == via[0].URL.Scheme {
			...
			// Cross-host or scheme-downgrade hop: Go strips standard auth
			// headers (Authorization, Cookie) but not custom ones, so a custom
			// API-key header would be forwarded verbatim to the redirect
			// target. Delete it explicitly.
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "twelvelabs-pp-cli/1.3.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export TWELVELABS_X_API_KEY=<your-token> or twelvelabs-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.TwelvelabsXApiKey)
	}
