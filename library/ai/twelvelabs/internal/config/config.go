...
type Config struct {
	...
	Path              string            `toml:"-"`
	TwelvelabsXApiKey string            `toml:"x_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TWELVELABS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "twelvelabs-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TWELVELABS_X_API_KEY"); v != "" {
		cfg.TwelvelabsXApiKey = v
		cfg.AuthSource = "env:TWELVELABS_X_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.TwelvelabsXApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TWELVELABS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
