# Wavespeed Printed CLI Agent Guide

This directory is a generated `wavespeed-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
wavespeed-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
wavespeed-pp-cli which "<capability>" --json
wavespeed-pp-cli <command> --help
```
...
```bash
wavespeed-pp-cli <command> --agent
```
...
```bash
wavespeed-pp-cli <command> --help
wavespeed-pp-cli <command> --dry-run --agent
```
