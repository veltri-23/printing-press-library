# Vagaro Printed CLI Agent Guide

This directory is a generated `vagaro-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
vagaro-pp-cli doctor --json
vagaro-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
vagaro-pp-cli which "<capability>" --json
...
```bash
vagaro-pp-cli <command> --help
vagaro-pp-cli <command> --dry-run --agent
```
