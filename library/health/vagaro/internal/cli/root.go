...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "vagaro-pp-cli",
		Short: `Vagaro CLI — Every Vagaro discovery feature, plus the marketplace-wide availability search, price comparison, and local database no …`,
		Long: `Vagaro CLI — Every Vagaro discovery feature, plus the marketplace-wide availability search, price comparison, and local database no …
...
Health check: run 'vagaro-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("vagaro-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'vagaro-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 4, "Max requests per second (0 to disable); the adaptive limiter backs off on 429")
	...
	rootCmd.AddCommand(newVagaroSyncCmd(flags))
	...
	rootCmd.AddCommand(newNovelMarketCmd(flags))
	rootCmd.AddCommand(newNovelMenuDiffCmd(flags))
	...
	rootCmd.AddCommand(newClassesPromotedCmd(flags))
	rootCmd.AddCommand(newListingsPromotedCmd(flags))
