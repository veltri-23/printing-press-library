...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VAGARO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VAGARO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vagaro-pp-cli/feedback")
...
When ` + "`VAGARO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VAGARO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vagaro-pp-cli feedback list
  vagaro-pp-cli feedback list --limit 5
  vagaro-pp-cli feedback list --json`,
