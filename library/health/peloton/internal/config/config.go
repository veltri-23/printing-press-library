...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("PELOTON_OAUTH_USERNAME"); v != "" {
		cfg.PelotonOauthUsername = v
		cfg.markEnvOverride("PelotonOauthUsername")
		cfg.AuthSource = "env:PELOTON_OAUTH_USERNAME"
		cfg.CredentialSource = "env:PELOTON_OAUTH_USERNAME"
	}
	if v := os.Getenv("PELOTON_OAUTH_PASSWORD"); v != "" {
		cfg.PelotonOauthPassword = v
		cfg.markEnvOverride("PelotonOauthPassword")
		cfg.AuthSource = "env:PELOTON_OAUTH_PASSWORD"
		cfg.CredentialSource = "env:PELOTON_OAUTH_PASSWORD"
	}
	...
	if v := os.Getenv("PELOTON_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("PELOTON_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	...
	if v := strings.TrimSpace(os.Getenv("PELOTON_USER_ID")); v != "" {
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("PELOTON_CONFIG"); path != "" {
...
func (c *Config) AuthHeader() string {
	...
	// the client ID as an authorization credential is rejected by the API.
	if c.AccessToken != "" {
...
func (c *Config) hasCredentialFields() bool {
	...
	if c.PelotonOauthUsername != "" {
		return true
	}
	if c.PelotonOauthPassword != "" {
		return true
	}
	return false
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		PelotonOauthUsername: c.PelotonOauthUsername,
		PelotonOauthPassword: c.PelotonOauthPassword,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.PelotonOauthUsername = creds.PelotonOauthUsername
	c.PelotonOauthPassword = creds.PelotonOauthPassword
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.PelotonOauthUsername = ""
	delete(c.envOverrides, "PelotonOauthUsername")
	c.updateFileConfigField("PelotonOauthUsername")
	c.PelotonOauthPassword = ""
	delete(c.envOverrides, "PelotonOauthPassword")
	c.updateFileConfigField("PelotonOauthPassword")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PelotonOauthUsername"] {
			out.PelotonOauthUsername = c.fileConfig.PelotonOauthUsername
		}
		if c.envOverrides["PelotonOauthPassword"] {
			out.PelotonOauthPassword = c.fileConfig.PelotonOauthPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PelotonOauthUsername":
		c.fileConfig.PelotonOauthUsername = c.PelotonOauthUsername
	case "PelotonOauthPassword":
		c.fileConfig.PelotonOauthPassword = c.PelotonOauthPassword
	}
}
