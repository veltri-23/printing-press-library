...
In local mode: searches locally synced data only.`,
  ...
  peloton-pp-cli search "error timeout"

  # Force local search only
  peloton-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  peloton-pp-cli search "status" --type account --data-source local
  # JSON output for piping
  peloton-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v2/ride/archived
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v2/ride/archived", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'peloton-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
