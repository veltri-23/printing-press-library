...
func resolveReadWithStrategyResponsePathAndJSONGuard(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, responsePath string, guardLiveJSON bool, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and local fallback unavailable: %w\n\nOriginal error: %v", fallbackErr, err)
		}
...
// resolveLocal reads data from the local SQLite store. It refuses endpoint
// query parameters because the store cannot faithfully apply provider filters.
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	if len(params) > 0 {
		return nil, DataProvenance{}, fmt.Errorf("local store cannot apply endpoint filters; use --data-source live or the dedicated offline command")
	}
	db, err := openStoreForRead(ctx, "peloton-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'peloton-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'peloton-pp-cli sync' first")
	}
	defer db.Close()
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'peloton-pp-cli sync' first", resourceType)
		}
