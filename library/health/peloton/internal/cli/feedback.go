...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PELOTON_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PELOTON_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "peloton-pp-cli/feedback")
...
When ` + "`PELOTON_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PELOTON_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  peloton-pp-cli feedback list
  peloton-pp-cli feedback list --limit 5
  peloton-pp-cli feedback list --json`,
