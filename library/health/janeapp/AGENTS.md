# Jane App Printed CLI Agent Guide

This directory is a generated `janeapp-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
janeapp-pp-cli doctor --json
janeapp-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
janeapp-pp-cli which "<capability>" --json
janeapp-pp-cli <command> --help
```
...
```bash
janeapp-pp-cli <command> --help
janeapp-pp-cli <command> --dry-run --agent
```
