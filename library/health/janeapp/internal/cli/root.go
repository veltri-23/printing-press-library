...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "janeapp-pp-cli",
		Short: `Book, view, and manage your Jane (janeapp.com) appointments across every clinic from the terminal — unified agenda, next-opening finder, and availability watch the patient portal can't do.`,
		Long: `Book, view, and manage your Jane (janeapp.com) appointments across every clinic from the terminal — unified agenda, next-opening finder, and availability watch the patient portal can't do.
...
Health check: run 'janeapp-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("janeapp-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'janeapp-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.clinicName, "clinic", "", "Which Jane clinic to target (see 'janeapp-pp-cli clinic list')")
	...
	rootCmd.AddCommand(newClinicCmd(flags))
	rootCmd.AddCommand(newAppointmentsViewCmd(flags))
	...
	rootCmd.AddCommand(newNovelConflictCheckCmd(flags))
	...
	rootCmd.AddCommand(newStaffPromotedCmd(flags))
	rootCmd.AddCommand(newTreatmentsPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if err := applyActiveClinic(f, cfg); err != nil {
		return nil, err
	}
...
// applyActiveClinic overrides cfg.BaseURL and the session cookie with the
// resolved clinic's, so every command targets the right Jane tenant. The
// JANEAPP_BASE_URL env override (used by verify/dogfood mock servers) always
// wins, so a resolved clinic never redirects a test run away from its mock.
func applyActiveClinic(f *rootFlags, cfg *config.Config) error {
	if os.Getenv("JANEAPP_BASE_URL") != "" {
		return nil
	}
	clinic, err := resolveActiveClinic(f)
	if err != nil {
		return err
	}
	if clinic == nil {
	...
	if clinic.BaseURL != "" {
		cfg.BaseURL = clinic.BaseURL
	}
	if clinic.Session != "" {
		cfg.AccessToken = clinic.Session
	}
	return nil
}

// newClientForClinic builds a client pinned to a specific clinic, for
// --all-clinics fan-out where several tenants are queried in one command.
func (f *rootFlags) newClientForClinic(clinic *Clinic) (*client.Client, error) {
	cfg, err := config.Load(f.configPath)
	if err != nil {
		return nil, configErr(err)
	}
	if os.Getenv("JANEAPP_BASE_URL") == "" && clinic != nil {
		if clinic.BaseURL != "" {
			cfg.BaseURL = clinic.BaseURL
		}
		if clinic.Session != "" {
