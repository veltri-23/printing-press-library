...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JANEAPP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JANEAPP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "janeapp-pp-cli/feedback")
...
When ` + "`JANEAPP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JANEAPP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  janeapp-pp-cli feedback list
  janeapp-pp-cli feedback list --limit 5
  janeapp-pp-cli feedback list --json`,
