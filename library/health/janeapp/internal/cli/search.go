...
Run sync first to populate the local search index.`,
  ...
  janeapp-pp-cli search "status"

  # Force local search explicitly
  janeapp-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  janeapp-pp-cli search "status" --type appointments --data-source local
  # JSON output for piping
  janeapp-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'janeapp-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
