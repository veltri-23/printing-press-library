...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  janeapp-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  janeapp-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"appointments", "disciplines", "locations", "staff", "treatments"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  janeapp-pp-cli workflow status

  # Show status as JSON
  janeapp-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("janeapp-pp-cli")
			}
