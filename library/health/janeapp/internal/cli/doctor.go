...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  janeapp-pp-cli doctor
  janeapp-pp-cli doctor --json
  janeapp-pp-cli doctor --fail-on warn
  janeapp-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				// Reflect the active Jane clinic so doctor reports the real
				// target host + session rather than the placeholder base URL.
				if aerr := applyActiveClinic(flags, cfg); aerr != nil {
					report["clinic"] = fmt.Sprintf("error: %s", aerr)
				}
				if clinic, cerr := resolveActiveClinic(flags); cerr == nil && clinic != nil {
					report["clinic"] = clinic.Name
				}
				report["config"] = "ok"
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "janeapp-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".janeapp.com"
				}
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "janeapp-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("janeapp-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'janeapp-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'janeapp-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'janeapp-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'janeapp-pp-cli sync' to refresh."
	}
	return report
}
