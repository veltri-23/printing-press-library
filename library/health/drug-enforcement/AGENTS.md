# Drug Enforcement Printed CLI Agent Guide

This directory is a generated `drug-enforcement-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
drug-enforcement-pp-cli doctor --json
drug-enforcement-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
drug-enforcement-pp-cli which "<capability>" --json
drug-enforcement-pp-cli <command> --help
```
...
```bash
drug-enforcement-pp-cli <command> --agent
```
...
```bash
drug-enforcement-pp-cli <command> --help
drug-enforcement-pp-cli <command> --dry-run --agent
```
