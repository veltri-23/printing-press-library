...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "drug-enforcement-pp-cli",
		Short: `Search FDA drug recall enforcement records — keyless, offline-cacheable, agent-native. Reports FDA facts only; never says a drug is safe.`,
		Long: `Search FDA drug recall enforcement records — keyless, offline-cacheable, agent-native. Reports FDA facts only; never says a drug is safe.
...
Run 'drug-enforcement-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("drug-enforcement-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'drug-enforcement-pp-cli profile list')")
	...
	rootCmd.AddCommand(newEnforcementPromotedCmd(flags))
	rootCmd.AddCommand(newCheckCmd(flags))
	rootCmd.AddCommand(newFirmCmd(flags))
	rootCmd.AddCommand(newRecentCmd(flags))
	rootCmd.AddCommand(newReferenceCmd(flags))
