...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  drug-enforcement-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  drug-enforcement-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  drug-enforcement-pp-cli workflow status

  # Show status as JSON
  drug-enforcement-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("drug-enforcement-pp-cli")
			}
