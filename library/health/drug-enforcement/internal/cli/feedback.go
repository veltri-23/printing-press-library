...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DRUG_ENFORCEMENT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DRUG_ENFORCEMENT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "drug-enforcement-pp-cli/feedback")
...
When ` + "`DRUG_ENFORCEMENT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DRUG_ENFORCEMENT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  drug-enforcement-pp-cli feedback list
  drug-enforcement-pp-cli feedback list --limit 5
  drug-enforcement-pp-cli feedback list --json`,
