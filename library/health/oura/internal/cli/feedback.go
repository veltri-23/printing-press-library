...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "oura-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OURA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OURA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "oura-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/oura-pp-cli/feedback.jsonl.
When ` + "`OURA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OURA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  oura-pp-cli feedback list
  oura-pp-cli feedback list --limit 5
  oura-pp-cli feedback list --json`,
