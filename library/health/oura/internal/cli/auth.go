...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Oura",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  oura-pp-cli auth setup\n  oura-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Register an app and copy your credentials at: https://cloud.ouraring.com/oauth/applications")
			fmt.Fprintln(w, "  Register an OAuth2 application, then run `auth login` for the authorization_code browser flow (scopes: daily, heartrate, personal, workout, session, tag, spo2Daily). Static personal access tokens were deprecated December 2025.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then run:")
			fmt.Fprintln(w, "  oura-pp-cli login")
			if !launch {
				return nil
			}
			launchURL := "https://cloud.ouraring.com/oauth/applications"
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("OURA_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("OURA_CLIENT_SECRET"), "OAuth2 client secret")
...
func runOAuthLogin(cmd *cobra.Command, flags *rootFlags, clientID, clientSecret string, port int) error {
	...
	scopes := []string{"daily", "email", "heartrate", "personal", "session", "spo2Daily", "tag", "workout"}
	if len(scopes) > 0 {
...
func resolveOAuthCredentials(cmd *cobra.Command, flags *rootFlags, cfg *config.Config, clientID, clientSecret string) (string, string, error) {
	...
	if flags.noInput {
		return "", "", fmt.Errorf("OAuth2 client ID is required; pass --client-id, set OURA_CLIENT_ID, or run 'oura-pp-cli login' without --no-input")
	}
...
func printOAuthCredentialHint(w io.Writer) {
	fmt.Fprintln(w, "Create OAuth credentials at: https://cloud.ouraring.com/oauth/applications")
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				fmt.Fprintf(w, "  %s Not authenticated. Run 'oura-pp-cli login' to authenticate.\n", red("FAIL"))
				return nil
			}
			...
			if cfg.TokenExpiry.IsZero() {
				...
				if cfg.RefreshToken != "" {
					...
					fmt.Fprintf(w, "  %s Token expired. Run 'oura-pp-cli login' to re-authenticate.\n", red("FAIL"))
				}
			}
