...
In local mode: searches locally synced data only.`,
  ...
  oura-pp-cli search "error timeout"

  # Force local search only
  oura-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  oura-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  oura-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'oura-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "sandbox":
				results, err = db.SearchSandbox(query, limit)
			case "usercollection":
				results, err = db.SearchUsercollection(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSandbox(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sandbox failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchUsercollection(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search usercollection failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
