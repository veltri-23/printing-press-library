...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  oura-pp-cli doctor
  oura-pp-cli doctor --json
  oura-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "oura-pp-cli login"
					report["auth_key_url"] = "https://cloud.ouraring.com/oauth/applications"
					report["auth_instructions"] = "Register an OAuth2 application, then run `auth login` for the authorization_code browser flow (scopes: daily, heartrate, personal, workout, session, tag, spo2Daily). Static personal access tokens were deprecated December 2025."
				} else {
					authConfigured = true
			...
			if os.Getenv("OURA_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "OURA_OAUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "OURA_OAUTH2")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "oura-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'oura-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'oura-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'oura-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'oura-pp-cli sync' to refresh."
	}
	return report
}
