...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OURA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "oura-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("OURA_OAUTH2"); v != "" {
		cfg.OuraOauth2 = v
		cfg.markEnvOverride("OuraOauth2")
		cfg.AuthSource = "env:OURA_OAUTH2"
	}
	...
	if cfg.AuthSource == "" && cfg.OuraOauth2 != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("OURA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("OURA_AUTHORIZATION_URL"); v != "" {
		cfg.AuthorizationURL = v
	}
	if v := os.Getenv("OURA_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.OuraOauth2 != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:OURA_OAUTH2"
		}
		return "Bearer " + c.OuraOauth2
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.OuraOauth2 = ""
	delete(c.envOverrides, "OuraOauth2")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["OuraOauth2"] {
			out.OuraOauth2 = c.fileConfig.OuraOauth2
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "OuraOauth2":
		c.fileConfig.OuraOauth2 = c.OuraOauth2
	}
}
