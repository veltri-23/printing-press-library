# Oura Printed CLI Agent Guide

This directory is a generated `oura-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
oura-pp-cli doctor --json
oura-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
oura-pp-cli which "<capability>" --json
...
```bash
oura-pp-cli <command> --help
oura-pp-cli <command> --dry-run --agent
```
