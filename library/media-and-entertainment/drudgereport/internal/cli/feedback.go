...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".drudgereport-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DRUDGEREPORT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DRUDGEREPORT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "drudgereport-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.drudgereport-pp-cli/feedback.jsonl.
When ` + "`DRUDGEREPORT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DRUDGEREPORT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  drudgereport-pp-cli feedback list
  drudgereport-pp-cli feedback list --limit 5
  drudgereport-pp-cli feedback list --json`,
