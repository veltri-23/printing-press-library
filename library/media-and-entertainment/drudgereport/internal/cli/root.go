...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "drudgereport-pp-cli",
		Short: `Drudge Report in your terminal, with the editorial signal (splash, red, slot, tenure) the live page broadcasts but no other scraper preserves.`,
		Long: `Drudge Report in your terminal, with the editorial signal (splash, red, slot, tenure) the live page broadcasts but no other scraper preserves.
...
Health check: run 'drudgereport-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("drudgereport-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'drudgereport-pp-cli profile list')")
	...
	// Removed: newFeedPromotedCmd (cross-host URL concat bug in generator — the spec uses
	// https://feedpress.me/drudgereportfeed which the client treats as path-under-base_url).
	// Removed: newPagePromotedCmd (returns raw 89 KB HTML; user-facing surface is splash/headlines/breaking).
	// Both are still referenced through generator-emitted machinery; only the user-facing AddCommand is dropped.
	_ = newFeedPromotedCmd
	_ = newPagePromotedCmd
	// Drudge novel commands (hand-coded; regen-merge re-injects these AddCommands).
	rootCmd.AddCommand(newSplashCmd(flags))
	rootCmd.AddCommand(newBreakingCmd(flags))
	rootCmd.AddCommand(newHeadlinesCmd(flags))
	rootCmd.AddCommand(newTailCmd(flags))
	rootCmd.AddCommand(newTenureCmd(flags))
	rootCmd.AddCommand(newSourcesCmd(flags))
	rootCmd.AddCommand(newOnDateCmd(flags))
	rootCmd.AddCommand(newBentCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("drudgereport-pp-cli %s\n", version)
		},
	}
}
