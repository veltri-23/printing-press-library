# Drudge Report Printed CLI Agent Guide

This directory is a generated `drudgereport-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
drudgereport-pp-cli doctor --json
drudgereport-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
drudgereport-pp-cli which "<capability>" --json
drudgereport-pp-cli <command> --help
```
...
```bash
drudgereport-pp-cli <command> --agent
```
...
```bash
drudgereport-pp-cli <command> --help
drudgereport-pp-cli <command> --dry-run --agent
```
