...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "twitch-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TWITCH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TWITCH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "twitch-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/twitch-pp-cli/feedback.jsonl.
When ` + "`TWITCH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TWITCH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twitch-pp-cli feedback list
  twitch-pp-cli feedback list --limit 5
  twitch-pp-cli feedback list --json`,
