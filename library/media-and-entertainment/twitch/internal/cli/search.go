...
In local mode: searches locally synced data only.`,
  ...
  twitch-pp-cli search "error timeout"

  # Force local search only
  twitch-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  twitch-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  twitch-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /search/channels
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/search/channels", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'twitch-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "content-classification-labels":
				results, err = db.SearchContentClassificationLabels(query, limit)
			case "extensions":
				results, err = db.SearchExtensions(query, limit)
			case "twitch-helix-search":
				results, err = db.SearchTwitchHelixSearch(query, limit)
			case "users":
				results, err = db.SearchUsers(query, limit)
			case "videos":
				results, err = db.SearchVideos(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchContentClassificationLabels(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search content_classification_labels failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchExtensions(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search extensions failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTwitchHelixSearch(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search twitch_helix_search failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchVideos(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search videos failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
