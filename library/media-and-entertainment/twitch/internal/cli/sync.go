...
Resource scoping:
  ...
  twitch-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  twitch-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  twitch-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  twitch-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  twitch-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("twitch-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/twitch-pp-cli/data.db)")
...
func resourceSupportsPagination(resource string) bool {
	...
	case "twitch-helix-analytics":
		return true
	case "twitch-helix-analytics-extensions":
		return true
	case "videos":
		return true
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "authorization":
		return db.UpsertAuthorization(data)
	case "bits":
		return db.UpsertBits(data)
	case "channel-points":
		return db.UpsertChannelPoints(data)
	case "channels":
		return db.UpsertChannels(data)
	case "charity":
		return db.UpsertCharity(data)
	case "chat":
		return db.UpsertChat(data)
	case "clips":
		return db.UpsertClips(data)
	case "content-classification-labels":
		return db.UpsertContentClassificationLabels(data)
	case "entitlements":
		return db.UpsertEntitlements(data)
	case "eventsub":
		return db.UpsertEventsub(data)
	case "extensions":
		return db.UpsertExtensions(data)
	case "games":
		return db.UpsertGames(data)
	case "goals":
		return db.UpsertGoals(data)
	case "guest-star":
		return db.UpsertGuestStar(data)
	case "moderation":
		return db.UpsertModeration(data)
	case "polls":
		return db.UpsertPolls(data)
	case "predictions":
		return db.UpsertPredictions(data)
	case "raids":
		return db.UpsertRaids(data)
	case "shared-chat":
		return db.UpsertSharedChat(data)
	case "streams":
	...
	case "twitch-helix-analytics":
		return db.UpsertTwitchHelixAnalytics(data)
	case "twitch-helix-search":
		return db.UpsertTwitchHelixSearch(data)
	case "users":
		return db.UpsertUsers(data)
	case "videos":
		return db.UpsertVideos(data)
	default:
...
func defaultSyncResources() []string {
	// Default sync targets the resources that an app access token
	// (client_credentials, this CLI's default grant) can fetch without a
	// required broadcaster_id/user_id parameter. The remaining resources in
	// knownSyncResourceNames stay reachable via --resources, but they need a
	// user OAuth token and/or required params, so listing them as defaults
	// would make out-of-box `sync` mostly access-policy warnings. Mirrors the
	// curated-default approach used by other OAuth CLIs in the library.
	return []string{
		"games-top",
		"streams",
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"twitch-helix-analytics-extensions",
		"users",
		"videos",
	}
	return names
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"bits":                              "/bits/cheermotes",
		"bits-extensions":                   "/bits/extensions",
		"bits-leaderboard":                  "/bits/leaderboard",
		"chat":                              "/chat/badges/global",
		"chat-emotes-global":                "/chat/emotes/global",
		"clips":                             "/clips",
		"content-classification-labels":     "/content_classification_labels",
		"entitlements":                      "/entitlements/drops",
		"eventsub":                          "/eventsub/conduits",
		"eventsub-subscriptions":            "/eventsub/subscriptions",
		"extensions":                        "/extensions/jwt/secrets",
		"games":                             "/games",
		"games-top":                         "/games/top",
		"streams":                           "/streams",
		"streams-markers":                   "/streams/markers",
		"tags":                              "/tags/streams",
		"teams":                             "/teams",
		"twitch-helix-analytics":            "/analytics/games",
		"twitch-helix-analytics-extensions": "/analytics/extensions",
		"users":                             "/users",
		"videos":                            "/videos",
	}
