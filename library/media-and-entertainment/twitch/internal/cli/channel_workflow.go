...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  twitch-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  twitch-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"bits", "bits-extensions", "bits-leaderboard", "chat", "chat-emotes-global", "clips", "content-classification-labels", "entitlements", "eventsub", "eventsub-subscriptions", "extensions", "games", "games-top", "streams", "streams-markers", "tags", "teams", "twitch-helix-analytics", "twitch-helix-analytics-extensions", "users", "videos"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  twitch-pp-cli workflow status

  # Show status as JSON
  twitch-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("twitch-pp-cli")
			}
