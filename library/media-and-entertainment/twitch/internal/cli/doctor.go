...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  twitch-pp-cli doctor
  twitch-pp-cli doctor --json
  twitch-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TWITCH_CLIENT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("TWITCH_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "TWITCH_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "TWITCH_CLIENT_ID set during auth login")
			}
			if os.Getenv("TWITCH_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "TWITCH_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "TWITCH_CLIENT_SECRET set during auth login")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "twitch-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'twitch-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'twitch-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'twitch-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'twitch-pp-cli sync' to refresh."
	}
	return report
}
