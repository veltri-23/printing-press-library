...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "twitch-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Twitch Helix requires the Client-Id header on every request in
		// addition to the OAuth bearer; the ID must match the client the
		// token was minted for. Without it the API returns 401
		// "Client-Id header required". Set before the configurable Headers
		// map so an explicit override there still wins.
		if c.Config != nil && c.Config.ClientID != "" {
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if c.Config != nil && c.Config.ClientID != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "Client-Id", maskToken(c.Config.ClientID))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "twitch-pp-cli auth login or twitch-pp-cli auth set-token <token>")
}
