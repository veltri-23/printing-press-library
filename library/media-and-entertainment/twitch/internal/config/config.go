...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TWITCH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "twitch-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("TWITCH_CLIENT_ID"); v != "" {
		...
		cfg.AuthSource = "env:TWITCH_CLIENT_ID"
	}
	if v := os.Getenv("TWITCH_CLIENT_SECRET"); v != "" {
		cfg.ClientSecret = v
		cfg.markEnvOverride("ClientSecret")
		cfg.AuthSource = "env:TWITCH_CLIENT_SECRET"
	}
	...
	if v := os.Getenv("TWITCH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("TWITCH_TOKEN_URL"); v != "" {
		cfg.TokenURL = v
	}
	return cfg, nil
}
