# Twitch Printed CLI Agent Guide

This directory is a generated `twitch-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
twitch-pp-cli doctor --json
twitch-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
twitch-pp-cli which "<capability>" --json
twitch-pp-cli <command> --help
```
...
```bash
twitch-pp-cli <command> --help
twitch-pp-cli <command> --dry-run --agent
```
