# DICE Printed CLI Agent Guide

This directory is a generated `dice-fm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
dice-fm-pp-cli doctor --json
dice-fm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
dice-fm-pp-cli which "<capability>" --json
...
```bash
dice-fm-pp-cli <command> --help
dice-fm-pp-cli <command> --dry-run --agent
```
