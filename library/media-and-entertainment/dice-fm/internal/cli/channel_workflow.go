...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  dice-fm-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  dice-fm-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"events", "extras", "genres", "orders", "returns", "tickets", "transfers"}
			totalSynced := 0
			...
			for _, resource := range resources {
				// workflow archive is a full backfill with no incremental floor,
				// so no where-input is applied (matches the prior sinceTS="" call).
				res := syncResource(cmd.Context(), c, s, resource, nil, full, 0, false, false)
				if res.Err != nil {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  dice-fm-pp-cli workflow status

  # Show status as JSON
  dice-fm-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("dice-fm-pp-cli")
			}
