...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dice-fm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DICE_FM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DICE_FM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dice-fm-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dice-fm-pp-cli/feedback.jsonl.
When ` + "`DICE_FM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DICE_FM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dice-fm-pp-cli feedback list
  dice-fm-pp-cli feedback list --limit 5
  dice-fm-pp-cli feedback list --json`,
