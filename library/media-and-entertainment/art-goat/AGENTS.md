# art-goat Printed CLI Agent Guide

This directory is a generated `art-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
art-goat-pp-cli doctor --json
art-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
art-goat-pp-cli which "<capability>" --json
art-goat-pp-cli <command> --help
```
...
```bash
art-goat-pp-cli <command> --help
art-goat-pp-cli <command> --dry-run --agent
```
