...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ART_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ART_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "art-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.art-goat-pp-cli/feedback.jsonl.
When ` + "`ART_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ART_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  art-goat-pp-cli feedback list
  art-goat-pp-cli feedback list --limit 5
  art-goat-pp-cli feedback list --json`,
