...
func profileStorePath() (string, error) {
	...
	dir := filepath.Join(home, ".art-goat-pp-cli")
...
present (other than --profile and --config).`,
		Example: `  art-goat-pp-cli profile save my-defaults --json --compact
  art-goat-pp-cli profile save tonight-defaults --region US`,
	...
	cmd.Flags().StringVar(&description, "description", "", "Human-readable description shown in 'profile list' — required to keep saved profiles documented")
...
func newProfileUseCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  art-goat-pp-cli profile use my-defaults
  art-goat-pp-cli profile use tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newProfileListCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  art-goat-pp-cli profile list
...
func newProfileShowCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  art-goat-pp-cli profile show my-defaults
  art-goat-pp-cli profile show tonight-defaults --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			p, err := GetProfile(args[0])
			if err != nil {
...
func newProfileDeleteCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: `  art-goat-pp-cli profile delete my-defaults --yes
  art-goat-pp-cli profile delete old-profile --yes --json`,
		Args: cobra.ExactArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			name := args[0]
			s, err := loadProfileStore()
