...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'art-goat-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set it with: export ART_GOAT_API_KEY=<your-key>"+
			"\n      Run 'art-goat-pp-cli doctor' to check auth status.", err))
...
// linkLabelFor returns the label to use when printing a work's source_url.
// Some sources (notably npmtw) store Wikipedia URLs rather than a museum
// permalink, so labeling those rows as "Link:" misleads users into
// expecting the source's own catalog. Inspect the URL host and surface
// the actual destination when it doesn't match the source itself.
func linkLabelFor(source, sourceURL string) string {
	if sourceURL == "" {
		return "Link"
	}
	lower := strings.ToLower(sourceURL)
	switch {
	case strings.Contains(lower, "wikipedia.org") || strings.Contains(lower, "wikimedia.org"):
		return "Wikipedia"
	case strings.Contains(lower, "nasa.gov"):
		return "NASA"
	}
	return "Link"
}
