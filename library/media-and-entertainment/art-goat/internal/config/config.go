...
type Config struct {
	...
	Path          string            `toml:"-"`
	ArtGoatApiKey string            `toml:"goat_api_key"`
	NasaApiKey    string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("ART_GOAT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "art-goat-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("ART_GOAT_API_KEY"); v != "" {
		cfg.ArtGoatApiKey = v
		cfg.AuthSource = "env:ART_GOAT_API_KEY"
	}
	if v := os.Getenv("NASA_API_KEY"); v != "" {
		cfg.NasaApiKey = v
		cfg.AuthSource = "env:NASA_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ArtGoatApiKey != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.NasaApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("ART_GOAT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
