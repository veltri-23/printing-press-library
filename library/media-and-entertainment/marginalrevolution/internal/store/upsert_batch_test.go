...
func TestListIDsUsesParameterizedResourceType(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if err := s.Upsert("posts", "post-1", json.RawMessage(`{"id":"post-1"}`)); err != nil {
		t.Fatalf("upsert: %v", err)
	}

	ids, err := s.ListIDs(`posts; DROP TABLE resources; --`)
	if err != nil {
		t.Fatalf("ListIDs with malicious resource type should query safely: %v", err)
	}
	if len(ids) != 0 {
		t.Fatalf("malicious resource type returned IDs: %#v", ids)
	}

	var count int
	if err := s.DB().QueryRow(`SELECT COUNT(*) FROM resources`).Scan(&count); err != nil {
		t.Fatalf("resources table missing after ListIDs: %v", err)
	}
	if count != 1 {
		t.Fatalf("resources count = %d, want 1", count)
	}
}

func TestResolveByNameBindsJSONPath(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if err := s.Upsert("posts", "post-1", json.RawMessage(`{"name":"Market notes","author":{"name":"Tyler"}}`)); err != nil {
		t.Fatalf("upsert: %v", err)
	}

	got, err := s.ResolveByName("posts", "Market notes", `name') OR 1=1 --`, "name")
	if err != nil {
		t.Fatalf("ResolveByName with safe fallback field: %v", err)
	}
	if got != "post-1" {
		t.Fatalf("ResolveByName = %q, want post-1", got)
	}

	got, err = s.ResolveByName("posts", "Tyler", "author.name")
	if err != nil {
		t.Fatalf("ResolveByName nested safe field: %v", err)
	}
	if got != "post-1" {
		t.Fatalf("ResolveByName nested = %q, want post-1", got)
	}
}
