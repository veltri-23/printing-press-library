...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".marginalrevolution-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MARGINALREVOLUTION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MARGINALREVOLUTION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "marginalrevolution-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.marginalrevolution-pp-cli/feedback.jsonl.
When ` + "`MARGINALREVOLUTION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MARGINALREVOLUTION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  marginalrevolution-pp-cli feedback list
  marginalrevolution-pp-cli feedback list --limit 5
  marginalrevolution-pp-cli feedback list --json`,
