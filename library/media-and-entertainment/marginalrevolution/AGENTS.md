# Marginalrevolution Printed CLI Agent Guide

This directory is a generated `marginalrevolution-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
marginalrevolution-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
marginalrevolution-pp-cli which "<capability>" --json
...
```bash
marginalrevolution-pp-cli <command> --agent
```
...
```bash
marginalrevolution-pp-cli <command> --help
marginalrevolution-pp-cli <command> --dry-run --agent
```
