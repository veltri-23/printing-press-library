# Bandsintown Printed CLI Agent Guide

This directory is a generated `bandsintown-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
bandsintown-pp-cli doctor --json
bandsintown-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
bandsintown-pp-cli which "<capability>" --json
...
```bash
bandsintown-pp-cli <command> --agent
```
...
```bash
bandsintown-pp-cli <command> --help
bandsintown-pp-cli <command> --dry-run --agent
```
