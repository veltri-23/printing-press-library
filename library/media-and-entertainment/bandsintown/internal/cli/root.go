...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "bandsintown-pp-cli",
		Short: `Bandsintown CLI — Turn Bandsintown's two read-only endpoints into a tour-routing brain: local cache, calendar-gap detection, lineup co-bi…`,
		Long: `Bandsintown CLI — Turn Bandsintown's two read-only endpoints into a tour-routing brain: local cache, calendar-gap detection, lineup co-bi…
...
Health check: run 'bandsintown-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("bandsintown-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'bandsintown-pp-cli profile list')")
	...
	rootCmd.AddCommand(newArtistsPromotedCmd(flags))
	rootCmd.AddCommand(newEventsPromotedCmd(flags))
	// Novel transcendence commands (Double Deer routing intelligence)
	...
	rootCmd.AddCommand(newLineupCmd(flags))
	rootCmd.AddCommand(newSnapshotCmd(flags))
	rootCmd.AddCommand(newTrendCmd(flags))
	rootCmd.AddCommand(newSeaRadarCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("bandsintown-pp-cli %s\n", version)
		},
	}
}
