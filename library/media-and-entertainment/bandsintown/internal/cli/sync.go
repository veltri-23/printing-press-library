...
Exit codes & warnings:
  ...
  bandsintown-pp-cli sync

  # Sync specific resources only
  bandsintown-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  bandsintown-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  bandsintown-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  bandsintown-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  bandsintown-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/bandsintown-pp-cli/data.db)")
