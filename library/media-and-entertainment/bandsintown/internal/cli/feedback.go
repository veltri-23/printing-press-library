...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".bandsintown-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BANDSINTOWN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BANDSINTOWN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "bandsintown-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.bandsintown-pp-cli/feedback.jsonl.
When ` + "`BANDSINTOWN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BANDSINTOWN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  bandsintown-pp-cli feedback list
  bandsintown-pp-cli feedback list --limit 5
  bandsintown-pp-cli feedback list --json`,
