# Eventbrite Printed CLI Agent Guide

This directory is a generated `eventbrite-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
eventbrite-pp-cli doctor --json
eventbrite-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
eventbrite-pp-cli which "<capability>" --json
eventbrite-pp-cli <command> --help
```
...
```bash
eventbrite-pp-cli <command> --agent
```
...
```bash
eventbrite-pp-cli <command> --help
eventbrite-pp-cli <command> --dry-run --agent
```
