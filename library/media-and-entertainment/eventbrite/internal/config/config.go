...
type Config struct {
	...
	Path             string            `toml:"-"`
	EventbriteApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("EVENTBRITE_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "eventbrite-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("EVENTBRITE_API_KEY"); v != "" {
		cfg.EventbriteApiKey = v
		cfg.AuthSource = "env:EVENTBRITE_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.EventbriteApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("EVENTBRITE_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
