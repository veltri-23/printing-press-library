...
func RegisterIntents(s *server.MCPServer) {
	s.AddTool(
		mcplib.NewTool("door_check_in_roster_offline",
			mcplib.WithDescription("Prints the synced attendee roster for one event with checked-in status as CSV for a will-call sheet — no live API call required."),
			mcplib.WithBoolean("csv", mcplib.Description("Override the recipe's --csv value.")),
		),
		handleDoorCheckInRosterOffline,
	)
}

// handleDoorCheckInRosterOffline runs the door_check_in_roster_offline recipe intent: Prints the synced attendee roster for one event with checked-in status as CSV for a will-call sheet — no live API call required.
func handleDoorCheckInRosterOffline(ctx context.Context, req mcplib.CallToolRequest) (*mcplib.CallToolResult, error) {
	...
	args = appendRecipeBoolFlag(args, "csv", input["csv"], true)
