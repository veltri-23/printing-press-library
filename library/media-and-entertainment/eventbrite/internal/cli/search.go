...
In local mode: searches locally synced data only.`,
  ...
  eventbrite-pp-cli search "error timeout"

  # Force local search only
  eventbrite-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  eventbrite-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  eventbrite-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /events/{event_id}/teams/search/
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/events/{event_id}/teams/search/", map[string]string{
					"term": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'eventbrite-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
