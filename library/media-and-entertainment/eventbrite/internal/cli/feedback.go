...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".eventbrite-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EVENTBRITE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EVENTBRITE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "eventbrite-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.eventbrite-pp-cli/feedback.jsonl.
When ` + "`EVENTBRITE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EVENTBRITE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eventbrite-pp-cli feedback list
  eventbrite-pp-cli feedback list --limit 5
  eventbrite-pp-cli feedback list --json`,
