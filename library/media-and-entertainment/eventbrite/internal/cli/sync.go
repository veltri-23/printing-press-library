...
Resource scoping:
  ...
  eventbrite-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  eventbrite-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  eventbrite-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  eventbrite-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  eventbrite-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("eventbrite-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/eventbrite-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "balance_events":
		return db.UpsertBalanceEvents(data)
	case "categories":
		return db.UpsertCategories(data)
	case "discounts":
		return db.UpsertDiscounts(data)
	case "event_canned_questions":
		return db.UpsertEventCannedQuestions(data)
	case "events":
		return db.UpsertEvents(data)
	case "events_attendees":
		return db.UpsertEventsAttendees(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "events_canned_questions":
		return db.UpsertEventsCannedQuestions(data)
	case "capacity_tier":
		return db.UpsertCapacityTier(data)
	case "copy":
		return db.UpsertCopy(data)
	case "description":
		return db.UpsertDescription(data)
	case "display_settings":
		return db.UpsertDisplaySettings(data)
	case "inventory_tiers":
		return db.UpsertInventoryTiers(data)
	case "events_orders":
		return db.UpsertEventsOrders(data)
	case "publish":
		return db.UpsertPublish(data)
	case "questions":
		return db.UpsertQuestions(data)
	case "schedules":
		return db.UpsertSchedules(data)
	case "events_seatmaps":
		return db.UpsertEventsSeatmaps(data)
	case "structured_content":
		return db.UpsertStructuredContent(data)
	case "teams":
		return db.UpsertTeams(data)
	case "ticket_buyer_settings":
		return db.UpsertTicketBuyerSettings(data)
	case "ticket_classes":
		return db.UpsertTicketClasses(data)
	case "unpublish":
		return db.UpsertUnpublish(data)
	case "formats":
		return db.UpsertFormats(data)
	case "media":
		return db.UpsertMedia(data)
	case "orders":
		return db.UpsertOrders(data)
	case "organizations_attendees":
		return db.UpsertOrganizationsAttendees(data)
	case "organizations_discounts":
		return db.UpsertOrganizationsDiscounts(data)
	case "organizations_events":
		return db.UpsertOrganizationsEvents(data)
	case "members":
		return db.UpsertMembers(data)
	case "organizations_orders":
		return db.UpsertOrganizationsOrders(data)
	case "roles":
		return db.UpsertRoles(data)
	case "organizations_seatmaps":
		return db.UpsertOrganizationsSeatmaps(data)
	case "text_overrides":
		return db.UpsertTextOverrides(data)
	case "organizations_ticket_groups":
		return db.UpsertOrganizationsTicketGroups(data)
	case "organizations_venues":
		return db.UpsertOrganizationsVenues(data)
	case "organizations_webhooks":
		return db.UpsertOrganizationsWebhooks(data)
	case "series":
		return db.UpsertSeries(data)
	case "series_events":
		return db.UpsertSeriesEvents(data)
	case "subcategories":
		return db.UpsertSubcategories(data)
	case "ticket-groups":
		return db.UpsertTicketGroups(data)
	case "users":
		return db.UpsertUsers(data)
	case "users_orders":
		return db.UpsertUsersOrders(data)
	case "users_organizations":
		return db.UpsertUsersOrganizations(data)
	case "venues":
		return db.UpsertVenues(data)
	case "venues_events":
		return db.UpsertVenuesEvents(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"reports-attendees":      "/reports/attendees/",
		"subcategories":          "/subcategories/",
		"users":                  "/users/me/",
		"users-me-organizations": "/users/me/organizations/",
		"webhooks":               "/webhooks/",
	}
