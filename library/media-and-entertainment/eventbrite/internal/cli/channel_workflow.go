...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  eventbrite-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  eventbrite-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "formats", "media", "reports", "reports-attendees", "subcategories", "users", "users-me-organizations", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  eventbrite-pp-cli workflow status

  # Show status as JSON
  eventbrite-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("eventbrite-pp-cli")
			}
