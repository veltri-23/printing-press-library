...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".tella-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TELLA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TELLA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tella-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.tella-pp-cli/feedback.jsonl.
When ` + "`TELLA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TELLA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tella-pp-cli feedback list
  tella-pp-cli feedback list --limit 5
  tella-pp-cli feedback list --json`,
