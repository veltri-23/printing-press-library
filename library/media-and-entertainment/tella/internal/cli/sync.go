...
Exit codes & warnings:
  ...
  tella-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tella-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tella-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tella-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tella-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/tella-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "playlists_videos":
		return db.UpsertPlaylistsVideos(data)
	case "clips":
