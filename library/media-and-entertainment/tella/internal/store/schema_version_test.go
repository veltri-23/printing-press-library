...
// TestMigrate_AddsColumnsOnUpgrade_PlaylistsVideos verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_PlaylistsVideos(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE playlists_videos (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(playlists_videos)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from playlists_videos after migrate", want)
		}
	}
}
