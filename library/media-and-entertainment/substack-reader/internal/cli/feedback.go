...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUBSTACK_READER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUBSTACK_READER_FEEDBACK_AUTO_SEND")))
...
When ` + "`SUBSTACK_READER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUBSTACK_READER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
