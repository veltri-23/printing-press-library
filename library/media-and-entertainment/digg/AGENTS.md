...
This directory is a generated `digg-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
digg-pp-cli doctor --json
digg-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
digg-pp-cli which "<capability>" --json
...
```bash
digg-pp-cli <command> --help
digg-pp-cli <command> --dry-run --agent
```
