...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".digg-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DIGG_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DIGG_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "digg-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.digg-pp-cli/feedback.jsonl.
When ` + "`DIGG_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DIGG_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  digg-pp-cli feedback list
  digg-pp-cli feedback list --limit 5
  digg-pp-cli feedback list --json`,
