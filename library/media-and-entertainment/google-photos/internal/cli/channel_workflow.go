...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  google-photos-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  google-photos-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"albums", "media_items", "picker"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  google-photos-pp-cli workflow status

  # Show status as JSON
  google-photos-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("google-photos-pp-cli")
			}
