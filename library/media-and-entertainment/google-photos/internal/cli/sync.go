...
Exit codes & warnings:
  ...
  google-photos-pp-cli sync

  # Sync specific resources only
  google-photos-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  google-photos-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  google-photos-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  google-photos-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  google-photos-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "media_items":
		return db.UpsertMediaItems(data)
	case "picker":
		return db.UpsertPicker(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"albums":      "/v1/albums",
		"media_items": "/v1/mediaItems",
		"picker":      "/v1/mediaItems",
	}
