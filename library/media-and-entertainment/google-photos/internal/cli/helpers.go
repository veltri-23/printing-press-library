...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'google-photos-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: your token may have expired. Re-run 'google-photos-pp-cli auth login' to re-authenticate"+
			"\n      or: export GOOGLE_PHOTOS_TOKEN=<your-token>"+
			"\n      Get a key at: https://console.cloud.google.com/apis/credentials"+
			"\n      Run 'google-photos-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your token may lack required scopes. Re-run 'google-photos-pp-cli auth login' to re-authorize"+
			"\n      or: export GOOGLE_PHOTOS_TOKEN=<your-token>"+
			"\n      Get a key at: https://console.cloud.google.com/apis/credentials"+
			"\n      Run 'google-photos-pp-cli doctor' to check auth status.", err))
