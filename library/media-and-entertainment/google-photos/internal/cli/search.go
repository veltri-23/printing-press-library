...
In local mode: searches locally synced data only.`,
  ...
  google-photos-pp-cli search "error timeout"

  # Force local search only
  google-photos-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  google-photos-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  google-photos-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v1/mediaItems:search
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'google-photos-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/google-photos-pp-cli/data.db)")
