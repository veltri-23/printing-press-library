...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".google-photos-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOOGLE_PHOTOS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOOGLE_PHOTOS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "google-photos-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.google-photos-pp-cli/feedback.jsonl.
When ` + "`GOOGLE_PHOTOS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOOGLE_PHOTOS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  google-photos-pp-cli feedback list
  google-photos-pp-cli feedback list --limit 5
  google-photos-pp-cli feedback list --json`,
