# PodcastIndex Printed CLI Agent Guide

This directory is a generated `podcastindex-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
podcastindex-pp-cli doctor --json
podcastindex-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
podcastindex-pp-cli which "<capability>" --json
podcastindex-pp-cli <command> --help
```
...
```bash
podcastindex-pp-cli <command> --agent
```
...
```bash
podcastindex-pp-cli <command> --help
podcastindex-pp-cli <command> --dry-run --agent
```
