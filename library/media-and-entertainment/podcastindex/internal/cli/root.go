...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "podcastindex-pp-cli",
		Short: `Podcastindex CLI — The only PodcastIndex tool that is agent-native and works offline: search, resolve, and analyse any podcast from one si…`,
		Long: `Podcastindex CLI — The only PodcastIndex tool that is agent-native and works offline: search, resolve, and analyse any podcast from one si…

Highlights (not in the official API docs):
  • tgrep   Search inside actual episode transcripts, not just titles and descriptions.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'podcastindex-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("podcastindex-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'podcastindex-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPodcastsCmd(flags))
	...
	rootCmd.AddCommand(newNovelTgrepCmd(flags))
