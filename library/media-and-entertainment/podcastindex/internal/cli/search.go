...
In local mode: searches locally synced data only.`,
  ...
  podcastindex-pp-cli search "error timeout"

  # Force local search only
  podcastindex-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  podcastindex-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  podcastindex-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'podcastindex-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "recent":
				results, err = db.SearchRecent(query, limit)
			case "static":
				results, err = db.SearchStatic(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchRecent(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search recent failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchStatic(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search static failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
