...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  podcastindex-pp-cli doctor
  podcastindex-pp-cli doctor --json
  podcastindex-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export PODCASTINDEX_KEY=<your-key>"
					report["auth_key_url"] = "https://github.com/tbowers/python-podcastindex-org-lambda"
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "PODCASTINDEX_KEY")
				if os.Getenv("PODCASTINDEX_KEY") != "" {
					authEnvSet = append(authEnvSet, "PODCASTINDEX_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("PODCASTINDEX_KEY") != "" {
				authEnvSet = append(authEnvSet, "PODCASTINDEX_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "PODCASTINDEX_KEY optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "PODCASTINDEX_ORG_API_KEY")
				if os.Getenv("PODCASTINDEX_ORG_API_KEY") != "" {
					authEnvSet = append(authEnvSet, "PODCASTINDEX_ORG_API_KEY")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("PODCASTINDEX_ORG_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "PODCASTINDEX_ORG_API_KEY")
			} else {
				authEnvInfo = append(authEnvInfo, "PODCASTINDEX_ORG_API_KEY optional")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.PodcastindexOrgAuthorization
				}
				recordAdditionalAuthEnv("PODCASTINDEX_ORG_AUTHORIZATION", configuredValue)
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.PodcastindexOrgDate
				}
				recordAdditionalAuthEnv("PODCASTINDEX_ORG_DATE", configuredValue)
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.PodcastindexOrgUserAgent
				}
				recordAdditionalAuthEnv("PODCASTINDEX_ORG_USER_AGENT", configuredValue)
			}
			switch {
			...
			default:
				report["env_vars"] = fmt.Sprintf("OK %d/%d available", len(authEnvSet), 5)
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "podcastindex-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'podcastindex-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'podcastindex-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'podcastindex-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'podcastindex-pp-cli sync' to refresh."
	}
	return report
}
