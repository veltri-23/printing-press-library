...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  podcastindex-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  podcastindex-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "episodes", "podcasts", "podcasts-bymedium", "podcasts-bytag", "podcasts-trending", "recent", "recent-episodes", "recent-newfeeds", "recent-newvaluefeeds", "static", "static-public-podcastindex-dead-feeds-csv", "static-public-podcastindex-feeds-db-tgz", "static-stats-v4vmusic-opml", "static-tracking-feed-value-blocks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  podcastindex-pp-cli workflow status

  # Show status as JSON
  podcastindex-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("podcastindex-pp-cli")
			}
