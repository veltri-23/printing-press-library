...
Resource scoping:
  ...
  podcastindex-pp-cli sync

  # Sync specific resources only
  podcastindex-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  podcastindex-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  podcastindex-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  podcastindex-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  podcastindex-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/podcastindex-pp-cli/data.db)")
...
func syncResourceSinceParam(resource string) string {
	...
	case "recent":
		return "since"
	case "recent-newfeeds":
		return "since"
	case "recent-newvaluefeeds":
		return "since"
	}
	return ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "hub":
		return db.UpsertHub(data)
	case "podcasts":
		return db.UpsertPodcasts(data)
	case "recent":
		return db.UpsertRecent(data)
	case "static":
		return db.UpsertStatic(data)
	case "stats":
		return db.UpsertStats(data)
	case "value":
		return db.UpsertValue(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"static",
		"static-public-podcastindex-dead-feeds-csv",
		"static-public-podcastindex-feeds-db-tgz",
		"static-stats-v4vmusic-opml",
		"static-tracking-feed-value-blocks",
	}
}
...
func knownSyncResourceNames() []string {
	names := []string{
		...
		"static-public-podcastindex-feeds-db-tgz",
		"static-stats-v4vmusic-opml",
		"static-tracking-episode-value-blocks",
		"static-tracking-feed-value-blocks",
	}
	return names
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"podcasts":             "/podcasts/dead",
		"podcasts-bymedium":    "/podcasts/bymedium",
		"podcasts-bytag":       "/podcasts/bytag",
		"podcasts-trending":    "/podcasts/trending",
		"recent":               "/recent/feeds",
		"recent-episodes":      "/recent/episodes",
		"recent-newfeeds":      "/recent/newfeeds",
		"recent-newvaluefeeds": "/recent/newvaluefeeds",
		"recent-soundbites":    "/recent/soundbites",
		"static":               "/static/stats/v4vmusic.rss",
		"static-public-podcastindex-dead-feeds-csv": "/static/public/podcastindex_dead_feeds.csv",
		"static-public-podcastindex-feeds-db-tgz":   "/static/public/podcastindex_feeds.db.tgz",
		"static-stats-v4vmusic-opml":                "/static/stats/v4vmusic.opml",
		"static-tracking-episode-value-blocks":      "/static/tracking/episodeValueBlocks",
		"static-tracking-feed-value-blocks":         "/static/tracking/feedValueBlocks",
	}
