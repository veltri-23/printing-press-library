...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "podcastindex-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PODCASTINDEX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PODCASTINDEX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "podcastindex-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/podcastindex-pp-cli/feedback.jsonl.
When ` + "`PODCASTINDEX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PODCASTINDEX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  podcastindex-pp-cli feedback list
  podcastindex-pp-cli feedback list --limit 5
  podcastindex-pp-cli feedback list --json`,
