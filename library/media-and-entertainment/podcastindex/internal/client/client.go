...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "podcastindex-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			if v := c.Config.PodcastindexOrgAuthorization; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export PODCASTINDEX_ORG_AUTHORIZATION=<your-token>")
				}
				req.Header.Set("Authorization", v)
			}
			if v := c.Config.PodcastindexOrgDate; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export PODCASTINDEX_ORG_DATE=<your-token>")
				}
				req.Header.Set("X-Auth-Date", v)
			}
			if v := c.Config.PodcastindexOrgUserAgent; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export PODCASTINDEX_ORG_USER_AGENT=<your-token>")
				}
		...
		// PodcastIndex requires a per-request SHA1 signature (X-Auth-Key,
		// X-Auth-Date, Authorization=sha1hex(key+secret+date)). The templated
		// composed-auth block above only sets static values; override them with
		// freshly computed ones here so every request authenticates. See
		// podcastindex_signer.go.
		c.signPodcastIndex(req)
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "X-Auth-Key", maskToken(authHeader))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export PODCASTINDEX_KEY=<your-token> or podcastindex-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.PodcastindexKey)
		addCredential(c.Config.PodcastindexOrgApiKey)
		addCredential(c.Config.PodcastindexOrgAuthorization)
		addCredential(c.Config.PodcastindexOrgDate)
		addCredential(c.Config.PodcastindexOrgUserAgent)
	}
