...
Resource scoping:
  ...
  podcast-goat-pp-cli sync

  # Sync specific resources only
  podcast-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  podcast-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  podcast-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  podcast-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  podcast-goat-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/podcast-goat-pp-cli/data.db)")
