...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  podcast-goat-pp-cli doctor
  podcast-goat-pp-cli doctor --json
  podcast-goat-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "podcast-goat-pp-cli auth login-service --service <huberman|acquired|founders|peterattia|spotify>"
				} else {
					authConfigured = true
			...
			// External tool presence — the YouTube adapter shells out to
			// yt-dlp. If neither PATH yt-dlp nor the lazy-download sidecar
			// is present, every YouTube URL fails at fetch time. Surfacing
			// at doctor time avoids that footgun.
			report["yt_dlp"] = collectYtDlpReport()
			...
			// yt-dlp section: surface PATH vs sidecar vs missing so users
			// don't discover the dependency at first YouTube fetch.
			if ytAny, ok := report["yt_dlp"]; ok {
				if ytRep, ok := ytAny.(map[string]any); ok {
					renderYtDlpReport(w, ytRep)
				}
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	// podcast-goat keeps its store at ~/.config/podcast-goat/podcast-goat.db
	// (the brief's documented location), not the generator-default
	// ~/.local/share/podcast-goat-pp-cli/data.db. Reporting the wrong path
	// here makes 'doctor' say the cache doesn't exist when it does.
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run a paid or free `episode get` to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'podcast-goat-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'podcast-goat-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'podcast-goat-pp-cli sync' to refresh."
	}
	return report
}
