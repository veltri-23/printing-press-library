...
// PATCH: v0.1 founders cookie tier (auth + first-time-capture honest error).
...
const (
	adapterName = "founders"
	service     = "founders"
	publicHost  = "founderspodcast.com"
)
...
var hostRE = regexp.MustCompile(`(?i)^https?://(www\.|member\.)?founderspodcast\.com/`)
...
func (a *Adapter) Fetch(ctx context.Context, url string) (*transcript.Transcript, error) {
	if !source.HasCookie(service) {
		return nil, &source.CookieMissingError{
			Service: service,
			Hint:    "run `podcast-goat-pp-cli auth login --chrome --service founders` after logging in at founderspodcast.com",
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("founders GET %s: %w", url, err)
	}
	...
	return nil, &source.NotImplementedError{
		...
		NeedsCapture: true,
		Detail: fmt.Sprintf(
			"founderspodcast.com member HTML requires first-time browser capture. Public host is %s; "+
				"as a workaround try `episode get %s --paid --provider taddy` (David Senra's show ships podcast-provided transcripts).",
			publicHost, url,
		),
	}
}
