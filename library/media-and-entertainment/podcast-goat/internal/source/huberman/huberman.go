...
// PATCH: v0.1 huberman cookie tier (auth + first-time-capture honest error).
...
const (
	adapterName = "huberman"
	service     = "huberman"
	publicHost  = "hubermanlab.com"
)
...
var hostRE = regexp.MustCompile(`(?i)^https?://(www\.|member\.)?hubermanlab\.com/`)
...
func (a *Adapter) Fetch(ctx context.Context, url string) (*transcript.Transcript, error) {
	if !source.HasCookie(service) {
		return nil, &source.CookieMissingError{
			Service: service,
			Hint:    "run `podcast-goat-pp-cli auth login --chrome --service huberman` after logging in at hubermanlab.com",
		}
	}
	...
	if err != nil {
		return nil, fmt.Errorf("huberman GET %s: %w", url, err)
	}
	...
	return nil, &source.NotImplementedError{
		...
		NeedsCapture: true,
		Detail: fmt.Sprintf(
			"hubermanlab.com Supercast HTML shape requires first-time browser capture. Public host is %s; "+
				"as a workaround try `episode get %s --paid --provider spoken` for free public episodes.",
			publicHost, url,
		),
	}
}
