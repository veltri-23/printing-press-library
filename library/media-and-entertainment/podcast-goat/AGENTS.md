# Podcast GOAT Printed CLI Agent Guide

This directory is a generated `podcast-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
podcast-goat-pp-cli doctor --json
podcast-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
podcast-goat-pp-cli which "<capability>" --json
podcast-goat-pp-cli <command> --help
```
...
```bash
podcast-goat-pp-cli <command> --agent
```
...
```bash
podcast-goat-pp-cli <command> --help
podcast-goat-pp-cli <command> --dry-run --agent
```
