...
Resource scoping:
  ...
  pexels-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pexels-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pexels-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pexels-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pexels-pp-cli sync --latest-only`,
...
func determinePaginationDefaults() paginationDefaults {
	// Pexels paginates by integer `page` (1-based) with `per_page` page size
	// (max 80). The generic offset/`limit` defaults trip Pexels' request
	// validation: an unrecognized `limit` query param returns HTTP 401
	// "Missing API key" even on otherwise-public read endpoints, so sync MUST
	// use the real param names.
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"collections-featured": "/collections/featured",
		"photos":               "/curated",
