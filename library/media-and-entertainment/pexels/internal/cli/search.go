...
func extractSearchResults(data json.RawMessage) []json.RawMessage {
	...
	// Try common wrapper paths. Pexels-specific envelopes (photos/videos/media/
	// collections) are checked first because Pexels search/list responses wrap
	// the array under those keys, not the generic "data"/"results".
	var wrapped map[string]json.RawMessage
	if json.Unmarshal(data, &wrapped) == nil {
		for _, key := range []string{"photos", "videos", "media", "collections", "data", "results", "items", "records", "entries"} {
...
In local mode: searches locally synced data only.`,
  ...
  pexels-pp-cli search "error timeout"

  # Force local search only
  pexels-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pexels-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pexels-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pexels-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "collections":
				results, err = db.SearchCollections(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCollections(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search collections failed: %w", searchErr)
					}
