...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PEXELS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PEXELS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pexels-pp-cli/feedback")
...
When ` + "`PEXELS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PEXELS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pexels-pp-cli feedback list
  pexels-pp-cli feedback list --limit 5
  pexels-pp-cli feedback list --json`,
