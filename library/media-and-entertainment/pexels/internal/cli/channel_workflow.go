...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pexels-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pexels-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"collections", "collections-featured", "photos", "videos"}
			// Cap pages per resource so a live-dogfood archive of a high-volume
			// resource (photos/videos have thousands of pages) finishes inside
			// the matrix's per-command timeout. Mirrors newSyncCmd's dogfood cap.
			archiveMaxPages := 100
			...
			for _, resource := range resources {
				res := syncResource(cmd.Context(), c, s, resource, "", full, archiveMaxPages, false, nil, syncEventWriter)
				if res.Err != nil {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pexels-pp-cli workflow status

  # Show status as JSON
  pexels-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pexels-pp-cli")
			}
