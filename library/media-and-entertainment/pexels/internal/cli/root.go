...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pexels-pp-cli",
		Short: `Pexels CLI — Every Pexels photo, video, and collection endpoint — plus a local store that adds quota forecasting, dedup-aware bulk d…`,
		Long: `Pexels CLI — Every Pexels photo, video, and collection endpoint — plus a local store that adds quota forecasting, dedup-aware bulk d…
...
Health check: run 'pexels-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pexels-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pexels-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPhotosCmd(flags))
	...
	rootCmd.AddCommand(newNovelAttributionCmd(flags))
	rootCmd.AddCommand(newNovelDownloadCmd(flags))
	rootCmd.AddCommand(newNovelQuotaCmd(flags))
	rootCmd.AddCommand(newNovelResolveCmd(flags))
