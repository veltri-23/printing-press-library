...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	maxRetries := 3
	// Under the dogfood/verify harnesses each command has a tight wall-clock
	// timeout, so a 429 that triggers multiple 5s backoff waits would blow the
	// budget and report as a timeout rather than a clean rate-limit. Fail fast
	// there; production keeps the full retry resilience.
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "pexels-pp-cli/0.1 (+https://github.com/mvanhorn/printing-press-library)")
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "pexels-pp-cli/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export PEXELS_API_KEY=<your-token> or pexels-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.PexelsApiKey)
	}
