# Pexels Printed CLI Agent Guide

This directory is a generated `pexels-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pexels-pp-cli doctor --json
pexels-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pexels-pp-cli which "<capability>" --json
pexels-pp-cli <command> --help
```
...
```bash
pexels-pp-cli <command> --help
pexels-pp-cli <command> --dry-run --agent
```
