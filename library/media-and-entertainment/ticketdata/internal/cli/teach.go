...
// ticketdata-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/ticketdata/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/ticketdata/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/ticketdata/internal/learn/patterns"
...
const noLearnEnvVar = "TICKETDATA_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("ticketdata-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true", "pp:happy-args": "--query=cheapest ariana date;--resource=22323960;--resource-type=events"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  ticketdata-pp-cli teach --query "cheapest ariana date" --resource 22323960 --resource-type events`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// An explicit --json request produces the confirmation envelope even
			// though --quiet defaults true for silent background use; only a bare
			// (non-JSON) success stays silent.
			if flags.asJSON {
...
when learnings exist.`,
		Example: `  ticketdata-pp-cli recall "<question>" --agent
  ticketdata-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ticketdata-pp-cli learnings list --agent
  ticketdata-pp-cli learnings list --query "<substring>"
  ticketdata-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  ticketdata-pp-cli learnings forget "<question>" --resource <id>
  ticketdata-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
func newTeachPatternCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:happy-args": "--query-template=items in {entity};--resource-template=GROUP-{entity:category};--entity-kind=category;--resource-type=items"},
...
a whole family.`,
		Example: `  ticketdata-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  ticketdata-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
