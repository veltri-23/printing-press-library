...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TICKETDATA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TICKETDATA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ticketdata-pp-cli/feedback")
...
When ` + "`TICKETDATA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TICKETDATA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ticketdata-pp-cli feedback list
  ticketdata-pp-cli feedback list --limit 5
  ticketdata-pp-cli feedback list --json`,
