# Skool Printed CLI Agent Guide

This directory is a generated `skool-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
skool-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
skool-pp-cli which "<capability>" --json
...
```bash
skool-pp-cli <command> --help
skool-pp-cli <command> --dry-run --agent
```
