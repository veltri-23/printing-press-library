...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	var syncCommunityOverride string
	var full bool
	var since string
	var concurrency int
	var dbPath string
...
Exit codes & warnings:
  ...
  skool-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Tag every row written during this sync with the active community
			// slug so cross-community SQL can group/filter cleanly. Resolution
			// order: --community flag, then SKOOL_COMMUNITY env var, then the
			// template_vars.community config field. The override also
			// rewrites the template var so all subsequent endpoint calls in
			// this sync hit the right community.
			activeCommunity := syncCommunityOverride
			if activeCommunity == "" && c.Config != nil {
				activeCommunity = c.Config.TemplateVars["community"]
			}
			if activeCommunity != "" {
				if c.Config != nil {
					...
					c.Config.TemplateVars["community"] = activeCommunity
				}
				db.SetCommunityTag(activeCommunity)
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/skool-pp-cli/data.db)")
	cmd.Flags().StringVar(&syncCommunityOverride, "community", "", "Sync this community (overrides template_vars.community in config). Rows are tagged with this slug so cross-community SQL stays clean.")
...
// Preserved from multi-community feature work.
