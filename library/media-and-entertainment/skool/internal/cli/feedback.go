...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".skool-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SKOOL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SKOOL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/media-and-entertainment/skool/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.skool-pp-cli/feedback.jsonl.
When ` + "`SKOOL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SKOOL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  skool-pp-cli feedback list
  skool-pp-cli feedback list --limit 5
  skool-pp-cli feedback list --json`,
