...
var readCommandResources = map[string][]string{
	...
	"hackernews-pp-cli stories search": {"stories"},
	"hackernews-pp-cli updates":        {"updates"},
	"hackernews-pp-cli updates list":   {"updates"},
	"hackernews-pp-cli updates get":    {"updates"},
	"hackernews-pp-cli updates search": {"updates"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "HACKERNEWS_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale hackernews-pp-cli cache (refresh failed: %v)\n", err)
