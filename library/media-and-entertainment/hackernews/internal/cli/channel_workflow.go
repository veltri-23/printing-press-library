...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  hackernews-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  hackernews-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"stories", "updates"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  hackernews-pp-cli workflow status

  # Show status as JSON
  hackernews-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("hackernews-pp-cli")
			}
