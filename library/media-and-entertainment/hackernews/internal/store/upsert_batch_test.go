...
// TestUpsertBatch_StoresScalarStoryIDs covers Hacker News Firebase list
// endpoints such as /topstories.json, which return bare integer ID arrays
// instead of objects. The sync path normalizes those scalars into {"id": ...}
// rows so story-list syncs populate the canonical stories resource.
func TestUpsertBatch_StoresScalarStoryIDs(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("stories", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	if stored != len(items) {
	...
	var typed int
	if err := s.DB().QueryRow(`SELECT COUNT(*) FROM stories`).Scan(&typed); err != nil {
		t.Fatalf("count stories: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("stories count = %d, want %d", typed, len(items))
	}
}
