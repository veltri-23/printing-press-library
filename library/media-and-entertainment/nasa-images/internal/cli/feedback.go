...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".nasa-images-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NASA_IMAGES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NASA_IMAGES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nasa-images-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.nasa-images-pp-cli/feedback.jsonl.
When ` + "`NASA_IMAGES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NASA_IMAGES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nasa-images-pp-cli feedback list
  nasa-images-pp-cli feedback list --limit 5
  nasa-images-pp-cli feedback list --json`,
