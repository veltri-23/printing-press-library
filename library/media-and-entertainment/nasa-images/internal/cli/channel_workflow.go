...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  nasa-images-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  nasa-images-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// In --json mode the workflow archive must emit a single JSON
			// document on stdout. syncResource writes its NDJSON event
			// stream (sync_start / sync_warning / sync_complete) directly to
			// os.Stdout, which would precede the summary JSON and break any
			// single-document parser (e.g. dogfood's json_fidelity check).
			// Swap os.Stdout to /dev/null for the sync loop ONLY, restoring
			// before the summary JSON encode so the summary lands on the
			// real stdout. cmd.OutOrStdout() resolves at call time, so the
			// restore must happen before json.NewEncoder reads it.
			...
			for _, resource := range resources {
				...
				if !flags.asJSON {
					// Skip the per-resource summary line in JSON mode so the
					// combined stdout+stderr a JSON-parsing consumer sees
					// (incl. dogfood's json_fidelity check) stays valid JSON.
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  nasa-images-pp-cli workflow status

  # Show status as JSON
  nasa-images-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("nasa-images-pp-cli")
			}
