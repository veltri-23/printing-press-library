...
Resource scoping:
  ...
  nasa-images-pp-cli sync

  # Sync specific resources only
  nasa-images-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nasa-images-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nasa-images-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nasa-images-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nasa-images-pp-cli sync --latest-only`,
...
func syncResource(c interface {
	...
	for {
		...
		// NASA's /search refuses requests carrying only page/page_size — at
		// least one content parameter (q, media_type, year_start, nasa_id,
		// etc.) is required. When syncing the media resource and the caller
		// supplied no content param, default media_type to "image" so a bare
		// `sync` returns useful data instead of HTTP 400. The dedicated
		// `mirror search` command is the preferred populator and stays
		// explicit. Hand-patched per dogfood feedback —
		// recorded in .printing-press-patches.json.
		if resource == "media" {
			hasContent := false
			for _, k := range []string{"q", "media_type", "year_start", "year_end", "center", "photographer", "secondary_creator", "keywords", "location", "description", "description_508", "title", "nasa_id"} {
				if _, ok := params[k]; ok {
					hasContent = true
					break
				}
