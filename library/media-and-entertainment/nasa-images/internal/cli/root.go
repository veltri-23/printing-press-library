...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nasa-images-pp-cli",
		Short: `Nasa Images CLI — Every NASA Image and Video Library endpoint, plus a local SQLite mirror, FTS search, resumable bulk download, caption t…`,
		Long: `Nasa Images CLI — Every NASA Image and Video Library endpoint, plus a local SQLite mirror, FTS search, resumable bulk download, caption t…
...
Health check: run 'nasa-images-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nasa-images-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'nasa-images-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAlbumsPromotedCmd(flags))
	rootCmd.AddCommand(newAssetsPromotedCmd(flags))
	rootCmd.AddCommand(newCaptionsPromotedCmd(flags))
	rootCmd.AddCommand(newMediaPromotedCmd(flags))
	rootCmd.AddCommand(newMetadataPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-written novel commands. See Phase 3 / absorb-manifest transcendence.
	rootCmd.AddCommand(newMirrorCmd(flags))
	rootCmd.AddCommand(newDownloadCmd(flags))
	rootCmd.AddCommand(newRecentCmd(flags))
	rootCmd.AddCommand(newCenterCmd(flags))
	rootCmd.AddCommand(newUnusedInCmd(flags))
	rootCmd.AddCommand(newTimelineCmd(flags))
	rootCmd.AddCommand(newCitationCmd(flags))

	// Attach 'fetch' subcommands to the spec-emitted parent commands so they
	// read as 'captions fetch <nasa_id>' and 'metadata fetch <nasa_id>'.
	captionsParent, _, _ := rootCmd.Find([]string{"captions"})
	if captionsParent != nil {
		captionsParent.AddCommand(newCaptionsFetchCmd(flags))
	}
	metadataParent, _, _ := rootCmd.Find([]string{"metadata"})
	if metadataParent != nil {
		metadataParent.AddCommand(newMetadataFetchCmd(flags))
	}
	assetsParent, _, _ := rootCmd.Find([]string{"assets"})
	if assetsParent != nil {
		assetsParent.AddCommand(newAssetsBestCmd(flags))
	}
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("nasa-images-pp-cli %s\n", version)
		},
	}
}
