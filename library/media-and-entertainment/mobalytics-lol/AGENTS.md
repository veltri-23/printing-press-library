# Mobalytics LoL Printed CLI Agent Guide

This directory is a generated `mobalytics-lol-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
mobalytics-lol-pp-cli doctor --json
mobalytics-lol-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
mobalytics-lol-pp-cli which "<capability>" --json
mobalytics-lol-pp-cli <command> --help
```
...
```bash
mobalytics-lol-pp-cli <command> --agent
```
...
```bash
mobalytics-lol-pp-cli <command> --help
mobalytics-lol-pp-cli <command> --dry-run --agent
```
