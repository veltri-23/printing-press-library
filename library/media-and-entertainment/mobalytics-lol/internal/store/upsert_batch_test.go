...
func TestUpsertBatch_PopulatesDataTable(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "cdn_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "cdn_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "cdn_id": "test-parent-001"}`),
	}
...
func TestUpsertBatch_TypedFailureDoesNotStrandDataGeneric(t *testing.T) {
	...
	// Items deliberately omit "cdn_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	if typed != 0 {
		t.Fatalf("data count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "cdn_id")
	}
}
