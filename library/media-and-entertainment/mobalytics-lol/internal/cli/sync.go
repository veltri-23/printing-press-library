...
Resource scoping:
  ...
  mobalytics-lol-pp-cli sync

  # Sync specific resources only
  mobalytics-lol-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  mobalytics-lol-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  mobalytics-lol-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  mobalytics-lol-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  mobalytics-lol-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("mobalytics-lol-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/mobalytics-lol-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	if effectiveSince != "" && sinceParam == "" {
		if humanFriendly {
			...
			fmt.Fprintf(os.Stderr, `{"event":"sync_warning","resource":"%s","reason":"resource_not_incremental","message":"endpoint does not declare a temporal filter parameter; incremental sync has no effect for this resource"}`+"\n", resource)
		}
		effectiveSince = ""
	}
	...
	for {
		...
		if isDryRunResponse(data) {
			if !humanFriendly {
				fmt.Fprintf(os.Stderr, `{"event":"sync_dryrun","resource":"%s"}`+"\n", resource)
			}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"versions-json": "/api/versions.json",
	}
