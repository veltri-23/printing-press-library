...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  mobalytics-lol-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  mobalytics-lol-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"versions-json"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  mobalytics-lol-pp-cli workflow status

  # Show status as JSON
  mobalytics-lol-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("mobalytics-lol-pp-cli")
			}
