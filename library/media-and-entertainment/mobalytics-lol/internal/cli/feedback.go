...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".mobalytics-lol-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOBALYTICS_LOL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOBALYTICS_LOL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mobalytics-lol-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.mobalytics-lol-pp-cli/feedback.jsonl.
When ` + "`MOBALYTICS_LOL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOBALYTICS_LOL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mobalytics-lol-pp-cli feedback list
  mobalytics-lol-pp-cli feedback list --limit 5
  mobalytics-lol-pp-cli feedback list --json`,
