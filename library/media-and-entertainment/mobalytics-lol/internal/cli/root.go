...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "mobalytics-lol-pp-cli",
		Short: `Mobalytics Lol CLI — Every League of Legends aggregator, plus a local SQLite for cross-champion queries no single page surfaces.`,
		Long: `Mobalytics Lol CLI — Every League of Legends aggregator, plus a local SQLite for cross-champion queries no single page surfaces.
...
Health check: run 'mobalytics-lol-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("mobalytics-lol-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'mobalytics-lol-pp-cli profile list')")
	...
	rootCmd.AddCommand(newVersionsJsonPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())
	// Mobalytics novel commands (Phase 3 hand-built).
	rootCmd.AddCommand(newTierListCmd(flags))
	rootCmd.AddCommand(newChampionCmd(flags))
	rootCmd.AddCommand(newCounterPoolCmd(flags))
	rootCmd.AddCommand(newMetaShiftCmd(flags))
	rootCmd.AddCommand(newCompareCmd(flags))
	rootCmd.AddCommand(newItemSetCmd(flags))
	rootCmd.AddCommand(newDuoFinderCmd(flags))
	rootCmd.AddCommand(newPoolDigestCmd(flags))
	rootCmd.AddCommand(newPowerSpikeCmd(flags))
	rootCmd.AddCommand(newFlexCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("mobalytics-lol-pp-cli %s\n", version)
		},
	}
}
