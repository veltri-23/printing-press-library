...
func newClipsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List your library (walks the opaque next_cursor; use --all to drain)",
		Example:     "  suno-pp-cli clips list",
		Annotations: map[string]string{"pp:endpoint": "clips.list", "pp:method": "POST", "pp:path": "/api/feed/v3", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Enrich each clip with its workspace membership (clips list/top/grep
			// surface a workspace column). Best-effort + local: a missing or
			// unsynced store leaves the live response untouched.
			if !flags.dryRun {
				data = enrichClipsWithWorkspace(cmd.Context(), data)
			}
	...
	cmd.Flags().StringVar(&bodyCursor, "cursor", "", "Opaque pagination cursor (the next_cursor from the prior page; omit for first page)")
	cmd.Flags().IntVar(&bodyLimit, "limit", 20, "Page size")
