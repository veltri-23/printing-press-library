...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "suno-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUNO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUNO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "suno-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/suno-pp-cli/feedback.jsonl.
When ` + "`SUNO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUNO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  suno-pp-cli feedback list
  suno-pp-cli feedback list --limit 5
  suno-pp-cli feedback list --json`,
