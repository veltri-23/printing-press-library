...
This directory is a generated `spotify-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
spotify-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
spotify-pp-cli which "<capability>" --json
...
```bash
spotify-pp-cli <command> --help
spotify-pp-cli <command> --dry-run --agent
```
