...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SPOTIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SPOTIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "spotify-pp-cli/feedback")
...
When ` + "`SPOTIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SPOTIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  spotify-pp-cli feedback list
  spotify-pp-cli feedback list --limit 5
  spotify-pp-cli feedback list --json`,
