...
Resource scoping:
  ...
  spotify-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  spotify-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  spotify-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  spotify-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  spotify-pp-cli sync --latest-only`,
...
func determinePaginationDefaults(resource string) paginationDefaults {
	...
	case "audiobooks/audiobooks_chapters":
		return paginationDefaults{
			cursorParam: "offset",
			cursorType:  "offset",
			limitParam:  "limit",
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "albums":
		return db.UpsertAlbums(data)
	case "albums_tracks":
		return db.UpsertAlbumsTracks(data)
	case "artists":
		return db.UpsertArtists(data)
	case "artists_albums":
		return db.UpsertArtistsAlbums(data)
	case "related_artists":
		return db.UpsertRelatedArtists(data)
	case "top_tracks":
		return db.UpsertTopTracks(data)
	case "audio-features":
		return db.UpsertAudioFeatures(data)
	case "audiobooks":
		return db.UpsertAudiobooks(data)
	case "audiobooks_chapters":
		return db.UpsertAudiobooksChapters(data)
	case "browse":
		return db.UpsertBrowse(data)
	case "chapters":
		return db.UpsertChapters(data)
	case "episodes":
		return db.UpsertEpisodes(data)
	case "me":
		return db.UpsertMe(data)
	case "playlists":
		return db.UpsertPlaylists(data)
	case "followers":
	...
	case "playlists_tracks":
		return db.UpsertPlaylistsTracks(data)
	case "shows":
		return db.UpsertShows(data)
	case "shows_episodes":
		return db.UpsertShowsEpisodes(data)
	case "tracks":
		return db.UpsertTracks(data)
	case "users_playlists":
		return db.UpsertUsersPlaylists(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"chapters":                  "/chapters",
		"episodes":                  "/episodes",
		"items":                     "/playlists/{playlist_id}/items",
		"me":                        "/me",
		"me-albums":                 "/me/albums",
		"me-audiobooks":             "/me/audiobooks",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "albums_tracks", ParentTable: "albums", ParentIDParam: "id", PathTemplate: "/albums/{id}/tracks", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.album", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "artists_albums", ParentTable: "artists", ParentIDParam: "id", PathTemplate: "/artists/{id}/albums", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.artist", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "audiobooks_chapters", ParentTable: "audiobooks", ParentIDParam: "id", PathTemplate: "/audiobooks/{id}/chapters", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.audiobook", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
		{Name: "shows_episodes", ParentTable: "shows", ParentIDParam: "id", PathTemplate: "/shows/{id}/episodes", KeyField: "", ReconcileMode: "per_parent", GenericScopeJSONPath: "$.show", PathParams: []dependentPathParamDef{
			{Param: "id", Field: "id"},
		}},
	}
}
...
func responsePathForResource(resource, path string) []string {
	...
	case "albums\x00/albums":
		return []string{"albums"}
	case "artists\x00/artists":
		return []string{"artists"}
	case "audio-features\x00/audio-features":
		return []string{"audio_features"}
	case "audiobooks\x00/audiobooks":
		return []string{"audiobooks"}
	case "browse\x00/browse/categories/{category_id}":
		return []string{"icons"}
	case "chapters\x00/chapters":
		return []string{"chapters"}
	case "episodes\x00/episodes":
		return []string{"episodes"}
	case "markets\x00/markets":
		return []string{"markets"}
	case "me\x00/me/playlists":
		return []string{"images"}
	case "me\x00/me/player/devices":
		return []string{"devices"}
	case "me\x00/me":
		return []string{"images"}
	case "me\x00/me/player/queue":
		return []string{"queue"}
	case "playlists\x00/playlists/{playlist_id}":
		return []string{"images"}
	case "recommendations\x00/recommendations/available-genre-seeds":
		return []string{"genres"}
	case "shows\x00/shows":
		return []string{"shows"}
	case "tracks\x00/tracks":
		return []string{"tracks"}
	case "users\x00/users/{user_id}":
		return []string{"images"}
	}
	return nil
}
