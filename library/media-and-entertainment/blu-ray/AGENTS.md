# Blu-ray.com Printed CLI Agent Guide

This directory is a generated `blu-ray-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
blu-ray-pp-cli doctor --json
blu-ray-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
blu-ray-pp-cli which "<capability>" --json
...
```bash
blu-ray-pp-cli <command> --help
blu-ray-pp-cli <command> --dry-run --agent
```
