...
func newSitemapNewsCmd(flags *rootFlags) *cobra.Command {

	cmd := &cobra.Command{
		Use:         "news",
		Short:       "Compressed news sitemap — each entry has title + publication_date inline (no per-story fetch needed for enumeration).",
		Example:     "  blu-ray-pp-cli sitemap news",
		Annotations: map[string]string{"pp:endpoint": "sitemap.news", "pp:method": "GET", "pp:path": "/sitemap/sitemap_news.xml.gz", "mcp:read-only": "true"},
