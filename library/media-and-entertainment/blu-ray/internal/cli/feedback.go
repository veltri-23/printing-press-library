...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "blu-ray-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BLU_RAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BLU_RAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "blu-ray-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/blu-ray-pp-cli/feedback.jsonl.
When ` + "`BLU_RAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BLU_RAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  blu-ray-pp-cli feedback list
  blu-ray-pp-cli feedback list --limit 5
  blu-ray-pp-cli feedback list --json`,
