...
func newCalendarReleasesCmd(flags *rootFlags) *cobra.Command {
	var flagYear int
	...
	cmd := &cobra.Command{
		Use:         "releases",
		Short:       "Release calendar page for a given year, optionally filtered by country and format.",
		Example:     "  blu-ray-pp-cli calendar releases",
		Annotations: map[string]string{"pp:endpoint": "calendar.releases", "pp:method": "GET", "pp:path": "/movies/releasedates.php", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if cmd.Flags().Changed("format") {
				allowedFormat := []string{"4K", "bluray", "3D", "DVD"}
				validFormat := false
	...
	cmd.Flags().IntVar(&flagYear, "year", 0, "Calendar year (2006-onwards). Defaults to current year when omitted.")
	cmd.Flags().StringVar(&flagCountry, "country", "", "Country filter (e.g. USA).")
	cmd.Flags().StringVar(&flagFormat, "format", "", "Format filter. (one of: 4K, bluray, 3D, DVD)")
