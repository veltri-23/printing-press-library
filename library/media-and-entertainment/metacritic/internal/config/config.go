...
type Config struct {
	...
	MetacriticApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://backend.metacritic.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("METACRITIC_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "metacritic-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("METACRITIC_API_KEY"); v != "" {
		cfg.MetacriticApiKey = v
		cfg.markEnvOverride("MetacriticApiKey")
		cfg.AuthSource = "env:METACRITIC_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.MetacriticApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("METACRITIC_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.MetacriticApiKey = token
	delete(c.envOverrides, "MetacriticApiKey")
	c.updateFileConfigField("MetacriticApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["MetacriticApiKey"] {
			out.MetacriticApiKey = c.fileConfig.MetacriticApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "MetacriticApiKey":
		c.fileConfig.MetacriticApiKey = c.MetacriticApiKey
	}
}
