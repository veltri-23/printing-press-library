...
In local mode: searches locally synced data only.`,
  ...
  metacritic-pp-cli search "error timeout"

  # Force local search only
  metacritic-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  metacritic-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  metacritic-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /finder/metacritic/search/{query}/web
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/finder/metacritic/search/{query}/web", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'metacritic-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/metacritic-pp-cli/data.db)")
