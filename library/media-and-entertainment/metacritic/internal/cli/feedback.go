...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "metacritic-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("METACRITIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("METACRITIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/media-and-entertainment/metacritic/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/metacritic-pp-cli/feedback.jsonl.
When ` + "`METACRITIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`METACRITIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  metacritic-pp-cli feedback list
  metacritic-pp-cli feedback list --limit 5
  metacritic-pp-cli feedback list --json`,
