# Metacritic Printed CLI Agent Guide

This directory is a generated `metacritic-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
metacritic-pp-cli doctor --json
metacritic-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
metacritic-pp-cli which "<capability>" --json
metacritic-pp-cli <command> --help
```
...
```bash
metacritic-pp-cli <command> --agent
```
...
```bash
metacritic-pp-cli <command> --help
metacritic-pp-cli <command> --dry-run --agent
```
