...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pokeapi-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POKEAPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POKEAPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pokeapi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pokeapi-pp-cli/feedback.jsonl.
When ` + "`POKEAPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POKEAPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
