...
type rootFlags struct {
	...
	// invocation. Mirrors the SOCCER_GOAT_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "soccer-goat-pp-cli",
		Short: `Soccer Goat CLI — Type any player or club and get market value, EA FC rating, potential, and stats in one report, joined across sources t…`,
		Long: `Soccer Goat CLI — Type any player or club and get market value, EA FC rating, potential, and stats in one report, joined across sources t…
...
Health check: run 'soccer-goat-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("soccer-goat-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'soccer-goat-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelOverUnderRatedCmd(flags))
	rootCmd.AddCommand(newNovelPlayerCmd(flags))
	rootCmd.AddCommand(newNovelPotentialGapCmd(flags))
	rootCmd.AddCommand(newNovelTeamCmd(flags))
	rootCmd.AddCommand(newNovelWonderkidsCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newClubsPromotedCmd(flags))
	rootCmd.AddCommand(newCompetitionsPromotedCmd(flags))
	rootCmd.AddCommand(newPlayersPromotedCmd(flags))
