...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SOCCER_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SOCCER_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "soccer-goat-pp-cli/feedback")
...
When ` + "`SOCCER_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SOCCER_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  soccer-goat-pp-cli feedback list
  soccer-goat-pp-cli feedback list --limit 5
  soccer-goat-pp-cli feedback list --json`,
