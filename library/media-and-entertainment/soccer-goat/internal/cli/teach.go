...
// soccer-goat-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/soccer-goat/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/soccer-goat/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/soccer-goat/internal/learn/patterns"
...
const noLearnEnvVar = "SOCCER_GOAT_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("soccer-goat-pp-cli")
}
...
func newTeachCmd(flags *rootFlags, learnCfg *entities.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true", "pp:happy-args": "--query=value of mbappe;--resource=players/342229;--resource-type=players"},
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  soccer-goat-pp-cli teach --query "value of mbappe" --resource-type players --resource players/342229`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// An explicit --json is a request for machine output, so it
			// overrides the silent-by-default (--quiet) contract used by
			// background fire-and-forget invocations (which never pass --json).
			if flags.asJSON {
...
when learnings exist.`,
		Example: `  soccer-goat-pp-cli recall "<question>" --agent
  soccer-goat-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded learnings, filterable by query, source, resource, action, min-confidence, or warnings-only",
		Example: `  soccer-goat-pp-cli learnings list --agent
  soccer-goat-pp-cli learnings list --query "<substring>"
  soccer-goat-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  soccer-goat-pp-cli learnings forget "<question>" --resource <id>
  soccer-goat-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  soccer-goat-pp-cli teach-pattern \
    --query-template "items in {entity}" \
    --resource-template "GROUP-{entity:category}" \
    --resource-type "items" \
    --entity-kind "category" \
    --strategy substitute`,
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true", "pp:happy-args": "--query-template=items in {entity};--resource-template=GROUP-{entity:category};--resource-type=items;--entity-kind=category;--strategy=substitute"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  soccer-goat-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
