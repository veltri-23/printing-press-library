...
In local mode: searches locally synced data only.`,
  ...
  soccer-goat-pp-cli search "error timeout"

  # Force local search only
  soccer-goat-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  soccer-goat-pp-cli search "status" --type clubs --data-source local
  # JSON output for piping
  soccer-goat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /clubs/search/{club_name}
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/clubs/search/{club_name}", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'soccer-goat-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
