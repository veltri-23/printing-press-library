# Podscan Printed CLI Agent Guide

This directory is a generated `podscan-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
podscan-pp-cli doctor --json
podscan-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
podscan-pp-cli which "<capability>" --json
...
```bash
podscan-pp-cli <command> --help
podscan-pp-cli <command> --dry-run --agent
```
