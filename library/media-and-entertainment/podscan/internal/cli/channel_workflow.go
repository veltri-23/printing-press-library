...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  podscan-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  podscan-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"alerts", "categories", "exports", "exports-podcasts"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  podscan-pp-cli workflow status

  # Show status as JSON
  podscan-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("podscan-pp-cli")
			}
