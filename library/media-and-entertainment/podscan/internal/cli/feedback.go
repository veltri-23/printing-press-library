...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".podscan-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PODSCAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PODSCAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "podscan-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.podscan-pp-cli/feedback.jsonl.
When ` + "`PODSCAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PODSCAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  podscan-pp-cli feedback list
  podscan-pp-cli feedback list --limit 5
  podscan-pp-cli feedback list --json`,
