# Boardgamegeek Printed CLI Agent Guide

This directory is a generated `boardgamegeek-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
boardgamegeek-pp-cli doctor --json
boardgamegeek-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
boardgamegeek-pp-cli which "<capability>" --json
boardgamegeek-pp-cli <command> --help
```
...
```bash
boardgamegeek-pp-cli <command> --agent
```
...
```bash
boardgamegeek-pp-cli <command> --help
boardgamegeek-pp-cli <command> --dry-run --agent
```
