...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "boardgamegeek-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BOARDGAMEGEEK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BOARDGAMEGEEK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "boardgamegeek-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/boardgamegeek-pp-cli/feedback.jsonl.
When ` + "`BOARDGAMEGEEK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BOARDGAMEGEEK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  boardgamegeek-pp-cli feedback list
  boardgamegeek-pp-cli feedback list --limit 5
  boardgamegeek-pp-cli feedback list --json`,
