...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  boardgamegeek-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  boardgamegeek-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  boardgamegeek-pp-cli workflow status

  # Show status as JSON
  boardgamegeek-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("boardgamegeek-pp-cli")
			}
