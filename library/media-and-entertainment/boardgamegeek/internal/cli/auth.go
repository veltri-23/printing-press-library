...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Boardgamegeek Xml Api2",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  boardgamegeek-pp-cli auth setup\n  boardgamegeek-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://boardgamegeek.com/applications")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export BGG_TOKEN=\"your-token-here\"")
			fmt.Fprintln(w, "  export BOARDGAMEGEEK_TOKEN=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://boardgamegeek.com/applications"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  boardgamegeek-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  boardgamegeek-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  boardgamegeek-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("BGG_TOKEN") != "" {
				envStillSet = "BGG_TOKEN"
			}
			if envStillSet == "" && os.Getenv("BOARDGAMEGEEK_TOKEN") != "" {
				envStillSet = "BOARDGAMEGEEK_TOKEN"
			}
