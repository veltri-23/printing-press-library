...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  boardgamegeek-pp-cli doctor
  boardgamegeek-pp-cli doctor --json
  boardgamegeek-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export BGG_TOKEN=\"your-token-here\" BOARDGAMEGEEK_TOKEN=\"your-token-here\""
					report["auth_key_url"] = "https://boardgamegeek.com/applications"
				} else {
					authConfigured = true
			...
			if os.Getenv("BGG_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BGG_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BGG_TOKEN")
			}
			if os.Getenv("BOARDGAMEGEEK_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BOARDGAMEGEEK_TOKEN")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "BOARDGAMEGEEK_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "boardgamegeek-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("boardgamegeek-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'boardgamegeek-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'boardgamegeek-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'boardgamegeek-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'boardgamegeek-pp-cli sync' to refresh."
	}
	return report
}
