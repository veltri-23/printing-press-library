...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "boardgamegeek-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "boardgamegeek-pp-cli/2.0.0")
		}
		...
		if resp.StatusCode < 400 {
			...
			// XML-only endpoints (response_format: xml) are normalized to a
			// generic JSON document so --json, --select, and table output work
			// unchanged. Non-XML responses pass through untouched; a decode
			// failure returns the raw bytes rather than corrupting the body.
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export BGG_TOKEN=<your-token> or boardgamegeek-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.BggToken)
		addCredential(c.Config.BoardgamegeekToken)
	}
