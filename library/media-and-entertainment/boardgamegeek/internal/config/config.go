...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("BOARDGAMEGEEK_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "boardgamegeek-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("BGG_TOKEN"); v != "" {
		cfg.BggToken = v
		cfg.markEnvOverride("BggToken")
		cfg.AuthSource = "env:BGG_TOKEN"
	}
	if v := os.Getenv("BOARDGAMEGEEK_TOKEN"); v != "" {
		cfg.BoardgamegeekToken = v
		cfg.markEnvOverride("BoardgamegeekToken")
		cfg.AuthSource = "env:BOARDGAMEGEEK_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.BggToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.BoardgamegeekToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("BOARDGAMEGEEK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.BggToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BGG_TOKEN"
		}
		return "Bearer " + c.BggToken
	}
	if c.BoardgamegeekToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:BOARDGAMEGEEK_TOKEN"
		}
		return "Bearer " + c.BoardgamegeekToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.BggToken = ""
	delete(c.envOverrides, "BggToken")
	c.BoardgamegeekToken = ""
	delete(c.envOverrides, "BoardgamegeekToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["BggToken"] {
			out.BggToken = c.fileConfig.BggToken
		}
		if c.envOverrides["BoardgamegeekToken"] {
			out.BoardgamegeekToken = c.fileConfig.BoardgamegeekToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "BggToken":
		c.fileConfig.BggToken = c.BggToken
	case "BoardgamegeekToken":
		c.fileConfig.BoardgamegeekToken = c.BoardgamegeekToken
	}
}
