# Medium Printed CLI Agent Guide

This directory is a generated `medium-reader-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
medium-reader-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
medium-reader-pp-cli which "<capability>" --json
medium-reader-pp-cli <command> --help
```
...
```bash
medium-reader-pp-cli <command> --agent
```
...
```bash
medium-reader-pp-cli <command> --help
medium-reader-pp-cli <command> --dry-run --agent
```
