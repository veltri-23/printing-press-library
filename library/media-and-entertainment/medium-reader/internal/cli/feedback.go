...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "medium-reader-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MEDIUM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MEDIUM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "medium-reader-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/medium-reader-pp-cli/feedback.jsonl.
When ` + "`MEDIUM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MEDIUM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  medium-reader-pp-cli feedback list
  medium-reader-pp-cli feedback list --limit 5
  medium-reader-pp-cli feedback list --json`,
