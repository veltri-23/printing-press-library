...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "medium-reader-pp-cli",
		Short: `Medium CLI — Read any Medium author, publication, or tag as a local, full-text-searchable corpus.`,
		Long: `Medium CLI — Read any Medium author, publication, or tag as a local, full-text-searchable corpus.

Highlights (not in the official API docs):
  • author-archive   Mirror a writer's entire body of work into local SQLite, full-text searchable offline.
...
Health check: run 'medium-reader-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("medium-reader-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'medium-reader-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.cookieFile, "cookie-file", "", "Path to a flat-JSON Medium session cookie file {\"sid\":\"..\",\"uid\":\"..\"} (Tier 1; optional, unlocks member full bodies). Also reads MEDIUM_SESSION / MEDIUM_COOKIE_FILE.")
	...
	rootCmd.AddCommand(newFeedCmd(flags))   // v2: RSS feed surface (Tier 0, keyless)
	rootCmd.AddCommand(newReadCmd(flags))   // v2: page-parse read surface (Tier 0, keyless)
	rootCmd.AddCommand(newSearchCmd(flags)) // v2: GraphQL search surface (Tier 0, keyless)
	...
	rootCmd.AddCommand(newNovelAuthorArchiveCmd(flags))
	rootCmd.AddCommand(newNovelAuthorCompareCmd(flags))
	rootCmd.AddCommand(newNovelCorpusCmd(flags))
