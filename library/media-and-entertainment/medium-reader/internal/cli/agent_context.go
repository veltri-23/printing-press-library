...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	// v2 is $0/no-key: no auth is required (Tier 0, anonymous). The only
	// credential is the user's OWN, optional Medium session cookie, which
	// unlocks member full bodies on the read path (Tier 1).
	envVars := []agentContextAuthEnvVar{
		{
			...
			Required:    false,
			Sensitive:   true,
			Description: "Optional. Your own Medium session cookie (\"sid=..; uid=..\") to unlock member full bodies (Tier 1). Every command works without it (Tier 0, anonymous). Also accepts --cookie-file / MEDIUM_COOKIE_FILE.",
		},
	}
	authMode := "none"
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "medium-reader-pp-cli",
			Description: "Read any Medium author, publication, or tag as a local",
