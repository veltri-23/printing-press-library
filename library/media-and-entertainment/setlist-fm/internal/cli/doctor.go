...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  setlist-fm-pp-cli doctor
  setlist-fm-pp-cli doctor --json
  setlist-fm-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Accept either SETLISTFM_API_KEY (no underscore — setlistfm-js convention)
			// or SETLIST_FM_API_KEY (underscore — setlist-fm-client convention).
			switch {
			case os.Getenv("SETLISTFM_API_KEY") != "":
				authEnvSet = append(authEnvSet, "SETLISTFM_API_KEY")
			case os.Getenv("SETLIST_FM_API_KEY") != "":
				authEnvSet = append(authEnvSet, "SETLIST_FM_API_KEY")
			default:
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "setlist-fm-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'setlist-fm-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'setlist-fm-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'setlist-fm-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'setlist-fm-pp-cli sync' to refresh."
	}
	return report
}
