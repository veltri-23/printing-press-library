...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".setlist-fm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SETLIST_FM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SETLIST_FM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "setlist-fm-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.setlist-fm-pp-cli/feedback.jsonl.
When ` + "`SETLIST_FM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SETLIST_FM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  setlist-fm-pp-cli feedback list
  setlist-fm-pp-cli feedback list --limit 5
  setlist-fm-pp-cli feedback list --json`,
