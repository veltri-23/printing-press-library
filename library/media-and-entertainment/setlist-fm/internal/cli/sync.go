...
Exit codes & warnings:
  ...
  setlist-fm-pp-cli sync

  # Sync specific resources only
  setlist-fm-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  setlist-fm-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  setlist-fm-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  setlist-fm-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  setlist-fm-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/setlist-fm-pp-cli/data.db)")
