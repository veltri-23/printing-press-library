# Setlist.fm Printed CLI Agent Guide

This directory is a generated `setlist-fm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
setlist-fm-pp-cli doctor --json
setlist-fm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
setlist-fm-pp-cli which "<capability>" --json
setlist-fm-pp-cli <command> --help
```
...
```bash
setlist-fm-pp-cli <command> --agent
```
...
```bash
setlist-fm-pp-cli <command> --help
setlist-fm-pp-cli <command> --dry-run --agent
```
