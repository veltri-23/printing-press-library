...
This directory is a generated `substack-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
substack-pp-cli doctor --json
substack-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
substack-pp-cli which "<capability>" --json
...
```bash
substack-pp-cli <command> --help
substack-pp-cli <command> --dry-run --agent
```
