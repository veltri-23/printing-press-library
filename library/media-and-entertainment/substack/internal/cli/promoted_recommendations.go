...
func newRecommendationsPromotedCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:        "List the publications a publication recommends",
			...
			if len(args) < 1 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s <%s>", cmd.CommandPath(), "publication_id"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("publication_id is required\nUsage: %s <%s>", cmd.CommandPath(), "publication_id"))
			}
			path = replacePathParam(path, "publication_id", args[0])
			params := map[string]string{}
			data, prov, err := resolveReadWithStrategy(cmd.Context(), c, flags, "auto", "recommendations", false, path, params, nil, cmd.ErrOrStderr())
			if err != nil {
