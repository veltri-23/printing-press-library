...
profile by name when the installed backend supports it.`,
		Example: `  substack-pp-cli auth login --chrome
  substack-pp-cli auth login --browser
  substack-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if !browserFlag {
				fmt.Fprintln(cmd.OutOrStdout(), "Use --chrome or --browser to authenticate from your browser session:")
				fmt.Fprintf(cmd.OutOrStdout(), "  substack-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  substack-pp-cli auth login --browser\n")
				return nil
			}
			...
			if !fromPressAuth {
				...
				// Custom-domain publications keep the Creator-session cookie
				// (connect.sid) on the custom domain, not .substack.com. Capture
				// it into the jar so Creator endpoints work after the 301 hop.
				captureCreatorDomainCookie(w, tool, profileDir)
			} // end if !fromPressAuth
			if fromPressAuth {
				// The press-auth path resolved no Chrome tool above; capture the
				// custom-domain Creator cookie independently (best-effort — the
				// helper self-detects a cookie tool and no-ops if none exists).
				captureCreatorDomainCookie(w, cookieTool{}, "")
			}
			...
			// substack.sid is the real session credential. connect.sid is carried
			// when present but is not required: live Substack sessions do not always
			// set it, and demanding it blocks otherwise-valid logged-in users.
			requiredCookies := []string{"substack.sid"}
			knownCookies := []string{"substack.sid", "connect.sid"}
			...
			kept := make([]string, 0, len(knownCookies))
			for _, name := range knownCookies {
			...
			// client's cookie jar.
			jarCookies := make(map[string]string, len(knownCookies))
			for _, name := range knownCookies {
				if v, ok := cookieMap[name]; ok {
					jarCookies[name] = v
				}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  substack-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  substack-pp-cli auth login --chrome\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  substack-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  substack-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
