...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  substack-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  substack-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "drafts", "inbox", "inbox-posts", "posts", "posts-published", "posts-ranked", "profiles", "sections", "subs", "tags"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  substack-pp-cli workflow status

  # Show status as JSON
  substack-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("substack-pp-cli")
			}
