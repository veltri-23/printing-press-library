...
func cookieJarPath() string {
	...
	return filepath.Join(homeDir, ".local", "share", "substack-pp-cli", "cookies.json")
}
...
func mergeAndWriteCookieRows(path string, rows []persistedCookie) error {
	...
	// Normalize the leading dot out of the domain when keying: a host-only row
	// ("trevinsays.com", written by the login flow's WriteCookieJarFromMap) and
	// a domain row (".trevinsays.com", written by a server Set-Cookie hop) are
	// the same cookie. Without this they key distinctly and both survive, so the
	// jar later sends two connect.sid values and Creator endpoints 403.
	cookieKey := func(r persistedCookie) string {
		return strings.TrimPrefix(r.Domain, ".") + "|" + r.Path + "|" + r.Name
	}
