...
// espn-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
func learningsStateDir() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "espn-pp-cli")
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("espn-pp-cli")
}
...
~/.local/share/espn-pp-cli/teach.log, safe to fire-and-forget.

Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  espn-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  espn-pp-cli recall "<question>" --agent
  espn-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  espn-pp-cli learnings list --agent
  espn-pp-cli learnings list --query "<substring>"
  espn-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  espn-pp-cli learnings forget "<question>" --resource <id>
  espn-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  espn-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  espn-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
