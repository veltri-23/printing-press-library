...
// PATCH: feat-comments-and-handle-resolution — opt-in context propagation via Client.WithContext + http.NewRequestWithContext, so callers that thread cmd.Context() get --timeout / Ctrl+C honored on outbound HTTP. Replaced bare time.Sleep in 429 and 5xx retry paths with sleepCtx so --timeout / SIGINT wake the process during retry waits instead of blocking up to MaxRetryWait = 60s. Backward compatible: ctx defaults to context.Background() when unset. Generator-side fix to push cmd.Context() through resolvePaginatedRead is upstream/separate.
...
// WithContext returns a shallow copy of the client whose outbound HTTP
// requests will carry ctx. Use it once at the top of a command's RunE so
// --timeout, Ctrl+C, and cmd.Context() cancellations propagate down to
// in-flight Data API calls.
...
// sleepCtx waits for d, returning ctx.Err() if the context is cancelled
// first. Replaces bare time.Sleep in retry paths so --timeout and SIGINT
// wake the process within milliseconds instead of waiting out the full
// retry-after window (up to MaxRetryWait = 60s).
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "youtube-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req, err := http.NewRequestWithContext(c.requestContext(), method, targetURL, bodyReader)
		if err != nil {
		...
		if resp.StatusCode == 429 && attempt < maxRetries {
			...
			if err := sleepCtx(c.requestContext(), wait); err != nil {
				return nil, 0, err
			}
			lastErr = apiErr
			continue
