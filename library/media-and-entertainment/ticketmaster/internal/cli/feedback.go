...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ticketmaster-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TICKETMASTER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TICKETMASTER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ticketmaster-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ticketmaster-pp-cli/feedback.jsonl.
When ` + "`TICKETMASTER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TICKETMASTER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ticketmaster-pp-cli feedback list
  ticketmaster-pp-cli feedback list --limit 5
  ticketmaster-pp-cli feedback list --json`,
