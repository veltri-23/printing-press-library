...
// ticketmaster-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/ticketmaster/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/ticketmaster/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/ticketmaster/internal/learn/patterns"
...
const noLearnEnvVar = "TICKETMASTER_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("ticketmaster-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  ticketmaster-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  ticketmaster-pp-cli recall "<question>" --agent
  ticketmaster-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ticketmaster-pp-cli learnings list --agent
  ticketmaster-pp-cli learnings list --query "<substring>"
  ticketmaster-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  ticketmaster-pp-cli learnings forget "<question>" --resource <id>
  ticketmaster-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  ticketmaster-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  ticketmaster-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
