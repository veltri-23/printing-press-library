...
Resource scoping:
  ...
  ticketmaster-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ticketmaster-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ticketmaster-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ticketmaster-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ticketmaster-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "attractions":
		return db.UpsertAttractions(data)
	case "classifications":
		return db.UpsertClassifications(data)
	case "events":
