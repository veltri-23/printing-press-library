...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ticketmaster-pp-cli doctor
  ticketmaster-pp-cli doctor --json
  ticketmaster-pp-cli doctor --fail-on warn
  ticketmaster-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export TICKETMASTER_API_KEY=\"your-token-here\""
					report["auth_key_url"] = "https://developer-acct.ticketmaster.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("TICKETMASTER_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "TICKETMASTER_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TICKETMASTER_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "ticketmaster-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'ticketmaster-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'ticketmaster-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'ticketmaster-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'ticketmaster-pp-cli sync' to refresh."
	}
	return report
}
