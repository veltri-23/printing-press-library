...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ticketmaster-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ticketmaster-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"attractions", "classifications", "events", "suggest", "venues"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ticketmaster-pp-cli workflow status

  # Show status as JSON
  ticketmaster-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ticketmaster-pp-cli")
			}
