...
func resetCredentialEnv(t *testing.T) (home, configPath string) {
	...
	for _, name := range []string{
		"TICKETMASTER_CONFIG",
		"TICKETMASTER_CONFIG_DIR",
		"TICKETMASTER_DATA_DIR",
		"TICKETMASTER_STATE_DIR",
		"TICKETMASTER_CACHE_DIR",
		"TICKETMASTER_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
	...
	return home, filepath.Join(home, ".config", "ticketmaster-pp-cli", "config.toml")
}
...
func TestCorruptCredentialsFallsBackToLegacyConfig(t *testing.T) {
	...
	credentialsPath := filepath.Join(home, ".local", "share", "ticketmaster-pp-cli", "credentials.toml")
...
func TestCorruptCredentialsFallsBackToEnvCredential(t *testing.T) {
	...
	t.Setenv("TICKETMASTER_API_KEY", "env-secret")
	credentialsPath := filepath.Join(home, ".local", "share", "ticketmaster-pp-cli", "credentials.toml")
	if err := os.MkdirAll(filepath.Dir(credentialsPath), 0o700); err != nil {
		t.Fatalf("mkdir credentials dir: %v", err)
	}
...
func TestAuthWriteScrubsLegacyConfigWhenRelocated(t *testing.T) {
	...
	activeConfigPath := filepath.Join(newConfigDir, "ticketmaster-pp-cli", "config.toml")
	...
	credsPath := filepath.Join(newDataDir, "ticketmaster-pp-cli", "credentials.toml")
