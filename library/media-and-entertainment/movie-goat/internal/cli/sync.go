...
Exit codes & warnings:
  ...
  movie-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  movie-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  movie-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  movie-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  movie-goat-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Under --json, emit a final summary to stdout so callers that pipe
			// stdout to jq see a single valid JSON object. Stderr stays as the
			// NDJSON event stream for streaming consumers.
			if flags.asJSON {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "movies":
		return db.UpsertMovies(data)
	case "tv":
		return db.UpsertTv(data)
	case "seasons":
		return db.UpsertSeasons(data)
	case "people":
		return db.UpsertPeople(data)
	case "discover":
		return db.UpsertDiscover(data)
	case "trending":
		return db.UpsertTrending(data)
	default:
