...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".archive-is-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARCHIVE_IS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARCHIVE_IS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "archive-is-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.archive-is-pp-cli/feedback.jsonl.
When ` + "`ARCHIVE_IS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARCHIVE_IS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
