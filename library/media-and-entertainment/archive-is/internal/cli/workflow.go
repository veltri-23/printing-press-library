...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: `  # Archive all resources to local store
  archive-is-pp-cli workflow archive
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  archive-is-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("archive-is-pp-cli")
			}
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  # Show status as JSON
  archive-is-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("archive-is-pp-cli")
			}
