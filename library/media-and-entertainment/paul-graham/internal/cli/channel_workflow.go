...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  paul-graham-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  paul-graham-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"articles-html"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  paul-graham-pp-cli workflow status

  # Show status as JSON
  paul-graham-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("paul-graham-pp-cli")
			}
