...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "paul-graham-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAUL_GRAHAM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAUL_GRAHAM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "paul-graham-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/paul-graham-pp-cli/feedback.jsonl.
When ` + "`PAUL_GRAHAM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAUL_GRAHAM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  paul-graham-pp-cli feedback list
  paul-graham-pp-cli feedback list --limit 5
  paul-graham-pp-cli feedback list --json`,
