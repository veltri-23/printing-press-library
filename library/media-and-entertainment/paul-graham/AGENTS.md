# Paul Graham Printed CLI Agent Guide

This directory is a generated `paul-graham-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
paul-graham-pp-cli doctor --json
paul-graham-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
paul-graham-pp-cli which "<capability>" --json
paul-graham-pp-cli <command> --help
```
...
```bash
paul-graham-pp-cli <command> --agent
```
...
```bash
paul-graham-pp-cli <command> --help
paul-graham-pp-cli <command> --dry-run --agent
```
