...
Run sync first to populate the local search index.`,
  ...
  lightroom-classic-pp-cli search "status"

  # Force local search explicitly
  lightroom-classic-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  lightroom-classic-pp-cli search "status" --type catalog --data-source local
  # JSON output for piping
  lightroom-classic-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'lightroom-classic-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
