...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LIGHTROOM_CLASSIC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LIGHTROOM_CLASSIC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lightroom-classic-pp-cli/feedback")
...
When ` + "`LIGHTROOM_CLASSIC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LIGHTROOM_CLASSIC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lightroom-classic-pp-cli feedback list
  lightroom-classic-pp-cli feedback list --limit 5
  lightroom-classic-pp-cli feedback list --json`,
