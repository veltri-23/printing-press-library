...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lightroom-classic-pp-cli doctor
  lightroom-classic-pp-cli doctor --json
  lightroom-classic-pp-cli doctor --fail-on warn
  lightroom-classic-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "lightroom-classic-pp-cli", suggestion)
						} else {
				...
				// Local-sqlite CLI: health means the catalog, not an API.
				collectCatalogReport(cmd.Context(), flags, report)
			}
			...
			if chAny, ok := report["catalog_health"]; ok {
				if chRep, ok := chAny.(map[string]any); ok {
					renderCatalogHealth(w, chRep)
				}
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("lightroom-classic-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'lightroom-classic-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'lightroom-classic-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'lightroom-classic-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'lightroom-classic-pp-cli sync' to refresh."
	}
	return report
}
