...
type rootFlags struct {
	...
	// invocation. Mirrors the LIGHTROOM_CLASSIC_NO_LEARN env var.
	noLearn      bool
	selectFields string
	configPath   string
	// catalog is the .lrcat path override; empty means env var then discovery.
	catalog       string
	homePath      string
	profileName   string
	deliverSpec   string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "lightroom-classic-pp-cli",
		Short: `Lightroom Classic CLI — Your Lightroom Classic catalog, queryable from the terminal — streaks, daily picks, and shot lists with zero exports…`,
		Long: `Lightroom Classic CLI — Your Lightroom Classic catalog, queryable from the terminal — streaks, daily picks, and shot lists with zero exports and zero writes.
...
Health check: run 'lightroom-classic-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("lightroom-classic-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.catalog, "catalog", "", "Path to the .lrcat catalog (default: $LIGHTROOM_CLASSIC_CATALOG, then newest under ~/Pictures)")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'lightroom-classic-pp-cli profile list')")
	...
	// Local-sqlite CLI: the catalog IS the data store. sync and tail assume an
	// HTTP API and are hidden — the commands still exist for framework probes.
	syncCmd := newSyncCmd(flags)
	syncCmd.Hidden = true
	syncCmd.RunE = func(cmd *cobra.Command, args []string) error {
		// The .lrcat catalog is already local; there is no API to sync from.
		if flags.asJSON || flags.agent {
			fmt.Fprintln(cmd.OutOrStdout(), `{"status":"noop","reason":"the .lrcat catalog is the local data store; commands read it directly"}`)
			return nil
		}
		fmt.Fprintln(cmd.OutOrStdout(), "nothing to sync: the .lrcat catalog is already local; commands read it directly")
		return nil
	}
	rootCmd.AddCommand(syncCmd)
	tailCmd := newTailCmd(flags)
	tailCmd.Hidden = true
	rootCmd.AddCommand(tailCmd)
	rootCmd.AddCommand(newAnalyticsCmd(flags))
	rootCmd.AddCommand(newWorkflowCmd(flags))
	rootCmd.AddCommand(newNovelBacklogCmd(flags))
	rootCmd.AddCommand(newNovelFunnelCmd(flags))
	rootCmd.AddCommand(newNovelOnThisDayCmd(flags))
	rootCmd.AddCommand(newNovelPickOfDayCmd(flags))
	rootCmd.AddCommand(newNovelProjectCmd(flags))
	rootCmd.AddCommand(newNovelStatsCmd(flags))
	rootCmd.AddCommand(newNovelStreaksCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newPhotosPromotedCmd(flags))
	rootCmd.AddCommand(makeCollectionsCmd(flags))
	rootCmd.AddCommand(makeKeywordsCmd(flags))
	rootCmd.AddCommand(makeCamerasCmd(flags))
	rootCmd.AddCommand(makeLensesCmd(flags))
