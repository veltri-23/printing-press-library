...
Resource scoping:
  ...
  lightroom-classic-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  lightroom-classic-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  lightroom-classic-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  lightroom-classic-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  lightroom-classic-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "catalog":
		return db.UpsertCatalog(data)
	case "photos":
		return db.UpsertPhotos(data)
	default:
