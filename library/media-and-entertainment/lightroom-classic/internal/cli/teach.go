...
// lightroom-classic-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/lightroom-classic/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/lightroom-classic/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/media-and-entertainment/lightroom-classic/internal/learn/patterns"
...
const noLearnEnvVar = "LIGHTROOM_CLASSIC_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("lightroom-classic-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  lightroom-classic-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  lightroom-classic-pp-cli recall "<question>" --agent
  lightroom-classic-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lightroom-classic-pp-cli learnings list --agent
  lightroom-classic-pp-cli learnings list --query "<substring>"
  lightroom-classic-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  lightroom-classic-pp-cli learnings forget "<question>" --resource <id>
  lightroom-classic-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  lightroom-classic-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  lightroom-classic-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
