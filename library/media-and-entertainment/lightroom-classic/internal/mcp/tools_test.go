...
func TestMCPPathResolutionMatchesCLIResolverWithHomeEnv(t *testing.T) {
	...
	t.Setenv("LIGHTROOM_CLASSIC_HOME", root)
...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "lightroom-classic-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "lightroom-classic-pp-cli", "data.db"); gotDB != want {
...
func resetMCPPathEnv(t *testing.T) string {
	...
	for _, name := range []string{
		"LIGHTROOM_CLASSIC_CONFIG",
		"LIGHTROOM_CLASSIC_CONFIG_DIR",
		"LIGHTROOM_CLASSIC_DATA_DIR",
		"LIGHTROOM_CLASSIC_STATE_DIR",
		"LIGHTROOM_CLASSIC_CACHE_DIR",
		"LIGHTROOM_CLASSIC_HOME",
		"XDG_CONFIG_HOME",
		"XDG_DATA_HOME",
		"XDG_STATE_HOME",
		"XDG_CACHE_HOME",
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("lightroom-classic", "test")
	RegisterTools(s)
...
func TestMCPLocalStoreMetaIncludesOldestSyncedAt(t *testing.T) {
	...
	if _, err := db.DB().Exec(`INSERT INTO sync_state(resource_type, last_synced_at, total_count) VALUES (?, ?, ?)`, "catalog", syncedAt, 0); err != nil {
