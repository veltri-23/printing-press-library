# Goodreads Printed CLI Agent Guide

This directory is a generated `goodreads-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
goodreads-pp-cli doctor --json
goodreads-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
goodreads-pp-cli which "<capability>" --json
goodreads-pp-cli <command> --help
```
...
```bash
goodreads-pp-cli <command> --agent
```
...
```bash
goodreads-pp-cli <command> --help
goodreads-pp-cli <command> --dry-run --agent
```
