...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".goodreads-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOODREADS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOODREADS_FEEDBACK_AUTO_SEND")))
...
func appendFeedback(entry FeedbackEntry) error {
	...
	defer f.Close()
	// O_CREATE does not change the mode of an already-existing file, so a
	// feedback ledger created by an older 0o644 build would stay
	// world-readable. Re-secure to owner-only on every append.
	_ = os.Chmod(p, 0o600)
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "goodreads-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.goodreads-pp-cli/feedback.jsonl.
When ` + "`GOODREADS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOODREADS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  goodreads-pp-cli feedback list
  goodreads-pp-cli feedback list --limit 5
  goodreads-pp-cli feedback list --json`,
