...
Resource scoping:
  ...
  goodreads-pp-cli sync

  # Sync specific resources only
  goodreads-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  goodreads-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  goodreads-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  goodreads-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  goodreads-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/goodreads-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "load_more":
		return db.UpsertLoadMore(data)
	case "note":
		return db.UpsertNote(data)
	case "share":
...
func normalizeSyncResourceAliases(resources []string) []string {
	...
	for _, resource := range resources {
		if cliutil.IsVerifyEnv() && resource == "repos" {
			normalized = append(normalized, "opensearch-xml")
			continue
		}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"amazon-purchases":         "/amazon_purchases",
		"friend":                   "/friend",
		"friend-requests":          "/friend/requests",
		"genres":                   "/genres",
		"message":                  "/message/inbox",
		"message-mark-all-as-read": "/message/mark_all_as_read",
		"notifications":            "/notifications",
		"opensearch-xml":           "/opensearch.xml",
		"quotes":                   "/quotes/list",
		"recommendations":          "/recommendations",
		"recommendations-to-me":    "/recommendations/to_me",
		...
		"tooltips":                 "/tooltips",
		"topic":                    "/topic",
		"user":                     "/user/top_readers",
		"user-best-reviewers":      "/user/best_reviewers",
		"user-following":           "/user_following/most_followed",
		"user-top-reviewers":       "/user/top_reviewers",
	}
