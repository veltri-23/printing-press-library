...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  goodreads-pp-cli doctor
  goodreads-pp-cli doctor --json
  goodreads-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export GOODREADS_GOODREADS_COOKIE_SESSION=<your-goodreads-session-cookie>"
					report["auth_docs_url"] = "https://www.goodreads.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("GOODREADS_GOODREADS_COOKIE_SESSION") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "GOODREADS_GOODREADS_COOKIE_SESSION")
			} else {
				authEnvInfo = append(authEnvInfo, "GOODREADS_GOODREADS_COOKIE_SESSION set with auth set-token")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "goodreads-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("goodreads-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'goodreads-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'goodreads-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'goodreads-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'goodreads-pp-cli sync' to refresh."
	}
	return report
}
