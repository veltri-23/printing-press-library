...
func (r *recordingRoundTripper) RoundTrip(req *http.Request) (*http.Response, error) {
	r.calls++
	r.lastReq = req.Clone(req.Context())
...
func TestClient_VerifyShortCircuit_NoEnv(t *testing.T) {
	...
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GOODREADS_PP_ALLOW_WRITES", "1")
	c, rec := newClientWithRecorder(t)
...
func TestClient_APIKeyCookieAuthUsesCookieHeader(t *testing.T) {
	c, rec := newClientWithRecorder(t)
	c.Config.GoodreadsGoodreadsCookieSession = "session-cookie-value"

	_, _, err := c.do(context.Background(), "GET", "/test", nil, nil, nil)
	if err != nil {
		t.Fatalf("do(GET) returned error: %v", err)
	}
	if rec.lastReq == nil {
		t.Fatal("expected recorded request")
	}
	if got := rec.lastReq.Header.Get("_session_id2"); got != "" {
		t.Fatalf("unexpected custom _session_id2 header = %q; want cookie auth", got)
	}
	cookie, err := rec.lastReq.Cookie("_session_id2")
	if err != nil {
		t.Fatalf("missing _session_id2 cookie: %v", err)
	}
	if cookie.Value != "session-cookie-value" {
		t.Fatalf("cookie value = %q, want %q", cookie.Value, "session-cookie-value")
	}
}

func TestClient_GoodreadsWriteRequiresApproval(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GOODREADS_PP_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)

	_, _, err := c.do(context.Background(), "POST", "/shelf/add_to_shelf", nil, nil, nil)
	if !errors.Is(err, ErrWriteRequiresApproval) {
		t.Fatalf("err = %v, want ErrWriteRequiresApproval", err)
	}
	if rec.calls != 0 {
		t.Fatalf("write without approval attempted %d HTTP calls; want 0", rec.calls)
	}
}

func TestClient_GoodreadsWriteAllowsDryRun(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GOODREADS_PP_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)
	...
	_, _, err := c.do(context.Background(), "POST", "/shelf/add_to_shelf", nil, nil, nil)
	if err != nil {
		t.Fatalf("dry-run write returned error: %v", err)
	}
	if rec.calls != 0 {
		t.Fatalf("dry-run write attempted %d HTTP calls; want 0", rec.calls)
	}
}

func TestClient_GoodreadsWriteAllowsExplicitEnv(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GOODREADS_PP_ALLOW_WRITES", "1")
	c, rec := newClientWithRecorder(t)
	...
	if rec.calls != 1 {
		t.Fatalf("approved write attempted %d HTTP calls; want 1", rec.calls)
	}
}

func TestClient_GoodreadsGetMutationRequiresApproval(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	t.Setenv("GOODREADS_PP_ALLOW_WRITES", "")
	c, rec := newClientWithRecorder(t)

	_, _, err := c.do(context.Background(), "GET", "/message/mark_all_as_read", nil, nil, nil)
	if !errors.Is(err, ErrWriteRequiresApproval) {
		t.Fatalf("err = %v, want ErrWriteRequiresApproval", err)
	}
	if rec.calls != 0 {
		t.Fatalf("GET mutation without approval attempted %d HTTP calls; want 0", rec.calls)
	}
}
