...
// TestUpsertBatch_PopulatesLoadMoreTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed load_more table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesLoadMoreTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("load_more", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "load_more").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "load_more")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count load_more: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("load_more count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandLoadMoreGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandLoadMoreGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("load_more", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("load_more count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "notes_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandNoteGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("note count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "notes_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandShareGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("share count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "notes_id")
	}
}
