...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FEDEX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FEDEX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fedex-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.fedex-pp-cli/feedback.jsonl.
When ` + "`FEDEX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FEDEX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
