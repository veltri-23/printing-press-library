...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 403") && strings.Contains(msg, "/track/"):
		...
		// API; you cannot select Ship/Rate/Address AND Track in the same
		// project. The token mints fine and other endpoints work, but Track
		// returns FORBIDDEN.ERROR until you create a Track-only project and
		// use its credentials. This is a portal-level constraint, not a code
		// or scope-grant issue. Documented at https://resources.myshipi.com/
		// carriers/fedex/why-fedex-tracking-needs-different-api-details/.
		return authErr(fmt.Errorf("%w\nhint: FedEx requires a separate developer-portal project for the Track API."+
			"\n      Your current credentials are valid (other endpoints work) but the project they belong to does not have"+
			"\n      Track enabled — Ship/Rate/Address and Track are mutually exclusive in the same project (Oct 2023 policy)."+
			"\n      Fix:"+
			"\n        1. developer.fedex.com -> My Projects -> Create New Project"+
			"\n        2. On 'Select API(s)', tick ONLY Track API"+
			"\n        3. Use that new project's Client ID + Secret for tracking calls"+
			"\n      Background: https://resources.myshipi.com/carriers/fedex/why-fedex-tracking-needs-different-api-details/", err))
...
// parseWeightArg parses a weight string that optionally carries a unit
// suffix (lb, lbs, kg). Returns the numeric value, the resolved unit
// (empty when the input had no suffix — caller's --units flag wins),
// and an error when the value is not a number.
//
// Accepts: "5", "5lb", "5 lb", "2.5kg", "2.5KG"
func parseWeightArg(raw, defaultUnits string) (float64, string, error) {
	s := strings.TrimSpace(raw)
	if s == "" {
		return 0, "", nil
	}
	// Find the boundary between numeric prefix and any trailing unit.
	end := 0
	for end < len(s) {
		c := s[end]
		if (c >= '0' && c <= '9') || c == '.' || c == '-' || c == '+' {
	...
	if end == 0 {
		return 0, "", fmt.Errorf("invalid weight %q: expected a number, optionally followed by lb/kg", raw)
	}
	num, err := strconv.ParseFloat(s[:end], 64)
	if err != nil {
		return 0, "", fmt.Errorf("invalid weight %q: %w", raw, err)
	}
	suffix := strings.ToLower(strings.TrimSpace(s[end:]))
	switch suffix {
	case "":
		return num, "", nil
	...
	case "kg", "kgs", "kilogram", "kilograms":
		return num, "KG", nil
	default:
		return 0, "", fmt.Errorf("invalid weight unit %q in %q: expected lb or kg", suffix, raw)
	}
}
