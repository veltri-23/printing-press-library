# Harris Teeter Printed CLI Agent Guide

This directory is a generated `harris-teeter-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
harris-teeter-pp-cli doctor --json
harris-teeter-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
harris-teeter-pp-cli which "<capability>" --json
harris-teeter-pp-cli <command> --help
```
...
```bash
harris-teeter-pp-cli <command> --agent
```
...
```bash
harris-teeter-pp-cli <command> --help
harris-teeter-pp-cli <command> --dry-run --agent
```
