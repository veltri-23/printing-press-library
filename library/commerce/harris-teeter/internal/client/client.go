...
type lafObject struct {
	Modality       lafModality `json:"modality"`
	Sources        []lafSource `json:"sources"`
	AssortmentKeys []string    `json:"assortmentKeys,omitempty"`
	ListingKeys    []string    `json:"listingKeys"`
}

type lafModality struct {
	Type            string         `json:"type"`
	HandoffLocation lafLocation    `json:"handoffLocation"`
	HandoffAddress  lafAddressWrap `json:"handoffAddress,omitempty"`
}

type lafLocation struct {
	StoreID    string `json:"storeId"`
	FacilityID string `json:"facilityId"`
}

type lafAddressWrap struct {
	Address  lafAddress `json:"address"`
	Location lafLatLng  `json:"location"`
}

type lafAddress struct {
...
type lafLatLng struct {
	Lat float64 `json:"lat"`
	Lng float64 `json:"lng"`
}

type lafSource struct {
	StoreID    string `json:"storeId"`
	FacilityID string `json:"facilityId"`
}
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "harris-teeter-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		c.applyHarrisTeeterHeaders(req, params)
...
// PATCH: Harris Teeter Atlas product endpoints require browser-observed LAF/modality headers.
func (c *Client) applyHarrisTeeterHeaders(req *http.Request, params map[string]string) {
	if !strings.Contains(req.URL.Host, "harristeeter.com") || !strings.HasPrefix(req.URL.Path, "/atlas/") {
		return
	}
	locationID := firstNonEmpty(params["filter.locationId"], os.Getenv("HARRIS_TEETER_LOCATION_ID"), "09700096")
	facilityID := firstNonEmpty(os.Getenv("HARRIS_TEETER_FACILITY_ID"), "12956")
	modalityType := firstNonEmpty(params["filter.fulfillmentMethods"], os.Getenv("HARRIS_TEETER_MODALITY_TYPE"), "IN_STORE")
	if strings.Contains(modalityType, ",") {
		modalityType = strings.Split(modalityType, ",")[0]
	}
	req.Header.Set("Accept", "application/json, text/plain, */*")
	req.Header.Set("X-Kroger-Channel", "WEB")
	req.Header.Set("User-Time-Zone", firstNonEmpty(os.Getenv("TZ"), "America/New_York"))
	req.Header.Set("X-Facility-Id", facilityID)
	req.Header.Set("X-Modality-Type", modalityType)
	req.Header.Set("X-Modality", fmt.Sprintf(`{"type":%q,"locationId":%q}`, modalityType, locationID))
	req.Header.Set("X-Laf-Object", firstNonEmpty(os.Getenv("HARRIS_TEETER_LAF_OBJECT"), defaultLAFObject(locationID, facilityID)))
}

func defaultLAFObject(locationID, facilityID string) string {
	address := lafAddressWrap{
		Address: lafAddress{
			AddressLines:  []string{"5920 Carolina Beach Rd"},
			CityTown:      "Wilmington",
			Name:          "Beau Rivage Marketplace",
			PostalCode:    "28412",
			StateProvince: "NC",
			Residential:   false,
			CountryCode:   "US",
		},
		Location: lafLatLng{Lat: 34.1247729, Lng: -77.9024682},
	}
	objects := []lafObject{
		{
			Modality: lafModality{
				Type:            "IN_STORE",
				HandoffLocation: lafLocation{StoreID: locationID, FacilityID: facilityID},
				HandoffAddress:  address,
			},
			Sources:        []lafSource{{StoreID: locationID, FacilityID: facilityID}},
			AssortmentKeys: []string{"86219f6b-6123-4cd3-9a47-b02ddcf82f32"},
			ListingKeys:    []string{locationID},
		},
		{
			Modality: lafModality{
				Type:            "PICKUP",
			...
			Sources:        []lafSource{{StoreID: locationID, FacilityID: facilityID}},
			AssortmentKeys: []string{"4a9b54c3-b603-452a-a028-14da0f91a405"},
			ListingKeys:    []string{locationID},
		},
	}
