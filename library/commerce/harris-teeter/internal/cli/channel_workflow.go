...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  harris-teeter-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  harris-teeter-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"cart", "coupons", "lists", "products"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  harris-teeter-pp-cli workflow status

  # Show status as JSON
  harris-teeter-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("harris-teeter-pp-cli")
			}
