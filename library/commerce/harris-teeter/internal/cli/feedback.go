...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".harris-teeter-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HARRIS_TEETER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HARRIS_TEETER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "harris-teeter-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.harris-teeter-pp-cli/feedback.jsonl.
When ` + "`HARRIS_TEETER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HARRIS_TEETER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  harris-teeter-pp-cli feedback list
  harris-teeter-pp-cli feedback list --limit 5
  harris-teeter-pp-cli feedback list --json`,
