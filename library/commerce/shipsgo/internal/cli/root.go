...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "shipsgo-pp-cli",
		Short: `Shipsgo CLI — Every ShipsGo endpoint, plus an RFQ workspace, lane analytics, and an offline shipment book no other tool offers.`,
		Long: `Shipsgo CLI — Every ShipsGo endpoint, plus an RFQ workspace, lane analytics, and an offline shipment book no other tool offers.
...
Health check: run 'shipsgo-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("shipsgo-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'shipsgo-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAirCmd(flags))
	rootCmd.AddCommand(newOceanCmd(flags))
	...
	rootCmd.AddCommand(newNovelCreditBudgetCmd(flags))
	rootCmd.AddCommand(newNovelDigestCmd(flags))
	rootCmd.AddCommand(newNovelDupeCheckCmd(flags))
	rootCmd.AddCommand(newNovelEtaDriftCmd(flags))
	rootCmd.AddCommand(newNovelFollowersBroadcastCmd(flags))
	rootCmd.AddCommand(newNovelLaneCmd(flags))
	rootCmd.AddCommand(newNovelReliabilityCmd(flags))
	rootCmd.AddCommand(newNovelRfqCmd(flags))
	rootCmd.AddCommand(newNovelWebhookCmd(flags))
