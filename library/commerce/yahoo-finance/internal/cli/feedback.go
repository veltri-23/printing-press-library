...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "yahoo-finance-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("YAHOO_FINANCE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("YAHOO_FINANCE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "yahoo-finance-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/yahoo-finance-pp-cli/feedback.jsonl.
When ` + "`YAHOO_FINANCE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`YAHOO_FINANCE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  yahoo-finance-pp-cli feedback list
  yahoo-finance-pp-cli feedback list --limit 5
  yahoo-finance-pp-cli feedback list --json`,
