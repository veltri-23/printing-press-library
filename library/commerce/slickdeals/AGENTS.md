# Slickdeals Printed CLI Agent Guide

This directory is a generated `slickdeals-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
slickdeals-pp-cli doctor --json
slickdeals-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
slickdeals-pp-cli which "<capability>" --json
slickdeals-pp-cli <command> --help
```
...
```bash
slickdeals-pp-cli <command> --agent
```
...
```bash
slickdeals-pp-cli <command> --help
slickdeals-pp-cli <command> --dry-run --agent
```
