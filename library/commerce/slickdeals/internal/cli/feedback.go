...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".slickdeals-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SLICKDEALS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SLICKDEALS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "slickdeals-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.slickdeals-pp-cli/feedback.jsonl.
When ` + "`SLICKDEALS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SLICKDEALS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  slickdeals-pp-cli feedback list
  slickdeals-pp-cli feedback list --limit 5
  slickdeals-pp-cli feedback list --json`,
