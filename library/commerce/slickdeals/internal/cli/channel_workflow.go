...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  slickdeals-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  slickdeals-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"ajax", "frontpage", "frontpage-recommendation-carousel-recommendations", "web-api"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  slickdeals-pp-cli workflow status

  # Show status as JSON
  slickdeals-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("slickdeals-pp-cli")
			}
