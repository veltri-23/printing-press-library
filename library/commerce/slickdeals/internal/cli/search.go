...
In local mode: searches locally synced data only.`,
  ...
  slickdeals-pp-cli search "error timeout"

  # Force local search only
  slickdeals-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  slickdeals-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  slickdeals-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// --live routes to the Slickdeals RSS search endpoint. This is
			// the v0.2 live search path; the local FTS branch below is the
			...
			if live {
				...
				prov := DataProvenance{Source: "live", Reason: "rss_search", ResourceType: "search"}
				wrapped, err := wrapWithProvenance(data, prov)
				if err != nil {
					return err
				}
				printProvenance(cmd, len(items), prov)
				return printOutput(cmd.OutOrStdout(), wrapped, flags.asJSON || !isTerminal(cmd.OutOrStdout()))
			}
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'slickdeals-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
