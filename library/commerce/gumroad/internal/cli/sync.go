...
Exit codes & warnings:
  ...
  gumroad-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gumroad-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gumroad-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gumroad-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gumroad-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gumroad-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "offer_codes":
		return db.UpsertOfferCodes(data)
	case "products_subscribers":
		return db.UpsertProductsSubscribers(data)
	case "variant_categories":
		return db.UpsertVariantCategories(data)
	case "resource-subscriptions":
		return db.UpsertResourceSubscriptions(data)
	case "sales":
		return db.UpsertSales(data)
	case "mark_as_shipped":
		return db.UpsertMarkAsShipped(data)
	case "refund":
		return db.UpsertRefund(data)
	case "resend_receipt":
		return db.UpsertResendReceipt(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"payouts":                "/payouts",
		"products":               "/products",
		"resource-subscriptions": "/resource_subscriptions",
		"sales":                  "/sales",
		"tax-forms":              "/tax_forms",
	}
