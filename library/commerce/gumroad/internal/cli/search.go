...
In local mode: searches locally synced data only.`,
  ...
  gumroad-pp-cli search "error timeout"

  # Force local search only
  gumroad-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  gumroad-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  gumroad-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'gumroad-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "":
				// PATCH: No type filter should use the generic FTS index instead of returning no rows.
				results, err = db.Search(query, limit)
			default:
				// PATCH: Honor --type by limiting local FTS matches to the requested resource.
				results, err = db.SearchResourceType(resourceType, query, limit)
			}
			if err != nil {
