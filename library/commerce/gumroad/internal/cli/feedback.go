...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".gumroad-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GUMROAD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GUMROAD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gumroad-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.gumroad-pp-cli/feedback.jsonl.
When ` + "`GUMROAD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GUMROAD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gumroad-pp-cli feedback list
  gumroad-pp-cli feedback list --limit 5
  gumroad-pp-cli feedback list --json`,
