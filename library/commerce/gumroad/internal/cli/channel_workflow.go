...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  gumroad-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  gumroad-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"payouts", "products", "resource-subscriptions", "sales", "tax-forms"}
			totalSynced := 0
			...
			for _, resource := range resources {
				res := syncResource(c, s, resource, "", full, 100)
				if res.Err != nil {
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  gumroad-pp-cli workflow status

  # Show status as JSON
  gumroad-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("gumroad-pp-cli")
			}
