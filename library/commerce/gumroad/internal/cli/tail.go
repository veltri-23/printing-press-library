...
native streaming instead of polling.`,
  ...
  gumroad-pp-cli tail --interval 10s

  # Tail a specific resource
  gumroad-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  gumroad-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// PATCH: Tail output must respect Cobra writers so MCP callers receive events.
			out := cmd.OutOrStdout()
			errOut := cmd.ErrOrStderr()
			enc := json.NewEncoder(out)

			if follow {
				fmt.Fprintf(errOut, "Tailing %s every %s (Ctrl+C to stop)\n", resource, interval)
			} else {
				fmt.Fprintf(errOut, "Polling %s once\n", resource)
			}

			// Initial fetch
			if err := fetchAndEmit(c, path, enc); err != nil {
				fmt.Fprintf(errOut, "warning: initial fetch failed: %v\n", err)
			}

			// PATCH: Honor the documented single-poll mode instead of entering the ticker loop.
			if !follow {
				return nil
			}
			...
			for {
				...
				case <-ticker.C:
					if err := fetchAndEmit(c, path, enc); err != nil {
						fmt.Fprintf(errOut, "warning: poll failed: %v\n", err)
					}
				}
			}
		},
