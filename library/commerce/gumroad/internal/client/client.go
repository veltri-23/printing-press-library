...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "gumroad-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "gumroad-pp-cli/1.0.0")
		}
...
func (c *Client) authHeader() (string, error) {
	...
	if c.Config.AuthHeaderVal == "" && c.Config.AccessToken != "" && !c.Config.TokenExpiry.IsZero() && time.Now().After(c.Config.TokenExpiry) {
		// PATCH: Fail loudly for expired tokens instead of silently sending stale credentials.
		return "", fmt.Errorf("access token expired; run `gumroad-pp-cli auth set-token <token>` with a fresh Gumroad access token")
	}
