# Gumroad Printed CLI Agent Guide

This directory is a generated `gumroad-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gumroad-pp-cli doctor --json
gumroad-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gumroad-pp-cli which "<capability>" --json
gumroad-pp-cli <command> --help
```
...
```bash
gumroad-pp-cli <command> --help
gumroad-pp-cli <command> --dry-run --agent
```
