		...
		if raw, _ := args["args"].(string); strings.TrimSpace(raw) != "" {
			for _, token := range splitShellArgs(raw) {
				if strings.HasPrefix(token, "-") {
					return mcplib.NewToolResultError(fmt.Sprintf("refusing flag-like token in args passthrough: %s", token)), nil
				}
				finalArgs = append(finalArgs, token)
			}
		}
