...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".doordash-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOORDASH_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOORDASH_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "doordash-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.doordash-pp-cli/feedback.jsonl.
When ` + "`DOORDASH_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOORDASH_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  doordash-pp-cli feedback list
  doordash-pp-cli feedback list --limit 5
  doordash-pp-cli feedback list --json`,
