...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "doordash-pp-cli",
		Short: "Manage doordash resources via the doordash API",
		Long: `Manage doordash resources via the doordash API.
...
Run 'doordash-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("doordash-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'doordash-pp-cli profile list')")
	...
	// PATCH: add read-only DoorDash convenience wrappers above raw GraphQL endpoints.
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newMenuCmd(flags))
	rootCmd.AddCommand(newItemOptionsCmd(flags))
	rootCmd.AddCommand(newConvenienceSearchCmd(flags))
	rootCmd.AddCommand(newRecentOrdersCmd(flags))
	rootCmd.AddCommand(newAddressesCmd(flags))
	rootCmd.AddCommand(newPaymentMethodsCmd(flags))
	// PATCH: add guarded DoorDash cart UX commands above the raw GraphQL endpoints.
