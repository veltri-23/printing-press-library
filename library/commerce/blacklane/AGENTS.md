# Blacklane Printed CLI Agent Guide

This directory is a generated `blacklane-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
blacklane-pp-cli agent-context --pretty
```
...
```bash
blacklane-pp-cli <command> --agent
```
...
```bash
blacklane-pp-cli <command> --help
blacklane-pp-cli <command> --dry-run --agent
```
