...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "blacklane-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BLACKLANE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BLACKLANE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "blacklane-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/blacklane-pp-cli/feedback.jsonl.
When ` + "`BLACKLANE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BLACKLANE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  blacklane-pp-cli feedback list
  blacklane-pp-cli feedback list --limit 5
  blacklane-pp-cli feedback list --json`,
