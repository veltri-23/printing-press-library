...
Resource scoping:
  ...
  blacklane-pp-cli sync

  # Sync specific resources only
  blacklane-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  blacklane-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  blacklane-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  blacklane-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  blacklane-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "blacklane-pp-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
