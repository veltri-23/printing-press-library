# Amazon Orders Printed CLI Agent Guide

This directory is a generated `amazon-orders-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
amazon-orders-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
amazon-orders-pp-cli which "<capability>" --json
amazon-orders-pp-cli <command> --help
```
...
```bash
amazon-orders-pp-cli <command> --agent
```
...
```bash
amazon-orders-pp-cli <command> --help
amazon-orders-pp-cli <command> --dry-run --agent
```
