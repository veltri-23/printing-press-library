...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  amazon-orders-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  amazon-orders-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"orders", "transactions"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  amazon-orders-pp-cli workflow status

  # Show status as JSON
  amazon-orders-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("amazon-orders-pp-cli")
			}
