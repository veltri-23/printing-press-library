...
Exit codes & warnings:
  ...
  amazon-orders-pp-cli sync

  # Sync specific resources only
  amazon-orders-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  amazon-orders-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  amazon-orders-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  amazon-orders-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  amazon-orders-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/amazon-orders-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"orders":       "/your-orders/orders",
		"transactions": "/cpe/yourpayments/transactions",
	}
