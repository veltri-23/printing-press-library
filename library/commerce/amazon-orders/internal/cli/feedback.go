...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".amazon-orders-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMAZON_ORDERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMAZON_ORDERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "amazon-orders-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.amazon-orders-pp-cli/feedback.jsonl.
When ` + "`AMAZON_ORDERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMAZON_ORDERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-orders-pp-cli feedback list
  amazon-orders-pp-cli feedback list --limit 5
  amazon-orders-pp-cli feedback list --json`,
