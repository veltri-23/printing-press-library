...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".facebook-marketplace-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FACEBOOK_MARKET_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FACEBOOK_MARKET_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "facebook-marketplace-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.facebook-marketplace-pp-cli/feedback.jsonl.
When ` + "`FACEBOOK_MARKET_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FACEBOOK_MARKET_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "facebook-marketplace-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  facebook-marketplace-pp-cli feedback list
  facebook-marketplace-pp-cli feedback list --limit 5
  facebook-marketplace-pp-cli feedback list --json`,
