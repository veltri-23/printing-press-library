...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  facebook-marketplace-pp-cli doctor
  facebook-marketplace-pp-cli doctor --json
  facebook-marketplace-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "facebook-marketplace-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".facebook.com"
					if proofOK, proofDetail := browserSessionProofStatusForAuth(cfg, header); proofOK {
			...
			if os.Getenv("FACEBOOK_MARKET_COOKIES") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "FACEBOOK_MARKET_COOKIES")
			} else {
				authEnvInfo = append(authEnvInfo, "FACEBOOK_MARKET_COOKIES populated automatically by auth login --chrome")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					} else if proofOK, proofDetail := browserSessionProofStatusForAuth(cfg, authHeader); proofOK {
			...
			if err := recordDoctorPass(report); err != nil {
				report["doctor_pass_marker"] = fmt.Sprintf("error: %s", err)
			}
