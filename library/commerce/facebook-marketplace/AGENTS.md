# Facebook Marketplace Printed CLI Agent Guide

This directory is a generated `facebook-marketplace-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
facebook-marketplace-pp-cli doctor --json
facebook-marketplace-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
facebook-marketplace-pp-cli which "<capability>" --json
...
```bash
facebook-marketplace-pp-cli <command> --agent
```
...
```bash
facebook-marketplace-pp-cli <command> --help
facebook-marketplace-pp-cli <command> --dry-run --agent
```
