# Restaurant365 Odata Printed CLI Agent Guide

This directory is a generated `restaurant365-odata-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
restaurant365-odata-pp-cli doctor --json
restaurant365-odata-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
restaurant365-odata-pp-cli which "<capability>" --json
restaurant365-odata-pp-cli <command> --help
```
...
```bash
restaurant365-odata-pp-cli <command> --agent
```
...
```bash
restaurant365-odata-pp-cli <command> --help
restaurant365-odata-pp-cli <command> --dry-run --agent
```
