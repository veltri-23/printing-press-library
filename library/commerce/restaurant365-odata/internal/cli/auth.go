...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Restaurant365 OData Connector",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  restaurant365-odata-pp-cli auth setup\n  restaurant365-odata-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Restaurant365 OData uses HTTP Basic authentication.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export RESTAURANT365_ODATA_USERNAME=\"domain\\\\username\"")
			fmt.Fprintln(w, "  export RESTAURANT365_ODATA_PASSWORD=\"<password>\"")
			fmt.Fprintln(w, "  export RESTAURANT365_ODATA_BASE_URL=\"https://odata.restaurant365.net/api/v2/views\"")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Or save them locally:")
			fmt.Fprintln(w, "  restaurant365-odata-pp-cli auth set-token \"domain\\\\username\" \"<password>\"")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  restaurant365-odata-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "")
				fmt.Fprintln(w, "Set Restaurant365 OData credentials:")
				fmt.Fprintln(w, "  export RESTAURANT365_ODATA_USERNAME=\"your-username\"")
				fmt.Fprintln(w, "  export RESTAURANT365_ODATA_PASSWORD=\"your-password\"")
				fmt.Fprintf(w, "  restaurant365-odata-pp-cli auth set-token <username> <password>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Short:   "Save Restaurant365 OData username and password to the config file",
		Example: "  restaurant365-odata-pp-cli auth set-token \"domain\\\\username\" \"PASSWORD\"",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			cfg.AuthHeaderVal = ""
			if err := cfg.SaveBasicCredentials(args[0], args[1]); err != nil {
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  restaurant365-odata-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("RESTAURANT365_ODATA_USERNAME") != "" {
				envStillSet = "RESTAURANT365_ODATA_USERNAME"
			}
			if envStillSet == "" && os.Getenv("RESTAURANT365_ODATA_PASSWORD") != "" {
				envStillSet = "RESTAURANT365_ODATA_PASSWORD"
			}
			if envStillSet == "" && os.Getenv("R365_ODATA_USERNAME") != "" {
				envStillSet = "R365_ODATA_USERNAME"
			}
			if envStillSet == "" && os.Getenv("R365_ODATA_PASSWORD") != "" {
				envStillSet = "R365_ODATA_PASSWORD"
			}
