...
Resource scoping:
  ...
  restaurant365-odata-pp-cli sync

  # Sync specific resources only
  restaurant365-odata-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  restaurant365-odata-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  restaurant365-odata-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  restaurant365-odata-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  restaurant365-odata-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources = canonicalSyncResourceNames(resources)
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/restaurant365-odata-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if resourceSupportsPagination(resource) {
			...
			if pageSize.orderByParam != "" {
				if orderBy := syncResourceOrderBy(resource); orderBy != "" {
					params[pageSize.orderByParam] = orderBy
				}
			}
		}
...
func resourceSupportsPagination(resource string) bool {
	resource = canonicalSyncResourceName(resource)
	switch resource {
	case "company", "employee", "entity-deleted", "glaccount", "item", "job-title", "labor-detail", "location", "payroll-summary", "posemployee", "sales-detail", "sales-employee", "sales-payment", "transaction", "transaction-detail":
		return true
	}
	return false
}

func canonicalSyncResourceName(resource string) string {
...
func canonicalSyncResourceNames(resources []string) []string {
...
func syncResourceOrderBy(resource string) string {
	if idField := resourceIDFieldOverrides[canonicalSyncResourceName(resource)]; idField != "" {
		return idField + " asc"
	}
	return ""
}
...
var resourceIDFieldOverrides = map[string]string{
	"company":            "companyId",
	"employee":           "employeeId",
	"entity-deleted":     "entityDeletedId",
	"glaccount":          "glAccountId",
	"item":               "itemId",
	"job-title":          "jobTitleId",
	"labor-detail":       "laborId",
	"location":           "locationId",
	"payroll-summary":    "payrollSummaryId",
	"posemployee":        "posEmployeeId",
	"sales-detail":       "salesDetailId",
	"sales-employee":     "salesEmployeeId",
	"sales-payment":      "salesPaymentId",
	"transaction":        "transactionId",
	"transaction-detail": "transactionDetailId",
}
...
var pageItemKeys = []string{
	"value", "Value", "data", "results", "items", "records", "nodes", "entries", "features",
