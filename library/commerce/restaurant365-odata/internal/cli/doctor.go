...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  restaurant365-odata-pp-cli doctor
  restaurant365-odata-pp-cli doctor --json
  restaurant365-odata-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export RESTAURANT365_ODATA_USERNAME=\"domain\\\\username\" and RESTAURANT365_ODATA_PASSWORD=\"<password>\""
				} else {
					authConfigured = true
			...
			usernameEnvSet := os.Getenv("RESTAURANT365_ODATA_USERNAME") != "" || os.Getenv("R365_ODATA_USERNAME") != ""
			passwordEnvSet := os.Getenv("RESTAURANT365_ODATA_PASSWORD") != "" || os.Getenv("R365_ODATA_PASSWORD") != ""
			if usernameEnvSet {
				authEnvSet = append(authEnvSet, "RESTAURANT365_ODATA_USERNAME")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RESTAURANT365_ODATA_USERNAME or R365_ODATA_USERNAME")
			}
			if passwordEnvSet {
				authEnvSet = append(authEnvSet, "RESTAURANT365_ODATA_PASSWORD")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "RESTAURANT365_ODATA_PASSWORD or R365_ODATA_PASSWORD")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Restaurant365's OData service is a named-view surface. Probe
					// $metadata rather than "/" so doctor verifies the same surface
					// data-engineering commands depend on.
					metadata, reachErr := fetchR365Metadata(cmd.Context(), c)
					var reachAPIErr *client.APIError
					switch {
					case reachErr == nil:
						report["api"] = "metadata reachable"
						report["credentials"] = "verified with $metadata"
						report["views_detected"] = len(metadata)
					case errors.As(reachErr, &reachAPIErr):
						...
						if vendor := looksLikeDoctorInterstitial([]byte(reachAPIErr.Body)); vendor != "" {
							...
							report["api"] = fmt.Sprintf("error: metadata probe failed (HTTP %d at $metadata)", status)
						}
					default:
						report["api"] = fmt.Sprintf("unreachable: %s", reachErr)
					}

					// Step 2: If metadata could not be reached, tell the operator
					// whether credentials are loaded so they know what to fix next.
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm credentials end-to-end.", "restaurant365-odata-pp-cli", suggestion)
						} else {
							report["credentials"] = "present, not verified. Run any read command to confirm credentials end-to-end."
						}
					}
				}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("restaurant365-odata-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'restaurant365-odata-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'restaurant365-odata-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'restaurant365-odata-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'restaurant365-odata-pp-cli sync' to refresh."
	}
	return report
}
