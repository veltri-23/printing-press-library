...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "restaurant365-odata-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RESTAURANT365_ODATA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RESTAURANT365_ODATA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "restaurant365-odata-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/restaurant365-odata-pp-cli/feedback.jsonl.
When ` + "`RESTAURANT365_ODATA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RESTAURANT365_ODATA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "restaurant365-odata-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  restaurant365-odata-pp-cli feedback list
  restaurant365-odata-pp-cli feedback list --limit 5
  restaurant365-odata-pp-cli feedback list --json`,
