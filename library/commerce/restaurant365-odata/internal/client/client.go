...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "restaurant365-odata-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "restaurant365-odata-pp-cli/1.0.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export RESTAURANT365_ODATA_USERNAME=\"domain\\\\username\" and RESTAURANT365_ODATA_PASSWORD=\"<password>\"")
}
...
func authPlaceholderCredentialErrorWithSetup(cfg *config.Config, setup string) error {
	...
	return fmt.Errorf("%w configured in %s; set real Restaurant365 OData credentials with: %s", ErrPlaceholderCredential, location, setup)
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.Restaurant365OdataPassword)
	}
