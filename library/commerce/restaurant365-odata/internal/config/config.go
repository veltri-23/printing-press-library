...
type Config struct {
	...
	Path                       string            `toml:"-"`
	Restaurant365OdataUsername string            `toml:"odata_username"`
	Restaurant365OdataPassword string            `toml:"odata_password"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("RESTAURANT365_ODATA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "restaurant365-odata-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("RESTAURANT365_ODATA_USERNAME"); v != "" {
		cfg.Restaurant365OdataUsername = v
		cfg.AuthSource = "env:RESTAURANT365_ODATA_USERNAME"
	}
	if v := os.Getenv("R365_ODATA_USERNAME"); v != "" {
		cfg.Restaurant365OdataUsername = v
		cfg.AuthSource = "env:R365_ODATA_USERNAME"
	}
	if v := os.Getenv("RESTAURANT365_ODATA_PASSWORD"); v != "" {
		cfg.Restaurant365OdataPassword = v
		cfg.AuthSource = "env:RESTAURANT365_ODATA_PASSWORD"
	}
	if v := os.Getenv("R365_ODATA_PASSWORD"); v != "" {
		cfg.Restaurant365OdataPassword = v
		cfg.AuthSource = "env:R365_ODATA_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.Restaurant365OdataUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.Restaurant365OdataPassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("RESTAURANT365_ODATA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	if v := os.Getenv("R365_ODATA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.Restaurant365OdataUsername == "" || c.Restaurant365OdataPassword == "" {
		return ""
	}
	credentials := c.Restaurant365OdataUsername + ":" + c.Restaurant365OdataPassword
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.Restaurant365OdataUsername = token
	return c.save()
}

func (c *Config) SaveBasicCredentials(username, password string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.Restaurant365OdataUsername = username
	c.Restaurant365OdataPassword = password
	return c.save()
}
...
func (c *Config) ClearTokens() error {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.Restaurant365OdataUsername = ""
	c.Restaurant365OdataPassword = ""
	return c.save()
}
