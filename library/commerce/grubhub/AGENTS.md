# Grubhub Printed CLI Agent Guide

This directory is a generated `grubhub-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
grubhub-pp-cli doctor --json
grubhub-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
grubhub-pp-cli which "<capability>" --json
grubhub-pp-cli <command> --help
```
...
```bash
grubhub-pp-cli <command> --help
grubhub-pp-cli <command> --dry-run --agent
```
