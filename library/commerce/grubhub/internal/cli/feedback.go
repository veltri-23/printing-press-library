...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "grubhub-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GRUBHUB_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GRUBHUB_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "grubhub-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/grubhub-pp-cli/feedback.jsonl.
When ` + "`GRUBHUB_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GRUBHUB_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  grubhub-pp-cli feedback list
  grubhub-pp-cli feedback list --limit 5
  grubhub-pp-cli feedback list --json`,
