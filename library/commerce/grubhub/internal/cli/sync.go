...
Resource scoping:
  ...
  grubhub-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  grubhub-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  grubhub-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  grubhub-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  grubhub-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "grubhub-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/grubhub-pp-cli/data.db)")
