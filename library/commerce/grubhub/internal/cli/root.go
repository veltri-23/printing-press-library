...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "grubhub-pp-cli",
		Short: `Search restaurants, compare delivery fees and ETAs, browse full menus, and surface deals across Grubhub — agent-native, with offline menu search no Grubhub tool has.`,
		Long: `Search restaurants, compare delivery fees and ETAs, browse full menus, and surface deals across Grubhub — agent-native, with offline menu search no Grubhub tool has.
...
Health check: run 'grubhub-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("grubhub-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'grubhub-pp-cli profile list')")
	...
	rootCmd.PersistentPreRunE = func(cmd *cobra.Command, args []string) error {
		...
		// Grubhub uses a runtime-minted anonymous bearer (no user key). The
		// friendly commands (near/compare/dish/...) mint on their own path; this
		// covers the generated raw `restaurants` subtree so it works without a
		// separate `auth login`. Skipped under verify/dry-run to avoid network.
		if needsGrubhubToken(cmd) && !flags.dryRun && !cliutil.IsVerifyEnv() {
			_, _ = grubhub.EnsureToken(cmd.Context(), flags.configPath)
		}
		return nil
	}
	...
	rootCmd.AddCommand(newNovelDishCmd(flags))
	rootCmd.AddCommand(newNovelPickCmd(flags))
