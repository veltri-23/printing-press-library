...
type Config struct {
	...
	GrubhubToken  string            `toml:"token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api-gtm.grubhub.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("GRUBHUB_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "grubhub-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GRUBHUB_TOKEN"); v != "" {
		cfg.GrubhubToken = v
		cfg.markEnvOverride("GrubhubToken")
		cfg.AuthSource = "env:GRUBHUB_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.GrubhubToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GRUBHUB_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GrubhubToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:GRUBHUB_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"token":         c.GrubhubToken,
			"GRUBHUB_TOKEN": c.GrubhubToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.GrubhubToken = ""
	delete(c.envOverrides, "GrubhubToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GrubhubToken"] {
			out.GrubhubToken = c.fileConfig.GrubhubToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GrubhubToken":
		c.fileConfig.GrubhubToken = c.GrubhubToken
	}
}
