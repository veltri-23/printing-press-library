...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "daraz-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DARAZ_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DARAZ_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "daraz-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/daraz-pp-cli/feedback.jsonl.
When ` + "`DARAZ_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DARAZ_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  daraz-pp-cli feedback list
  daraz-pp-cli feedback list --limit 5
  daraz-pp-cli feedback list --json`,
