...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "daraz-pp-cli",
		Short: `Daraz CLI — Every Daraz product search, plus a local price database, real-deal ranking, and drop alerts no Daraz scraper has.`,
		Long: `Daraz CLI — Every Daraz product search, plus a local price database, real-deal ranking, and drop alerts no Daraz scraper has.
...
Health check: run 'daraz-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("daraz-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'daraz-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelPriceHistoryCmd(flags))
	rootCmd.AddCommand(newNovelSellerCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelValueCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	productsCmd := newProductsPromotedCmd(flags)
	productsCmd.AddCommand(newProductsGetCmd(flags))
	rootCmd.AddCommand(productsCmd)
	rootCmd.AddCommand(newReviewsPromotedCmd(flags))
