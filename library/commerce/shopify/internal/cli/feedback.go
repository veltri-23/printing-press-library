...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "shopify-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHOPIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHOPIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "shopify-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/shopify-pp-cli/feedback.jsonl.
When ` + "`SHOPIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHOPIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shopify-pp-cli feedback list
  shopify-pp-cli feedback list --limit 5
  shopify-pp-cli feedback list --json`,
