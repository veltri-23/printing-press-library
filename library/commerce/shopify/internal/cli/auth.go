...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  shopify-pp-cli auth setup\n  shopify-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://shopify.dev/docs/apps/build/authentication-authorization/access-tokens")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export SHOPIFY_ACCESS_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  shopify-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://shopify.dev/docs/apps/build/authentication-authorization/access-tokens"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  shopify-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export SHOPIFY_ACCESS_TOKEN=\"your-token-here\"")
				fmt.Fprintf(w, "  shopify-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  shopify-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  shopify-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SHOPIFY_ACCESS_TOKEN") != "" {
				envStillSet = "SHOPIFY_ACCESS_TOKEN"
			}
