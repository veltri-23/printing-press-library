...
In local mode: searches locally synced data only.`,
  ...
  shopify-pp-cli search "error timeout"

  # Force local search only
  shopify-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  shopify-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  shopify-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'shopify-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "abandoned-checkouts":
				results, err = db.SearchAbandonedCheckouts(query, limit)
			case "orders":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchAbandonedCheckouts(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search abandoned_checkouts failed: %w", searchErr)
					}
