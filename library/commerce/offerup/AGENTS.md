# OfferUp Printed CLI Agent Guide

This directory is a generated `offerup-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
offerup-pp-cli doctor --json
offerup-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
offerup-pp-cli which "<capability>" --json
offerup-pp-cli <command> --help
```
...
```bash
offerup-pp-cli <command> --help
offerup-pp-cli <command> --dry-run --agent
```
