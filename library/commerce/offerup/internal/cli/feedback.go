...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "offerup-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OFFERUP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OFFERUP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "offerup-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/offerup-pp-cli/feedback.jsonl.
When ` + "`OFFERUP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OFFERUP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  offerup-pp-cli feedback list
  offerup-pp-cli feedback list --limit 5
  offerup-pp-cli feedback list --json`,
