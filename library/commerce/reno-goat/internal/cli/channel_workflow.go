...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  reno-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  reno-goat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"brands", "compare", "find-related", "find-similar", "product-search", "reviews", "stores"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  reno-goat-pp-cli workflow status

  # Show status as JSON
  reno-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("reno-goat-pp-cli")
			}
