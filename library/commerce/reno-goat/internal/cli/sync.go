...
Resource scoping:
  ...
  reno-goat-pp-cli sync

  # Sync specific resources only
  reno-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  reno-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  reno-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  reno-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  reno-goat-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"compare":        "/compare",
		"find-related":   "/graphql",
		"find-similar":   "/graphql",
		"product-search": "/graphql",
		"reviews":        "/graphql",
		"stores":         "/search/stores.json",
	}
