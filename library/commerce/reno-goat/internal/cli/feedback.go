...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "reno-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RENO_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RENO_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/commerce/reno-goat/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/reno-goat-pp-cli/feedback.jsonl.
When ` + "`RENO_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RENO_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  reno-goat-pp-cli feedback list
  reno-goat-pp-cli feedback list --limit 5
  reno-goat-pp-cli feedback list --json`,
