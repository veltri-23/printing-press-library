...
In local mode: searches locally synced data only.`,
  ...
  reno-goat-pp-cli search "error timeout"

  # Force local search only
  reno-goat-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  reno-goat-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  reno-goat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /graphql
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'reno-goat-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/reno-goat-pp-cli/data.db)")
