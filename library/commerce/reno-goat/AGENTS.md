# Reno Goat Printed CLI Agent Guide

This directory is a generated `reno-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
reno-goat-pp-cli doctor --json
reno-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
reno-goat-pp-cli which "<capability>" --json
reno-goat-pp-cli <command> --help
```
...
```bash
reno-goat-pp-cli <command> --agent
```
...
```bash
reno-goat-pp-cli <command> --help
reno-goat-pp-cli <command> --dry-run --agent
```
