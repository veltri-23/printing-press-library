# Costco Printed CLI Agent Guide

This directory is a generated `costco-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
costco-pp-cli doctor --json
costco-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
costco-pp-cli which "<capability>" --json
...
```bash
costco-pp-cli <command> --help
costco-pp-cli <command> --dry-run --agent
```
