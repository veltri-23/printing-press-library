...
type Config struct {
	...
	CostcoIdToken    string          `toml:"id_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://ecom-api.costco.com/ebusiness/order/v1",
	}
	...
	// Env var overrides
	if v := os.Getenv("COSTCO_ID_TOKEN"); v != "" {
		cfg.CostcoIdToken = v
		cfg.markEnvOverride("CostcoIdToken")
		cfg.AuthSource = "env:COSTCO_ID_TOKEN"
		cfg.CredentialSource = "env:COSTCO_ID_TOKEN"
	}
	...
	if v := os.Getenv("COSTCO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("COSTCO_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "costco-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.CostcoIdToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:COSTCO_ID_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"id_token":        c.CostcoIdToken,
			"COSTCO_ID_TOKEN": c.CostcoIdToken,
			"token":           c.CostcoIdToken,
		})
	}
	if c.AccessToken != "" {
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.CostcoIdToken = creds.CostcoIdToken
}
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.CostcoIdToken = ""
	delete(c.envOverrides, "CostcoIdToken")
	c.updateFileConfigField("CostcoIdToken")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CostcoIdToken"] {
			out.CostcoIdToken = c.fileConfig.CostcoIdToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CostcoIdToken":
		c.fileConfig.CostcoIdToken = c.CostcoIdToken
	}
}
