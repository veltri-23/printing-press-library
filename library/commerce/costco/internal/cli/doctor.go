...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  costco-pp-cli doctor
  costco-pp-cli doctor --json
  costco-pp-cli doctor --fail-on warn
  costco-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set it with: costco-pp-cli auth set-token <token> or export COSTCO_ID_TOKEN=\"your-token-here\""
					report["auth_docs_url"] = "https://ecom-api.costco.com"
				} else {
					authConfigured = true
					report["auth"] = "configured"
					report["auth_source"] = cfg.AuthSource
					// The Costco idToken is a short-lived JWT (~15 min). Decode
					// its exp so doctor can tell the user when to re-paste a
					// fresh token — the most common failure mode for this CLI.
					if cfg.CostcoIdToken != "" {
						if exp, ok := decodeJWTExp(cfg.CostcoIdToken); ok {
							report["token_expires"] = exp.UTC().Format(time.RFC3339)
							if time.Now().After(exp) {
								report["token_status"] = "EXPIRED — re-copy localStorage.idToken from a logged-in costco.com tab and run: costco-pp-cli auth set-token <token>"
							} else {
								report["token_status"] = fmt.Sprintf("valid for ~%s", time.Until(exp).Round(time.Second))
							}
						}
					}
				}
			...
			if os.Getenv("COSTCO_ID_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "COSTCO_ID_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "COSTCO_ID_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "costco-pp-cli", suggestion)
						} else {
