...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COSTCO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COSTCO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "costco-pp-cli/feedback")
...
When ` + "`COSTCO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COSTCO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  costco-pp-cli feedback list
  costco-pp-cli feedback list --limit 5
  costco-pp-cli feedback list --json`,
