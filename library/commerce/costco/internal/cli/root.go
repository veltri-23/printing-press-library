...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "costco-pp-cli",
		Short: `Costco CLI — Your complete Costco receipt history as data — past the website's 2-year wall, in a local database the site never gives…`,
		Long: `Costco CLI — Your complete Costco receipt history as data — past the website's 2-year wall, in a local database the site never gives…
...
Health check: run 'costco-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("costco-pp-cli {{ .Version }}\n")
	...
	rootCmd.AddCommand(newNovelHistoryDepthCmd(flags))
	rootCmd.AddCommand(newNovelItemHistoryCmd(flags))
	rootCmd.AddCommand(newNovelReturnsWindowCmd(flags))
	rootCmd.AddCommand(newNovelSavingsCmd(flags))
	rootCmd.AddCommand(newNovelSpendCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newRawPromotedCmd(flags))
	// Hand-built Costco receipt commands (GraphQL endpoint; see costco_graphql.go).
	rootCmd.AddCommand(newReceiptsCmd(flags))
	rootCmd.AddCommand(newReceiptCmd(flags))
	rootCmd.AddCommand(newCountsCmd(flags))
