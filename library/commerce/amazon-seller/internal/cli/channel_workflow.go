...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  amazon-seller-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  amazon-seller-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"fba-inventory", "orders", "reports", "sellers"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  amazon-seller-pp-cli workflow status

  # Show status as JSON
  amazon-seller-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("amazon-seller-pp-cli")
			}
