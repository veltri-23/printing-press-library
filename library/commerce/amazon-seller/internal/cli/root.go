...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "amazon-seller-pp-cli",
		Short: `Operate an Amazon seller account from the terminal: inventory, orders, reports, listings, and catalog.`,
		Long: `Operate an Amazon seller account from the terminal: inventory, orders, reports, listings, and catalog.

Add --agent to any command for JSON output + non-interactive mode.
Run 'amazon-seller-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("amazon-seller-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'amazon-seller-pp-cli profile list')")
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("amazon-seller-pp-cli %s\n", version)
		},
	}
}
