...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-seller-pp-cli doctor
  amazon-seller-pp-cli doctor --json
  amazon-seller-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				if cfg.ClientID != "" && cfg.ClientSecret != "" && cfg.RefreshToken != "" {
					...
					report["auth_key_url"] = "https://sellercentral.amazon.com/sellingpartner/developerconsole"
				}
			}
			...
			if os.Getenv("SP_API_LWA_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "SP_API_LWA_CLIENT_ID")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SP_API_LWA_CLIENT_ID")
			}
			if os.Getenv("SP_API_LWA_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "SP_API_LWA_CLIENT_SECRET")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SP_API_LWA_CLIENT_SECRET")
			}
			if os.Getenv("SP_API_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SP_API_REFRESH_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SP_API_REFRESH_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" && !refreshTokenConfigured {
						...
						verifyPath := "/sellers/v1/marketplaceParticipations"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("amazon-seller-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'amazon-seller-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'amazon-seller-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'amazon-seller-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'amazon-seller-pp-cli sync' to refresh."
	}
	return report
}
