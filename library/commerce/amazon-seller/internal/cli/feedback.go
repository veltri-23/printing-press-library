...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".amazon-seller-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMAZON_SELLER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMAZON_SELLER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "amazon-seller-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.amazon-seller-pp-cli/feedback.jsonl.
When ` + "`AMAZON_SELLER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMAZON_SELLER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-seller-pp-cli feedback list
  amazon-seller-pp-cli feedback list --limit 5
  amazon-seller-pp-cli feedback list --json`,
