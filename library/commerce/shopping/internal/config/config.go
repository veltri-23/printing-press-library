...
type Config struct {
	...
	Path           string            `toml:"-"`
	ShoppingApiKey string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.lemmebuyit.com/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SHOPPING_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "shopping-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SHOPPING_API_KEY"); v != "" {
		cfg.ShoppingApiKey = v
		cfg.AuthSource = "env:SHOPPING_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ShoppingApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SHOPPING_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
