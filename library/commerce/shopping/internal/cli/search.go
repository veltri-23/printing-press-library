...
In local mode: searches locally synced data only.`,
  ...
  shopping-pp-cli search "error timeout"

  # Force local search only
  shopping-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  shopping-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  shopping-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'shopping-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "retailers":
				results, err = db.SearchRetailers(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchRetailers(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search retailers failed: %w", searchErr)
					}
