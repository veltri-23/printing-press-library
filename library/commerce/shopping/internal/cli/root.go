...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "shopping-pp-cli",
		Short: `Shopping CLI — Every LemmeBuyIt retail endpoint, plus a local store that compares one UPC across 70+ retailers, ranks weekly price dro…`,
		Long: `Shopping CLI — Every LemmeBuyIt retail endpoint, plus a local store that compares one UPC across 70+ retailers, ranks weekly price dro…
...
Health check: run 'shopping-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("shopping-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'shopping-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelArbitrageCmd(flags))
	rootCmd.AddCommand(newNovelCompareCmd(flags))
	rootCmd.AddCommand(newNovelDealsCmd(flags))
	rootCmd.AddCommand(newNovelIndexCmd(flags))
	rootCmd.AddCommand(newNovelLeaderboardCmd(flags))
	...
	rootCmd.AddCommand(newRetailersPromotedCmd(flags))
