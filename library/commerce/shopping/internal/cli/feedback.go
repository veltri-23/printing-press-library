...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "shopping-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SHOPPING_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SHOPPING_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "shopping-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/shopping-pp-cli/feedback.jsonl.
When ` + "`SHOPPING_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SHOPPING_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  shopping-pp-cli feedback list
  shopping-pp-cli feedback list --limit 5
  shopping-pp-cli feedback list --json`,
