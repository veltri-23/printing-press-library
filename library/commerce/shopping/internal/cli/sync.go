...
Resource scoping:
  ...
  shopping-pp-cli sync

  # Sync specific resources only
  shopping-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  shopping-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  shopping-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  shopping-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  shopping-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/shopping-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "amazon":
		return db.UpsertAmazon(data)
	case "retailers":
		return db.UpsertRetailers(data)
	case "categories":
		return db.UpsertCategories(data)
	case "products":
		return db.UpsertProducts(data)
	case "shopping":
		return db.UpsertShopping(data)
	case "status":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "products", ParentTable: "retailers", ParentIDParam: "retailer_id", PathTemplate: "/retailers/{retailer_id}/products", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "retailer_id", Field: "id"},
		}},
		{Name: "shopping", ParentTable: "retailers", ParentIDParam: "retailer_id", PathTemplate: "/retailers/{retailer_id}/shopping/products", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "retailer_id", Field: "id"},
		}},
	}
}
...
func syncDependentResource(ctx context.Context, c interface {
	...
	// Under live-dogfood, bound the dependent-parent fan-out so a wide parent
	// table (e.g. dozens of retailers) cannot blow the flat 30s per-command
	// timeout. The per-resource page cap alone is insufficient when a dependent
	// has many parents.
	if cliutil.IsDogfoodEnv() && len(parentRows) > 2 {
		parentRows = parentRows[:2]
	}
