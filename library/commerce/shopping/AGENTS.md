# Shopping Printed CLI Agent Guide

This directory is a generated `shopping-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
shopping-pp-cli doctor --json
shopping-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
shopping-pp-cli which "<capability>" --json
shopping-pp-cli <command> --help
```
...
```bash
shopping-pp-cli <command> --help
shopping-pp-cli <command> --dry-run --agent
```
