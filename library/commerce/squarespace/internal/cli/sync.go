...
Exit codes & warnings:
  ...
  squarespace-pp-cli sync

  # Sync specific resources only
  squarespace-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  squarespace-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  squarespace-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  squarespace-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  squarespace-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("squarespace-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/squarespace-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "1-0":
		return db.UpsertV10(data)
	case "commerce":
		return db.UpsertCommerce(data)
	case "address_book":
...
func defaultSyncResources() []string {
	return []string{
		...
		"1-0-commerce-orders",
		"1-0-commerce-store-pages",
		"1-0-commerce-transactions",
		"1-0-webhook-subscriptions",
		"commerce",
	}
}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"1-0":                       "/1.0/profiles",
		"1-0-commerce-inventory":    "/1.0/commerce/inventory",
		"1-0-commerce-orders":       "/1.0/commerce/orders",
		"1-0-commerce-store-pages":  "/1.0/commerce/store_pages",
		"1-0-commerce-transactions": "/1.0/commerce/transactions",
		"1-0-webhook-subscriptions": "/1.0/webhook_subscriptions",
		"commerce":                  "/v2/commerce/products",
	}
