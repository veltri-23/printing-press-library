...
func TestRootCmd_HasSiteAndPageAliases(t *testing.T) {
	root := RootCmd()
...
func TestRootCmd_HasAccountDashboardCommands(t *testing.T) {
	root := RootCmd()
	...
	for _, path := range [][]string{
		...
		{"account", "briefs"},
		{"account", "domains"},
		{"account", "domain-summaries"},
		{"account", "domain", "get"},
		{"account", "domain", "whois"},
		{"account", "domain", "registrar-info"},
		{"account", "domain", "certificates"},
		{"account", "domain", "is-unparked"},
		{"account", "domain", "category"},
		{"account", "domain", "permissions"},
		{"account", "domain", "forwarding-presets"},
		{"account", "domain", "custom-records"},
		{"account", "domain", "presets"},
		{"account", "domain", "email-forwarding"},
		{"account", "domain", "email-mx-conflicts"},
		{"account", "domain", "billing-eligibility"},
		{"account", "domain", "billing-valid-terms"},
		{"account", "domain", "google-workspace-pricing"},
		{"account", "website", "get"},
		{"account", "website", "domains"},
		{"account", "website", "contributors"},
		{"account", "site", "domains"},
	} {
		cmd, _, err := root.Find(path)
		if err != nil {
			t.Fatalf("find %v: %v", path, err)
