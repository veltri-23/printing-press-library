...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  squarespace-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  squarespace-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"1-0", "1-0-commerce-inventory", "1-0-commerce-orders", "1-0-commerce-store-pages", "1-0-commerce-transactions", "1-0-webhook-subscriptions", "commerce"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  squarespace-pp-cli workflow status

  # Show status as JSON
  squarespace-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("squarespace-pp-cli")
			}
