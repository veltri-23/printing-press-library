...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  squarespace-pp-cli doctor
  squarespace-pp-cli doctor --json
  squarespace-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export COMMERCE_AUTHORIZATION=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("COMMERCE_AUTHORIZATION") != "" {
				authEnvSet = append(authEnvSet, "COMMERCE_AUTHORIZATION")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "COMMERCE_AUTHORIZATION")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'squarespace-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'squarespace-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'squarespace-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'squarespace-pp-cli sync' to refresh."
	}
	return report
}
