...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".squarespace-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SQUARESPACE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SQUARESPACE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "squarespace-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.squarespace-pp-cli/feedback.jsonl.
When ` + "`SQUARESPACE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SQUARESPACE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  squarespace-pp-cli feedback list
  squarespace-pp-cli feedback list --limit 5
  squarespace-pp-cli feedback list --json`,
