...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "squarespace-pp-cli",
		Short: "Manage squarespace resources via the squarespace API",
		Long: `Manage squarespace resources via the squarespace API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'squarespace-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("squarespace-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'squarespace-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCommerceCmd(flags))
	rootCmd.AddCommand(newContactsCmd(flags))
	rootCmd.AddCommand(newSiteCmd(flags))
	rootCmd.AddCommand(newStorePagesAliasCmd(flags))
	...
	rootCmd.AddCommand(newCommerceAnalyticsPromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("squarespace-pp-cli %s\n", version)
		},
	}
}
