...
func TestUpsertBatch_PopulatesV10Table(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("1-0", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "1-0").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "1_0")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count 1_0: %v", err)
	}
	if typed != len(items) {
...
// TestUpsertBatch_PopulatesCommerceTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed commerce table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesCommerceTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("commerce", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "commerce").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "commerce")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count commerce: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("commerce count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_PopulatesAddressBookTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed address_book table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesAddressBookTable(t *testing.T) {
	...
	if _, _, err := s.UpsertBatch("address_book", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "address_book").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "address_book")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count address_book: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("address_book count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
