...
// TestMigrate_AddsColumnsOnUpgrade_V10 verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_V10(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE "1_0" (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	rows, err := s.DB().Query(`PRAGMA table_info("1_0")`)
	if err != nil {
	...
	for _, want := range []string{
		...
		"order_number",
		"payment_state",
		"price_tax_interpretation",
		"shipping_option_name",
		"shipping_option_service_type",
		"testmode",
		"client_id",
		"endpoint_url",
		"updated_on",
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from 1_0 after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Commerce verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Commerce(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE commerce (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(commerce)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from commerce after migrate", want)
		}
	}
}
...
func TestMigrate_AddsColumnsOnUpgrade_AddressBook(t *testing.T) {
	...
	if _, err := raw.Exec(`CREATE TABLE address_book (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
		synced_at DATETIME DEFAULT CURRENT_TIMESTAMP
	)`); err != nil {
	...
	// The migration must have added every generated column.
	rows, err := s.DB().Query(`PRAGMA table_info(address_book)`)
	if err != nil {
		t.Fatalf("table_info: %v", err)
	}
	defer rows.Close()
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from address_book after migrate", want)
		}
	}
}
