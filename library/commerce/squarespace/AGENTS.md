# Squarespace Printed CLI Agent Guide

This directory is a generated `squarespace-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
squarespace-pp-cli doctor --json
squarespace-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
squarespace-pp-cli which "<capability>" --json
squarespace-pp-cli <command> --help
```
...
```bash
squarespace-pp-cli <command> --agent
```
...
```bash
squarespace-pp-cli <command> --help
squarespace-pp-cli <command> --dry-run --agent
```
