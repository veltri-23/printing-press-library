...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "printify-pp-cli",
		Short: `Printify CLI — Create and audit Printify products with manifest-driven image uploads, personalization checks, and local proofing.`,
		Long: `Printify CLI — Create and audit Printify products with manifest-driven image uploads, personalization checks, and local proofing.
...
Health check: run 'printify-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("printify-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'printify-pp-cli profile list')")
	...
	rootCmd.AddCommand(newShopsJsonPromotedCmd(flags))
	rootCmd.AddCommand(newUploadsJsonPromotedCmd(flags))
	rootCmd.AddCommand(newPersonalizationAuditCmd(flags))
	rootCmd.AddCommand(newPlacementMatrixCmd(flags))
	rootCmd.AddCommand(newCatalogMarginMatrixCmd(flags))
	rootCmd.AddCommand(newProductDriftCmd(flags))
	rootCmd.AddCommand(newPersonalizationBatchCmd(flags))
	rootCmd.AddCommand(newAssetReuseCmd(flags))
	rootCmd.AddCommand(newFulfillmentRiskCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the installed printify-pp-cli version for scripts and support diagnostics",
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("printify-pp-cli %s\n", version)
		},
	}
}
