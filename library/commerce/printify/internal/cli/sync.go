...
Resource scoping:
  ...
  printify-pp-cli sync

  # Sync specific resources only
  printify-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  printify-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  printify-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  printify-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  printify-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/printify-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "connection_json":
		return db.UpsertConnectionJson(data)
	case "orders":
		return db.UpsertOrders(data)
	case "orders_json":
		return db.UpsertOrdersJson(data)
	case "products":
		return db.UpsertProducts(data)
	case "products_json":
		return db.UpsertProductsJson(data)
	case "webhooks":
		return db.UpsertWebhooks(data)
	case "webhooks_json":
		return db.UpsertWebhooksJson(data)
	case "archive_json":
		return db.UpsertArchiveJson(data)
	case "uploads-json":
		return db.UpsertUploadsJson(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"catalog-print-providers-json": "/v1/catalog/print_providers.json",
		"shops-json":                   "/v1/shops.json",
		"uploads-json":                 "/v1/uploads.json",
	}
