...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "printify-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PRINTIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PRINTIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "printify-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/printify-pp-cli/feedback.jsonl.
When ` + "`PRINTIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PRINTIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  printify-pp-cli feedback list
  printify-pp-cli feedback list --limit 5
  printify-pp-cli feedback list --json`,
