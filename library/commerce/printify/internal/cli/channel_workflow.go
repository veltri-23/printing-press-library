...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  printify-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  printify-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"catalog", "catalog-print-providers-json", "shops-json", "uploads-json"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  printify-pp-cli workflow status

  # Show status as JSON
  printify-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("printify-pp-cli")
			}
