...
// TestUpsertBatch_PopulatesConnectionJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed connection_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesConnectionJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "test-001", "shops_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-002", "shops_id": "test-parent-001"}`),
		json.RawMessage(`{"id": "test-003", "shops_id": "test-parent-001"}`),
	}
	if _, _, err := s.UpsertBatch("connection_json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "connection_json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "connection_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count connection_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("connection_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandConnectionJsonGeneric exercises
...
func TestUpsertBatch_TypedFailureDoesNotStrandConnectionJsonGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// Items deliberately omit "shops_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
	...
	stored, extractFailures, err := s.UpsertBatch("connection_json", items)
	if err != nil {
	...
	if typed != 0 {
		t.Fatalf("connection_json count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandOrdersGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("orders count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}

// TestUpsertBatch_PopulatesOrdersJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed orders_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesOrdersJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("orders_json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "orders_json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "orders_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count orders_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("orders_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandOrdersJsonGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandOrdersJsonGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("orders_json", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("orders_json count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandProductsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("products count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}

// TestUpsertBatch_PopulatesProductsJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed products_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesProductsJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("products_json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "products_json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "products_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count products_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("products_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandProductsJsonGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandProductsJsonGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("products_json", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("products_json count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandWebhooksGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("webhooks count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}

// TestUpsertBatch_PopulatesWebhooksJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed webhooks_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesWebhooksJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("webhooks_json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "webhooks_json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "webhooks_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count webhooks_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("webhooks_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

// TestUpsertBatch_TypedFailureDoesNotStrandWebhooksJsonGeneric exercises
// the savepoint isolation around the typed-table dispatch. The fixture omits
// the NOT NULL parent FK column so the typed insert fails; the savepoint
// rolls back only the typed projection. The generic resources row inserted
// just before must survive. Regression for issue #1392, where a single
// outer transaction caused typed-table failures to cascade and silently
// discard every successfully fetched API row.
func TestUpsertBatch_TypedFailureDoesNotStrandWebhooksJsonGeneric(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	stored, extractFailures, err := s.UpsertBatch("webhooks_json", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v (typed-table failure must not propagate)", err)
	}
	if stored != len(items) {
	...
	if typed != 0 {
		t.Fatalf("webhooks_json count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "shops_id")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandArchiveJsonGeneric(t *testing.T) {
	...
	// Items deliberately omit "uploads_id" so the typed NOT NULL
	// constraint fires.
	items := []json.RawMessage{
		json.RawMessage(`{"id": "orphan-001"}`),
		json.RawMessage(`{"id": "orphan-002"}`),
	...
	if typed != 0 {
		t.Fatalf("archive_json count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "uploads_id")
	}
}

// TestUpsertBatch_PopulatesUploadsJsonTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed uploads_json table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesUploadsJsonTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("uploads-json", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "uploads-json").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "uploads_json")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count uploads_json: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("uploads_json count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}
