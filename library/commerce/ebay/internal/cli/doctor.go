		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "ebay-pp-cli auth login --chrome"
				} else {
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".ebay.com"
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run ebay-pp-cli auth login --chrome"
	}
	var proof doctorBrowserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run ebay-pp-cli auth login --chrome"
	}
	if proof.APIName != "ebay" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != ".ebay.com" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/deals" {
...
func collectCacheReport(staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("ebay-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'ebay-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'ebay-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'ebay-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'ebay-pp-cli sync' to refresh."
	}
	return report
}
