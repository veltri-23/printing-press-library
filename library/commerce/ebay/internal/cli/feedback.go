...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ebay-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EBAY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EBAY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ebay-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ebay-pp-cli/feedback.jsonl.
When ` + "`EBAY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EBAY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
