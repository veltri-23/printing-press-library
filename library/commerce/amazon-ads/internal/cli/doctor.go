...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  amazon-ads-pp-cli doctor
  amazon-ads-pp-cli doctor --json
  amazon-ads-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				if cfg.RefreshToken == "" {
					...
					report["auth_hint"] = "run 'amazon-ads-pp-cli auth login --port 8085' after setting the Amazon redirect URL to " + defaultOAuthCallbackURL
					report["auth_key_url"] = "https://advertising.amazon.com/API/docs/en-us/guides/get-started/overview"
				} else {
					authConfigured = true
			...
			if os.Getenv("AMAZON_ADS_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "AMAZON_ADS_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "AMAZON_ADS_CLIENT_ID set during auth login")
			}
			if os.Getenv("AMAZON_ADS_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "AMAZON_ADS_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "AMAZON_ADS_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("AMAZON_ADS_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "AMAZON_ADS_REFRESH_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "AMAZON_ADS_REFRESH_TOKEN set during auth login")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.AmazonAdsApiClientId
				}
				recordAdditionalAuthEnv("AMAZON_ADS_API_CLIENT_ID", configuredValue)
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.AmazonAdsProfileId
				}
				recordAdditionalAuthEnv("AMAZON_ADS_PROFILE_ID", configuredValue)
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					// Step 2: Validate credentials with an authenticated probe. Use
					// the normal client auth path instead of cfg.AuthHeader() directly:
					// refresh-token configs often have no cached access token yet, and
					// doctor should verify that exchange just like real API commands do.
					hasCredentialMaterial := cfg.AuthHeader() != "" || cfg.RefreshToken != ""
					if hasCredentialMaterial {
						...
						authHeaders := map[string]string{"User-Agent": "amazon-ads-pp-cli"}
						verifyPath := "/v2/profiles"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'amazon-ads-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'amazon-ads-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'amazon-ads-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'amazon-ads-pp-cli sync' to refresh."
	}
	return report
}
