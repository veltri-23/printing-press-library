...
func newSdCreateBrandSafetyDenyListDomainsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create-brand-safety-deny-list-domains",
		Short:       "Creates one or more domains to add to a Brand Safety Deny List. The Brand Safety Deny List is at the advertiser level.",
		Example:     "  amazon-ads-pp-cli sd create-brand-safety-deny-list-domains",
		Annotations: map[string]string{"pp:endpoint": "sd.create-brand-safety-deny-list-domains", "pp:method": "POST", "pp:path": "/sd/brandSafety/deny"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if asyncJobID := ExtractJobID(data, "requestId"); asyncJobID != "" {
				_ = RecordJob(JobRow{
					JobID:          asyncJobID,
					Resource:       "sd",
					Endpoint:       "create-brand-safety-deny-list-domains",
					Status:         "submitted",
					StatusResource: "sd",
					StatusEndpoint: "get",
				})
				if flagWait {
					...
					final, werr := WaitForJob(ctx, c, "/sd/optimizationRules/{optimizationRuleId}", asyncJobID, WaitOptions{
