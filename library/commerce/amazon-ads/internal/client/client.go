...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "amazon-ads-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			if v := c.Config.AmazonAdsApiClientId; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export AMAZON_ADS_API_CLIENT_ID=<your-token>")
				}
				req.Header.Set("Amazon-Advertising-API-ClientId", v)
			}
			if v := c.Config.AmazonAdsProfileId; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export AMAZON_ADS_PROFILE_ID=<your-token>")
				}
				req.Header.Set("Amazon-Advertising-API-Scope", v)
			}
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "amazon-ads-pp-cli/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export AMAZON_ADS_CLIENT_ID=<your-token> or amazon-ads-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.AmazonAdsClientId)
		addCredential(c.Config.AmazonAdsClientSecret)
		addCredential(c.Config.AmazonAdsRefreshToken)
		addCredential(c.Config.AmazonAdsApiClientId)
	}
