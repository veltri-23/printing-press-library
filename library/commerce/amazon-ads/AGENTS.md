# Amazon Ads Printed CLI Agent Guide

This directory is a generated `amazon-ads-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
amazon-ads-pp-cli doctor --json
amazon-ads-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
amazon-ads-pp-cli which "<capability>" --json
amazon-ads-pp-cli <command> --help
```
...
```bash
amazon-ads-pp-cli <command> --agent
```
...
```bash
amazon-ads-pp-cli <command> --help
amazon-ads-pp-cli <command> --dry-run --agent
```
