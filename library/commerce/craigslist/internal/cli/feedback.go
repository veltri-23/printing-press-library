...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".craigslist-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CRAIGSLIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CRAIGSLIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "craigslist-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.craigslist-pp-cli/feedback.jsonl.
When ` + "`CRAIGSLIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CRAIGSLIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
