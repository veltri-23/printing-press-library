...
var readCommandResources = map[string][]string{
	"craigslist-pp-cli postings":        {"postings"},
	"craigslist-pp-cli postings list":   {"postings"},
	"craigslist-pp-cli postings get":    {"postings"},
	"craigslist-pp-cli postings search": {"postings"},
}
...
func cachePolicy() cliutil.Policy {
	...
	envOptOut := "CRAIGSLIST_NO_AUTO_REFRESH"
...
func autoRefreshIfStale(ctx context.Context, flags *rootFlags, resources []string) (meta cliutil.FreshnessMeta) {
	...
	if err := runAutoRefresh(refreshCtx, flags, db, resources); err != nil {
		fmt.Fprintf(os.Stderr, "warning: using stale craigslist-pp-cli cache (refresh failed: %v)\n", err)
