...
Exit codes & warnings:
  ...
  craigslist-pp-cli sync

  # Sync specific resources only
  craigslist-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  craigslist-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  craigslist-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  craigslist-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  craigslist-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/craigslist-pp-cli/data.db)")
