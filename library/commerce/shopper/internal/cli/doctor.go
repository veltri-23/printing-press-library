...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
  ...
  shopper-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("SHOPPER_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "SHOPPER_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SHOPPER_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "shopper-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'shopper-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'shopper-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	default:
		...
		report["hint"] = "Some resources are older than stale_after; run 'shopper-pp-cli sync' to refresh."
	}
	return report
}
