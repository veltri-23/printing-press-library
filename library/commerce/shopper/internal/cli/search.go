...
In local mode: searches locally synced data only.`,
  ...
  shopper-pp-cli search "error timeout"

  # Force local search only
  shopper-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  shopper-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  shopper-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /catalog/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/catalog/search", map[string]any{
					"query":    query,
					"brands":   []any{},
					"metadata": []any{},
					"types":    []any{},
