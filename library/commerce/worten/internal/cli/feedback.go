...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "worten-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WORTEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WORTEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/emmassist-co/worten-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/worten-pp-cli/feedback.jsonl.
When ` + "`WORTEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WORTEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  worten-pp-cli feedback list
  worten-pp-cli feedback list --limit 5
  worten-pp-cli feedback list --json`,
