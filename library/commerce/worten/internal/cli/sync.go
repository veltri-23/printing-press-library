...
Resource scoping:
  ...
  worten-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  worten-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  worten-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  worten-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  worten-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "worten-pp-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/worten-pp-cli/data.db)")
