# Worten Printed CLI Agent Guide

This directory is a generated `worten-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
worten-pp-cli doctor --json
worten-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
worten-pp-cli which "<capability>" --json
worten-pp-cli <command> --help
```
...
```bash
worten-pp-cli <command> --help
worten-pp-cli <command> --dry-run --agent
```
