...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cf-domain-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CF_DOMAIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CF_DOMAIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/cloud/cf-domain/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.github.com/mvanhorn/printing-press-library/library/cloud/cf-domain/feedback.jsonl.
When ` + "`CF_DOMAIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CF_DOMAIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cf-domain-pp-cli feedback list
  cf-domain-pp-cli feedback list --limit 5
  cf-domain-pp-cli feedback list --json`,
