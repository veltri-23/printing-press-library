...
Exit codes & warnings:
  ...
  cf-domain-pp-cli sync

  # Sync specific resources only
  cf-domain-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  cf-domain-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  cf-domain-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  cf-domain-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  cf-domain-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/mvanhorn/printing-press-library/library/cloud/cf-domain/data.db)")
