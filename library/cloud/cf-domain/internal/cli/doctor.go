...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cf-domain-pp-cli doctor
  cf-domain-pp-cli doctor --json
  cf-domain-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CLOUDFLARE_API_TOKEN=<your-token>"
				} else {
					authConfigured = true
			...
			if os.Getenv("CLOUDFLARE_API_TOKEN") != "" {
			...
			} else if os.Getenv("CLOUDFLARE_REGISTRAR_DOMAINS_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "CLOUDFLARE_REGISTRAR_DOMAINS_BEARER_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLOUDFLARE_API_TOKEN")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("cf-domain-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'cf-domain-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'cf-domain-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'cf-domain-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'cf-domain-pp-cli sync' to refresh."
	}
	return report
}
