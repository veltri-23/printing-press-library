# Cf Domain Printed CLI Agent Guide

This directory is a generated `cf-domain-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cf-domain-pp-cli doctor --json
cf-domain-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cf-domain-pp-cli which "<capability>" --json
cf-domain-pp-cli <command> --help
```
...
```bash
cf-domain-pp-cli <command> --agent
```
...
```bash
cf-domain-pp-cli <command> --help
cf-domain-pp-cli <command> --dry-run --agent
```
