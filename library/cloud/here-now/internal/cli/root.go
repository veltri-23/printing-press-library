...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "here-now-pp-cli",
		Short: `Here Now CLI — Publish a folder to a live URL in one command, mirror your Drives offline, and never lose an anonymous site to the 24-h…`,
		Long: `Here Now CLI — Publish a folder to a live URL in one command, mirror your Drives offline, and never lose an anonymous site to the 24-h…
...
Health check: run 'here-now-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("here-now-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'here-now-pp-cli profile list')")
	...
	// drives gains the novel sync/diff subcommands (the singular `drive` group
	// is eliminated — its sync/diff now live under the real plural resource).
	drivesCmd := newDrivesCmd(flags)
	drivesCmd.AddCommand(newNovelDriveSyncCmd(flags))
	drivesCmd.AddCommand(newNovelDriveDiffCmd(flags))
	rootCmd.AddCommand(drivesCmd)
	rootCmd.AddCommand(newHandleCmd(flags))
	rootCmd.AddCommand(newHereNowAuthCmd(flags))
	rootCmd.AddCommand(newLinksCmd(flags))
	rootCmd.AddCommand(newMeCmd(flags))
	rootCmd.AddCommand(newProfileResourceCmd(flags))
	// publish gains the novel `dir` command alongside the generated verbs and
	// the `resume` subcommand it already registers internally.
	publishCmd := newPublishCmd(flags)
	publishCmd.AddCommand(newNovelPublishDirCmd(flags))
	rootCmd.AddCommand(publishCmd)
	rootCmd.AddCommand(newPublishesCmd(flags))
	...
	// Novel top-level groups: each registered exactly once. `sites` parents
	// `sites stale`; `claims` parents `expiring` and `redeem`. The singular
	// `drive` group and the duplicate registrations from the generator are
	// gone (drive sync/diff now hang off `drives`; publish dir/resume off
	// `publish`).
	rootCmd.AddCommand(newNovelClaimsCmd(flags))
	rootCmd.AddCommand(newNovelSitesCmd(flags))
	rootCmd.AddCommand(newNovelUsageCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newHereNowAnalyticsPromotedCmd(flags))
