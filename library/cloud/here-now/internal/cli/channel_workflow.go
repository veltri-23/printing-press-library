...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  here-now-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  here-now-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"domains", "drives", "links", "me", "profile_resource", "publishes"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  here-now-pp-cli workflow status

  # Show status as JSON
  here-now-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("here-now-pp-cli")
			}
