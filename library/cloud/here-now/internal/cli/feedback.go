...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "here-now-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HERE_NOW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HERE_NOW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "here-now-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/here-now-pp-cli/feedback.jsonl.
When ` + "`HERE_NOW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HERE_NOW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  here-now-pp-cli feedback list
  here-now-pp-cli feedback list --limit 5
  here-now-pp-cli feedback list --json`,
