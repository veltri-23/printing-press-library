...
Resource scoping:
  ...
  here-now-pp-cli sync

  # Sync specific resources only
  here-now-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  here-now-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  here-now-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  here-now-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  here-now-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("here-now-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/here-now-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "drives":
		return db.UpsertDrives(data)
	case "files":
		return db.UpsertFiles(data)
	case "tokens":
		return db.UpsertTokens(data)
	case "handle":
		return db.UpsertHandle(data)
	case "here-now-analytics":
		return db.UpsertHereNowAnalytics(data)
	case "here-now-auth":
		return db.UpsertHereNowAuth(data)
	case "links":
		return db.UpsertLinks(data)
	case "profile_resource":
		return db.UpsertProfileResource(data)
	case "publish":
		return db.UpsertPublish(data)
	case "claim":
		return db.UpsertClaim(data)
	case "finalize":
		return db.UpsertFinalize(data)
	case "metadata":
		return db.UpsertMetadata(data)
	case "uploads":
		return db.UpsertUploads(data)
	case "publishes":
		return db.UpsertPublishes(data)
	case "analytics":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"links":            "/api/v1/links",
		"me":               "/api/v1/me/variables",
		"profile_resource": "/api/v1/profile/sites",
		"publishes":        "/api/v1/publishes",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "data", ParentTable: "publishes", ParentIDParam: "slug", PathTemplate: "/api/v1/publishes/{slug}/data/{collection}", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "slug", Field: "slug"},
			{Param: "collection", Field: "data_id"},
		}},
		{Name: "files", ParentTable: "drives", ParentIDParam: "driveId", PathTemplate: "/api/v1/drives/{driveId}/files", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "driveId", Field: "id"},
		}},
	}
}
