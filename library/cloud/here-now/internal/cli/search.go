...
In local mode: searches locally synced data only.`,
  ...
  here-now-pp-cli search "error timeout"

  # Force local search only
  here-now-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  here-now-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  here-now-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v1/publishes/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v1/publishes/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'here-now-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
