...
type Config struct {
	...
	Path          string            `toml:"-"`
	HerenowApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("HERE_NOW_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "here-now-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("HERENOW_API_KEY"); v != "" {
		cfg.HerenowApiKey = v
		cfg.AuthSource = "env:HERENOW_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.HerenowApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("HERE_NOW_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.HerenowApiKey != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:HERENOW_API_KEY"
		}
		return "Bearer " + c.HerenowApiKey
	}
	if c.AccessToken != "" {
