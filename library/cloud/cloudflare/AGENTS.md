# Cloudflare Printed CLI Agent Guide

This directory is a generated `cloudflare-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
cloudflare-pp-cli doctor --json
cloudflare-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
cloudflare-pp-cli which "<capability>" --json
cloudflare-pp-cli <command> --help
```
...
```bash
cloudflare-pp-cli <command> --agent
```
...
```bash
cloudflare-pp-cli <command> --help
cloudflare-pp-cli <command> --dry-run --agent
```
