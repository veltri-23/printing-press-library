...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cloudflare-pp-cli doctor
  cloudflare-pp-cli doctor --json
  cloudflare-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Run 'cloudflare-pp-cli auth setup' for credential setup steps."
					report["auth_key_url"] = "https://dash.cloudflare.com/profile/api-tokens"
					report["auth_instructions"] = "Create a scoped API token in the Cloudflare dashboard; agents should prefer workflow recipes and avoid global API keys."
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "CLOUDFLARE_API_TOKEN")
				if os.Getenv("CLOUDFLARE_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "CLOUDFLARE_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("CLOUDFLARE_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "CLOUDFLARE_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "CLOUDFLARE_API_TOKEN optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "cloudflare-pp-cli"
						verifyPath := "/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'cloudflare-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'cloudflare-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'cloudflare-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'cloudflare-pp-cli sync' to refresh."
	}
	return report
}
