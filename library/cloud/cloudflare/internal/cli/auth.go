...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Cloudflare",
		RunE:  parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newAuthSetupCmd(flags))
	cmd.AddCommand(newAuthStatusCmd(flags))
	cmd.AddCommand(newAuthDoctorCmd(flags))
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  cloudflare-pp-cli auth setup\n  cloudflare-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://dash.cloudflare.com/profile/api-tokens")
			fmt.Fprintln(w, "  Create a scoped API token in the Cloudflare dashboard; agents should prefer workflow recipes and avoid global API keys.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Optional request credentials:")
			fmt.Fprintln(w, "  export CLOUDFLARE_API_TOKEN=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://dash.cloudflare.com/profile/api-tokens"
...
func newAuthDoctorCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		Use:         "doctor",
		Short:       "Check Cloudflare API token reachability and token-management access",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			report := map[string]any{
				"auth_source": c.Config.AuthSource,
				"base_url":    c.BaseURL,
			}
			verifyStatus, verifyErr := c.ProbeGet(cmd.Context(), "/user/tokens/verify")
			report["verify_status"] = verifyStatus
			report["verify_ok"] = verifyErr == nil
			if verifyErr != nil {
				report["verify_error"] = verifyErr.Error()
			}
			groupsStatus, groupsErr := c.ProbeGet(cmd.Context(), "/user/tokens/permission_groups")
			report["permission_groups_status"] = groupsStatus
			...
			fmt.Fprintf(cmd.OutOrStdout(), "auth source: %s\nverify: %v (HTTP %d)\npermission groups: %v (HTTP %d)\n", c.Config.AuthSource, report["verify_ok"], verifyStatus, report["permission_groups_ok"], groupsStatus)
			return nil
		},
	}
}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  cloudflare-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  cloudflare-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  cloudflare-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
