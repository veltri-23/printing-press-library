...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "cloudflare-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLOUDFLARE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLOUDFLARE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cloudflare-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/cloudflare-pp-cli/feedback.jsonl.
When ` + "`CLOUDFLARE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLOUDFLARE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cloudflare-pp-cli feedback list
  cloudflare-pp-cli feedback list --limit 5
  cloudflare-pp-cli feedback list --json`,
