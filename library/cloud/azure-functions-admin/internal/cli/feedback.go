...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "azure-functions-admin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AZURE_FUNCTIONS_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AZURE_FUNCTIONS_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "azure-functions-admin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/azure-functions-admin-pp-cli/feedback.jsonl.
When ` + "`AZURE_FUNCTIONS_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AZURE_FUNCTIONS_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "azure-functions-admin-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  azure-functions-admin-pp-cli feedback list
  azure-functions-admin-pp-cli feedback list --limit 5
  azure-functions-admin-pp-cli feedback list --json`,
