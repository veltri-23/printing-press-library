...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  azure-functions-admin-pp-cli doctor
  azure-functions-admin-pp-cli doctor --json
  azure-functions-admin-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export AZURE_TENANT_ID=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("AZURE_TENANT_ID") != "" {
				authEnvSet = append(authEnvSet, "AZURE_TENANT_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AZURE_TENANT_ID")
			}
			if os.Getenv("AZURE_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "AZURE_CLIENT_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AZURE_CLIENT_ID")
			}
			if os.Getenv("AZURE_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "AZURE_CLIENT_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AZURE_CLIENT_SECRET")
			}
			if os.Getenv("AZURE_SUBSCRIPTION_ID") != "" {
				authEnvSet = append(authEnvSet, "AZURE_SUBSCRIPTION_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "AZURE_SUBSCRIPTION_ID")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/subscriptions?api-version=2022-12-01"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "azure-functions-admin-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("azure-functions-admin-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'azure-functions-admin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'azure-functions-admin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'azure-functions-admin-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'azure-functions-admin-pp-cli sync' to refresh."
	}
	return report
}
