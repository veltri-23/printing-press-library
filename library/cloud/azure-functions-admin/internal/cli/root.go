...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "azure-functions-admin-pp-cli",
		Short: `Azure Functions Admin CLI — Inspect, audit, and right-size your Azure Functions from the terminal — cold-start trends, config drift, and plan-fit a…`,
		Long: `Azure Functions Admin CLI — Inspect, audit, and right-size your Azure Functions from the terminal — cold-start trends, config drift, and plan-fit a…
...
Health check: run 'azure-functions-admin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("azure-functions-admin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'azure-functions-admin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelColdstartCmd(flags))
	rootCmd.AddCommand(newNovelDriftCmd(flags))
	rootCmd.AddCommand(newNovelFailuresCmd(flags))
	rootCmd.AddCommand(newNovelPlanFitCmd(flags))
	rootCmd.AddCommand(newNovelScalingCmd(flags))
	rootCmd.AddCommand(newNovelSecretsAuditCmd(flags))
	rootCmd.AddCommand(newNovelStaleCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newFunctionsPromotedCmd(flags))
	rootCmd.AddCommand(newPlansPromotedCmd(flags))
	rootCmd.AddCommand(newSettingsPromotedCmd(flags))
	rootCmd.AddCommand(newSlotsPromotedCmd(flags))
	rootCmd.AddCommand(newSubscriptionsPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// Azure ARM requires a live Azure AD bearer token, which the spec-derived
	// static auth cannot produce. When no explicit auth header is configured,
	// inject a real token from DefaultAzureCredential. Best-effort: on failure
	// (no creds) leave it empty so dry-run/doctor still work and live calls
	// surface a clear auth error. (Regen note: this edits generated newClient;
	// flag for review on any future regen-merge.)
	if cfg.AuthHeaderVal == "" {
		if bearer, tErr := azure.ARMBearer(context.Background()); tErr == nil {
			cfg.AuthHeaderVal = bearer
		}
	}
