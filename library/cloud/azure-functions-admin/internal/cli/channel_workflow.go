...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  azure-functions-admin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  azure-functions-admin-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  azure-functions-admin-pp-cli workflow status

  # Show status as JSON
  azure-functions-admin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("azure-functions-admin-pp-cli")
			}
