...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Azure Functions Admin",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  azure-functions-admin-pp-cli auth setup\n  azure-functions-admin-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export AZURE_TENANT_ID=\"<your-token>\"")
			fmt.Fprintln(w, "  export AZURE_CLIENT_ID=\"<your-token>\"")
			fmt.Fprintln(w, "  export AZURE_CLIENT_SECRET=\"<your-token>\"")
			fmt.Fprintln(w, "  export AZURE_SUBSCRIPTION_ID=\"<your-token>\"")
			fmt.Fprintln(w, "  azure-functions-admin-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  azure-functions-admin-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export AZURE_TENANT_ID=\"your-token-here\"")
				fmt.Fprintln(w, "  export AZURE_CLIENT_ID=\"your-token-here\"")
				fmt.Fprintln(w, "  export AZURE_CLIENT_SECRET=\"your-token-here\"")
				fmt.Fprintln(w, "  export AZURE_SUBSCRIPTION_ID=\"your-token-here\"")
				fmt.Fprintf(w, "  azure-functions-admin-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  azure-functions-admin-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  azure-functions-admin-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("AZURE_TENANT_ID") != "" {
				envStillSet = "AZURE_TENANT_ID"
			}
			if envStillSet == "" && os.Getenv("AZURE_CLIENT_ID") != "" {
				envStillSet = "AZURE_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("AZURE_CLIENT_SECRET") != "" {
				envStillSet = "AZURE_CLIENT_SECRET"
			}
			if envStillSet == "" && os.Getenv("AZURE_SUBSCRIPTION_ID") != "" {
				envStillSet = "AZURE_SUBSCRIPTION_ID"
			}
