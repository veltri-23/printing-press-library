...
Resource scoping:
  ...
  azure-functions-admin-pp-cli sync

  # Sync specific resources only
  azure-functions-admin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  azure-functions-admin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  azure-functions-admin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  azure-functions-admin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  azure-functions-admin-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/azure-functions-admin-pp-cli/data.db)")
