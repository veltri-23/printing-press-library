...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	result, err := handleColdStartCheckBeforeAPlanDecision(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesAndRejectsArgs(t *testing.T) {
	...
	got := recipeToolText(t, result)
	if !strings.Contains(got, "--app") || !strings.Contains(got, "required-value") {
		t.Fatalf("string override for --app missing from %q", got)
	}
}
