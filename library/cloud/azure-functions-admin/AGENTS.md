# Azure Functions Printed CLI Agent Guide

This directory is a generated `azure-functions-admin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
azure-functions-admin-pp-cli doctor --json
azure-functions-admin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
azure-functions-admin-pp-cli which "<capability>" --json
azure-functions-admin-pp-cli <command> --help
```
...
```bash
azure-functions-admin-pp-cli <command> --agent
```
...
```bash
azure-functions-admin-pp-cli <command> --help
azure-functions-admin-pp-cli <command> --dry-run --agent
```
