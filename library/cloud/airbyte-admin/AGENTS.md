# Airbyte Admin Printed CLI Agent Guide

This directory is a generated `airbyte-admin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
airbyte-admin-pp-cli doctor --json
airbyte-admin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airbyte-admin-pp-cli which "<capability>" --json
airbyte-admin-pp-cli <command> --help
```
...
```bash
airbyte-admin-pp-cli <command> --agent
```
...
```bash
airbyte-admin-pp-cli <command> --help
airbyte-admin-pp-cli <command> --dry-run --agent
```
