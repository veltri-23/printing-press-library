...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/cloud/airbyte-admin/internal/cliutil"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airbyte-admin-pp-cli doctor
  airbyte-admin-pp-cli doctor --json
  airbyte-admin-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.AuthHeader() != "" {
				...
				report["auth"] = fmt.Sprintf("configured (%s)", source)
			} else {
				report["auth"] = "optional (set AIRBYTE_ADMIN_TOKEN, AIRBYTE_ADMIN_AUTH_HEADER, or auth_header in config for protected Airbyte APIs)"
			}

			// Check auth environment variables
			report["env_vars"] = "optional: AIRBYTE_ADMIN_BASE_URL, AIRBYTE_ADMIN_TOKEN, AIRBYTE_ADMIN_AUTH_HEADER"
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "airbyte-admin-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("airbyte-admin-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'airbyte-admin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'airbyte-admin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'airbyte-admin-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'airbyte-admin-pp-cli sync' to refresh."
	}
	return report
}
