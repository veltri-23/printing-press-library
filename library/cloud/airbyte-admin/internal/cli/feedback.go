...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "airbyte-admin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRBYTE_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRBYTE_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airbyte-admin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/airbyte-admin-pp-cli/feedback.jsonl.
When ` + "`AIRBYTE_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRBYTE_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airbyte-admin-pp-cli feedback list
  airbyte-admin-pp-cli feedback list --limit 5
  airbyte-admin-pp-cli feedback list --json`,
