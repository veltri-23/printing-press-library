# Digitalocean Printed CLI Agent Guide

This directory is a generated `digitalocean-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
digitalocean-pp-cli doctor --json
digitalocean-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
digitalocean-pp-cli which "<capability>" --json
...
```bash
digitalocean-pp-cli <command> --agent
```
...
```bash
digitalocean-pp-cli <command> --help
digitalocean-pp-cli <command> --dry-run --agent
```
