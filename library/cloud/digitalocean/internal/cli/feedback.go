...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".digitalocean-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DIGITALOCEAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DIGITALOCEAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "digitalocean-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.digitalocean-pp-cli/feedback.jsonl.
When ` + "`DIGITALOCEAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DIGITALOCEAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  digitalocean-pp-cli feedback list
  digitalocean-pp-cli feedback list --limit 5
  digitalocean-pp-cli feedback list --json`,
