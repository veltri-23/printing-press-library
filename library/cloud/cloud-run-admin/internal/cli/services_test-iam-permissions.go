...
func newServicesTestIamPermissionsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&bodyPermissions, "permissions", "", "The set of permissions to check for the `resource`. Permissions with wildcards (such as `*` or `storage.*`) are not...")
