...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "cloud-run-admin-pp-cli",
		Short: `Cloud Run Admin CLI — A focused Cloud Run Admin API CLI with agent-native output, local inventory, and command discovery.`,
		Long: `Cloud Run Admin CLI — A focused Cloud Run Admin API CLI with agent-native output, local inventory, and command discovery.
...
Health check: run 'cloud-run-admin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'cloud-run-admin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCloudRunAdminJobsCmd(flags))
	rootCmd.AddCommand(newExecutionsCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("cloud-run-admin-pp-cli %s\n", version)
		},
	}
}
