...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cloud-run-admin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLOUD_RUN_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLOUD_RUN_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cloud-run-admin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cloud-run-admin-pp-cli/feedback.jsonl.
When ` + "`CLOUD_RUN_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLOUD_RUN_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cloud-run-admin-pp-cli feedback list
  cloud-run-admin-pp-cli feedback list --limit 5
  cloud-run-admin-pp-cli feedback list --json`,
