...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cloud-run-admin-pp-cli doctor
  cloud-run-admin-pp-cli doctor --json
  cloud-run-admin-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CLOUD_RUN_ADMIN_OAUTH2C=<your-key>"
				} else {
			...
			if os.Getenv("CLOUD_RUN_ADMIN_OAUTH2C") != "" {
				authEnvSet = append(authEnvSet, "CLOUD_RUN_ADMIN_OAUTH2C")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CLOUD_RUN_ADMIN_OAUTH2C")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("cloud-run-admin-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'cloud-run-admin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'cloud-run-admin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'cloud-run-admin-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'cloud-run-admin-pp-cli sync' to refresh."
	}
	return report
}
