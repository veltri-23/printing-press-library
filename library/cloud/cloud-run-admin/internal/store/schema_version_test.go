...
// TestMigrate_AddsColumnsOnUpgrade_CloudRunAdminJobs verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_CloudRunAdminJobs(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE cloud_run_admin_jobs (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(cloud_run_admin_jobs)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from cloud_run_admin_jobs after migrate", want)
		}
	}
}
