...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HT_ML_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HT_ML_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ht-ml-pp-cli/feedback")
...
When ` + "`HT_ML_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HT_ML_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ht-ml-pp-cli feedback list
  ht-ml-pp-cli feedback list --limit 5
  ht-ml-pp-cli feedback list --json`,
