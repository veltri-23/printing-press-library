...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ht-ml-pp-cli",
		Short: `Ht Ml CLI — The only ht-ml.app tool that remembers what you published, a local registry of every site, its once-only update_key, a…`,
		Long: `Ht Ml CLI — The only ht-ml.app tool that remembers what you published, a local registry of every site, its once-only update_key, a…
...
Health check: run 'ht-ml-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ht-ml-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ht-ml-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelKeysCmd(flags))
	rootCmd.AddCommand(newNovelListCmd(flags))
	rootCmd.AddCommand(newNovelRepublishCmd(flags))
	rootCmd.AddCommand(newNovelRollbackCmd(flags))
	rootCmd.AddCommand(newNovelScanCmd(flags))
	...
	rootCmd.AddCommand(newPasswordCmd(flags))
