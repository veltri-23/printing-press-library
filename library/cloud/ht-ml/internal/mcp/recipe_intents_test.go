...
func TestRecipeIntentHandlerBuildsRecipeArgs(t *testing.T) {
	...
	result, err := handleAuditEverySiteForBrokenImages(context.Background(), req)
	if err != nil {
...
func TestRecipeIntentHandlerOverridesParams(t *testing.T) {
	...
	if strings.Contains(got, "--missing-only") {
		t.Fatalf("boolean false override should suppress --missing-only: %q", got)
	}
}
