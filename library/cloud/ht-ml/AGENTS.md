# ht-ml.app Printed CLI Agent Guide

This directory is a generated `ht-ml-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ht-ml-pp-cli doctor --json
ht-ml-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ht-ml-pp-cli which "<capability>" --json
...
```bash
ht-ml-pp-cli <command> --help
ht-ml-pp-cli <command> --dry-run --agent
```
