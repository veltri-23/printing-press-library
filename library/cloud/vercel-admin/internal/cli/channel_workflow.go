...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  vercel-admin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  vercel-admin-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"access-groups", "aliases", "billing", "connect", "deployments", "domains", "edge-config", "env", "events", "integrations", "integrations-git-namespaces", "integrations-search-repo", "log-drains", "microfrontends", "observability", "projects", "registrar", "sandboxes", "sandboxes-drives", "sandboxes-sessions", "sandboxes-snapshots", "teams"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  vercel-admin-pp-cli workflow status

  # Show status as JSON
  vercel-admin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("vercel-admin-pp-cli")
			}
