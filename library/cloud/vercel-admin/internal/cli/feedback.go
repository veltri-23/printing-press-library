...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "vercel-admin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VERCEL_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VERCEL_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vercel-admin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/vercel-admin-pp-cli/feedback.jsonl.
When ` + "`VERCEL_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VERCEL_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vercel-admin-pp-cli feedback list
  vercel-admin-pp-cli feedback list --limit 5
  vercel-admin-pp-cli feedback list --json`,
