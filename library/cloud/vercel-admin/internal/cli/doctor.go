...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vercel-admin-pp-cli doctor
  vercel-admin-pp-cli doctor --json
  vercel-admin-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export VERCEL_ADMIN_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://vercel.com/support"
				} else {
					authConfigured = true
			...
			if os.Getenv("VERCEL_ADMIN_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "VERCEL_ADMIN_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "VERCEL_ADMIN_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "vercel-admin-pp-cli"
						verifyPath := "/v2/user"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'vercel-admin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'vercel-admin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'vercel-admin-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'vercel-admin-pp-cli sync' to refresh."
	}
	return report
}
