...
type Config struct {
	...
	VercelAdminToken string            `toml:"admin_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("VERCEL_ADMIN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "vercel-admin-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("VERCEL_ADMIN_TOKEN"); v != "" {
		cfg.VercelAdminToken = v
		cfg.markEnvOverride("VercelAdminToken")
		cfg.AuthSource = "env:VERCEL_ADMIN_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.VercelAdminToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("VERCEL_ADMIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.VercelAdminToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:VERCEL_ADMIN_TOKEN"
		}
		return "Bearer " + c.VercelAdminToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientSecret")
	c.VercelAdminToken = ""
	delete(c.envOverrides, "VercelAdminToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["VercelAdminToken"] {
			out.VercelAdminToken = c.fileConfig.VercelAdminToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "VercelAdminToken":
		c.fileConfig.VercelAdminToken = c.VercelAdminToken
	}
}
