...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'gohighlevel-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: gohighlevel-pp-cli auth set-token <token>"+
			"\n      or: export GHL_PIT_TOKEN=<your-token>"+
			"\n      Run 'gohighlevel-pp-cli doctor' to check auth status.", err))
	...
	for {
		...
		if json.Unmarshal(data, &items) == nil {
			...
			if json.Unmarshal(data, &obj) == nil {
				...
				// Check for next cursor (supports string or numeric cursors)
				if nextCursorPath != "" {
					if tokenRaw, ok := rawAtPath(obj, nextCursorPath); ok {
						...
						var tokenNum json.Number
						dec := json.NewDecoder(strings.NewReader(string(tokenRaw)))
						dec.UseNumber()
						if err := dec.Decode(&tokenNum); err == nil && tokenNum != "" {
							clean[cursorParam] = string(tokenNum)
							continue
						}
					}
				}
