...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Gohighlevel CLI — The terminal for GoHighLevel. Bulk ops, dedup, and pipeline reports in seconds — local cache, agent-native JSON, multi-…`,
		Long: `Gohighlevel CLI — The terminal for GoHighLevel. Bulk ops, dedup, and pipeline reports in seconds — local cache, agent-native JSON, multi-…
...
See README.md or the bundled SKILL.md for recipes.`,
	...
	rootCmd.AddCommand(newSurveysPromotedCmd(flags))
	rootCmd.AddCommand(newWorkflowsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-coded transcendence commands (regen-safe additions).
	rootCmd.AddCommand(newOppCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// GHL requires a `Version` HTTP header on every request; default to
	// 2021-07-28 when no config file overrides it. Conversations endpoints
	// want 2021-04-15 — that override is handled at the resource layer.
	if cfg.Headers == nil {
		cfg.Headers = map[string]string{}
	}
	if _, ok := cfg.Headers["Version"]; !ok {
		cfg.Headers["Version"] = "2021-07-28"
	}
