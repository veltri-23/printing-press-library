...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".gohighlevel-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GOHIGHLEVEL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GOHIGHLEVEL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/sales-and-crm/gohighlevel/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.gohighlevel-pp-cli/feedback.jsonl.
When ` + "`GOHIGHLEVEL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GOHIGHLEVEL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gohighlevel-pp-cli feedback list
  gohighlevel-pp-cli feedback list --limit 5
  gohighlevel-pp-cli feedback list --json`,
