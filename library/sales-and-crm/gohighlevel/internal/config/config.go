...
type Config struct {
	...
	Path          string            `toml:"-"`
	GhlPitToken   string            `toml:"pit_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GOHIGHLEVEL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gohighlevel-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GHL_PIT_TOKEN"); v != "" {
		cfg.GhlPitToken = v
		cfg.AuthSource = "env:GHL_PIT_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.GhlPitToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("GOHIGHLEVEL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GhlPitToken != "" {
		c.AuthSource = "env:GHL_PIT_TOKEN"
		return "Bearer " + c.GhlPitToken
	}
	if c.AccessToken != "" {
		c.AuthSource = "oauth2"
