# GoHighLevel Printed CLI Agent Guide

This directory is a generated `gohighlevel-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gohighlevel-pp-cli which "<capability>" --json
gohighlevel-pp-cli <command> --help
```
...
```bash
gohighlevel-pp-cli <command> --agent
```
...
```bash
gohighlevel-pp-cli <command> --help
gohighlevel-pp-cli <command> --dry-run --agent
```
