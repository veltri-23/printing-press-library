...
func newPhoneCallsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "calls-list",
		Short:       "List voice calls, paginated.",
		Example:     "  gorgias-pp-cli phone calls-list",
		Annotations: map[string]string{"pp:endpoint": "phone.calls-list", "pp:method": "GET", "pp:path": "/phone/voice-calls", "mcp:read-only": "true"},
	...
	cmd.Flags().StringVar(&flagTicketId, "ticket-id", "", "Filter calls to a single ticket.")
