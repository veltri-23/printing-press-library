...
// feedbackFilePath returns the XDG-state path for the feedback ledger
// (default ~/.local/state/gorgias-pp-cli/feedback.jsonl). $XDG_STATE_HOME
// overrides the parent dir when set. On first call, lazily migrates any
// legacy ~/.gorgias-pp-cli/feedback.jsonl from before the XDG fix so
// users don't lose entries when they upgrade.
func feedbackFilePath() (string, error) {
	dir := filepath.Join(xdgStateHome(), "gorgias-pp-cli")
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return "", fmt.Errorf("creating state dir: %w", err)
	}
	newPath := filepath.Join(dir, "feedback.jsonl")
	migrateLegacyDotfile("feedback.jsonl", newPath)
	return newPath, nil
}

// migrateLegacyDotfile moves a file from ~/.gorgias-pp-cli/<name> to newPath
// if the new file does not yet exist. Best-effort; failures are silent so a
// missing/unreadable legacy file never breaks the CLI.
func migrateLegacyDotfile(name, newPath string) {
	...
	legacy := filepath.Join(home, ".gorgias-pp-cli", name)
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GORGIAS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GORGIAS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gorgias-pp-cli/feedback")
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gorgias-pp-cli feedback list
  gorgias-pp-cli feedback list --limit 5
  gorgias-pp-cli feedback list --json`,
