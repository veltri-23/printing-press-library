...
func newGorgiasJobsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create",
		Short:       "Kick off an asynchronous Gorgias job.",
		Example:     "  gorgias-pp-cli gorgias-jobs create --type example-value",
		Annotations: map[string]string{"pp:endpoint": "gorgias-jobs.create", "pp:method": "POST", "pp:path": "/jobs"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if !stdinBody {
				if !cmd.Flags().Changed("params") && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "params")
				}
	...
	cmd.Flags().StringVar(&bodyType, "type", "", "The type of the job.")
