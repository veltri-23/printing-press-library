...
func newCustomersMergeCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  gorgias-pp-cli customers merge --source-id 123456789 --target-id 123456789",
		Annotations: map[string]string{"pp:endpoint": "customers.merge", "pp:method": "PUT", "pp:path": "/customers/merge"},
	...
	cmd.Flags().StringVar(&flagSourceId, "source-id", "", "The ID of the customer to merge (which will be merged into the targeted customer).")
	cmd.Flags().StringVar(&flagTargetId, "target-id", "", "The ID of the targeted customer (which will still be here after the merge).")
