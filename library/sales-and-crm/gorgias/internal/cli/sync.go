...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Pulls API data into a local SQLite mirror so subsequent reads (search, sql, analytics,\n" +
			"pm load/orphans/stale) run instantly and without rate-limit risk.\n\n" +
			"Default scope is the curated top-level resources (tickets, customers, messages, tags,\n" +
			"macros, users, teams, views, satisfaction-surveys, custom-fields, rules, integrations).\n" +
			"Use `--resources <a,b>` to scope; pass dependent names (e.g. `tickets,messages`) to\n" +
			...
			"`--max-pages` caps each resource for testing. `--strict` aborts the whole sync on the\n" +
			"first per-resource error rather than the default best-effort behavior.\n\n" +
			"The DB lives at $XDG_DATA_HOME/gorgias-pp-cli/data.db (default ~/.local/share/...);\n" +
			"override with `--db`.",
		Example: `  # Sync all resources
  gorgias-pp-cli sync

  # Sync specific resources only
  gorgias-pp-cli sync --resources tickets,messages

  # Full resync (ignore previous checkpoint)
  gorgias-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gorgias-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gorgias-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gorgias-pp-cli sync --latest-only`,
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"phone-voice-call-events":     "/phone/voice-call-events",
		"phone-voice-call-recordings": "/phone/voice-call-recordings",
		"rules":                       "/rules",
