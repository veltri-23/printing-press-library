...
func newOrphansCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "orphans",
		Short: "Find tickets missing key fields (assignee, team, priority, tags)",
		Example: `  # Find orphaned items
  gorgias-pp-cli orphans

  # Output as JSON
  gorgias-pp-cli orphans --json`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			checkFields := []struct {
				...
				{[]string{"$.assignee_user", "$.assigneeId", "$.assignee_id", "$.assignee"}, "assignee"},
				{[]string{"$.assignee_team", "$.team", "$.team_id"}, "team"},
				{[]string{"$.priority", "$.priorityId", "$.priority_id"}, "priority"},
				{[]string{"$.tags", "$.labels", "$.labelIds", "$.label_ids"}, "tags"},
			}
