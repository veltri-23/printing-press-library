...
func newViewsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create",
		Short:       "Create a saved view — a filtered ticket list (e.g.",
		Example:     "  gorgias-pp-cli views create --slug example-value",
		Annotations: map[string]string{"pp:endpoint": "views.create", "pp:method": "POST", "pp:path": "/views"},
