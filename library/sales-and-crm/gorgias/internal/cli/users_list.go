...
func newUsersListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List users.",
		Example:     "  gorgias-pp-cli users list",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flagEmail != "" {
				params["email"] = flagEmail
			}
			if flagSearch != "" {
				params["search"] = flagSearch
			}
			if flagAvailableFirst {
				params["available_first"] = "true"
			}
			data, prov, err := resolveRead(cmd.Context(), c, flags, "users", true, path, params, nil)
			if err != nil {
	...
	cmd.Flags().StringVar(&flagExternalId, "external-id", "", "Filter by external_id.")
	cmd.Flags().StringVar(&flagEmail, "email", "", "Filter by email.")
	cmd.Flags().StringVar(&flagSearch, "search", "", "Substring search across user names.")
	cmd.Flags().BoolVar(&flagAvailableFirst, "available-first", false, "Sort available users first.")
