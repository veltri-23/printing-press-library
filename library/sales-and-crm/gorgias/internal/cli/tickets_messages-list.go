...
func newTicketsMessagesListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "List all messages on ticket (`id`) in chronological order.",
		Example:     "  gorgias-pp-cli tickets messages-list 123456789",
		Annotations: map[string]string{"pp:endpoint": "tickets.messages-list", "pp:method": "GET", "pp:path": "/tickets/{id}/messages", "mcp:read-only": "true"},
	...
	cmd.Flags().StringVar(&flagTicketId, "ticket-id", "", "Filter to a single ticket (overrides the positional <ticket_id>).")
