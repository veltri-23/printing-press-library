...
func newTailCmd(flags *rootFlags) *cobra.Command {
  ...
  gorgias-pp-cli tail --interval 10s

  # Tail a specific resource
  gorgias-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering (events is Gorgias's audit log)
  gorgias-pp-cli tail events --interval 30s | jq 'select(.object_type == "Ticket")'`,
