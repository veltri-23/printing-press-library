...
func newRulesListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "list",
		Short:       "List all automation rules with their order, enabled state, and summary.",
		Example:     "  gorgias-pp-cli rules list",
		Annotations: map[string]string{"pp:endpoint": "rules.list", "pp:method": "GET", "pp:path": "/rules", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			data, prov, err := resolveRead(cmd.Context(), c, flags, "rules", true, path, params, nil)
			if err != nil {
