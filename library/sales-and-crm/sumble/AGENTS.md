# Sumble Printed CLI Agent Guide

This directory is a generated `sumble-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sumble-pp-cli doctor --json
sumble-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sumble-pp-cli which "<capability>" --json
sumble-pp-cli <command> --help
```
...
```bash
sumble-pp-cli <command> --help
sumble-pp-cli <command> --dry-run --agent
```
