...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".sumble-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUMBLE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUMBLE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sumble-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.sumble-pp-cli/feedback.jsonl.
When ` + "`SUMBLE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUMBLE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sumble-pp-cli feedback list
  sumble-pp-cli feedback list --limit 5
  sumble-pp-cli feedback list --json`,
