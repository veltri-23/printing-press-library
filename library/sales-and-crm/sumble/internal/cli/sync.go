...
Resource scoping:
  ...
  sumble-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sumble-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sumble-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sumble-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sumble-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/sumble-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "contact-lists":
		return db.UpsertContactLists(data)
	case "organization-lists":
		return db.UpsertOrganizationLists(data)
	case "organizations":
		return db.UpsertOrganizations(data)
	case "people":
		return db.UpsertPeople(data)
	case "postings":
		return db.UpsertPostings(data)
	case "technologies":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"organization-lists": "/organization-lists",
	}
