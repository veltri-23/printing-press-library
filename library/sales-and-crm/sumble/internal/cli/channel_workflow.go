...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  sumble-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  sumble-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"contact-lists", "organization-lists"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  sumble-pp-cli workflow status

  # Show status as JSON
  sumble-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("sumble-pp-cli")
			}
