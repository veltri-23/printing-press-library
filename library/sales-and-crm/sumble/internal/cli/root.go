...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sumble-pp-cli",
		Short: `Every Sumble v6 feature, plus cost estimates before billed calls, a running credit balance, a budget guard, and a local SQLite cache so you never pay twice.`,
		Long: `Every Sumble v6 feature, plus cost estimates before billed calls, a running credit balance, a budget guard, and a local SQLite cache so you never pay twice.
...
Health check: run 'sumble-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sumble-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sumble-pp-cli profile list')")
	...
	rootCmd.AddCommand(newContactListsCmd(flags))
	rootCmd.AddCommand(newOrganizationListsCmd(flags))
	rootCmd.AddCommand(newOrganizationsCmd(flags))
	rootCmd.AddCommand(newPeopleCmd(flags))
	rootCmd.AddCommand(newPostingsCmd(flags))
	...
	// Credit-economy commands (hand-authored novel features).
	rootCmd.AddCommand(newCostEstimateCmd(flags))
	...
	rootCmd.AddCommand(newStackDiffCmd(flags))
