...
func TestUpsertBatch_TypedFailureDoesNotStrandThreadsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("threads count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "conversations_id")
	}
}

// TestUpsertBatch_KeysThreadsByChildAndParent verifies dependent
...
func TestUpsertBatch_KeysThreadsByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("threads count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "conversations_id" = ?`, "threads")
...
func TestUpsertBatch_SetsThreadsParentID(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "child-001", "conversations_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-002", "conversations_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-003", "conversations_id": "parent-B", "parent_id": "parent-B"}`),
	}
	if _, _, err := s.UpsertBatch("threads", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
...
// TestUpsertBatch_ThreadsSearchableViaGenericFTS verifies that a
...
// non-FTS5 `--type threads` fallback), so populating it is the
...
func TestUpsertBatch_ThreadsSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// field is matchable regardless of which columns threads declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "conversations_id": "parent-A", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("threads", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "threads")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandFieldsGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("fields count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "mailboxes_id")
	}
}
...
func TestUpsertBatch_KeysFieldsByChildAndParent(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "shared_child", "mailboxes_id": "parent_A"}`),
		json.RawMessage(`{"id": "shared", "mailboxes_id": "child_parent_A"}`),
		json.RawMessage(`{"id": "shared_child", "mailboxes_id": "parent_B"}`),
		json.RawMessage(`{"id": "other_child", "mailboxes_id": "parent_A"}`),
	}
	stored, extractFailures, err := s.UpsertBatch("fields", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	...
	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "mailboxes_id" = ?`, "fields")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by mailboxes_id: %v", err)
	}
	if parentMatches != 2 {
		t.Fatalf("mailboxes_id=parent_A count = %d, want 2", parentMatches)
	}
}
...
func TestUpsertBatch_SetsFieldsParentID(t *testing.T) {
	...
	items := []json.RawMessage{
		json.RawMessage(`{"id": "child-001", "mailboxes_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-002", "mailboxes_id": "parent-A", "parent_id": "parent-A"}`),
		json.RawMessage(`{"id": "child-003", "mailboxes_id": "parent-B", "parent_id": "parent-B"}`),
	}
	if _, _, err := s.UpsertBatch("fields", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
...
// TestUpsertBatch_FieldsSearchableViaGenericFTS verifies that a
// dependent-resource row stored via UpsertBatch — the exact path dependent
// sync uses (upsertResourceBatch -> UpsertBatch) — is reachable through the
// generic resources_fts index via Store.Search. That index is the substrate
// both `search` surfaces query (the empty-type search directly, and the
// non-FTS5 `--type fields` fallback), so populating it is the
// precondition issue #2629 reported missing — dependent resources stored in
// the typed table but absent from any FTS index. This asserts the store layer
// where the claimed defect lived; it does not exercise the CLI search wiring.
func TestUpsertBatch_FieldsSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// resources_fts indexes the full JSON blob, so a distinctive token in any
	// field is matchable regardless of which columns fields declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "mailboxes_id": "parent-A", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("fields", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	// Assert the probe row actually landed before searching for it: a PK
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "fields")
	}
}
...
func TestUpsertBatch_TypedFailureDoesNotStrandFoldersGeneric(t *testing.T) {
	...
	if typed != 0 {
		t.Fatalf("folders count = %d, want 0 (typed insert violated NOT NULL on %q)", typed, "mailboxes_id")
	}
}

// TestUpsertBatch_KeysFoldersByChildAndParent verifies dependent
// sub-collection rows with the same child id but different parent ids do not
// overwrite one another in either the generic resources table or the typed
// table. Regression for issue #2471.
func TestUpsertBatch_KeysFoldersByChildAndParent(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if typed != len(items) {
		t.Fatalf("folders count = %d, want %d (typed rows collapsed on child id)", typed, len(items))
	}

	var parentMatches int
	parentQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s" WHERE "mailboxes_id" = ?`, "folders")
	if err := db.QueryRow(parentQuery, "parent_A").Scan(&parentMatches); err != nil {
		t.Fatalf("count by mailboxes_id: %v", err)
	}
	if parentMatches != 2 {
...
// TestUpsertBatch_FoldersSearchableViaGenericFTS verifies that a
// dependent-resource row stored via UpsertBatch — the exact path dependent
// sync uses (upsertResourceBatch -> UpsertBatch) — is reachable through the
// generic resources_fts index via Store.Search. That index is the substrate
// both `search` surfaces query (the empty-type search directly, and the
// non-FTS5 `--type folders` fallback), so populating it is the
// precondition issue #2629 reported missing — dependent resources stored in
// the typed table but absent from any FTS index. This asserts the store layer
// where the claimed defect lived; it does not exercise the CLI search wiring.
func TestUpsertBatch_FoldersSearchableViaGenericFTS(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	// resources_fts indexes the full JSON blob, so a distinctive token in any
	// field is matchable regardless of which columns folders declares.
	const probe = "zqxjpp2629probe"
	items := []json.RawMessage{
		json.RawMessage(`{"id": "search-001", "pp_fts_probe": "` + probe + `", "mailboxes_id": "parent-A", "parent_id": "parent-A"}`),
	}
	stored, _, err := s.UpsertBatch("folders", items)
	if err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	// Assert the probe row actually landed before searching for it: a PK
	...
	if len(results) == 0 {
		t.Fatalf("dependent resource %q stored but not searchable via generic resources_fts (issue #2629)", "folders")
	}
}
