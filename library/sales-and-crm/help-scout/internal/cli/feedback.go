...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "help-scout-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HELP_SCOUT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HELP_SCOUT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/sales-and-crm/help-scout/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/github.com/mvanhorn/printing-press-library/library/sales-and-crm/help-scout/feedback.jsonl.
When ` + "`HELP_SCOUT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HELP_SCOUT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  help-scout-pp-cli feedback list
  help-scout-pp-cli feedback list --limit 5
  help-scout-pp-cli feedback list --json`,
