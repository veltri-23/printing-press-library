// Copyright 2026 Deb Mukherjee and contributors. Licensed under Apache-2.0. See LICENSE.
// Generated by CLI Printing Press (https://github.com/mvanhorn/cli-printing-press). DO NOT EDIT.

package cli

import (
	"github.com/spf13/cobra"
)

func newMailboxesCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:         "mailboxes",
		Short:       "List and get mailboxes",
		Hidden:      true,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE:        parentNoSubcommandRunE(flags),
	}

	cmd.AddCommand(newMailboxesGetMailboxCmd(flags))
	cmd.AddCommand(newMailboxesListCmd(flags))
	cmd.AddCommand(newMailboxesFieldsCmd(flags))
	cmd.AddCommand(newMailboxesFoldersCmd(flags))
	return cmd
}
