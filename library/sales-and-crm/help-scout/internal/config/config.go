...
type Config struct {
	...
	HelpScoutBearerAuth string            `toml:"scout_bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("HELP_SCOUT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "help-scout-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("HELP_SCOUT_BEARER_AUTH"); v != "" {
		cfg.HelpScoutBearerAuth = v
		cfg.markEnvOverride("HelpScoutBearerAuth")
		cfg.AuthSource = "env:HELP_SCOUT_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.HelpScoutBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("HELP_SCOUT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.HelpScoutBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:HELP_SCOUT_BEARER_AUTH"
		}
		return "Bearer " + c.HelpScoutBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.HelpScoutBearerAuth = ""
	delete(c.envOverrides, "HelpScoutBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["HelpScoutBearerAuth"] {
			out.HelpScoutBearerAuth = c.fileConfig.HelpScoutBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "HelpScoutBearerAuth":
		c.fileConfig.HelpScoutBearerAuth = c.HelpScoutBearerAuth
	}
}
