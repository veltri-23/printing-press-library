...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  copper-pp-cli doctor
  copper-pp-cli doctor --json
  copper-pp-cli doctor --fail-on warn
  copper-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set your API key with: export COPPER_API_KEY=\"your-token-here\""
				} else {
					authConfigured = true
			...
			if os.Getenv("COPPER_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "COPPER_API_KEY")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "COPPER_API_KEY")
			}
			{
				configuredValue := ""
				if cfg != nil {
					configuredValue = cfg.CopperUserEmail
				}
				recordAdditionalAuthEnv("COPPER_USER_EMAIL", configuredValue)
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["X-PW-Application"] = "developer_api"
						authHeaders["User-Agent"] = "copper-pp-cli"
						verifyPath := "/account"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'copper-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'copper-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'copper-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'copper-pp-cli sync' to refresh."
	}
	return report
}
