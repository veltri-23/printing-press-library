...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  copper-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  copper-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activities", "activity-types", "companies", "contact-types", "custom-activity-types", "custom-field-definitions", "customer-sources", "lead-statuses", "leads", "loss-reasons", "opportunities", "people", "pipeline-stages", "pipelines", "projects", "tags", "tasks", "users", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  copper-pp-cli workflow status

  # Show status as JSON
  copper-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("copper-pp-cli")
			}
