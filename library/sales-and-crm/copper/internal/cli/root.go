...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "copper-pp-cli",
		Short: `Copper CLI — The Copper CRM command line no one else built: full CRUD plus a local database, weighted pipeline forecasting, stale-de…`,
		Long: `Copper CLI — The Copper CRM command line no one else built: full CRUD plus a local database, weighted pipeline forecasting, stale-de…
...
Health check: run 'copper-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("copper-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'copper-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCustomActivityTypesCmd(flags))
	rootCmd.AddCommand(newCustomFieldDefinitionsCmd(flags))
	...
	rootCmd.AddCommand(newNovelDedupeCmd(flags))
	...
	rootCmd.AddCommand(newNovelUpsertCmd(flags))
	rootCmd.AddCommand(newNovelWhoCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAccountPromotedCmd(flags))
	rootCmd.AddCommand(newActivityTypesPromotedCmd(flags))
	rootCmd.AddCommand(newContactTypesPromotedCmd(flags))
	rootCmd.AddCommand(newCustomerSourcesPromotedCmd(flags))
	rootCmd.AddCommand(newLeadStatusesPromotedCmd(flags))
	rootCmd.AddCommand(newLossReasonsPromotedCmd(flags))
	rootCmd.AddCommand(newPipelineStagesPromotedCmd(flags))
