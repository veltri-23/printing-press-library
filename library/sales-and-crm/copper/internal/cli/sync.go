...
Resource scoping:
  ...
  copper-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  copper-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  copper-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  copper-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  copper-pp-cli sync --latest-only`,
...
func syncResourceBodyParamTypes(resource string) map[string]string {
	...
	case "activities":
		return map[string]string{
			...
			"parent":                "string",
			"minimum_activity_date": "int",
			"maximum_activity_date": "int",
		}
	case "companies":
		return map[string]string{
			"page_number":    "int",
	...
	case "opportunities":
		return map[string]string{
			...
			"tags":                     "string",
			"followed":                 "int",
			"minimum_monetary_value":   "float",
			"maximum_monetary_value":   "float",
			"minimum_close_date":       "string",
			"maximum_close_date":       "string",
			"minimum_interaction_date": "int",
			"maximum_interaction_date": "int",
			"minimum_modified_date":    "int",
			"maximum_modified_date":    "int",
		}
	case "people":
		return map[string]string{
			"page_number":    "int",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"custom-activity-types":    "/custom_activity_types",
		"custom-field-definitions": "/custom_field_definitions",
