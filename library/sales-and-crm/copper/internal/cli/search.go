...
In local mode: searches locally synced data only.`,
  ...
  copper-pp-cli search "error timeout"

  # Force local search only
  copper-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  copper-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  copper-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /tasks/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/tasks/search", map[string]any{
					"q":              query,
					"page_number":    1,
					"page_size":      20,
					"sort_by":        "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'copper-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "people":
				results, err = db.SearchPeople(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchPeople(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search people failed: %w", searchErr)
					}
