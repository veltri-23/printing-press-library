...
func newWebhooksCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example:     "  copper-pp-cli webhooks create --target example-value",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if bodyEvent != "" {
					var parsedEvent any
					if err := json.Unmarshal([]byte(bodyEvent), &parsedEvent); err != nil {
	...
	cmd.Flags().StringVar(&bodyTarget, "target", "", "Target URL to receive notifications")
	cmd.Flags().StringVar(&bodyType, "type", "", "Entity type to subscribe to (lead, person, company, opportunity, project, task)")
	cmd.Flags().StringVar(&bodyEvent, "event", "", "JSON array of events (new, update, delete) or a single event string")
	cmd.Flags().StringVar(&bodySecret, "secret", "", "JSON secret object for signed payloads")
