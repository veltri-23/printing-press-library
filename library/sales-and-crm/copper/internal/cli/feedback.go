...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COPPER_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COPPER_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "copper-pp-cli/feedback")
...
When ` + "`COPPER_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COPPER_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  copper-pp-cli feedback list
  copper-pp-cli feedback list --limit 5
  copper-pp-cli feedback list --json`,
