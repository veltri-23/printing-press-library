	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(req.Context()); err == nil && h != "" {
				req.Header.Set("X-PW-AccessToken", h)
			}
		} else {
			// Cross-host hop: Go strips standard auth headers (Authorization,
			// Cookie) but not custom ones, so custom credential headers would
			// be forwarded verbatim to the redirect target. Delete every
			// credential header explicitly — both the access token and the
			// user-email header are credentials that must not cross hosts.
			req.Header.Del("X-PW-AccessToken")
			req.Header.Del("X-PW-UserEmail")
		}
		return nil
	}
	return c
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if authHeader != "" {
			req.Header.Set("X-PW-AccessToken", authHeader)
		}
		...
		if c.Config != nil {
			if v := c.Config.CopperUserEmail; v != "" {
				if authHeaderLooksLikePlaceholderCredential(v) {
					return nil, 0, authPlaceholderCredentialErrorWithSetup(c.Config, "export COPPER_USER_EMAIL=<your-token>")
				}
				req.Header.Set("X-PW-UserEmail", v)
			}
		}
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "copper-pp-cli/0.1.0")
		}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "X-PW-AccessToken", maskToken(authHeader))
	}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export COPPER_API_KEY=<your-token> or copper-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.CopperApiKey)
	}
