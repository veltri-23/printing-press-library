...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".eu-tenders-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EU_TENDERS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EU_TENDERS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "eu-tenders-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.eu-tenders-pp-cli/feedback.jsonl.
When ` + "`EU_TENDERS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EU_TENDERS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  eu-tenders-pp-cli feedback list
  eu-tenders-pp-cli feedback list --limit 5
  eu-tenders-pp-cli feedback list --json`,
