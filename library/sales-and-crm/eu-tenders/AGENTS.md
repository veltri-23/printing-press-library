# Eu Tenders Printed CLI Agent Guide

This directory is a generated `eu-tenders-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
eu-tenders-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
eu-tenders-pp-cli which "<capability>" --json
eu-tenders-pp-cli <command> --help
```
...
```bash
eu-tenders-pp-cli <command> --agent
```
...
```bash
eu-tenders-pp-cli <command> --help
eu-tenders-pp-cli <command> --dry-run --agent
```
