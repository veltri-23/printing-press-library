...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".conduyt-crm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CONDUYT_CRM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CONDUYT_CRM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "conduyt-crm-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.conduyt-crm-pp-cli/feedback.jsonl.
When ` + "`CONDUYT_CRM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CONDUYT_CRM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  conduyt-crm-pp-cli feedback list
  conduyt-crm-pp-cli feedback list --limit 5
  conduyt-crm-pp-cli feedback list --json`,
