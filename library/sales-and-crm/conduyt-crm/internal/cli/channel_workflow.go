...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  conduyt-crm-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  conduyt-crm-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activities", "admin", "api-keys", "appointments", "automation-executions", "automations", "automations-actions", "automations-condition-fields", "automations-events", "automations-templates", "booking-pages", "calendar", "calendar-callback-google", "calendar-callback-microsoft", "calendar-connections", "calendars", "calls", "chat", "companies", "conduyt-auth", "contacts", "contacts-duplicates", "contacts-smart-views", "conversations", "custom-fields", "deals", "document-templates", "drip-campaigns", "drip-enrollments", "emails", "emails-sequences", "emails-templates", "files", "forms", "imports", "integrations", "integrations-zapier-subscribe", "invoices", "messages", "notes", "notifications", "pipelines", "products", "reports", "scoring-rules", "smart-lists", "tags", "tasks", "users", "webhook-logs", "webhooks", "webhooks-manage", "workflows"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  conduyt-crm-pp-cli workflow status

  # Show status as JSON
  conduyt-crm-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("conduyt-crm-pp-cli")
			}
