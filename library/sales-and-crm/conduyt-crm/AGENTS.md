# Conduyt Crm Printed CLI Agent Guide

This directory is a generated `conduyt-crm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
conduyt-crm-pp-cli doctor --json
...
```bash
conduyt-crm-pp-cli which "<capability>" --json
...
```bash
conduyt-crm-pp-cli <command> --agent
```
...
```bash
conduyt-crm-pp-cli <command> --help
conduyt-crm-pp-cli <command> --dry-run --agent
```
