...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".servicetitan-pricebook-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SERVICETITAN_PRICEBOOK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SERVICETITAN_PRICEBOOK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "servicetitan-pricebook-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.servicetitan-pricebook-pp-cli/feedback.jsonl.
When ` + "`SERVICETITAN_PRICEBOOK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SERVICETITAN_PRICEBOOK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "servicetitan-pricebook-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-pricebook-pp-cli feedback list
  servicetitan-pricebook-pp-cli feedback list --limit 5
  servicetitan-pricebook-pp-cli feedback list --json`,
