...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Pricebook",
	}
...
Mint an OAuth2 bearer token via POST https://auth.servicetitan.io/connect/token with
grant_type=client_credentials and cache it on disk. Subsequent commands
auto-refresh the token before expiry.

Credentials default to ST_CLIENT_ID (Client ID) and ST_CLIENT_SECRET (Client Secret) environment variables.
`),
		Example: strings.Trim(`
  # Use env vars
  servicetitan-pricebook-pp-cli auth login

  # Explicit credentials
  servicetitan-pricebook-pp-cli auth login --client-id <id> --client-secret <secret>
`, "\n"),
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if clientID == "" {
				clientID = os.Getenv("ST_CLIENT_ID")
			}
			if clientSecret == "" {
				clientSecret = os.Getenv("ST_CLIENT_SECRET")
			}
			if clientID == "" || clientSecret == "" {
				return authErr(fmt.Errorf("client ID and secret required (set --client-id/--client-secret or ST_CLIENT_ID/ST_CLIENT_SECRET)"))
			}
	...
	cmd.Flags().StringVar(&clientID, "client-id", "", "OAuth2 Client ID; defaults to $ST_CLIENT_ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", "", "OAuth2 Client Secret; defaults to $ST_CLIENT_SECRET")
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  servicetitan-pricebook-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export ST_CLIENT_ID=\"<client-id>\"")
				fmt.Fprintln(w, "  export ST_CLIENT_SECRET=\"<client-secret>\"")
				fmt.Fprintf(w, "  servicetitan-pricebook-pp-cli auth login\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  servicetitan-pricebook-pp-cli auth set-token <bearer-jwt>",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  servicetitan-pricebook-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("ST_CLIENT_ID") != "" {
				envStillSet = "ST_CLIENT_ID"
			}
			if envStillSet == "" && os.Getenv("ST_CLIENT_SECRET") != "" {
				envStillSet = "ST_CLIENT_SECRET"
			}
