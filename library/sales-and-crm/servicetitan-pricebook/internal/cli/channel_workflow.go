...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  servicetitan-pricebook-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  servicetitan-pricebook-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  servicetitan-pricebook-pp-cli workflow status

  # Show status as JSON
  servicetitan-pricebook-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("servicetitan-pricebook-pp-cli")
			}
