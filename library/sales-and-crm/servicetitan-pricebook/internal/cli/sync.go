...
Exit codes & warnings:
  ...
  servicetitan-pricebook-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  servicetitan-pricebook-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  servicetitan-pricebook-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  servicetitan-pricebook-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  servicetitan-pricebook-pp-cli sync --latest-only`,
...
func defaultSyncResources() []string {
	// Resource names MUST match the resourceType strings the generated
	// get-list commands pass to resolvePaginatedRead, so `sync` and the
	// per-resource list/cache commands and the novel audit commands all
	// read/write the same store keys.
	return []string{
		"categories",
		"equipment",
		"materials",
...
// ST_TENANT_ID (the spec's info.x-tenant-env-var) since flat-list sync has
// no parent context to fill it from.
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"categories":            "/tenant/{tenant}/categories",
		"equipment":             "/tenant/{tenant}/equipment",
		"materials":             "/tenant/{tenant}/materials",
		"services":              "/tenant/{tenant}/services",
		"discounts-and-fees":    "/tenant/{tenant}/discounts-and-fees",
		"materialsmarkup":       "/tenant/{tenant}/materialsmarkup",
		"clientspecificpricing": "/tenant/{tenant}/clientspecificpricing",
	}
	p, ok := paths[resource]
	if !ok {
		return "", fmt.Errorf("unknown sync resource %q", resource)
	}
	if tenant := strings.TrimSpace(os.Getenv("ST_TENANT_ID")); tenant != "" {
		p = strings.ReplaceAll(p, "{tenant}", tenant)
	}
	return p, nil
}
