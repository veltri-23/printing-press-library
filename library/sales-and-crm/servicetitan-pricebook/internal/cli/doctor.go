...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  servicetitan-pricebook-pp-cli doctor
  servicetitan-pricebook-pp-cli doctor --json
  servicetitan-pricebook-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// credentials actually present (ST-App-Key + client_id/secret),
			// not from a not-yet-minted bearer.
			authConfigured := false
			if cfg != nil {
				header := cfg.AuthHeader()
				stAppKey := cfg.StAppKey
				clientID := cfg.ClientID
				clientSecret := cfg.ClientSecret
				composedReady := stAppKey != "" && clientID != "" && clientSecret != ""
				switch {
				case header != "":
					authConfigured = true
				...
				case composedReady:
					authConfigured = true
					report["auth"] = "configured (composed: ST-App-Key + OAuth2 client_credentials; bearer minted on first call)"
					if cfg.AuthSource != "" {
						report["auth_source"] = cfg.AuthSource
					}
				default:
					...
					missing := []string{}
					if stAppKey == "" {
						missing = append(missing, "ST_APP_KEY")
					}
					if clientID == "" {
						missing = append(missing, "ST_CLIENT_ID")
					}
					if clientSecret == "" {
						missing = append(missing, "ST_CLIENT_SECRET")
					}
					report["auth_hint"] = "export " + strings.Join(missing, " ") + " (ServiceTitan composed auth needs all three)"
				}
			}
			...
			if os.Getenv("ST_APP_KEY") != "" {
				authEnvSet = append(authEnvSet, "ST_APP_KEY")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ST_APP_KEY")
			}
			if os.Getenv("ST_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "ST_CLIENT_ID")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ST_CLIENT_ID")
			}
			if os.Getenv("ST_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "ST_CLIENT_SECRET")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ST_CLIENT_SECRET")
			}
			if os.Getenv("ST_TENANT_ID") != "" {
				authEnvSet = append(authEnvSet, "ST_TENANT_ID")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ST_TENANT_ID")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'servicetitan-pricebook-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'servicetitan-pricebook-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'servicetitan-pricebook-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'servicetitan-pricebook-pp-cli sync' to refresh."
	}
	return report
}
