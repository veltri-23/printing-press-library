...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "servicetitan-pricebook-pp-cli", "http")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// endpoint returns 401 even with a valid token.
		if c.Config != nil && c.Config.StAppKey != "" {
			req.Header.Set("ST-App-Key", c.Config.StAppKey)
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "servicetitan-pricebook-pp-cli/1.0")
		}
...
func resolveClientCredentials(cfg *config.Config) (string, string) {
	...
	if id == "" {
		id = os.Getenv("ST_CLIENT_ID")
	}
	if secret == "" {
		secret = os.Getenv("ST_CLIENT_SECRET")
	}
	return id, secret
}
