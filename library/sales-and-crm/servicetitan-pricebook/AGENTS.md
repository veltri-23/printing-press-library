# ServiceTitan Pricebook Printed CLI Agent Guide

This directory is a generated `servicetitan-pricebook-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
servicetitan-pricebook-pp-cli doctor --json
servicetitan-pricebook-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
servicetitan-pricebook-pp-cli which "<capability>" --json
...
```bash
servicetitan-pricebook-pp-cli <command> --agent
```
...
```bash
servicetitan-pricebook-pp-cli <command> --help
servicetitan-pricebook-pp-cli <command> --dry-run --agent
```
