...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  intercom-pp-cli doctor
  intercom-pp-cli doctor --json
  intercom-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				...
				// PATCH(intercom-region): apply --region flag override so doctor
				// reports the URL real commands will actually use. Mirrors the
				// same override in (*rootFlags).newClient().
				if flags.region != "" {
					if url, ok := config.RegionToBaseURL(flags.region); ok {
						cfg.BaseURL = url
					}
				}
				report["config"] = "ok"
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export INTERCOM_ACCESS_TOKEN=<your-key>"
					report["auth_docs_url"] = "https://developers.intercom.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("INTERCOM_ACCESS_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "INTERCOM_ACCESS_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "INTERCOM_ACCESS_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "intercom-pp-cli"
						verifyPath := "/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'intercom-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'intercom-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'intercom-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'intercom-pp-cli sync' to refresh."
	}
	return report
}
