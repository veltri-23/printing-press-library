...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'intercom-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your token. Set it with: intercom-pp-cli auth set-token <token>"+
			"\n      or: export INTERCOM_ACCESS_TOKEN=<your-token>"+
			"\n      See API docs: https://developers.intercom.com"+
			"\n      Run 'intercom-pp-cli doctor' to check auth status.", err))
...
func printOutput(w io.Writer, data json.RawMessage, asJSON bool) error {
	...
	if asJSON {
		// PATCH(json-string-control-bytes): some APIs (Intercom is one)
		// emit raw control bytes inside string values. Strict JSON parsers
		// (jq, Python's strict mode, RFC-compliant readers) reject the
		// response with "control characters from U+0000 through U+001F must
		// be escaped". Sanitize before write so downstream agents that pipe
		// our stdout through jq don't break on legitimate responses.
		sanitized := escapeJSONStringControlBytes(data)
		enc := json.NewEncoder(w)
		enc.SetIndent("", "  ")
		return enc.Encode(json.RawMessage(sanitized))
	}
...
// escapeJSONStringControlBytes walks JSON bytes and escapes any raw control
// characters (U+0000 through U+001F) that appear inside a string literal,
// leaving structural punctuation and inter-token whitespace untouched. Some
// APIs (Intercom) emit such control bytes verbatim inside string values,
// which strict JSON parsers like jq reject. This pass produces bytes safe
// for any RFC-compliant downstream parser.
//
// PATCH(json-string-control-bytes): see printOutput for the call site.
func escapeJSONStringControlBytes(data []byte) []byte {
