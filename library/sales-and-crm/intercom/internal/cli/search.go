...
In local mode: searches locally synced data only.`,
  ...
  intercom-pp-cli search "error timeout"

  # Force local search only
  intercom-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  intercom-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  intercom-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /tickets/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/tickets/search", map[string]any{
					"query":      query,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'intercom-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
