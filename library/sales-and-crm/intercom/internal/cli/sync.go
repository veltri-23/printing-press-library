...
Resource scoping:
  ...
  intercom-pp-cli sync

  # Sync specific resources only
  intercom-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  intercom-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  intercom-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  intercom-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  intercom-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/intercom-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "contacts_companies":
		return db.UpsertContactsCompanies(data)
	case "contacts_notes":
		return db.UpsertContactsNotes(data)
	case "contacts_segments":
		return db.UpsertContactsSegments(data)
	case "subscriptions":
	...
	case "parts":
		return db.UpsertParts(data)
	case "conversations_reply":
		return db.UpsertConversationsReply(data)
	case "conversations_tags":
		return db.UpsertConversationsTags(data)
	case "attributes":
		return db.UpsertAttributes(data)
	case "tickets_reply":
		return db.UpsertTicketsReply(data)
	case "tickets_tags":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"admins":                    "/admins",
		"ai":                        "/ai/external_pages",
		"ai-content-import-sources": "/ai/content_import_sources",
		"articles":                  "/articles",
		"companies":                 "/companies",
		"companies-scroll":          "/companies/scroll",
		"contacts":                  "/contacts",
		"conversations":             "/conversations",
		"data-attributes":           "/data_attributes",
		"help-center":               "/help_center/collections",
		"help-center-help-centers":  "/help_center/help_centers",
		"internal-articles":         "/internal_articles",
		"news":                      "/news/newsfeeds",
		"news-news-items":           "/news/news_items",
		"segments":                  "/segments",
		"subscription-types":        "/subscription_types",
		"tags":                      "/tags",
		"teams":                     "/teams",
		"ticket-states":             "/ticket_states",
