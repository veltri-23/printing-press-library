...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "intercom-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INTERCOM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INTERCOM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "intercom-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/intercom-pp-cli/feedback.jsonl.
When ` + "`INTERCOM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INTERCOM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  intercom-pp-cli feedback list
  intercom-pp-cli feedback list --limit 5
  intercom-pp-cli feedback list --json`,
