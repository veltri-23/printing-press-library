...
type rootFlags struct {
	...
	configPath          string
	profileName         string
	// PATCH(intercom-region): --region us|eu|au overrides INTERCOM_BASE_URL via the
	// regionToBaseURL mapping in config.go. Flag wins over INTERCOM_REGION env var.
	region        string
	deliverSpec   string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "intercom-pp-cli",
		Short: `Intercom CLI — Every Intercom resource as a typed CLI with offline sync and EU/AU regions.`,
		Long: `Intercom CLI — Every Intercom resource as a typed CLI with offline sync and EU/AU regions.
...
Health check: run 'intercom-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("intercom-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'intercom-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	// PATCH(intercom-region): regional Intercom REST host. us/eu/au map to
	// api.intercom.io / api.eu.intercom.io / api.au.intercom.io respectively.
	// Wins over INTERCOM_REGION env var; INTERCOM_BASE_URL still overrides both.
	rootCmd.PersistentFlags().StringVar(&flags.region, "region", "", "Intercom region: us, eu, or au (overrides INTERCOM_REGION env var)")
	...
	// PATCH(intercom-calls-v2-14): /calls/* hand-built subtree (see calls.go).
	...
	rootCmd.AddCommand(newContactCmd(flags))
	rootCmd.AddCommand(newConversationsCmd(flags))
	rootCmd.AddCommand(newCustomObjectInstancesCmd(flags))
	rootCmd.AddCommand(newDataAttributesCmd(flags))
	rootCmd.AddCommand(newEventsCmd(flags))
	rootCmd.AddCommand(newHelpCenterCmd(flags))
	rootCmd.AddCommand(newIntercomExportCmd(flags))
	rootCmd.AddCommand(newInternalArticlesCmd(flags))
	...
	rootCmd.AddCommand(newTicketTypesCmd(flags))
	rootCmd.AddCommand(newTicketsCmd(flags))
	rootCmd.AddCommand(newVisitorsCmd(flags))
	...
	rootCmd.AddCommand(newPhoneCallRedirectsPromotedCmd(flags))
	rootCmd.AddCommand(newSubscriptionTypesPromotedCmd(flags))
	rootCmd.AddCommand(newTicketStatesPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// PATCH(intercom-region): --region flag wins over the env-var-derived
	// cfg.BaseURL. Unknown values are ignored silently so a typo doesn't
	// nuke the working US default; doctor will continue to surface the
	// effective base_url so users notice.
	if f.region != "" {
		if url, ok := config.RegionToBaseURL(f.region); ok {
			cfg.BaseURL = url
		}
	}
