...
func newTicketsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().StringVar(&bodyContacts, "contacts", "", "The list of contacts (users or leads) affected by this ticket. Currently only one is allowed")
	cmd.Flags().IntVar(&bodyCreatedAt, "created-at", 0, "The time the ticket was created. If not provided, the current time will be used.")
	cmd.Flags().BoolVar(&bodySkipNotifications, "skip-notifications", false, "Option to disable notifications when a Ticket is created.")
	cmd.Flags().StringVar(&bodyTicketAttributes, "ticket-attributes", "", "The attributes set on the ticket.")
	cmd.Flags().StringVar(&bodyTicketTypeId, "ticket-type-id", "", "The ID of the type of ticket you want to create")
