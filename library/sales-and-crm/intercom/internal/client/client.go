...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "intercom-pp-cli", "http")
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "intercom-pp-cli/2.13")
		}
		// PATCH(intercom-version): pin to API version 2.13 on every request.
		// Without this header, Intercom silently routes calls to the workspace's
		// default version (which may be older), breaking commands that depend on
		// 2.13 response shapes (e.g. nested predicates in /conversations/search).
		// Per-endpoint overrides via `headerOverrides` still win.
		if req.Header.Get("Intercom-Version") == "" {
			req.Header.Set("Intercom-Version", "2.13")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export INTERCOM_ACCESS_TOKEN=<your-token> or intercom-pp-cli auth set-token <token>")
}
