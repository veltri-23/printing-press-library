# Intercom Printed CLI Agent Guide

This directory is a generated `intercom-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
intercom-pp-cli doctor --json
intercom-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
intercom-pp-cli which "<capability>" --json
intercom-pp-cli <command> --help
```
...
```bash
intercom-pp-cli <command> --help
intercom-pp-cli <command> --dry-run --agent
```
