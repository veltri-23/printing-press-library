...
Resource scoping:
  ...
  salesloft-pp-cli sync

  # Sync specific resources only
  salesloft-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  salesloft-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  salesloft-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  salesloft-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  salesloft-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "salesloft-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/mvanhorn/printing-press-library/library/sales-and-crm/salesloft/data.db)")
