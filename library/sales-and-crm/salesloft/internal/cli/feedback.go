...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "salesloft-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SALESLOFT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SALESLOFT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/sales-and-crm/salesloft/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/github.com/mvanhorn/printing-press-library/library/sales-and-crm/salesloft/feedback.jsonl.
When ` + "`SALESLOFT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SALESLOFT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  salesloft-pp-cli feedback list
  salesloft-pp-cli feedback list --limit 5
  salesloft-pp-cli feedback list --json`,
