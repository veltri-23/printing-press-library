...
type Config struct {
	...
	SalesloftBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SALESLOFT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "salesloft-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SALESLOFT_BEARER_AUTH"); v != "" {
		cfg.SalesloftBearerAuth = v
		cfg.markEnvOverride("SalesloftBearerAuth")
		cfg.AuthSource = "env:SALESLOFT_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.SalesloftBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SALESLOFT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SalesloftBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SALESLOFT_BEARER_AUTH"
		}
		return "Bearer " + c.SalesloftBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.SalesloftBearerAuth = ""
	delete(c.envOverrides, "SalesloftBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SalesloftBearerAuth"] {
			out.SalesloftBearerAuth = c.fileConfig.SalesloftBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SalesloftBearerAuth":
		c.fileConfig.SalesloftBearerAuth = c.SalesloftBearerAuth
	}
}
