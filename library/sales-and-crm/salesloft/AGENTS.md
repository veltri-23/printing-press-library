# Salesloft Printed CLI Agent Guide

This directory is a generated `salesloft-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
salesloft-pp-cli doctor --json
salesloft-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
salesloft-pp-cli which "<capability>" --json
salesloft-pp-cli <command> --help
```
...
```bash
salesloft-pp-cli <command> --agent
```
...
```bash
salesloft-pp-cli <command> --help
salesloft-pp-cli <command> --dry-run --agent
```
