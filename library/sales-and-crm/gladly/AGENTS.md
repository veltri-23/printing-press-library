# Gladly Printed CLI Agent Guide

This directory is a generated `gladly-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gladly-pp-cli doctor --json
gladly-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gladly-pp-cli which "<capability>" --json
gladly-pp-cli <command> --help
```
...
```bash
gladly-pp-cli <command> --help
gladly-pp-cli <command> --dry-run --agent
```
