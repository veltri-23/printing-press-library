...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gladly-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GLADLY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GLADLY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gladly-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gladly-pp-cli/feedback.jsonl.
When ` + "`GLADLY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GLADLY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gladly-pp-cli feedback list
  gladly-pp-cli feedback list --limit 5
  gladly-pp-cli feedback list --json`,
