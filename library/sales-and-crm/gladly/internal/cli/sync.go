...
Resource scoping:
  ...
  gladly-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gladly-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gladly-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gladly-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gladly-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "gladly-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gladly-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"conversation-items": "/api/v1/conversation-items",
