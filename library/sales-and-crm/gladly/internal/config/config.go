...
type Config struct {
	...
	GladlyUsername string            `toml:"username"`
	GladlyPassword string            `toml:"password"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.gladly.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("GLADLY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gladly-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GLADLY_USERNAME"); v != "" {
		cfg.GladlyUsername = v
		cfg.markEnvOverride("GladlyUsername")
		cfg.AuthSource = "env:GLADLY_USERNAME"
	}
	if v := os.Getenv("GLADLY_PASSWORD"); v != "" {
		cfg.GladlyPassword = v
		cfg.markEnvOverride("GladlyPassword")
		cfg.AuthSource = "env:GLADLY_PASSWORD"
	}
	...
	if cfg.AuthSource == "" && cfg.GladlyUsername != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.GladlyPassword != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GLADLY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.GladlyUsername == "" || c.GladlyPassword == "" {
		return ""
	}
	credentials := c.GladlyUsername + ":" + c.GladlyPassword
...
// consults. Gladly requires both username and password; storing just one value
// would make AuthHeader return an empty string and leave requests
// unauthenticated.
//
...
func (c *Config) SaveCredential(username, password string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.GladlyUsername = username
	c.GladlyPassword = password
	delete(c.envOverrides, "GladlyUsername")
	delete(c.envOverrides, "GladlyPassword")
	c.updateFileConfigField("GladlyUsername")
	c.updateFileConfigField("GladlyPassword")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GladlyUsername"] {
			out.GladlyUsername = c.fileConfig.GladlyUsername
		}
		if c.envOverrides["GladlyPassword"] {
			out.GladlyPassword = c.fileConfig.GladlyPassword
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GladlyUsername":
		c.fileConfig.GladlyUsername = c.GladlyUsername
	case "GladlyPassword":
		c.fileConfig.GladlyPassword = c.GladlyPassword
	}
}
