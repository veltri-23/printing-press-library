...
Once synced, use the 'search' command for instant full-text search.`,
  ...
  contact-goat-pp-cli sync

  # Sync specific resources only
  contact-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  contact-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  contact-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  contact-goat-pp-cli sync --concurrency 8`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "suggested_posts":
		return db.UpsertSuggestedPosts(data)
	default:
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		"dynamo":        "/api/dynamo/recent",
		"feed":          "/api/feed",
		"friends":       "/api/friends/list",
		"notifications": "/api/notifications",
		"research":      "/api/research/recent",
	}
