...
In local mode: searches locally synced data only.`,
  ...
  contact-goat-pp-cli search "error timeout"

  # Force local search only
  contact-goat-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  contact-goat-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  contact-goat-pp-cli search "critical" --json --limit 20`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
			query := args[0]
			// This API has a search endpoint: GET /api/dynamo
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/api/dynamo", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'contact-goat-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/contact-goat-pp-cli/data.db)")
