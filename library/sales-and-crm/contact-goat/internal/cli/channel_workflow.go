...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  contact-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  contact-goat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"dynamo", "feed", "friends", "notifications", "research"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  contact-goat-pp-cli workflow status

  # Show status as JSON
  contact-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("contact-goat-pp-cli")
			}
