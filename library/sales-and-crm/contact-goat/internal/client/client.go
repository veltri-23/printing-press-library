...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "contact-goat-pp-cli")
	return &Client{
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	const maxRetries = 3
	var lastErr error
	clerkRefreshed := false

	for attempt := 0; attempt <= maxRetries; attempt++ {
		// Proactive rate limiting — wait before sending
		c.limiter.Wait()

		// Proactive Clerk session refresh (short-lived JWT, ~60s TTL).
		// No-op when cookie auth is not active.
		if err := c.MaybeRefreshSession(); err != nil {
			fmt.Fprintf(os.Stderr, "clerk session refresh failed: %v\n", err)
		}
		...
		// When cookie auth is active, the cookiejar handles Cookie
		// headers — don't paste the raw JWT into a header ourselves.
		if !c.HasCookieAuth() {
			...
			if authHeader != "" {
				req.Header.Set("__session", authHeader)
			}
		}
		...
		clerkAuthStatus := resp.Header.Get("x-clerk-auth-status")
		resp.Body.Close()
		if err != nil {
		...
		// Clerk signed-out on cookie-auth requests: refresh once and retry.
		// Happens on both 401 and 204 per the sniffed traffic (Happenstance
		// returns an empty 204 with x-clerk-auth-status: signed-out when the
		// __session JWT has expired mid-flight).
		if c.HasCookieAuth() && !clerkRefreshed &&
			(resp.StatusCode == 401 || resp.StatusCode == 204) &&
			strings.EqualFold(clerkAuthStatus, "signed-out") {
			clerkRefreshed = true
			if refreshErr := c.refreshClerkSession(); refreshErr != nil {
				return nil, resp.StatusCode, fmt.Errorf("clerk session refresh failed: %w", refreshErr)
			}
			continue
		}
