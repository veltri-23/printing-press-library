# Gainsight Printed CLI Agent Guide

This directory is a generated `gainsight-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
gainsight-pp-cli doctor --json
gainsight-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gainsight-pp-cli which "<capability>" --json
gainsight-pp-cli <command> --help
```
...
```bash
gainsight-pp-cli <command> --agent
```
...
```bash
gainsight-pp-cli <command> --help
gainsight-pp-cli <command> --dry-run --agent
```
