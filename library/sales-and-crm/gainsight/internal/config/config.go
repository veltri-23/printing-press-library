...
type Config struct {
	...
	GainsightAccessKeyAuth string            `toml:"access_key_auth"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://example.gainsightcloud.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("GAINSIGHT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "gainsight-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("GAINSIGHT_ACCESS_KEY_AUTH"); v != "" {
		cfg.GainsightAccessKeyAuth = v
		cfg.markEnvOverride("GainsightAccessKeyAuth")
		cfg.AuthSource = "env:GAINSIGHT_ACCESS_KEY_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.GainsightAccessKeyAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("GAINSIGHT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.GainsightAccessKeyAuth
	if token == "" {
		return ""
	}
	return token
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.GainsightAccessKeyAuth = token
	delete(c.envOverrides, "GainsightAccessKeyAuth")
	c.updateFileConfigField("GainsightAccessKeyAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["GainsightAccessKeyAuth"] {
			out.GainsightAccessKeyAuth = c.fileConfig.GainsightAccessKeyAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "GainsightAccessKeyAuth":
		c.fileConfig.GainsightAccessKeyAuth = c.GainsightAccessKeyAuth
	}
}
