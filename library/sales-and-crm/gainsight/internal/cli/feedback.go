...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gainsight-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GAINSIGHT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GAINSIGHT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gainsight-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gainsight-pp-cli/feedback.jsonl.
When ` + "`GAINSIGHT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GAINSIGHT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gainsight-pp-cli feedback list
  gainsight-pp-cli feedback list --limit 5
  gainsight-pp-cli feedback list --json`,
