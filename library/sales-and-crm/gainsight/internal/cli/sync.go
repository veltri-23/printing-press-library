...
Resource scoping:
  ...
  gainsight-pp-cli sync

  # Sync specific resources only
  gainsight-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  gainsight-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  gainsight-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  gainsight-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  gainsight-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "gainsight-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/gainsight-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"cockpit":       "/v1/cockpit/ctas",
		"companies":     "/v1/companies",
		"data":          "/v1/data/objects",
		"healthscores":  "/v1/healthscores",
		"people":        "/v1/people",
		"relationships": "/v1/relationships",
		"successplans":  "/v1/successplans",
		"timeline":      "/v1/timeline/activities",
	}
