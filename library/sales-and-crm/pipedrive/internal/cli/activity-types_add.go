...
func newActivityTypesAddCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Adds a new activity type.",
		Example:     "  pipedrive-pp-cli activity-types add --icon-key task",
		Annotations: map[string]string{"pp:endpoint": "activity-types.add", "pp:method": "POST", "pp:path": "/activityTypes"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("icon-key") && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "icon-key")
				}
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "activity-types", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "activity-types", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "activity-types", partialFailure.Message))
						}
						return nil
					}
				} else {
