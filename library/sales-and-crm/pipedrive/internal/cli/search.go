...
In local mode: searches locally synced data only.`,
  ...
  pipedrive-pp-cli search "error timeout"

  # Force local search only
  pipedrive-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  pipedrive-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  pipedrive-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /leads/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/leads/search", map[string]string{
					"term": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'pipedrive-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "call-logs":
				results, err = db.SearchCallLogs(query, limit)
			case "deals":
				results, err = db.SearchDeals(query, limit)
			case "files":
				results, err = db.SearchFiles(query, limit)
			case "permission-sets":
				results, err = db.SearchPermissionSets(query, limit)
			case "project-templates":
				results, err = db.SearchProjectTemplates(query, limit)
			case "projects":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchCallLogs(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search call_logs failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchDeals(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search deals failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchPermissionSets(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search permission_sets failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProjectTemplates(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search project_templates failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
