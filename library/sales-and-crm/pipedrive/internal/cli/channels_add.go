...
func newChannelsAddCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Adds a new messaging channel, only admins are able to register new channels.",
		Example:     "  pipedrive-pp-cli channels add --name example-resource",
		Annotations: map[string]string{"pp:endpoint": "channels.add", "pp:method": "POST", "pp:path": "/channels"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				...
				if !cmd.Flags().Changed("provider-channel-id") && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "provider-channel-id")
				}
			}
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("template-support") {
	...
	cmd.Flags().BoolVar(&bodyTemplateSupport, "template-support", false, "If true, enables templates logic on UI. Requires getTemplates endpoint implemented.")
