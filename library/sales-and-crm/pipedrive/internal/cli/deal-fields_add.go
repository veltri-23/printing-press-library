...
func newDealFieldsAddCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Adds a new deal field. For more information, see the tutorial for adding a new custom field .",
		Example:     "  pipedrive-pp-cli deal-fields add --field-type address",
		Annotations: map[string]string{"pp:endpoint": "deal-fields.add", "pp:method": "POST", "pp:path": "/dealFields"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "deal-fields", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "deal-fields", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "deal-fields", partialFailure.Message))
						}
						return nil
					}
				} else {
