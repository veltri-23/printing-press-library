...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pipedrive-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pipedrive-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"call-logs", "currencies", "deals", "files", "filters-helpers", "lead-labels", "lead-sources", "leads", "leads-archived", "notes", "product-fields", "project-templates", "projects", "projects-boards", "tasks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pipedrive-pp-cli workflow status

  # Show status as JSON
  pipedrive-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pipedrive-pp-cli")
			}
