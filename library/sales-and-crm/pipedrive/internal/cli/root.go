...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "pipedrive-pp-cli",
		Short: "Manage pipedrive resources via the pipedrive API",
		Long: `Manage pipedrive resources via the pipedrive API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'pipedrive-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("pipedrive-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'pipedrive-pp-cli profile list')")
	...
	rootCmd.AddCommand(newActivityFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newBillingPromotedCmd(flags))
	rootCmd.AddCommand(newCurrenciesPromotedCmd(flags))
	rootCmd.AddCommand(newLeadFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newLeadSourcesPromotedCmd(flags))
	rootCmd.AddCommand(newNoteFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newRecentsPromotedCmd(flags))
	rootCmd.AddCommand(newUserConnectionsPromotedCmd(flags))
	rootCmd.AddCommand(newUserSettingsPromotedCmd(flags))
