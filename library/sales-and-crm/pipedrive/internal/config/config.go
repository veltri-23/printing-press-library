...
type Config struct {
	...
	PipedriveApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	if path == "" {
		...
		path = filepath.Join(home, ".config", "pipedrive-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("PIPEDRIVE_API_KEY"); v != "" {
		cfg.PipedriveApiKey = v
		cfg.markEnvOverride("PipedriveApiKey")
		cfg.AuthSource = "env:PIPEDRIVE_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.PipedriveApiKey != "" {
		cfg.AuthSource = "config"
	}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.PipedriveApiKey = token
	delete(c.envOverrides, "PipedriveApiKey")
	c.updateFileConfigField("PipedriveApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["PipedriveApiKey"] {
			out.PipedriveApiKey = c.fileConfig.PipedriveApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "PipedriveApiKey":
		c.fileConfig.PipedriveApiKey = c.PipedriveApiKey
	}
}
