# Pipedrive Printed CLI Agent Guide

This directory is a generated `pipedrive-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pipedrive-pp-cli doctor --json
pipedrive-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pipedrive-pp-cli which "<capability>" --json
pipedrive-pp-cli <command> --help
```
...
```bash
pipedrive-pp-cli <command> --agent
```
...
```bash
pipedrive-pp-cli <command> --help
pipedrive-pp-cli <command> --dry-run --agent
```
