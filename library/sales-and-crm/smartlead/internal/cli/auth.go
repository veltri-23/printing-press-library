...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  smartlead-pp-cli auth setup\n  smartlead-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "No setup URL is configured for this CLI; check the API's docs.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export SMARTLEAD_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  smartlead-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
