...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  smartlead-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"campaigns", "client", "email-accounts", "leads"}
			totalSynced := 0
			...
			if flags.asJSON {
				// PATCH(archive-json-ndjson): keep --json output line-delimited.
				// Emit the summary compact (one line), not indented: the sync
				// events syncResource streams to stdout are NDJSON, so a
				// multi-line final object would break the line-delimited JSON
				// contract that `sync --json` also follows.
