...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "analytics_by_date":
		return db.UpsertAnalyticsByDate(data)
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "campaigns_leads", ParentTable: "campaigns", ParentIDParam: "campaign_id", PathTemplate: "/campaigns/{campaign_id}/leads", KeyField: ""},
		{Name: "statistics", ParentTable: "campaigns", ParentIDParam: "campaign_id", PathTemplate: "/campaigns/{campaign_id}/statistics", KeyField: ""},
	}
}
...
func syncDependentResource(c interface {
	...
	for idx, parentID := range parentIDs {
		...
		for {
			...
			// PATCH(dependent-fk): inject the typed table's NOT NULL parent FK
			// column so dependent rows land in the typed tables, not just resources.
			// Inject parent_id AND the typed-table FK column into each item
			// before upserting. The typed dependent tables declare a NOT NULL
			// "<parent>_id" column (e.g. campaigns_id); without injecting it
			// the typed insert fails its NOT NULL constraint and the row only
			// lands in the generic resources table.
			fkColumn := strings.ReplaceAll(dep.ParentTable, "-", "_") + "_id"
			for i, item := range items {
				var obj map[string]json.RawMessage
				if err := json.Unmarshal(item, &obj); err == nil {
					parentIDJSON, _ := json.Marshal(parentID)
					obj["parent_id"] = parentIDJSON
					if _, exists := obj[fkColumn]; !exists {
						obj[fkColumn] = parentIDJSON
					}
