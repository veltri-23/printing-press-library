# Roadside America Printed CLI Agent Guide

This directory is a generated `roadside-america-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
roadside-america-pp-cli doctor --json
roadside-america-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
roadside-america-pp-cli which "<capability>" --json
roadside-america-pp-cli <command> --help
```
...
```bash
roadside-america-pp-cli <command> --agent
```
...
```bash
roadside-america-pp-cli <command> --help
roadside-america-pp-cli <command> --dry-run --agent
```
