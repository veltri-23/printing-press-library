...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "roadside-america-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ROADSIDE_AMERICA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ROADSIDE_AMERICA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "roadside-america-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/roadside-america-pp-cli/feedback.jsonl.
When ` + "`ROADSIDE_AMERICA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ROADSIDE_AMERICA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  roadside-america-pp-cli feedback list
  roadside-america-pp-cli feedback list --limit 5
  roadside-america-pp-cli feedback list --json`,
