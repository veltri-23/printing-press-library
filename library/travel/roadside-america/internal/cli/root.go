...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "roadside-america-pp-cli",
		Short: `Roadside America CLI — Every offbeat roadside attraction on RoadsideAmerica.com — with a local SQLite cache, offline search, and superlative c…`,
		Long: `Roadside America CLI — Every offbeat roadside attraction on RoadsideAmerica.com — with a local SQLite cache, offline search, and superlative c…
...
Health check: run 'roadside-america-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("roadside-america-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'roadside-america-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0.34, "Max requests per second; default ~1 req/3s is a polite scrape rate (0 to disable)")
	...
	rootCmd.AddCommand(newRoadsideSyncCmd(flags))
	...
	// Hand-authored top-level commands (structured RoadsideAmerica.com surfaces).
