# Inside the D Printed CLI Agent Guide

This directory is a generated `visit-detroit-blog-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
visit-detroit-blog-pp-cli doctor --json
visit-detroit-blog-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
visit-detroit-blog-pp-cli which "<capability>" --json
visit-detroit-blog-pp-cli <command> --help
```
...
```bash
visit-detroit-blog-pp-cli <command> --agent
```
...
```bash
visit-detroit-blog-pp-cli <command> --help
visit-detroit-blog-pp-cli <command> --dry-run --agent
```
