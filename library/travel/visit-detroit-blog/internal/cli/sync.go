...
Resource scoping:
  ...
  visit-detroit-blog-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  visit-detroit-blog-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  visit-detroit-blog-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  visit-detroit-blog-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  visit-detroit-blog-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "visit-detroit-blog-pp-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
			...
			for i := 0; i < concurrency; i++ {
				...
				go func() {
					...
					for resource := range work {
						var res syncResult
						if resource == blogResource {
							// Blogs come from Algolia (POST search), not a GET list
							// endpoint, so they bypass the generic GET sync path.
							res = syncBlogsAlgolia(cmd.Context(), c, db, full, maxPages)
						} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/visit-detroit-blog-pp-cli/data.db)")
