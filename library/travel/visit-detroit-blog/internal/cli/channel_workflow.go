...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  visit-detroit-blog-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  visit-detroit-blog-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  visit-detroit-blog-pp-cli workflow status

  # Show status as JSON
  visit-detroit-blog-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("visit-detroit-blog-pp-cli")
			}
