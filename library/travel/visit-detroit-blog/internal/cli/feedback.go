...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".visit-detroit-blog-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VISIT_DETROIT_BLOG_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VISIT_DETROIT_BLOG_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "visit-detroit-blog-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.visit-detroit-blog-pp-cli/feedback.jsonl.
When ` + "`VISIT_DETROIT_BLOG_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VISIT_DETROIT_BLOG_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  visit-detroit-blog-pp-cli feedback list
  visit-detroit-blog-pp-cli feedback list --limit 5
  visit-detroit-blog-pp-cli feedback list --json`,
