...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("POINTHOUND_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("POINTHOUND_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pointhound-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pointhound-pp-cli/feedback.jsonl.
When ` + "`POINTHOUND_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`POINTHOUND_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pointhound-pp-cli feedback list
  pointhound-pp-cli feedback list --limit 5
  pointhound-pp-cli feedback list --json`,
