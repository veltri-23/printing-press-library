...
Exit codes & warnings:
  ...
  pointhound-pp-cli sync

  # Sync specific resources only
  pointhound-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pointhound-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pointhound-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pointhound-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pointhound-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pointhound-pp-cli/data.db)")
