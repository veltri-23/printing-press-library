...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      See API docs: https://www.pointhound.com"+
			"\n      Run 'pointhound-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      See API docs: https://www.pointhound.com"+
			"\n      Run 'pointhound-pp-cli doctor' to check auth status.", err))
...
func compactListFields(items []map[string]any) json.RawMessage {
	keepFields := map[string]bool{
		...
		// Domain-specific signal fields. Without these the value-prop
		// of explore-deal-rating (surfacing dealRating/isTracked) is
		// erased under --agent/--compact.
		"dealRating": true, "isTracked": true,
		// Locale / geo
