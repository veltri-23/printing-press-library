# Pointhound Printed CLI Agent Guide

This directory is a generated `pointhound-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pointhound-pp-cli doctor --json
pointhound-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pointhound-pp-cli which "<capability>" --json
...
```bash
pointhound-pp-cli <command> --agent
```
...
```bash
pointhound-pp-cli <command> --help
pointhound-pp-cli <command> --dry-run --agent
```
