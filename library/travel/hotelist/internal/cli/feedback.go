...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "hotelist-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOTELIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOTELIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hotelist-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/hotelist-pp-cli/feedback.jsonl.
When ` + "`HOTELIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOTELIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hotelist-pp-cli feedback list
  hotelist-pp-cli feedback list --limit 5
  hotelist-pp-cli feedback list --json`,
