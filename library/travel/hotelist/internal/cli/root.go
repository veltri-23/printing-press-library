...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "hotelist-pp-cli",
		Short: `Hotelist CLI — Every Hotelist feature, plus a local SQLite mirror that ranks rating-per-dollar across whole countries, compares chains…`,
		Long: `Hotelist CLI — Every Hotelist feature, plus a local SQLite mirror that ranks rating-per-dollar across whole countries, compares chains…
...
Health check: run 'hotelist-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("hotelist-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'hotelist-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelChainCompareCmd(flags))
	rootCmd.AddCommand(newNovelChainConsistencyCmd(flags))
	rootCmd.AddCommand(newNovelCorridorCmd(flags))
	rootCmd.AddCommand(newNovelPriceCliffCmd(flags))
	rootCmd.AddCommand(newNovelRankCountryCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newHotelPromotedCmd(flags))
	// Hand-authored core commands (search/filter/value/show + stats).
