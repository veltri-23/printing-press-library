# Hotelist Printed CLI Agent Guide

This directory is a generated `hotelist-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hotelist-pp-cli doctor --json
hotelist-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hotelist-pp-cli which "<capability>" --json
hotelist-pp-cli <command> --help
```
...
```bash
hotelist-pp-cli <command> --help
hotelist-pp-cli <command> --dry-run --agent
```
