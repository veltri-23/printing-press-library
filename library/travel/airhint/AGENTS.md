# Airhint Printed CLI Agent Guide

This directory is a generated `airhint-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
airhint-pp-cli doctor --json
airhint-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
airhint-pp-cli which "<capability>" --json
airhint-pp-cli <command> --help
```
...
```bash
airhint-pp-cli <command> --help
airhint-pp-cli <command> --dry-run --agent
```
