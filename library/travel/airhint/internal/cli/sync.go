...
Resource scoping:
  ...
  airhint-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  airhint-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  airhint-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  airhint-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  airhint-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "airhint-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/airhint-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "airport-autocomplete":
		return db.UpsertAirportAutocomplete(data)
	case "cheapest-airline-deal-month":
		return db.UpsertCheapestAirlineDealMonth(data)
	case "flights":
		return db.UpsertFlights(data)
	case "predict":
		return db.UpsertPredict(data)
	default:
