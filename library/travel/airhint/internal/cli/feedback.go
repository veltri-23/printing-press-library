...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "airhint-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AIRHINT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AIRHINT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "airhint-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/airhint-pp-cli/feedback.jsonl.
When ` + "`AIRHINT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AIRHINT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  airhint-pp-cli feedback list
  airhint-pp-cli feedback list --limit 5
  airhint-pp-cli feedback list --json`,
