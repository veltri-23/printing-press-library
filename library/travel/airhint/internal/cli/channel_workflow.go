...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newWorkflowPredictSweepCmd(flags))
	cmd.AddCommand(newWorkflowCompareRoutesCmd(flags))
	cmd.AddCommand(newWorkflowCheapestWindowCmd(flags))
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  airhint-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  airhint-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  airhint-pp-cli workflow status

  # Show status as JSON
  airhint-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("airhint-pp-cli")
			}
