...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "airhint-pp-cli",
		Short: "Manage airhint resources via the airhint API",
		Long: `Manage airhint resources via the airhint API.
...
Run 'airhint-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("airhint-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'airhint-pp-cli profile list')")
	...
	rootCmd.AddCommand(newFlightsCmd(flags))
	...
	rootCmd.AddCommand(newAirportAutocompletePromotedCmd(flags))
	rootCmd.AddCommand(newAirportNamesPromotedCmd(flags))
	rootCmd.AddCommand(newCheapestAirlineDealMonthPromotedCmd(flags))
	rootCmd.AddCommand(newCheapestDealMonthPromotedCmd(flags))
	rootCmd.AddCommand(newPredictPromotedCmd(flags))
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	// AirHint's API endpoints require these headers to return JSON instead of 403.
	if cfg.Headers == nil {
		cfg.Headers = make(map[string]string)
	}
	if cfg.Headers["X-Requested-With"] == "" {
		cfg.Headers["X-Requested-With"] = "XMLHttpRequest"
	}
	if cfg.Headers["Referer"] == "" {
		cfg.Headers["Referer"] = "https://www.airhint.com/ryanair-predictor"
	}
