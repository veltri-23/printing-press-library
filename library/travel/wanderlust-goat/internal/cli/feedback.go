...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".wanderlust-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("WANDERLUST_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("WANDERLUST_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "wanderlust-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.wanderlust-goat-pp-cli/feedback.jsonl.
When ` + "`WANDERLUST_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`WANDERLUST_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  wanderlust-goat-pp-cli feedback list
  wanderlust-goat-pp-cli feedback list --limit 5
  wanderlust-goat-pp-cli feedback list --json`,
