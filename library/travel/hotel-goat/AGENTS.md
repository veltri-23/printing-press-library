# Google Hotels Printed CLI Agent Guide

This directory is a generated `hotel-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
hotel-goat-pp-cli doctor --json
hotel-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
hotel-goat-pp-cli which "<capability>" --json
...
```bash
hotel-goat-pp-cli <command> --agent
```
...
```bash
hotel-goat-pp-cli <command> --help
hotel-goat-pp-cli <command> --dry-run --agent
```
