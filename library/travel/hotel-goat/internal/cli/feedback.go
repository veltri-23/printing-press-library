...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".hotel-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOTEL_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOTEL_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hotel-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.hotel-goat-pp-cli/feedback.jsonl.
When ` + "`HOTEL_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOTEL_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hotel-goat-pp-cli feedback list
  hotel-goat-pp-cli feedback list --limit 5
  hotel-goat-pp-cli feedback list --json`,
