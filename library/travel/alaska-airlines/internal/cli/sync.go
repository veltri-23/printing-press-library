...
Exit codes & warnings:
  ...
  alaska-airlines-pp-cli sync

  # Sync specific resources only
  alaska-airlines-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  alaska-airlines-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  alaska-airlines-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  alaska-airlines-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  alaska-airlines-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/alaska-airlines-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"airports": "/search/api/citySearch/getAllAirports",
	}
