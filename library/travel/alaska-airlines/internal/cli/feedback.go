...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".alaska-airlines-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ALASKA_AIRLINES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ALASKA_AIRLINES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "alaska-airlines-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.alaska-airlines-pp-cli/feedback.jsonl.
When ` + "`ALASKA_AIRLINES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ALASKA_AIRLINES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  alaska-airlines-pp-cli feedback list
  alaska-airlines-pp-cli feedback list --limit 5
  alaska-airlines-pp-cli feedback list --json`,
