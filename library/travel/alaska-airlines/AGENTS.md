# Alaska Airlines Printed CLI Agent Guide

This directory is a generated `alaska-airlines-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
alaska-airlines-pp-cli doctor --json
alaska-airlines-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
alaska-airlines-pp-cli which "<capability>" --json
alaska-airlines-pp-cli <command> --help
```
...
```bash
alaska-airlines-pp-cli <command> --agent
```
...
```bash
alaska-airlines-pp-cli <command> --help
alaska-airlines-pp-cli <command> --dry-run --agent
```
