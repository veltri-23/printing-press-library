...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".hotel-tonight-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("HOTEL_TONIGHT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("HOTEL_TONIGHT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "hotel-tonight-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.hotel-tonight-pp-cli/feedback.jsonl.
When ` + "`HOTEL_TONIGHT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`HOTEL_TONIGHT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  hotel-tonight-pp-cli feedback list
  hotel-tonight-pp-cli feedback list --limit 5
  hotel-tonight-pp-cli feedback list --json`,
