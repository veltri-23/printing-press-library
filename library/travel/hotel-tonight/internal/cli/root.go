...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "hotel-tonight-pp-cli",
		Short: `Last-minute hotel deals with a local price-history database no HotelTonight client has: snapshot deals over time, watch a city for drops, and rank by real % off.`,
		Long: `Last-minute hotel deals with a local price-history database no HotelTonight client has: snapshot deals over time, watch a city for drops, and rank by real % off.
...
Health check: run 'hotel-tonight-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("hotel-tonight-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'hotel-tonight-pp-cli profile list')")
	...
	// Novel features (hand-authored; see ht_*.go). Re-injected by regen-merge.
	rootCmd.AddCommand(newDealsCmd(flags))
	rootCmd.AddCommand(newWatchCmd(flags))
	rootCmd.AddCommand(newHistoryCmd(flags))
	rootCmd.AddCommand(newVerdictCmd(flags))
	rootCmd.AddCommand(newCompareNeighborhoodsCmd(flags))
	rootCmd.AddCommand(newDatescanCmd(flags))
	rootCmd.AddCommand(newDailyDropCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("hotel-tonight-pp-cli %s\n", version)
		},
	}
}
