...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".delta-trip-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DELTA_TRIP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DELTA_TRIP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "delta-trip-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.delta-trip-pp-cli/feedback.jsonl.
When ` + "`DELTA_TRIP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DELTA_TRIP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  delta-trip-pp-cli feedback list
  delta-trip-pp-cli feedback list --limit 5
  delta-trip-pp-cli feedback list --json`,
