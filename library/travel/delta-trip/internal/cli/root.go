...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "delta-trip-pp-cli",
		Short: "Manage delta-trip resources via the delta-trip API",
		Long: `Manage delta-trip resources via the delta-trip API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'delta-trip-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("delta-trip-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'delta-trip-pp-cli profile list')")
	...
	rootCmd.AddCommand(newSeatMapCmd(flags))
	rootCmd.AddCommand(newBaggagePromotedCmd(flags))
	rootCmd.AddCommand(newFlightsPromotedCmd(flags))
	rootCmd.AddCommand(newSeatsPromotedCmd(flags))
	rootCmd.AddCommand(newTripsPromotedCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("delta-trip-pp-cli %s\n", version)
		},
	}
}
