# Delta Trip Printed CLI Agent Guide

This directory is a generated `delta-trip-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
delta-trip-pp-cli doctor --json
delta-trip-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
delta-trip-pp-cli which "<capability>" --json
delta-trip-pp-cli <command> --help
```
...
```bash
delta-trip-pp-cli <command> --agent
```
...
```bash
delta-trip-pp-cli <command> --help
delta-trip-pp-cli <command> --dry-run --agent
```
