# TBM Bordeaux Printed CLI Agent Guide

This directory is a generated `infotbm-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
infotbm-pp-cli doctor --json
infotbm-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
infotbm-pp-cli which "<capability>" --json
infotbm-pp-cli <command> --help
```
...
```bash
infotbm-pp-cli <command> --help
infotbm-pp-cli <command> --dry-run --agent
```
