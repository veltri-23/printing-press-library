...
type Config struct {
	...
	Path          string            `json:"-"`
	InfotbmApiKey string            `json:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://bdx.mecatran.com/utw/ws",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("INFOTBM_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "infotbm-pp-cli", "config.json")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("INFOTBM_API_KEY"); v != "" {
		cfg.InfotbmApiKey = v
		cfg.AuthSource = "env:INFOTBM_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.InfotbmApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("INFOTBM_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
