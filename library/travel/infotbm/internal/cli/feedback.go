...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "infotbm-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INFOTBM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INFOTBM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "infotbm-pp-cli/feedback")
	...
	defer io.Copy(io.Discard, resp.Body) //nolint:errcheck // drain for connection reuse
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/infotbm-pp-cli/feedback.jsonl.
When ` + "`INFOTBM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INFOTBM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  infotbm-pp-cli feedback list
  infotbm-pp-cli feedback list --limit 5
  infotbm-pp-cli feedback list --json`,
