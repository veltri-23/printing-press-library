...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SEATS_AERO_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "seats-aero-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SEATS_AERO_API_KEY"); v != "" {
		cfg.SeatsAeroPartnerPartnerAuthorization = v
		cfg.AuthSource = "env:SEATS_AERO_API_KEY"
	}
	if v := os.Getenv("SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION"); v != "" {
		cfg.SeatsAeroPartnerPartnerAuthorization = v
		cfg.AuthSource = "env:SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SEATS_AERO_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.SeatsAeroPartnerPartnerAuthorization
	if token == "" {
		return ""
	}
	if c.SeatsAeroPartnerPartnerAuthorization == "" {
		return ""
	}
	return token
}
