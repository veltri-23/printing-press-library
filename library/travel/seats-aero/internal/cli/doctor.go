...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  seats-aero-pp-cli doctor
  seats-aero-pp-cli doctor --json
  seats-aero-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SEATS_AERO_API_KEY=***"
				} else {
			...
			if os.Getenv("SEATS_AERO_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "SEATS_AERO_API_KEY")
			} else if os.Getenv("SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION") != "" {
				authEnvSet = append(authEnvSet, "SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SEATS_AERO_API_KEY (or SEATS_AERO_PARTNER_PARTNER_AUTHORIZATION)")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["Partner-Authorization"] = authHeader
						authHeaders["User-Agent"] = "seats-aero-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'seats-aero-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'seats-aero-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'seats-aero-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'seats-aero-pp-cli sync' to refresh."
	}
	return report
}
