...
Exit codes & warnings:
  ...
  seats-aero-pp-cli sync

  # Sync specific resources only
  seats-aero-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  seats-aero-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  seats-aero-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  seats-aero-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  seats-aero-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/seats-aero-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "seats_aero_partner_search":
		return db.UpsertSeatsAeroPartnerSearch(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"routes":                    "/routes",
		"seats-aero-partner-search": "/search",
	}
