...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  seats-aero-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  seats-aero-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"availability", "routes", "seats-aero-partner-search"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  seats-aero-pp-cli workflow status

  # Show status as JSON
  seats-aero-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("seats-aero-pp-cli")
			}
