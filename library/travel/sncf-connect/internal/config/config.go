...
type Config struct {
	...
	Path          string            `toml:"-"`
	SncfApiKey    string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("NAVITIA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sncf-connect-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SNCF_API_KEY"); v != "" {
		cfg.SncfApiKey = v
		cfg.AuthSource = "env:SNCF_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SncfApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("NAVITIA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
