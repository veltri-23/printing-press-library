...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".sncf-connect-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NAVITIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NAVITIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sncf-connect-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.sncf-connect-pp-cli/feedback.jsonl.
When ` + "`NAVITIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NAVITIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sncf-connect-pp-cli feedback list
  sncf-connect-pp-cli feedback list --limit 5
  sncf-connect-pp-cli feedback list --json`,
