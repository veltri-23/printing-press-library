# Sncf Connect Printed CLI Agent Guide

This directory is a generated `sncf-connect-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sncf-connect-pp-cli doctor --json
sncf-connect-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sncf-connect-pp-cli which "<capability>" --json
sncf-connect-pp-cli <command> --help
```
...
```bash
sncf-connect-pp-cli <command> --agent
```
...
```bash
sncf-connect-pp-cli <command> --help
sncf-connect-pp-cli <command> --dry-run --agent
```
