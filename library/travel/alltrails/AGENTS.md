# Alltrails Printed CLI Agent Guide

This directory is a generated `alltrails-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
alltrails-pp-cli doctor --json
alltrails-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
alltrails-pp-cli which "<capability>" --json
alltrails-pp-cli <command> --help
```
...
```bash
alltrails-pp-cli <command> --agent
```
...
```bash
alltrails-pp-cli <command> --help
alltrails-pp-cli <command> --dry-run --agent
```
