...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "alltrails-pp-cli",
		Short: "Manage alltrails resources via the alltrails API",
		Long: `Manage alltrails resources via the alltrails API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'alltrails-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("alltrails-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().BoolVar(&flags.liveWrite, "live-write", false, "Allow a write command to attempt live execution after the ALLTRAILS_PP_ALLOW_WRITES=1 env gate")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'alltrails-pp-cli profile list')")
	...
	rootCmd.AddCommand(newAlltrailsCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		Use:   "version",
		Short: "Print the alltrails-pp-cli binary version",
