...
Resource scoping:
  ...
  alltrails-pp-cli sync

  # Sync specific resources only
  alltrails-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  alltrails-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  alltrails-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  alltrails-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  alltrails-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/alltrails-pp-cli/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"routing_info": "/api/alltrails/geo-service/routing/info",
	}
