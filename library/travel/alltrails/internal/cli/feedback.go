...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "alltrails-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ALLTRAILS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ALLTRAILS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "alltrails-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/alltrails-pp-cli/feedback.jsonl.
When ` + "`ALLTRAILS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ALLTRAILS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  alltrails-pp-cli feedback list
  alltrails-pp-cli feedback list --limit 5
  alltrails-pp-cli feedback list --json`,
