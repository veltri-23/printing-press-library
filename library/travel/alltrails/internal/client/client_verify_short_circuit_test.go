...
// TestClient_LiveWriteBarrier_NoEnv pins the PP operator path: with no
// verify env vars and no explicit approval env, mutating verbs fail before
// auth and transport.
func TestClient_LiveWriteBarrier_NoEnv(t *testing.T) {
	...
	_, _, err := c.do(context.Background(), "DELETE", "/test", nil, nil, nil)

	if err == nil {
		t.Fatal("mutating verb without approval env should fail")
	}
	if rec.calls != 0 {
		t.Fatalf("write barrier must not dial; recorder saw %d calls", rec.calls)
	}
}

// TestClient_LiveWriteBarrier_ApprovalEnv pins the human-approval escape
// hatch: after explicit opt-in, normal operator writes can reach transport.
func TestClient_LiveWriteBarrier_ApprovalEnv(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "")
	...
	if rec.calls < 1 {
		t.Fatalf("%s=1 should dial normally; recorder saw %d calls", AllowWritesEnv, rec.calls)
	}
}

// TestClient_LiveWriteBarrier_LiveHTTPAloneDoesNotApprove confirms the
// verifier's live-HTTP flag is not a public write approval by itself.
func TestClient_LiveWriteBarrier_LiveHTTPAloneDoesNotApprove(t *testing.T) {
	t.Setenv("PRINTING_PRESS_VERIFY", "")
	t.Setenv("PRINTING_PRESS_VERIFY_LIVE_HTTP", "1")
	t.Setenv(AllowWritesEnv, "")
	c, rec := newClientWithRecorder(t)
	...
	if err == nil {
		t.Fatal("PRINTING_PRESS_VERIFY_LIVE_HTTP without PRINTING_PRESS_VERIFY must not approve writes")
	}
	if rec.calls != 0 {
		t.Fatalf("write barrier must not dial; recorder saw %d calls", rec.calls)
	}
