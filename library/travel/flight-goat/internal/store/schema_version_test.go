...
func TestMigrate_AddsColumnsOnUpgrade_Blocked(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from blocked after migrate", want)
		}
	}
}
...
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Airports(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
...
// TestMigrate_AddsColumnsOnUpgrade_Weather verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Weather(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
...
// TestMigrate_AddsColumnsOnUpgrade_Intents verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Intents(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from intents after migrate", want)
		}
	}
}
...
// TestMigrate_AddsColumnsOnUpgrade_Position verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Position(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from position after migrate", want)
		}
	}
}

// TestMigrate_AddsColumnsOnUpgrade_Route verifies that opening a
// database created by an older binary succeeds and adds newly generated
// columns before CREATE INDEX runs against the pre-existing table. Regression
// coverage for parent_id upgrades and indexed generated columns.
func TestMigrate_AddsColumnsOnUpgrade_Route(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from route after migrate", want)
		}
	}
}
