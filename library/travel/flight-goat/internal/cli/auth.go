...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage FLIGHT_GOAT_API_KEY_AUTH credentials",
	}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  flight-goat-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if header == "" {
				...
				fmt.Fprintln(w, "  export FLIGHT_GOAT_API_KEY_AUTH=\"your-token-here\"")
				fmt.Fprintf(w, "  flight-goat-pp-cli auth set-token <token>\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  flight-goat-pp-cli auth set-token sk_live_abc123",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  flight-goat-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if os.Getenv("FLIGHT_GOAT_API_KEY_AUTH") != "" {
				fmt.Fprintf(cmd.OutOrStdout(), "Config cleared. Note: FLIGHT_GOAT_API_KEY_AUTH env var is still set.\n")
				return nil
			}
