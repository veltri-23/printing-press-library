...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  flight-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  flight-goat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"airports", "alerts", "flights", "foresight", "operators"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  flight-goat-pp-cli workflow status

  # Show status as JSON
  flight-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("flight-goat-pp-cli")
			}
