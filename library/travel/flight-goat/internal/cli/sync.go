...
Exit codes & warnings:
  ...
  flight-goat-pp-cli sync

  # Sync specific resources only
  flight-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  flight-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  flight-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  flight-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  flight-goat-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("flight-goat-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/flight-goat-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "operators":
		return db.UpsertOperators(data)
	case "canonical":
		return db.UpsertCanonical(data)
	case "flights":
		return db.UpsertFlights(data)
	case "disruption_counts":
		return db.UpsertDisruptionCounts(data)
	case "foresight":
		return db.UpsertForesight(data)
	case "history":
		return db.UpsertHistory(data)
	case "airports":
		return db.UpsertAirports(data)
	case "routes":
		return db.UpsertRoutes(data)
	case "weather":
		return db.UpsertWeather(data)
	case "delays":
		return db.UpsertDelays(data)
	case "nearby":
		return db.UpsertNearby(data)
	case "track":
		return db.UpsertTrack(data)
	case "intents":
		return db.UpsertIntents(data)
	case "map":
		return db.UpsertMap(data)
	case "position":
		return db.UpsertPosition(data)
	case "route":
		return db.UpsertRoute(data)
	default:
...
func syncResourcePath(resource string) string {
	paths := map[string]string{
		...
		"foresight": "/foresight/flights/search/advanced",
		"operators": "/operators",
	}
