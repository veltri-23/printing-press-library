...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("FLIGHT_GOAT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "flight-goat-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("FLIGHT_GOAT_API_KEY_AUTH"); v != "" {
		cfg.FlightGoatApiKeyAuth = v
		cfg.AuthSource = "env:FLIGHT_GOAT_API_KEY_AUTH"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("FLIGHT_GOAT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.FlightGoatApiKeyAuth
	if token == "" {
		return ""
	}
	if c.FlightGoatApiKeyAuth == "" {
		return ""
	}
	return token
}
