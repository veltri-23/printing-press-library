...
Resource scoping:
  ...
  uk-train-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  uk-train-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  uk-train-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  uk-train-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  uk-train-goat-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "uk-train-goat-pp-cli sync: no bulk-list endpoints in spec; populate the store via single-fetch commands instead.")
				} else {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/uk-train-goat-pp-cli/data.db)")
