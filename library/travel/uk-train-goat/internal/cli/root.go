...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "uk-train-goat-pp-cli",
		Short: `Uk Train Goat CLI — The only Go-native, agent-native, MCP-exposed UK rail CLI; live boards, journey planning, and a local SQLite store that…`,
		Long: `Uk Train Goat CLI — The only Go-native, agent-native, MCP-exposed UK rail CLI; live boards, journey planning, and a local SQLite store that…
...
Health check: run 'uk-train-goat-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("uk-train-goat-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'uk-train-goat-pp-cli profile list')")
	...
	rootCmd.AddCommand(newArrivalsCmd(flags))
	rootCmd.AddCommand(newJourneyCmd(flags))
	rootCmd.AddCommand(newServiceCmd(flags))
	rootCmd.AddCommand(newFareCmd(flags))
	rootCmd.AddCommand(newStationsCmd(flags))
