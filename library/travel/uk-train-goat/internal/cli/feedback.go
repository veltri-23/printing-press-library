...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "uk-train-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("UK_TRAIN_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("UK_TRAIN_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/travel/uk-train-goat/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/uk-train-goat-pp-cli/feedback.jsonl.
When ` + "`UK_TRAIN_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`UK_TRAIN_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  uk-train-goat-pp-cli feedback list
  uk-train-goat-pp-cli feedback list --limit 5
  uk-train-goat-pp-cli feedback list --json`,
