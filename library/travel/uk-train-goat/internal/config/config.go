...
type Config struct {
	...
	Path          string            `toml:"-"`
	LdbwsApiToken string            `toml:"api_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://lite.realtime.nationalrail.co.uk/OpenLDBWS/ldb12.asmx",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("UK_TRAIN_GOAT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "uk-train-goat-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("LDBWS_API_TOKEN"); v != "" {
		cfg.LdbwsApiToken = v
		cfg.AuthSource = "env:LDBWS_API_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.LdbwsApiToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("UK_TRAIN_GOAT_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"token":           token,
		"api_token":       c.LdbwsApiToken,
		"LDBWS_API_TOKEN": c.LdbwsApiToken,
	}
