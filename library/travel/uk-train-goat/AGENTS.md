# UK Train Goat Printed CLI Agent Guide

This directory is a generated `uk-train-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
uk-train-goat-pp-cli doctor --json
uk-train-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
uk-train-goat-pp-cli which "<capability>" --json
uk-train-goat-pp-cli <command> --help
```
...
```bash
uk-train-goat-pp-cli <command> --agent
```
...
```bash
uk-train-goat-pp-cli <command> --help
uk-train-goat-pp-cli <command> --dry-run --agent
```
