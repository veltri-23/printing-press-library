# Tripadvisor Printed CLI Agent Guide

This directory is a generated `tripadvisor-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
tripadvisor-pp-cli doctor --json
tripadvisor-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
tripadvisor-pp-cli which "<capability>" --json
tripadvisor-pp-cli <command> --help
```
...
```bash
tripadvisor-pp-cli <command> --agent
```
...
```bash
tripadvisor-pp-cli <command> --help
tripadvisor-pp-cli <command> --dry-run --agent
```
