...
Resource scoping:
  ...
  tripadvisor-pp-cli sync

  # Sync specific resources only
  tripadvisor-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tripadvisor-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tripadvisor-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tripadvisor-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tripadvisor-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "find":
		return db.UpsertFind(data)
	case "near":
		return db.UpsertNear(data)
	case "photos":
		return db.UpsertPhotos(data)
	case "reviews":
		return db.UpsertReviews(data)
	case "show":
		return db.UpsertShow(data)
	default:
