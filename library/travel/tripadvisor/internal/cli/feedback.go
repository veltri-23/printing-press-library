...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "tripadvisor-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TRIPADVISOR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TRIPADVISOR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tripadvisor-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/tripadvisor-pp-cli/feedback.jsonl.
When ` + "`TRIPADVISOR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TRIPADVISOR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tripadvisor-pp-cli feedback list
  tripadvisor-pp-cli feedback list --limit 5
  tripadvisor-pp-cli feedback list --json`,
