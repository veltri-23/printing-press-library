...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tripadvisor-pp-cli doctor
  tripadvisor-pp-cli doctor --json
  tripadvisor-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TRIPADVISOR_API_KEY=<your-key>"
					report["auth_key_url"] = "https://www.tripadvisor.com/developers"
					report["auth_instructions"] = "Sign in, create an API key, then set an IP (your public IPv4 as a /32) or domain restriction before the key is revealed. Domain-restricted keys also require a Referer header on every request."
				} else {
					authConfigured = true
			...
			if os.Getenv("TRIPADVISOR_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "TRIPADVISOR_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TRIPADVISOR_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "tripadvisor-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("tripadvisor-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'tripadvisor-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'tripadvisor-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'tripadvisor-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'tripadvisor-pp-cli sync' to refresh."
	}
	return report
}
