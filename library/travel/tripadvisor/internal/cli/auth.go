...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Tripadvisor",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  tripadvisor-pp-cli auth setup\n  tripadvisor-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://www.tripadvisor.com/developers")
			fmt.Fprintln(w, "  Sign in, create an API key, then set an IP (your public IPv4 as a /32) or domain restriction before the key is revealed. Domain-restricted keys also require a Referer header on every request.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export TRIPADVISOR_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  tripadvisor-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://www.tripadvisor.com/developers"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tripadvisor-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export TRIPADVISOR_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  tripadvisor-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tripadvisor-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tripadvisor-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("TRIPADVISOR_API_KEY") != "" {
				envStillSet = "TRIPADVISOR_API_KEY"
			}
