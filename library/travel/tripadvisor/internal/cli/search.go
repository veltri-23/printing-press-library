...
In local mode: searches locally synced data only.`,
  ...
  tripadvisor-pp-cli search "error timeout"

  # Force local search only
  tripadvisor-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  tripadvisor-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  tripadvisor-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /location/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/location/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'tripadvisor-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/tripadvisor-pp-cli/data.db)")
