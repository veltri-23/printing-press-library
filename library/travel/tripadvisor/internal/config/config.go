...
type Config struct {
	...
	Path              string            `toml:"-"`
	TripadvisorApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TRIPADVISOR_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "tripadvisor-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TRIPADVISOR_API_KEY"); v != "" {
		cfg.TripadvisorApiKey = v
		cfg.AuthSource = "env:TRIPADVISOR_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.TripadvisorApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TRIPADVISOR_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.TripadvisorApiKey
	if token == "" {
		return ""
	}
	if c.TripadvisorApiKey == "" {
		return ""
	}
	return token
}
