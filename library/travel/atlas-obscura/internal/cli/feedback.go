...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "atlas-obscura-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ATLAS_OBSCURA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ATLAS_OBSCURA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "atlas-obscura-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/atlas-obscura-pp-cli/feedback.jsonl.
When ` + "`ATLAS_OBSCURA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ATLAS_OBSCURA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  atlas-obscura-pp-cli feedback list
  atlas-obscura-pp-cli feedback list --limit 5
  atlas-obscura-pp-cli feedback list --json`,
