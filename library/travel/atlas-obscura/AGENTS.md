# Atlas Obscura Printed CLI Agent Guide

This directory is a generated `atlas-obscura-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
atlas-obscura-pp-cli doctor --json
atlas-obscura-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
atlas-obscura-pp-cli which "<capability>" --json
atlas-obscura-pp-cli <command> --help
```
...
```bash
atlas-obscura-pp-cli <command> --agent
```
...
```bash
atlas-obscura-pp-cli <command> --help
atlas-obscura-pp-cli <command> --dry-run --agent
```
