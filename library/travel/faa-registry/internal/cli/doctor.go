...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  faa-registry-pp-cli doctor
  faa-registry-pp-cli doctor --json
  faa-registry-pp-cli doctor --fail-on warn
  faa-registry-pp-cli doctor --fail-on stale`,
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "faa-registry-pp-cli", suggestion)
						} else {
