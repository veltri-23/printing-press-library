...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FAA_REGISTRY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FAA_REGISTRY_FEEDBACK_AUTO_SEND")))
...
When ` + "`FAA_REGISTRY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FAA_REGISTRY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  faa-registry-pp-cli feedback list
  faa-registry-pp-cli feedback list --limit 5
  faa-registry-pp-cli feedback list --json`,
