...
Exit codes & warnings:
  ...
  open-meteo-pp-cli sync

  # Sync specific resources only
  open-meteo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  open-meteo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  open-meteo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  open-meteo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  open-meteo-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("open-meteo-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/open-meteo-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "air_quality":
		return db.UpsertAirQuality(data)
	case "flood":
		return db.UpsertFlood(data)
	case "climate":
		return db.UpsertClimate(data)
	case "ensemble":
		return db.UpsertEnsemble(data)
	case "seasonal":
		return db.UpsertSeasonal(data)
	case "geocode":
		return db.UpsertGeocode(data)
	case "forecast":
		return db.UpsertForecast(data)
	case "marine":
		return db.UpsertMarine(data)
	case "archive":
