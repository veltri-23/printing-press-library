...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".open-meteo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPEN_METEO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPEN_METEO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "open-meteo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.open-meteo-pp-cli/feedback.jsonl.
When ` + "`OPEN_METEO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPEN_METEO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
