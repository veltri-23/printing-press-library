...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/open-meteo/internal/customertier"
	"io"
	"math"
	"net/http"
	"net/url"
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "open-meteo-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	// Customer-tier routing: when OPEN_METEO_API_KEY is set, swap the host to
	// the customer-* equivalent and append the apikey query parameter. The
	// transformation is a no-op when the env var is unset, preserving the
	// free-tier default behavior.
	if rewritten, ok := customertier.RewriteURL(targetURL); ok {
		targetURL = rewritten
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "open-meteo-pp-cli/0.1.0")
