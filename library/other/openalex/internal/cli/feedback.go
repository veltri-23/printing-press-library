...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".openalex-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENALEX_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENALEX_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openalex-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.openalex-pp-cli/feedback.jsonl.
When ` + "`OPENALEX_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENALEX_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openalex-pp-cli feedback list
  openalex-pp-cli feedback list --limit 5
  openalex-pp-cli feedback list --json`,
