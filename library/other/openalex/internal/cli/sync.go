...
Exit codes & warnings:
  ...
  openalex-pp-cli sync

  # Sync specific resources only
  openalex-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openalex-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openalex-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openalex-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openalex-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/openalex-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "awards":
		return db.UpsertAwards(data)
	case "concepts":
		return db.UpsertConcepts(data)
	case "continents":
		return db.UpsertContinents(data)
	case "countries":
	...
	case "funders":
		return db.UpsertFunders(data)
	case "institutions":
		return db.UpsertInstitutions(data)
	case "keywords":
		return db.UpsertKeywords(data)
	case "languages":
		return db.UpsertLanguages(data)
	case "publishers":
		return db.UpsertPublishers(data)
	case "sdgs":
		return db.UpsertSdgs(data)
	case "sources":
		return db.UpsertSources(data)
	case "subfields":
		return db.UpsertSubfields(data)
	case "topics":
		return db.UpsertTopics(data)
	case "works":
		return db.UpsertWorks(data)
	default:
