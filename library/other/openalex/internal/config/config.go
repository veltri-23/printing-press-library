...
type Config struct {
	...
	Path           string            `toml:"-"`
	OpenalexApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("OPENALEX_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "openalex-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if cfg.AuthSource == "" && cfg.OpenalexApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("OPENALEX_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
