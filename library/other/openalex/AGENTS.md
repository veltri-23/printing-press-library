# Openalex Printed CLI Agent Guide

This directory is a generated `openalex-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
openalex-pp-cli doctor --json
openalex-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
openalex-pp-cli which "<capability>" --json
openalex-pp-cli <command> --help
```
...
```bash
openalex-pp-cli <command> --help
openalex-pp-cli <command> --dry-run --agent
```
