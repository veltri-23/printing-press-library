# FRED Printed CLI Agent Guide

This directory is a generated `fred-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
fred-pp-cli doctor --json
fred-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
fred-pp-cli which "<capability>" --json
fred-pp-cli <command> --help
```
...
```bash
fred-pp-cli <command> --help
fred-pp-cli <command> --dry-run --agent
```
