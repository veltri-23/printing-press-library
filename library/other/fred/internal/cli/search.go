...
In local mode: searches locally synced data only.`,
  ...
  fred-pp-cli search "error timeout"

  # Force local search only
  fred-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  fred-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  fred-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /series/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/series/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'fred-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "category":
				results, err = db.SearchCategory(query, limit)
			case "release":
				results, err = db.SearchRelease(query, limit)
			case "series":
				results, err = db.SearchSeries(query, limit)
			case "source":
				results, err = db.SearchSource(query, limit)
			case "tags":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchCategory(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search category failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchRelease(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search release failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSource(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search source failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/fred-pp-cli/data.db)")
