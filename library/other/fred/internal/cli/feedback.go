...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "fred-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FRED_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FRED_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fred-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/fred-pp-cli/feedback.jsonl.
When ` + "`FRED_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FRED_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fred-pp-cli feedback list
  fred-pp-cli feedback list --limit 5
  fred-pp-cli feedback list --json`,
