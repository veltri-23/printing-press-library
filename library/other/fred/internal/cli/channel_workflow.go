...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  fred-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  fred-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"category", "category-children", "category-related", "category-series", "release", "release-dates", "release-releases", "release-series", "release-sources", "series", "series-categories", "series-release", "series-updates", "source", "source-releases", "source-sources"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  fred-pp-cli workflow status

  # Show status as JSON
  fred-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("fred-pp-cli")
			}
