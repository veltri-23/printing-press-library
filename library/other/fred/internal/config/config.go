...
type Config struct {
	...
	FredApiKey    string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("FRED_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "fred-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("FRED_API_KEY"); v != "" {
		cfg.FredApiKey = v
		cfg.markEnvOverride("FredApiKey")
		cfg.AuthSource = "env:FRED_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.FredApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("FRED_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.FredApiKey = token
	delete(c.envOverrides, "FredApiKey")
	c.updateFileConfigField("FredApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FredApiKey"] {
			out.FredApiKey = c.fileConfig.FredApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FredApiKey":
		c.fileConfig.FredApiKey = c.FredApiKey
	}
}
