# FINRA Printed CLI Agent Guide

This directory is a generated `finra-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
finra-pp-cli doctor --json
finra-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
finra-pp-cli which "<capability>" --json
...
```bash
finra-pp-cli <command> --help
finra-pp-cli <command> --dry-run --agent
```
