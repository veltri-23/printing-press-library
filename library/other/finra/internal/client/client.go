...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "finra-pp-cli/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "finra-pp-cli auth login or finra-pp-cli auth set-token <token>")
}
...
func (c *Client) mintClientCredentials(ctx context.Context, clientID, clientSecret string) error {
	...
	// FINRA's FIP token endpoint expects grant_type ONLY as a query
	// parameter on tokenURL (already embedded above) with an EMPTY POST
	// body. Sending a form-encoded body — including a body whose
	// grant_type value matches the query string — makes FIP reject the
	// request with {"error":"unsupported_grant_type"}; confirmed via
	// direct curl testing against the QA/test environment. This deviates
	// from RFC 6749 3.2 (grant_type normally belongs in the body), so it
	// is handled here rather than in the shared OAuth2 client_credentials
	// generator template.
	...
	// FINRA's FIP token endpoint returns expires_in as a JSON-encoded
	// string (e.g. "43170"), not a JSON number, despite RFC 6749 6
	// describing it as a number; confirmed via a live token mint.
	// cliutil.ExtractInt accepts either shape.
	...
	expiry := time.Time{}
	if expiresIn, ok := cliutil.ExtractInt(rawFields, "expires_in"); ok && expiresIn > 0 {
