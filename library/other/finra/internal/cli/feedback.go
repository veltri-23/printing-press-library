...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FINRA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FINRA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "finra-pp-cli/feedback")
...
When ` + "`FINRA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FINRA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  finra-pp-cli feedback list
  finra-pp-cli feedback list --limit 5
  finra-pp-cli feedback list --json`,
