...
Run sync first to populate the local search index.`,
  ...
  finra-pp-cli search "status"

  # Force local search explicitly
  finra-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  finra-pp-cli search "status" --type async --data-source local
  # JSON output for piping
  finra-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'finra-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "catalog":
				results, err = db.SearchCatalog(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchCatalog(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search catalog failed: %w", searchErr)
					}
