...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "finra-pp-cli",
		Short: `FINRA CLI — Every FINRA Query API dataset, plus local trend detection and batch registration checks no other FINRA tool offers.`,
		Long: `FINRA CLI — Every FINRA Query API dataset, plus local trend detection and batch registration checks no other FINRA tool offers.
...
Health check: run 'finra-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("finra-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'finra-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelComplaintsCmd(flags))
	rootCmd.AddCommand(newNovelFixedincomeCmd(flags))
	rootCmd.AddCommand(newNovelOtcCmd(flags))
	rootCmd.AddCommand(newNovelRegistrationCmd(flags))
	rootCmd.AddCommand(newNovelRegshoCmd(flags))
	rootCmd.AddCommand(newNovelTraceCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAsyncPromotedCmd(flags))
	rootCmd.AddCommand(newCatalogPromotedCmd(flags))
	rootCmd.AddCommand(newMetadataPromotedCmd(flags))
	rootCmd.AddCommand(newPartitionsPromotedCmd(flags))
