...
This directory is a generated `defillama-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
defillama-pp-cli doctor --json
defillama-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
defillama-pp-cli which "<capability>" --json
...
```bash
defillama-pp-cli <command> --agent
```
...
```bash
defillama-pp-cli <command> --help
defillama-pp-cli <command> --dry-run --agent
```
