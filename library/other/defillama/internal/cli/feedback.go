...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "defillama-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DEFILLAMA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DEFILLAMA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "defillama-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/defillama-pp-cli/feedback.jsonl.
When ` + "`DEFILLAMA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DEFILLAMA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  defillama-pp-cli feedback list
  defillama-pp-cli feedback list --limit 5
  defillama-pp-cli feedback list --json`,
