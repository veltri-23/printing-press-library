...
large datasets as it has no memory pressure.`,
  ...
  defillama-pp-cli export <resource> --format jsonl --output data.jsonl

  # Export with limit
  defillama-pp-cli export <resource> --format jsonl --limit 1000

  # Pipe to another tool
  defillama-pp-cli export <resource> --format jsonl | jq '.id'`,
		Args: cobra.MinimumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			validResources := map[string]bool{
				"pools":            true,
				"protocols":        true,
				"stablecoinchains": true,
				"stablecoinprices": true,
				"stablecoins":      true,
			}
