...
In local mode: searches locally synced data only.`,
  ...
  defillama-pp-cli search "error timeout"

  # Force local search only
  defillama-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  defillama-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  defillama-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'defillama-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "entities":
				results, err = db.SearchEntities(query, limit)
			case "metrics":
				results, err = db.SearchMetrics(query, limit)
			case "protocol":
				results, err = db.SearchProtocol(query, limit)
			case "protocols":
				results, err = db.SearchProtocols(query, limit)
			case "summary":
				results, err = db.SearchSummary(query, limit)
			case "token-protocols":
				results, err = db.SearchTokenProtocols(query, limit)
			case "treasuries":
				results, err = db.SearchTreasuries(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchEntities(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search entities failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchMetrics(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search metrics failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProtocol(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search protocol failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchProtocols(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search protocols failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchSummary(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search summary failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTokenProtocols(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search token_protocols failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchTreasuries(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search treasuries failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
