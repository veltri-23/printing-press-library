...
Resource scoping:
  ...
  defillama-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  defillama-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  defillama-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  defillama-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  defillama-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("defillama-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/defillama-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "bridges":
		return db.UpsertBridges(data)
	case "chains":
		return db.UpsertChains(data)
	case "chart":
		return db.UpsertChart(data)
	case "chart_category":
		return db.UpsertChartCategory(data)
	case "chart_chain":
		return db.UpsertChartChain(data)
	case "chain_breakdown":
		return db.UpsertChainBreakdown(data)
	case "chart_protocol":
		return db.UpsertChartProtocol(data)
	case "protocol_breakdown":
		return db.UpsertProtocolBreakdown(data)
	case "dat":
		return db.UpsertDat(data)
	case "emissions":
		return db.UpsertEmissions(data)
	case "entities":
		return db.UpsertEntities(data)
	case "equities":
		return db.UpsertEquities(data)
	case "etfs":
		return db.UpsertEtfs(data)
	case "fdv":
		return db.UpsertFdv(data)
	case "hacks":
		return db.UpsertHacks(data)
	case "historical-chain-tvl":
		return db.UpsertHistoricalChainTvl(data)
	case "metrics":
		return db.UpsertMetrics(data)
	case "metrics_category":
		return db.UpsertMetricsCategory(data)
	case "metrics_chain":
		return db.UpsertMetricsChain(data)
	case "metrics_protocol":
		return db.UpsertMetricsProtocol(data)
	case "overview":
		return db.UpsertOverview(data)
	case "pools":
		return db.UpsertPools(data)
	case "protocol":
		return db.UpsertProtocol(data)
	case "protocols":
		return db.UpsertProtocols(data)
	case "rwa":
		return db.UpsertRwa(data)
	case "stablecoin":
		return db.UpsertStablecoin(data)
	case "stablecoinchains":
		return db.UpsertStablecoinchains(data)
	case "stablecoincharts":
		return db.UpsertStablecoincharts(data)
	case "stablecoinprices":
		return db.UpsertStablecoinprices(data)
	case "stablecoins":
		return db.UpsertStablecoins(data)
	case "summary":
		return db.UpsertSummary(data)
	case "token-protocols":
		return db.UpsertTokenProtocols(data)
	case "treasuries":
		return db.UpsertTreasuries(data)
	case "yields":
		return db.UpsertYields(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"chains",
		"chart",
		"chart-fork-protocol-breakdown",
		"chart-oracle-chain-breakdown",
		"chart-oracle-protocol-breakdown",
		"dat",
		"emissions",
		"entities",
		"equities",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"chain-assets":                    "/api/chainAssets",
		"chains":                          "/v2/chains",
		"chart":                           "/api/v2/chart/oracle",
		"chart-fork-protocol-breakdown":   "/api/v2/chart/fork/protocol-breakdown",
		"chart-oracle-chain-breakdown":    "/api/v2/chart/oracle/chain-breakdown",
		"chart-oracle-protocol-breakdown": "/api/v2/chart/oracle/protocol-breakdown",
		"dat":                             "/dat/institutions",
		"emissions":                       "/api/emissions",
		"entities":                        "/api/entities",
		"equities":                        "/equities/v1/companies",
		"etfs":                            "/etfs/flows",
		"etfs-snapshot":                   "/etfs/snapshot",
		"forks":                           "/api/forks",
		"hacks":                           "/api/hacks",
		"historical-chain-tvl":            "/v2/historicalChainTvl",
		"metrics":                         "/api/v2/metrics/fork",
		"metrics-oracle":                  "/api/v2/metrics/oracle",
		"oracles":                         "/api/oracles",
		"pools":                           "/pools",
		"protocols":                       "/protocols",
		"raises":                          "/api/raises",
		"rwa":                             "/rwa/list",
		"rwa-chart-chain-breakdown":       "/rwa/chart/chain-breakdown",
		"rwa-current":                     "/rwa/current",
		"rwa-stats":                       "/rwa/stats",
		...
		"yields-pools-old":                "/yields/poolsOld",
	}
