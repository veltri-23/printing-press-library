...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  weather-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  weather-goat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("weather-goat-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/weather-goat-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  weather-goat-pp-cli workflow status

  # Show status as JSON
  weather-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("weather-goat-pp-cli")
			}
