...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "redfin-pp-cli",
		Short: `Search Redfin homes from the terminal with offline sync, ranking, diff, sold comps, and market trends.`,
		Long: `Search Redfin homes from the terminal with offline sync, ranking, diff, sold comps, and market trends.
...
Health check: run 'redfin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("redfin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'redfin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCompsCmd(flags))
	rootCmd.AddCommand(newDropsCmd(flags))
	rootCmd.AddCommand(newSummaryCmd(flags))
	rootCmd.AddCommand(newTrendsCmd(flags))
	rootCmd.AddCommand(newAppreciationCmd(flags))
	rootCmd.AddCommand(newRedfinExportCmd(flags))
