...
Exit codes & warnings:
  ...
  redfin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  redfin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  redfin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  redfin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  redfin-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/redfin-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "homes":
		return db.UpsertHomes(data)
	case "market":
		return db.UpsertMarket(data)
	default:
