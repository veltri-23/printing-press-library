...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".redfin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("REDFIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("REDFIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "redfin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.redfin-pp-cli/feedback.jsonl.
When ` + "`REDFIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`REDFIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  redfin-pp-cli feedback list
  redfin-pp-cli feedback list --limit 5
  redfin-pp-cli feedback list --json`,
