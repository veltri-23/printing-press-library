...
func TestSearchListingsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("listings", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchListings(query, 10)
		if err != nil {
			t.Fatalf("SearchListings(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchListings(%q) returned no results", query)
		}
	}
}
