...
Resource scoping:
  ...
  motohunt-pp-cli sync

  # Sync specific resources only
  motohunt-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  motohunt-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  motohunt-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  motohunt-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  motohunt-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/motohunt-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "listings":
		...
		return db.UpsertListings(typedData)
	default:
