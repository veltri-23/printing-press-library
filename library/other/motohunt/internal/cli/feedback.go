...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "motohunt-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOTOHUNT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOTOHUNT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "motohunt-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/motohunt-pp-cli/feedback.jsonl.
When ` + "`MOTOHUNT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOTOHUNT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  motohunt-pp-cli feedback list
  motohunt-pp-cli feedback list --limit 5
  motohunt-pp-cli feedback list --json`,
