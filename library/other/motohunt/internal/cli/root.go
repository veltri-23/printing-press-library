...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "motohunt-pp-cli",
		Short: `Motohunt CLI — Search motorcycle and ATV listings from the terminal — with MotoHunt's MSRP/average-price/deal-rating data exposed as s…`,
		Long: `Motohunt CLI — Search motorcycle and ATV listings from the terminal — with MotoHunt's MSRP/average-price/deal-rating data exposed as s…
...
Health check: run 'motohunt-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("motohunt-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'motohunt-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.deliverSpec, "deliver", "", "Route output to a sink: stdout (default), file:<path>, webhook:<url>")
	rootCmd.PersistentFlags().Float64Var(&flags.rateLimit, "rate-limit", 0, "Max requests per second (0 to disable)")
	rootCmd.PersistentFlags().StringVar(&flags.site, "site", "moto", "Marketplace: moto (motohunt.com) or atv (atvhunt.com)")
	...
	// The generated `listings` endpoints do thin HTML link/page extraction and
	// are fully superseded by the richer hand-written `search` and `get`
	// commands (parsed cards + structured detail + price research). Keep them in
	// the binary but hidden so the CLI presents one clear search/get surface.
	listingsCmd := newListingsCmd(flags)
	listingsCmd.Hidden = true
	...
	// Hand-written rich goquery commands (Phase 3) — replace the generated
	// novel-command stubs with the real scraping implementations.
	rootCmd.AddCommand(newSearchCmd(flags))
	rootCmd.AddCommand(newGetCmd(flags))
	rootCmd.AddCommand(newMakesCmd(flags))
	rootCmd.AddCommand(newModelsCmd(flags))
	rootCmd.AddCommand(newDealCmd(flags))
