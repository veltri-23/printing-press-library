...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  motohunt-pp-cli doctor
  motohunt-pp-cli doctor --json
  motohunt-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/motorcycles-for-sale"
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "motohunt-pp-cli", suggestion)
						} else {
			...
			// Selector-health probe: fetch a known search on each site, parse
			// cards, and assert >=1 card with non-empty title+id. Catches
			// scrape drift (the rich goquery commands are selector-coupled).
			report["selectors"] = collectSelectorReport(cmd.Context(), flags)
			...
			if selAny, ok := report["selectors"]; ok {
				if selRep, ok := selAny.(map[string]any); ok {
					renderSelectorReport(w, selRep)
				}
			}
			return doctorExitForFailOn(failOn, report)
		},
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("motohunt-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'motohunt-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'motohunt-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'motohunt-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'motohunt-pp-cli sync' to refresh."
	}
	return report
}
