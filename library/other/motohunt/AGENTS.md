# MotoHunt Printed CLI Agent Guide

This directory is a generated `motohunt-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
motohunt-pp-cli doctor --json
motohunt-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
motohunt-pp-cli which "<capability>" --json
motohunt-pp-cli <command> --help
```
...
```bash
motohunt-pp-cli <command> --help
motohunt-pp-cli <command> --dry-run --agent
```
