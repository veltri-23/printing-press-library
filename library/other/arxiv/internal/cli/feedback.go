...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".arxiv-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARXIV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARXIV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "arxiv-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.arxiv-pp-cli/feedback.jsonl.
When ` + "`ARXIV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARXIV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  arxiv-pp-cli feedback list
  arxiv-pp-cli feedback list --limit 5
  arxiv-pp-cli feedback list --json`,
