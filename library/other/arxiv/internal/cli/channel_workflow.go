...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  arxiv-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  arxiv-pp-cli workflow archive --full`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/arxiv-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  arxiv-pp-cli workflow status

  # Show status as JSON
  arxiv-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("arxiv-pp-cli")
			}
