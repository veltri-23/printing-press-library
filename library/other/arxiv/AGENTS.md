# Arxiv Printed CLI Agent Guide

This directory is a generated `arxiv-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
arxiv-pp-cli doctor --json
arxiv-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
arxiv-pp-cli which "<capability>" --json
...
```bash
arxiv-pp-cli <command> --help
arxiv-pp-cli <command> --dry-run --agent
```
