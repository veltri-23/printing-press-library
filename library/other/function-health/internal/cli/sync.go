...
Resource scoping:
  ...
  function-health-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  function-health-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  function-health-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  function-health-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  function-health-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/function-health-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	// Some Function Health endpoints return a single structured envelope, or a
	// list of records with no extractable primary key:
	//   - results-report: `{"data":{"biomarkerResultsRecord":[...]}}`
	//   - recommendations: `{"type":"recommendations","recommendations":[...groups]}`
	//   - notes: `[{"requisition_id":...,"notes":[{"note":...,"category":...}]}]`
	//     (keyed by requisition_id, which the generic id-extractor doesn't look for)
	// The generic pagination/ID-extraction path can't store these — every "item"
	// lacks an extractable primary key — which is why the transcend commands that
	// read these resource types came up empty. Fetch once and store the whole body
	// under a fixed key.
	if resource == "results-report" || resource == "recommendations" || resource == "notes" {
		...
		// ReplaceSingleton (not Upsert) so the type holds exactly one row: an
		// earlier sync may have stored this payload under a different id via the
		// generic id-extraction fallback, and a second row would double-count
		// when loadAllResults concatenates every results-report row.
		if err := db.ReplaceSingleton(resource, "current", data); err != nil {
			return syncResult{Resource: resource, Err: fmt.Errorf("store %s: %w", resource, err), Duration: time.Since(started)}
		}
		_ = db.SaveSyncState(resource, "", 1)
		if !humanFriendly {
			fmt.Fprintf(syncEvents, `{"event":"sync_complete","resource":"%s","total":1,"duration_ms":%d}`+"\n", resource, time.Since(started).Milliseconds())
		}
		return syncResult{Resource: resource, Count: 1, Duration: time.Since(started)}
	}
...
func defaultSyncResources() []string {
	return []string{
		...
		"notifications",
		"recommendations",
		// results-report (the structured /results-report envelope) is the
		// headline endpoint every transcend command reads. The raw /results
		// list ("results") returns ID-less requisition docs that don't cache
		// and aren't queryable, so it is intentionally NOT in the default set.
		"results-report",
		"schedules",
		"visits",
	}
