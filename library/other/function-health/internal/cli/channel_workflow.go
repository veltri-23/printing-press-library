...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  function-health-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  function-health-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"biomarkers", "categories", "notes", "notifications", "recommendations", "results", "schedules", "visits"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  function-health-pp-cli workflow status

  # Show status as JSON
  function-health-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("function-health-pp-cli")
			}
