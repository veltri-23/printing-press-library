...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "function-health-pp-cli",
		Short: `Pull every lab round from your Function Health account, store every biomarker in a local SQLite with FTS5, compare biomarker drift across rounds, and export branded PDFs for your doctors.`,
		Long: `Pull every lab round from your Function Health account, store every biomarker in a local SQLite with FTS5, compare biomarker drift across rounds, and export branded PDFs for your doctors.
...
Highlights (not in the official API docs):
  ...
  • bundle   Composes a single Markdown file with a biomarker's full history, every clinician note mentioning it (FTS5), Function's optimal range, and relevant recommendations — ready to paste into Claude or Chat…
  • recommendations stale   Recommendations (supplements / foods) whose target biomarker is STILL outside Function's optimal range, joined by Quest code. The /recommendations endpoint has no issued-date, so staleness is by outc…

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'function-health-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("function-health-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'function-health-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBiologicalAgeCmd(flags))
	rootCmd.AddCommand(newBiomarkersCmd(flags))
	rootCmd.AddCommand(newRequisitionsCmd(flags))
	rootCmd.AddCommand(newResultsCmd(flags))
	...
	exportCmd := newExportCmd(flags)
	exportCmd.AddCommand(newNovelExportPdfForDoctorCmd(flags))
	...
	rootCmd.AddCommand(newNovelBiomarkerCmd(flags))
	rootCmd.AddCommand(newNovelBundleCmd(flags))
	rootCmd.AddCommand(newNovelCategoryCmd(flags))
	rootCmd.AddCommand(newNovelGoatCmd(flags))
	...
	rootCmd.AddCommand(newWearablesPromotedCmd(flags))
