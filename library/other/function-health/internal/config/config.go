...
type Config struct {
	...
	Path                string            `toml:"-"`
	FunctionHealthToken string            `toml:"health_token"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://member-app-mid.functionhealth.com/api/v1",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("FUNCTION_HEALTH_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "function-health-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("FUNCTION_HEALTH_TOKEN"); v != "" {
		cfg.FunctionHealthToken = v
		cfg.AuthSource = "env:FUNCTION_HEALTH_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.FunctionHealthToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("FUNCTION_HEALTH_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.FunctionHealthToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:FUNCTION_HEALTH_TOKEN"
		}
		return applyAuthFormat("Bearer {token}", map[string]string{
			"health_token":          c.FunctionHealthToken,
			"FUNCTION_HEALTH_TOKEN": c.FunctionHealthToken,
			"token":                 c.FunctionHealthToken,
		})
	}
	if c.AccessToken != "" {
