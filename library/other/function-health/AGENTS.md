# Function Health Printed CLI Agent Guide

This directory is a generated `function-health-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
function-health-pp-cli doctor --json
function-health-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
function-health-pp-cli which "<capability>" --json
function-health-pp-cli <command> --help
```
...
```bash
function-health-pp-cli <command> --agent
```
...
```bash
function-health-pp-cli <command> --help
function-health-pp-cli <command> --dry-run --agent
```
