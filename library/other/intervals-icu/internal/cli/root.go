...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "intervals-icu-pp-cli",
		Short: `Intervals Icu CLI — The first intervals.icu CLI with a local training database — offline search, SQL, and fitness/form analytics no other t…`,
		Long: `Intervals Icu CLI — The first intervals.icu CLI with a local training database — offline search, SQL, and fitness/form analytics no other t…
...
Health check: run 'intervals-icu-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("intervals-icu-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'intervals-icu-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelCurveCmd(flags))
	rootCmd.AddCommand(newNovelFormCmd(flags))
	rootCmd.AddCommand(newNovelGearCmd(flags))
	rootCmd.AddCommand(newNovelSinceCmd(flags))
	rootCmd.AddCommand(newNovelWellnessCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newAthletePlansPromotedCmd(flags))
	rootCmd.AddCommand(newDisconnectAppPromotedCmd(flags))
	rootCmd.AddCommand(newDownloadWorkoutExtPromotedCmd(flags))
	rootCmd.AddCommand(newPaceDistancesPromotedCmd(flags))
	rootCmd.AddCommand(newSharedEventPromotedCmd(flags))
