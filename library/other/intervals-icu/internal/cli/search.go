...
In local mode: searches locally synced data only.`,
  ...
  intervals-icu-pp-cli search "error timeout"

  # Force local search only
  intervals-icu-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  intervals-icu-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  intervals-icu-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /api/v1/athlete/{id}/activities/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/api/v1/athlete/{id}/activities/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'intervals-icu-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "activity":
				results, err = db.SearchActivity(query, limit)
			case "chats":
				results, err = db.SearchChats(query, limit)
			case "shared-event":
				results, err = db.SearchSharedEvent(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchActivity(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search activity failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchSharedEvent(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search shared_event failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
