...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  intervals-icu-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  intervals-icu-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"pace-distances"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  intervals-icu-pp-cli workflow status

  # Show status as JSON
  intervals-icu-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("intervals-icu-pp-cli")
			}
