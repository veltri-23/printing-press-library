...
Resource scoping:
  ...
  intervals-icu-pp-cli sync

  # Sync specific resources only
  intervals-icu-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  intervals-icu-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  intervals-icu-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  intervals-icu-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  intervals-icu-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("intervals-icu-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/intervals-icu-pp-cli/data.db)")
...
func syncResourceSinceParam(resource string) string {
	switch resource {
	case "activity", "wellness", "events":
		// intervals.icu date-range endpoints filter the start of the window
		// with `oldest`; newest defaults to now server-side.
		return "oldest"
	}
	return ""
}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "best_efforts":
		return db.UpsertBestEfforts(data)
	case "delete_intervals":
		return db.UpsertDeleteIntervals(data)
	case "file":
		return db.UpsertFile(data)
	case "fit_file":
		return db.UpsertFitFile(data)
	case "gap_histogram":
		return db.UpsertGapHistogram(data)
	case "gpx_file":
		return db.UpsertGpxFile(data)
	case "hr_curve_ext":
		return db.UpsertHrCurveExt(data)
	case "hr_histogram":
		return db.UpsertHrHistogram(data)
	case "hr_load_model":
		return db.UpsertHrLoadModel(data)
	case "interval_stats":
		return db.UpsertIntervalStats(data)
	case "intervals":
		return db.UpsertIntervals(data)
	case "map":
		return db.UpsertMap(data)
	case "activity_messages":
		return db.UpsertActivityMessages(data)
	case "pace_curve_ext":
		return db.UpsertPaceCurveExt(data)
	case "pace_histogram":
		return db.UpsertPaceHistogram(data)
	case "power_curve_ext":
		return db.UpsertPowerCurveExt(data)
	case "activity_power_curves_ext":
		return db.UpsertActivityPowerCurvesExt(data)
	case "power_histogram":
		return db.UpsertPowerHistogram(data)
	case "power_spike_model":
		return db.UpsertPowerSpikeModel(data)
	case "power_vs_hr_ext":
		return db.UpsertPowerVsHrExt(data)
	case "segments":
		return db.UpsertSegments(data)
	case "split_interval":
		return db.UpsertSplitInterval(data)
	case "streams":
		return db.UpsertStreams(data)
	case "streams_csv":
		return db.UpsertStreamsCsv(data)
	case "streams_ext":
		return db.UpsertStreamsExt(data)
	case "time_at_hr":
		return db.UpsertTimeAtHr(data)
	case "tombstone":
		return db.UpsertTombstone(data)
	case "weather_summary":
		return db.UpsertWeatherSummary(data)
	case "athlete":
		return db.UpsertAthlete(data)
	case "activities":
		return db.UpsertActivities(data)
	case "activities_around":
		return db.UpsertActivitiesAround(data)
	case "activities_csv":
		return db.UpsertActivitiesCsv(data)
	case "activity_hr_curves_ext":
		return db.UpsertActivityHrCurvesExt(data)
	case "activity_pace_curves_ext":
		return db.UpsertActivityPaceCurvesExt(data)
	case "activity_tags":
		return db.UpsertActivityTags(data)
	case "apply_plan_changes":
		return db.UpsertApplyPlanChanges(data)
	case "athlete_summary_ext":
		return db.UpsertAthleteSummaryExt(data)
	case "athlete_chats":
		return db.UpsertAthleteChats(data)
	case "custom_item":
		return db.UpsertCustomItem(data)
	case "custom_item_indexes":
		return db.UpsertCustomItemIndexes(data)
	case "download_fit_files":
		return db.UpsertDownloadFitFiles(data)
	case "athlete_download_workout_ext":
		return db.UpsertAthleteDownloadWorkoutExt(data)
	case "duplicate_events":
		return db.UpsertDuplicateEvents(data)
	case "duplicate_workouts":
		return db.UpsertDuplicateWorkouts(data)
	case "event_tags":
		return db.UpsertEventTags(data)
	case "events":
		return db.UpsertEvents(data)
	case "events_format":
		return db.UpsertEventsFormat(data)
	case "fitness_model_events":
		return db.UpsertFitnessModelEvents(data)
	case "folders":
		return db.UpsertFolders(data)
	case "gear":
		return db.UpsertGear(data)
	case "gear_ext":
		return db.UpsertGearExt(data)
	case "groups":
		return db.UpsertGroups(data)
	case "hr_curves_ext":
		return db.UpsertHrCurvesExt(data)
	case "mmp_model":
		return db.UpsertMmpModel(data)
	case "pace_curves_ext":
		return db.UpsertPaceCurvesExt(data)
	case "athlete_power_curves_ext":
		return db.UpsertAthletePowerCurvesExt(data)
	case "power_hr_curve":
		return db.UpsertPowerHrCurve(data)
	case "profile":
	...
	case "sport_settings":
		return db.UpsertSportSettings(data)
	case "training_plan":
		return db.UpsertTrainingPlan(data)
	case "weather_config":
		return db.UpsertWeatherConfig(data)
	case "weather_forecast":
		return db.UpsertWeatherForecast(data)
	case "wellness":
		return db.UpsertWellness(data)
	case "wellness_bulk":
		return db.UpsertWellnessBulk(data)
	case "wellness_ext":
		return db.UpsertWellnessExt(data)
	case "workout_tags":
		return db.UpsertWorkoutTags(data)
	case "workouts":
		return db.UpsertWorkouts(data)
	case "workouts_zip":
		return db.UpsertWorkoutsZip(data)
	case "chats":
		return db.UpsertChats(data)
	case "chats_messages":
		return db.UpsertChatsMessages(data)
	case "shared-event":
		return db.UpsertSharedEvent(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	// Athlete-scoped resources resolve {id} to the configured athlete (default
	// "0", which intervals.icu maps to the key owner). Resolving here lets the
	// generic sync engine populate the local training store for the resources
	// that matter; without it only the parameter-free /pace_distances syncs.
	aid := os.Getenv("INTERVALS_ICU_ATHLETE_ID")
	if aid == "" {
		aid = "0"
	}
	paths := map[string]string{
		"pace-distances": "/api/v1/pace_distances",
		"activity":       "/api/v1/athlete/" + aid + "/activities",
		"wellness":       "/api/v1/athlete/" + aid + "/wellness",
		"events":         "/api/v1/athlete/" + aid + "/events",
		"gear":           "/api/v1/athlete/" + aid + "/gear",
		"workouts":       "/api/v1/athlete/" + aid + "/workouts",
	}
