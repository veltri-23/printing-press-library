...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "intervals-icu-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("INTERVALS_ICU_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("INTERVALS_ICU_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "intervals-icu-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/intervals-icu-pp-cli/feedback.jsonl.
When ` + "`INTERVALS_ICU_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`INTERVALS_ICU_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  intervals-icu-pp-cli feedback list
  intervals-icu-pp-cli feedback list --limit 5
  intervals-icu-pp-cli feedback list --json`,
