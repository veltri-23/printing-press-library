...
type Config struct {
	...
	Path               string            `toml:"-"`
	IntervalsIcuApiKey string            `toml:"icu_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("INTERVALS_ICU_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "intervals-icu-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("INTERVALS_ICU_API_KEY"); v != "" {
		cfg.IntervalsIcuApiKey = v
		cfg.AuthSource = "env:INTERVALS_ICU_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.IntervalsIcuApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("INTERVALS_ICU_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.IntervalsIcuApiKey == "" {
		return ""
	}
	// intervals.icu HTTP Basic uses a constant username "API_KEY" with the
	// personal key as the password: base64("API_KEY:<key>").
	credentials := "API_KEY:" + c.IntervalsIcuApiKey
