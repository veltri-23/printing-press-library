# Intervals.icu Printed CLI Agent Guide

This directory is a generated `intervals-icu-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
intervals-icu-pp-cli doctor --json
intervals-icu-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
intervals-icu-pp-cli which "<capability>" --json
intervals-icu-pp-cli <command> --help
```
...
```bash
intervals-icu-pp-cli <command> --agent
```
...
```bash
intervals-icu-pp-cli <command> --help
intervals-icu-pp-cli <command> --dry-run --agent
```
