# Great Clips Printed CLI Agent Guide

This directory is a generated `greatclips-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
greatclips-pp-cli doctor --json
greatclips-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
greatclips-pp-cli which "<capability>" --json
greatclips-pp-cli <command> --help
```
...
```bash
greatclips-pp-cli <command> --agent
```
...
```bash
greatclips-pp-cli <command> --help
greatclips-pp-cli <command> --dry-run --agent
```
