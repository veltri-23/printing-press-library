...
type Client struct {
	...
	// PreRequestHook fires once per HTTP request after headers are
	// applied and before HTTPClient.Do. The hook may mutate req.URL,
	// headers, or body. Returning an error short-circuits the request.
	// Used to inject the ICS Net Check-In ?t=&s= signature on
	// stylewaretouch.net hosts; see internal/cli/icssign_hook.go.
	//
	// Hand-added extension point not emitted by the Printing Press
	// generator. See docs/REGEN-NOTES.md.
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "greatclips-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if c.Config != nil {
			...
			// Cookie-based auth: GreatClips uses HttpOnly session cookies
			// scoped per host (Auth0). Attach Config.HostCookies[host] as
			// the Cookie header so commands authenticate without a Bearer.
			if req.URL != nil {
				if cookieVal := c.Config.HostCookies[req.URL.Hostname()]; cookieVal != "" {
		...
		// PreRequestHook fires after auth/cookies/headers are set so it
		// can read the final outgoing state and mutate the URL or body
		// (e.g., add HMAC-signed query parameters).
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	// Run the PreRequestHook against a synthetic request so dry-run
	// output reflects URL mutations (e.g., HMAC-signed query params)
	// that the live path would apply. The hook reads body and
	// mutates req.URL.RawQuery; we then print the mutated URL.
	...
	if c.PreRequestHook != nil {
		...
		if synthReq, err := http.NewRequest(method, targetURL, bodyReader); err == nil {
			if hookErr := c.PreRequestHook(synthReq); hookErr == nil {
				// Mask the s= signature value; the actual signature
				// should never appear in logs or dry-run output.
				if maskedURL, ok := maskSigQueryParam(synthReq.URL.String()); ok {
					displayURL = maskedURL
				} else {
					displayURL = synthReq.URL.String()
				}
			}
		}
	}
...
func (c *Client) refreshAccessToken() error {
	...
	// PATCH(refresh-token-flow-not-wired): greatclips mints tokens via
	// the auth0silent /authorize flow and does not run an OAuth2
	// refresh_token exchange. The previous body left tokenURL hardcoded
	// to "" and short-circuited, so authHeader() silently returned a
	// stale token when the cached one expired (greptile P1). Surface
	// that explicitly so the user re-runs `greatclips auth login`
	// (which mints fresh cookies + a fresh token via auth0silent.Mint)
	// instead of seeing an opaque 401 from the next API call.
	return fmt.Errorf("greatclips access token expired and no refresh-token flow is configured; re-run `greatclips auth login` to mint a fresh token")
}
...
// maskSigQueryParam replaces the value of the "s" query parameter in
// the given URL with "****" while preserving all other params and the
// URL structure. Returns (maskedURL, true) when an "s" param was found
// and replaced, or (originalURL, false) otherwise.
//
// Used in --dry-run output so the HMAC signature never appears in logs
// or shell history.
func maskSigQueryParam(rawURL string) (string, bool) {
