...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".greatclips-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GREATCLIPS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GREATCLIPS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "greatclips-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.greatclips-pp-cli/feedback.jsonl.
When ` + "`GREATCLIPS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GREATCLIPS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  greatclips-pp-cli feedback list
  greatclips-pp-cli feedback list --limit 5
  greatclips-pp-cli feedback list --json`,
