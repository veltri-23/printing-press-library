...
type Config struct {
	...
	Path          string            `toml:"-"`
	NumistaApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("NUMISTA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "numista-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("NUMISTA_API_KEY"); v != "" {
		cfg.NumistaApiKey = v
		cfg.AuthSource = "env:NUMISTA_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.NumistaApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("NUMISTA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
