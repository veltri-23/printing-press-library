...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "numista-pp-cli", "http")
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			if h, err := c.authHeader(); err == nil && h != "" {
				req.Header.Set("Numista-API-Key", h)
			}
		}
		return nil
	}
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	if body != nil {
		...
		if err != nil {
			wrapped := fmt.Errorf("marshaling body: %w", err)
			logError(0, wrapped)
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if err != nil {
			wrapped := fmt.Errorf("creating request: %w", err)
			logError(0, wrapped)
			return nil, 0, wrapped
		}
		if bodyBytes != nil {
		...
		if authHeader != "" {
			req.Header.Set("Numista-API-Key", authHeader)
		}
		if c.Config != nil {
		...
		resp.Body.Close()
		if err != nil {
			wrapped := fmt.Errorf("reading response: %w", err)
			logError(resp.StatusCode, wrapped)
			return nil, 0, wrapped
		}
		if !binaryResponse {
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if authHeader != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "Numista-API-Key", maskToken(authHeader))
	}
