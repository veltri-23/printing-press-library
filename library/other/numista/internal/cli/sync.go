...
Resource scoping:
  ...
  numista-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  numista-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  numista-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  numista-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  numista-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// command timeout is ~30s. A full multi-resource paginated sync
			// against a real API never fits — force --latest-only + max-pages 1
			// + concurrency 1 so dogfood verifies the sync wiring works end-to-end
			// without actually fanning out across thousands of records. Real
			// `sync` invocations are untouched.
	...
	cmd.Flags().IntVar(&concurrency, "concurrency", 1, "Number of parallel sync workers (default 1 because Numista's free 2000/month quota makes parallelism cost-ineffective and SQLite's single-writer lock contends under default-4)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "mints":
		return db.UpsertMints(data)
	case "oauth-token":
		return db.UpsertOauthToken(data)
	case "types":
		return db.UpsertTypes(data)
	case "issues":
		return db.UpsertIssues(data)
	case "collected_items":
		return db.UpsertCollectedItems(data)
	case "collections":
		return db.UpsertCollections(data)
	default:
...
func defaultSyncResources() []string {
	// oauth-token is intentionally omitted: it is an auth-flow endpoint that
	// requires per-call parameters (code or scope) and is not a syncable
	// catalogue resource. It is still callable by name via
	// --resources=oauth-token when a user knows what they are doing.
	return []string{
		"catalogues",
		"issuers",
		"mints",
