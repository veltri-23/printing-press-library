...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".numista-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NUMISTA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NUMISTA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "numista-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.numista-pp-cli/feedback.jsonl.
When ` + "`NUMISTA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NUMISTA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  numista-pp-cli feedback list
  numista-pp-cli feedback list --limit 5
  numista-pp-cli feedback list --json`,
