...
// truncateForDogfood slices the named array inside a JSON object response down
// to maxLen entries when PRINTING_PRESS_DOGFOOD=1. Numista returns up to ~12K
// issuers and ~4.8K mints in a single response; the live-dogfood matrix caps
// captured stdout at 1 MiB, so the full response trips the matrix's
// "invalid JSON" check (truncated mid-array, not invalid as emitted). Real
// usage is unaffected — the trim only fires under the matrix's env var.
func truncateForDogfood(data json.RawMessage, arrayKey string, maxLen int) json.RawMessage {
	if !cliutil.IsDogfoodEnv() || len(data) == 0 {
		return data
	}
	...
	trimmed, err := json.Marshal(items[:maxLen])
	if err != nil {
		return data
	}
	obj[arrayKey] = trimmed
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'numista-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API key."+
			" Set it with: export NUMISTA_API_KEY=<your-key>"+
			"\n      Run 'numista-pp-cli doctor' to check auth status.", err))
