# Numista Printed CLI Agent Guide

This directory is a generated `numista-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
numista-pp-cli doctor --json
numista-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
numista-pp-cli which "<capability>" --json
...
```bash
numista-pp-cli <command> --help
numista-pp-cli <command> --dry-run --agent
```
