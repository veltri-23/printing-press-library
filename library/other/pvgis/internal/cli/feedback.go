...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".pvgis-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PVGIS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PVGIS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "pvgis-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.pvgis-pp-cli/feedback.jsonl.
When ` + "`PVGIS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PVGIS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  pvgis-pp-cli feedback list
  pvgis-pp-cli feedback list --limit 5
  pvgis-pp-cli feedback list --json`,
