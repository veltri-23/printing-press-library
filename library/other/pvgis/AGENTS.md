# PVGIS Printed CLI Agent Guide

This directory is a generated `pvgis-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
pvgis-pp-cli doctor --json
pvgis-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
pvgis-pp-cli which "<capability>" --json
pvgis-pp-cli <command> --help
```
...
```bash
pvgis-pp-cli <command> --help
pvgis-pp-cli <command> --dry-run --agent
```
