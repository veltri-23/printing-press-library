...
Resource scoping:
  ...
  nhc-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nhc-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nhc-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nhc-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nhc-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nhc-pp-cli/data.db)")
