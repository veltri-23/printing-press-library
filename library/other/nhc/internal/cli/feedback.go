...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "nhc-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NHC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NHC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nhc-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/nhc-pp-cli/feedback.jsonl.
When ` + "`NHC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NHC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nhc-pp-cli feedback list
  nhc-pp-cli feedback list --limit 5
  nhc-pp-cli feedback list --json`,
