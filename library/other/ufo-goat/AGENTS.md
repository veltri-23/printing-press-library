# Ufo Goat Printed CLI Agent Guide

This directory is a generated `ufo-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ufo-goat-pp-cli doctor --json
ufo-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ufo-goat-pp-cli which "<capability>" --json
...
```bash
ufo-goat-pp-cli <command> --help
ufo-goat-pp-cli <command> --dry-run --agent
```
