...
In local mode: searches locally synced data only.`,
  ...
  american-reindustrialization-pp-cli search "error timeout"

  # Force local search only
  american-reindustrialization-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  american-reindustrialization-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  american-reindustrialization-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /tags/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/tags/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'american-reindustrialization-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "categories":
				results, err = db.SearchCategories(query, limit)
			case "openings":
				results, err = db.SearchOpenings(query, limit)
			case "tags":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchCategories(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search categories failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchOpenings(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search openings failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/american-reindustrialization-pp-cli/data.db)")
