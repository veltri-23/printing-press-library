...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "american-reindustrialization-pp-cli",
		Short: `Browse, slice, and analyze the curated American Reindustrialization directory — with diffs, geo clusters, sector heatmaps, and offline SQL no website view shows.`,
		Long: `Browse, slice, and analyze the curated American Reindustrialization directory — with diffs, geo clusters, sector heatmaps, and offline SQL no website view shows.
...
Health check: run 'american-reindustrialization-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("american-reindustrialization-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'american-reindustrialization-pp-cli profile list')")
	...
	companiesCmd := newCompaniesCmd(flags)
	companiesCmd.Hidden = false
	companiesCmd.AddCommand(newCompaniesTopHiringCmd(flags))
	companiesCmd.AddCommand(newCompaniesProfileCmd(flags))
	companiesCmd.AddCommand(newCompaniesCohortsCmd(flags))
	rootCmd.AddCommand(companiesCmd)
	openingsCmd := newOpeningsCmd(flags)
	openingsCmd.Hidden = false
	openingsCmd.AddCommand(newOpeningsFindCmd(flags))
	openingsCmd.AddCommand(newOpeningsSalaryStatsCmd(flags))
	rootCmd.AddCommand(openingsCmd)
	rootCmd.AddCommand(newTagsCmd(flags))
	analyticsCmd := newAnalyticsCmd(flags)
	analyticsCmd.AddCommand(newAnalyticsSectorHeatmapCmd(flags))
	analyticsCmd.AddCommand(newAnalyticsFundingBySectorCmd(flags))
	analyticsCmd.AddCommand(newAnalyticsGeoClustersCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("american-reindustrialization-pp-cli %s\n", version)
		},
	}
}
