...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  american-reindustrialization-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  american-reindustrialization-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// syncResource writes per-event NDJSON to os.Stdout. With --json
			// the caller expects a single parseable JSON document on stdout;
			// route those events to stderr for the duration of the sync
			// calls so the only thing on stdout is the final summary.
			origStdout := os.Stdout
			if flags.asJSON {
				os.Stdout = os.Stderr
			}

			resources := []string{"categories", "companies", "news", "openings", "openings-categories", "openings-companies", "openings-tags", "openings-titles", "tags"}
			totalSynced := 0
			...
			if flags.asJSON {
				// Restore stdout for the summary so the JSON document lands
				// on the caller's stdout, not the stderr redirect above.
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  american-reindustrialization-pp-cli workflow status

  # Show status as JSON
  american-reindustrialization-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("american-reindustrialization-pp-cli")
			}
