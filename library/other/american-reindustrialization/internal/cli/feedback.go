...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".american-reindustrialization-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("AMERICAN_REINDUSTRIALIZATION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("AMERICAN_REINDUSTRIALIZATION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "american-reindustrialization-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.american-reindustrialization-pp-cli/feedback.jsonl.
When ` + "`AMERICAN_REINDUSTRIALIZATION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`AMERICAN_REINDUSTRIALIZATION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "american-reindustrialization-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  american-reindustrialization-pp-cli feedback list
  american-reindustrialization-pp-cli feedback list --limit 5
  american-reindustrialization-pp-cli feedback list --json`,
