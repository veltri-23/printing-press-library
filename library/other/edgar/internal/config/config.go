...
type Config struct {
	...
	Path                  string            `toml:"-"`
	CompanyPpContactEmail string            `toml:"pp_contact_email"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("EDGAR_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "edgar-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("COMPANY_PP_CONTACT_EMAIL"); v != "" {
		cfg.CompanyPpContactEmail = v
		cfg.AuthSource = "env:COMPANY_PP_CONTACT_EMAIL"
	}
	...
	if cfg.AuthSource == "" && cfg.CompanyPpContactEmail != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("EDGAR_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	replacements := map[string]string{
		"email":                    token,
		"token":                    token,
		"pp_contact_email":         c.CompanyPpContactEmail,
		"COMPANY_PP_CONTACT_EMAIL": c.CompanyPpContactEmail,
	}
	return applyAuthFormat("lodestar-edgar-pp-cli {email}", replacements)
}
