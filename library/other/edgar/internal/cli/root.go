...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "edgar-pp-cli",
		Short: `Agent-native CLI for SEC EDGAR — primary-source bundles and insider-summary shaped for LODESTAR conviction research`,
		Long: `Agent-native CLI for SEC EDGAR — primary-source bundles and insider-summary shaped for LODESTAR conviction research
...
Health check: run 'edgar-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("edgar-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'edgar-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCompanyfactsPromotedCmd(flags))
	rootCmd.AddCommand(newEftsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-built EDGAR commands (LODESTAR value-add)
	rootCmd.AddCommand(newAccessionCmd(flags))
	rootCmd.AddCommand(newSinceCmd(flags))
	rootCmd.AddCommand(newEightKItemsCmd(flags))
	rootCmd.AddCommand(newSectionsCmd(flags))
	rootCmd.AddCommand(newFTSCmd(flags))
	rootCmd.AddCommand(newXBRLPivotCmd(flags))
	rootCmd.AddCommand(newOwnershipCrossesCmd(flags))
	rootCmd.AddCommand(newGovernanceFlagsCmd(flags))
	rootCmd.AddCommand(newInsiderSummaryCmd(flags))
	rootCmd.AddCommand(newInsiderFollowthroughCmd(flags))
	rootCmd.AddCommand(newPrimarySourcesCmd(flags))
	rootCmd.AddCommand(newFilingsByTickerCmd(flags))
