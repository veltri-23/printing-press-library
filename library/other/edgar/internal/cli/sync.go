...
Exit codes & warnings:
  ...
  edgar-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  edgar-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  edgar-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  edgar-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  edgar-pp-cli sync --latest-only

  # Sync historical data for a date range
  edgar-pp-cli sync --dates 20250101-20250131`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/edgar-pp-cli/data.db)")
