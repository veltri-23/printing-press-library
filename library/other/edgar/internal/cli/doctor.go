...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  edgar-pp-cli doctor
  edgar-pp-cli doctor --json
  edgar-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export COMPANY_PP_CONTACT_EMAIL=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("COMPANY_PP_CONTACT_EMAIL") != "" {
				authEnvSet = append(authEnvSet, "COMPANY_PP_CONTACT_EMAIL")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "COMPANY_PP_CONTACT_EMAIL")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/submissions/CIK0000320193.json"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("edgar-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'edgar-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'edgar-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'edgar-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'edgar-pp-cli sync' to refresh."
	}
	return report
}
