...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".edgar-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EDGAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EDGAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "edgar-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.edgar-pp-cli/feedback.jsonl.
When ` + "`EDGAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EDGAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  edgar-pp-cli feedback list
  edgar-pp-cli feedback list --limit 5
  edgar-pp-cli feedback list --json`,
