...
func TestLooksLikeJWT(t *testing.T) {
	...
	// syntheticRS256 is a realistically-sized RS256 token (header ~64 chars,
	// payload ~430 chars, signature ~344 chars). Synthetic — not a credential.
	syntheticRS256 := syntheticJWT()
	...
	cases := []struct {
		...
		{"synthetic RS256 token", syntheticRS256, true},
		{"exactly at 150-char floor", atFloor150, true},
		{"realistic minimum HS256", minimalHS256, true},
		{"with Bearer prefix", "Bearer " + syntheticRS256, true},
...
func syntheticJWT() string {
	header := strings.Join([]string{
		"eyJhbGciOiJS",
		"UzI1NiIsInR5cCI6IkpXVCIsImtpZCI6",
		"ImFvNi1PYzAyQlZHTF9GbnBDeE5JMiJ9",
	}, "")
	return header + "." + strings.Repeat("a", 430) + "." + strings.Repeat("b", 344)
}
