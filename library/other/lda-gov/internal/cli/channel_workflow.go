...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  lda-gov-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  lda-gov-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"clients", "constants", "constants-contribution-itemtypes", "constants-filing-filingtypes", "constants-filing-governmententities", "constants-filing-lobbyingactivityissues", "constants-general-countries", "constants-lobbyist-prefixes", "constants-lobbyist-suffixes", "filings", "lobbyists", "registrants"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  lda-gov-pp-cli workflow status

  # Show status as JSON
  lda-gov-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("lda-gov-pp-cli")
			}
