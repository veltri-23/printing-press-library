...
import (
	...
	"encoding/json"
	"fmt"
	"github.com/mvanhorn/printing-press-library/library/other/lda-gov/internal/cliutil"
...
Resource scoping:
  ...
  lda-gov-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  lda-gov-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  lda-gov-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  lda-gov-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  lda-gov-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("lda-gov-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/lda-gov-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "constants":
		return db.UpsertConstants(data)
	case "contributions":
		return db.UpsertContributions(data)
	case "filings":
		return db.UpsertFilings(data)
	case "lobbyists":
		return db.UpsertLobbyists(data)
	case "registrants":
		return db.UpsertRegistrants(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"constants":                               "/api/v1/constants/general/states/",
