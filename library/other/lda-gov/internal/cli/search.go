...
In local mode: searches locally synced data only.`,
  ...
  lda-gov-pp-cli search "error timeout"

  # Force local search only
  lda-gov-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  lda-gov-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  lda-gov-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'lda-gov-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "registrants":
				results, err = db.SearchRegistrants(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchRegistrants(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search registrants failed: %w", searchErr)
					}
