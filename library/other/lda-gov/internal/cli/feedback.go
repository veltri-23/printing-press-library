...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "lda-gov-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LDA_GOV_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LDA_GOV_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "lda-gov-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/lda-gov-pp-cli/feedback.jsonl.
When ` + "`LDA_GOV_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LDA_GOV_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  lda-gov-pp-cli feedback list
  lda-gov-pp-cli feedback list --limit 5
  lda-gov-pp-cli feedback list --json`,
