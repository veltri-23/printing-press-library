# LDA.gov Printed CLI Agent Guide

This directory is a generated `lda-gov-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
lda-gov-pp-cli doctor --json
lda-gov-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
lda-gov-pp-cli which "<capability>" --json
lda-gov-pp-cli <command> --help
```
...
```bash
lda-gov-pp-cli <command> --help
lda-gov-pp-cli <command> --dry-run --agent
```
