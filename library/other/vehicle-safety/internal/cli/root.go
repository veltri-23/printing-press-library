...
type rootFlags struct {
	...
	// invocation. Mirrors the VEHICLE_SAFETY_NO_LEARN env var.
	noLearn       bool
	selectFields  string
	configPath    string
	homePath      string
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "vehicle-safety-pp-cli",
		Short: `Vehicle Safety CLI — Build source-attributed vehicle safety dossiers, honest comparisons, and recall-change reports from NHTSA data.`,
		Long: `Vehicle Safety CLI — Build source-attributed vehicle safety dossiers, honest comparisons, and recall-change reports from NHTSA data.
...
Health check: run 'vehicle-safety-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("vehicle-safety-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'vehicle-safety-pp-cli profile list')")
	...
	rootCmd.AddCommand(newRecallsCmd(flags))
	rootCmd.AddCommand(newSafetyRatingsCmd(flags))
	...
	rootCmd.AddCommand(newNovelBulletinBridgeCmd(flags))
	rootCmd.AddCommand(newNovelCompareCmd(flags))
	rootCmd.AddCommand(newNovelDossierCmd(flags))
	rootCmd.AddCommand(newNovelRecallCoverageCmd(flags))
	rootCmd.AddCommand(newNovelSignalsCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newComplaintsPromotedCmd(flags))
	rootCmd.AddCommand(newVehiclesPromotedCmd(flags))
