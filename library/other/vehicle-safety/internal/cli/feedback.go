...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("VEHICLE_SAFETY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("VEHICLE_SAFETY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "vehicle-safety-pp-cli/feedback")
...
When ` + "`VEHICLE_SAFETY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`VEHICLE_SAFETY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  vehicle-safety-pp-cli feedback list
  vehicle-safety-pp-cli feedback list --limit 5
  vehicle-safety-pp-cli feedback list --json`,
