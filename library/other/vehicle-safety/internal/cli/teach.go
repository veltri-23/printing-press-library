...
// vehicle-safety-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/other/vehicle-safety/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/other/vehicle-safety/internal/learn/patterns"
...
const noLearnEnvVar = "VEHICLE_SAFETY_NO_LEARN"
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("vehicle-safety-pp-cli")
}
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  vehicle-safety-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  vehicle-safety-pp-cli recall "<question>" --agent
  vehicle-safety-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List recorded search learnings and their stored resource mappings",
		Example: `  vehicle-safety-pp-cli learnings list --agent
  vehicle-safety-pp-cli learnings list --query "<substring>"
  vehicle-safety-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  vehicle-safety-pp-cli learnings forget "<question>" --resource <id>
  vehicle-safety-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
a whole family.`,
		Example: `  vehicle-safety-pp-cli teach-pattern \
...
cannot be taught — they are derived from the canonical input.`,
		Example: `  vehicle-safety-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
