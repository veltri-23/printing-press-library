...
Run sync first to populate the local search index.`,
  ...
  vehicle-safety-pp-cli search "status"

  # Force local search explicitly
  vehicle-safety-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  vehicle-safety-pp-cli search "status" --type complaints --data-source local
  # JSON output for piping
  vehicle-safety-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'vehicle-safety-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
