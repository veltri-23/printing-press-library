...
Resource scoping:
  ...
  from a prior sync.`,
		Example: `  # Sync complaints for one concrete vehicle query
	  vehicle-safety-pp-cli sync --resources complaints --resource-param complaints:modelYear=2020 --resource-param complaints:make=Honda --resource-param complaints:model=Civic

  # Sync specific resources only
  vehicle-safety-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  vehicle-safety-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  vehicle-safety-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  vehicle-safety-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  vehicle-safety-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "complaints":
		return db.UpsertComplaints(data)
	case "recalls":
		return db.UpsertRecalls(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{ // #nosec G101 -- endpoint paths, not credentials.
		...
		"products":                     "/products/vehicle/makes",
		"products-vehicle-model-years": "/products/vehicle/modelYears",
		"products-vehicle-models":      "/products/vehicle/models",
		"recalls":                      "/recalls/campaignNumber",
		"recalls-recalls-by-vehicle":   "/recalls/recallsByVehicle",
	}
...
func responsePathForResource(resource, path string) []string {
	...
	case "complaints\x00/complaints/complaintsByVehicle":
		return []string{"results"}
	case "recalls\x00/recalls/campaignNumber":
		return []string{"results"}
	case "recalls\x00/recalls/recallsByVehicle":
		return []string{"results"}
	}
	return nil
}
