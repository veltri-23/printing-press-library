...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  vehicle-safety-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  vehicle-safety-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"products-vehicle-model-years"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  vehicle-safety-pp-cli workflow status

  # Show status as JSON
  vehicle-safety-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("vehicle-safety-pp-cli")
			}
