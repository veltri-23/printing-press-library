...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "ars-sicilia-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARS_SICILIA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARS_SICILIA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ars-sicilia-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/ars-sicilia-pp-cli/feedback.jsonl.
When ` + "`ARS_SICILIA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARS_SICILIA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ars-sicilia-pp-cli feedback list
  ars-sicilia-pp-cli feedback list --limit 5
  ars-sicilia-pp-cli feedback list --json`,
