...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "gisis-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GISIS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GISIS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "gisis-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/gisis-pp-cli/feedback.jsonl.
When ` + "`GISIS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GISIS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  gisis-pp-cli feedback list
  gisis-pp-cli feedback list --limit 5
  gisis-pp-cli feedback list --json`,
