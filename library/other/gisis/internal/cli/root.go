...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "gisis-pp-cli",
		Short: `Authoritative IMO ship particulars on the command line, plus a local cache that turns one-shot lookups into a compounding vessel index.`,
		Long: `Authoritative IMO ship particulars on the command line, plus a local cache that turns one-shot lookups into a compounding vessel index.
...
Health check: run 'gisis-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("gisis-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'gisis-pp-cli profile list')")
	...
	rootCmd.AddCommand(newOwnerParentCmd(flags))
	rootCmd.AddCommand(newShipParentCmd(flags))
