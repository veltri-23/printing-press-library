# GISIS Printed CLI Agent Guide

This directory is a generated `gisis-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
gisis-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
gisis-pp-cli which "<capability>" --json
gisis-pp-cli <command> --help
```
...
```bash
gisis-pp-cli <command> --help
gisis-pp-cli <command> --dry-run --agent
```
