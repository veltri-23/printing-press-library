...
Resource scoping:
  ...
  nypl-digital-collections-pp-cli sync

  # Sync specific resources only
  nypl-digital-collections-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nypl-digital-collections-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nypl-digital-collections-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nypl-digital-collections-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nypl-digital-collections-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nypl-digital-collections-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "collections_items":
		return db.UpsertCollectionsItems(data)
	case "subcollections":
		return db.UpsertSubcollections(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "collections_items", ParentTable: "collections", ParentIDParam: "uuid", PathTemplate: "/collections/{uuid}/items", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "uuid", Field: "uuid"},
		}},
		{Name: "subcollections", ParentTable: "collections", ParentIDParam: "uuid", PathTemplate: "/collections/{uuid}/subcollections", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "uuid", Field: "uuid"},
		}},
	}
}
