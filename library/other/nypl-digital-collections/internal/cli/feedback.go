...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "nypl-digital-collections-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NYPL_DIGITAL_COLLECTIONS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NYPL_DIGITAL_COLLECTIONS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nypl-digital-collections-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/nypl-digital-collections-pp-cli/feedback.jsonl.
When ` + "`NYPL_DIGITAL_COLLECTIONS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NYPL_DIGITAL_COLLECTIONS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "nypl-digital-collections-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nypl-digital-collections-pp-cli feedback list
  nypl-digital-collections-pp-cli feedback list --limit 5
  nypl-digital-collections-pp-cli feedback list --json`,
