...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nypl-digital-collections-pp-cli doctor
  nypl-digital-collections-pp-cli doctor --json
  nypl-digital-collections-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export NYPL_DIGITAL_COLLECTIONS_NYPL_TOKEN=<your-key>"
				} else {
					authConfigured = true
			...
			if os.Getenv("NYPL_DIGITAL_COLLECTIONS_NYPL_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "NYPL_DIGITAL_COLLECTIONS_NYPL_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NYPL_DIGITAL_COLLECTIONS_NYPL_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "nypl-digital-collections-pp-cli", suggestion)
						} else {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("nypl-digital-collections-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'nypl-digital-collections-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'nypl-digital-collections-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'nypl-digital-collections-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'nypl-digital-collections-pp-cli sync' to refresh."
	}
	return report
}
