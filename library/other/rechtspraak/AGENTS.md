# Rechtspraak Printed CLI Agent Guide

This directory is a generated `rechtspraak-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
rechtspraak-pp-cli doctor --json
rechtspraak-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
rechtspraak-pp-cli which "<capability>" --json
rechtspraak-pp-cli <command> --help
```
...
```bash
rechtspraak-pp-cli <command> --agent
```
...
```bash
rechtspraak-pp-cli <command> --help
rechtspraak-pp-cli <command> --dry-run --agent
```
