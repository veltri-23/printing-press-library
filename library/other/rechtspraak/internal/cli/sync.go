...
Resource scoping:
  ...
  rechtspraak-pp-cli sync

  # Sync specific resources only
  rechtspraak-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  rechtspraak-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  rechtspraak-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  rechtspraak-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  rechtspraak-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("rechtspraak-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/rechtspraak-pp-cli/data.db)")
	...
	// Register the novel `sync archive` subcommand. The implementation in
	// sync_archive.go is currently a deferred-to-v0.2 stub (the archive
	// ZIP format is not yet probed), but the command needs to be wired so
	// it appears in help output and matches README/SKILL references that
	// document `sync archive` as part of the value proposition.
	cmd.AddCommand(newNovelSyncArchiveCmd(flags))
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "courts":
		return db.UpsertCourts(data)
	case "foreign-courts":
		return db.UpsertForeignCourts(data)
	case "procedures":
		return db.UpsertProcedures(data)
	case "relations":
		return db.UpsertRelations(data)
	case "subjects":
		return db.UpsertSubjects(data)
	case "uitspraken":
		return db.UpsertUitspraken(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"courts":            "/Waardelijst/Instanties",
		"foreign-courts":    "/Waardelijst/InstantiesBuitenlands",
		"foreign-decisions": "/Waardelijst/NietNederlandseUitspraken",
		"procedures":        "/Waardelijst/Proceduresoorten",
		"relations":         "/Waardelijst/FormeleRelaties",
		"subjects":          "/Waardelijst/Rechtsgebieden",
		"uitspraken":        "/uitspraken/zoeken",
	}
