...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  rechtspraak-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  rechtspraak-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"courts", "foreign-courts", "foreign-decisions", "procedures", "relations", "subjects", "uitspraken"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  rechtspraak-pp-cli workflow status

  # Show status as JSON
  rechtspraak-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("rechtspraak-pp-cli")
			}
