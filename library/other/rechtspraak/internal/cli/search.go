...
In local mode: searches locally synced data only.`,
  ...
  rechtspraak-pp-cli search "error timeout"

  # Force local search only
  rechtspraak-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  rechtspraak-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  rechtspraak-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /uitspraken/zoeken
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/uitspraken/zoeken", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'rechtspraak-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "uitspraken":
				results, err = db.SearchUitspraken(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchUitspraken(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search uitspraken failed: %w", searchErr)
					}
