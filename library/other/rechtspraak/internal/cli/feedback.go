...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "rechtspraak-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RECHTSPRAAK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RECHTSPRAAK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rechtspraak-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/rechtspraak-pp-cli/feedback.jsonl.
When ` + "`RECHTSPRAAK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RECHTSPRAAK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rechtspraak-pp-cli feedback list
  rechtspraak-pp-cli feedback list --limit 5
  rechtspraak-pp-cli feedback list --json`,
