...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rechtspraak-pp-cli",
		Short: `Rechtspraak CLI — The first CLI for Dutch court decisions with rich local narrowing for long-tail legal keywords, an appeal-chain walker,…`,
		Long: `Rechtspraak CLI — The first CLI for Dutch court decisions with rich local narrowing for long-tail legal keywords, an appeal-chain walker,…
...
Highlights (not in the official API docs):
  ...
  • narrow   Read an ECLI list from stdin (or piped from search/watch/sync), apply local include/exclude/phrase/regex filters against the synced FTS5 index, and emit the narrowed list — the defining workflow for …

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'rechtspraak-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rechtspraak-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'rechtspraak-pp-cli profile list')")
	...
	rootCmd.AddCommand(newUitsprakenCmd(flags))
	...
	rootCmd.AddCommand(newNovelChainCmd(flags))
	rootCmd.AddCommand(newNovelCitationsCmd(flags))
	rootCmd.AddCommand(newNovelCodeCmd(flags))
	rootCmd.AddCommand(newNovelConclusieCmd(flags))
	rootCmd.AddCommand(newNovelDossierCmd(flags))
	rootCmd.AddCommand(newNovelEcliCmd(flags))
	rootCmd.AddCommand(newNovelMcpCmd(flags))
	rootCmd.AddCommand(newNovelNarrowCmd(flags))
	rootCmd.AddCommand(newNovelWatchCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newCourtsPromotedCmd(flags))
	rootCmd.AddCommand(newForeignCourtsPromotedCmd(flags))
	rootCmd.AddCommand(newForeignDecisionsPromotedCmd(flags))
	rootCmd.AddCommand(newProceduresPromotedCmd(flags))
	rootCmd.AddCommand(newRelationsPromotedCmd(flags))
	rootCmd.AddCommand(newSubjectsPromotedCmd(flags))
