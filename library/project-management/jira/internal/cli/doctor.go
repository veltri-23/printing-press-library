...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jira-pp-cli doctor
  jira-pp-cli doctor --json
  jira-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export JIRA_API_TOKEN=<token> JIRA_EMAIL=<email> JIRA_BASE_URL=https://your-domain.atlassian.net"
					report["auth_docs_url"] = "http://www.atlassian.com"
				} else {
					authConfigured = true
			...
			if os.Getenv("JIRA_API_TOKEN") != "" && os.Getenv("JIRA_EMAIL") != "" {
				authEnvSet = append(authEnvSet, "JIRA_API_TOKEN", "JIRA_EMAIL")
			} else if os.Getenv("JIRA_OAUTH2") != "" {
				authEnvSet = append(authEnvSet, "JIRA_OAUTH2")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "JIRA_API_TOKEN+JIRA_EMAIL or JIRA_OAUTH2")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'jira-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'jira-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'jira-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'jira-pp-cli sync' to refresh."
	}
	return report
}
