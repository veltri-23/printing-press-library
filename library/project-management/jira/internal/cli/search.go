...
In local mode: searches locally synced data only.`,
  ...
  jira-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// PATCH: search-endpoint-410 — old /rest/api/3/search returns 410; use JQL endpoint
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/rest/api/3/search/jql", map[string]string{
					"jql":        "text ~ \"" + query + "\" ORDER BY updated DESC",
					"maxResults": "25",
					"fields":     "summary,status,issuetype,project,assignee,priority,updated",
				})
				if getErr == nil {
