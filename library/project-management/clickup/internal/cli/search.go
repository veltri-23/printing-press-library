...
In local mode: searches locally synced data only.`,
  ...
  clickup-pp-cli search "error timeout"

  # Force local search only
  clickup-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  clickup-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  clickup-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v3/workspaces/{workspace_id}/docs
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get("/v3/workspaces/{workspace_id}/docs", map[string]string{
					"q": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'clickup-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "list":
				results, err = db.SearchList(query, limit)
			case "task":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchList(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search list failed: %w", searchErr)
					}
