...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".clickup-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CLICKUP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CLICKUP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "clickup-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.clickup-pp-cli/feedback.jsonl.
When ` + "`CLICKUP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CLICKUP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  clickup-pp-cli feedback list
  clickup-pp-cli feedback list --limit 5
  clickup-pp-cli feedback list --json`,
