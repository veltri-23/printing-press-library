...
type Config struct {
	...
	Path                      string            `toml:"-"`
	ClickupAuthorizationToken string            `toml:"authorization_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CLICKUP_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "clickup-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("CLICKUP_AUTHORIZATION_TOKEN"); v != "" {
		cfg.ClickupAuthorizationToken = v
		cfg.AuthSource = "env:CLICKUP_AUTHORIZATION_TOKEN"
	}
	...
	if cfg.AuthSource == "" && cfg.ClickupAuthorizationToken != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("CLICKUP_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.ClickupAuthorizationToken
	if token == "" {
		return ""
	}
	if c.ClickupAuthorizationToken == "" {
		return ""
	}
	return token
}
...
func (c *Config) SaveCredential(token string) error {
	c.AuthHeaderVal = ""
	c.AccessToken = ""
	c.ClickupAuthorizationToken = token
	return c.save()
}
