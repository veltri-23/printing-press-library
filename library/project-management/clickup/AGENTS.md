# ClickUp Printed CLI Agent Guide

This directory is a generated `clickup-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
clickup-pp-cli doctor --json
clickup-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
clickup-pp-cli which "<capability>" --json
clickup-pp-cli <command> --help
```
...
```bash
clickup-pp-cli <command> --help
clickup-pp-cli <command> --dry-run --agent
```
