...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".linear-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("LINEAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("LINEAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "linear-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.linear-pp-cli/feedback.jsonl.
When ` + "`LINEAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`LINEAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  linear-pp-cli feedback list
  linear-pp-cli feedback list --limit 5
  linear-pp-cli feedback list --json`,
