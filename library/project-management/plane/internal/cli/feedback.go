...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "plane-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PLANE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PLANE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "plane-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/plane-pp-cli/feedback.jsonl.
When ` + "`PLANE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PLANE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  plane-pp-cli feedback list
  plane-pp-cli feedback list --limit 5
  plane-pp-cli feedback list --json`,
