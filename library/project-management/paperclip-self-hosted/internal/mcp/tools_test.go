...
func TestMCPPathResolutionMatchesCLIResolverWithPlatformDefaults(t *testing.T) {
	...
	if want := filepath.Join(home, ".config", "paperclip-pp-cli", "config.toml"); cfg.Path != want {
	...
	if want := filepath.Join(home, ".local", "share", "paperclip-pp-cli", "data.db"); gotDB != want {
...
func TestMCPRegisterToolsPreservesTypedSpecialTools(t *testing.T) {
	s := server.NewMCPServer("paperclip", "test")
	RegisterTools(s)
...
func TestReplaceMCPPathParamEscapesAsSingleSegment(t *testing.T) {
	got := replaceMCPPathParam("/api/items/{id}/details", "{id}", "../a/b?admin=true")
	if got != "/api/items/..%2Fa%2Fb%3Fadmin=true/details" {
		t.Fatalf("replaceMCPPathParam = %q", got)
	}
}

func TestMCPContextIncludesCurrentToolCountAndAgentContext(t *testing.T) {
	...
	if err := json.Unmarshal([]byte(mcpTextContent(t, result)), &payload); err != nil {
		t.Fatalf("context result is invalid JSON: %v", err)
	}
	server := server.NewMCPServer("paperclip", "test")
	RegisterTools(server)
	if want := len(server.ListTools()); payload.ToolCount != want {
		t.Fatalf("tool_count = %d, registered tools = %d", payload.ToolCount, want)
	}
	...
	if err := json.Unmarshal(payload.AgentContext, &agentContext); err != nil {
		t.Fatalf("agent_context is invalid JSON: %v", err)
	}
	if agentContext.Auth.Mode != "multi" || len(agentContext.Auth.EnvVars) != 4 {
		t.Fatalf("agent auth context = %#v", agentContext.Auth)
	}
	if len(agentContext.Commands) == 0 {
		t.Fatal("agent_context omitted command discovery")
	}
}
...
func TestValidateReadOnlyQuery_RejectsBypassVectors(t *testing.T) {
	rejected := []string{
		...
		"/* outer /* not nested */ */ SELECT 1", // SQLite doesn't nest, so trailing "*/" closes; second SELECT remains. Reject â€” the gate must err on the side of caution when the leading shape is suspicious.
		"-- only a comment",
		"/* only a comment */",
		"",
		"   ",
