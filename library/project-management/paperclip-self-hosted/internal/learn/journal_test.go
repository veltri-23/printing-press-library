...
// which itself imports internal/learn â€” an in-package test would be
// an import cycle.
package learn_test
...
func withJournalHome(t *testing.T) string {
	...
	t.Setenv("PAPERCLIP_NO_LEARN", "")
	t.Setenv("PAPERCLIP_LEARN_NO_CAPTURE", "")
	t.Setenv("PAPERCLIP_LEARN_SESSION", "")
	return dir
}
...
func runCLI(t *testing.T, args ...string) error {
	t.Helper()
	oldArgs := os.Args
	os.Args = append([]string{"paperclip-pp-cli"}, args...)
...
func TestJournal_SessionKeyFromEnv(t *testing.T) {
	withJournalHome(t)
	t.Setenv("PAPERCLIP_LEARN_SESSION", "session-abc123")
	if err := runCLI(t, "version"); err != nil {
		t.Fatalf("version: %v", err)
	}
	entries := readAllJournalEntries(t)
...
func TestJournal_DisableSwitchesWriteNothing(t *testing.T) {
	cases := []struct {
		...
		{name: "master no-learn env", env: map[string]string{"PAPERCLIP_NO_LEARN": "1"}},
		{name: "capture-only env", env: map[string]string{"PAPERCLIP_LEARN_NO_CAPTURE": "1"}},
...
func TestJournal_OffsetSurvivesRolloverAsSegmentByte(t *testing.T) {
	...
	// keep working: everything strictly after it â€” the new segment â€”
