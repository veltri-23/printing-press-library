...
// teach-to-reuse joins by row id â€” an alias-mediated or paraphrased
...
// promoted normalized form) â€” internal plumbing for the command
...
func Recall(ctx context.Context, db *sql.DB, query string, opts Opts) (Result, error) {
	...
	// noise filter â€” only entities a recall actually failed to resolve
	...
	rows, err := db.QueryContext(ctx, `SELECT l.id, l.query_pattern, COALESCE(l.query_entities, ''),
		COALESCE(l.venue, ''), COALESCE(l.resource_type, ''), l.resource_id, l.action,
		COALESCE(l.alias_target, ''), l.source, l.confidence, l.created_at, l.last_observed_at,
		r.data
		FROM search_learnings l
		LEFT JOIN resources r ON r.resource_type = l.resource_type AND r.id = l.resource_id
		WHERE l.confidence >= ?`, minConf)
	if err != nil {
	...
	for rows.Next() {
		var (
			...
			resourceData   sql.NullString
		)
		if err := rows.Scan(&learningID, &queryPattern, &storedEntities, &venue, &resourceType,
			&resourceID, &action, &aliasTarget, &source, &confidence, &createdAt, &lastObserved, &resourceData); err != nil {
		...
		validateResourceData(resourceData, cfg, &hit, normalized.Entities, storedEntitySlice, opts.ResourceTypeFields)
	...
	// no lookup coverage under any source points the agent at `sync` â€”
...
func validateResource(ctx context.Context, db *sql.DB, cfg *entities.Config, hit *Hit, queryEntities, storedEntitySlice []string, fieldsByType map[string][]string) {
	...
	validateResourceData(sql.NullString{String: data, Valid: err == nil}, cfg, hit, queryEntities, storedEntitySlice, fieldsByType)
}

func validateResourceData(data sql.NullString, cfg *entities.Config, hit *Hit, queryEntities, storedEntitySlice []string, fieldsByType map[string][]string) {
	if !data.Valid {
	...
	resourceEntities := ResourceEntitiesFromJSON([]byte(data.String), fields, cfg)
...
const candidateCommandBinary = "paperclip-pp-cli"
...
// +prefix) so bare numeric identifiers â€” resource ids, timestamps,
// zip+4 codes â€” never trip the warning.
...
// proceed â€” never block, never redact silently. Shared by the teach
