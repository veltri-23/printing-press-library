...
// AppendJournalEntry) so scenarios control every field â€” including the
...
// from the store's own migration path â€” real SQLite, no fixtures.
...
func TestSynthesis_HappyPathCreatesQuarantinedCandidate(t *testing.T) {
	...
	// row â€” only `learnings confirm` materializes.
