...
// connections stay â‰ˆ budgetÃ—latency (a handful) regardless of --concurrency;
...
func newHTTPClient(timeout time.Duration, jar http.CookieJar) *http.Client {
	...
	// IdleConnTimeout, and dialer keep-alive survive â€” a bare struct would break
...
// "auto" â€” pace by the server's advertised budget (X-Ratelimit-* headers), with
...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	httpClient.CheckRedirect = func(req *http.Request, via []*http.Request) error {
		...
		if req.URL.Host == via[0].URL.Host {
			...
			if c.Config != nil && c.Config.CookieHeader() != "" {
				req.Header.Set("Cookie", c.Config.CookieHeader())
			}
		} else {
...
// PostQueryWithParams is a POST that does not mutate remote state â€” used
...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// minting, and the success-branch invalidateCache() call below â€” so
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		// Proactive rate limiting â€” wait before sending
		c.limiter.Wait()
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "paperclip-pp-cli/1.0.0")
		}
		...
		if err != nil {
			...
			if !requestCanRetry(method, readOnlyIntent) {
				return nil, 0, lastErr
			}
			// Transient network failure (connection reset, DNS blip, request
			// timeout). Back off before retrying â€” same exponential schedule as
			// the 5xx path below â€” so a brief outage does not burn every attempt
		...
		if resp.StatusCode < 400 {
			...
			// â€” return a self-describing base64 envelope instead. Textual and
		...
		if resp.StatusCode == 429 && attempt < maxRetries && requestCanRetry(method, readOnlyIntent) {
		...
		if resp.StatusCode >= 500 && attempt < maxRetries && requestCanRetry(method, readOnlyIntent) {
			wait := time.Duration(math.Pow(2, float64(attempt))) * time.Second
...
// call â€” the caller is responsible for passing cached auth material only.
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	if body != nil {
		...
		if json.Unmarshal(body, &pretty) == nil {
			...
			_ = enc.Encode(redactSensitiveJSON(pretty))
		}
	}
	if authHeader != "" {
	...
	if c.Config != nil && c.Config.CookieHeader() != "" {
		fmt.Fprintf(os.Stderr, "  %s: %s\n", "Cookie", maskToken(c.Config.CookieHeader()))
	}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.SessionCookie)
		addCredential(c.Config.BoardAPIKey)
		addCredential(c.Config.AgentToken)
	}
	sort.SliceStable(masks, func(i, j int) bool {
		return len(masks[i].needle) > len(masks[j].needle)
	})
	masked := maskSensitivePathSegments(text)
...
func requestCanRetry(method string, readOnlyIntent bool) bool {
	if readOnlyIntent {
		return true
	}
	switch method {
...
func redactSensitiveJSON(value any) any {
	...
	case []any:
		...
		for i, child := range typed {
			out[i] = redactSensitiveJSON(child)
		}
		return out
	default:
		return value
...
func sensitiveJSONKey(key string) bool {
	...
	for _, marker := range []string{"token", "secret", "password", "cookie", "auth", "apikey", "privatekey", "accesskey"} {
...
func maskSensitivePathSegments(raw string) string {
	for _, marker := range []string{"/invites/", "/board-claim/"} {
		for offset := 0; ; {
			start := strings.Index(raw[offset:], marker)
			if start < 0 {
				break
			}
	...
	for i := 0; i+1 < len(segments); i++ {
		if segments[i] == "invites" || segments[i] == "board-claim" {
	...
	decoded, err := url.PathUnescape(parsed.RawPath)
	if err != nil {
		return raw
	}
	parsed.Path = decoded
