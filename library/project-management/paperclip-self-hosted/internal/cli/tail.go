...
native streaming instead of polling.`,
  ...
  paperclip-pp-cli tail --interval 10s

  # Tail a specific resource
  paperclip-pp-cli tail messages --interval 5s

  # Pipe to jq for filtering
  paperclip-pp-cli tail events --interval 30s | jq 'select(.type == "error")'`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := validateTailOptions(follow, interval); err != nil {
				return err
			}
			...
			// Initial fetch
			if err := fetchInitialTail(cmd.Context(), c, path, enc, follow); err != nil {
				return err
			}
			if !follow {
				return nil
...
func validateTailOptions(follow bool, interval time.Duration) error {
	if follow && interval <= 0 {
		return usageErr(fmt.Errorf("--interval must be greater than zero"))
	}
	return nil
}

func fetchInitialTail(ctx context.Context, c interface {
	Get(context.Context, string, map[string]string) (json.RawMessage, error)
}, path string, enc *json.Encoder, follow bool) error {
	err := fetchAndEmit(ctx, c, path, enc)
	if err == nil {
		return nil
	}
	if !follow {
