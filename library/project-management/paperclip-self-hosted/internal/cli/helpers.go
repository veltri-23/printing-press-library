...
// string â€” required for diagnosing 4xx responses on endpoints whose OpenAPI
...
// pretty-printed multi-line JSON â€” embedding it raw broke the NDJSON stream.
...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'paperclip-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. This API is configured without credentials, so the service may be blocking the request by rate limit, geography, bot protection, or endpoint policy."+
			"\n      Run 'paperclip-pp-cli doctor' to check connectivity.", err))
...
// isValidUUID returns true when v matches the canonical 8-4-4-4-12 hex UUID
// shape (with or without dashes). Paperclip path parameters use UUIDs; reject
// obvious non-UUID inputs (like dogfood sentinels) up-front instead of letting
// a permissive upstream return data masquerading as a successful lookup.
...
// isLikelyID accepts real paperclip path-parameter IDs while still rejecting
// obvious garbage. Real IDs come from a list companion (most often a UUID from
// the API; some paperclip instances return slug-shaped local IDs like
// "<local-user>"). Test sentinels like `__printing_press_invalid__` and
// arbitrary strings fail this check and are rejected early.
func isLikelyID(v string) bool {
	if v == "" {
		return false
	}
	if isValidUUID(v) {
	...
	// Paperclip local-dev instances return slug-shaped IDs.
	if strings.HasPrefix(v, "local-") && len(v) > len("local-") {
		return true
	}
	return false
}
...
// Supports direct case-insensitive match and camelCaseâ†’kebab-case conversion.
...
// "markdown" â€” those fields are payload on `get` commands and stripping them
...
// - Human in terminal: isTerminal()=true â†’ table
// - --human-friendly: force human output, even when stdout is piped
// - Claude Code/Codex bash tool: stdout piped â†’ JSON
// - --json/--csv/--compact/--agent: machine format â†’ JSON
...
// prioritizeFields orders fields by importance: identity â†’ temporal â†’ status â†’ other.
...
func prioritizeFields(item map[string]any, includeComplex bool) []string {
	// Priority tiers â€” matched against the last segment of the field name.
	// "OrderDate" â†’ last segment "date" â†’ tier 1 (temporal).
	// "BuildingName" â†’ last segment "name" â†’ tier 0... but we want to avoid this.
	...
	// Exact matches (full field name, case-insensitive) â€” highest confidence
	...
	// Suffix patterns â€” match when the field ends with this word (after splitting)
...
// splitCamelCase splits "OrderDate" â†’ ["order", "date"], "statusCode" â†’ ["status", "code"],
// "page_size" â†’ ["page", "size"].
...
// printAutoCards renders items as labeled cards â€” one block per item.
...
func printAutoCards(w io.Writer, items []map[string]any) error {
	...
	for i, item := range items {
		...
		// Remaining fields indented â€” skip empty, zero, and false values
...
// Looks for: qty/count â†’ name/title â†’ size â†’ price, in that order.
func formatObjectSummary(obj map[string]any) string {
	...
	// Name â€” check nested objects too (e.g., Side1.Name)
...
// Suppressed when stdout is piped or redirected â€” the JSON response envelope
