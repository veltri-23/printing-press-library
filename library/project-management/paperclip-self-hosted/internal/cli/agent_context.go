...
type agentContextSummary struct {
	...
	Auth                       agentContextAuth             `json:"auth"`
	Paths                      agentContextPaths            `json:"paths"`
	Commands                   []agentContextCommandSummary `json:"commands"`
	AvailableProfiles          []string                     `json:"available_profiles"`
	FeedbackEndpointConfigured bool                         `json:"feedback_endpoint_configured"`
	LearnProtocol              string                       `json:"learn_protocol"`
}

type agentContextCommandSummary struct {
	Name        string                       `json:"name"`
	Subcommands []agentContextCommandSummary `json:"subcommands,omitempty"`
}
...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	envVars := []agentContextAuthEnvVar{
		{Name: "PAPERCLIP_AUTH_MODE", Kind: "mode", Required: false, Sensitive: false, Description: "auto, board-session, board-api-key, agent-bearer, or none"},
		{Name: "PAPERCLIP_SESSION_COOKIE", Kind: "cookie", Required: false, Sensitive: true, Description: "Session cookie value for board-session auth"},
		{Name: "PAPERCLIP_API_KEY", Kind: "token", Required: false, Sensitive: true, Description: "Board API key for board-api-key auth"},
		{Name: "PAPERCLIP_AGENT_TOKEN", Kind: "token", Required: false, Sensitive: true, Description: "Agent bearer token for agent-bearer auth"},
	}
...
// AgentContext returns the same runtime discovery payload used by the CLI's
// agent-context command so the MCP context tool has transport parity.
func AgentContext(rootCmd *cobra.Command) any {
	full := buildAgentContext(rootCmd)
	return agentContextSummary{
		SchemaVersion:              full.SchemaVersion,
		CLI:                        full.CLI,
		Auth:                       full.Auth,
		Paths:                      full.Paths,
		Commands:                   collectAgentCommandSummary(rootCmd),
		AvailableProfiles:          full.AvailableProfiles,
		FeedbackEndpointConfigured: full.FeedbackEndpointConfigured,
		LearnProtocol:              full.LearnProtocol,
	}
}

func collectAgentCommandSummary(cmd *cobra.Command) []agentContextCommandSummary {
	children := cmd.Commands()
	sort.Slice(children, func(i, j int) bool { return children[i].Name() < children[j].Name() })
	out := make([]agentContextCommandSummary, 0, len(children))
	for _, child := range children {
		if child.Name() == "agent-context" {
			continue
		}
		out = append(out, agentContextCommandSummary{
			Name:        child.Name(),
			Subcommands: collectAgentCommandSummary(child),
		})
	}
	return out
}
