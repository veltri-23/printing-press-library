...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  paperclip-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  paperclip-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"adapters", "admin", "agents", "agents-me-inbox-lite", "agents-me-inbox-mine", "board-api-keys", "cli-auth", "companies", "companies-issues", "companies-stats", "instance", "instance-scheduler-heartbeats", "instance-settings-experimental", "instance-settings-general", "issues", "llms", "llms-agent-configuration-txt", "openapi-json", "plugins", "plugins-examples", "plugins-tools", "plugins-ui-contributions", "sidebar-preferences", "skills", "skills-available", "skills-catalog", "teams"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  paperclip-pp-cli workflow status

  # Show status as JSON
  paperclip-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("paperclip-pp-cli")
			}
