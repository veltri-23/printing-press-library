...
func twoPlaybookFS() fstest.MapFS {
  ...
  "steps": [{"cmd": "paperclip-pp-cli --help", "purpose": "noop"}],
  "entity_slots": [],
...
// marker is present â€” otherwise the bump would have no effect on
// existing users.
...
// "the [amend foo] flag" â€” the regex anchor refuses that shape.
...
func TestPlaybookInit_SkipsPlaybookWithoutExamples(t *testing.T) {
	...
	noExamplesFS := fstest.MapFS{
		"empty_examples.json": &fstest.MapFile{
			Data: []byte(`{
  "steps": [{"cmd": "paperclip-pp-cli --help", "purpose": "no-op"}],
  "entity_slots": [],
  "expected_tool_calls": 1
}`),
		},
...
func TestPlaybookInit_FailureLeavesSentinelStale(t *testing.T) {
	...
	brokenFS := fstest.MapFS{
		...
		// A valid playbook alongside the broken one â€” the loop must
