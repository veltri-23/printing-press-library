...
func resolveReadWithStrategyResponsePathAndJSONGuard(ctx context.Context, c *client.Client, flags *rootFlags, strategy string, resourceType string, isList bool, path string, params map[string]string, headers map[string]string, responsePath string, guardLiveJSON bool, hintWriter io.Writer) (json.RawMessage, DataProvenance, error) {
	...
	default: // "auto"
		...
		if !isNetworkError(err) {
			// HTTP 4xx/5xx errors propagate â€” not a fallback case
			return nil, DataProvenance{}, err
		}
		// Network error â€” try local fallback
		...
		if fallbackErr != nil {
			return nil, DataProvenance{}, fmt.Errorf("API unreachable and no local data. Run 'paperclip-pp-cli sync' to enable offline access.\n\nOriginal error: %w", err)
		}
...
var listEnvelopeMetadataKeys = map[string]bool{
	// list wrappers themselves â€” must stay in sync with pageItemKeys
...
// FTS search covers everything the user has looked up â€” not just explicit syncs.
...
func writeThroughCache(ctx context.Context, resourceType string, data json.RawMessage) {
	db, err := store.OpenWithContext(ctx, defaultDBPath("paperclip-pp-cli"))
	if err != nil {
		return
	}
	defer db.Close()
	...
	if json.Unmarshal(data, &items) != nil || len(items) == 0 {
		items = nil
		// Try object â€” check for common envelope patterns (results, data, items)
...
func resolveLocal(ctx context.Context, flags *rootFlags, hintWriter io.Writer, resourceType string, isList bool, path string, params map[string]string, reason string) (json.RawMessage, DataProvenance, error) {
	db, err := openStoreForRead(ctx, "paperclip-pp-cli")
	if err != nil {
		return nil, DataProvenance{}, fmt.Errorf("opening local database: %w\nRun 'paperclip-pp-cli sync' first.", err)
	}
	if db == nil {
		return nil, DataProvenance{}, fmt.Errorf("no local data. Run 'paperclip-pp-cli sync' first")
	}
	defer db.Close()
	...
	if len(params) > 0 {
		fmt.Fprintf(os.Stderr, "warning: local data is unfiltered â€” endpoint filters are not applied to cached data\n")
	}
	...
	if isList {
		...
		if len(items) == 0 {
			return nil, DataProvenance{}, fmt.Errorf("no local data for %q. Run 'paperclip-pp-cli sync' first", resourceType)
		}
	...
	// Get by ID â€” extract the last path segment as the ID
	...
	if err != nil {
		if errors.Is(err, sql.ErrNoRows) {
			return nil, DataProvenance{}, fmt.Errorf("resource %q with ID %q not found in local store. Run 'paperclip-pp-cli sync' first", resourceType, id)
		}
		return nil, DataProvenance{}, fmt.Errorf("querying local store: %w", err)
	}
