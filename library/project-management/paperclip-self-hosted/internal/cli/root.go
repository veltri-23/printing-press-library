...
type rootFlags struct {
	...
	// invocation. Mirrors the PAPERCLIP_NO_LEARN env var.
	noLearn bool
...
func Execute() (retErr error) {
	...
	if err != nil && isCobraUsageError(err) {
		...
		// default and emits 1 â€” clobbering the conventional code-2 for
...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "paperclip-pp-cli",
		Short: "Manage paperclip resources via the paperclip API",
		Long: `Manage paperclip resources via the paperclip API.

Add --agent to any command for JSON output + non-interactive mode.
Run 'paperclip-pp-cli doctor' to verify auth and connectivity.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("paperclip-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'paperclip-pp-cli profile list')")
	rootCmd.PersistentFlags().StringVar(&flags.authMode, "auth-mode", "", "Authentication mode: auto, board-session, board-api-key, agent-bearer, or none")
	rootCmd.PersistentFlags().StringVar(&flags.sessionCookie, "session-cookie", "", "Paperclip board session cookie value (or PAPERCLIP_SESSION_COOKIE)")
	rootCmd.PersistentFlags().StringVar(&flags.boardAPIKey, "api-key", "", "Paperclip board API key (or PAPERCLIP_API_KEY)")
	rootCmd.PersistentFlags().StringVar(&flags.agentToken, "agent-token", "", "Paperclip agent API key or JWT (or PAPERCLIP_AGENT_TOKEN)")
	...
	rootCmd.AddCommand(newAdaptersCmd(flags))
	rootCmd.AddCommand(newAdminCmd(flags))
	rootCmd.AddCommand(newAgentsCmd(flags))
	rootCmd.AddCommand(newAssetsCmd(flags))
	rootCmd.AddCommand(newAuthCmd(flags))
	rootCmd.AddCommand(newBoardApiKeysCmd(flags))
	rootCmd.AddCommand(newCliAuthCmd(flags))
	rootCmd.AddCommand(newCloudUpstreamsCmd(flags))
	rootCmd.AddCommand(newCompaniesCmd(flags))
	rootCmd.AddCommand(newEnvironmentsCmd(flags))
	rootCmd.AddCommand(newExecutionWorkspacesCmd(flags))
	...
	rootCmd.AddCommand(newJoinRequestsCmd(flags))
	rootCmd.AddCommand(newLlmsCmd(flags))
	rootCmd.AddCommand(newPluginsCmd(flags))
	rootCmd.AddCommand(newProjectsCmd(flags))
	rootCmd.AddCommand(newRoutineTriggersCmd(flags))
	rootCmd.AddCommand(newRoutinesCmd(flags))
	rootCmd.AddCommand(newSecretProviderConfigsCmd(flags))
	rootCmd.AddCommand(newSecretsCmd(flags))
	rootCmd.AddCommand(newSidebarPreferencesCmd(flags))
	rootCmd.AddCommand(newSkillsCmd(flags))
	rootCmd.AddCommand(newTeamsCmd(flags))
	rootCmd.AddCommand(newWorkProductsCmd(flags))
	rootCmd.AddCommand(newWorkspaceOperationsCmd(flags))
	...
	rootCmd.AddCommand(newApprovalsPromotedCmd(flags))
	rootCmd.AddCommand(newAttachmentsPromotedCmd(flags))
	rootCmd.AddCommand(newBoardPromotedCmd(flags))
	rootCmd.AddCommand(newBoardClaimPromotedCmd(flags))
	rootCmd.AddCommand(newBootstrapPromotedCmd(flags))
	rootCmd.AddCommand(newEnvironmentCustomImageSetupSessionsPromotedCmd(flags))
	rootCmd.AddCommand(newEnvironmentLeasesPromotedCmd(flags))
	rootCmd.AddCommand(newFeedbackTracesPromotedCmd(flags))
	rootCmd.AddCommand(newHeartbeatRunsPromotedCmd(flags))
	rootCmd.AddCommand(newInvitesPromotedCmd(flags))
	rootCmd.AddCommand(newLabelsPromotedCmd(flags))
...
// command names only â€” an unmatched token may be a positional value
...
// never trigger a derivation pass â€” deriving off the learn surface's
...
// swallowed â€” derivation may never fail, slow, or add output to the
...
// The derivation pairing rule only heals flags that resolve nowhere â€”
...
func (f *rootFlags) newClient() (*client.Client, error) {
	...
	if f.authMode != "" {
		cfg.AuthMode = f.authMode
	}
	if f.sessionCookie != "" {
		cfg.SessionCookie = f.sessionCookie
	}
	if f.boardAPIKey != "" {
		cfg.BoardAPIKey = f.boardAPIKey
	}
	if f.agentToken != "" {
