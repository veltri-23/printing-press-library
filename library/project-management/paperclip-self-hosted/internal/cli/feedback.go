...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("PAPERCLIP_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("PAPERCLIP_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "paperclip-pp-cli/feedback")
...
When ` + "`PAPERCLIP_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`PAPERCLIP_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  paperclip-pp-cli feedback list
  paperclip-pp-cli feedback list --limit 5
  paperclip-pp-cli feedback list --json`,
