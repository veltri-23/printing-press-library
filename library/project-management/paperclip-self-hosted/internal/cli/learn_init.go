...
// depth â€” spec parse already validated them) log a warning and are
...
// makes this idempotent â€” calling it twice produces the same DB state
...
// failure must never abort the CLI â€” the recall path returns the
...
func runLearnInitOnce(ctx context.Context) {
	learnInitOnce.Do(func() {
		...
		if err != nil {
			fmt.Fprintf(os.Stderr, "warning: paperclip-pp-cli: learn init: open store: %v\n", err)
			return
		}
		defer s.Close()
		if err := initLearn(ctx, s.DB()); err != nil {
			fmt.Fprintf(os.Stderr, "warning: paperclip-pp-cli: learn init: %v\n", err)
		}
	})
}
