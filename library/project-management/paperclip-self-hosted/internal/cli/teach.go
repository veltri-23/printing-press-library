...
// paperclip-pp-cli CLI: `teach` (fire-and-forget, silent on success,
...
import (
	...
	"github.com/mvanhorn/printing-press-library/library/project-management/paperclip-self-hosted/internal/learn/entities"
	"github.com/mvanhorn/printing-press-library/library/project-management/paperclip-self-hosted/internal/learn/lookups"
	"github.com/mvanhorn/printing-press-library/library/project-management/paperclip-self-hosted/internal/learn/patterns"
...
const noLearnEnvVar = "PAPERCLIP_NO_LEARN"
...
// failure leaks nothing to the parent shell's stderr â€” the diagnosis
...
func learnDBPath(explicit string) string {
	...
	return defaultDBPath("paperclip-pp-cli")
}

// newTeachCmd builds the `teach` cobra command â€” the LLM-facing write
...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  paperclip-pp-cli teach --query "<question>" --resource-type <type> \
    --resource <id> --resource <id> &`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// tables. Warn-only by contract â€” name the rule on stderr,
			...
			// explicit `learnings confirm`. Best-effort â€” failures log to
	...
	cmd.Flags().StringSliceVar(&resources, "resource", nil, "Resource ID â€” repeat for multiple (required)")
	...
	cmd.Flags().BoolVar(&quiet, "quiet", true, "Silent on success (default true â€” designed for background invocation)")
...
Empty match returns {"found": false, "results": []} with exit 0 â€” this
...
when learnings exist.`,
		Example: `  paperclip-pp-cli recall "<question>" --agent
  paperclip-pp-cli recall "<question>" --agent --min-confidence 2`,
...
// resolved a playbook. Called strictly after learn.Recall returns â€”
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  paperclip-pp-cli learnings list --agent
  paperclip-pp-cli learnings list --query "<substring>"
  paperclip-pp-cli learnings list --warnings --agent`,
		Annotations: map[string]string{"mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  paperclip-pp-cli learnings forget "<question>" --resource <id>
  paperclip-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
...
func newTeachPatternCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// Bare invocation intentionally exits 2 (silent usage error to
		// teach.log); declare it so `cli-printing-press verify` and the
		// live dogfood matrix score this honestly.
		Annotations: map[string]string{"pp:typed-exit-codes": "0,2", "mcp:local-write": "true"},
...
a whole family.`,
		Example: `  paperclip-pp-cli teach-pattern \
...
cannot be taught â€” they are derived from the canonical input.`,
		Example: `  paperclip-pp-cli teach-lookup --kind country --canonical "United States" --value USA`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dryRunOK(flags) {
				return nil
			}
