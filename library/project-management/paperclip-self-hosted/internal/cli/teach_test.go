...
func TestTeachCommand_RespectsNoLearnEnvVar(t *testing.T) {
	...
	// DB file should not exist â€” teach exited before opening.
...
func TestSkipLearnHook_FrameworkCommandsSkipped(t *testing.T) {
	cases := []string{
		...
		"paperclip-pp-cli help items",
		"paperclip-pp-cli sync",
		"paperclip-pp-cli profile list",
		"paperclip-pp-cli feedback list",
		"paperclip-pp-cli which",
		"paperclip-pp-cli agent-context",
		"paperclip-pp-cli completion bash",
		"paperclip-pp-cli version",
	}
...
func TestSkipLearnHook_NovelCommandsNotSkipped(t *testing.T) {
	novel := []string{
		...
		"paperclip-pp-cli recall",
		"paperclip-pp-cli learnings",
		"paperclip-pp-cli teach-pattern",
		"paperclip-pp-cli teach-lookup",
		"paperclip-pp-cli items list",
	}
...
func TestSkipLearnHook_FrameworkCommandsDoNotCreateDefaultDB(t *testing.T) {
	...
	for _, args := range cases {
		...
		if _, err := os.Stat(defaultDBPath("paperclip-pp-cli")); err == nil {
...
// canonical loop end to end â€” cold recall, teach, warm recall â€” and
...
// reuse â€” but the recall_hit's matched_row_id still credits the
...
func TestLearnEvents_AliasMediatedRecallCreditsTaughtRowID(t *testing.T) {
	...
	if hits[0].FamilyHash == teaches[0].FamilyHash {
		t.Errorf("alias recall family hash %q should differ from teach family hash %q â€” the scenario must exercise the row-id join",
...
// store must swallow the failure (to teach.log) â€” recall's result was
...
// the same file â€” the shape of a backgrounded teach racing a recall
// in another process â€” and asserts every event lands. Run with -race
