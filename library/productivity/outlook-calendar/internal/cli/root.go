...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "outlook-calendar-pp-cli",
		Short: `Drive your personal Microsoft 365 calendar from agentic workflows: read, write, sync, and run offline analytics like conflicts, freetime, and recurring-drift that no Graph endpoint exposes.`,
		Long: `Drive your personal Microsoft 365 calendar from agentic workflows: read, write, sync, and run offline analytics like conflicts, freetime, and recurring-drift that no Graph endpoint exposes.
...
Health check: run 'outlook-calendar-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("outlook-calendar-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'outlook-calendar-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDeltaCmd(flags))
	...
	// Hand-built (Phase 3): novel commands powered by the local SQLite store.
	rootCmd.AddCommand(newConflictsCmd(flags))
	rootCmd.AddCommand(newFreetimeCmd(flags))
	rootCmd.AddCommand(newReviewCmd(flags))
	rootCmd.AddCommand(newPendingCmd(flags))
	rootCmd.AddCommand(newRecurringDriftCmd(flags))
	rootCmd.AddCommand(newPrepCmd(flags))
	rootCmd.AddCommand(newWithCmd(flags))
	rootCmd.AddCommand(newTzAuditCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("outlook-calendar-pp-cli %s\n", version)
		},
	}
}
