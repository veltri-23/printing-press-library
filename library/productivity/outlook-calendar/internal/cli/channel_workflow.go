...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  outlook-calendar-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  outlook-calendar-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calendars", "categories", "delta", "events"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  outlook-calendar-pp-cli workflow status

  # Show status as JSON
  outlook-calendar-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("outlook-calendar-pp-cli")
			}
