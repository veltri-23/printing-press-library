...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".outlook-calendar-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OUTLOOK_CALENDAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OUTLOOK_CALENDAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "outlook-calendar-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.outlook-calendar-pp-cli/feedback.jsonl.
When ` + "`OUTLOOK_CALENDAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OUTLOOK_CALENDAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  outlook-calendar-pp-cli feedback list
  outlook-calendar-pp-cli feedback list --limit 5
  outlook-calendar-pp-cli feedback list --json`,
