...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "outlook-calendar-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		req.Header.Set("User-Agent", "outlook-calendar-pp-cli/0.1")
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "outlook-calendar-pp-cli/0.1.0")
		}
...
func (c *Client) refreshAccessToken() error {
	...
	// Hand-built (Phase 3): Microsoft Identity /common token endpoint. This is
	// the public-client, personal-account-friendly authority. Override via
	// OUTLOOK_CALENDAR_AUTHORITY for tenant-specific or /consumers flows.
	authority := os.Getenv("OUTLOOK_CALENDAR_AUTHORITY")
	if authority == "" {
		authority = "https://login.microsoftonline.com/common"
	}
	tokenURL := strings.TrimRight(authority, "/") + "/oauth2/v2.0/token"

	params := url.Values{
		"grant_type":    {"refresh_token"},
		"refresh_token": {c.Config.RefreshToken},
		"client_id":     {c.Config.ClientID},
		// Personal-account-friendly default scope set; matches what auth login asks for
