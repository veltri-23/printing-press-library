# Nylas Printed CLI Agent Guide

This directory is a generated `nylas-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nylas-pp-cli doctor --json
nylas-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
nylas-pp-cli which "<capability>" --json
nylas-pp-cli <command> --help
```
...
```bash
nylas-pp-cli <command> --help
nylas-pp-cli <command> --dry-run --agent
```
