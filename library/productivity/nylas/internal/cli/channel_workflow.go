...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  nylas-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// workflow archive wraps syncResource in a single-JSON envelope.
			// Without this, syncResource writes streaming sync_start /
			// sync_complete NDJSON events to stdout that mix with the
			// final envelope and break JSON-fidelity for piped consumers.
			// See sync.go: quietSyncEvents.
			quietSyncEvents = true
			defer func() { quietSyncEvents = false }()

			resources := []string{"admin", "applications", "applications-redirect-uris", "channels", "channels-pubsub", "connect", "connectors", "grants", "grants-me", "lists", "migration-tools", "notetakers", "policies", "rules", "templates", "webhooks", "workflows", "workspaces"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  nylas-pp-cli workflow status

  # Show status as JSON
  nylas-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("nylas-pp-cli")
			}
