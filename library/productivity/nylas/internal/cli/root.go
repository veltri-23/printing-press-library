...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "nylas-pp-cli",
		Short: `Nylas CLI — Every Nylas API, plus a local SQLite mirror, cross-grant search, and confirm-by-hash sending no other Nylas tool has.`,
		Long: `Nylas CLI — Every Nylas API, plus a local SQLite mirror, cross-grant search, and confirm-by-hash sending no other Nylas tool has.
...
Health check: run 'nylas-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("nylas-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'nylas-pp-cli profile list')")
	...
	rootCmd.AddCommand(newConnectorsCmd(flags))
	rootCmd.AddCommand(newDomainsCmd(flags))
	rootCmd.AddCommand(newGrantsCmd(flags))
	rootCmd.AddCommand(newListsCmd(flags))
	rootCmd.AddCommand(newMigrationToolsCmd(flags))
	rootCmd.AddCommand(newNotetakersCmd(flags))
	...
	rootCmd.AddCommand(newProvidersPromotedCmd(flags))
