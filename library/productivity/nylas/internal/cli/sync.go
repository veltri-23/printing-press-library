...
// quietSyncEvents suppresses the streaming sync_start / sync_warning /
// sync_dryrun / sync_summary NDJSON lines that syncResource normally
// writes to stdout when --human-friendly is off. Callers that wrap
// syncResource in a single-JSON envelope (workflow archive) set this
// true so their --json output stays a single value rather than a mix
// of streaming events plus a final object. The sync command itself
// leaves this false so its NDJSON contract is preserved.
var quietSyncEvents bool
...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		// mcp:long-running tells live-dogfood probes that this command
		// can legitimately exceed the default 30s probe timeout on
		// full-application syncs. The probe should either chunk by
		// --resources or apply a longer per-test budget (5-10 min is
		// typical for a full Nylas v3 application). Without this hint,
		// happy_path probes report exit -1 from a SIGKILL the moment
		// they cross 30s, even though the run is healthy.
...
Runtime:
  A full multi-resource sync against an active Nylas application can take
  several minutes (typically 1-10 minutes depending on grant count and
  history depth). Use --resources to narrow the scope for faster iteration,
  or --latest-only to refresh just the head of each resource. Incremental
  resumes from the last successful sync, so re-runs after the first full
  sync are typically seconds.
...
Resource scoping:
  ...
  nylas-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  nylas-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  nylas-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  nylas-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  nylas-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/nylas-pp-cli/data.db)")
...
func syncResource(c interface {
	...
	// emitEvents controls NDJSON streaming-event writes inside this
	// function. Callers that wrap syncResource in a single-JSON envelope
	// (workflow archive) set quietSyncEvents=true so this stays false
	// and stdout remains a single JSON value. The sync command leaves
	// quietSyncEvents=false so its NDJSON contract is preserved.
	emitEvents := !humanFriendly && !quietSyncEvents
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "connectors":
		return db.UpsertConnectors(data)
	case "creds":
		return db.UpsertCreds(data)
	case "domains_messages":
		return db.UpsertDomainsMessages(data)
	case "grants":
		return db.UpsertGrants(data)
	case "attachment_uploads":
		return db.UpsertAttachmentUploads(data)
	case "attachments":
		return db.UpsertAttachments(data)
	case "grants_calendars":
		return db.UpsertGrantsCalendars(data)
	case "consolidated_order":
		return db.UpsertConsolidatedOrder(data)
	case "consolidated_return":
		return db.UpsertConsolidatedReturn(data)
	case "consolidated_shipment":
		return db.UpsertConsolidatedShipment(data)
	case "contacts":
	...
	case "grants_messages":
		return db.UpsertGrantsMessages(data)
	case "grants_notetakers":
		return db.UpsertGrantsNotetakers(data)
	case "resources":
		return db.UpsertResources(data)
	case "rule_evaluations":
		return db.UpsertRuleEvaluations(data)
	case "grants_scheduling":
		return db.UpsertGrantsScheduling(data)
	case "signatures":
		return db.UpsertSignatures(data)
	case "grants_templates":
		return db.UpsertGrantsTemplates(data)
	case "threads":
		return db.UpsertThreads(data)
	case "grants_workflows":
		return db.UpsertGrantsWorkflows(data)
	case "lists":
		return db.UpsertLists(data)
	case "items":
		return db.UpsertItems(data)
	case "migration-tools":
		return db.UpsertMigrationTools(data)
	case "notetakers":
		return db.UpsertNotetakers(data)
	case "cancel":
		return db.UpsertCancel(data)
	case "history":
		return db.UpsertHistory(data)
	case "leave":
		return db.UpsertLeave(data)
	case "media":
	...
	case "manual_assign":
		return db.UpsertManualAssign(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"admin":                      "/v3/admin/domains",
		"applications":               "/v3/applications",
		"applications-redirect-uris": "/v3/applications/redirect-uris",
		"channels":                   "/v3/channels/sns",
		"channels-pubsub":            "/v3/channels/pubsub",
		"connect":                    "/v3/connect/tokeninfo",
		"connectors":                 "/v3/connectors",
		"grants":                     "/v3/grants",
		"grants-me":                  "/v3/grants/me",
		"lists":                      "/v3/lists",
		"migration-tools":            "/v3/migration-tools/jobs",
		"notetakers":                 "/v3/notetakers",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "consolidated_order", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/consolidated-order", KeyField: ""},
		{Name: "consolidated_return", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/consolidated-return", KeyField: ""},
		{Name: "consolidated_shipment", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/consolidated-shipment", KeyField: ""},
		{Name: "contacts", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/contacts", KeyField: ""},
		{Name: "creds", ParentTable: "connectors", ParentIDParam: "provider", PathTemplate: "/v3/connectors/{provider}/creds", KeyField: ""},
		{Name: "drafts", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/drafts", KeyField: ""},
		{Name: "events", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/events", KeyField: ""},
		{Name: "folders", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/folders", KeyField: ""},
		{Name: "grants_calendars", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/calendars", KeyField: ""},
		{Name: "grants_messages", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/messages", KeyField: ""},
		{Name: "grants_notetakers", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/notetakers", KeyField: ""},
		{Name: "grants_scheduling", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/scheduling/configurations", KeyField: ""},
		{Name: "grants_templates", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/templates", KeyField: ""},
		{Name: "grants_workflows", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/workflows", KeyField: ""},
		{Name: "items", ParentTable: "lists", ParentIDParam: "list_id", PathTemplate: "/v3/lists/{list_id}/items", KeyField: ""},
		{Name: "resources", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/resources", KeyField: ""},
		{Name: "rule_evaluations", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/rule-evaluations", KeyField: ""},
		{Name: "signatures", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/signatures", KeyField: ""},
		{Name: "threads", ParentTable: "grants", ParentIDParam: "grant_id", PathTemplate: "/v3/grants/{grant_id}/threads", KeyField: ""},
	}
}
...
func syncDependentResource(c interface {
	...
	started := time.Now()
	// See syncResource: callers that wrap us in a single-JSON envelope
	// (workflow archive) set quietSyncEvents=true to silence streaming
	// NDJSON events without changing the human-friendly stderr lines.
	emitEvents := !humanFriendly && !quietSyncEvents
