...
// to ~/.nylas-pp-cli/jobs.jsonl; the latest row for a given JobID wins
...
func jobsFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".nylas-pp-cli")
...
~/.nylas-pp-cli/jobs.jsonl. This command lists, inspects, and prunes them.
...
and submitted/updated timestamps. Use --all to see every recorded state
transition rather than just the latest row per job.`,
		Example: `  # Show the most recent 50 jobs
  nylas-pp-cli jobs list

  # Show every recorded state transition
  nylas-pp-cli jobs list --all

  # Limit the table
  nylas-pp-cli jobs list --limit 10`,
...
func newJobsGetCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Long: `Look up a single job by its ID in the local job ledger
(~/.nylas-pp-cli/jobs.jsonl) and print its latest known state. Use this
to check whether a background job has completed without re-polling the
API.`,
		Example: `  # Get the latest state of a job
  nylas-pp-cli jobs get job_abc123

  # JSON output for scripting
  nylas-pp-cli jobs get job_abc123 --json`,
		Annotations: map[string]string{"mcp:read-only": "true"},
...
func newJobsPruneCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Compact the local job ledger by deleting rows whose latest update is
older than the given duration. The default cutoff is 7 days (168h). Use
this to keep ~/.nylas-pp-cli/jobs.jsonl from growing unbounded on
long-lived workstations.`,
		Example: `  # Remove jobs older than the default 7 days
  nylas-pp-cli jobs prune

  # Remove jobs older than 24 hours
  nylas-pp-cli jobs prune --older-than 24h

  # Dry-run via --json (prune still executes; output shows what was pruned)
  nylas-pp-cli jobs prune --older-than 30d --json`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			rows, err := readJobRows()
			if err != nil {
				return err
