...
func buildAgentContext(rootCmd *cobra.Command) agentContext {
	envVars := []agentContextAuthEnvVar{
		{
			...
			Required:    true,
			Sensitive:   true,
			Description: "Nylas v3 API key (Bearer). Get one at https://dashboard-v3.nylas.com.",
		},
	}
	...
	return agentContext{
		...
		CLI: agentContextCLI{
			Name:        "nylas-pp-cli",
			Description: "Every Nylas API, plus a local SQLite mirror, cross-grant search, and confirm-by-hash sending no other Nylas tool has.",
