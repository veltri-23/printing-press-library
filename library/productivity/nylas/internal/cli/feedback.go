...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NYLAS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NYLAS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nylas-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.nylas-pp-cli/feedback.jsonl.
When ` + "`NYLAS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NYLAS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nylas-pp-cli feedback list
  nylas-pp-cli feedback list --limit 5
  nylas-pp-cli feedback list --json`,
