...
In local mode: searches locally synced data only.`,
  ...
  nylas-pp-cli search "error timeout"

  # Force local search only
  nylas-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  nylas-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  nylas-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			case "rules":
				results, err = db.SearchRules(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchRules(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search rules failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
