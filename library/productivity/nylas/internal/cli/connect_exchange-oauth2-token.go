...
func newConnectExchangeOauth2TokenCmd(flags *rootFlags) *cobra.Command {
	var flagBodyJSON string
	var stdinBody bool

	cmd := &cobra.Command{
		Use:         "exchange-oauth2-token",
		Short:       "The standard OAuth token endpoint for Hosted Authentication. This endpoint doesn't require authentication, as it is...",
		Example:     "  nylas-pp-cli connect exchange-oauth2-token",
		Annotations: map[string]string{"pp:endpoint": "connect.exchange-oauth2-token", "pp:method": "POST", "pp:path": "/v3/connect/token"},
