...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Nylas",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  nylas-pp-cli auth setup\n  nylas-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://dashboard-v3.nylas.com")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export NYLAS_API_KEY=\"<your-token>\"")
			fmt.Fprintln(w, "  nylas-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://dashboard-v3.nylas.com"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nylas-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export NYLAS_API_KEY=\"your-token-here\"")
				fmt.Fprintf(w, "  nylas-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nylas-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nylas-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if os.Getenv("NYLAS_API_KEY") != "" {
				envStillSet = "NYLAS_API_KEY"
			} else if os.Getenv("NYLAS_ACCESS_TOKEN") != "" {
				envStillSet = "NYLAS_ACCESS_TOKEN"
			}
