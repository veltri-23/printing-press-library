# Freshservice Printed CLI Agent Guide

This directory is a generated `freshservice-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
freshservice-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
freshservice-pp-cli which "<capability>" --json
freshservice-pp-cli <command> --help
```
...
```bash
freshservice-pp-cli <command> --agent
```
...
```bash
freshservice-pp-cli <command> --help
freshservice-pp-cli <command> --dry-run --agent
```
