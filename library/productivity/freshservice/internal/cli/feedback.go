...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".freshservice-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FRESHSERVICE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FRESHSERVICE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "freshservice-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.freshservice-pp-cli/feedback.jsonl.
When ` + "`FRESHSERVICE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FRESHSERVICE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  freshservice-pp-cli feedback list
  freshservice-pp-cli feedback list --limit 5
  freshservice-pp-cli feedback list --json`,
