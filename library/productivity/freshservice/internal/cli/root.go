...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "freshservice-pp-cli",
		Short: `Freshservice CLI — Every Freshservice operation in one Go binary — with offline search, SLA intelligence, and agent-native JSON that no ot…`,
		Long: `Freshservice CLI — Every Freshservice operation in one Go binary — with offline search, SLA intelligence, and agent-native JSON that no ot…
...
Health check: run 'freshservice-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("freshservice-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'freshservice-pp-cli profile list')")
	...
	rootCmd.AddCommand(newCannedResponsesCmd(flags))
	rootCmd.AddCommand(newChangesCmd(flags))
	rootCmd.AddCommand(newGroupsCmd(flags))
	rootCmd.AddCommand(newProductsCmd(flags))
	rootCmd.AddCommand(newRequestersCmd(flags))
	rootCmd.AddCommand(newServiceCatalogCmd(flags))
	rootCmd.AddCommand(newSolutionsCmd(flags))
	...
	rootCmd.AddCommand(newAgentFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newChangeFormFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newContractsPromotedCmd(flags))
	rootCmd.AddCommand(newDepartmentsPromotedCmd(flags))
	rootCmd.AddCommand(newLocationsPromotedCmd(flags))
	rootCmd.AddCommand(newRequesterFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newTicketFormFieldsPromotedCmd(flags))
	rootCmd.AddCommand(newVendorsPromotedCmd(flags))
	rootCmd.AddCommand(newWorkspacesPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Freshservice-specific transcendence (novel) commands. Each operates over
	// the locally synced corpus; run `freshservice-pp-cli sync` first.
	rootCmd.AddCommand(newBreachRiskCmd(flags))
	rootCmd.AddCommand(newMyQueueCmd(flags))
	rootCmd.AddCommand(newWorkloadCmd(flags))
	rootCmd.AddCommand(newChangeCollisionsCmd(flags))
	rootCmd.AddCommand(newRecurrenceCmd(flags))
	rootCmd.AddCommand(newKBGapsCmd(flags))
	rootCmd.AddCommand(newOrphanAssetsCmd(flags))
	rootCmd.AddCommand(newDeptSLACmd(flags))
	rootCmd.AddCommand(newOnCallGapCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("freshservice-pp-cli %s\n", version)
		},
	}
}
