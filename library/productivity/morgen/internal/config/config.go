...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("MORGEN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "morgen-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("MORGEN_API_KEY"); v != "" {
		cfg.MorgenApiKey = v
		cfg.AuthSource = "env:MORGEN_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.MorgenApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("MORGEN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
