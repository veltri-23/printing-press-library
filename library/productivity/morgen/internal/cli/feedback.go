...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".morgen-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MORGEN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MORGEN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "morgen-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.morgen-pp-cli/feedback.jsonl.
When ` + "`MORGEN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MORGEN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  morgen-pp-cli feedback list
  morgen-pp-cli feedback list --limit 5
  morgen-pp-cli feedback list --json`,
