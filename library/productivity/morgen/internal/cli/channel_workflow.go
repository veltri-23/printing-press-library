...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  morgen-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  morgen-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calendars", "integrations", "tags", "tasks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  morgen-pp-cli workflow status

  # Show status as JSON
  morgen-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("morgen-pp-cli")
			}
