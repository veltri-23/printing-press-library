# Morgen Printed CLI Agent Guide

This directory is a generated `morgen-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
morgen-pp-cli doctor --json
morgen-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
morgen-pp-cli which "<capability>" --json
morgen-pp-cli <command> --help
```
...
```bash
morgen-pp-cli <command> --help
morgen-pp-cli <command> --dry-run --agent
```
