...
Resource scoping:
  ...
  zoom-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zoom-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zoom-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zoom-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zoom-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("zoom-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/zoom-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "managed_domains":
		return db.UpsertManagedDomains(data)
	case "options":
	...
	case "invitation":
		return db.UpsertInvitation(data)
	case "livestream":
		return db.UpsertLivestream(data)
	case "meetings_polls":
		return db.UpsertMeetingsPolls(data)
	case "meetings_recordings":
		return db.UpsertMeetingsRecordings(data)
	case "meetings_registrants":
		return db.UpsertMeetingsRegistrants(data)
	case "meetings_status":
		return db.UpsertMeetingsStatus(data)
	case "past_meetings_instances":
		return db.UpsertPastMeetingsInstances(data)
	case "participants":
		return db.UpsertParticipants(data)
	case "past_webinars_instances":
		return db.UpsertPastWebinarsInstances(data)
	case "report":
		return db.UpsertReport(data)
	case "assistants":
		return db.UpsertAssistants(data)
	case "email":
		return db.UpsertEmail(data)
	case "users_meetings":
		return db.UpsertUsersMeetings(data)
	case "pac":
		return db.UpsertPac(data)
	case "password":
	...
	case "users_recordings":
		return db.UpsertUsersRecordings(data)
	case "schedulers":
		return db.UpsertSchedulers(data)
	case "users_settings":
		return db.UpsertUsersSettings(data)
	case "users_status":
		return db.UpsertUsersStatus(data)
	case "token":
		return db.UpsertToken(data)
	case "users_tsp":
		return db.UpsertUsersTsp(data)
	case "users_webinars":
		return db.UpsertUsersWebinars(data)
	case "panelists":
		return db.UpsertPanelists(data)
	case "webinars_polls":
		return db.UpsertWebinarsPolls(data)
	case "webinars_registrants":
		return db.UpsertWebinarsRegistrants(data)
	case "webinars_status":
		return db.UpsertWebinarsStatus(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "report", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/report/users/{userId}/meetings", KeyField: ""},
		{Name: "users_meetings", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/meetings", KeyField: ""},
		{Name: "users_recordings", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/recordings", KeyField: ""},
		{Name: "users_webinars", ParentTable: "users", ParentIDParam: "userId", PathTemplate: "/users/{userId}/webinars", KeyField: ""},
	}
}
