...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".zoom-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZOOM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZOOM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zoom-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.zoom-pp-cli/feedback.jsonl.
When ` + "`ZOOM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZOOM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoom-pp-cli feedback list
  zoom-pp-cli feedback list --limit 5
  zoom-pp-cli feedback list --json`,
