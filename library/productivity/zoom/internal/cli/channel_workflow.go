...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zoom-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zoom-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"accounts", "h323", "metrics", "tracking-fields", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zoom-pp-cli workflow status

  # Show status as JSON
  zoom-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zoom-pp-cli")
			}
