...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "zoom-pp-cli",
		Short: `Zoom CLI — The first Zoom CLI that joins your local desktop app, your on-disk recordings, and your cloud account into one agent-na…`,
		Long: `Zoom CLI — The first Zoom CLI that joins your local desktop app, your on-disk recordings, and your cloud account into one agent-na…
...
Highlights (not in the official API docs):
  ...
  • notes todos   Scans ingested notes for action-item patterns ('TODO:', 'Action:', '[ ]', '- [ ]', 'Action Item:', 'Next:', 'Follow up:', 'Owner:') and emits a structured to-do list with source meeting topic + date …

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'zoom-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zoom-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'zoom-pp-cli profile list')")
	...
	rootCmd.AddCommand(newPastMeetingsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Hand-authored Zoom commands (local desktop + transcendence). These are
	// re-injected on `regen-merge` per the printing-press protocol.
	rootCmd.AddCommand(newZoomAuthCmd(flags))
	rootCmd.AddCommand(newZoomJoinCmd(flags))
	rootCmd.AddCommand(newZoomStartCmd(flags))
	rootCmd.AddCommand(newZoomSavedCmd(flags))
	rootCmd.AddCommand(newZoomMuteCmd(flags))
	rootCmd.AddCommand(newZoomUnmuteCmd(flags))
	rootCmd.AddCommand(newZoomVideoCmd(flags))
	rootCmd.AddCommand(newZoomLeaveCmd(flags))
	rootCmd.AddCommand(newZoomStatusCmd(flags))
	rootCmd.AddCommand(newZoomRecordingsCmd(flags))
	rootCmd.AddCommand(newZoomFindCmd(flags))
	rootCmd.AddCommand(newZoomStorageCmd(flags))
	rootCmd.AddCommand(newZoomTodayCmd(flags))
	rootCmd.AddCommand(newZoomScheduleCmd(flags))
	rootCmd.AddCommand(newZoomNotesCmd(flags))
