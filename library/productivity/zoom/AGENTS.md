# Zoom Printed CLI Agent Guide

This directory is a generated `zoom-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
zoom-pp-cli doctor --json
zoom-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
zoom-pp-cli which "<capability>" --json
zoom-pp-cli <command> --help
```
...
```bash
zoom-pp-cli <command> --help
zoom-pp-cli <command> --dry-run --agent
```
