...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("GIUSTIZIA_AMMINISTRATIVA_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "giustizia-amministrativa-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("GIUSTIZIA_AMMINISTRATIVA_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
