...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "giustizia-amministrativa-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("GIUSTIZIA_AMMINISTRATIVA_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("GIUSTIZIA_AMMINISTRATIVA_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "giustizia-amministrativa-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/giustizia-amministrativa-pp-cli/feedback.jsonl.
When ` + "`GIUSTIZIA_AMMINISTRATIVA_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`GIUSTIZIA_AMMINISTRATIVA_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
maintainer sees it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			entry := FeedbackEntry{
				Text:      text,
				CLI:       "giustizia-amministrativa-pp-cli",
				Version:   version,
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  giustizia-amministrativa-pp-cli feedback list
  giustizia-amministrativa-pp-cli feedback list --limit 5
  giustizia-amministrativa-pp-cli feedback list --json`,
