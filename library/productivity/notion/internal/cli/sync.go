...
Exit codes & warnings:
  ...
  notion-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  notion-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  notion-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  notion-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  notion-pp-cli sync --latest-only`,
	...
	cmd.AddCommand(newSyncPagesCmd(flags))
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "views":
		return db.UpsertViews(data)
	case "queries":
		return db.UpsertQueries(data)
	default:
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "queries", ParentTable: "views", ParentIDParam: "view_id", PathTemplate: "/v1/views/{view_id}/queries/{query_id}"},
	}
}
