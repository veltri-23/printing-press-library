...
In local mode: searches locally synced data only.`,
  ...
  notion-pp-cli search "error timeout"

  # Force local search only
  notion-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  notion-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  notion-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v1/search
