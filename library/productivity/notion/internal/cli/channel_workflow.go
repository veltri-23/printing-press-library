...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  notion-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  notion-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"comments", "custom-emojis", "file-uploads", "users", "views"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  notion-pp-cli workflow status

  # Show status as JSON
  notion-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("notion-pp-cli")
			}
