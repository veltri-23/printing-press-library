...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  notion-pp-cli doctor
  notion-pp-cli doctor --json
  notion-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export NOTION_BEARER_AUTH=<your-key>"
				} else {
			...
			if os.Getenv("NOTION_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "NOTION_BEARER_AUTH")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "NOTION_BEARER_AUTH")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["Notion-Version"] = "2026-03-11"
						authHeaders["User-Agent"] = "notion-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'notion-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'notion-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'notion-pp-cli sync' to refresh."
	}
	return report
}
