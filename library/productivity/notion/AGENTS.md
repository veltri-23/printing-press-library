# Notion Printed CLI Agent Guide

This directory is a generated `notion-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
notion-pp-cli doctor --json
notion-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
notion-pp-cli which "<capability>" --json
...
```bash
notion-pp-cli <command> --help
notion-pp-cli <command> --dry-run --agent
```
