...
func newGraphCmd(opts *RootOptions) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "graph",
		Short:       "Build a navigation graph (page nodes and from_visit -> to edges) over --since, as JSON or Graphviz --format dot",
		Example:     strings.Trim("chrome-history-pp-cli graph --since 7d --format dot", "\n"),
