...
Resource scoping:
  ...
  sendgrid-pp-cli sync

  # Sync specific resources only
  sendgrid-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sendgrid-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sendgrid-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sendgrid-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sendgrid-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if latestOnly {
				if since == "" {
					...
					_, _ = fmt.Fprintln(os.Stderr, "warning: --latest-only ignored because --since is set; --since takes precedence")
				}
			}
			...
			for res := range results {
				if res.Err != nil {
					...
					if humanFriendly {
						_, _ = fmt.Fprintf(os.Stderr, "  %s: warning: %v\n", res.Resource, res.Warn)
					}
					warnCount++
				} else {
					if humanFriendly {
						_, _ = fmt.Fprintf(os.Stderr, "  %s: %d synced (done)\n", res.Resource, res.Count)
					}
			...
			if humanFriendly {
				if warnCount > 0 {
					_, _ = fmt.Fprintf(os.Stderr, "Sync complete: %d records across %d resources (%d warned, %.1fs)\n",
						totalSynced, totalResources, warnCount, elapsed.Seconds())
				} else {
					_, _ = fmt.Fprintf(os.Stderr, "Sync complete: %d records across %d resources (%.1fs)\n",
						totalSynced, totalResources, elapsed.Seconds())
				}
			} else {
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_summary","total_records":%d,"resources":%d,"success":%d,"warned":%d,"errored":%d,"duration_ms":%d}`+"\n",
			...
			if errCount > 0 && !strict && criticalErrCount == 0 && successCount > 0 {
				if !humanFriendly {
					...
					_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","reason":"exit_policy_default_changed","errored":%d,"message":"%s"}`+"\n",
						errCount, strings.ReplaceAll(msg, `"`, `\"`))
				} else {
					_, _ = fmt.Fprintf(os.Stderr, "warning: %d resource(s) failed but exit code is 0 because the new default treats non-critical failures as warnings. Pass --strict to restore the old behavior, or annotate critical resources with x-critical: true.\n", errCount)
				}
			}
			return nil
		},
...
func syncResource(c interface {
	...
	if !humanFriendly {
		_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_start","resource":"%s"}`+"\n", resource)
	}
	...
	if missingKeys := unresolvedPathKeyRE.FindAllString(path, -1); len(missingKeys) > 0 {
		if !humanFriendly {
			...
			_, _ = fmt.Fprintf(os.Stderr, "  %s skipped (requires parent context: %s)\n",
	...
	if effectiveSince != "" && sinceParam == "" {
		if humanFriendly {
			_, _ = fmt.Fprintf(os.Stderr, "  %s: incremental sync ignored (endpoint declares no temporal filter; falling back to full pagination)\n", resource)
		} else {
			_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","reason":"resource_not_incremental","message":"endpoint does not declare a temporal filter parameter; incremental sync has no effect for this resource"}`+"\n", resource)
		}
		effectiveSince = ""
	}
	...
	for {
		...
		if err != nil {
			if w, ok := isSyncAccessWarning(err); ok {
				if !humanFriendly {
					_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","status":%d,"reason":"%s","message":"%s"}`+"\n",
			...
			if !humanFriendly {
				_, _ = fmt.Fprintln(os.Stdout, syncErrorJSON(resource, "", err))
			}
		...
		if isDryRunResponse(data) {
			if !humanFriendly {
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_dryrun","resource":"%s"}`+"\n", resource)
			}
		...
		if len(items) > 0 && stored == 0 {
			if humanFriendly {
				_, _ = fmt.Fprintf(os.Stderr, "warning: %s returned %d items but stored 0 — the local store will be empty for this resource. Likely cause: scalar item shape rather than objects with extractable IDs.\n", resource, len(items))
			} else {
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_anomaly","resource":"%s","consumed":%d,"stored":0,"reason":"all_items_failed_id_extraction"}`+"\n", resource, len(items))
			}
			anomalyEmitted = true
			...
			if humanFriendly {
				...
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_anomaly","resource":"%s","consumed":%d,"stored":%d,"count":%d,"reason":"primary_key_unresolved"}`+"\n", resource, len(items), stored, extractFailures)
			}
			anomalyEmitted = true
		}
		...
		if humanFriendly {
			if currentRate > 0 {
				_, _ = fmt.Fprintf(os.Stderr, "\r  %s: %d synced [%.1f req/s]", resource, atomic.LoadInt64(&progressCount), currentRate)
			} else {
				_, _ = fmt.Fprintf(os.Stderr, "\r  %s: %d synced", resource, atomic.LoadInt64(&progressCount))
			}
		} else {
			if currentRate > 0 {
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_progress","resource":"%s","fetched":%d,"rate_rps":%.1f}`+"\n", resource, atomic.LoadInt64(&progressCount), currentRate)
			} else {
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_progress","resource":"%s","fetched":%d}`+"\n", resource, atomic.LoadInt64(&progressCount))
			}
		}
		...
		if maxPages > 0 && pagesFetched >= maxPages {
			if !latestOnly {
				if humanFriendly {
					...
					_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","reason":"max_pages_cap_hit","message":"reached --max-pages cap of %d; data may be truncated. Re-run with --max-pages 0 (unlimited) or higher to verify."}`+"\n", resource, maxPages)
				}
			}
			break
		}
		...
		if nextCursor != "" && nextCursor == lastNextCursor {
			if humanFriendly {
				...
				_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","reason":"stuck_pagination","message":"API returned the same next cursor across two pages for resource %s; aborting to prevent budget waste."}`+"\n", resource, resource)
			}
			break
		}
	...
	if consumedTotal > 0 && totalCount == 0 && extractFailureTotal < consumedTotal {
		if humanFriendly {
			...
			_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_anomaly","resource":"%s","consumed":%d,"stored":0,"extract_failures":%d,"reason":"stored_count_zero_after_extraction"}`+"\n", resource, consumedTotal, extractFailureTotal)
		}
	}

	if !humanFriendly {
		_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_complete","resource":"%s","total":%d,"duration_ms":%d}`+"\n", resource, totalCount, time.Since(started).Milliseconds())
	}
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "asm":
		return db.UpsertAsm(data)
	case "browsers":
		return db.UpsertBrowsers(data)
	case "campaigns":
	...
	case "clients_stats":
		return db.UpsertClientsStats(data)
	case "contactdb":
		return db.UpsertContactdb(data)
	case "designs":
		return db.UpsertDesigns(data)
	case "devices":
		return db.UpsertDevices(data)
	case "geo":
		return db.UpsertGeo(data)
	case "ips":
		return db.UpsertIps(data)
	case "logs":
		return db.UpsertLogs(data)
	case "mail":
		return db.UpsertMail(data)
	case "mail-settings":
		return db.UpsertMailSettings(data)
	case "mailbox-providers":
		return db.UpsertMailboxProviders(data)
	case "marketing":
		return db.UpsertMarketing(data)
	case "messages":
		return db.UpsertMessages(data)
	case "partners":
		return db.UpsertPartners(data)
	case "recipients":
		return db.UpsertRecipients(data)
	case "send-ips":
		return db.UpsertSendIps(data)
	case "senders":
		return db.UpsertSenders(data)
	case "resend_verification":
		return db.UpsertResendVerification(data)
	case "sso":
		return db.UpsertSso(data)
	case "stats":
		return db.UpsertStats(data)
	case "subusers":
		return db.UpsertSubusers(data)
	case "credits":
		return db.UpsertCredits(data)
	case "subusers_ips":
		return db.UpsertSubusersIps(data)
	case "subusers_stats":
		return db.UpsertSubusersStats(data)
	case "website_access":
		return db.UpsertWebsiteAccess(data)
	case "suppression":
		return db.UpsertSuppression(data)
	case "teammates":
		return db.UpsertTeammates(data)
	case "subuser_access":
		return db.UpsertSubuserAccess(data)
	case "templates":
		return db.UpsertTemplates(data)
	case "versions":
		return db.UpsertVersions(data)
	case "tracking-settings":
		return db.UpsertTrackingSettings(data)
	case "user":
		return db.UpsertUser(data)
	case "whitelabel":
		return db.UpsertWhitelabel(data)
	default:
...
func defaultSyncResources() []string {
	return []string{
		...
		"categories",
		"contactdb",
		"contactdb-custom-fields",
		"contactdb-exports",
		"contactdb-recipients",
		"contactdb-recipients-count",
		"contactdb-reserved-fields",
		"contactdb-segments",
		"contactdb-status",
		"designs",
		"designs-pre-builts",
		...
		"ips-warmup",
		"mail-settings",
		"mail-settings-address-whitelist",
		...
		"suppression",
		"suppression-bounces",
		"suppression-bounces-classifications",
		"suppression-invalid-emails",
		"suppression-spam-reports",
		"suppression-unsubscribes",
		"teammates",
		"teammates-pending",
		"templates",
		"tracking-settings",
		"tracking-settings-click",
		"tracking-settings-google-analytics",
		"tracking-settings-open",
		"tracking-settings-subscription",
		"user",
		"user-account",
		"user-credits",
		"user-profile",
		"user-scheduled-sends",
		"user-settings-enforced-tls",
		"user-username",
		"user-webhooks-parse-settings",
		"user-webhooks-security-policies",
		"validations",
		"verified-senders",
		"verified-senders-domains",
		"verified-senders-steps-completed",
		"whitelabel",
		"whitelabel-domains",
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"access-settings":                     "/v3/access_settings/activity",
		"access-settings-whitelist":           "/v3/access_settings/whitelist",
		"alerts":                              "/v3/alerts",
		"api-keys":                            "/v3/api_keys",
		"asm":                                 "/v3/asm/groups",
		"asm-suppressions":                    "/v3/asm/suppressions",
		"campaigns":                           "/v3/campaigns",
		"categories":                          "/v3/categories",
		"contactdb":                           "/v3/contactdb/lists",
		"contactdb-custom-fields":             "/v3/contactdb/custom_fields",
		"contactdb-exports":                   "/v3/contactdb/exports",
		"contactdb-recipients":                "/v3/contactdb/recipients",
		"contactdb-recipients-count":          "/v3/contactdb/recipients/count",
		"contactdb-reserved-fields":           "/v3/contactdb/reserved_fields",
		"contactdb-segments":                  "/v3/contactdb/segments",
		"contactdb-status":                    "/v3/contactdb/status",
		"designs":                             "/v3/designs",
		"designs-pre-builts":                  "/v3/designs/pre-builts",
		"ips":                                 "/v3/ips",
		"ips-assigned":                        "/v3/ips/assigned",
		"ips-pools":                           "/v3/ips/pools",
		"ips-remaining":                       "/v3/ips/remaining",
		"ips-warmup":                          "/v3/ips/warmup",
		"mail-settings":                       "/v3/mail_settings",
		"mail-settings-address-whitelist":     "/v3/mail_settings/address_whitelist",
		"mail-settings-bounce-purge":          "/v3/mail_settings/bounce_purge",
		"mail-settings-footer":                "/v3/mail_settings/footer",
		"mail-settings-forward-bounce":        "/v3/mail_settings/forward_bounce",
		"mail-settings-forward-spam":          "/v3/mail_settings/forward_spam",
		"mail-settings-template":              "/v3/mail_settings/template",
		"marketing":                           "/v3/marketing/lists",
		"marketing-contacts":                  "/v3/marketing/contacts",
		"marketing-contacts-count":            "/v3/marketing/contacts/count",
		"marketing-contacts-exports":          "/v3/marketing/contacts/exports",
		"marketing-field-definitions":         "/v3/marketing/field_definitions",
		"marketing-integrations":              "/v3/marketing/integrations",
		"marketing-segments":                  "/v3/marketing/segments",
		"marketing-segments-2-0":              "/v3/marketing/segments/2.0",
		"marketing-senders":                   "/v3/marketing/senders",
		"marketing-singlesends":               "/v3/marketing/singlesends",
		"marketing-singlesends-categories":    "/v3/marketing/singlesends/categories",
		"marketing-stats-automations":         "/v3/marketing/stats/automations",
		"marketing-stats-singlesends":         "/v3/marketing/stats/singlesends",
		"partner-settings":                    "/v3/partner_settings",
		"partners":                            "/v3/partners/accounts",
		"partners-offerings":                  "/v3/partners/offerings",
		"scopes":                              "/v3/scopes",
		"send-ips":                            "/v3/send_ips/ips",
		"send-ips-pools":                      "/v3/send_ips/pools",
		"senders":                             "/v3/senders",
		"sso":                                 "/v3/sso/integrations",
		"subusers":                            "/v3/subusers",
		"subusers-reputations":                "/v3/subusers/reputations",
		"suppression":                         "/v3/suppression/blocks",
		"suppression-bounces":                 "/v3/suppression/bounces",
		"suppression-bounces-classifications": "/v3/suppression/bounces/classifications",
		"suppression-invalid-emails":          "/v3/suppression/invalid_emails",
		"suppression-spam-reports":            "/v3/suppression/spam_reports",
		"suppression-unsubscribes":            "/v3/suppression/unsubscribes",
		"teammates":                           "/v3/teammates",
		"teammates-pending":                   "/v3/teammates/pending",
		"templates":                           "/v3/templates",
		"tracking-settings":                   "/v3/tracking_settings",
		"tracking-settings-click":             "/v3/tracking_settings/click",
		"tracking-settings-google-analytics":  "/v3/tracking_settings/google_analytics",
		"tracking-settings-open":              "/v3/tracking_settings/open",
		"tracking-settings-subscription":      "/v3/tracking_settings/subscription",
		"user":                                "/v3/user/email",
		"user-account":                        "/v3/user/account",
		"user-credits":                        "/v3/user/credits",
		"user-profile":                        "/v3/user/profile",
		"user-scheduled-sends":                "/v3/user/scheduled_sends",
		"user-settings-enforced-tls":          "/v3/user/settings/enforced_tls",
		"user-username":                       "/v3/user/username",
		"user-webhooks-parse-settings":        "/v3/user/webhooks/parse/settings",
		"user-webhooks-security-policies":     "/v3/user/webhooks/security/policies",
		"validations":                         "/v3/validations/email/jobs",
		"verified-senders":                    "/v3/verified_senders",
		"verified-senders-domains":            "/v3/verified_senders/domains",
		"verified-senders-steps-completed":    "/v3/verified_senders/steps_completed",
		"whitelabel":                          "/v3/whitelabel/ips",
		"whitelabel-domains":                  "/v3/whitelabel/domains",
		"whitelabel-domains-default":          "/v3/whitelabel/domains/default",
		"whitelabel-links":                    "/v3/whitelabel/links",
		"whitelabel-links-default":            "/v3/whitelabel/links/default",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "send_ips", ParentTable: "ips", ParentIDParam: "ip", PathTemplate: "/v3/send_ips/ips/{ip}/subusers", KeyField: ""},
		{Name: "subuser_access", ParentTable: "teammates", ParentIDParam: "teammate_name", PathTemplate: "/v3/teammates/{teammate_name}/subuser_access", KeyField: ""},
		{Name: "subusers_stats", ParentTable: "subusers", ParentIDParam: "subuser_name", PathTemplate: "/v3/subusers/{subuser_name}/stats/monthly", KeyField: ""},
	}
}
...
func syncDependentResource(c interface {
	...
	if err != nil || len(parentIDs) == 0 {
		if len(parentIDs) == 0 {
			if humanFriendly {
				_, _ = fmt.Fprintf(os.Stderr, "  %s: skipping (parent table %s is empty, sync it first)\n", dep.Name, dep.ParentTable)
			}
	...
	if humanFriendly {
		_, _ = fmt.Fprintf(os.Stderr, "  %s: syncing for %d %s parents\n", dep.Name, len(parentIDs), dep.ParentTable)
	}
	...
	for idx, parentID := range parentIDs {
		...
		if humanFriendly {
			_, _ = fmt.Fprintf(os.Stderr, "\r  %s: syncing for %s (%d/%d parents)", dep.Name, dep.ParentTable, idx+1, len(parentIDs))
		}
		...
		for {
			...
			if err != nil {
				...
				if w, ok := isSyncAccessWarning(err); ok {
					...
					if humanFriendly {
						...
						_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","parent":"%s","status":%d,"reason":"%s","message":"%s"}`+"\n",
					...
					_, _ = fmt.Fprintln(os.Stdout, syncErrorJSON(dep.Name, parentID, err))
				}
				break
			}
			...
			if len(items) > 0 && stored == 0 && !depAnomalyEmitted {
				if humanFriendly {
					...
					_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_anomaly","resource":"%s","parent":"%s","consumed":%d,"stored":0,"reason":"all_items_failed_id_extraction"}`+"\n", dep.Name, parentID, len(items))
				}
				...
				if humanFriendly {
					...
					_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_anomaly","resource":"%s","parent":"%s","consumed":%d,"stored":%d,"count":%d,"reason":"primary_key_unresolved"}`+"\n", dep.Name, parentID, len(items), stored, extractFailures)
				}
				depAnomalyEmitted = true
			}
			...
			if maxPages > 0 && pagesFetched >= maxPages {
				if !latestOnly {
					if humanFriendly {
						...
						_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","parent":"%s","reason":"max_pages_cap_hit","message":"reached --max-pages cap of %d; data may be truncated. Re-run with --max-pages 0 (unlimited) or higher to verify."}`+"\n", dep.Name, parentID, maxPages)
					}
				}
				break
			}
			...
			if nextCursor != "" && nextCursor == lastNextCursor {
				if humanFriendly {
					...
					_, _ = fmt.Fprintf(os.Stdout, `{"event":"sync_warning","resource":"%s","parent":"%s","reason":"stuck_pagination","message":"API returned the same next cursor across two pages for resource %s; aborting to prevent budget waste."}`+"\n", dep.Name, parentID, dep.Name)
				}
				break
			}
			lastNextCursor = nextCursor
