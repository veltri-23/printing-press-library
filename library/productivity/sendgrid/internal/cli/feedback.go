...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".sendgrid-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SENDGRID_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SENDGRID_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sendgrid-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.sendgrid-pp-cli/feedback.jsonl.
When ` + "`SENDGRID_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SENDGRID_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sendgrid-pp-cli feedback list
  sendgrid-pp-cli feedback list --limit 5
  sendgrid-pp-cli feedback list --json`,
