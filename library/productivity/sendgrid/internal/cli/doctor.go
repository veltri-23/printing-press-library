...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sendgrid-pp-cli doctor
  sendgrid-pp-cli doctor --json
  sendgrid-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export SENDGRID_API_KEY=<your-key>"
					report["auth_docs_url"] = "https://support.sendgrid.com/hc/en-us"
				} else {
					authConfigured = true
			...
			if os.Getenv("SENDGRID_API_KEY") != "" {
				authEnvSet = append(authEnvSet, "SENDGRID_API_KEY")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "SENDGRID_API_KEY")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "sendgrid-pp-cli"
			...
			if docsURL, ok := report["auth_docs_url"]; ok {
				_, _ = fmt.Fprintf(w, "  See API docs: %v\n", docsURL)
			}
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'sendgrid-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'sendgrid-pp-cli sync' to populate."
		return report
	}
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'sendgrid-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'sendgrid-pp-cli sync' to refresh."
	}
	return report
}
