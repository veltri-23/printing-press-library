...
In local mode: searches locally synced data only.`,
  ...
  sendgrid-pp-cli search "error timeout"

  # Force local search only
  sendgrid-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  sendgrid-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  sendgrid-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v3/marketing/contacts/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/v3/marketing/contacts/search", map[string]any{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'sendgrid-pp-cli sync' first to populate the local database", err)
			}
			...
			switch resourceType {
			case "asm":
				results, err = db.SearchAsm(query, limit)
			case "campaigns":
				results, err = db.SearchCampaigns(query, limit)
			case "contactdb":
				results, err = db.SearchContactdb(query, limit)
			case "marketing":
				results, err = db.SearchMarketing(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchAsm(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search asm failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchCampaigns(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search campaigns failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchContactdb(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search contactdb failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchMarketing(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search marketing failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
