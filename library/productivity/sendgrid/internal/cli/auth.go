...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Twilio Sendgrid",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  sendgrid-pp-cli auth setup\n  sendgrid-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			_, _ = fmt.Fprintln(w, "See API docs: https://support.sendgrid.com/hc/en-us")
			_, _ = fmt.Fprintln(w, "")
			_, _ = fmt.Fprintln(w, "Then set:")
			_, _ = fmt.Fprintln(w, "  export SENDGRID_API_KEY=\"<your-token>\"")
			_, _ = fmt.Fprintln(w, "  sendgrid-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://support.sendgrid.com/hc/en-us"
			if cliutil.IsVerifyEnv() {
				_, _ = fmt.Fprintf(w, "would launch: %s\n", launchURL)
				return nil
			}
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sendgrid-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				_, _ = fmt.Fprintln(w, "  export SENDGRID_API_KEY=\"your-token-here\"")
				_, _ = fmt.Fprintf(w, "  sendgrid-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sendgrid-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  sendgrid-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("SENDGRID_API_KEY") != "" {
				envStillSet = "SENDGRID_API_KEY"
			}
