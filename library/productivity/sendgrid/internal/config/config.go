...
type Config struct {
	...
	Path           string            `toml:"-"`
	SendgridApiKey string            `toml:"api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SENDGRID_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sendgrid-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("SENDGRID_API_KEY"); v != "" {
		...
		cfg.AuthSource = "env:SENDGRID_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SendgridApiKey != "" {
		cfg.AuthSource = "config"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("SENDGRID_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SendgridApiKey != "" {
		c.AuthSource = "env:SENDGRID_API_KEY"
		return "Bearer " + c.SendgridApiKey
	}
	if c.AccessToken != "" {
		c.AuthSource = "oauth2"
