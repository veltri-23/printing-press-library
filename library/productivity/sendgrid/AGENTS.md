# Twilio SendGrid Printed CLI Agent Guide

This directory is a generated `sendgrid-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sendgrid-pp-cli doctor --json
sendgrid-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sendgrid-pp-cli which "<capability>" --json
sendgrid-pp-cli <command> --help
```
...
```bash
sendgrid-pp-cli <command> --help
sendgrid-pp-cli <command> --dry-run --agent
```
