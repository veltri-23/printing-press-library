...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mobbin-pp-cli doctor
  mobbin-pp-cli doctor --json
  mobbin-pp-cli doctor --fail-on warn
  mobbin-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "mobbin-pp-cli auth login --chrome"
				} else {
					authConfigured = true
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = "mobbin.com"
				}
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					healthPath := "/api/searchable-apps/web"
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "mobbin-pp-cli/0.1.0 (+https://github.com/mvanhorn/cli-printing-press)"
						authHeaders["Accept"] = "application/json, text/html, */*"
						authHeaders["Origin"] = "https://mobbin.com"
						authHeaders["Referer"] = "https://mobbin.com/"
						verifyPath := "https://ujasntkfphywizsdaapi.supabase.co/rest/v1/workspaces?select=id&limit=1"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'mobbin-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'mobbin-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "empty"
		report["hint"] = "Cache is empty; run 'mobbin-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'mobbin-pp-cli sync' to refresh."
	}
	return report
}
