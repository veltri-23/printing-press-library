...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Mobbin",
...
Use --chrome to read cookies from Chrome for mobbin.com.
...
profile by name when the installed backend supports it.`,
		Example: `  mobbin-pp-cli auth login --chrome
  mobbin-pp-cli auth login --browser
  mobbin-pp-cli auth login --cookies-file storage-state.json
  mobbin-pp-cli auth login --chrome --profile "Work"`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !browserFlag && cookiesFile == "" {
				...
				fmt.Fprintf(cmd.OutOrStdout(), "  mobbin-pp-cli auth login --chrome\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  mobbin-pp-cli auth login --browser\n")
				fmt.Fprintf(cmd.OutOrStdout(), "  mobbin-pp-cli auth login --cookies-file storage-state.json\n")
				return nil
			}
			if browserFlag && cookiesFile != "" {
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mobbin-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			if header == "" {
				...
				fmt.Fprintf(w, "  mobbin-pp-cli auth login --chrome\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}

			fmt.Fprintln(w, green("Authenticated"))
			fmt.Fprintf(w, "  Source: %s\n", cfg.AuthSource)
			fmt.Fprintf(w, "  Domain: mobbin.com\n")
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mobbin-pp-cli auth set-token eyJhbGciOi...",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  mobbin-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
...
func extractViaPycookiecheat(tool cookieTool, domain, profileDir string) (string, error) {
	...
	var script string
	// Build Python string literals with json.Marshal rather than raw %s
	// interpolation: profileDir (and thus cookiePath) comes from the
	// user-controlled --profile flag, so a stray quote or backslash could
	// otherwise break out of the literal and inject Python. Forward slashes
	// also keep Windows backslashes from being read as escapes.
