...
func newSyncCmd(flags *rootFlags) *cobra.Command {
	...
	// Domain-population phase flags (Mobbin mirror tables).
	var domainPlatformsCSV string
	var domainLimitPerCategory int
	var domainAppsPerPlatform int
	var domainNoScrape bool
	var domainIncludeCollections bool
	var noDomain bool
...
Resource scoping:
  ...
  mobbin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  mobbin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  mobbin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  mobbin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  mobbin-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Mobbin domain-population phase runs BEFORE the framework
			// exit-code policy: the only flat-list resource (workspaces)
			// needs auth, so the framework phase syncs nothing without a
			// session. Domain population is the real data path (apps,
			// screens, dictionary) and its row count counts toward a
			// successful sync so the framework "nothing synced" error does
			// not mask a working domain sync. Runs under verify so the mock
			// server populates the domain tables (data_pipeline gate); only
			// --dry-run and --no-domain skip.
			domainRows := 0
			if !noDomain && !c.DryRun {
				domainRows = runDomainSync(cmd.Context(), c, db, domainSyncOpts{
					Platforms:          splitCSV(domainPlatformsCSV),
					LimitPerCategory:   domainLimitPerCategory,
					AppsPerPlatform:    domainAppsPerPlatform,
					NoScrape:           domainNoScrape,
					IncludeCollections: domainIncludeCollections,
				}, os.Stderr)
			}
			...
			if successCount == 0 && domainRows == 0 {
	...
	cmd.Flags().StringVar(&domainPlatformsCSV, "platform", "web,ios", "Domain phase: comma-separated platforms to populate (web, ios, android)")
	cmd.Flags().IntVar(&domainLimitPerCategory, "limit-per-category", 50, "Domain phase: apps returned per popular category")
	cmd.Flags().IntVar(&domainAppsPerPlatform, "apps-per-platform", 200, "Domain phase: max apps per platform to HTML-scrape for flows/screens")
	cmd.Flags().BoolVar(&domainNoScrape, "no-scrape", false, "Domain phase: skip the app-page HTML scrape pass (flows/app_versions)")
	cmd.Flags().BoolVar(&domainIncludeCollections, "include-collections", false, "Domain phase: also sync user collections")
	cmd.Flags().BoolVar(&noDomain, "no-domain", false, "Skip the Mobbin domain-population phase (framework resource sync only)")
