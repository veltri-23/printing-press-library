...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "mobbin-pp-cli",
		Short: `Browse Mobbin's 621,500+ shipped UI screens from the terminal, with offline SQLite mirror and design-deck workflows no other Mobbin tool ships.`,
		Long: `Browse Mobbin's 621,500+ shipped UI screens from the terminal, with offline SQLite mirror and design-deck workflows no other Mobbin tool ships.
...
Health check: run 'mobbin-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("mobbin-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'mobbin-pp-cli profile list')")
	...
	rootCmd.AddCommand(newBenchCmd(flags))
	rootCmd.AddCommand(newCrossCmd(flags))
	rootCmd.AddCommand(newDeckCmd(flags))
	rootCmd.AddCommand(newDriftCmd(flags))
	rootCmd.AddCommand(newGrabCmd(flags))
	...
	rootCmd.AddCommand(newPatternsCmd(flags))
	rootCmd.AddCommand(newElementsCmd(flags))
	...
	rootCmd.AddCommand(newFiltersPromotedCmd(flags))
	rootCmd.AddCommand(newFlowsPromotedCmd(flags))
	rootCmd.AddCommand(newScreensPromotedCmd(flags))
