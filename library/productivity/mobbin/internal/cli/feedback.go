...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MOBBIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MOBBIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "mobbin-pp-cli/feedback")
...
When ` + "`MOBBIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MOBBIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  mobbin-pp-cli feedback list
  mobbin-pp-cli feedback list --limit 5
  mobbin-pp-cli feedback list --json`,
