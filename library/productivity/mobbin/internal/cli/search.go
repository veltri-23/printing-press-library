...
In local mode: searches locally synced data only.`,
  ...
  mobbin-pp-cli search "error timeout"

  # Force local search only
  mobbin-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  mobbin-pp-cli search "status" --type apps --data-source local
  # JSON output for piping
  mobbin-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /api/search-bar/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/api/search-bar/search", map[string]any{
					"query":      query,
					"experience": "apps",
					"platform":   "ios",
				})
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'mobbin-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
