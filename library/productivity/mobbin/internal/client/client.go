...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// Cookie-auth: send the full stored cookie header verbatim for
		// website-origin (mobbin.com) requests. CookieCredential() holds the
		// raw "name=val; name=val" jar captured at login — all session
		// cookies, including the split sb-...-auth-token.0/.1 in ORIGINAL
		// form. Mobbin's /api endpoints reject a single reassembled token
		// ("unauthenticated") and require the whole jar. The Supabase REST
		// branch below deletes this Cookie and swaps in the reassembled
		// Bearer + apikey for its own host, so the two auth modes don't mix.
		if c.Config != nil {
			if cookie := c.Config.CookieCredential(); cookie != "" {
				req.Header.Set("Cookie", cookie)
			}
		}
		req.Header.Set("User-Agent", "mobbin-pp-cli/0.1.0 (+https://github.com/mvanhorn/cli-printing-press)")
		req.Header.Set("Accept", "application/json, text/html, */*")
		req.Header.Set("Origin", "https://mobbin.com")
		...
		// Mobbin API-truth: Supabase PostgREST endpoints (collections,
		// workspaces) authenticate with a bearer token reassembled from the
		// split SSR cookie plus the publishable apikey, not the session
		// cookie. Drop any Cookie the jar would attach and set the REST
		// headers here so nothing above overwrites them.
		if c.Config != nil && isSupabaseRESTURL(req.URL) {
			req.Header.Del("Cookie")
			token, err := c.Config.SupabaseAccessToken()
			if err != nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "mobbin-pp-cli auth set-token <token>")
}
