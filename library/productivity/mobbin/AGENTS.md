# Mobbin Printed CLI Agent Guide

This directory is a generated `mobbin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
mobbin-pp-cli doctor --json
mobbin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
mobbin-pp-cli which "<capability>" --json
mobbin-pp-cli <command> --help
```
...
```bash
mobbin-pp-cli <command> --help
mobbin-pp-cli <command> --dry-run --agent
```
