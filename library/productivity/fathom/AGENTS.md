# Fathom Printed CLI Agent Guide

This directory is a generated `fathom-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
fathom-pp-cli doctor --json
fathom-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
fathom-pp-cli which "<capability>" --json
...
```bash
fathom-pp-cli <command> --help
fathom-pp-cli <command> --dry-run --agent
```
