...
Exit codes & warnings:
  ...
  fathom-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  fathom-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  fathom-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  fathom-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  fathom-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/fathom-pp-cli/data.db)")
...
func syncResource(c interface {
	...
	// PATCH: Fathom's cursor is a positional token in a descending-order list.
	// For incremental syncs, the saved cursor points to the oldest meetings
	// already fetched — resuming from it would re-fetch stale data rather than
	// new meetings. Instead, start from page 1 (newest) and let created_after
	// filter to only new meetings. Only resume the cursor on --full restarts
	// where we genuinely want to continue an interrupted backfill.
	cursor := existingCursor
	if !full && effectiveSince != "" {
		cursor = ""
	}
	...
	for {
		...
		// PATCH: Fathom /meetings endpoint omits rich fields by default.
		// Always request all include flags so action_items, transcript,
		// summary, crm_matches, and calendar_invitees are populated.
		...
		// PATCH: Fathom uses next_cursor presence (not has_more) to signal more pages.
		// Treat a non-empty next_cursor as hasMore=true even when the API omits has_more.
		effectiveHasMore := hasMore || nextCursor != ""
		if !effectiveHasMore || len(items) == 0 {
			break
		}
...
// PATCH: Fathom uses "cursor" as the query param (not "after"); spec declared
// cursor_param: cursor but the profiler defaulted to "after".
...
// PATCH: Fathom uses "created_after" (ISO 8601) for time-based filtering, not "since".
