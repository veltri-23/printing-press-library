...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".fathom-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FATHOM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FATHOM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fathom-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.fathom-pp-cli/feedback.jsonl.
When ` + "`FATHOM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FATHOM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fathom-pp-cli feedback list
  fathom-pp-cli feedback list --limit 5
  fathom-pp-cli feedback list --json`,
