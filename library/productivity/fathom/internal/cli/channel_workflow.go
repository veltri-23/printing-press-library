...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  fathom-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  fathom-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"meetings", "team-members", "teams"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  fathom-pp-cli workflow status

  # Show status as JSON
  fathom-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("fathom-pp-cli")
			}
