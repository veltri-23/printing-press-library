# Calendly Printed CLI Agent Guide

This directory is a generated `calendly-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
calendly-pp-cli doctor --json
calendly-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
calendly-pp-cli which "<capability>" --json
calendly-pp-cli <command> --help
```
...
```bash
calendly-pp-cli <command> --help
calendly-pp-cli <command> --dry-run --agent
```
