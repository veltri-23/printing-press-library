...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "calendly-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CALENDLY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CALENDLY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "github.com/mvanhorn/printing-press-library/library/productivity/calendly/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/github.com/mvanhorn/printing-press-library/library/productivity/calendly/feedback.jsonl.
When ` + "`CALENDLY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CALENDLY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  calendly-pp-cli feedback list
  calendly-pp-cli feedback list --limit 5
  calendly-pp-cli feedback list --json`,
