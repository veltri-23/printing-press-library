...
Resource scoping:
  ...
  calendly-pp-cli sync

  # Sync specific resources only
  calendly-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  calendly-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  calendly-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  calendly-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  calendly-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/github.com/mvanhorn/printing-press-library/library/productivity/calendly/data.db)")
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"event-type-memberships":   "/event_type_memberships",
		"event-types":              "/event_types",
		"organization-memberships": "/organization_memberships",
		"routing-forms":            "/routing_forms",
		"webhook-subscriptions":    "/webhook_subscriptions",
	}
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "submissions", ParentTable: "routing-forms", ParentIDParam: "uuid", PathTemplate: "/routing_forms/{uuid}/submissions", KeyField: "", PathParams: []dependentPathParamDef{
