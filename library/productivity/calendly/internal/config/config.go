...
type Config struct {
	...
	CalendlyBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("CALENDLY_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "calendly-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("CALENDLY_BEARER_AUTH"); v != "" {
		cfg.CalendlyBearerAuth = v
		cfg.markEnvOverride("CalendlyBearerAuth")
		cfg.AuthSource = "env:CALENDLY_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.CalendlyBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("CALENDLY_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.CalendlyBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:CALENDLY_BEARER_AUTH"
		}
		return "Bearer " + c.CalendlyBearerAuth
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.CalendlyBearerAuth = ""
	delete(c.envOverrides, "CalendlyBearerAuth")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["CalendlyBearerAuth"] {
			out.CalendlyBearerAuth = c.fileConfig.CalendlyBearerAuth
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "CalendlyBearerAuth":
		c.fileConfig.CalendlyBearerAuth = c.CalendlyBearerAuth
	}
}
