...
// TestMigrate_AddsColumnsOnUpgrade_TechmemeSearch verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_TechmemeSearch(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE techmeme_search (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(techmeme_search)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from techmeme_search after migrate", want)
		}
	}
}
