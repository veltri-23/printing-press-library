...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".techmeme-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TECHMEME_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TECHMEME_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "techmeme-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.techmeme-pp-cli/feedback.jsonl.
When ` + "`TECHMEME_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TECHMEME_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  techmeme-pp-cli feedback list
  techmeme-pp-cli feedback list --limit 5
  techmeme-pp-cli feedback list --json`,
