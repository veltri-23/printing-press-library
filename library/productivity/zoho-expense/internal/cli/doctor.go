...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoho-expense-pp-cli doctor
  zoho-expense-pp-cli doctor --json
  zoho-expense-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if os.Getenv("ZOHO_EXPENSE_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_EXPENSE_CLIENT_ID")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_EXPENSE_CLIENT_ID set during auth login")
			}
			if os.Getenv("ZOHO_EXPENSE_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_EXPENSE_CLIENT_SECRET")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_EXPENSE_CLIENT_SECRET set during auth login")
			}
			if os.Getenv("ZOHO_EXPENSE_REFRESH_TOKEN") != "" || authHeader != "" {
				authEnvSet = append(authEnvSet, "ZOHO_EXPENSE_REFRESH_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "ZOHO_EXPENSE_REFRESH_TOKEN populated automatically by auth login")
			}
			if os.Getenv("ZOHO_EXPENSE_ORGANIZATION_ID") != "" {
				authEnvSet = append(authEnvSet, "ZOHO_EXPENSE_ORGANIZATION_ID")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "ZOHO_EXPENSE_ORGANIZATION_ID")
			}
			switch {
			...
			default:
				report["env_vars"] = fmt.Sprintf("OK %d/%d available", len(authEnvSet), 4)
			}
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "zoho-expense-pp-cli"
						verifyPath := "/organizations"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'zoho-expense-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'zoho-expense-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'zoho-expense-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'zoho-expense-pp-cli sync' to refresh."
	}
	return report
}
