...
In local mode: searches locally synced data only.`,
  ...
  zoho-expense-pp-cli search "error timeout"

  # Force local search only
  zoho-expense-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  zoho-expense-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  zoho-expense-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'zoho-expense-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "expense_categories":
				results, err = db.SearchExpenseCategories(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchExpenseCategories(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search expense_categories failed: %w", searchErr)
					}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/zoho-expense-pp-cli/data.db)")
