...
Resource scoping:
  ...
  zoho-expense-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  zoho-expense-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  zoho-expense-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  zoho-expense-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  zoho-expense-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "expense_categories":
		return db.UpsertExpenseCategories(data)
	case "expense_reports":
		return db.UpsertExpenseReports(data)
	case "expenses":
	...
	case "reporting_tags":
		return db.UpsertReportingTags(data)
	case "taxes":
		return db.UpsertTaxes(data)
	case "trips":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"expenses":           "/reports/expensedetails",
