...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".zoho-expense-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ZOHO_EXPENSE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ZOHO_EXPENSE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "zoho-expense-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.zoho-expense-pp-cli/feedback.jsonl.
When ` + "`ZOHO_EXPENSE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ZOHO_EXPENSE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  zoho-expense-pp-cli feedback list
  zoho-expense-pp-cli feedback list --limit 5
  zoho-expense-pp-cli feedback list --json`,
