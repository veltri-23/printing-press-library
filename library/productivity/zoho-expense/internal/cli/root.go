...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Short: `Upload receipts, tag expenses, and bundle monthly reports on Zoho Expense from the terminal`,
		Long: `Upload receipts, tag expenses, and bundle monthly reports on Zoho Expense from the terminal

Highlights (not in the official API docs):
  • invoice ingest   Batch upload a folder of invoices to Zoho Expense, SHA256-dedup against the local store, poll autoscan in parallel, and auto-tag from learned merchant memory — the headline workflow for AI agents tha…
...
Health check: run 'zoho-expense-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("zoho-expense-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'zoho-expense-pp-cli profile list')")
	...
	rootCmd.AddCommand(newExpenseCategoriesCmd(flags))
	rootCmd.AddCommand(newExpenseReportsCmd(flags))
	...
	rootCmd.AddCommand(newReportingTagsCmd(flags))
	rootCmd.AddCommand(newTaxesCmd(flags))
	...
	rootCmd.AddCommand(newReceiptsPromotedCmd(flags))
	rootCmd.AddCommand(newVersionCliCmd())

	// Novel hand-authored commands (see internal/cli/zoho_*.go).
	rootCmd.AddCommand(newOrgCmd(flags))
	rootCmd.AddCommand(newReceiptCmd(flags))
	rootCmd.AddCommand(newInvoiceCmd(flags))
	rootCmd.AddCommand(newExpenseTagCmd(flags))
	rootCmd.AddCommand(newExpenseUntaggedCmd(flags))
	rootCmd.AddCommand(newCloseCmd(flags))
	rootCmd.AddCommand(newMerchantCmd(flags))
	rootCmd.AddCommand(newGSTSplitCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("zoho-expense-pp-cli %s\n", version)
		},
	}
}
