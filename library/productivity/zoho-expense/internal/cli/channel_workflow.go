...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  zoho-expense-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  zoho-expense-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"currencies", "customers", "expense_categories", "expense_reports", "expenses", "organizations", "projects", "reporting_tags", "taxes", "trips", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  zoho-expense-pp-cli workflow status

  # Show status as JSON
  zoho-expense-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("zoho-expense-pp-cli")
			}
