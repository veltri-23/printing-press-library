...
type Config struct {
	...
	Path           string    `toml:"-"`
	OpensnowApiKey string    `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://api.opensnow.com",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("OPENSNOW_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "opensnow-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("OPENSNOW_API_KEY"); v != "" {
		cfg.OpensnowApiKey = v
		cfg.AuthSource = "env:OPENSNOW_API_KEY"
	}

	// Base URL override (used by printing-press verify to point at mock/test servers)
	if v := os.Getenv("OPENSNOW_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
