...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".opensnow-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENSNOW_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENSNOW_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "opensnow-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.opensnow-pp-cli/feedback.jsonl.
When ` + "`OPENSNOW_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENSNOW_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  opensnow-pp-cli feedback list
  opensnow-pp-cli feedback list --limit 5
  opensnow-pp-cli feedback list --json`,
