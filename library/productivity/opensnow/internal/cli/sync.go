...
Exit codes & warnings:
  ...
  opensnow-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  opensnow-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  opensnow-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  opensnow-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  opensnow-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/opensnow-pp-cli/data.db)")
