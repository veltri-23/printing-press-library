...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".slack-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SLACK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SLACK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "slack-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.slack-pp-cli/feedback.jsonl.
When ` + "`SLACK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SLACK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
