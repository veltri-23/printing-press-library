...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  slack-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  slack-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"conversations", "emoji", "files", "reactions", "reminders", "stars", "usergroups", "users"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  slack-pp-cli workflow status

  # Show status as JSON
  slack-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("slack-pp-cli")
			}
