...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TICKTICK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TICKTICK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ticktick-pp-cli/feedback")
...
When ` + "`TICKTICK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TICKTICK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ticktick-pp-cli feedback list
  ticktick-pp-cli feedback list --limit 5
  ticktick-pp-cli feedback list --json`,
