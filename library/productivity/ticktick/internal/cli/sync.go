...
Resource scoping:
  ...
  ticktick-pp-cli sync

  # Sync specific resources only
  ticktick-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ticktick-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ticktick-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ticktick-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ticktick-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "completed":
		return db.UpsertCompleted(data)
	case "focus":
		return db.UpsertFocus(data)
	case "habits":
		return db.UpsertHabits(data)
	case "projects":
...
func responsePathForResource(resource, path string) []string {
	...
	case "tasks\x00/batch/check/0":
		return []string{"syncTaskBean.update"}
	}
	return nil
}
