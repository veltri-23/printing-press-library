...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "ticktick-pp-cli",
		Short: `Every TickTick surface — tasks, habits, focus, daily notes — with a corruption-proof daily-note contract and offline analytics no other TickTick tool has.`,
		Long: `TickTick CLI covering every TickTick surface — tasks, habits, focus, and daily notes — with a corruption-proof daily-note contract and an offline SQLite mirror for search and analytics.
...
Health check: run 'ticktick-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("ticktick-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'ticktick-pp-cli profile list')")
	...
	rootCmd.AddCommand(newHabitsCmd(flags))
	rootCmd.AddCommand(newTasksCmd(flags))
	rootCmd.AddCommand(newDoctorCmd(flags))
	authCmd := newAuthCmd(flags)
	authCmd.AddCommand(newAuthSignonCmd(flags))
	...
	rootCmd.AddCommand(newNovelNoteCmd(flags))
	rootCmd.AddCommand(newNovelReviewCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newCompletedPromotedCmd(flags))
	rootCmd.AddCommand(newFocusPromotedCmd(flags))
