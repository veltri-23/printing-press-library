...
Run sync first to populate the local search index.`,
  ...
  ticktick-pp-cli search "status"

  # Force local search explicitly
  ticktick-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  ticktick-pp-cli search "status" --type completed --data-source local
  # JSON output for piping
  ticktick-pp-cli search "status" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'ticktick-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "completed":
				results, err = db.SearchCompleted(query, limit)
			case "tags":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchCompleted(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search completed failed: %w", searchErr)
					}
