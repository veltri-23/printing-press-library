...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ticktick-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ticktick-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"completed", "focus", "habits", "projects", "tasks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ticktick-pp-cli workflow status

  # Show status as JSON
  ticktick-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ticktick-pp-cli")
			}
