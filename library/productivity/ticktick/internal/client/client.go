...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// X-Device is required by several V2 endpoints (e.g. /batch/check/0 returns
		// access_forbidden without it). Kept here beside the spec-declared User-Agent;
		// config [headers] can override it.
		if req.Header.Get("X-Device") == "" {
			req.Header.Set("X-Device", `{"platform":"web","os":"Windows 10","device":"Chrome 126.0.0.0","name":"","version":6070,"channel":"website"}`)
		}
		if c.Config != nil {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "ticktick-pp-cli/0.1.0")
		}
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export TICKTICK_SESSION_TOKEN=<your-token> or ticktick-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.TicktickSessionToken)
	}
