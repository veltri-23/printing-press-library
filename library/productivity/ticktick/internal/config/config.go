...
type Config struct {
	...
	TicktickSessionToken string          `toml:"session_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("TICKTICK_SESSION_TOKEN"); v != "" {
		cfg.TicktickSessionToken = v
		cfg.markEnvOverride("TicktickSessionToken")
		cfg.AuthSource = "env:TICKTICK_SESSION_TOKEN"
		cfg.CredentialSource = "env:TICKTICK_SESSION_TOKEN"
	}
	...
	if v := os.Getenv("TICKTICK_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("TICKTICK_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "ticktick-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	token := c.TicktickSessionToken
	if token == "" {
		return ""
	}
	if c.TicktickSessionToken == "" {
		return ""
	}
	replacements := map[string]string{
		"token":                  token,
		"session_token":          c.TicktickSessionToken,
		"TICKTICK_SESSION_TOKEN": c.TicktickSessionToken,
	}
	return applyAuthFormat("t={token}", replacements)
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		TicktickSessionToken: c.TicktickSessionToken,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.TicktickSessionToken = creds.TicktickSessionToken
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.TicktickSessionToken = token
	delete(c.envOverrides, "TicktickSessionToken")
	c.updateFileConfigField("TicktickSessionToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["TicktickSessionToken"] {
			out.TicktickSessionToken = c.fileConfig.TicktickSessionToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "TicktickSessionToken":
		c.fileConfig.TicktickSessionToken = c.TicktickSessionToken
	}
}
