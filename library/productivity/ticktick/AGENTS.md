# TickTick Printed CLI Agent Guide

This directory is a generated `ticktick-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ticktick-pp-cli doctor --json
ticktick-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ticktick-pp-cli which "<capability>" --json
ticktick-pp-cli <command> --help
```
...
```bash
ticktick-pp-cli <command> --help
ticktick-pp-cli <command> --dry-run --agent
```
