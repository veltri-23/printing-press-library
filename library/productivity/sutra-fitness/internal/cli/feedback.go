...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "sutra-fitness-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUTRA_FITNESS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUTRA_FITNESS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "sutra-fitness-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/sutra-fitness-pp-cli/feedback.jsonl.
When ` + "`SUTRA_FITNESS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUTRA_FITNESS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sutra-fitness-pp-cli feedback list
  sutra-fitness-pp-cli feedback list --limit 5
  sutra-fitness-pp-cli feedback list --json`,
