...
Resource scoping:
  ...
  sutra-fitness-pp-cli sync

  # Sync specific resources only
  sutra-fitness-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  sutra-fitness-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  sutra-fitness-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  sutra-fitness-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  sutra-fitness-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Dry-run / verify probes short-circuit before any network call or
			// the partner-id requirement, so `sync --dry-run` is a clean exit 0.
			if dryRunOK(flags) {
				return nil
			}
			...
			// partnerId scopes every Sutra endpoint. Require it for real syncs;
			// verify/mock runs synthesize a placeholder in syncResourcePath.
			if !cliutil.IsVerifyEnv() && os.Getenv("SUTRA_PARTNER_ID") == "" {
				return configErr(fmt.Errorf("SUTRA_PARTNER_ID is required for sync; set it with: export SUTRA_PARTNER_ID=<your-partner-id>"))
			}

			// Split flat resources (handled by the worker pool) from
			// parent-keyed dependents (reservations per class, rooms per
			// location) handled by syncDependentChildren after the flat pass.
			flatResources, dependentResources := partitionSyncResources(resources)
			...
			if len(resources) == 0 {
				if humanFriendly {
					fmt.Fprintln(os.Stderr, "sutra-fitness-pp-cli sync: no default sync resources in spec; use --resources for explicit sync targets or populate the store via single-fetch commands.")
				} else {
			...
			// Enqueue flat resources only; dependents run after the pool.
			...
			// Parent-keyed dependents (reservations per class, rooms per
			// location): the flat worker pool cannot fill these because their
			// paths need a parent ID beyond {partnerId}.
			depRecords, depSuccess, depWarn, depErr := syncDependentChildren(cmd.Context(), c, db, dependentResources, maxPages, syncEventWriter)
			totalSynced += depRecords
			successCount += depSuccess
			warnCount += depWarn
			errCount += depErr
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/sutra-fitness-pp-cli/data.db)")
...
func syncResource(ctx context.Context, c interface {
	...
	for {
		...
		if resourceSupportsPagination(resource) {
			...
			if cursor != "" {
				// Decode the Sutra cursor so the client's query builder
				// single-encodes it; see decodeSutraCursor.
				params[pageSize.cursorParam] = decodeSutraCursor(cursor)
			}
		}
...
func resourceSupportsPagination(resource string) bool {
	switch resource {
	case "locations", "classes", "clients", "purchases", "referrals":
		// All flat Sutra list endpoints support limit + start_after cursor
		// pagination (pagination.nextStartAfterId / hasMore).
		return true
	}
	return false
}
...
func syncResourceSinceParam(resource string) string {
	switch resource {
	case "locations", "clients", "purchases", "referrals":
		// These list endpoints declare updated_at_min for incremental sync.
		// "classes" intentionally omitted: it filters by start_date/end_date,
		// not updated_at, so a synthetic since would reach the API as an
		// unknown query key.
		return "updated_at_min"
	}
	return ""
}
...
func extractPaginationFromEnvelope(envelope map[string]json.RawMessage, cursorParam string) (string, bool) {
	...
	cursorKeys := []string{
		...
		"nextStartAfterId", "next_start_after_id",
	}
	if nextCursor == "" {
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "rooms":
		return db.UpsertRooms(data)
	case "purchases":
		return db.UpsertPurchases(data)
	case "referrals":
		return db.UpsertReferrals(data)
	default:
...
func defaultSyncResources() []string {
	// Flat (top-level) resources only. Dependents (reservations, rooms) are
	// cascaded in by partitionSyncResources because classes and locations are
	// listed here.
	return []string{"locations", "classes", "clients", "purchases", "referrals"}
}
...
func knownSyncResourceNames() []string {
	return []string{"locations", "classes", "clients", "purchases", "referrals", "reservations", "rooms"}
}
...
func syncResourcePath(resource string) (string, error) {
	// Every Sutra path is scoped under /{partnerId}/. Real syncs are gated on
	// SUTRA_PARTNER_ID before this is reached; the "partner" fallback only
	// keeps verify/mock paths resolvable. Dependents (reservations, rooms) are
	// handled by syncDependentChildren and intentionally absent here.
	pid := os.Getenv("SUTRA_PARTNER_ID")
	if pid == "" {
		pid = "partner"
	}
	paths := map[string]string{
		"locations": "/" + pid + "/locations",
		"classes":   "/" + pid + "/classes",
		"clients":   "/" + pid + "/clients",
		"purchases": "/" + pid + "/purchases",
		"referrals": "/" + pid + "/referrals",
	}
