...
In local mode: searches locally synced data only.`,
  ...
  sutra-fitness-pp-cli search "error timeout"

  # Force local search only
  sutra-fitness-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  sutra-fitness-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  sutra-fitness-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'sutra-fitness-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "classes":
				results, err = db.SearchClasses(query, limit)
			case "locations":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchClasses(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search classes failed: %w", searchErr)
					}
