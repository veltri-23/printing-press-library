...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "sutra-fitness-pp-cli",
		Short: `Sutra Fitness CLI — Every Sutra (Arketa) Partner API resource plus a local mirror and the studio analytics the dashboard buries.`,
		Long: `Sutra Fitness CLI — Every Sutra (Arketa) Partner API resource plus a local mirror and the studio analytics the dashboard buries.
...
Health check: run 'sutra-fitness-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("sutra-fitness-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'sutra-fitness-pp-cli profile list')")
	...
	rootCmd.AddCommand(newNovelChurnCmd(flags))
	rootCmd.AddCommand(newNovelExpiringCmd(flags))
	rootCmd.AddCommand(newNovelLtvCmd(flags))
	rootCmd.AddCommand(newNovelNoShowsCmd(flags))
	rootCmd.AddCommand(newNovelReferralFunnelCmd(flags))
	rootCmd.AddCommand(newNovelRevenueCmd(flags))
	rootCmd.AddCommand(newNovelScorecardCmd(flags))
	rootCmd.AddCommand(newNovelUtilizationCmd(flags))
	rootCmd.AddCommand(newAPICmd(flags))
	rootCmd.AddCommand(newLocationsPromotedCmd(flags))
	rootCmd.AddCommand(newPurchasesPromotedCmd(flags))
	rootCmd.AddCommand(newReferralsPromotedCmd(flags))
