...
type Config struct {
	...
	SutraApiKey   string            `toml:"api_key"`
}

func Load(configPath string) (*Config, error) {
	cfg := &Config{
		BaseURL: "https://us-central1-sutra-prod.cloudfunctions.net/partnerApi/v0",
	}

	// Resolve config path
	path := configPath
	if path == "" {
		path = os.Getenv("SUTRA_FITNESS_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "sutra-fitness-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SUTRA_API_KEY"); v != "" {
		cfg.SutraApiKey = v
		cfg.markEnvOverride("SutraApiKey")
		cfg.AuthSource = "env:SUTRA_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.SutraApiKey != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SUTRA_FITNESS_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.SutraApiKey = token
	delete(c.envOverrides, "SutraApiKey")
	c.updateFileConfigField("SutraApiKey")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SutraApiKey"] {
			out.SutraApiKey = c.fileConfig.SutraApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SutraApiKey":
		c.fileConfig.SutraApiKey = c.SutraApiKey
	}
}
