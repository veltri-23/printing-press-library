# Sutra Printed CLI Agent Guide

This directory is a generated `sutra-fitness-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
sutra-fitness-pp-cli doctor --json
sutra-fitness-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
sutra-fitness-pp-cli which "<capability>" --json
sutra-fitness-pp-cli <command> --help
```
...
```bash
sutra-fitness-pp-cli <command> --agent
```
...
```bash
sutra-fitness-pp-cli <command> --help
sutra-fitness-pp-cli <command> --dry-run --agent
```
