...
Disabling: pass --no-learn or set ` + noLearnEnvVar + `=true.`,
		Example: `  human-goat-pp-cli teach --query "<question>" --resource-type <type> \
...
when learnings exist.`,
		Example: `  human-goat-pp-cli recall "<question>" --agent
  human-goat-pp-cli recall "<question>" --agent --min-confidence 2`,
...
func newLearningsListCmd(flags *rootFlags) *cobra.Command {
  ...
  human-goat-pp-cli learnings list --query "<substring>"
...
Requires at least one of --resource, --action, or --all.`,
		Example: `  human-goat-pp-cli learnings forget "<question>" --resource <id>
  human-goat-pp-cli learnings forget "<question>" --all`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				return cmd.Help()
			}
