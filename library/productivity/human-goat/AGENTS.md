...
```bash
human-goat-pp-cli which "<capability>" --json
human-goat-pp-cli <command> --help
```
...
```bash
human-goat-pp-cli <command> --agent
```
...
```bash
human-goat-pp-cli <command> --help
human-goat-pp-cli <command> --dry-run --agent
```
...
3. After answering, always fire `human-goat-pp-cli teach --query "<question>" --resource <id> --resource-type <type> &` in the background - teaching is unconditional and is the anchor that triggers playbook synthesis. Teach the structural question with identifiers stripped (no names, emails, phone numbers, account ids); the CLI warns on obvious PII shapes but does not block.
