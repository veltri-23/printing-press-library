# Fireflies Printed CLI Agent Guide

This directory is a generated `fireflies-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
fireflies-pp-cli doctor --json
fireflies-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
fireflies-pp-cli which "<capability>" --json
...
```bash
fireflies-pp-cli <command> --agent
```
...
```bash
fireflies-pp-cli <command> --help
fireflies-pp-cli <command> --dry-run --agent
```
