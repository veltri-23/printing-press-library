...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".fireflies-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FIREFLIES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FIREFLIES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "fireflies-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.fireflies-pp-cli/feedback.jsonl.
When ` + "`FIREFLIES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FIREFLIES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  fireflies-pp-cli feedback list
  fireflies-pp-cli feedback list --limit 5
  fireflies-pp-cli feedback list --json`,
