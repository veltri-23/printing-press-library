...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("EXPENSIFY_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "expensify-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.ExpensifyAuthToken != "" {
		return applyAuthFormat("ExpensifyToken {authToken}", map[string]string{
			"authToken":            c.ExpensifyAuthToken,
			"auth_token":           c.ExpensifyAuthToken,
			"EXPENSIFY_AUTH_TOKEN": c.ExpensifyAuthToken,
			"token":                c.ExpensifyAuthToken,
		})
	}
...
func (c *Config) clearCredentialFields() {
	...
	c.ClientID = ""
	c.ClientSecret = ""
	c.ExpensifyAuthToken = ""
	c.ExpensifyPartnerUserId = ""
	c.ExpensifyPartnerUserSecret = ""
}
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		ExpensifyAuthToken:         c.ExpensifyAuthToken,
		ExpensifyPartnerUserId:     c.ExpensifyPartnerUserId,
		ExpensifyPartnerUserSecret: c.ExpensifyPartnerUserSecret,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.ExpensifyAuthToken = creds.ExpensifyAuthToken
	c.ExpensifyPartnerUserId = creds.ExpensifyPartnerUserId
	c.ExpensifyPartnerUserSecret = creds.ExpensifyPartnerUserSecret
}
...
func (c *Config) SaveCredential(token string) error {
	...
	delete(c.envOverrides, "ExpensifyAuthToken")
	c.updateFileConfigField("ExpensifyAuthToken")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ExpensifyAuthToken")
	c.ExpensifyPartnerUserId = ""
	delete(c.envOverrides, "ExpensifyPartnerUserId")
	c.updateFileConfigField("ExpensifyPartnerUserId")
	c.ExpensifyPartnerUserSecret = ""
	delete(c.envOverrides, "ExpensifyPartnerUserSecret")
	c.updateFileConfigField("ExpensifyPartnerUserSecret")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["ExpensifyAuthToken"] {
			out.ExpensifyAuthToken = c.fileConfig.ExpensifyAuthToken
		}
		if c.envOverrides["ExpensifyPartnerUserId"] {
			out.ExpensifyPartnerUserId = c.fileConfig.ExpensifyPartnerUserId
		}
		if c.envOverrides["ExpensifyPartnerUserSecret"] {
			out.ExpensifyPartnerUserSecret = c.fileConfig.ExpensifyPartnerUserSecret
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "ExpensifyAuthToken":
		c.fileConfig.ExpensifyAuthToken = c.ExpensifyAuthToken
	case "ExpensifyPartnerUserId":
		c.fileConfig.ExpensifyPartnerUserId = c.ExpensifyPartnerUserId
	case "ExpensifyPartnerUserSecret":
		c.fileConfig.ExpensifyPartnerUserSecret = c.ExpensifyPartnerUserSecret
	}
}
