...
func (c *Client) doInternal(ctx context.Context, method, path string, params map[string]string, body any, headerOverrides map[string]string, readOnlyIntent bool) (json.RawMessage, int, error) {
	...
	// Expensify wire-protocol override. The live API ignores the modeled
	// `Authorization: ExpensifyToken {authToken}` header and instead expects a
	// form-encoded body carrying authToken plus every field. Rebuild the request
	// accordingly and drop the auth header (the token rides in the body).
	// See internal/client/expensify_request.go.
	expensifyContentType := ""
	{
		eURL, eBody, eCT, eErr := c.buildExpensifyRequest(method, path, body)
		if eErr != nil {
			return nil, 0, eErr
		}
		targetURL = eURL
		bodyBytes = eBody
		expensifyContentType = eCT
		authHeader = "" // token travels in the form body, not a header
	}
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if bodyBytes != nil {
			ct := "application/json"
			if expensifyContentType != "" {
				ct = expensifyContentType
			}
...
func (c *Client) dryRun(method, targetURL, path string, params map[string]string, body []byte, headerOverrides map[string]string, authHeader string) (json.RawMessage, int, error) {
	...
	_ = queryPrinted
	if body != nil {
		// Expensify request bodies are form-encoded; show the decoded fields with
		// authToken masked. Fall back to JSON, then raw, on parse failure.
		if form, perr := url.ParseQuery(string(body)); perr == nil && len(form) > 0 && !strings.HasPrefix(strings.TrimSpace(string(body)), "{") {
		...
		} else if pretty := json.RawMessage(nil); json.Unmarshal(body, &pretty) == nil {
...
func authPlaceholderCredentialError(cfg *config.Config) error {
	return authPlaceholderCredentialErrorWithSetup(cfg, "export EXPENSIFY_AUTH_TOKEN=<your-token> or expensify-pp-cli auth set-token <token>")
}
...
func (c *Client) maskCredentialText(text string, extraCredentials ...string) string {
	...
	if c != nil && c.Config != nil {
		...
		addCredential(c.Config.ExpensifyAuthToken)
		addCredential(c.Config.ExpensifyPartnerUserId)
		addCredential(c.Config.ExpensifyPartnerUserSecret)
	}
