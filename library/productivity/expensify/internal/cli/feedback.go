...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("EXPENSIFY_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("EXPENSIFY_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "expensify-pp-cli/feedback")
...
When ` + "`EXPENSIFY_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`EXPENSIFY_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  expensify-pp-cli feedback list
  expensify-pp-cli feedback list --limit 5
  expensify-pp-cli feedback list --json`,
