...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  expensify-pp-cli doctor
  expensify-pp-cli doctor --json
  expensify-pp-cli doctor --fail-on warn
  expensify-pp-cli doctor --fail-on stale`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "Set credentials with: export EXPENSIFY_AUTH_TOKEN=\"your-token-here\" EXPENSIFY_PARTNER_USER_ID=\"your-token-here\" EXPENSIFY_PARTNER_USER_SECRET=\"your-token-here\""
					report["auth_key_url"] = "https://www.expensify.com/tools/integrations/"
				} else {
					authConfigured = true
			...
			if os.Getenv("EXPENSIFY_AUTH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "EXPENSIFY_AUTH_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXPENSIFY_AUTH_TOKEN")
			}
			if os.Getenv("EXPENSIFY_PARTNER_USER_ID") != "" {
				authEnvSet = append(authEnvSet, "EXPENSIFY_PARTNER_USER_ID")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXPENSIFY_PARTNER_USER_ID")
			}
			if os.Getenv("EXPENSIFY_PARTNER_USER_SECRET") != "" {
				authEnvSet = append(authEnvSet, "EXPENSIFY_PARTNER_USER_SECRET")
			} else if authConfigured {
				authSource, _ := report["auth_source"].(string)
				if authSource == "" {
					authSource = "config"
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "EXPENSIFY_PARTNER_USER_SECRET")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						if suggestion != "" {
							report["credentials"] = fmt.Sprintf("present, not verified. Run `%s %s` to confirm the token works end-to-end.", "expensify-pp-cli", suggestion)
						} else {
