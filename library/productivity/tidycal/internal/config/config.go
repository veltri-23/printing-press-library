...
type Config struct {
	...
	Path              string            `toml:"-"`
	TidycalApiToken   string            `toml:"api_token"`
	TidycalBearerAuth string            `toml:"bearer_auth"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("TIDYCAL_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "tidycal-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("TIDYCAL_API_TOKEN"); v != "" {
		cfg.TidycalApiToken = v
		cfg.AuthSource = "env:TIDYCAL_API_TOKEN"
	}
	if v := os.Getenv("TIDYCAL_BEARER_AUTH"); v != "" {
		cfg.TidycalBearerAuth = v
		cfg.AuthSource = "env:TIDYCAL_BEARER_AUTH"
	}
	...
	if cfg.AuthSource == "" && cfg.TidycalApiToken != "" {
		cfg.AuthSource = "config"
	}
	if cfg.AuthSource == "" && cfg.TidycalBearerAuth != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("TIDYCAL_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.TidycalApiToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TIDYCAL_API_TOKEN"
		}
		return "Bearer " + c.TidycalApiToken
	}
	if c.TidycalBearerAuth != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:TIDYCAL_BEARER_AUTH"
		}
		return "Bearer " + c.TidycalBearerAuth
	}
	if c.AccessToken != "" {
