...
In local mode: searches locally synced data only.`,
  ...
  tidycal-pp-cli search "error timeout"

  # Force local search only
  tidycal-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  tidycal-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  tidycal-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'tidycal-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "booking-types":
				results, err = db.SearchBookingTypes(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchBookingTypes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search booking_types failed: %w", searchErr)
					}
