...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for TidyCal",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  tidycal-pp-cli auth setup\n  tidycal-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://tidycal.com/integrations/oauth")
			fmt.Fprintln(w, "  Create a TidyCal personal access token and pass it as a bearer token.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export TIDYCAL_API_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export TIDYCAL_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  tidycal-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://tidycal.com/integrations/oauth"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tidycal-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export TIDYCAL_API_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export TIDYCAL_BEARER_AUTH=\"your-token-here\"")
				fmt.Fprintf(w, "  tidycal-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tidycal-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  tidycal-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("TIDYCAL_API_TOKEN") != "" {
				envStillSet = "TIDYCAL_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("TIDYCAL_BEARER_AUTH") != "" {
				envStillSet = "TIDYCAL_BEARER_AUTH"
			}
