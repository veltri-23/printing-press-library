...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "tidycal-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TIDYCAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TIDYCAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "tidycal-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/tidycal-pp-cli/feedback.jsonl.
When ` + "`TIDYCAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TIDYCAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tidycal-pp-cli feedback list
  tidycal-pp-cli feedback list --limit 5
  tidycal-pp-cli feedback list --json`,
