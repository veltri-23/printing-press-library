...
Resource scoping:
  ...
  tidycal-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  tidycal-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  tidycal-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  tidycal-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  tidycal-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/tidycal-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "booking-types":
		return db.UpsertBookingTypes(data)
	case "booking_types_bookings":
		return db.UpsertBookingTypesBookings(data)
	case "timeslots":
		return db.UpsertTimeslots(data)
	case "bookings":
	...
	case "teams_booking_types":
		return db.UpsertTeamsBookingTypes(data)
	case "teams_bookings":
		return db.UpsertTeamsBookings(data)
	case "users":
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "teams_booking_types", ParentTable: "teams", ParentIDParam: "team", PathTemplate: "/teams/{team}/booking-types", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "team", Field: "team"},
		}},
		{Name: "teams_bookings", ParentTable: "teams", ParentIDParam: "team", PathTemplate: "/teams/{team}/bookings", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "team", Field: "team"},
		}},
		{Name: "users", ParentTable: "teams", ParentIDParam: "team", PathTemplate: "/teams/{team}/users", KeyField: "", PathParams: []dependentPathParamDef{
			{Param: "team", Field: "team"},
		}},
	}
}
