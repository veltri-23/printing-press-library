...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  tidycal-pp-cli doctor
  tidycal-pp-cli doctor --json
  tidycal-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export TIDYCAL_API_TOKEN=<your-key>"
					report["auth_key_url"] = "https://tidycal.com/integrations/oauth"
					report["auth_instructions"] = "Create a TidyCal personal access token and pass it as a bearer token."
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "TIDYCAL_API_TOKEN")
				if os.Getenv("TIDYCAL_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "TIDYCAL_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("TIDYCAL_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TIDYCAL_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "TIDYCAL_API_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "TIDYCAL_BEARER_AUTH")
				if os.Getenv("TIDYCAL_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "TIDYCAL_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("TIDYCAL_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "TIDYCAL_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "TIDYCAL_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "tidycal-pp-cli"
						verifyPath := "/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("tidycal-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'tidycal-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'tidycal-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'tidycal-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'tidycal-pp-cli sync' to refresh."
	}
	return report
}
