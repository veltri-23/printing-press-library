...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".cal-com-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("CAL_COM_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("CAL_COM_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "cal-com-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.cal-com-pp-cli/feedback.jsonl.
When ` + "`CAL_COM_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`CAL_COM_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  cal-com-pp-cli feedback list
  cal-com-pp-cli feedback list --limit 5
  cal-com-pp-cli feedback list --json`,
