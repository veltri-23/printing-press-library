# Toodledo Printed CLI Agent Guide

This directory is a generated `toodledo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
toodledo-pp-cli doctor --json
toodledo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
toodledo-pp-cli which "<capability>" --json
toodledo-pp-cli <command> --help
```
...
```bash
toodledo-pp-cli <command> --help
toodledo-pp-cli <command> --dry-run --agent
```
