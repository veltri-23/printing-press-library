...
func TestSearchGoalsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("goals", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchGoals(query, 10)
		if err != nil {
			t.Fatalf("SearchGoals(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchGoals(%q) returned no results", query)
		}
	}
}
...
func TestSearchListsQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("lists", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchLists(query, 10)
		if err != nil {
			t.Fatalf("SearchLists(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchLists(%q) returned no results", query)
		}
	}
}
...
func TestSearchNotesQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	for _, query := range []string{
		...
		results, err := s.SearchNotes(query, 10)
		if err != nil {
			t.Fatalf("SearchNotes(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchNotes(%q) returned no results", query)
		}
	}
}

// TestUpsertBatch_PopulatesOutlinesTable verifies that UpsertBatch
// dispatches paginated items into both the generic resources table AND the
// typed outlines table. Regression for issue #268: before the fix, paginated
// syncs only filled the generic resources table, so domain commands that
// query the typed table saw zero rows.
func TestUpsertBatch_PopulatesOutlinesTable(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if _, _, err := s.UpsertBatch("outlines", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	}
	...
	var generic int
	if err := db.QueryRow(`SELECT COUNT(*) FROM resources WHERE resource_type = ?`, "outlines").Scan(&generic); err != nil {
		t.Fatalf("count resources: %v", err)
	}
	if generic != len(items) {
		t.Fatalf("resources count = %d, want %d", generic, len(items))
	...
	var typed int
	typedQuery := fmt.Sprintf(`SELECT COUNT(*) FROM "%s"`, "outlines")
	if err := db.QueryRow(typedQuery).Scan(&typed); err != nil {
		t.Fatalf("count outlines: %v", err)
	}
	if typed != len(items) {
		t.Fatalf("outlines count = %d, want %d (typed table not populated by UpsertBatch)", typed, len(items))
	}
}

func TestSearchOutlinesQuotesFTSQuerySyntax(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	s, err := Open(dbPath)
	if err != nil {
		t.Fatalf("open: %v", err)
	...
	if stored, failed, err := s.UpsertBatch("outlines", items); err != nil {
		t.Fatalf("UpsertBatch: %v", err)
	} else if failed != 0 || stored != len(items) {
		t.Fatalf("UpsertBatch stored=%d failed=%d, want stored=%d failed=0", stored, failed, len(items))
	}
	...
	for _, query := range []string{
		...
		results, err := s.SearchOutlines(query, 10)
		if err != nil {
			t.Fatalf("SearchOutlines(%q): %v", query, err)
		}
		if len(results) == 0 {
			t.Fatalf("SearchOutlines(%q) returned no results", query)
		}
	}
}
