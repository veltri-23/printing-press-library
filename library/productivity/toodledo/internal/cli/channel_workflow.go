...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  toodledo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  toodledo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"contexts", "folders", "goals", "lists", "lists-deleted-php", "locations", "notes", "notes-deleted-php", "outlines", "outlines-deleted-php", "tasks", "tasks-deleted-php"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  toodledo-pp-cli workflow status

  # Show status as JSON
  toodledo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("toodledo-pp-cli")
			}
