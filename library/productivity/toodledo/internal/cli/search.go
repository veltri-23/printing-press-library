...
In local mode: searches locally synced data only.`,
  ...
  toodledo-pp-cli search "error timeout"

  # Force local search only
  toodledo-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  toodledo-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  toodledo-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'toodledo-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "goals":
				results, err = db.SearchGoals(query, limit)
			case "lists":
				results, err = db.SearchLists(query, limit)
			case "locations":
				results, err = db.SearchLocations(query, limit)
			case "notes":
				results, err = db.SearchNotes(query, limit)
			case "outlines":
				results, err = db.SearchOutlines(query, limit)
			case "tasks":
			...
			case "":
				...
				{
					partial, searchErr := db.SearchGoals(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search goals failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchLists(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search lists failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchLocations(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search locations failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchNotes(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search notes failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchOutlines(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search outlines failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/toodledo-pp-cli/data.db)")
