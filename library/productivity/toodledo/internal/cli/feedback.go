...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "toodledo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("TOODLEDO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("TOODLEDO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "toodledo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/toodledo-pp-cli/feedback.jsonl.
When ` + "`TOODLEDO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`TOODLEDO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  toodledo-pp-cli feedback list
  toodledo-pp-cli feedback list --limit 5
  toodledo-pp-cli feedback list --json`,
