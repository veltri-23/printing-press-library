...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			...
			"\n      Run 'toodledo-pp-cli doctor' to check auth status."+
	...
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: your token may have expired. Re-run 'toodledo-pp-cli auth login' to re-authenticate"+
			"\n      or: export TOODLEDO_CLIENT_ID=<your-token>"+
			"\n      Run 'toodledo-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Your token may lack required scopes. Re-run 'toodledo-pp-cli auth login' to re-authorize"+
			"\n      or: export TOODLEDO_CLIENT_ID=<your-token>"+
			"\n      Run 'toodledo-pp-cli doctor' to check auth status.", err))
