...
Resource scoping:
  ...
  breezedoc-pp-cli sync

  # Sync specific resources only
  breezedoc-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  breezedoc-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  breezedoc-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  breezedoc-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  breezedoc-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/breezedoc-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "documents_recipients":
		return db.UpsertDocumentsRecipients(data)
	case "documents_send":
		return db.UpsertDocumentsSend(data)
	case "invoices":
		return db.UpsertInvoices(data)
	case "invoices_send":
		return db.UpsertInvoicesSend(data)
	case "recipients":
		return db.UpsertRecipients(data)
	case "teams_documents":
		return db.UpsertTeamsDocuments(data)
	case "teams_templates":
		return db.UpsertTeamsTemplates(data)
	case "templates":
		return db.UpsertTemplates(data)
	case "create_document":
		return db.UpsertCreateDocument(data)
	default:
