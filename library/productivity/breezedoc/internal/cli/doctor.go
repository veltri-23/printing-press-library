...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  breezedoc-pp-cli doctor
  breezedoc-pp-cli doctor --json
  breezedoc-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export BREEZEDOC_API_TOKEN=<your-key>"
					report["auth_key_url"] = "https://breezedoc.com/integrations/api"
					report["auth_instructions"] = "Create a personal access token under Integrations -> API and send it as an Authorization bearer token."
				} else {
					authConfigured = true
			...
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "BREEZEDOC_API_TOKEN")
				if os.Getenv("BREEZEDOC_API_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "BREEZEDOC_API_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("BREEZEDOC_API_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BREEZEDOC_API_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "BREEZEDOC_API_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "BREEZEDOC_BEARER_TOKEN")
				if os.Getenv("BREEZEDOC_BEARER_TOKEN") != "" {
					authEnvSet = append(authEnvSet, "BREEZEDOC_BEARER_TOKEN")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("BREEZEDOC_BEARER_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "BREEZEDOC_BEARER_TOKEN")
			} else {
				authEnvInfo = append(authEnvInfo, "BREEZEDOC_BEARER_TOKEN optional")
			}
			if strings.Contains("", " OR ") {
				authEnvOptionalNames = append(authEnvOptionalNames, "BREEZEDOC_BEARER_AUTH")
				if os.Getenv("BREEZEDOC_BEARER_AUTH") != "" {
					authEnvSet = append(authEnvSet, "BREEZEDOC_BEARER_AUTH")
					authEnvOptionalSatisfied = true
				}
			} else if os.Getenv("BREEZEDOC_BEARER_AUTH") != "" {
				authEnvSet = append(authEnvSet, "BREEZEDOC_BEARER_AUTH")
			} else {
				authEnvInfo = append(authEnvInfo, "BREEZEDOC_BEARER_AUTH optional")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					if authHeader == "" {
						...
						authHeaders["User-Agent"] = "breezedoc-pp-cli"
						verifyPath := "/me"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'breezedoc-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'breezedoc-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'breezedoc-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'breezedoc-pp-cli sync' to refresh."
	}
	return report
}
