...
func newWorkflowCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.AddCommand(newWorkflowInvoiceLifecycleCmd(flags))
	cmd.AddCommand(newWorkflowClientWorkspaceSnapshotCmd(flags))
	cmd.AddCommand(newWorkflowDocumentFollowUpCmd(flags))
	cmd.AddCommand(newWorkflowSignaturePacketPrepCmd(flags))
...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  breezedoc-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  breezedoc-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"documents", "invoices", "recipients", "templates"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  breezedoc-pp-cli workflow status

  # Show status as JSON
  breezedoc-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("breezedoc-pp-cli")
			}
