...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for BreezeDoc",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  breezedoc-pp-cli auth setup\n  breezedoc-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://breezedoc.com/integrations/api")
			fmt.Fprintln(w, "  Create a personal access token under Integrations -> API and send it as an Authorization bearer token.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export BREEZEDOC_API_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export BREEZEDOC_BEARER_TOKEN=\"<your-token>\"")
			fmt.Fprintln(w, "  export BREEZEDOC_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  breezedoc-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://breezedoc.com/integrations/api"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  breezedoc-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export BREEZEDOC_API_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export BREEZEDOC_BEARER_TOKEN=\"your-token-here\"")
				fmt.Fprintln(w, "  export BREEZEDOC_BEARER_AUTH=\"your-token-here\"")
				fmt.Fprintf(w, "  breezedoc-pp-cli auth set-token <token>\n")
				return authErr(fmt.Errorf("no credentials configured"))
			}
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  breezedoc-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  breezedoc-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("BREEZEDOC_API_TOKEN") != "" {
				envStillSet = "BREEZEDOC_API_TOKEN"
			}
			if envStillSet == "" && os.Getenv("BREEZEDOC_BEARER_TOKEN") != "" {
				envStillSet = "BREEZEDOC_BEARER_TOKEN"
			}
			if envStillSet == "" && os.Getenv("BREEZEDOC_BEARER_AUTH") != "" {
				envStillSet = "BREEZEDOC_BEARER_AUTH"
			}
