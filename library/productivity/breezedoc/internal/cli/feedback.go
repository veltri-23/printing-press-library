...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "breezedoc-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("BREEZEDOC_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("BREEZEDOC_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "breezedoc-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/breezedoc-pp-cli/feedback.jsonl.
When ` + "`BREEZEDOC_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`BREEZEDOC_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  breezedoc-pp-cli feedback list
  breezedoc-pp-cli feedback list --limit 5
  breezedoc-pp-cli feedback list --json`,
