# Breezedoc Printed CLI Agent Guide

This directory is a generated `breezedoc-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
breezedoc-pp-cli doctor --json
breezedoc-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
breezedoc-pp-cli which "<capability>" --json
breezedoc-pp-cli <command> --help
```
...
```bash
breezedoc-pp-cli <command> --agent
```
...
```bash
breezedoc-pp-cli <command> --help
breezedoc-pp-cli <command> --dry-run --agent
```
