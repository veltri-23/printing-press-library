# Supermemory Admin Printed CLI Agent Guide

This directory is a generated `supermemory-admin-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
supermemory-admin-pp-cli doctor --json
supermemory-admin-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
supermemory-admin-pp-cli which "<capability>" --json
...
```bash
supermemory-admin-pp-cli <command> --agent
```

Use `SUPERMEMORY_ADMIN_PROJECT` when work should stay inside one Supermemory project. The CLI maps it to the `x-sm-project` header for every request.

Before running an unfamiliar command that may mutate remote state, inspect its help and prefer a dry run:

```bash
supermemory-admin-pp-cli <command> --help
supermemory-admin-pp-cli <command> --dry-run --agent
```
