...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".local", "share", "supermemory-admin-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUPERMEMORY_ADMIN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUPERMEMORY_ADMIN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "supermemory-admin-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.local/share/supermemory-admin-pp-cli/feedback.jsonl.
When ` + "`SUPERMEMORY_ADMIN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUPERMEMORY_ADMIN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  supermemory-admin-pp-cli feedback list
  supermemory-admin-pp-cli feedback list --limit 5
  supermemory-admin-pp-cli feedback list --json`,
