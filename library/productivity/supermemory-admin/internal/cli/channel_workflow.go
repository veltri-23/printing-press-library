...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  supermemory-admin-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  supermemory-admin-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  supermemory-admin-pp-cli workflow status

  # Show status as JSON
  supermemory-admin-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("supermemory-admin-pp-cli")
			}
