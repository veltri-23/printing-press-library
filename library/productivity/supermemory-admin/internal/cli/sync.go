...
Resource scoping:
  ...
  supermemory-admin-pp-cli sync

  # Sync specific resources only
  supermemory-admin-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  supermemory-admin-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  supermemory-admin-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  supermemory-admin-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  supermemory-admin-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/supermemory-admin-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "configure":
		return db.UpsertConfigure(data)
	case "connection":
		return db.UpsertConnection(data)
	case "connections_documents":
		return db.UpsertConnectionsDocuments(data)
	case "import":
		return db.UpsertImport(data)
	case "container-tags":
		return db.UpsertContainerTags(data)
	case "documents":
		return db.UpsertDocuments(data)
	case "chunks":
		return db.UpsertChunks(data)
	case "file_url":
		return db.UpsertFileUrl(data)
	case "memories":
		return db.UpsertMemories(data)
	case "settings":
		return db.UpsertSettings(data)
	case "supermemory_recall":
		return db.UpsertSupermemoryRecall(data)
	default:
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"documents": "/v3/documents/processing",
	}
