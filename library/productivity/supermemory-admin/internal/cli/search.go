...
In local mode: searches locally synced data only.`,
  ...
  supermemory-admin-pp-cli search "error timeout"

  # Force local search only
  supermemory-admin-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  supermemory-admin-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  supermemory-admin-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /v3/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post(cmd.Context(), "/v3/search", map[string]any{
					"q":                  query,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'supermemory-admin-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "documents":
				results, err = db.SearchDocuments(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchDocuments(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search documents failed: %w", searchErr)
					}
