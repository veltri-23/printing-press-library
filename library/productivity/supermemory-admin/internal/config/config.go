...
type Config struct {
	...
	SupermemoryAdminToken string            `toml:"admin_token"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("SUPERMEMORY_ADMIN_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "supermemory-admin-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("SUPERMEMORY_ADMIN_TOKEN"); v != "" {
		cfg.SupermemoryAdminToken = v
		cfg.markEnvOverride("SupermemoryAdminToken")
		cfg.AuthSource = "env:SUPERMEMORY_ADMIN_TOKEN"
	}
	if v := os.Getenv("SUPERMEMORY_ADMIN_PROJECT"); v != "" {
		if cfg.Headers == nil {
			cfg.Headers = map[string]string{}
		}
		cfg.Headers["x-sm-project"] = v
	}
	...
	if cfg.AuthSource == "" && cfg.SupermemoryAdminToken != "" {
		cfg.AuthSource = "config"
	}
	...
	if v := os.Getenv("SUPERMEMORY_ADMIN_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func (c *Config) AuthHeader() string {
	...
	if c.SupermemoryAdminToken != "" {
		if c.AuthSource == "" {
			c.AuthSource = "env:SUPERMEMORY_ADMIN_TOKEN"
		}
		return "Bearer " + c.SupermemoryAdminToken
	}
	if c.AccessToken != "" {
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("ClientID")
	c.updateFileConfigField("ClientSecret")
	c.SupermemoryAdminToken = ""
	delete(c.envOverrides, "SupermemoryAdminToken")
	return c.save()
}
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["SupermemoryAdminToken"] {
			out.SupermemoryAdminToken = c.fileConfig.SupermemoryAdminToken
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "SupermemoryAdminToken":
		c.fileConfig.SupermemoryAdminToken = c.SupermemoryAdminToken
	}
}
