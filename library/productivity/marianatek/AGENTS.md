# Mariana Tek Printed CLI Agent Guide

This directory is a generated `marianatek-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
marianatek-pp-cli doctor --json
marianatek-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
marianatek-pp-cli which "<capability>" --json
marianatek-pp-cli <command> --help
```
...
```bash
marianatek-pp-cli <command> --agent
```
...
```bash
marianatek-pp-cli <command> --help
marianatek-pp-cli <command> --dry-run --agent
```
