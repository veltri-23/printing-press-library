...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  marianatek-pp-cli doctor
  marianatek-pp-cli doctor --json
  marianatek-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export CUSTOMER_OAUTH_AUTHORIZATION=<your-key>"
					report["auth_key_url"] = "https://tools.ietf.org/html/rfc6750"
				} else {
					authConfigured = true
			...
			if os.Getenv("CUSTOMER_OAUTH_AUTHORIZATION") != "" {
				authEnvSet = append(authEnvSet, "CUSTOMER_OAUTH_AUTHORIZATION")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "CUSTOMER_OAUTH_AUTHORIZATION")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'marianatek-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'marianatek-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'marianatek-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'marianatek-pp-cli sync' to refresh."
	}
	return report
}
