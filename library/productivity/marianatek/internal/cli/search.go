...
In local mode: searches locally synced data only.`,
  ...
  marianatek-pp-cli search "error timeout"

  # Force local search only
  marianatek-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  marianatek-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  marianatek-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'marianatek-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "schedule-locations":
				results, err = db.SearchScheduleLocations(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchScheduleLocations(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search schedule_locations failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
