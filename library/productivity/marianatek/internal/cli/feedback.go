...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".marianatek-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MARIANATEK_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MARIANATEK_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "marianatek-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.marianatek-pp-cli/feedback.jsonl.
When ` + "`MARIANATEK_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MARIANATEK_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  marianatek-pp-cli feedback list
  marianatek-pp-cli feedback list --limit 5
  marianatek-pp-cli feedback list --json`,
