...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "etherpad-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "etherpad-pp-cli/1.3.1")
		}
...
func (c *Client) refreshAccessToken() error {
	...
	// PATCH(oidc-urls-from-base-url, remove-dead-tokenurl-guard): derive token URL
	// from cfg.BaseURL (or cfg.OIDCIssuer) via TokenURL() and drop the dead
	// `if tokenURL == ""` guard that used to live here. See
	// .printing-press-patches.json patches[0] and patches[1].
	//
	...
	// PATCH(expires-in-zero-guard-on-login): parallel guard to auth.go — refresh
	// stays robust when expires_in is 0 or missing.
	expiry := time.Time{}
