...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Etherpad",
	}
...
func newAuthLoginCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// PATCH(oidc-urls-from-base-url): use cfg.AuthURL() (derived from BaseURL)
			// instead of hardcoded http://localhost:9001. See .printing-press-patches.json patches[0].
			authURL := cfg.AuthURL()
			params := url.Values{
				"client_id":     {clientID},
			...
			// PATCH(oidc-urls-from-base-url): use cfg.TokenURL() (derived from BaseURL).
			...
			// PATCH(expires-in-zero-guard-on-login): guard ExpiresIn==0 so the
			// initial token isn't stored with expiry == time.Now(). Mirrors the
			// existing refreshAccessToken guard in client.go. See
			// .printing-press-patches.json patches[2].
			//
			...
			// PATCH(expires-in-zero-guard-on-login): mirror the ExpiresIn==0
			// guard above — print a "no expiry provided" message instead of
			// the Go zero time (0001-01-01T00:00:00Z), which reads as a real
			// future timestamp to an inexperienced user.
			if expiry.IsZero() {
	...
	cmd.Flags().StringVar(&clientID, "client-id", os.Getenv("ETHERPAD_CLIENT_ID"), "OAuth2 client ID")
	cmd.Flags().StringVar(&clientSecret, "client-secret", os.Getenv("ETHERPAD_CLIENT_SECRET"), "OAuth2 client secret")
