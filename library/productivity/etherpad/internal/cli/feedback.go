...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".etherpad-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ETHERPAD_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ETHERPAD_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "etherpad-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.etherpad-pp-cli/feedback.jsonl.
When ` + "`ETHERPAD_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ETHERPAD_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  etherpad-pp-cli feedback list
  etherpad-pp-cli feedback list --limit 5
  etherpad-pp-cli feedback list --json`,
