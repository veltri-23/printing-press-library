...
Exit codes & warnings:
  ...
  etherpad-pp-cli sync

  # Sync specific resources only
  etherpad-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  etherpad-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  etherpad-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  etherpad-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  etherpad-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("etherpad-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/etherpad-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "anonymize_author":
		return db.UpsertAnonymizeAuthor(data)
	case "append_chat_message":
		return db.UpsertAppendChatMessage(data)
	case "append_text":
		return db.UpsertAppendText(data)
	case "check_token":
		return db.UpsertCheckToken(data)
	case "compact_pad":
		return db.UpsertCompactPad(data)
	case "copy_pad":
		return db.UpsertCopyPad(data)
	case "copy_pad_without_history":
		return db.UpsertCopyPadWithoutHistory(data)
	case "create_author":
		return db.UpsertCreateAuthor(data)
	case "create_author_if_not_exists_for":
		return db.UpsertCreateAuthorIfNotExistsFor(data)
	case "create_diff_html":
		return db.UpsertCreateDiffHtml(data)
	case "create_group":
		return db.UpsertCreateGroup(data)
	case "create_group_if_not_exists_for":
		return db.UpsertCreateGroupIfNotExistsFor(data)
	case "create_group_pad":
		return db.UpsertCreateGroupPad(data)
	case "create_pad":
		return db.UpsertCreatePad(data)
	case "create_session":
		return db.UpsertCreateSession(data)
	case "delete_group":
		return db.UpsertDeleteGroup(data)
	case "delete_pad":
		return db.UpsertDeletePad(data)
	case "delete_session":
		return db.UpsertDeleteSession(data)
	case "get_attribute_pool":
		return db.UpsertGetAttributePool(data)
	case "get_author_name":
		return db.UpsertGetAuthorName(data)
	case "get_chat_head":
		return db.UpsertGetChatHead(data)
	case "get_chat_history":
		return db.UpsertGetChatHistory(data)
	case "get_html":
		return db.UpsertGetHtml(data)
	case "get_last_edited":
		return db.UpsertGetLastEdited(data)
	case "get_pad_id":
		return db.UpsertGetPadId(data)
	case "get_public_status":
		return db.UpsertGetPublicStatus(data)
	case "get_read_only_id":
		return db.UpsertGetReadOnlyId(data)
	case "get_revision_changeset":
		return db.UpsertGetRevisionChangeset(data)
	case "get_revisions_count":
		return db.UpsertGetRevisionsCount(data)
	case "get_saved_revisions_count":
		return db.UpsertGetSavedRevisionsCount(data)
	case "get_session_info":
		return db.UpsertGetSessionInfo(data)
	case "get_stats":
		return db.UpsertGetStats(data)
	case "get_text":
		return db.UpsertGetText(data)
	case "list_all_groups":
		return db.UpsertListAllGroups(data)
	case "list_all_pads":
		return db.UpsertListAllPads(data)
	case "list_authors_of_pad":
		return db.UpsertListAuthorsOfPad(data)
	case "list_pads":
		return db.UpsertListPads(data)
	case "list_pads_of_author":
		return db.UpsertListPadsOfAuthor(data)
	case "list_saved_revisions":
		return db.UpsertListSavedRevisions(data)
	case "list_sessions_of_author":
		return db.UpsertListSessionsOfAuthor(data)
	case "list_sessions_of_group":
		return db.UpsertListSessionsOfGroup(data)
	case "move_pad":
		return db.UpsertMovePad(data)
	case "pad_users":
		return db.UpsertPadUsers(data)
	case "pad_users_count":
		return db.UpsertPadUsersCount(data)
	case "restore_revision":
		return db.UpsertRestoreRevision(data)
	case "save_revision":
		return db.UpsertSaveRevision(data)
	case "send_clients_message":
		return db.UpsertSendClientsMessage(data)
	case "set_html":
		return db.UpsertSetHtml(data)
	case "set_public_status":
		return db.UpsertSetPublicStatus(data)
	case "set_text":
		return db.UpsertSetText(data)
	default:
