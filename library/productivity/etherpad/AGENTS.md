# Etherpad Printed CLI Agent Guide

This directory is a generated `etherpad-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
etherpad-pp-cli doctor --json
etherpad-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
etherpad-pp-cli which "<capability>" --json
...
```bash
etherpad-pp-cli <command> --help
etherpad-pp-cli <command> --dry-run --agent
```
