# Superhuman Printed CLI Agent Guide

This directory is a generated `superhuman-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
superhuman-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
superhuman-pp-cli which "<capability>" --json
superhuman-pp-cli <command> --help
```
...
```bash
superhuman-pp-cli <command> --agent
```
...
```bash
superhuman-pp-cli <command> --help
superhuman-pp-cli <command> --dry-run --agent
```
