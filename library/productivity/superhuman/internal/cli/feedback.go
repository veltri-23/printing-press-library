...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SUPERHUMAN_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SUPERHUMAN_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "superhuman-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.superhuman-pp-cli/feedback.jsonl.
When ` + "`SUPERHUMAN_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SUPERHUMAN_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  superhuman-pp-cli feedback list
  superhuman-pp-cli feedback list --limit 5
  superhuman-pp-cli feedback list --json`,
