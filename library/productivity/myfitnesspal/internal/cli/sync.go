...
Exit codes & warnings:
  ...
  myfitnesspal-pp-cli sync

  # Sync specific resources only
  myfitnesspal-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  myfitnesspal-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  myfitnesspal-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  myfitnesspal-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  myfitnesspal-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/myfitnesspal-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "api_user":
		return db.UpsertApiUser(data)
	case "diary":
		return db.UpsertDiary(data)
	case "food":
		return db.UpsertFood(data)
	case "measurement":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"measurement": "/api/user-measurements/measurements/types",
	}
