...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".myfitnesspal-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("MYFITNESSPAL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("MYFITNESSPAL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "myfitnesspal-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.myfitnesspal-pp-cli/feedback.jsonl.
When ` + "`MYFITNESSPAL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`MYFITNESSPAL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  myfitnesspal-pp-cli feedback list
  myfitnesspal-pp-cli feedback list --limit 5
  myfitnesspal-pp-cli feedback list --json`,
