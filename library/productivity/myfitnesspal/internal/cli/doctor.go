...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  myfitnesspal-pp-cli doctor
  myfitnesspal-pp-cli doctor --json
  myfitnesspal-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "myfitnesspal-pp-cli auth login --chrome"
				} else {
					report["auth"] = "configured (browser session)"
					report["auth_source"] = cfg.AuthSource
					report["auth_domain"] = ".myfitnesspal.com"
...
func browserSessionProofStatus(cfg *config.Config, authHeader string) (bool, string) {
	...
	if err != nil {
		return false, "proof not found; run myfitnesspal-pp-cli auth login --chrome"
	}
	var proof doctorBrowserSessionProof
	if err := json.Unmarshal(data, &proof); err != nil {
		return false, "proof is not valid JSON; re-run myfitnesspal-pp-cli auth login --chrome"
	}
	if proof.APIName != "myfitnesspal" {
		return false, "proof belongs to a different CLI"
	}
	if proof.CookieDomain != ".myfitnesspal.com" {
		return false, "proof belongs to a different cookie domain"
	}
	if proof.ValidationPath != "/user/auth_token" {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'myfitnesspal-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'myfitnesspal-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'myfitnesspal-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'myfitnesspal-pp-cli sync' to refresh."
	}
	return report
}
