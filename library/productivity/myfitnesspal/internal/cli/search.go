...
In local mode: searches locally synced data only.`,
  ...
  myfitnesspal-pp-cli search "error timeout"

  # Force local search only
  myfitnesspal-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  myfitnesspal-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  myfitnesspal-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /food/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/food/search", map[string]any{
					"search":             query,
					"meal":               10,
					"date":               "",
					"authenticity_token": "",
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'myfitnesspal-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
