...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "myfitnesspal-pp-cli",
		Short: `Pull your MyFitnessPal diary, foods, exercises, and measurements into a local SQLite store, run trend reports offline, and export per-food CSV that the official UI never gives you.`,
		Long: `Pull your MyFitnessPal diary, foods, exercises, and measurements into a local SQLite store, run trend reports offline, and export per-food CSV that the official UI never gives you.
...
Health check: run 'myfitnesspal-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("myfitnesspal-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'myfitnesspal-pp-cli profile list')")
	...
	rootCmd.AddCommand(newDiaryCmd(flags))
	...
	rootCmd.AddCommand(newApiUserPromotedCmd(flags))
	rootCmd.AddCommand(newExercisePromotedCmd(flags))
	rootCmd.AddCommand(newGoalsPromotedCmd(flags))
	rootCmd.AddCommand(newNotePromotedCmd(flags))
	rootCmd.AddCommand(newReportsPromotedCmd(flags))
	rootCmd.AddCommand(newWaterPromotedCmd(flags))

	// HAND-ADDED: novel transcendence commands. Built against the local
	// diary_entry table populated by `pull-diary`.
	rootCmd.AddCommand(newPullDiaryCmd(flags))
	rootCmd.AddCommand(newExportCmd(flags))
	rootCmd.AddCommand(newDiaryFindCmd(flags))
...
func newVersionCliCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			fmt.Printf("myfitnesspal-pp-cli %s\n", version)
		},
	}
}
