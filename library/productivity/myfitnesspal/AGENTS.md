# MyFitnessPal Printed CLI Agent Guide

This directory is a generated `myfitnesspal-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
myfitnesspal-pp-cli doctor --json
myfitnesspal-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
myfitnesspal-pp-cli which "<capability>" --json
...
```bash
myfitnesspal-pp-cli <command> --agent
```
...
```bash
myfitnesspal-pp-cli <command> --help
myfitnesspal-pp-cli <command> --dry-run --agent
```
