...
func New(cfg *config.Config, timeout time.Duration, rateLimit float64) *Client {
	...
	cacheDir := filepath.Join(homeDir, ".cache", "figma-pp-cli")
...
func (c *Client) do(method, path string, params map[string]string, body any, headerOverrides map[string]string) (json.RawMessage, int, error) {
	...
	for attempt := 0; attempt <= maxRetries; attempt++ {
		...
		// PAT goes in X-Figma-Token (Figma's primary auth header), not Authorization Bearer.
		// authHeader() prefers PAT when set; the value carries an "X-Figma-Token: " prefix
		// that we strip and route to the right header.
		if authHeader != "" {
			if strings.HasPrefix(authHeader, "X-Figma-Token: ") {
				req.Header.Set("X-Figma-Token", strings.TrimPrefix(authHeader, "X-Figma-Token: "))
			} else {
		...
		if req.Header.Get("User-Agent") == "" {
			req.Header.Set("User-Agent", "figma-pp-cli/0.38.0")
		}
