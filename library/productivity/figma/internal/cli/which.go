...
var whichIndex = []whichEntry{
	...
	{Command: "webhooks test", Description: "Pull Figma's webhook request log and replay stored payloads (with original headers and HMAC) against an arbitrary target URL.", Group: "Agent-native plumbing", WhyItMatters: "Use when iterating on a new webhook handler — replay yesterday's failed deliveries against your local server without re-triggering upstream events."},
	{Command: "variables explain", Description: "Flat list of every node and component that references a given variable across a file.", Group: "Local state that compounds", WhyItMatters: "First call when planning a variable rename or deprecation — shows the blast radius before you touch anything."},
}
...
Exit codes:
  ...
  2  no confident match - the query did not score against any indexed capability; fall back to '--help' or 'search' if this CLI has one`,
		Example: `  figma-pp-cli which "stale tickets"
  figma-pp-cli which "bottleneck"
  figma-pp-cli which --limit 1 "send message"
  figma-pp-cli which                                # list the full capability index`,
