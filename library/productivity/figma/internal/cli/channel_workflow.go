...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  figma-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  figma-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"activity-logs", "me", "payments", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  figma-pp-cli workflow status

  # Show status as JSON
  figma-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("figma-pp-cli")
			}
