...
func newFigmaAnalyticsGetLibraryVariableActionsCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "get-library-variable-actions <file_key>",
		Short:       "Returns a list of library analytics variable actions data broken down by the requested dimension.",
		Example:     "  figma-pp-cli figma-analytics get-library-variable-actions your-token-here --group-by example-value",
		Annotations: map[string]string{"pp:endpoint": "figma-analytics.get-library-variable-actions", "pp:method": "GET", "pp:path": "/v1/analytics/libraries/{file_key}/variable/actions", "mcp:read-only": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("group-by") {
				allowedGroupBy := []string{"variable", "team"}
				validGroupBy := false
	...
	cmd.Flags().StringVar(&flagGroupBy, "group-by", "", "A dimension to group returned analytics data by. (one of: variable, team)")
