...
Exit codes & warnings:
  ...
  figma-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  figma-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  figma-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  figma-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  figma-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "figma_analytics":
		return db.UpsertFigmaAnalytics(data)
	case "files":
		return db.UpsertFiles(data)
	case "comments":
		return db.UpsertComments(data)
	case "files_component_sets":
		return db.UpsertFilesComponentSets(data)
	case "files_components":
		return db.UpsertFilesComponents(data)
	case "files_dev_resources":
		return db.UpsertFilesDevResources(data)
	case "files_images":
		return db.UpsertFilesImages(data)
	case "meta":
		return db.UpsertMeta(data)
	case "nodes":
		return db.UpsertNodes(data)
	case "files_styles":
		return db.UpsertFilesStyles(data)
	case "variables":
		return db.UpsertVariables(data)
	case "versions":
		return db.UpsertVersions(data)
	case "oembed":
		return db.UpsertOembed(data)
	case "projects_files":
		return db.UpsertProjectsFiles(data)
	case "teams_component_sets":
		return db.UpsertTeamsComponentSets(data)
	case "teams_components":
		return db.UpsertTeamsComponents(data)
	case "teams_projects":
		return db.UpsertTeamsProjects(data)
	case "teams_styles":
		return db.UpsertTeamsStyles(data)
	case "teams_webhooks":
		return db.UpsertTeamsWebhooks(data)
	case "webhooks":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"activity-logs": "/v1/activity_logs",
		"me":            "/v1/me",
