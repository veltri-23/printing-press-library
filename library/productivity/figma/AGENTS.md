# Figma Printed CLI Agent Guide

This directory is a generated `figma-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
figma-pp-cli doctor --json
figma-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
figma-pp-cli which "<capability>" --json
...
```bash
figma-pp-cli <command> --help
figma-pp-cli <command> --dry-run --agent
```
