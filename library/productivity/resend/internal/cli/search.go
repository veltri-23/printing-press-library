...
In local mode: searches locally synced data only.`,
  ...
  resend-pp-cli search "error timeout"

  # Force local search only
  resend-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  resend-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  resend-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'resend-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "":
				...
				{
					partial, searchErr := db.SearchTopics(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search topics failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
