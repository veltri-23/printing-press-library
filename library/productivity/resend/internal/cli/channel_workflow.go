...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  resend-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  resend-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"api-keys", "audiences", "automations", "broadcasts", "contact-properties", "contacts", "domains", "emails", "emails-receiving", "events", "logs", "segments", "templates", "topics", "webhooks"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  resend-pp-cli workflow status

  # Show status as JSON
  resend-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("resend-pp-cli")
			}
