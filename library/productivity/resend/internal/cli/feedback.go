...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".resend-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RESEND_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RESEND_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "resend-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.resend-pp-cli/feedback.jsonl.
When ` + "`RESEND_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RESEND_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  resend-pp-cli feedback list
  resend-pp-cli feedback list --limit 5
  resend-pp-cli feedback list --json`,
