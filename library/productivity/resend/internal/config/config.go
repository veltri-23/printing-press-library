...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("RESEND_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "resend-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	if v := os.Getenv("RESEND_API_KEY"); v != "" {
		cfg.ResendApiKey = v
		cfg.AuthSource = "env:RESEND_API_KEY"
	}
	...
	if cfg.AuthSource == "" && cfg.ResendApiKey != "" {
		cfg.AuthSource = "config"
	}
