# Resend Printed CLI Agent Guide

This directory is a generated `resend-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
resend-pp-cli doctor --json
resend-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
resend-pp-cli which "<capability>" --json
...
```bash
resend-pp-cli <command> --help
resend-pp-cli <command> --dry-run --agent
```
