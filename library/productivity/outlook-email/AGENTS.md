# Outlook Email Printed CLI Agent Guide

This directory is a generated `outlook-email-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
outlook-email-pp-cli doctor --json
outlook-email-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
outlook-email-pp-cli which "<capability>" --json
...
```bash
outlook-email-pp-cli <command> --agent
```
...
```bash
outlook-email-pp-cli <command> --help
outlook-email-pp-cli <command> --dry-run --agent
```
