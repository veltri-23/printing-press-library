...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".outlook-email-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OUTLOOK_EMAIL_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OUTLOOK_EMAIL_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "outlook-email-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.outlook-email-pp-cli/feedback.jsonl.
When ` + "`OUTLOOK_EMAIL_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OUTLOOK_EMAIL_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  outlook-email-pp-cli feedback list
  outlook-email-pp-cli feedback list --limit 5
  outlook-email-pp-cli feedback list --json`,
