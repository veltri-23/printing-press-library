...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  outlook-email-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  outlook-email-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"categories", "folders", "folders-delta", "inference", "messages", "messages-delta", "rules"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  outlook-email-pp-cli workflow status

  # Show status as JSON
  outlook-email-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("outlook-email-pp-cli")
			}
