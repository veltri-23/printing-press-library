...
Exit codes & warnings:
  ...
  outlook-email-pp-cli sync

  # Sync specific resources only
  outlook-email-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  outlook-email-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  outlook-email-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  outlook-email-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  outlook-email-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/outlook-email-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "inference":
		return db.UpsertInference(data)
	case "mailbox_settings":
		return db.UpsertMailboxSettings(data)
	case "messages":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"folders-delta":  "/me/mailFolders/delta",
		"inference":      "/me/inferenceClassification/overrides",
		"messages":       "/me/messages",
		"messages-delta": "/me/messages/delta",
