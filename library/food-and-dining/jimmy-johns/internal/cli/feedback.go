...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".jimmy-johns-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("JIMMY_JOHNS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("JIMMY_JOHNS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "jimmy-johns-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.jimmy-johns-pp-cli/feedback.jsonl.
When ` + "`JIMMY_JOHNS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`JIMMY_JOHNS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  jimmy-johns-pp-cli feedback list
  jimmy-johns-pp-cli feedback list --limit 5
  jimmy-johns-pp-cli feedback list --json`,
