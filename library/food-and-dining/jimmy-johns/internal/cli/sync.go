...
Exit codes & warnings:
  ...
  jimmy-johns-pp-cli sync

  # Sync specific resources only
  jimmy-johns-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  jimmy-johns-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  jimmy-johns-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  jimmy-johns-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  jimmy-johns-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/jimmy-johns-pp-cli/data.db)")
