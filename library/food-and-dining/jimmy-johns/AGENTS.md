# Jimmy John's Printed CLI Agent Guide

This directory is a generated `jimmy-johns-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
jimmy-johns-pp-cli doctor --json
jimmy-johns-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
jimmy-johns-pp-cli which "<capability>" --json
...
```bash
jimmy-johns-pp-cli <command> --agent
```
...
```bash
jimmy-johns-pp-cli <command> --help
jimmy-johns-pp-cli <command> --dry-run --agent
```
