...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".dominos-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("DOMINOS_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("DOMINOS_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "dominos-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.dominos-pp-cli/feedback.jsonl.
When ` + "`DOMINOS_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`DOMINOS_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  dominos-pp-cli feedback list
  dominos-pp-cli feedback list --limit 5
  dominos-pp-cli feedback list --json`,
