# AnyList Printed CLI Agent Guide

This directory is a generated `anylist-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
anylist-pp-cli doctor --json
anylist-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
anylist-pp-cli which "<capability>" --json
anylist-pp-cli <command> --help
```
...
```bash
anylist-pp-cli <command> --help
anylist-pp-cli <command> --dry-run --agent
```
