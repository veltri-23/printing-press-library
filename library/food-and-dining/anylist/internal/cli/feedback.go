...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ANYLIST_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ANYLIST_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "anylist-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.anylist-pp-cli/feedback.jsonl.
When ` + "`ANYLIST_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ANYLIST_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  anylist-pp-cli feedback list
  anylist-pp-cli feedback list --limit 5
  anylist-pp-cli feedback list --json`,
