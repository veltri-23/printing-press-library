...
Exit codes & warnings:
  ...
  ordertogo-pp-cli sync

  # Sync specific resources only
  ordertogo-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  ordertogo-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  ordertogo-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  ordertogo-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  ordertogo-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("ordertogo-pp-cli")
			}

			if flags.dryRun {
				// PATCH: Narrative validation dry-runs must not migrate or write
				// the user's SQLite store, especially when the default DB is read-only.
			...
			// PATCH: Order history sync uses OrderToGo's POST endpoint and also
			// populates the typed orders table for analytics commands.
			if orders {
				found := false
				...
				if !found {
					resources = append(resources, "orders")
				}
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/ordertogo-pp-cli/data.db)")
	...
	cmd.Flags().BoolVar(&orders, "orders", false, "Sync order history from /m/api/getmicmeshorders into the local orders table")
...
func syncResource(c interface {
	...
	if resource == "orders" {
		...
		count, err := db.UpsertOrdersFromAPI(data)
		if err != nil {
			return syncResult{Resource: resource, Err: fmt.Errorf("upserting orders: %w", err), Duration: time.Since(started)}
		}
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		"coupons":   "/m/api/coupons",
		"giftcards": "/m/api/giftcards",
		"orders":    "/m/api/getmicmeshorders",
		"rewards":   "/m/api/rewards",
	}
