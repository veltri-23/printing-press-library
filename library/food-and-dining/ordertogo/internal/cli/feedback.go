...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ordertogo-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ORDERTOGO_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ORDERTOGO_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ordertogo-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ordertogo-pp-cli/feedback.jsonl.
When ` + "`ORDERTOGO_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ORDERTOGO_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ordertogo-pp-cli feedback list
  ordertogo-pp-cli feedback list --limit 5
  ordertogo-pp-cli feedback list --json`,
