...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  ordertogo-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  ordertogo-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"coupons", "giftcards", "rewards"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  ordertogo-pp-cli workflow status

  # Show status as JSON
  ordertogo-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("ordertogo-pp-cli")
			}
