# OrderToGo Printed CLI Agent Guide

This directory is a generated `ordertogo-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ordertogo-pp-cli doctor --json
ordertogo-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ordertogo-pp-cli which "<capability>" --json
ordertogo-pp-cli <command> --help
```
...
```bash
ordertogo-pp-cli <command> --agent
```
...
```bash
ordertogo-pp-cli <command> --help
ordertogo-pp-cli <command> --dry-run --agent
```
