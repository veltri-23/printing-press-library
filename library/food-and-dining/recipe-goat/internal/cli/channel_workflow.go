...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  recipe-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  recipe-goat-pp-cli workflow archive --full`,
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  recipe-goat-pp-cli workflow status

  # Show status as JSON
  recipe-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("recipe-goat-pp-cli")
			}
