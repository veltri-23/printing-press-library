...
type Config struct {
	...
	Path          string    `toml:"-"`
	UsdaFdcApiKey string    `toml:"fdc_api_key"`
}
...
func Load(configPath string) (*Config, error) {
	...
	path := configPath
	if path == "" {
		path = os.Getenv("RECIPE_GOAT_CONFIG")
	}
	if path == "" {
		home, _ := os.UserHomeDir()
		path = filepath.Join(home, ".config", "recipe-goat-pp-cli", "config.toml")
	}
	cfg.Path = path
	...
	// Env var overrides
	if v := os.Getenv("USDA_FDC_API_KEY"); v != "" {
		cfg.UsdaFdcApiKey = v
		cfg.AuthSource = "env:USDA_FDC_API_KEY"
	}
