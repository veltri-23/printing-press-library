...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".coffee-goat-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("COFFEE_GOAT_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("COFFEE_GOAT_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "coffee-goat-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.coffee-goat-pp-cli/feedback.jsonl.
When ` + "`COFFEE_GOAT_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`COFFEE_GOAT_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  coffee-goat-pp-cli feedback list
  coffee-goat-pp-cli feedback list --limit 5
  coffee-goat-pp-cli feedback list --json`,
