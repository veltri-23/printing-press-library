...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  coffee-goat-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  coffee-goat-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// Delegate to the coffee-source orchestrator. Archive
			// mirrors `sync --source all`; the --full flag is
			// preserved for the YouTube cursor reset path.
			var totalSynced int
			for _, src := range []string{"shopify", "coffee-review", "youtube"} {
				r := runSource(cmd.Context(), s, src, "", full)
				totalSynced += r.Items
				fmt.Fprintf(cmd.ErrOrStderr(), "  %s: %s (%d items)\n", r.Name, r.Status, r.Items)
			}
			...
			fmt.Fprintf(cmd.OutOrStdout(), "Archived %d items to %s\n", totalSynced, dbPath)
			return nil
		},
	}

	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/coffee-goat-pp-cli/data.db)")
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  coffee-goat-pp-cli workflow status

  # Show status as JSON
  coffee-goat-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("coffee-goat-pp-cli")
			}
