# Coffee GOAT Printed CLI Agent Guide

This directory is a generated `coffee-goat-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
coffee-goat-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
coffee-goat-pp-cli which "<capability>" --json
coffee-goat-pp-cli <command> --help
```
...
```bash
coffee-goat-pp-cli <command> --agent
```
...
```bash
coffee-goat-pp-cli <command> --help
coffee-goat-pp-cli <command> --dry-run --agent
```
