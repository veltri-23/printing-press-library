...
Exit codes & warnings:
  ...
  pagliacci-pp-cli sync

  # Sync specific resources only
  pagliacci-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  pagliacci-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  pagliacci-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  pagliacci-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  pagliacci-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/pagliacci-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "gifts":
		return db.UpsertGifts(data)
	case "store":
		return db.UpsertStore(data)
	case "customer":
		return db.UpsertCustomer(data)
	case "cart":
		return db.UpsertCart(data)
	case "rewards":
		return db.UpsertRewards(data)
	case "menu":
		return db.UpsertMenu(data)
	case "scheduling":
