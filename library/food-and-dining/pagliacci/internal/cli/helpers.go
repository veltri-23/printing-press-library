...
func classifyAPIError(err error) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Run 'pagliacci-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your API credentials."+
			"\n      Run 'pagliacci-pp-cli doctor' to check auth status.", err))
...
//
// The exact-name allowlist preserves canonical record fields (id, name, etc.).
// The suffix/prefix rules then keep API-specific fields whose names *signal*
// that they are identifiers, references, names, timestamps, or money — e.g.,
// store_id, slice_name, line_total, createdAt, beginsAt, paid_at. Without
// these rules, list rows from any domain whose fields are namespaced
// (store_id rather than id) collapse to {} under --compact / --agent.
//
// Verbose body-shaped fields are still dropped (description, body, content,
// comments, attachments, html, markdown) symmetric with compactObjectFields.
func compactListFields(items []map[string]any) json.RawMessage {
	keepExact := map[string]bool{
		...
		"price": true, "total": true, "amount": true, "quantity": true, "count": true,
		"value": true, "currency": true, "address": true, "city": true,
	}
	stripExact := map[string]bool{
		"description": true, "body": true, "content": true,
		"comments": true, "attachments": true, "html": true, "markdown": true,
	}
	// Fields ending in any of these suffixes are kept as high-gravity.
	keepSuffixes := []string{
		"_id", "_ids", "Id", "Ids",
		"_name", "_names", "Name", "Names",
		"_at", "At",
		"_url", "Url",
		"_count", "Count",
		"_total", "Total",
	...
	hasKeepSuffix := func(k string) bool {
		for _, s := range keepSuffixes {
	...
	for _, item := range items {
		...
		for k, v := range item {
			...
			if keepExact[k] || hasKeepSuffix(k) {
				compact[k] = v
			}
		}
