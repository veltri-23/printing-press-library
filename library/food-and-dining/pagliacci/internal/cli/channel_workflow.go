...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  pagliacci-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  pagliacci-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"address", "credit", "customer", "gifts", "orders", "store"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  pagliacci-pp-cli workflow status

  # Show status as JSON
  pagliacci-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("pagliacci-pp-cli")
			}
