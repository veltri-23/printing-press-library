...
type Config struct {
	...
	FdcApiKey        string          `toml:"api_key"`
	// UsdaApiKey is the USDA_API_KEY env alias for the same api.data.gov key.
	// It is populated from the environment at runtime and read by
	// credentialValue()/hasCredentialFields(); it must NOT share the persisted
	// "api_key" TOML key with FdcApiKey. Two struct fields tagged the same key
	// made decode ambiguous (the on-disk api_key landed in UsdaApiKey, leaving
	// FdcApiKey empty) and risked duplicate-key emission on encode. FdcApiKey
	// is the sole persisted credential field; USDA_API_KEY stays env-only.
	UsdaApiKey string `toml:"-"`
}
...
func Load(configPath string) (*Config, error) {
	...
	// Env var overrides
	if v := os.Getenv("FDC_API_KEY"); v != "" {
		cfg.FdcApiKey = v
		cfg.markEnvOverride("FdcApiKey")
		cfg.AuthSource = "env:FDC_API_KEY"
		cfg.CredentialSource = "env:FDC_API_KEY"
	}
	if v := os.Getenv("USDA_API_KEY"); v != "" {
		cfg.UsdaApiKey = v
		cfg.markEnvOverride("UsdaApiKey")
		cfg.AuthSource = "env:USDA_API_KEY"
		cfg.CredentialSource = "env:USDA_API_KEY"
	}
	...
	if v := os.Getenv("NUTRITION_BASE_URL"); v != "" {
		cfg.BaseURL = v
	}
	return cfg, nil
}
...
func resolveConfigPath(configPath string) (string, bool, error) {
	...
	if path := os.Getenv("NUTRITION_CONFIG"); path != "" {
...
func LegacyConfigPath() (string, error) {
	...
	return filepath.Join(home, ".config", "nutrition-pp-cli", "config.toml"), nil
}
...
func (c *Config) AuthHeader() string {
	...
	// is set, keeping the CLI usable out of the box. Applied only at the point
	// the client reads the credential for the api_key query param, so config
...
func (c *Config) credentials() *cliutil.Credentials {
	return &cliutil.Credentials{
		...
		FdcApiKey:     c.FdcApiKey,
		UsdaApiKey:    c.UsdaApiKey,
	}
}
...
func (c *Config) applyCredentials(creds *cliutil.Credentials) {
	...
	c.FdcApiKey = creds.FdcApiKey
	c.UsdaApiKey = creds.UsdaApiKey
}
...
func (c *Config) SaveCredential(token string) error {
	...
	c.updateFileConfigField("AccessToken")
	c.FdcApiKey = token
	delete(c.envOverrides, "FdcApiKey")
	c.updateFileConfigField("FdcApiKey")
	if err := c.saveCredentialsFirst(); err != nil {
		return err
	}
	return c.save()
...
func (c *Config) ClearTokens() error {
	...
	c.updateFileConfigField("FdcApiKey")
	c.UsdaApiKey = ""
	delete(c.envOverrides, "UsdaApiKey")
	c.updateFileConfigField("UsdaApiKey")
	if c.AgentcookieManagedByExternalStore() {
		c.markAgentcookieManaged()
...
func (c *Config) configForSave() Config {
	out := *c
	if c.fileConfig != nil {
		if c.envOverrides["FdcApiKey"] {
			out.FdcApiKey = c.fileConfig.FdcApiKey
		}
		if c.envOverrides["UsdaApiKey"] {
			out.UsdaApiKey = c.fileConfig.UsdaApiKey
		}
	}
...
func (c *Config) updateFileConfigField(field string) {
	...
	case "FdcApiKey":
		c.fileConfig.FdcApiKey = c.FdcApiKey
	case "UsdaApiKey":
		c.fileConfig.UsdaApiKey = c.UsdaApiKey
	}
}
