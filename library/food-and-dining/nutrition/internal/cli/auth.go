...
func newAuthCmd(flags *rootFlags) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "auth",
		Short: "Manage authentication for Food Data Central",
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  nutrition-pp-cli auth setup\n  nutrition-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "See API docs: https://nal.altarama.com/reft100.aspx?key=FoodData")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export FDC_API_KEY=\"your-token-here\"")
			fmt.Fprintln(w, "  export USDA_API_KEY=\"your-token-here\"")
			if !launch {
				return nil
			}
			launchURL := "https://nal.altarama.com/reft100.aspx?key=FoodData"
...
func newAuthStatusCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nutrition-pp-cli auth status",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func newAuthSetTokenCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nutrition-pp-cli auth set-token YOUR_TOKEN_HERE",
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		Example: "  nutrition-pp-cli auth logout",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg, err := config.Load(flags.configPath)
			if err != nil {
				return configErr(err)
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("FDC_API_KEY") != "" {
				envStillSet = "FDC_API_KEY"
			}
			if envStillSet == "" && os.Getenv("USDA_API_KEY") != "" {
				envStillSet = "USDA_API_KEY"
			}
