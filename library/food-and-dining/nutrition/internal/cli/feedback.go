...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NUTRITION_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NUTRITION_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "nutrition-pp-cli/feedback")
...
When ` + "`NUTRITION_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NUTRITION_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  nutrition-pp-cli feedback list
  nutrition-pp-cli feedback list --limit 5
  nutrition-pp-cli feedback list --json`,
