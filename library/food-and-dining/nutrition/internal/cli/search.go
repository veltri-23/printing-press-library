...
In local mode: searches locally synced data only.`,
  ...
  nutrition-pp-cli search "error timeout"

  # Force local search only
  nutrition-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  nutrition-pp-cli search "status" --type food --data-source local
  # JSON output for piping
  nutrition-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: GET /v1/foods/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, getErr := c.Get(cmd.Context(), "/v1/foods/search", map[string]string{
					"query": query,
				})
				if getErr == nil {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'nutrition-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
