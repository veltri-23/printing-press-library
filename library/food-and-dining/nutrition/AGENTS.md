# Nutrition (USDA FoodData Central + NutritionValue.org) Printed CLI Agent Guide

This directory is a generated `nutrition-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
nutrition-pp-cli doctor --json
nutrition-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
nutrition-pp-cli which "<capability>" --json
nutrition-pp-cli <command> --help
```
...
```bash
nutrition-pp-cli <command> --agent
```
...
```bash
nutrition-pp-cli <command> --help
nutrition-pp-cli <command> --dry-run --agent
```
