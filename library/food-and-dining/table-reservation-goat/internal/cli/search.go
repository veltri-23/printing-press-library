...
In local mode: searches locally synced data only.`,
  ...
  table-reservation-goat-pp-cli search "error timeout"

  # Force local search only
  table-reservation-goat-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  table-reservation-goat-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  table-reservation-goat-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'table-reservation-goat-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "experiences":
				results, err = db.SearchExperiences(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchExperiences(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search experiences failed: %w", searchErr)
					}
