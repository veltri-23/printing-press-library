...
Exit codes & warnings:
  ...
  table-reservation-goat-pp-cli sync

  # Sync specific resources only
  table-reservation-goat-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  table-reservation-goat-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  table-reservation-goat-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  table-reservation-goat-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  table-reservation-goat-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/table-reservation-goat-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	switch resource {
	case "availability":
		return db.UpsertAvailability(data)
	case "experiences":
		return db.UpsertExperiences(data)
	case "me":
		return db.UpsertMe(data)
	case "reservations":
		return db.UpsertReservations(data)
	case "restaurants":
		return db.UpsertRestaurants(data)
	case "wishlist":
		return db.UpsertWishlist(data)
	default:
...
// defaultSyncResources returns the resources sync attempts when --resources
// is not given. v1 ships with no working sync target — every absorbed-feature
// path is a placeholder that 404s against opentable.com. Returning an empty
// slice makes `sync` exit 0 with a friendly "nothing configured" message
// instead of failing with stream errors. v0.2 will populate this with the
// resources whose source-client implementations land.
