# Rappi (Mexico) Printed CLI Agent Guide

This directory is a generated `rappi-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
rappi-pp-cli doctor --json
rappi-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
rappi-pp-cli which "<capability>" --json
rappi-pp-cli <command> --help
```
...
```bash
rappi-pp-cli <command> --help
rappi-pp-cli <command> --dry-run --agent
```
