...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		Use:   "rappi-pp-cli",
		Short: `Read-only Rappi Mexico catalog: restaurants by city/category, supermarkets and pharmacies by type, store details, plus offline SQLite search across the full crawled catalog.`,
		Long: `Read-only Rappi Mexico catalog: restaurants by city/category, supermarkets and pharmacies by type, store details, plus offline SQLite search across the full crawled catalog.
...
Health check: run 'rappi-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("rappi-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'rappi-pp-cli profile list')")
	...
	restaurantsCmd := newRestaurantsCmd(flags)
	// Novel transcendence commands attach under restaurants.
	restaurantsCmd.AddCommand(newRestaurantsTopCmd(flags))
	restaurantsCmd.AddCommand(newRestaurantsDiffCmd(flags))
	restaurantsCmd.AddCommand(newRestaurantsOpenCmd(flags))
	restaurantsCmd.AddCommand(newRestaurantsNearCmd(flags))
	restaurantsCmd.AddCommand(newRestaurantsBrandCmd(flags))
	restaurantsCmd.AddCommand(newRestaurantsByNeighborhoodCmd(flags))
	restaurantsCmd.AddCommand(newRestaurantsMultiCategoryCmd(flags))
	rootCmd.AddCommand(restaurantsCmd)
	storesCmd := newStoresCmd(flags)
	storesCmd.AddCommand(newStoresCoverageCmd(flags))
	storesCmd.AddCommand(newStoresCoverageDiffCmd(flags))
	storesCmd.AddCommand(newStoresAdjacencyCmd(flags))
