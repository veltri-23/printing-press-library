...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("RAPPI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("RAPPI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "rappi-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.rappi-pp-cli/feedback.jsonl.
When ` + "`RAPPI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`RAPPI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  rappi-pp-cli feedback list
  rappi-pp-cli feedback list --limit 5
  rappi-pp-cli feedback list --json`,
