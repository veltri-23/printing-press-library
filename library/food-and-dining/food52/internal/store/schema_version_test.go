...
// TestMigrate_AddsColumnsOnUpgrade_Recipes verifies that opening a
...
func TestMigrate_AddsColumnsOnUpgrade_Recipes(t *testing.T) {
	dbPath := filepath.Join(t.TempDir(), "data.db")
	...
	if _, err := raw.Exec(`CREATE TABLE recipes (
		id TEXT PRIMARY KEY,
		data JSON NOT NULL,
	...
	rows, err := s.DB().Query(`PRAGMA table_info(recipes)`)
	if err != nil {
	...
	for _, want := range []string{
		...
		if !hasColumn[want] {
			t.Fatalf("%s column missing from recipes after migrate", want)
		}
	}
}
