...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".food52-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("FOOD52_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("FOOD52_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "food52-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.food52-pp-cli/feedback.jsonl.
When ` + "`FOOD52_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`FOOD52_FEEDBACK_AUTO_SEND=true`" + `, the entry is
