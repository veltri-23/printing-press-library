...
func newRootCmd(flags *rootFlags) *cobra.Command {
	rootCmd := &cobra.Command{
		...
		Long: `Search and read Food52 recipes and articles from your terminal — ingredients, steps, ratings, and full article text without leaving the shell.

Highlights (not in the official API docs):
  • pantry match   Find Food52 recipes whose ingredients overlap your local pantry, ranked by cove…
  • search   Full-text search across every recipe and article you have synced, with type fil…
  ...
  • scale   Scale a recipe's ingredients to a different number of servings using its Schema…
  • print   Render a recipe as ingredients + numbered steps with no nav, no images, no ads,…
  • articles for-recipe   Find synced articles that mention a given recipe in their relatedReading.

Agent mode: add --agent to any command for JSON output + non-interactive mode.
Health check: run 'food52-pp-cli doctor' to verify auth and connectivity.
See README.md or the bundled SKILL.md for recipes.`,
		SilenceUsage: true,
		Version:      version,
	}
	rootCmd.SetVersionTemplate("food52-pp-cli {{ .Version }}\n")
	...
	rootCmd.PersistentFlags().StringVar(&flags.profileName, "profile", "", "Apply values from a saved profile (see 'food52-pp-cli profile list')")
