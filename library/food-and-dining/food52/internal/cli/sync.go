...
Exit codes & warnings:
  ...
  food52-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  food52-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  food52-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  food52-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  food52-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/food52-pp-cli/data.db)")
	...
	cmd.AddCommand(newSyncRecipesCmd(flags))
	cmd.AddCommand(newSyncArticlesCmd(flags))
