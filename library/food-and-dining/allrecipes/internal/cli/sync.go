...
Exit codes & warnings:
  ...
  allrecipes-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  allrecipes-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  allrecipes-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  allrecipes-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  allrecipes-pp-cli sync --latest-only`,
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/allrecipes-pp-cli/data.db)")
