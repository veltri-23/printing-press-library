...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".allrecipes-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ALLRECIPES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ALLRECIPES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "allrecipes-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.allrecipes-pp-cli/feedback.jsonl.
When ` + "`ALLRECIPES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ALLRECIPES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
