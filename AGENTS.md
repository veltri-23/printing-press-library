# Theclose Printed CLI Agent Guide

This directory is a generated `theclose-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
theclose-pp-cli doctor --json
theclose-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
theclose-pp-cli which "<capability>" --json
theclose-pp-cli <command> --help
```
...
```bash
theclose-pp-cli <command> --help
theclose-pp-cli <command> --dry-run --agent
```
