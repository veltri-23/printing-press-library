---
name: pp-fpi-india
description: "The full history of Foreign Portfolio Investor flows into India, synced locally and queryable offline — something neither NSDL nor CDSL offers today. Trigger phrases: `FPI flows into India`, `foreign portfolio investor data`, `NSDL FPI report`, `FII DII flow India`, `check FPI net investment`, `use fpi-india`, `run fpi-india`."
...
metadata:
  openclaw:
    ...
    install:
      - kind: go
        bins: [fpi-india-pp-cli]
        module: github.com/mvanhorn/printing-press-library/library/other/fpi-india/cmd/fpi-india-pp-cli
---
<!-- GENERATED FILE — DO NOT EDIT.
     This file is a verbatim mirror of library/other/fpi-india/SKILL.md,
     regenerated post-merge by tools/generate-skills/. Hand-edits here are
     silently overwritten on the next regen. Edit the library/ source instead.
     See the repository agent guide, section "Generated artifacts: registry.json, cli-skills/". -->

# FPI India — Printing Press CLI

## Prerequisites: Install the CLI

This skill drives the `fpi-india-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.** If it is missing, install it first:

1. Install via the Printing Press installer. It defaults binaries to `$HOME/.local/bin` on macOS/Linux and `%LOCALAPPDATA%\Programs\PrintingPress\bin` on Windows:
   ```bash
   npx -y @mvanhorn/printing-press-library install fpi-india --cli-only
   ```
2. Verify: `fpi-india-pp-cli --version`
...
Use this CLI for historical or current Indian FPI investment flow analysis: net-investment trends, year-over-year and streak analysis, sector rotation, assets-under-custody trends, FPI registry lookups, and regulatory limit utilization. It is the right tool whenever a task needs NSDL/CDSL FPI data queried, trended, or compared rather than eyeballed from a report page.

## Anti-triggers

Do not use this CLI for:
- Do not use this CLI for live/real-time trade execution or order placement — it is a read-only data and analytics tool.
- Do not use this CLI for individual investor portfolio holdings or demat account data — NSDL/CDSL's FPI reports are aggregate market-level data, not account-level.
- Do not use this CLI for forward-looking flow forecasts or investment advice — every command surfaces historical, already-published figures only.
...
**auc** — FPI assets under custody (AUC), by country and by category
...
**cdsl_reports** — CDSL daily FPI snapshot publication index and dated XLS snapshot download
...
**limits** — Per-company (ISIN) foreign investment limits: NRI limit %, FPI limit %, sectoral cap %, and current monitored FPI holding count

- `fpi-india-pp-cli limits` — Per-ISIN foreign investment limit monitoring data

**net_investment** — Historical FPI net-investment flows by asset class (equity, debt, hybrid, mutual funds, AIF), financial year and calendar year
...
**registry** — Registered FPI list, registration category counts, and DDP pendency
...
**sector** — Fortnightly sector-wise FPI investment data

- `fpi-india-pp-cli sector` — Fortnightly sector-wise FPI investment activity

**trades** — Trade-wise FPI equity and debt data
...
```bash
fpi-india-pp-cli which "<capability in your own words>"
```
...
Run `fpi-india-pp-cli doctor` to verify setup.
...
- **Filterable** — `--select` keeps a subset of fields. Dotted paths descend into nested structures; arrays traverse element-wise. Critical for keeping context small on verbose APIs:
  ...
  ```bash
  fpi-india-pp-cli limits --action GetFilmReport_Detail --u-name example-resource --agent --select id,name,status
  ```
...
- Use `--home <dir>` for one invocation, or set `FPI_INDIA_HOME=<dir>` to relocate all four path kinds under one root.
- Use per-kind env vars only when a specific kind must diverge: `FPI_INDIA_CONFIG_DIR`, `FPI_INDIA_DATA_DIR`, `FPI_INDIA_STATE_DIR`, `FPI_INDIA_CACHE_DIR`.
- Resolution order is per-kind env var, `--home`, `FPI_INDIA_HOME`, XDG (`XDG_CONFIG_HOME`, `XDG_DATA_HOME`, `XDG_STATE_HOME`, `XDG_CACHE_HOME`), then platform defaults.
- `config` contains settings like `config.toml` and profiles. `data` contains `credentials.toml`, `data.db`, cookies, and auth sidecars. `state` contains persisted queries, jobs, and `teach.log`. `cache` contains regenerable HTTP/cache files.
- Stored secrets live in `credentials.toml` under the data dir. Existing legacy `config.toml` secrets are read for compatibility and leave `config.toml` on the first auth write.
- Run `fpi-india-pp-cli doctor --fail-on warn` to surface path and credential-location warnings. `agent-context` exposes a schema v4 `paths` block for agents that need the resolved dirs.
...
Fleet precedence: an inherited per-kind env var overrides an explicit `--home` for that kind. Use `FPI_INDIA_HOME` or per-kind vars as durable fleet levers, and use `--home` only for a single invocation. Relocation is not reversible by unsetting env vars; move files manually before clearing `FPI_INDIA_HOME`, or `doctor` will not find credentials left under the former root.
...
```bash
fpi-india-pp-cli recall "<user's question>" --agent
```
...
```json
{
  ...
  "candidates": [
    { "id": 12, "class": "flag_alias | playbook_candidate",
      ...
      "rationale": "...",
      "next_action": ["<trial command>", "fpi-india-pp-cli learnings confirm 12"] }
  ],
  "playbook": {
    "query_family": "...",
    "playbook": {
...
if Playbook present:
    ...
    -> the Playbook's expected_tool_calls is a budget; if you find yourself running
       materially more, record the divergence via `fpi-india-pp-cli playbook amend`
       at end-of-session.
...
- `lookup_refresh_available` (top-level): an entity in the query has no lookup row yet, but synced data could provide one. Run `fpi-india-pp-cli sync` to refresh entity lookups.
...
```bash
fpi-india-pp-cli teach --query "<user's question>" --resource-type <type> --resource <id1> --resource <id2>
...
fpi-india-pp-cli teach-playbook \
  --query "<user's question>" \
  --playbook-file ~/playbooks/<shape>.json \
...
```bash
fpi-india-pp-cli playbook amend \
...
`fpi-india-pp-cli learnings stats` reports recall hit rate, teach-to-reuse, playbook resolution rate, and candidate confirm/reject counts from the local `learn_events` table. Rates are null until they have a denominator; everything stays on this machine. Use it to check whether the loop is earning its keep for this CLI.

### Disabling learning

- `--no-learn` on a single command short-circuits both `recall` and the `teach` write path. Use for deterministic agent flows or tests that must not be affected by accumulated learnings.
- `FPI_INDIA_NO_LEARN=true` in the environment globally disables the pipeline.
...
```
fpi-india-pp-cli feedback "the --since flag is inclusive but docs say exclusive"
fpi-india-pp-cli feedback --stdin < notes.txt
fpi-india-pp-cli feedback list --json --limit 10
```

Entries are stored locally as `feedback.jsonl` under the resolved data dir. They are never POSTed unless `FPI_INDIA_FEEDBACK_ENDPOINT` is set AND either `--send` is passed or `FPI_INDIA_FEEDBACK_AUTO_SEND=true`. Default behavior is local-only.
...
```
fpi-india-pp-cli profile save briefing --json
fpi-india-pp-cli --profile briefing limits --action GetFilmReport_Detail --u-name example-resource
fpi-india-pp-cli profile list --json
fpi-india-pp-cli profile show briefing
fpi-india-pp-cli profile delete briefing --yes
```
...
1. **Empty, `help`, or `--help`** → show `fpi-india-pp-cli --help` output
...
2. Register with Claude Code:
   ```bash
   claude mcp add fpi-india-pp-mcp -- fpi-india-pp-mcp
   ```
3. Verify: `claude mcp list`

## Direct Use

1. Check if installed: `which fpi-india-pp-cli`
   If not found, offer to install (see Prerequisites at the top of this skill).
2. Match the user query to the best command from the Unique Capabilities and Command Reference above.
3. Execute with the `--agent` flag:
   ```bash
   fpi-india-pp-cli <command> [subcommand] [args] --agent
   ```
4. If ambiguous, drill into subcommand help: `fpi-india-pp-cli <command> --help`.
